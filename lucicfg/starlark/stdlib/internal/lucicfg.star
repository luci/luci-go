...
def _config(
    ...
    Args:
      ...
      tracked_files: a list of glob patterns that define a subset of files under
          ...
          `<glob pattern>` (a "positive" glob) or `!<glob pattern>` (a
          "negative" glob). A file under `config_dir` is considered tracked if
          its slash-separated path matches any of the positive globs and none of
          the negative globs. If a pattern starts with `**/`, the rest of it is
          ...
          emitted: if this set is not empty, only files that are in this set
          will be actually written to the disk (and all other files are
          discarded). This is beneficial when `lucicfg` is used to generate only
          a subset of config files, e.g. during the migration from handcrafted
          to generated configs. Knowing the tracked files set is also important
          when some generated file disappears from `lucicfg` output: it must be
          deleted from the disk as well. To do this, `lucicfg` needs to know
          what files are safe to delete. If `tracked_files` is empty (default),
          `lucicfg` will save all generated files and will never delete any file
          (in this case it is responsibility of the caller to make sure no stale
          output remains).
      fail_on_warnings: if set to True treat validation warnings as errors.
          ...
          If set to True via `lucicfg.config` and you want to override it to
          False via command line flags use `-fail-on-warnings=false`.
      lint_checks: a list of linter rules to apply in `lucicfg validate`. The
          ...
          **TODO** for a list of available checks. Default is `['none']` for
          now.
    """
    ...
    **Danger**: Using `-var` without `-emit-to-stdout` is generally wrong, since
    ...
    Args:
      ...
      validator: a callback called as `validator(value)` from `set(value)` and
          ...
          function that returns the value to be assigned to the variable
          (usually just `value` itself, but conversions are allowed, including
          type changes).
      expose_as: an optional string identifier to make this var settable via
          ...
          variable is auto-initialized to its default value (which must be
          string or None). Variables declared with `expose_as` are not settable
          via `set()` at all, they appear as "set" already the moment they are
...
def _rule(*, impl, defaults = None):
    ...
    Args:
      ...
      defaults: a dict with keys matching the rule arguments and values of type
          ...
          for a rule within some exec scope (see lucicfg.var(...) for more
          details about scoping). These vars become the public API of the rule.
          Callers can set them via `rule.defaults.<name>.set(...)`. `impl`
          callback can get them via `ctx.defaults.<name>.get()`. It is up to
          the rule's author to define vars for fields that can have defaults,
          document them in the rule doc, and finally use them from `impl`
          callback.
