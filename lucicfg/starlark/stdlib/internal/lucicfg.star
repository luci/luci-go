...
def _config(
  ...
  Args:
    ...
    check_fmt: if set to True verify Starlark files are properly formatted when
        running `lucicfg validate`. Improperly formatted files can be fixed by
        running `lucicfg fmt`. Default is False for now.
