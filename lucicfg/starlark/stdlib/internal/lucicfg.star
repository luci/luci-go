...
def _config(
  ...
  Args:
    ...
    tracked_files: a list of glob patterns that define a subset of files under
        ...
        glob). A file under `config_dir` (or any of its subdirectories) is
        considered tracked if its base name (not path!) matches any of the
        positive globs and none of the negative globs. `tracked_files` can be
        used to limit what files are actually written to disk: if this set is
        not empty, only files that are in this set will be actually written to
        disk (and all other files are discarded). This is beneficial when
        `lucicfg` is used to generate only a subset of config files, e.g. during
        the  migration from handcrafted to generated configs. Knowing the
        tracked files set is also important when some generated file disappears
        from `lucicfg` output: it must be deleted from the disk as well. To do
        this, `lucicfg` needs to know what files are safe to delete. If
        `tracked_files` is empty (default), `lucicfg` will save all generated
