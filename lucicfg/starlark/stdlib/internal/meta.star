...
def _config(
  ...
  Args:
    config_service_host: a hostname of a LUCI Config Service to send validation
        requests to. Default is whatever hardcoded in `lucicfg` binary, usually
    ...
    config_dir: a directory to place generated configs into, relative to the
        directory that contains the entry point \*.star script. ".." is allowed.
        The directory will be created if it is absent. Default is "generated".
    tracked_files: a list of globs patterns that define a subset of files under
        ...
        `['*-dev.cfg']` of dev. If `tracked_files` is empty, lucicfg will never
        delete any files. In this case it is responsibility of the caller to
        make sure no stale output remains.
    fail_on_warnings: True to treat validation warnings as errors. To override
        to False via command line flags use `-fail-on-warnings=false`.
  """
