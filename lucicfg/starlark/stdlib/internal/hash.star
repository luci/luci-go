...
def _size(algo):
  """Returns the number of bytes hash.digest(algo, ...) would return.

  Args:
    algo: a hashing algorithm to use, e.g. `hash.SHA256`. Required.
  """
  if algo == hash.SHA256:
    return 32
  fail('hash.size: unknown hashing algorithm %r' % (algo,))


def _digest(algo, blob):
  """Calculates the hash digest of `blob` using the given algorithm.
...
def _hexdigest(algo, blob):
  ...
  """
  return strutil.hex_encode(_digest(algo, blob))
