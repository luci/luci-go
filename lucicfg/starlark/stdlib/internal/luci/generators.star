...
def gen_milo_cfg(ctx):
  ...
  views = [
      v for v in graph.children(keys.project(), order_by=graph.DEFINITION_ORDER)
      if v.key.kind in (kinds.LIST_VIEW, kinds.CONSOLE_VIEW)
  ]
  if not views:
    return
...
def _milo_check_connections():
  ...
  entry_to_view = {
      kinds.LIST_VIEW_ENTRY: kinds.LIST_VIEW,
      kinds.CONSOLE_VIEW_ENTRY: kinds.CONSOLE_VIEW,
  }
  for e in graph.children(keys.milo()):
    view_kind = entry_to_view.get(e.key.kind)
    if not view_kind:
      continue
    views = graph.parents(e.key, view_kind)
    if len(views) == 0:
...
def _milo_console_pb(view, opts, project_name):
  """Given either LIST_VIEW or CONSOLE_VIEW node, returns milo_pb.Console."""
...
def _milo_console_view(view, opts, project_name):
  ...
  builders = []
  seen = {}
  for e in graph.children(view.key, kinds.CONSOLE_VIEW_ENTRY, order_by=graph.DEFINITION_ORDER):
    pb = _milo_builder_pb(e, view, project_name, seen)
    if pb:
