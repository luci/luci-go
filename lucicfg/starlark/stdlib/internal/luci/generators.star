...
def _cq_check_connections():
  """Ensures all cq_tryjob_verifier are connected to one and only cq_group."""
...
def _cq_config_group(cq_group, project):
  ...
  if not gerrit_cq_ability.committer_list:
    error('at least one CQ_COMMITTER acl.entry must be specified (either here on in luci.project)', trace=cq_group.trace)
...
def _cq_retry_config(retry_config):
  """cq.retry_config(...) => cq_pb.Verifiers.Tryjob.RetryConfig."""
  if not retry_config:
    return None
...
def _cq_tryjob_builder(verifier, cq_group, project, seen):
  ...
  if verifier.props.external:
    # Either have an externally supplied name.
