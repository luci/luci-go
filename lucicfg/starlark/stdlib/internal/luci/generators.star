...
def gen_scheduler_cfg(ctx):
  ...
  for bucket in buckets:
    ...
    for builder in graph.children(bucket.key, kinds.BUILDER):
      ...
      if targets and not builder.props.service_account:
        error(
            '%s needs service_account set, it triggers another builders (%s)' %
                (builder, targets),
            trace=builder.trace,
        )
      else:
  ...
  scheduler = get_service('scheduler', 'using LUCI Scheduler')
  buildbucket = get_service('buildbucket', 'using LUCI Scheduler')
  ...
  # Add Trigger entry for each gitiles poller.
...
def _scheduler_disambiguate_ids(nodes):
  ...
  # '<builder>-<bucket>' name happened to clash with some other '<builder>'.
  # This should be super rare. Lack of 'while' in starlark makes it difficult to
  # deal with automatically though, so we just give up and ask the user to pick
  # some other name.
  def use_name(name, node):
    if name in used:
      fail((
          '%s and %s cause ambiguities in the scheduler config file, ' +
          'pick names that don\'t end with a bucket name') % (node, used[name]))
    names[node] = name
    used[name] = node
  ...
  for nm, candidates in claims.items():
    if len(candidates) == 1:
      ...
      for node in candidates:
        use_name('%s-%s' % (nm, node.props.bucket), node)
