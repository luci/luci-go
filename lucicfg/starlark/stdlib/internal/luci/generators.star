...
def register():
    ...
    lucicfg.generator(impl = gen_tricium_cfg)  # must after gen_cq_cfg
...
def gen_tricium_cfg(ctx):
    ...
    tricium_config = None
    for cfg, cq_group in zip(tricium_configs, cq_groups):
        if cfg == None:
            continue
        elif tricium_config == None:
            tricium_config = struct(cfg = cfg, mapping_cq_group = cq_group)
        elif cfg != tricium_config.cfg:
            # if multiple config groups have defined Tricium analyzer, they
            # MUST generate the same Tricium project config.
            error(
                "%s is watching different set of Gerrit repos or defining different analyzers from %s" % (
                    cq_group,
                    tricium_config.mapping_cq_group,
                ),
    ...
    if tricium_config:
        ...
        cfg = tricium_config.cfg
        cfg.service_account = "%s@appspot.gserviceaccount.com" % service.app_id
        set_config(ctx, service.cfg_file, cfg)
    ...
    ret.repos = [
        tricium_pb.RepoDetails(
            gerrit_project = tricium_pb.RepoDetails.GerritProject(
                ...
                project = proj,
                git_url = "https://%s.googlesource.com" % host,
            ),
...
def _compute_func_name(recipe):
    """ Returns the normalized(alpha-numeric only) function name."""
