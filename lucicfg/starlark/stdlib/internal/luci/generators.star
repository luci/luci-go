...
def _buildbucket_builders(bucket, swarming_host):
  ...
  for node in graph.children(bucket.key, kinds.BUILDER):
    exe, recipe = _handle_executable(node)
...
def _handle_executable(node):
  ...
  Builder node => buildbucket_pb.Builder.Recipe | common_pb.Executable

  This function currently produces both Recipe and Executable definitions
  to maintain backwards compatibility in the config output for recipes.
  ...
  if executable.props.recipe:
    ...
    recipe_def = None
  return executable_def, recipe_def
