...
def _duration(attr, val, precision=time.second, default=None):
  ...
  Fails if rounding the duration to the given precision loses information.
  ...
  Returns:
    An integer: a whole number of 'precision' units in a given duration value.
  """
  if val == None:
    if default == None:
      fail('bad %r: missing' % attr)
  ...
  if res * precision != val:
    fail((
        'bad %r: losing precision when truncating to %s units, ' +
        'use time.truncate(...) to acknowledge') % (attr, precision))
