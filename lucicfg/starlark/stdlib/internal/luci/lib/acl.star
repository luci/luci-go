...
#   user: an user email.
#   group: a group name.
_elementary_ctor = gensym('acl.elementary')


def _role(name, project_level_only=False):
  """Defines a role.
...
def _entry(role, roles=None, group=None, groups=None, user=None, users=None):
  """An ACL entry: assigns given role (or roles) to given individuals or groups.

  Either 'role' or 'roles' (but not both) are required. 'group'/'groups' and
  'user'/'users' are optional. When using a singular form, the corresponding
  plural form is not allowed (and vice versa).

  Specifying 'group' and 'user' (or their plural equivalents) at once is
  allowed.

  Specifying an empty ACL entry (when 'group'/'groups' and 'user'/'users' are
  all empty) is allowed. It is ignored everywhere. Useful for things like:
  ...
  Args:
    role: a single acl.role to assign to principals.
    roles: a list of roles to assign, as alternative to 'role'.
    group: a group name to assign the role to.
    groups: a list of groups to assign the role to, as alternative to 'group'.
    user: a user email to assign the role to.
    users: a list of users to assign the role to, as alternative to 'user'.

  Returns:
    acl.entry struct, consider it opaque.
  """
  if (not role and not roles) or (role and roles):
    fail('either "role" or "roles" (but not both) are required')
  roles = [role] if role != None else roles

  if group and groups:
    fail('use either "group" or "groups", but not both at the same time')
  groups = [group] if group != None else (groups or [])

  if user and users:
    fail('use either "user" or "users", but not both at the same time')
  users = [user] if user != None else (users or [])

  for r in roles:
    validate.struct('role', r, _role_ctor)
  for g in groups:
    validate.string('group', g)
  for u in users:
    validate.string('user', u)
  ...
  for e in acls:
    ...
    for r in e.roles:
      if r.project_level_only and not project_level:
        fail('bad "acls": role %s can only be set on the project level' % r.name)
  return acls
...
def _normalize_acls(acls):
  ...
  Args:
    acls: an iterable of acl.entry objects to expand.
...
acl = struct(
    ...
    **_roles_dict([
        ...
        _role('LOGDOG_READER', project_level_only=True),
        # Writing logs under project's logdog prefix.
        _role('LOGDOG_WRITER', project_level_only=True),
