...
def _cache(path, *, name = None, wait_for_warm_cache = None):
    ...
    Args:
      ...
      wait_for_warm_cache: how long to wait (with minutes precision) for a bot
          ...
          (regardless whether it has the cache or not). If there are no bots
          with this cache at all, the build will skip waiting and will
          immediately fallback to any matching bot. By default (if unset or
          zero), there'll be no attempt to find a bot with this cache already
          warm: the build may or may not end up on a warm bot, there's no
          guarantee one way or another.
