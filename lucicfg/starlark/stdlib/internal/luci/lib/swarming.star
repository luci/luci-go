...
def _cache(path, *, name = None, wait_for_warm_cache = None):
    ...
    Args:
      ...
      name: identifier of the cache to mount to the path. Default is same value
        as `path` itself. Must be unique in the given builder definition (cannot
        mount the same cache to multiple paths).
      wait_for_warm_cache: how long to wait (with minutes precision) for a bot
        ...
        fallback to any matching bot. By default (if unset or zero), there'll be
        no attempt to find a bot with this cache already warm: the build may or
        may not end up on a warm bot, there's no guarantee one way or another.
...
def _dimension(value, *, expiration = None):
    ...
    Intended to be used as a value in `dimensions` dict of luci.builder(...)
