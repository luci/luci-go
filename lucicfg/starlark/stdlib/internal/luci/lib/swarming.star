...
#
# Fields:
#   path: str, where to mount the cache.
#   name: str, name of the cache to mount.
...
def _cache(path, name=None, wait_for_warm_cache=None):
  """A request for the bot to mount a named cache into a path.

  Each bot has a LRU of named caches: think of them as local named directories
  in some protected place that survive between tasks.
  ...
  appear as an empty directory. Otherwise it will contain whatever what left
  there by the previous build on that bot that mounted it, even if that build is
  completely irrelevant to the current build and just happened to use the same
  named cache.
  ...
  directory and all files inside) are evicted in LRU manner. Renaming a cache is
  equivalent to clearing it from the builder perspective. The files will still
  be there, but eventually will be purged by GC.

  Additionally, Buildbucket always implicitly requests to mount a special
  builder cache into 'builder' path:

      swarming.cache('builder', name=hash('<bucket>/<builder>'))
  ...
  explicitly named cache can be mounted into 'builder' path these builders.
  ...
  Args:
    path: path where the cache should be mounted into, relative to some known
        ...
        all. Must be unique in the given builder definition (can't mount
        multiple caches into the same path).
    name: identifier of the cache to mount into the path. Default is same
        value as 'path' itself.
