...
def _cache(path, *, name=None, wait_for_warm_cache=None):
  ...
  Args:
    ...
    name: identifier of the cache to mount to the path. Must be unique in the
        given builder definition (cannot mount the same cache to multiple
        paths). If set to '', wait_for_warm_cache must be set to time.zero. This
        effectively disable the cache. Otherwise, if unspecified and thus None,
        it defaults to the same value as `path`.
    wait_for_warm_cache: how long to wait (with minutes precision) for a bot
        ...
        fallback to any matching bot. By default (if unset), there'll be no
        attempt to find a bot with this cache already warm: the build may or may
        not end up on a warm bot, there's no guarantee one way or another. The
        value time.zero can be used in buildbucket to disable a default
        dimension set via the buildbucket template, like `builder`, but only if
        path is also empty.
  ...
  if name == '' and wait_for_warm_cache == time.zero:
    return _cache_ctor(
        path = path,
        name = name,
        wait_for_warm_cache = wait_for_warm_cache)
