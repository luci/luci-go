...
def _validate_test_presentation(attr, config):
    ...
    """
    return validate.struct(attr, config, _test_presentation_config_ctor, default = _test_presentation(), required = False)
...
def _test_presentation_to_dict(config):
    ...
    ret = {}
    ret["column_keys"] = config.column_keys
    ret["grouping_keys"] = config.grouping_keys
    return ret
