...
def _settings(*, enable=False, export_test_results=None):
  ...
  Args:
    ...
    export_test_results: list of resultdb_pb.BigQueryExport() protos,
        configurations for exporting specific subsets of test results to a
        designated BigQuery table, use resultdb.export_test_results(...)
        to create these.
  ...
  return buildbucket_pb.Builder.ResultDB(
      ...
      bq_exports = export_test_results or [],
  )
...
def _export_test_results(
  ...
  Args:
    ...
    predicate: A predicate_pb.TestResultPredicate() proto. If given, specifies
        ...
        Use resultdb.predicate(...) to generate this, if needed.
...
def _validate_settings(settings):
  ...
  Args:
    settings: A buildbucket_pb.Builder.ResultDB(), such as the one returned
    by resultdb.settings(...).
