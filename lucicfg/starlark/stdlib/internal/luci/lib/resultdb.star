...
def _settings(*, enable = False, bq_exports = None):
    ...
    Args:
      ...
      bq_exports: list of resultdb_pb.BigQueryExport() protos, configurations
          for exporting specific subsets of test results to a designated
          BigQuery table, use resultdb.export_test_results(...) to create these.
...
def _export_test_results(
    ...
    Args:
      ...
      predicate: A predicate_pb.TestResultPredicate() proto. If given, specifies
          the subset of test results to export to the above table, instead of
          all. Use resultdb.test_result_predicate(...) to generate this, if
          needed.
