...
def _notifiable_add(name, props, template, notified_by):
  """Adds a luci.notifiable node of the given kind.

  This is a shared portion of luci.notifier and luci.tree_closer implementation.

  Args:
    name: name of the notifier or tree closer.
    props: a dict with node props, will be mutated.
    template: an optional reference to a luci.notifier_template.
    notified_by: builders to link to.

  Returns:
    A keyset with luci.notifiable.
  """
