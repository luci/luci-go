...
keys = struct(
    ...
    # "external" entities, and their names don't really matter.
    #
    # Note that IDs of keys whose kind stars with '_' (like '_AUTOGEN' here),
    # are skipped when printing the key in error messages. Thus the internal
    # meaningless auto-generated part of the name isn't showing up in error
    # message.
    unique = lambda kind, name: graph.key('_AUTOGEN', str(sequence.next(kind)), kind, name),
)
