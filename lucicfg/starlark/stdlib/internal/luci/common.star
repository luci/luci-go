...
def _global_key(kind, attr, ref):
  ...
  """
  return graph.from_keyset(ref, kind) or graph.key(kind, validate.string(attr, ref))
...
def _bucket_scoped_key(kind, attr, ref):
  ...
  Returns:
    graph.key of the requested kind.
  """
  # If 'ref' is a keyset, a key with requested kind must be there.
  k = graph.from_keyset(ref, kind)
  if k != None:
    return k
  # Otherwise 'ref' must be a string.
...
keys = struct(
    ...
    # TODO(vadimsh): These currently require strings, not keysets. They are
    # currently not directly used by anything, only through 'builder_ref' and
    # 'triggerer' nodes. As such, they are never referenced via keysets.
