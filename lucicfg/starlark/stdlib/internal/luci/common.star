...
def _maybe_add_external_builder(ref):
  ...
  In the future such nodes will be defined when loading external project's
  config, so it becomes impossible to refer to an external builder that doesn't
  actually exist.
  """
...
def _view_add_entry(kind, view, builder, buildbot, props=None):
  ...
  referring to builders defined in another projects.
