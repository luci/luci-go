...
def _builder_ref_follow(ref_node, context_node):
  """Given a BUILDER_REF node, returns a BUILDER graph.node.
  ...
  builder.
   ...
   Note that the emitted error doesn't stop the generation (but will eventually
   mark it as failed, when all such errors are collected).
  ...
  # No ambiguity. Assert ref points to a builder.
  if len(variants) == 1:
    builder = variants[0]
    if builder.key.kind != kinds.BUILDER:
      fail('%s unexpectedly points to not-a-builder %s' % (ref_node, builder))
    return builder
