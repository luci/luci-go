...
def _builder_ref_add(name, target):
  ...
  Each builder has two such refs: one is bucket-scoped (for when the builder
  is referenced using its full name "<bucket>/<name>"), another is global (when
  the builder is referenced just as "<name>").
...
def _builder_ref_validate(ref_node, context_node):
  ...
  builder. Note that the emitted error doesn't stop the generation (but will
  eventually mark it as failed, when all such errors are collected).
  ...
  # Emit the error, but carry on the execution to collect more errors.
  if len(variants) != 1:
    error(
