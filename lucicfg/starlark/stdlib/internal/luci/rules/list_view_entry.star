...
def list_view_entry(builder=None, list_view=None, buildbot=None):
  ...
  Args:
    builder: a builder to add, see luci.builder(...). Can be omitted for extra
        deprecated case of Buildbot-only views. `buildbot` filed must be set
        in this case.
    ...
    buildbot: a reference to an equivalent Buildbot builder, given as
        `<master>/<builder>` string. Deprecated. Exists only while there are
        still Buildbot builders, to aid in the migration.
  """
  if builder != None:
