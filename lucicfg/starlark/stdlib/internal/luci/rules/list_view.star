...
def list_view(
  ...
  Or one by one via separate luci.list_view_entry(...) declarations:
  ...
  builders with equivalent Buildbot builders) use of luci.list_view_entry(...)
  ...
  Args:
    ...
    repo: URL of a git repository to display in the view rows, should start
        with `https://`.
    favicon: optional https URL to the favicon for this view, must be hosted on
        `storage.googleapis.com`. Defaults to `favicon` in lucicfg.milo(...).
    entries: a list of builders or luci.list_view_entry(...) objects to include
        into this view.
  """
  ...
  graph.add_node(key, props = {
      ...
      'repo': validate.string('repo', repo, regexp=r'https://.+', required=False),
  ...
  # automatically, so in the end all children have the same kind.
  for entry in validate.list('entries', entries):
    if not graph.is_keyset(entry) or not entry.has(kinds.LIST_VIEW_ENTRY):
