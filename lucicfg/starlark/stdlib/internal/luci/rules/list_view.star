...
def _list_view(
    ...
    Args:
      ...
      name: a name of this view, will show up in URLs. Note that names of
          luci.list_view(...) and luci.console_view(...) are in the same
          namespace i.e. defining a list view with the same name as some console
          view (and vice versa) causes an error. Required.
      title: a title of this view, will show up in UI. Defaults to `name`.
      favicon: optional https URL to the favicon for this view, must be hosted
          on `storage.googleapis.com`. Defaults to `favicon` in luci.milo(...).
      entries: a list of builders or luci.list_view_entry(...) entities to
          include into this view.
    """
