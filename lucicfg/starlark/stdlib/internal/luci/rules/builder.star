...
def _builder(
  ...
  Args:
    ...
    resultdb_settings: resultdb.settings(...) struct with a confiuration that
        defines if buildbucket should sync this builder's builds with ResultDB
        and which results to export to BigQuery, and to which project, dataset,
        and table.
