    ...
    **A** builds to trigger **B** builds via LUCI Scheduler's EmitTriggers RPC
    ...
    Args:
      ...
      service_account: an email of a service account to run the executable
          under: the executable (and various tools it calls, e.g. gsutil) will
          be able to make outbound HTTP calls that have an OAuth access token
          belonging to this service account (provided it is registered with
          LUCI). Supports the module-scoped default.
      ...
      dimensions: a dict with swarming dimensions, indicating requirements for
          ...
          are either strings (e.g. `Linux`), swarming.dimension(...) objects
          (for defining expiring dimensions) or lists of thereof. Supports the
      ...
      swarming_host: appspot hostname of a Swarming service to use for this
          builder instead of the default specified in luci.project(...). Use
          with great caution. Supports the module-scoped default.
      swarming_tags: a list of tags (`k:v` strings) to assign to the Swarming
          task that runs the builder. Each tag will also end up in
          `swarming_tag` Buildbucket tag, for example
          `swarming_tag:builder:release`. Supports the module-scoped defaults.
           They are joined with the explicitly passed tags.
      ...
      schedule: string with a cron schedule that describes when to run this
          builder. See [Defining cron schedules](#schedules_doc) for the
          expected format of this field. If None, the builder will not be
          running periodically.
      triggering_policy: scheduler.policy(...) struct with a configuration that
          defines when and how LUCI Scheduler should launch new builds in
          ...
          Buildbucket. By default, only one concurrent build is allowed and
          while it runs, triggering requests accumulate in a queue. Once the
          build finishes, if the queue is not empty, a new build starts right
          away, "consuming" all pending requests. See scheduler.policy(...) doc
          for more details. Supports the module-scoped default.
      ...
      experimental: if True, by default a new build in this builder will be
          ...
          behave differently (e.g. avoiding any side-effects). If None, defer
          the decision to Buildbucket service. Supports the module-scoped
          default.
      ...
      repo: URL of a primary git repository (starting with `https://`)
          ...
          should use to chronologically order builds on this builder. If
          unknown, builds will be ordered by creation time. If unset, will be
          taken from the configuration of luci.gitiles_poller(...) that trigger
          this builder if they all poll the same repo.
      ...
      resultdb_settings: A buildbucket_pb.Builder.ResultDB, such as one created
          ...
          Buildbucket:ResultDB integration should be enabled for this builder
          and which results to export to BigQuery.
      ...
      notifies: list of luci.notifier(...) or luci.tree_closer(...) the builder
          ...
          via `notified_by` field in luci.notifier(...) or
          luci.tree_closer(...).
    """
