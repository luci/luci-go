...
# Guard usage of luci.tree_closer behind an experiment until it is stable.
...
def _tree_closer(
  ...
  ***
  ...
  luci.tree_closer(...) objects subscribed to it they can decide whether to
  ...
  field in luci.builder(...). For that reason they share the same namespace,
  i.e. it is not allowed to have a luci.notifier(...) and a
  luci.tree_closer(...) with the same name.
