...
def builder_group(name, builders):
  ...
  # kind of node 'b' refers to and thus can't construct its precise key.
  for b in builders:
    graph.add_edge(key, keys.builder_ref(b))

  # Allow this builder group itself to be referenced from other nodes (e.g.
  # other builder groups) via its global name.
