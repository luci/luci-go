  ...
  * A ref belonging to the watched set has just been created.
  ...
  Args:
    ...
    schedule: string with a schedule that describes when to run one iteration
        of the poller. See (#durations_doc) for the expected format of this
        field. It is rare to use custom schedules for pollers. By default, the
        poller will run each 30 sec.
