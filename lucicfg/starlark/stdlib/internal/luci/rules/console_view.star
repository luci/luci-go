...
def _console_view(
    ...
    Args:
      ...
      name: a name of this console, will show up in URLs. Note that names of
          luci.console_view(...) and luci.list_view(...) are in the same
          namespace i.e. defining a console view with the same name as some list
      ...
      refs: a list of regular expressions that define the set of refs to pull
          ...
          with `^` or end with `$` as they will be added automatically.  If
          empty, defaults to `['refs/heads/master']`.
