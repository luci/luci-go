...
def console_view(
  ...
  Args:
    name: a name of this console, will show up in URLs. Note that names of
        ...
        i.e. defining a console view with same name as some list view (and
  ...
  keyset = view.add_view(keys.console_view(name), props = {
      'name': name,
  ...
  # Unlike in list_view, all entries here must be console_view_entry, since
  # we need annotations like 'category' and 'short_name' that can be given
  # only through console_view_entry.
  for entry in validate.list('entries', entries):
    graph.add_edge(
        parent = keyset.get(kinds.CONSOLE_VIEW),
        child = entry.get(kinds.CONSOLE_VIEW_ENTRY),
    )
