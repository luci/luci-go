...
def _notifier(
  ...
  Args:
    ...
    on_failure: if True, notify on each failure. Handles build vs transient
        (aka "infra") failures based on failure_type. Default is False.
    on_new_failure: if True, notify on a build failure unless the previous build
        was a failure too. Handles build vs transient (aka "infra") failures
        based on failure_type. Default is False.
    ...
    failure_type: enum specifying which build statuses to consider as failures.
        Options include build failures, transient (aka "infra") failures, or both.
        Default is build failures.
  ...
  failure_type =validate.int('failure_type', failure_type, required=False)
