...
def _notifier(
  ...
  Args:
    ...
    on_new_failure: if True, notify on a build failure unless the previous build
        was the same type of failure. Handles build vs transient (aka "infra")
        failures based on failure_type. Default is False.
    ...
    failure_type: a dict specifying which build statuses to consider as failures.
        Options include build failures and transient (aka "infra") failures.
        Default is None.
  ...
  failure_type =validate.str_dict('failure_type', failure_type)
