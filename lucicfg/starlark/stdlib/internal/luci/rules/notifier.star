...
def _notifier(
  ...
  field here or `notifies` field in luci.builder(...). Whenever something
  happens to a builder (e.g. a build fails), the builder "notifies" all
  luci.notifier(...) objects subscribed to it, and each notifier in turn filters
  ...
  ***
  ...
  on_change = validate.bool('on_change', on_change, required=False)
  ...
  if not(on_change or on_failure or on_new_failure or on_success):
    fail('at least one on_... condition is required')

  notify_email = validate.list('notify_email', notify_email)
  for e in notify_email:
    validate.string('notify_email', e)
  ...
  for repo in blamelist_repos_whitelist:
    validate.string('blamelist_repos_whitelist', repo, regexp=r'https://.+')
