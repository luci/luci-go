...
def cq_group(
  ...
  Args:
    name: a name of this group, to reference it in other rules. Doesn't show up
        anywhere in configs or UI. Required.
    ...
    acls: list of acl.entry(...) objects with ACLs specific for this CQ group.
        Only `acl.CQ_*` roles are significant. By default ACLs are inherited
    ...
    allow_submit_with_open_deps: controls how CQ full run behaves when the
        ...
        *this* CL depends). If set to False (default), the CQ will abort a full
        ...
        turn, if Gerrit project config allows this, Gerrit will execute submit
