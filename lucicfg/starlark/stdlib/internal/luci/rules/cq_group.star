...
def cq_group(
  ...
  Args:
    ...
    retry_config: a cq.retry_config(...) struct with parameters for deciding
        whether to retry a single build. See cq.retry_config(...) for details.
        Defaults to `cq.RETRY_TRANSIENT_FAILURES` if omitted. Pass
        `cq.RETRY_NONE` to completely disable retries.
  """
  ...
  # We require each repo to be associated with at most one cq.refset(...). This
  # will allow later to extend this struct with more fields without worrying how
  # to merge their values. There's also a global check in generators.star that
  # ensures a repo belong only to one cq_group.
  seen_repos = {}
  for w in validate.list('watch', watch, required=True):
    cqimpl.validate_refset('watch', w)
    if w.__repo_key in seen_repos:
      fail('bad "watch": repository %r is mentioned more than once, please merge the definitions' % w.__repo)
    seen_repos[w.__repo_key] = w
