...
def _project(
  ...
  Args:
    ...
    config_dir: a subdirectory of the config output directory to place generated
        LUCI configs under (see `config_dir` in lucicfg.config(...)). Default
        is `.`. Useful when using `lucicfg` to generate LUCI and non-LUCI
