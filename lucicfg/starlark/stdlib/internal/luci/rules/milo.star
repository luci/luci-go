...
def milo(
  ...
  Args:
    ...
    monorail_components: a list of the Monorail component that the bug
        should exist under, in the hierarchical `>` separated format, e.g.
        `Infra>Client>ChromeOS>CI`. Required if `monorail_project` is set,
        otherwise must be None.
    bug_summary: a text template for generating new bug's summary given
        a builder on whose page the user clicked the feedback link. Must be None
        if `monorail_project` is unset.
    bug_description: a text template for generating new bug's description given
        a builder on whose page the user clicked the feedback link. Must be None
        if `monorail_project` is unset.
  """
