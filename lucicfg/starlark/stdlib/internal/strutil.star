...
def _generate_name(fmt, inputs):
  ...
  Obeys following rules:
    ...
    4.  There are no guarantees whatsoever for names generated by different
        `lucicfg` runs, even if `generate_name` takes same `format` and
        `inputs`. They are usually repeatable, but this should not be relied
        upon.

  Args:
    format: a format string with single `%s` inside it. Required.
    inputs: an iterable of strings or stringifyable objects. Required.
