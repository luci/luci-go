...
//
// Never fails. May returns incomplete or even empty object if the given
// doc string has unrecognized format.
...
func parseFields(lines []string) []Field {
	...
	for len(lines) > 0 {
		...
		// Read the rest of the field description, it is indented.
		var block []string
		block, lines = readUntil(lines, func(_ *string, l string) bool {
			return l != "" && !hasLeadingSpace(l)
		})
...
func deindent(lines []string) []string {
	...
	// level (in number or whitespace runes) among non-empty lines.
	numRunesToSkip := inf
	for idx, l := range lines {
	...
	offsetToSkip := func(s string) int {
		idx := 0
		for pos, _ := range s {
	...
	// Cut the indentation by skipping 'numRunesToSkip' number or runes in each
	// non-empty line.
	for idx, l := range lines {
		if l != "" {
			lines[idx] = l[offsetToSkip(l):]
		}
	}
	return lines
}

// countLeadingSpace returns number of space runs at the suffix of a string.
...
// parseBlockTitle recognizes strings like "Args:" that indicate a beginning of
// a named block.
//
// We are pretty strict here to avoid misfiring on ':' that appear in sentences:
//   * The title should star with the upper case ("Args", not "args").
