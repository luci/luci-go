...
//
// It recognized top-level function declarations, top-level assignments (e.g.
...
//
// The top of the tress is represented by Module node.
type Node interface {
	// Name is a name of the entity this node defines.
	//
	// E.g. it's a name of a function, variable, constant, etc.
	//
	...
	// debugDump writes information about this subtree into 'w'.
	//
	// Ignores any errors from 'w'. Intended for tests only.
	debugDump(w io.Writer, indent string)
}
...
// debugDump writes information about this node into 'w'.
func (n *Var) debugDump(w io.Writer, indent string) {
...
// debugDump writes information about this node into 'w'.
func (n *Function) debugDump(w io.Writer, indent string) {
	fmt.Fprintf(w, "%s%s = func\n", indent, n.name)
}

// Reference is a node that represents <var> = a.b.c.
//
// It is either a top-level assignment, ora keyword argument in a function call
...
// debugDump writes information about this node into 'w'.
func (n *Reference) debugDump(w io.Writer, indent string) {
...
// debugDump writes information about this node into 'w'.
func (n *ExternalReference) debugDump(w io.Writer, indent string) {
...
// debugDumpNS is shared between Namespace and Module.
func (n *Namespace) debugDumpNS(w io.Writer, indent, kind string) {
	fmt.Fprintf(w, "%s%s = %s\n", indent, n.name, kind)
	if len(n.Nodes) != 0 {
		for _, n := range n.Nodes {
			n.debugDump(w, indent+"  ")
		}
	} else {
...
// debugDump writes information about this subtree into 'w'.
func (n *Namespace) debugDump(w io.Writer, indent string) {
	n.debugDumpNS(w, indent, "namespace")
}
...
// debugDump writes information about this subtree into 'w'.
func (n *Module) debugDump(w io.Writer, indent string) {
	n.debugDumpNS(w, indent, "module")
}
...
// caller.
//
// Only following forms are recognized:
