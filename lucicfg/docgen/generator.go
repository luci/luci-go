...
func (g *Generator) linkifySymbols(text string) string {
	return symRefRe.ReplaceAllStringFunc(text, func(match string) string {
		name := match
		if idx := strings.IndexRune(match, '('); idx != -1 {
			name = name[:idx]
		}
		if sym := g.links[name]; sym != nil {
...
// IsStruct returns true if this symbol represents a struct.
func (s *symbol) IsStruct() bool {
	_, yes := s.Symbol.(*symbols.Struct)
	return yes
}
...
func (s *symbol) Symbols() ([]*symbol, error) {
	...
	syms := s.Symbol.(*symbols.Struct).Symbols()
...
func (s *symbol) Anchor() string {
	// TODO(vadimsh): Do we need any normalization? a.b.c.d seem to work fine in
	// gitiles.
	return s.FullName
}
