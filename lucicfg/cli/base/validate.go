...
type ValidateParams struct {
	...
	Source []string           // paths in the package to starlark modules to lint
	Output lucicfg.Output     // generated output files to validate
	Meta   lucicfg.Meta       // validation options settable through Starlark

	// ConfigService returns LUCI Config RPC client that sends requests
	// to the given host.
	//
...
// Validate validate both input source code and generated config files.
//
// It is a common part of subcommands that validate configs.
//
// Source code is checked using buildifer linters and formatters, if enabled.
// This is controlled by CheckFmt and LintChecks meta arg.
//
...
//
// Dumps all validation errors to the logger. In addition to detailed validation
