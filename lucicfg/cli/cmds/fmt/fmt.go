...
func Cmd(params base.Parameters) *subcommands.Command {
	return &subcommands.Command{
		...
		ShortDesc: "formats *.star files using buildifier rules",
		LongDesc: `Applies standard formatting to the given Starlark files.

As positional arguments accepts zero or more paths, where each path is
either a file or a directory. Directories will be searched for *.star files
recursively. If not arguments are given, processes *.star files recursively
starting from the current directory.

By default rewrites unformatted files. Pass -dry-run flag to just check files
formatting without overwriting them.
`,
...
// expandDirectories replaces directories in `paths` with recursive listing or
// *.star files in them.
