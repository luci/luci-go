...
what's already on disk in -config-dir directory. By default uses semantic
comparison (i.e. config files on disk are deserialized and compared to the
generated files as objects). If -strict is used, compares files as byte blobs.
In this case 'validate' detects no changes if and only if 'generate' produces no
diff.

If configs on disk are different from generated ones, the subcommand exits with
non-zero exit code. Otherwise the configs are sent to LUCI Config service for
validation. Partitioning into config sets is specified in the Starlark code in
this case, -config-set flag is rejected if given.
...
statement. See its doc for more details.
		...
		CommandRun: func() subcommands.CommandRun {
			...
			vr.Flags.BoolVar(&vr.strict, "strict", false,
				"Use byte-by-byte comparison instead of comparing configs as messages.")
			return vr
		},
	}
}
