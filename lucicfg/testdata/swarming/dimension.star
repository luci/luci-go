...
def test_dimension_ctor():
  def eq(dim, value, expiration_mins):
    assert.eq(dim.value, value)
    assert.eq(dim.expiration_mins, expiration_mins)
  eq(swarming.dimension('abc'), 'abc', None)
  eq(swarming.dimension('abc', time.minute), 'abc', 1)
  eq(swarming.dimension('abc', 5*time.minute), 'abc', 5)
...
def test_validate_dimensions():
  call = lambda dims: swarmingimpl.validate_dimensions('dims', dims)
