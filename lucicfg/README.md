...
functionality provided by the kernel is pretty generic:
...
The builtins are wrapped in two layers of Starlark code:
  * [starlark/stdlib/internal], excluding `.../luci`: generic (not LUCI
    ...
    generators ([1], [2], [3]) as well as extend LUCI config generator. This
    ...
    lucicfg's public API, so it should be treated as such (no hacks, no
...
a version of the bundled Starlark library. The version string is explicitly set
in [version.go] and looks like `MAJOR.MINOR.PATCH`.

If a script is relying on a feature which is available only in some recent
...
Thus it is **very important** to update [version.go] before releasing changes:
  ...
  * Increment `PATCH` version when you make backwards compatible changes.
    A change is backward compatible if it doesn't reduce Starlark API surface
    and doesn't affect `lucicfg` emitted output (assuming inputs do not change).
    In particular, adding a new feature that doesn't affect existing features is
    backward compatible.
  * Increment `MINOR` version when you make backwards incompatible changes.
    Releasing such changes may require modifying user scripts or asking users
    to regenerate their configs to get a new output.
...
backward compatibility promises, hide it behind an experiment flag. Users will
need to opt-in to use it. See [starlark/stdlib/internal/experiments.star] for
more info.
...
2. Wait until it is rolled into infra.git. The roll will look like this
   [infra.git commit]. Notice its git hash `0302871c78495...`.
3. Eventually [CIPD package builders] will produce per-platform lucicfg
   [CIPD packages] tagged with infra.git's hash `git_revision:0302871c78495...`.
   Like [this one].
4. Land a depot_tools CL to release the new version to developers:
  1. Modify [cipd_manifest.txt]:
     ```
     # LUCI config generator
     infra/tools/luci/lucicfg/${platform} git_revision:0302871c78495...
     ```
  2. As instructed in the comments of this file, regenerate
     cipd_manifest.versions by running:
     ```
     cipd ensure-file-resolve -ensure-file cipd_manifest.txt
     ```
  3. Send a CL [like this], preferably describing what's new in the CL message.
5. Bots don't use depot_tools. To release the change to bots, modify
   [cr-buildbucket/settings.cfg] like [so].
...
do other things, then come back >30 min later, look up the necessary infra.git
DEPS roll (or just use the latest one) and proceed to steps 4 and 5.

[luci-go commit]: https://chromium.googlesource.com/infra/luci/luci-go/+/23d692c890707762ec0ca243cc41f972149c4126
[infra.git commit]: https://chromium.googlesource.com/infra/infra/+/0302871c78495a60c839ad59136187b2ca3f09a2
...
[this one]: https://chrome-infra-packages.appspot.com/p/infra/tools/luci/lucicfg/linux-amd64/+/git_revision:0302871c78495a60c839ad59136187b2ca3f09a2
