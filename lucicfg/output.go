...
// Bytes is a raw file body to put on disk.
func (m MessageDatum) Bytes() []byte {
	blob, err := starlarkprotov2.ToTextPB(m.Message)
	if err != nil {
		panic(err)
	}
	out := make([]byte, 0, len(m.Header)+len(blob))
...
//
// For output files based on proto messages uses semantic comparison, i.e. loads
// the file on disk as proto too and compares it the output as a proto message.
//
...
//
// By defaults writes only files that have semantic changes (based on Compare).
// If 'force' is true, writes all files no matter what.
//
// Returns a list of updated files and a list of files that are already
// up-to-date, same as Compare. If 'force' is true, the latter list is always
// empty.
//
// Creates missing directories. Not atomic. All files have mode 0666.
func (o Output) Write(dir string, force bool) (changed, unchanged []string, err error) {
