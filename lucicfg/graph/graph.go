...
func (g *Graph) Descendants(root *Key, orderBy, topology string, visitor Visitor) ([]*Node, error) {
	...
	switch topology {
	case "breadth":
		err = descBreadth(rootNode, orderBy, visitor, &visited, visitedSet)
	case "depth":
		err = descDepth(rootNode, orderBy, visitor, &visited, visitedSet)
	default:
		panic("impossible")
	}
...
func descBreadth(root *Node, orderBy string, visitor Visitor, visited *[]*Node, visitedSet map[*Node]struct{}) error {
	queue := list.New()
	queue.PushBack(root)
...
func descDepth(root *Node, orderBy string, visitor Visitor, visited *[]*Node, visitedSet map[*Node]struct{}) error {
	// Been here before?
	...
	// Go there, right now.
	for _, n := range next {
		if err := descDepth(n, orderBy, visitor, visited, visitedSet); err != nil {
			return err
		}
	}
