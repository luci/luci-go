...
func (g *Graph) AddNode(k *Key, props *starlark.Dict, trace *builtins.CapturedStacktrace) (*Node, error) {
	if k.set != &g.KeySet {
		return nil, fmt.Errorf("got a key %s from another graph", k)
	}
	if n, ok := g.nodes[k]; ok {
		return nil, &RedeclarationError{Key: k, Trace: trace, Previous: n.Trace}
	}
	if g.frozen {
		return nil, fmt.Errorf("cannot add node to a frozen graph")
	}

	// Make sure all keys in 'props' are strings, otherwise FromKeywords below
	// may panic.
	for _, k := range props.Keys() {
		if _, ok := k.(starlark.String); !ok {
			return nil, fmt.Errorf("non-string key %s in 'props'", k)
		}
	}
