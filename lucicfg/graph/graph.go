...
// EdgeRedeclarationError is returned when adding an existing edge.
//
...
// CycleError is returned when adding an edge that introduces a dependency
// cycle.
//
...
// A predeclared node is moved to the fully declared state by AddNode.
//
...
func (g *Graph) AddEdge(parent, child *Key, title string, trace *builtins.CapturedStacktrace) error {
	...
	if err := g.validateKey(parent); err != nil {
		return err
	}
	if err := g.validateKey(child); err != nil {
		return err
	}
	...
	err := edge.Child.visitDescendants(func(n *Node, path []*Edge) error {
		if n == edge.Parent {
			return &CycleError{
				Trace: trace,
				Edge:  edge,
