...
var ErrNotFinalized = errors.New("cannot query a graph under construction")
...
func (e *NodeRedeclarationError) Backtrace() string {
	return e.Trace.String() + "Error: " + e.Error()
}
...
func (e *DanglingEdgeError) Backtrace() string {
	return e.Edge.Trace.String() + "Error: " + e.Error()
}
...
// it.
//
// Once the construction is complete, the graph can be finalized via Finalize()
// call, which will check there are no dangling edges. Finalizing a graph
// makes it immutable (so AddNode/AddEdge return errors), and queriable.
//
...
// happen only once.
//
// Panics if the graph if finalized.
func (g *Graph) initNode(k *Key) *Node {
	if g.finalized {
		panic("the graph is finalized")
	}
...
func (g *Graph) AddNode(k *Key, props *starlark.Dict, trace *builtins.CapturedStacktrace) (*Node, error) {
	if g.finalized {
		return nil, fmt.Errorf("cannot add a node to a finalized graph")
	}
...
func (g *Graph) AddEdge(parent, child *Key, title string, trace *builtins.CapturedStacktrace) error {
	if g.finalized {
		return fmt.Errorf("cannot add an edge to a finalized graph")
	}
...
//
// Once finalized, the graph becomes queriably.
