...
//
// Assumes the graph have no cycles (as verified by AddEdge).
func (n *Node) visitDescendants(cb func(*Node, []*Edge) error, path []*Edge) error {
	if err := cb(n, path); err != nil {
		return err
	}
	for _, e := range n.children {
		if err := e.Child.visitDescendants(cb, append(path, e)); err != nil {
			return err
		}
	}
	return nil
