...
// Note: this package is tested entirely struct Starlark tests, in particular
...
//
// It exposes OpenScope/CloseScope that should be connected PreExec/PostExec
...
// scope hold values of variables that were assigned within an exec-ing module.
type scope struct {
	values map[ID]varValue
	thread *starlark.Thread // just for asserts
}

// varValue is a var value plus a stacktrace where it happened, for errors.
type varValue struct {
	value starlark.Value
	trace *builtins.CapturedStacktrace
}

// OpenScope starts a new scope for variables.
//
// All changes made within this scope are discarded when it is closed.
func (v *Vars) OpenScope(th *starlark.Thread) {
	v.scopes = append(v.scopes, &scope{thread: th})
}

// CloseScope discards all changes made to variables since previous OpenScope.
func (v *Vars) CloseScope(th *starlark.Thread) {
	...
	case v.scopes[len(v.scopes)-1].thread != th:
		// This check may fail if Interpreter stars using multiple goroutines to
		// execute 'exec's. If this ever happens, we'll need to move 'v.scopes' to
		// Starlark's TLS.
		panic("call to CloseScope with unexpected starlark thread")
	default:
		v.scopes = v.scopes[:len(v.scopes)-1]
	}
}

// ClearValues resets values of all variables, in all open scopes.
//
...
//
// If the variables is set returns (value, True, nil) or (None, False, nil)
// otherwise. Note that value can be None itself (if None was explicitly written
// to the variable via Set).
//
// Variable can be assigned or read only from threads that perform 'exec' calls,
// NOT when loading library modules via 'load' (the library modules must not
// have side effects or depend on a transient state in vars), or executing
...
// lookup finds the variables values in the current scope or any of parents.
//
// Used by both Set (to check that a var is unassigned) and Get (to read it).
