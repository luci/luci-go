...
//
// Returns an error if a variable with the same `exposeAs` name have already
// been exposed.
func (v *Vars) Declare(th *starlark.Thread, exposeAs string, presetVal starlark.Value) (ID, error) {
	...
	if exposeAs != "" {
		...
		// Put a preset value of the var (if it exposed as settable from outside) at
		// the bottom of the scope stack, to indicate it was set "before" Starlark
		// execution. v.lookup() will find it there, thus forbidding reassignments.
...
func (v *Vars) lookup(id ID) *varValue {
	...
	// there were declared (which may happen outside of EXEC context, so we can't
	// use regular Set there).
