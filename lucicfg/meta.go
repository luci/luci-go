...
// flags. Command line flags override what's in the Starlark.
//
...
type Meta struct {
	...
	// FlagSet passed to AddValidationFlags and AddOutputFlags.
	fs *flag.FlagSet
...
func (m *Meta) AddOutputFlags(fs *flag.FlagSet) {
	...
	fs.Var(luciflag.StringSlice(&m.TrackedFiles), "tracked-files", "Globs for files considered generated, see meta.config(...) doc for more info.")
}
...
// setFS assigns to m.fs.
func (m *Meta) setFS(fs *flag.FlagSet) {
	if m.fs == nil {
		m.fs = fs
	} else if m.fs != fs {
		panic("Add*Flags should receive same flag set")
	}
}
...
// this field inside 'm'.
//
// This used by both Starlark accessors and for processing CLI flags.
...
func (m *Meta) setField(k string, v starlark.Value) (err error) {
	ptr := m.fieldsMap()[k]
	if ptr == nil {
		return fmt.Errorf("set_meta: no such meta key %s", k)
	}
