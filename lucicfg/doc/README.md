...
lucicfg is using [buildifier] internally to format and lint Starlark code.
Buildifier is primarily indented for formatting bazel BUILD and \*.bzl files
with a standard convention, but it works with lucicfg's \*.star files reasonably
well too.
...
There are two ways to run lint checks:
  ...
  2. As part of `lucicfg validate <entry point>.star`. It will check only files
     ...
     [lucicfg.config(...)](#lucicfg.config), see below for examples.
...
-warn3, -warn4, ... `, where
  ...
  * `<initial set>` can be one of `none`, `default` or `all` and it
    identifies a set of linter checks to use as a base. `none` is an empty set,
    `all` is all checks known to buildifier (some of them may be incompatible
    with lucicfg Starlark code!) and `default` is a set of checks that are known
    to work well with lucicfg Starlark code. If `<initial set>` is omitted,
    `default` is used.
...
checks. Note that some of them are specific to Bazel and should never trigger
for lucicfg Starlark code.
...
doc nor can be disabled via `buildifier: disable=...`.
...
(Equivalent to running `lucicfg lint -checks default`).
...
(Equivalent to running `lucicfg lint -checks "none,+formatting"`).
...
lucicfg.config(
    ...
    lint_checks = ["default", "-function-docstring"],
)
```

(Equivalent to running `lucicfg lint -checks "default,-function-docstring"`).
