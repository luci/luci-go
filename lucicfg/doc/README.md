 ...
 ***
 ...
 Args:
   ...
   tree_status_host: a hostname of the project tree status app (if any) that
       ...
       CQ lands CLs. See `tree_status_host` in [luci.cq_group(...)](#luci.cq_group). Required.
...
a failing step matching this regex or lsit of regexes. May be combined
with `failed_step_regexp`, in which case it must also have a failed
step matching that regular expression.
   template: a [luci.notifier_template(...)](#luci.notifier_template) to use to format tree closure
