...
* **scheduler**: appspothostname of a LUCI Scheduler service to use (if any).
...
Defines configuration of the Milo service for this project.

Among other things, Milo service is a public user interface for displaying
...
A UI view that displays a list of builders.

Builders that belong to this view can be populated either right here:
...
Or one by one via separate [luci.list_view_entry(...)](#luci.list_view_entry) declarations:
...
Note that for the deprecated case of declaring Buildbot builders (or LUCI
builders with equivalent Buildbot builders) use of [luci.list_view_entry(...)](#luci.list_view_entry)
is required. It's the only way to provide a reference to a Buildbot builder.
...
* **repo**: URL of a git repository to display in the view rows, should start with `https://`.
* **favicon**: optional https URL to the favicon for this view, must be hosted on `storage.googleapis.com`. Defaults to `favicon` in lucicfg.milo(...).
* **entries**: a list of builders or [luci.list_view_entry(...)](#luci.list_view_entry) objects to include into this view.
...
* **builder**: a builder to add, see [luci.builder(...)](#luci.builder). Can be omitted for extra deprecated case of Buildbot-only views. `buildbot` filed must be set in this case.
...
* **buildbot**: a reference to an equivalent Buildbot builder, given as `<master>/<builder>` string. Deprecated. Exists only while there are still Buildbot builders, to aid in the migration.
