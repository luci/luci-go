...
luci.builder(...) and luci.gitiles_poller(...) rules have `schedule` field that
...
strings. Supported kinds of schedules (illustrated via examples):
  ...
  - `* 0 * * * *`: a crontab expression, in a syntax supported by
    ...
    **UTC clock**). Some examples:
    ...
    If when triggering a job, a previous invocation is still running, an
    overrun is recorded and the scheduled invocation is skipped. The next
  ...
  - `with 10s interval`: run the job in a loop, waiting 10s after finishing
     ...
     synchronized with wall clock at all.
  ...
  - `triggered` schedule indicates that the job is only started via some
    ...
    In luci.builder(...) this schedule is useful to make lucicfg setup a
    ...
    LUCI Scheduler API. In luci.gitiles_poller(...) this is useful to setup
