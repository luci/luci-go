...
interpreted as Golang [text templates]. Supported interpolations are the
...
Templates can be either defined inline as strings, or loaded from external
files via [io.read_file(...)](#io.read_file).
...
* **monorail_project**: optional Monorail project to file bugs into.
* **monorail_components**: a list of the Monorail component that the bug should exist under, in the hierarchical `>` separated format, e.g. `Infra>Client>ChromeOS>CI`. Required if `monorail_project` is set, otherwise must be None.
* **bug_summary**: a text template for generating new bug's summary given a builder on whose page the user clicked the feedback link. Must be None if `monorail_project` is unset.
* **bug_description**: a text template for generating new bug's description given a builder on whose page the user clicked the feedback link. Must be None if `monorail_project` is unset.
...
out to the left, while console groups are laid out on the right. Each oncall
