...
* **notifies**: list of [luci.notifier(...)](#luci.notifier) or [luci.tree_closer(...)](#luci.tree_closer) the builder notifies when it changes its status. This relation can also be defined via `notified_by` field in [luci.notifier(...)](#luci.notifier) and [luci.tree_closer(...)](#luci.tree_closer).
...
a `luci.notifiable` object, i.e. they both are something that "can be
notified" when a build finishes. They both are valid targets for `notifies`
field in [luci.builder(...)](#luci.builder). For that reason they share the same namespace,
i.e. it is not allowed to have a [luci.notifier(...)](#luci.notifier) and a
[luci.tree_closer(...)](#luci.tree_closer) with the same name.
...
Defines a template to use for notification sent by [luci.notifier(...)](#luci.notifier) and
...
* **name**: name of this template to reference it from [luci.notifier(...)](#luci.notifier) and [luci.tree_closer(...)](#luci.tree_closer) rules. Must match the regex `^[a-z][a-z0-9\_]*$`. Required.
...
The set of builders that are being observed is defined through `notified_by`
field here or `notifies` field in [luci.builder(...)](#luci.builder). Whenever a build
finishes, the builder "notifies" all (but usually just one)
[luci.tree_closer(...)](#luci.tree_closer) objects subscribed to it they can decide whether to
