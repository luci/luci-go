...
if overrides the value set via `core.meta` (if any).

`core.meta` may be called multiple times. The most recently set value is used
in the end, so think of `core.meta(var=...)` just as assigning to a variable.

#### Arguments {#core.meta-args}
...
* **fail_on_warnings**: True to treat validation warnings as errors.
