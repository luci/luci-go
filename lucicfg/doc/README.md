...
* **bq_table**: string of the form `<project>.<dataset>.<table>` where the parts respresent the BigQuery-enabled gcp project, dataset and table to export results.
* **predicate**: A predicate_pb.ArtifactPredicate() proto. If given, specifies the subset of text artifacts to export to the above table, instead of all. Use [resultdb.text_artifact_predicate(...)](#resultdb.text_artifact_predicate) to generate this, if needed.
...
### resultdb.text_artifact_predicate {#resultdb.text_artifact_predicate}

```python
resultdb.text_artifact_predicate(
    # Optional arguments.
...
#### Arguments {#resultdb.text_artifact_predicate-args}

* **test_result_predicate**: predicate_pb.TestResultPredicate(), a predicate of test results.
* **included_invocations**: bool, if false, invocation level artifacts will not be included.
* **test_results**: bool, if false, test result level artifacts will not be included.
* **content_type_regexp**: string, an artifact must have a content type matching this regular expression entirely, i.e. the expression is implicitly wrapped with ^ and $. Defaults to "text.*".


#### Returns  {#resultdb.text_artifact_predicate-returns}
