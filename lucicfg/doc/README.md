...
module where such defaults were declared. Supported fields and how their
explicitly passed values are combined with corresponding module-scoped defaults
(see [Specifying default values for all invocations of a rule](#module_defaults)
for more details):

| Field  | Merge semantics |
|--------|-----------------|
properties | merged on top. |
service_account | overrides. |
caches | joined. |
execution_timeout | overrides. |
dimensions | merged on top. |
priority | overrides. |
swarming_tags | joined. |
expiration_timeout | overrides. |
triggering_policy | overrides. |
build_numbers | overrides. |
experimental | overrides. |
task_template_canary_percentage | overrides. |
repo | overrides. |
luci_migration_host | overrides. |
