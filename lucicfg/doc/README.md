...
* **result_visibility**: can be used to restrict the visibility of the tryjob results in comments on Gerrit. Valid values are `cq.FULL_VISIBILITY` and `cq.RESTRICTED_VISIBILITY` constants.
...
values for `result_visibility` field of [luci.cq_group(...)](#luci.cq_group). See the proto for
the description.
  * **cq.FULL_VISIBILITY**: The CQ reports the summary markdown and a link to
    the buildbucket build id in Milo with the builder name in the URL in a
    Gerrit comment.
  * **cq.RESTRICTED_VISIBILITY**: The CQ reports a generic "Build failed:
