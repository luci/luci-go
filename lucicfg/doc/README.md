...
* **result_visibility**: can be used to restrict the visibility of the tryjob results in comments on Gerrit. Valid values are `cq.TRYJOB_RESULT_COMMENT_LEVEL_FULL` and `cq.TRYJOB_RESULT_COMMENT_LEVEL_RESTRICTED` constants. Default is to give full visibility: builder name and full summary markdown are including in the Gerrit comment.
...
`cq.TRYJOB_RESULT_COMMENT_LEVEL_*` constants define possible values for
`result_visibility` field of [luci.cq_group(...)](#luci.cq_group):
  * **cq.TRYJOB_RESULT_COMMENT_LEVEL_UNSET**: Equivalent to
    cq.TRYJOB_RESULT_COMMENT_LEVEL_FULL for now.
  * **cq.TRYJOB_RESULT_COMMENT_LEVEL_FULL**: The CQ reports the summary
    markdown and a link to the buildbucket build id in Milo with the builder
    name in the URL in a Gerrit comment.
  * **cq.TRYJOB_RESULT_COMMENT_LEVEL_RESTRICTED**: The CQ reports a generic
    "Build failed: https://ci.chromium.org/b/1234" with no summary markdown.
