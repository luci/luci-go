...
However, following restrictions apply:

* Most CQ features are not supported except for `location_regexp` and
`owner_whitelist`. If provided, they must meet following conditions:
...
may be provided.
* All analyzers should be declared in a single [luci.cq_group(...)](#luci.cq_group).
...
Note for migrate Tricium config in auxillary LUCI Projects
