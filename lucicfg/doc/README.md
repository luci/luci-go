...
* **monorail_project**: optional Monorail project to file bugs into when the user clicks the feedback link in a build page.
* **monorail_components**: a list of the Monorail component to assigned to a new bug, in the hierarchical `>`-separated format, e.g. `Infra>Client>ChromeOS>CI`. Required if `monorail_project` is set, otherwise must not be used.
* **bug_summary**: string with a text template for generating new bug's summary given a builder on whose page the user clicked the feedback link. Must not be used if `monorail_project` is unset.
* **bug_description**: string with a text template for generating new bug's description given a builder on whose page the user clicked the feedback link. Must not be used if `monorail_project` is unset.
