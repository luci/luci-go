...
field here or `notifies` field in [luci.builder(...)](#luci.builder). Whenever something
happens to a builder (e.g. a build fails), the builder "notifies" all
[luci.notifier(...)](#luci.notifier) objects subscribed to it, and each notifier in turn filters
the event and forwards it to corresponding recipients.

*** note
**Note:** Currently there can be at most one notifier assigned to a builder.
***
...
* **name**: name of this notifier to reference it from other rules.
...
* **notify_email**: an optional list of emails to send notifications to.
