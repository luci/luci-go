...
Buildifier is primarily indented for bazel BUILD and \*.bzl files, but it works
...
-warn3, -warn4, ... `, where
  ...
  * `<initial set>` can be one of **none**, **default** or **all** and it
    ...
    * **all** is all checks known to buildifier (some of them may be
...
checks. Note that many of them are specific to Bazel and should never trigger
for lucicfg Starlark code.

Additionally a check called `formatting` can be used to instruct lucicfg to
verify formatting of Starlark files. It is part of the **default** set. Note
that it is not a built-in buildifier check and thus it's not listed in the
buildifier docs nor can it be disabled via `buildifier: disable=...`.
...
This is equivalent to running `lucicfg lint -checks "default,-function-docstring"`.
