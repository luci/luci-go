...
* **resultdb_settings**: buildbucket_pb.Builder.ResultDB, A configuration that defines if Buildbucket:ResultDB integration should be enabled for this builder and which results to export to BigQuery, and to which project, dataset, and table.
...
```python
resultdb.settings(enable = None, export_test_results = None)
```
...
* **export_test_results**: list of resultdb_pb.BigQueryExport() protos, configurations for exporting specific subsets of test results to a designated BigQuery table, use [resultdb.export_test_results(...)](#resultdb.export_test_results) to create these.
...
* **predicate**: A predicate_pb.TestResultPredicate() proto. If given, specifies the subset of test results to export to the above table, instead of all.
...
### resultdb.predicate {#resultdb.predicate}
...
#### Arguments {#resultdb.predicate-args}
...
#### Returns  {#resultdb.predicate-returns}
...
Validates a ResultDB settings proto.

#### Arguments {#resultdb.validate_settings-args}

* **settings**: The protobuf populated [resultdb.settings(...)](#resultdb.settings)


#### Returns  {#resultdb.validate_settings-returns}

Validated proto, if it's the correct type.
