...
* **on_new_status**: a dict specifying which new build statuses to notify for. Notifes for each build status specified unless the previous build was the same status. Default is None.
* **on_occurrence**: a dict specifying which build statuses to notify for. Notifes for every build status specified. Default is None.
