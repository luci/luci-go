...
body can either specified inline right in the starlark script or loaded from
an external file via [io.read_file(...)](#io.read_file).
...
```python
# The actual email template using subtemplates defined below. In the real life
# it might be better to load such large template from an external file using
