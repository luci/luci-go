...
* **dest**: path to the output file, relative to the `config_dir` set bu [lucicfg.config(...)](#lucicfg.config). Must not start with `../`. Required.
...
* **config_dir**: a subdirectory of the config output directory to place generated LUCI configs under (see `config_dir` in [lucicfg.config(...)](#lucicfg.config)). Default is `.`. Useful when using `lucicfg` to generate LUCI and non-LUCI configs at the same time.
