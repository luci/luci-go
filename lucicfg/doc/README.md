...
`cq.MODE_*` constants define possible values for `additional_modes`
field of [luci.cq_group(...)](#luci.cq_group):
  * **cq.MODE_QUICK_DRY_RUN**: Only run partial tests in the CQ Run and do
    not submit.
...
* **name**: name of this mode. Must match regex "^[a-zA-Z][a-zA-Z0-9_-]{0,39}$".
* **cq_label_value**: the value of Commit-Queue label that MUST be set to when triggering a CQ Run in this mode.
* **triggering_label**: the Gerrit label that MUST also be set in order to trigger a CQ Run in this mode.
* **triggering_value**: tje value of the `triggering_label` that MUST be set to when triggering a CQ Run in this mode.
