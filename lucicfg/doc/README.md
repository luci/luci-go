...
Flags are registered in a subcommand only if the make sense for this
subcommand. For example, flags related to the remote config validation are not
exposed by inherently local subcommands.

`meta.config` may be called multiple times. The most recently set value is
used in the end, so think of `meta.config(var=...)` just as assigning to
a variable.

#### Arguments {#meta.config-args}

* **config_service_host**: a hostname of a LUCI Config Service to send validation requests to. Default is whatever hardcoded in `lucicfg` binary, usually `luci-config.appspot.com`.
...
* **config_dir**: a directory to place generated configs into, relative to the directory that contains the entry point \*.star script. ".." is allowed. The directory will be created if it is absent. Default is "generated".
* **tracked_files**: a list of globs patterns that define a subset of files under `config_dir` that are considered generated. This is important if some generated file disappears from `lucicfg` output: it must be deleted from the disk as well. To do this, `lucicfg` needs to know what files are safe to delete. Each entry is either `<glob pattern>` (a "positive" glob) or `!<glob pattern>` (a "negative" glob). A file under `config_dir` (or any of its subdirectories) is considered tracked if it matches any of the positive globs and none of the negative globs. For example, `tracked_files` for prod and dev projects co-hosted in the same directory may look like `['*.cfg', '!*-dev.cfg']` for prod and `['*-dev.cfg']` of dev. If `tracked_files` is empty, lucicfg will never delete any files. In this case it is responsibility of the caller to make sure no stale output remains.
* **fail_on_warnings**: True to treat validation warnings as errors. To override to False via command line flags use `-fail-on-warnings=false`.
