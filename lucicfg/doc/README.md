...
* **column_keys**: list of string keys that will be rendered as 'columns'. status is always the first column and name is always the last column (you don't need to specify them). A key must be one of the following: <ol>   <li>     'v.{variant_key}': variant.def[variant_key] of the test variant     (e.g. v.gpu).   </li> </ol> If None, defaults to [].
* **grouping_keys**: list of string keys that will be used for grouping tests. A key must be one of the following: <ol>   <li>'status': status of the test variant.</li>   <li>'name': name of the test variant.</li>   <li>     'v.{variant_key}': variant.def[variant_key] of the test variant     (e.g. v.gpu).   </li> </ol> If None, defaults to ['status']. Caveat: test variants with only expected results are not affected by   this setting and are always in their own group.
...
```python
resultdb.validate_test_presentation(attr, config = None)
```
