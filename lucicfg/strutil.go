...
func expandIntSet(s string) ([]string, error) {
	const (
		...
		COMMA_OR_MINUS // '{1|..4,5}' or '{1|,2}, expecting either ',' or '..', or '}'
	...
	addRange := func(l, r string) error {
		...
		ri, err := strconv.ParseUint(r, 10, 64)
		if err != nil {
			return fmt.Errorf("integer %q is too large", r)
		}

		// E.g. "5..2" is a bad range, should be "2..5".
		if li > ri {
			return fmt.Errorf("bad range - %d is larger than %d", li, ri)
		}
		...
		// If both strings have same length, use it as padding for the output.
		format := "%d"
		if len(l) == len(r) {
	...
	for _, tok := range tokenize(s) {
		...
		case COMMA_OR_MINUS:
			switch tok.typ {
			case TOK_COMMA:
				if err := addRange(rangeStart, rangeStart); err != nil {
					return nil, err
				}
				state = RANGE_START
			case TOK_RB:
...
type token struct {
	...
	val string // a single rune for TOK_RUNE, a sequence of digits for TOK_NUM
}
