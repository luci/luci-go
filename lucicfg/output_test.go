	...
	Convey("With temp dir", t, func() {
		...
		Convey("Works with protos", func() {
			...
			Convey("Ignores formatting", func() {
				...
				// If not forcing updates, correctly recognizes nothing has changed.
				changed, unchanged, err := out.Write(tmp, false)
				So(changed, ShouldHaveLength, 0)
				So(unchanged, ShouldResemble, []string{"m1", "m2"})
				So(err, ShouldBeNil)

				// Didn't touch it on disk.
				So(read("m2"), ShouldResemble, "i:     222")
			...
			Convey("Writes when detects real changes", func() {
				// Overwrite m2 with something semantically different.
				write("m2", "i: 333")

				// Detected and overwrote it.
				changed, unchanged, err := out.Write(tmp, false)
				So(changed, ShouldResemble, []string{"m2"})
				So(unchanged, ShouldResemble, []string{"m1"})
				So(err, ShouldBeNil)

				// Updated it on disk as well.
				So(read("m2"), ShouldResemble, "# Header\ni: 222\n")
			})

			Convey("Writes when detects bad proto", func() {
