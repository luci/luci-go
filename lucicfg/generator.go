...
func Generate(ctx context.Context, in Inputs) (*State, error) {
	...
	// Setup default proto loader, so *.star code can register descriptors there.
	// Hook up load("@proto//<path>", ...) to load proto modules through this
	// loader.
	...
	pkgs["proto"] = func(path string) (dict starlark.StringDict, _ string, err error) {
		...
		if err != nil {
			...
			// provided yet by *.star. If so, fallback to the built-in descriptor
			// sets.
			//
			// TODO(vadimsh): Get rid of this once all users provide descriptors
			// themselves.
			registerMisc.Do(func() {
				logging.Warningf(ctx, "DEPRECATED: Falling back to embedded proto descriptors to load %q", path)
