...
func init() {
	...
	declNative("enable_experiment", func(call nativeCall) (starlark.Value, error) {
		...
		if err := call.unpack(1, &list); err != nil {
			return nil, err
		}
		...
		for iter.Next(&x) {
			...
			if !ok {
				return nil, fmt.Errorf("enable_experiment: got %s, want string", x.Type())
			}
			if expID := str.GoString(); !call.State.experiments.Enable(expID) {
				logging.Warningf(call.Ctx, "enable_experiment: unknown experiment %q, registered experiments are %v",
					expID, call.State.experiments.Registered())
			}
		}
	...
	declNative("register_experiment", func(call nativeCall) (starlark.Value, error) {
		var expID starlark.String
		if err := call.unpack(1, &expID); err != nil {
			return nil, err
		}
		call.State.experiments.Register(expID.GoString())
		return starlark.None, nil
	})
	...
	declNative("is_experiment_enabled", func(call nativeCall) (starlark.Value, error) {
		...
		return starlark.Bool(call.State.experiments.IsEnabled(expID.GoString())), nil
	})
}
