...
// RuleSet is a helper for building Validator from a set of rules: each rules
// specifies a pattern for config set and files, and a validation function
// to apply to them.
//
...
// before the actual validation starts. The value of the placeholder is injected
// into the pattern string as is. So for example if the pattern is 'regex:...',
// the placeholder value can be a chunk of regexp.
//
// It is assumed the returned value doesn't change (in fact, it is getting
// cached after the fist call).
//
// The primary use case for this mechanism is too allow to register rule
// patterns that depend on not-yet know values during init() time.
//
...
// Add registers a validator for given configSet and path patterns.
//
...
func (r *RuleSet) Add(configSet, path string, cb Func) {
	...
	// Validate the pattern syntax by rendering them with some fake variable
	// values.
...
func (r *RuleSet) renderedConfigPattern(c context.Context, rule *rule) (*ConfigPattern, error) {
	...
	if err != nil {
		return nil, errors.Annotate(err, "failed to compiled config set pattern %q", rule.configSet).Err()
	}
	path, err := renderPatternString(rule.path, sub)
	if err != nil {
		return nil, errors.Annotate(err, "failed to compiled path pattern %q", rule.path).Err()
	}
