...
type PermissionsConfig struct {
	...
	// A union of all attributes across all permissions.
	//
	// Deprecated: Marked as deprecated in go.chromium.org/luci/auth_service/api/configspb/config.proto.
	Attribute []string `protobuf:"bytes,2,rep,name=attribute,proto3" json:"attribute,omitempty"`
	// All permissions across all defined roles.
	Permission    []*protocol.Permission `protobuf:"bytes,3,rep,name=permission,proto3" json:"permission,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *PermissionsConfig) GetPermission() []*protocol.Permission {
	if x != nil {
		return x.Permission
	}
	return nil
...
// another role, you can specify this role by name.
type PermissionsConfig_Role struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Identifier for a role, will have the form "role/<service>.<noun>". The
	// noun usually ends with "-er".
	//
	// Examples:
	//
	...
	//
	// Service name "luci.internal" is used for internal roles that are
	// forbidden to be referenced from project's realms.cfg.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// List of permissions associated with this role, same permission may not be
	// declared more than once per role.
	//
	// Only "name" is required and allowed. Other fields must be empty (their
	// values will be taken from "permission" list in the top-level
	// PermissionsConfig message). We use full Permission message here (instead
	// of a string) for historical reasons.
	Permissions []*protocol.Permission `protobuf:"bytes,2,rep,name=permissions,proto3" json:"permissions,omitempty"`
	// List of other roles to include in this role.
	//
	// All their permission will be added to the current role's permissions.
	Includes      []string `protobuf:"bytes,3,rep,name=includes,proto3" json:"includes,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
const file_go_chromium_org_luci_auth_service_api_configspb_config_proto_rawDesc = "" +
	...
	"\x10token_server_url\x18\x04 \x01(\tR\x0etokenServerUrl\"\xb1\x02\n" +
	"\x11PermissionsConfig\x128\n" +
	"\x04role\x18\x01 \x03(\v2$.auth.configs.PermissionsConfig.RoleR\x04role\x12 \n" +
	"\tattribute\x18\x02 \x03(\tB\x02\x18\x01R\tattribute\x12B\n" +
	"\n" +
	"permission\x18\x03 \x03(\v2\".components.auth.realms.PermissionR\n" +
	"permission\x1a|\n" +
	"\x04Role\x12\x12\n" +
...
var file_go_chromium_org_luci_auth_service_api_configspb_config_proto_depIdxs = []int32{
	...
	8, // 3: auth.configs.PermissionsConfig.permission:type_name -> components.auth.realms.Permission
	8, // 4: auth.configs.PermissionsConfig.Role.permissions:type_name -> components.auth.realms.Permission
