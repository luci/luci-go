...
func TestGetAuthGroup(t *testing.T) {
	...
	ftt.Run("Testing GetAuthGroup", t, func(t *ftt.Test) {
		...
		t.Run("handles empty name", func(t *ftt.Test) {
			...
			assert.Loosely(t, err, should.ErrLike(customerrors.ErrInvalidName))
...
func TestCreateAuthGroup(t *testing.T) {
	...
	ftt.Run("CreateAuthGroup", t, func(t *ftt.Test) {
		...
		t.Run("empty group name", func(t *ftt.Test) {
			...
			assert.Loosely(t, err, should.Equal(customerrors.ErrInvalidName))
		})
		...
		t.Run("group name that already exists", func(t *ftt.Test) {
			...
			_, err := CreateAuthGroup(ctx, group, "Go pRPC API")
			assert.Loosely(t, err, should.Equal(customerrors.ErrAlreadyExists))
		})
		...
		t.Run("invalid member identities", func(t *ftt.Test) {
			...
			_, err := CreateAuthGroup(ctx, group, "Go pRPC API")
			assert.Loosely(t, err, should.ErrLike(customerrors.ErrInvalidIdentity))
		...
		t.Run("with empty owners uses 'administrators' group", func(t *ftt.Test) {
			...
			createdGroup, err := CreateAuthGroup(ctx, group, "Go pRPC API")
			assert.Loosely(t, err, should.BeNil)
		...
		t.Run("updates AuthDB revision only on successful write", func(t *ftt.Test) {
			// Create a group.
			{
				...
				createdGroup1, err := CreateAuthGroup(ctx, group1, "Go pRPC API")
				assert.Loosely(t, err, should.BeNil)
			...
			// Create a second group.
			{
				...
				createdGroup2, err := CreateAuthGroup(ctx, group2, "Go pRPC API")
				assert.Loosely(t, err, should.BeNil)
			...
			{
				_, err := CreateAuthGroup(ctx, emptyAuthGroup(ctx, "foo2"), "Go pRPC API")
		...
		t.Run("creates historical group entities", func(t *ftt.Test) {
			// Create a group.
			{
				...
				_, err := CreateAuthGroup(ctx, group, "test historical comment")
				assert.Loosely(t, err, should.BeNil)
...
func TestUpdateAuthGroup(t *testing.T) {
	...
	ftt.Run("UpdateAuthGroup", t, func(t *ftt.Test) {
		...
		t.Run("can't update external group", func(t *ftt.Test) {
			...
			_, err := UpdateAuthGroup(ctx, group, nil, etag, "Go pRPC API")
		...
		t.Run("extra validation is completed for admin group", func(t *ftt.Test) {
			...
			t.Run("can't change admin owners", func(t *ftt.Test) {
				...
				_, err := UpdateAuthGroup(ctx, adminAuthGroup, nil, etag, "Go pRPC API")
		...
		t.Run("can't delete if etag doesn't match", func(t *ftt.Test) {
			assert.Loosely(t, datastore.Put(ctx, group), should.BeNil)
			_, err := UpdateAuthGroup(ctx, group, nil, "bad-etag", "Go pRPC API")
			assert.Loosely(t, err, should.ErrLike(customerrors.ErrConcurrentModification))
			assert.Loosely(t, taskScheduler.Tasks(), should.BeEmpty)
		})
		...
		t.Run("invalid member identities", func(t *ftt.Test) {
			...
			_, err := UpdateAuthGroup(ctx, group, &fieldmaskpb.FieldMask{Paths: []string{"members"}}, etag, "Go pRPC API")
			assert.Loosely(t, err, should.ErrLike(customerrors.ErrInvalidIdentity))
			assert.Loosely(t, err, should.ErrLike("bad identity string \"no-prefix@google.com\""))
			assert.Loosely(t, taskScheduler.Tasks(), should.BeEmpty)
		})
		...
		t.Run("project member identities", func(t *ftt.Test) {
			...
			updatedGroup, err := UpdateAuthGroup(ctx, group, &fieldmaskpb.FieldMask{Paths: []string{"members"}}, etag, "Go pRPC API")
			assert.Loosely(t, err, should.BeNil)
		...
		t.Run("invalid identity globs", func(t *ftt.Test) {
			...
			_, err := UpdateAuthGroup(ctx, group, &fieldmaskpb.FieldMask{Paths: []string{"globs"}}, etag, "Go pRPC API")
			assert.Loosely(t, err, should.ErrLike(customerrors.ErrInvalidIdentity))
			assert.Loosely(t, err, should.ErrLike("bad identity glob string \"*@no-prefix.com\""))
			assert.Loosely(t, taskScheduler.Tasks(), should.BeEmpty)
		})
		...
		t.Run("project identity globs", func(t *ftt.Test) {
			...
			updatedGroup, err := UpdateAuthGroup(ctx, group, &fieldmaskpb.FieldMask{Paths: []string{"globs"}}, etag, "Go pRPC API")
			assert.Loosely(t, err, should.BeNil)
		...
		t.Run("all nested groups must exist", func(t *ftt.Test) {
			...
			_, err := UpdateAuthGroup(ctx, group, &fieldmaskpb.FieldMask{Paths: []string{"nested"}}, etag, "Go pRPC API")
			assert.Loosely(t, err, should.ErrLike(customerrors.ErrInvalidReference))
		...
		t.Run("owner must exist", func(t *ftt.Test) {
			...
			_, err := UpdateAuthGroup(ctx, group, &fieldmaskpb.FieldMask{Paths: []string{"owners"}}, etag, "Go pRPC API")
			assert.Loosely(t, err, should.ErrLike("bar"))
			assert.Loosely(t, taskScheduler.Tasks(), should.BeEmpty)
		})
		...
		t.Run("with empty owners uses 'administrators' group", func(t *ftt.Test) {
			...
			updatedGroup, err := UpdateAuthGroup(ctx, group, &fieldmaskpb.FieldMask{Paths: []string{"owners"}}, etag, "Go pRPC API")
			assert.Loosely(t, err, should.BeNil)
		...
		t.Run("skips commit if no changes", func(t *ftt.Test) {
			...
			updatedGroup, err := UpdateAuthGroup(ctx, group, nil, etag, "Go pRPC API")
			assert.Loosely(t, err, should.BeNil)
		...
		t.Run("cyclic dependencies", func(t *ftt.Test) {
			...
			t.Run("self-reference", func(t *ftt.Test) {
				...
				_, err := UpdateAuthGroup(ctx, a, &fieldmaskpb.FieldMask{Paths: []string{"nested"}}, "", "Go pRPC API")
			...
			t.Run("cycle of length 2", func(t *ftt.Test) {
				...
				_, err := UpdateAuthGroup(ctx, b2, &fieldmaskpb.FieldMask{Paths: []string{"nested"}}, "", "Go pRPC API")
			...
			t.Run("cycle of length 3", func(t *ftt.Test) {
				...
				_, err := UpdateAuthGroup(ctx, c, &fieldmaskpb.FieldMask{Paths: []string{"nested"}}, "", "Go pRPC API")
...
func TestDeleteAuthGroup(t *testing.T) {
	...
	ftt.Run("DeleteAuthGroup", t, func(t *ftt.Test) {
		...
		t.Run("can't delete the admin group", func(t *ftt.Test) {
			err := DeleteAuthGroup(ctx, AdminGroup, "", "Go pRPC API")
			assert.Loosely(t, err, should.Equal(customerrors.ErrPermissionDenied))
		})
		...
		t.Run("can't delete external group", func(t *ftt.Test) {
			group.ID = "mdb/foo"
			assert.Loosely(t, datastore.Put(ctx, group), should.BeNil)
			err := DeleteAuthGroup(ctx, group.ID, "", "Go pRPC API")
		...
		t.Run("can't delete if etag doesn't match", func(t *ftt.Test) {
			assert.Loosely(t, datastore.Put(ctx, group), should.BeNil)
			err := DeleteAuthGroup(ctx, group.ID, "bad-etag", "Go pRPC API")
			assert.Loosely(t, err, should.ErrLike(customerrors.ErrConcurrentModification))
		})

		t.Run("group name that doesn't exist", func(t *ftt.Test) {
			err := DeleteAuthGroup(ctx, "non-existent-group", "", "Go pRPC API")
			assert.Loosely(t, err, should.Equal(datastore.ErrNoSuchEntity))
		})
		...
		t.Run("can't delete if group owns another group", func(t *ftt.Test) {
			...
			err := DeleteAuthGroup(ctx, group.ID, "", "Go pRPC API")
			assert.Loosely(t, err, should.ErrLike(customerrors.ErrReferencedEntity))
	...
	ftt.Run("Admin group validation works", t, func(t *ftt.Test) {
		...
		t.Run("must be self-owned", func(t *ftt.Test) {
			...
			assert.ErrIsLike(t, validateAdminGroup(ctx, g),
				customerrors.ErrInvalidArgument)
		...
		t.Run("cannot have globs", func(t *ftt.Test) {
			...
			assert.ErrIsLike(t, validateAdminGroup(ctx, g), customerrors.ErrInvalidArgument)
			assert.ErrIsLike(t, validateAdminGroup(ctx, g),
				"cannot have globs")
		})
