...
// Creates a new pRPC client with auth for the buildbucket server.
func NewClient(c context.Context, host string) (*prpc.Client, error) {
...
// a set of buckets.
func BucketPermissions(c context.Context, client *prpc.Client, buckets []string) (Permissions, error) {
	accessClient := access.NewAccessPRPCClient(client)
	...
	resp, err := accessClient.PermittedActions(c, &req)
	if err != nil {
		return nil, err
	}
