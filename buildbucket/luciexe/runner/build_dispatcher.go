...
type dispatchBuild struct {
	build *pb.Build

	// should be set to true for the very last build to process
	final bool
	// if `final`, is set to the error (if any) associated with processing this build
	bbError error
}
...
func mkBuildbucketBuffer(ctx context.Context, rawCB updateBuildCB) dispatcher.Channel {
	ret, err := dispatcher.NewChannel(ctx, &dispatcher.Options{
		...
		if data.Meta == nil {
			...
			if db.final {
				...
				// the build can fail.
				switch {
				case !protoutil.IsEnded(db.build.Status):
					panic(errors.Reason("build status %q is not final", db.build.Status).Err())
				case db.build.Status != pb.Status_SUCCESS:
		...
		db.bbError = transient.Tag.Apply(err)
		return db.bbError
	})
	if err != nil {
...
func mkLogdogBuffer(ctx context.Context, butler *runnerbutler.Server) (dispatcher.Channel, func()) {
	...
	ret, err := dispatcher.NewChannel(ctx, &dispatcher.Options{
		...
		if data.Meta == nil { // cache the assembled proto on Meta
			if bytes, err = proto.Marshal(db.build); err != nil {
				return err
			}
			data.Meta = bytes
		} else {
...
func runBuildDispatcher(ctx context.Context, butler *runnerbutler.Server, rawCB updateBuildCB) (buildChan chan<- *dispatchBuild, closeWait func(context.Context)) {
	input := make(chan *dispatchBuild)

	buildbucketBuf := mkBuildbucketBuffer(ctx, rawCB)
	...
	return input, func(ctx context.Context) {
		close(input)
