...
type runner struct {
	// UpdateBuild is called with the latest state of the build and the list
	// field paths that have changes.
	// Should return GRPC errors, e.g. status.Errorf. The errors MAY be wrapped
...
// latest state of build. r.UpdateBuild calls are sequential.
//
