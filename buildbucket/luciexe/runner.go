...
func setupSystemAuth(ctx context.Context, args *pb.RunnerArgs) (*auth.Authenticator, error) {
	...
	// The list of scopes we need is static, it depends only on what calls luciexe
	// itself is making.
	authOpts := infraenv.DefaultAuthOptions()
	authOpts.Scopes = []string{
		"https://www.googleapis.com/auth/cloud-platform", // for Logdog
		"https://www.googleapis.com/auth/userinfo.email", // for Buildbucket
	}

	a := auth.NewAuthenticator(ctx, auth.SilentLogin, authOpts)

	// When running locally this verifies we actually have the cached refresh
	// token.
	email, err := a.GetEmail()
	if err != nil {
		return nil, errors.Annotate(err, "failed to get email of system account").Err()
	}
	logging.Infof(ctx, "luciexe is running as %q", email)
	return a, nil
}
