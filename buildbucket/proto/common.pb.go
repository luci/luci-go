...
type Executable struct {
	...
	//
	// The 0th argument is taken as relative to the cipd_package root, so to
	// invoke the binary called "foo" in the root of the package, it is just
	// "foo". On Windows, this will automatically look first for "foo.exe"
	// followed by "foo.bat". Similarly, "subdir/foo" would expect the foo binary
	// to live in the top level "subdir" within the CIPD package.
	//
	...
	// field for passing task-specific data. Not only are the properties
	// JSON-typed, they can be modeled with a protobuf (using JSON PB encoding).
	// However, supplying additional arguments can occasionally be useful to, for
	// example, increase logging verbosity, or similar 'system level' settings
	// within the binary.
	//
	// Optional. If omitted, this defaults to `['luciexe']` (i.e. invoke the
	// binary 'luciexe' at the top of the CIPD pacakge, with no additional
	// arguments).
