...
type Step struct {
	...
	// A parent step MUST exist and MUST precede this step in the list of steps.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// The timestamp when the step started.
	// Required if status is STARTED, SUCCESS or FAILURE, or if the step has
	// children.
	// MUST NOT be after start_time of any of the children.
	StartTime *timestamp.Timestamp `protobuf:"bytes,2,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	// The timestamp when the step ended.
	// Required if status is terminal.
	// MUST NOT be before end_time of any of the children.
	...
	//   MUST be STARTED if status of any child is not terminal.
	//   MUST NOT be "better" than statuses of its children,
	//   where "better" relation is defined by the following order from good to bad:
	//     SUCCESS
	//     FAILURE
	//     INFRA_FAILURE
	//     CANCELED
	//   Note that this defines "better" relation only for some statuses.
	//   For those statuses where "better" is not defined, this rule does not apply.
...
func (*Step) Descriptor() ([]byte, []int) {
	return fileDescriptor_step_1bfaa9d06188aa97, []int{0}
}
...
func (*Step_Log) Descriptor() ([]byte, []int) {
	return fileDescriptor_step_1bfaa9d06188aa97, []int{0, 0}
}
...
func init() {
	proto.RegisterFile("go.chromium.org/luci/buildbucket/proto/step.proto", fileDescriptor_step_1bfaa9d06188aa97)
}

var fileDescriptor_step_1bfaa9d06188aa97 = []byte{
