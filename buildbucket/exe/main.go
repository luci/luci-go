...
func parseArgs(args []string) (*pb.ExecutorArgs, error) {
	fs := flag.FlagSet{}
	argsB64 := fs.String("args-b64gz", "", text.Doc(`
		(standard raw, unpadded base64)-encoded,
		zlib-compressed,
		binary buildbucket.v2.ExecutorArgs protobuf message.
	`))
...
// ExecutorMain runs LUCI executor, a program that runs a LUCI executable.
