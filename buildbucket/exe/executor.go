...
// executor runs a LUCI executable.
type executor struct {
	args         *pb.ExecutorArgs
	localLogFile string
}

// Run runs a user executable.
func (e *executor) Run(ctx context.Context, args *pb.ExecutorArgs) (*pb.Build, error) {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()

	// Validate and normalize parameters.
	args = proto.Clone(args).(*pb.ExecutorArgs)
	...
	logging.Infof(ctx, "ExecutorArgs: %s", argsJSON)

	// Prepare workdir.
	if err := e.setupWorkDir(args.WorkDir); err != nil {
		return nil, err
	}
	...
	logdogServ, err := e.startLogDog(ctx, args, systemAuth, listener)
	if err != nil {
	...
	err = e.runUserExecutable(ctx, args, userAuth, logdogServ, streamNamePrefix)
	if err != nil {
		return nil, err
	}
...
func (e *executor) setupWorkDir(workDir string) error {
...
func (e *executor) runUserExecutable(ctx context.Context, args *pb.ExecutorArgs, userAuth *authctx.Context, logdogServ *logdogServer, logdogNamespace string) (err error) {
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()
	...
	// Prepare user env.
	env, err := e.setupUserEnv(ctx, args, userAuth, logdogServ, logdogNamespace)
	if err != nil {
		return err
	}
	cmd.Env = env.Sorted()
	...
	if err := e.hookStdoutStderr(ctx, logdogServ, stdout, stderr, streamNamePrefix); err != nil {
		return err
	}
...
func (e *executor) setupUserEnv(ctx context.Context, args *pb.ExecutorArgs, userAuth *authctx.Context, logdogServ *logdogServer, logdogNamespace string) (environ.Env, error) {
	ret := environ.System()
	...
	// like to remove everything they find under TEMPDIR, and it breaks executor
...
func (e *executor) hookStdoutStderr(ctx context.Context, logdogServ *logdogServer, stdout, stderr io.ReadCloser, streamNamePrefix string) error {
...
// provided via ExecutorArgs.luci_system_account.
//
// The user context is used for actually running the user executable. It is the
// context executor starts with by default. On Swarming this will be the
// context associated with service account specified in the Swarming task
// definition.
func setupAuth(ctx context.Context, args *pb.ExecutorArgs) (system, user *authctx.Context, err error) {
	authArgs := args.GetAuth()

	// Construct authentication option with the set of scopes to be used through
	// out executor. This is superset of all scopes we might need. It is more
	...
	// parts of executor (LogDog client, Devshell proxy, etc).
	//
	...
	// one). Happens when running executor manually locally.
...
func normalizeArgs(a *pb.ExecutorArgs) error {
	switch {
...
func (e *executor) startLogDog(ctx context.Context, args *pb.ExecutorArgs, systemAuth *authctx.Context, listener *buildListener) (*logdogServer, error) {
...
func globalLogTags(args *pb.ExecutorArgs) map[string]string {
