...
// stepConverterFromURL returns stepConverter given an annotation URL as from BuildRunResult.
...
// convertBuildSteps converts a build via the root step, which contains only
// substeps, which are the actual steps of the build. This is the only function
// that should be used to convert annotation steps to Buildbucket steps.
// The provided context is used only for logging.
func (p *stepConverter) ConvertBuildSteps(c context.Context, ann *annotpb.Step) ([]*buildbucketpb.Step, error) {
	return p.convertSubsteps(c, ann.Substep, "")
}

// convertSubsteps converts substeps, which we expect to be Steps, not Logdog
// annotation streams.
func (p *stepConverter) convertSubsteps(c context.Context, annSubsteps []*annotpb.Step_Substep, stepPrefix string) ([]*buildbucketpb.Step, error) {
	var bbSteps, bbSubsteps []*buildbucketpb.Step
	var annSubstep *annotpb.Step
	var err error
	for _, annStep := range annSubsteps {
		annSubstep = annStep.GetStep()
		if annSubstep == nil {
			return nil, errors.Reason("unexpected non-Step substep %v", annStep).Err()
		}

		bbSubsteps, err = p.convertSteps(c, annSubstep, stepPrefix)
		if err != nil {
			return nil, err
		}
...
func (p *stepConverter) convertSteps(c context.Context, ann *annotpb.Step, stepPrefix string) ([]*buildbucketpb.Step, error) {
	...
	// Handle links.
	links, lines := p.convertLinks(c, ann)
	bb.Logs = links
	if len(lines) > 0 {
	...
	substeps, err := p.convertSubsteps(c, ann.Substep, bb.Name + StepSep)
	if err != nil {
		return nil, err
	}
...
func (p *stepConverter) convertLinks(c context.Context, ann *annotpb.Step) ([]*buildbucketpb.Step_Log, []string) {
	...
	bbLinks := make([]*buildbucketpb.Step_Log, 0, len(aLinks))
