...
//   * Link.isolate_object,
//   * Link.dm_link.
func ConvertBuildSteps(c context.Context, annSteps []*annotpb.Step_Substep, annAddr *types.StreamAddr) ([]*bp.Step, error) {
	prefix, _ := annAddr.Path.Split()
	sc := &stepConverter{
		defaultLogdogHost:   annAddr.Host,
		defaultLogdogPrefix: fmt.Sprintf("%s/%s", annAddr.Project, prefix),
		steps:               map[string]*bp.Step{},
	}
...
// annotation streams.
func (p *stepConverter) convertSubsteps(c context.Context, bbSteps *[]*bp.Step, annSubsteps []*annotpb.Step_Substep, stepPrefix string) ([]*bp.Step, error) {
	bbSubsteps := make([]*bp.Step, 0, len(annSubsteps))
...
// Mutates p.steps.
func (p *stepConverter) convertSteps(c context.Context, bbSteps *[]*bp.Step, ann *annotpb.Step, stepPrefix string) (*bp.Step, error) {
...
var statusPrecedence = map[bp.Status]int{
	bp.Status_CANCELED:      0,
	bp.Status_INFRA_FAILURE: 1,
	bp.Status_FAILURE:       2,
	bp.Status_SUCCESS:       3,
}

func (p *stepConverter) convertLinks(c context.Context, ann *annotpb.Step) ([]*bp.Step_Log, []string) {
	...
	bbLogs := make([]*bp.Step_Log, 0, len(aLinks))
