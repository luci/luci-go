...
func validateSearch(req *pb.SearchBuildsRequest) error {
	...
	if c := pr.GetOutputGitilesCommit(); c != nil {
		...
		case c.Id != "":
			...
			case !sha1Regex.MatchString(c.Id):
				return appstatus.Errorf(codes.InvalidArgument, "output_gitiles_commit.id must match %q", sha1Regex)
			}
		case c.Ref != "":
	...
	if pr.GetBuild() != nil && pr.CreateTime != nil {
		return appstatus.Errorf(codes.InvalidArgument, "build is mutually exclusive with create_time")
	}
