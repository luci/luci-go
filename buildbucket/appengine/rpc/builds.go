...
func logAndReturnUnimplemented(ctx context.Context, methodName string, rsp proto.Message, err error) error {
	// Log any error without an explicit grpc return code,
	// attaching codes.Internal to hide it from the caller.
	if _, ok := appstatus.Get(err); !ok {
		errors.Log(ctx, err)
		err = appstatus.Attachf(err, codes.Internal, "internal server error")
	}
