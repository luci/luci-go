...
func validateBuilderID(b *pb.BuilderID) error {
	...
	case !projRegex.MatchString(b.Project):
		return appstatus.Errorf(codes.InvalidArgument, "builder.project must match %q", projRegex)
	case b.Bucket != "" && !bucketRegex.MatchString(b.Bucket):
		return appstatus.Errorf(codes.InvalidArgument, "builder.bucket must match %q", bucketRegex)
	case b.Builder != "" && !builderRegex.MatchString(b.Builder):
		return appstatus.Errorf(codes.InvalidArgument, "builder.builder must match %q", builderRegex)
