...
func setDimensions(req *pb.ScheduleBuildRequest, cfg *pb.Builder, build *model.Build) {
	...
	for _, d := range dims {
		build.Proto.Infra.Swarming.TaskDimensions = append(build.Proto.Infra.Swarming.TaskDimensions, d...)
	}
	sort.Slice(build.Proto.Infra.Swarming.TaskDimensions, func(i, j int) bool {
		if build.Proto.Infra.Swarming.TaskDimensions[i].Key == build.Proto.Infra.Swarming.TaskDimensions[j].Key {
			return build.Proto.Infra.Swarming.TaskDimensions[i].Expiration.GetSeconds() < build.Proto.Infra.Swarming.TaskDimensions[j].Expiration.GetSeconds()
		}
		return build.Proto.Infra.Swarming.TaskDimensions[i].Key < build.Proto.Infra.Swarming.TaskDimensions[j].Key
	})
}
