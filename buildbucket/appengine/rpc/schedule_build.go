...
func setInput(req *pb.ScheduleBuildRequest, cfg *pb.Builder, build *model.Build) {
	...
	if cfg.GetRecipe() != nil {
		build.Proto.Input.Properties.Fields = make(map[string]*structpb.Value, len(cfg.Recipe.Properties))
		for k, v := range strpair.ParseMap(cfg.Recipe.Properties) {
			build.Proto.Input.Properties.Fields[k] = &structpb.Value{
				Kind: &structpb.Value_StringValue{
					// Keys should already be validated as having unique values.
					StringValue: v[0],
				},
			}
		}
	...
	if req.GetGitilesCommit() != nil {
		build.Proto.Input.GitilesCommit = req.GitilesCommit
	}

	if req.GetGerritChanges() != nil {
		build.Proto.Input.GerritChanges = req.GerritChanges
	}
}
