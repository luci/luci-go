...
func validateRequestedDimension(dim *pb.RequestedDimension) error {
	...
	case dim.Key == "caches":
		return errors.Annotate(errors.Reason("caches must not be specified").Err(), "key").Err()
	case dim.Key == "pool":
		return errors.Annotate(errors.Reason("pool must not be specified").Err(), "key").Err()
	case dim.Value == "":
