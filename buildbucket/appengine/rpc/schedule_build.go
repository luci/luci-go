		...
		for name, blds := range seq {
			...
			work <- func() error {
				...
				for i, b := range blds {
					...
					b.Tags = append(b.Tags, fmt.Sprintf("build_address:luci.%s.%s/%s/%d", b.Proto.Builder.Project, b.Proto.Builder.Bucket, b.Proto.Builder.Builder, b.Proto.Number))
					sort.Strings(b.Tags)
				}
				return nil
			}
...
func scheduleBuilds(ctx context.Context, reqs ...*pb.ScheduleBuildRequest) ([]*model.Build, error) {
	...
	for i := range blds {
		...
		if cfgs[fmt.Sprintf("%s/%s", blds[i].Proto.Builder.Project, blds[i].Proto.Builder.Bucket)][blds[i].Proto.Builder.Builder].BuildNumbers == pb.Toggle_YES {
