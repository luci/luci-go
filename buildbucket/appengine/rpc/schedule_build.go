...
func validateSchedule(req *pb.ScheduleBuildRequest) error {
	...
	case req.Builder != nil && teeErr(protoutil.ValidateRequiredBuilderID(req.Builder), &err) != nil:
		return err
	case req.Exe.GetCipdPackage() != "":
		return errors.Annotate(errors.Reason("cipd_package must not be specified").Err(), "exe").Err()
	case req.Exe.GetCipdVersion() != "" && teeErr(common.ValidateInstanceVersion(req.Exe.CipdVersion), &err) != nil:
		return errors.Annotate(errors.Annotate(err, "invalid cipd_version").Err(), "exe").Err()
