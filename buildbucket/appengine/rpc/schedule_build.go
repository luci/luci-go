...
func setTimeouts(req *pb.ScheduleBuildRequest, cfg *pb.Builder, build *model.Build) {
	...
	switch {
	case req.GetExecutionTimeout().GetSeconds() > 0:
		build.Proto.ExecutionTimeout = &durationpb.Duration{
			Seconds: req.ExecutionTimeout.Seconds,
		}
	...
	default:
		build.Proto.ExecutionTimeout = &durationpb.Duration{
			Seconds: int64((3 * time.Hour).Seconds()),
		}
	}

	switch {
	case req.GetGracePeriod().GetSeconds() > 0:
		build.Proto.GracePeriod = &durationpb.Duration{
			Seconds: req.GracePeriod.Seconds,
		}
	case cfg.GetGracePeriod().GetSeconds() > 0:
		build.Proto.GracePeriod = &durationpb.Duration{
			Seconds: cfg.GracePeriod.Seconds,
		}
	default:
		build.Proto.GracePeriod = &durationpb.Duration{
			Seconds: int64((30 * time.Second).Seconds()),
		}
	}

	switch {
	case req.GetSchedulingTimeout().GetSeconds() > 0:
		build.Proto.SchedulingTimeout = &durationpb.Duration{
			Seconds: req.SchedulingTimeout.Seconds,
		}
	...
	default:
		build.Proto.SchedulingTimeout = &durationpb.Duration{
			Seconds: int64((6 * time.Hour).Seconds()),
		}
	}
}
