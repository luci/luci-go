...
// scheduleRequestFromTemplate loads the given template_build_id if there is one. Does not
...
func scheduleRequestFromTemplate(ctx context.Context, req *pb.ScheduleBuildRequest) (*pb.ScheduleBuildRequest, error) {
	...
	inp := &model.BuildInputProperties{
		ID:    1,
		Build: datastore.KeyForObj(ctx, bld),
	}
	switch err := datastore.Get(ctx, inp); {
	case err == datastore.ErrNoSuchEntity:
		return nil, perm.NotFoundErr(ctx)
	case err != nil:
		return nil, errors.Annotate(err, "failed to fetch build: %d", bld.ID).Err()
	}
	if b.Input == nil {
		b.Input = &pb.Build_Input{}
	}
	b.Input.Properties = &inp.Proto.Struct
