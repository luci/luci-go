...
func getBuildForUpdate(ctx context.Context, fm *mask.Mask, req *pb.UpdateBuildRequest) (*model.Build, error) {
	...
	switch incl, err := fm.Includes("build.status"); {
	case err != nil:
	...
	if finalStatus == pb.Status_SCHEDULED {
		switch incl, err := fm.Includes("build.steps"); {
		case err != nil:
			// This is probably a bug in the server code.
		...
		switch incl, err := fm.Includes("build.output"); {
		case err != nil:
			// This is probably a bug in the server code.
...
func (*Builds) UpdateBuild(ctx context.Context, req *pb.UpdateBuildRequest) (*pb.Build, error) {
	...
	_, err = getBuildForUpdate(ctx, &fm, req)
	if err != nil {
		return nil, err
	}
