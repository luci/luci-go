	...
	return datastore.RunInTransaction(ctx, func(ctx context.Context) error {
		...
		if len(toSave) > 0 {
			if err := datastore.Put(ctx, toSave); err != nil {
				return errors.Annotate(err, "failed to update buildstore build(%d).output.properties", b.ID).Err()
			}
		}
		return nil
	}, nil)
...
func (*Builds) UpdateBuild(ctx context.Context, req *pb.UpdateBuildRequest) (*pb.Build, error) {
	...
	if req.GetBuild().GetId() > 0 {
		logging.Infof(ctx, "Received an UpdateBuild request for build-%d", req.Build.Id)
	}
