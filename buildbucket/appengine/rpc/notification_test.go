...
func TestNotification(t *testing.T) {
	...
	Convey("notifyPubsub", t, func() {
		...
		Convey("w/ callback", func() {
			txErr := datastore.RunInTransaction(ctx, func(ctx context.Context) error {
				cb := model.PubSubCallback{"token", "topic", "user_data"}
			...
			// callback pubsub notification
			So(tasks[0].Payload.(*taskdefs.NotifyPubSub).GetBuildId(), ShouldEqual, 123)
			So(tasks[0].Payload.(*taskdefs.NotifyPubSub).GetCallback(), ShouldBeTrue)
			// global pubsub notification
			So(tasks[1].Payload.(*taskdefs.NotifyPubSub).GetBuildId(), ShouldEqual, 123)
			So(tasks[1].Payload.(*taskdefs.NotifyPubSub).GetCallback(), ShouldBeFalse)
		})
	})
}
