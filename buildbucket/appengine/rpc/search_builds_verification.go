...
// verifySearch makes RPC calls to Python service, compares py and go results and returns py results.
func verifySearch(ctx context.Context, req *pb.SearchBuildsRequest) (*pb.SearchBuildsResponse, error) {
	...
	// get Go results.
	goResCh := make(chan *responseBundle)
	go func() {
		goRes, goErr := searchBuildsGoPath(ctx, req)
		goResCh <- &responseBundle{res: goRes, err: goErr}
	}()
	...
	case goResBundle := <-goResCh:
		compare(ctx, goResBundle, &responseBundle{res: pyRes, err: pyErr})
...
// compare is to compare py and go responses for SearchBuilds.
func compare(ctx context.Context, goResBundle *responseBundle, pyResBundle *responseBundle) {
	if goResBundle == nil {
	...
	diff := ShouldResembleProto(goResBundle.res.Builds, pyResBundle.res.Builds)
	if diff == "" {
		logging.Debugf(ctx, "search verification: no diff")
	} else {
		logging.Warningf(ctx, "search verification: diff found - %s", diff)
	}
}
