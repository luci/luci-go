...
// mode should be one of the enum - New, Append, Search
func validateTags(tags []*pb.StringPair, m mode) error {
	if tags == nil {
		return nil
	}
	var k, v string
	var seenBuilderTagValue string
	var seenGitilesCommit bool
	for _, tag := range tags {
		...
		if m == Append && blocklistKeyForAppend.Has(k) {
		...
		if k == "buildset" {
			...
			if gitilesCommitRegex.MatchString(v) {
				if seenGitilesCommit {
					return errors.Reason("More than one commits/gitiles buildset").Err()
				}
				seenGitilesCommit = true
			}
		}
		if k == "builder" && m == New {
		...
		if m != Search && reservedKeys.Has(k) {
