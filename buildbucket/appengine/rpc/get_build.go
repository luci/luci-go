...
func validateGet(req *pb.GetBuildRequest) error {
	...
	case req.GetBuilder() != nil && req.BuildNumber != 0:
		...
		case req.Builder.Bucket == "":
			return errors.Reason("builder.bucket is required").Err()
		case req.Builder.Builder == "":
			return errors.Reason("builder.builder is required").Err()
		}
	default:
...
func (*Builds) GetBuild(ctx context.Context, req *pb.GetBuildRequest) (*pb.Build, error) {
	if err := validateGet(req); err != nil {
		return nil, appstatus.Errorf(codes.InvalidArgument, err.Error())
	}
	m, err := getFieldMask(req.Fields)
	if err != nil {
		return nil, appstatus.Errorf(codes.InvalidArgument, errors.Annotate(err, "fields").Err().Error())
	}
	if req.Id == 0 {
