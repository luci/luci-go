...
func validateGetBuilder(req *pb.GetBuilderRequest) error {
	if err := protoutil.ValidateBuilderID(req.Id); err != nil {
		return errors.Annotate(err, "builder_id").Err()
	}

	return nil
}

// GetBuilder handles a request to retrieve builder. Implements pb.BuilderServer.
func (*Builders) GetBuilder(ctx context.Context, req *pb.GetBuilderRequest) (*pb.BuilderItem, error) {
	...
	// Check permissions.
	if err := canRead(ctx, req.GetId().Project, req.GetId().Bucket); err != nil {
		return nil, err
	}
