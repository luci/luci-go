...
func TestValidateUpdate(t *testing.T) {
	...
	Convey("validateUpdateBuildStatus", t, func() {
		...
		Convey("succeeds", func() {
			...
			So(validateUpdateBuildStatus(req), ShouldBeNil)
		})

		Convey("fails", func() {
			req.Build.Status = pb.Status_SCHEDULED
			So(validateUpdateBuildStatus(req), ShouldErrLike, "invalid status SCHEDULED for UpdateBuild")
		})
	})

	Convey("validateBuildTags", t, func() {
		req := &pb.UpdateBuildRequest{Build: &pb.Build{Id: 1}}
		So(validateUpdateBuildTags(req), ShouldBeNil)
