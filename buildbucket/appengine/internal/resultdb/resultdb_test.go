...
func TestCreateInvocations(t *testing.T) {
	...
	Convey("create invocations", t, func() {
		...
		Convey("build with number", func() {
			...
			So(builds[0].ResultDBUpdateToken, ShouldEqual, "token for build number 123")
			So(builds[0].Proto.Infra.Resultdb.Invocation, ShouldEqual, "invocations/build-1")
		})

		Convey("build id inv already exists while build number inv doesn't", func() {
			builds := []*model.Build{
				{
					ID: 1,
					Proto: pb.Build{
		...
		Convey("resultDB throws err", func() {
			...
			err := CreateInvocations(ctx, builds, cfgs)
			So(err, ShouldErrLike, "failed to make a CreateInvocation call for build id: 1: rpc error: code = DeadlineExceeded desc = timeout")
		})
