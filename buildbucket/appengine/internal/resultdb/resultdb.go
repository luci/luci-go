	...
	err = parallel.WorkPool(64, func(ch chan<- func() error) {
		for _, b := range builds {
			...
			ch <- func() error {
				...
				header := metadata.MD{}
				_, err = projToClient[proj].CreateInvocation(ctx, reqForBldID, grpc.Header(&header))
				if err = filterAlreadyExistsErr(err); err != nil {
					return errors.Annotate(err, "failed to make a CreateInvocation call for build id: %d", b.Proto.Id).Err()
				}
				...
				if b.Proto.Number > 0 {
					...
					_, err = projToClient[proj].CreateInvocation(ctx, &rdbPb.CreateInvocationRequest{
					...
					if err = filterAlreadyExistsErr(err); err != nil {
						return errors.Annotate(err, "failed to make a CreateInvocation call for build id: %d with build number: %d", b.Proto.Id, b.Proto.Number).Err()
					}
				}
