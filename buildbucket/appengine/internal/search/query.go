...
func (q *Query) fetchOnTagIndex(ctx context.Context) (*pb.SearchBuildsResponse, error) {
	...
	eHeap := minHeap(q.filterEntries(ctx, entries))
	heap.Init(&eHeap)
	...
	for len(results) < int(q.PageSize) {
		...
		for i, b := range builds {
			...
			if !buildTags.HasAll(q.Tags.Format()...) ||
...
func (q *Query) filterEntries(ctx context.Context, entries []model.TagIndexEntry) []model.TagIndexEntry {
	...
	for _, e := range entries {
		...
		} else if protoutil.FormatBucketID(q.Builder.Project, q.Builder.Bucket) != e.BucketID {
			continue
		}
