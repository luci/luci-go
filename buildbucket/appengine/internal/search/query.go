...
// FetchOnBuild fetch directly on Build entity and returns the SearchBuildsResponse matching the query.
func (q *Query) FetchOnBuild(ctx context.Context) (*pb.SearchBuildsResponse, error) {
	...
	// TODO(crbug/1090540): filtering by created_by after it's been migrated to string in db.
	...
	if q.BuildIdLow != nil || q.BuildIdHigh != nil {
		if q.BuildIdLow != nil {
			dq = dq.Gte("__key__", datastore.KeyForObj(ctx, &model.Build{ID: *q.BuildIdLow}))
		}
		if q.BuildIdHigh != nil {
			dq = dq.Lt("__key__", datastore.KeyForObj(ctx, &model.Build{ID: *q.BuildIdHigh}))
		}
	}

	idLow, idHigh := q.getIdRange()
	if idLow != nil {
...
// getIdRange returns the id range from either q.BuildIdLow/q.BuildIdHigh or q.StartTime/q.EndTime.
func (q *Query) getIdRange() (*int64, *int64) {
	if q.BuildIdLow != nil || q.BuildIdHigh != nil {
