...
func TestBuildMetrics(t *testing.T) {
	...
	Convey("Works", t, func() {
		...
		Convey("BuildCreated", func() {
			...
			BuildCreated(ctx, b)
			So(store.Get(ctx, BuildCounts.Created, time.Time{}, fv("")), ShouldEqual, 1)

			b.Tags = []string{"user_agent:gerrit"}
			BuildCreated(ctx, b)
			So(store.Get(ctx, BuildCounts.Created, time.Time{}, fv("gerrit")), ShouldEqual, 1)
		})
		...
		Convey("BuildStarted", func() {
			...
			Convey("build/Started", func() {
				b.Canary = false
				BuildStarted(ctx, b)
				So(store.Get(ctx, BuildCounts.Started, time.Time{}, fv(false)), ShouldEqual, 1)

				b.Canary = true
				BuildStarted(ctx, b)
				So(store.Get(ctx, BuildCounts.Started, time.Time{}, fv(true)), ShouldEqual, 1)
			})
			...
			Convey("build/scheduling_durations", func() {
				...
				BuildStarted(ctx, b)
				val := store.Get(ctx, BuildDurations.Scheduling, time.Time{}, fields)
		...
		Convey("BuildCompleted", func() {
			Convey("builds/Completed", func() {
				...
				So(store.Get(ctx, BuildCounts.Completed, time.Time{}, fields), ShouldEqual, 1)
			...
			Convey("builds/cycle_durations", func() {
				...
				BuildCompleted(ctx, b)
				val := store.Get(ctx, BuildDurations.Cycle, time.Time{}, fields)
				So(val.(*distribution.Distribution).Sum(), ShouldEqual, 33)
			})
			...
			Convey("builds/run_durations", func() {
				...
				BuildCompleted(ctx, b)
				val := store.Get(ctx, BuildDurations.Run, time.Time{}, fields)
		...
		Convey("BuildLeased", func() {
			b.Proto.Status = pb.Status_STARTED
			BuildLeased(ctx, b)
			So(store.Get(ctx, BuildCounts.Leased, time.Time{}, fv()), ShouldEqual, 1)
		})

		Convey("BuildLeaseExpired", func() {
			b.Status = pb.Status_STARTED
			BuildLeaseExpired(ctx, b)
			So(store.Get(ctx, BuildCounts.LeaseExpired, time.Time{}, fv("STARTED")), ShouldEqual, 1)
		})
	})
}
