...
func NewDispatcher(dsp *tq.Dispatcher) *Dispatcher {
	dsp.RegisterTaskClass(tq.TaskClass{
		...
		Custom: func(ctx context.Context, m proto.Message) (*tq.CustomPayload, error) {
			task, ok := m.(*taskdefs.CancelSwarmingTask)
			switch {
			case !ok:
				return nil, errors.Reason("unexpected payload type %T", m).Err()
			case task.GetHostname() == "":
				return nil, errors.Reason("hostname is required").Err()
			case task.GetTaskId() == "":
				return nil, errors.Reason("task_id is required").Err()
			}
