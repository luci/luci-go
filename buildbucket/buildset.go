...
type GitilesCommit struct {
	Host     string
	Project  string
	Revision string // i.e. commit hash
}
...
func ParseBuildSet(buildSet string) BuildSet {
	// fmt.Sscanf cannot be used for this parsing.

	const sep = "/"
	p := strings.Split(buildSet, sep)
	for _, c := range p {
		if c == "" {
			return nil
		}
	...
	case n == 5 && p[0] == "patch" && p[1] == "gerrit":
		...
		var err error
		if gerrit.Change, err = strconv.Atoi(p[3]); err != nil {
			return nil
		}
	...
	case n == 5 && p[0] == "patch" && p[1] == "rietveld":
		...
		var err error
		if rietveld.Issue, err = strconv.Atoi(p[3]); err != nil {
			return nil
		}
	...
	case n >= 5 && p[0] == "commit" && p[1] == "gitiles" && p[n-2] == "+" && looksLikeSha1(p[n-1]):
		return &GitilesCommit{
			Host:     p[2],
			Project:  strings.Join(p[3:n-2], sep), // exclude plus
			Revision: p[n-1],
		}
