...
type Input struct {
	// Properties is build properties passed to a recipe.
	//
	// If nil, (*Build).Parse will set it to a map[string]interface{}
	// and unmarshal all properties in parameters_json.
	// If non-nil, (*Build).Parse will reuse as is.
	// If a (*Build).Parse user knows properties that it needs,
