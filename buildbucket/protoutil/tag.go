...
func StringPairs(m strpair.Map) []*bp.StringPair {
	ret := make([]*bp.StringPair, 0, len(m))
	for k, vs := range m {
		for _, v := range vs {
			ret = append(ret, &bp.StringPair{Key: k, Value: v})
		}
	}
	SortStringPairs(ret)
	return ret
...
func SortStringPairs(pairs []*bp.StringPair) {
...
func BuildSets(b *bp.Build) []string {
	var result []string
...
func GitilesBuildSet(c *bp.GitilesCommit) string {
...
func GitilesRepoURL(c *bp.GitilesCommit) string {
