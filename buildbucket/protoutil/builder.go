...
func ParseBuilderID(dest *buildbucketpb.BuilderID, s string) error {
	*dest = buildbucketpb.BuilderID{}
	parts := strings.Split(s, "/")
	if len(parts) != 3 {
		return fmt.Errorf("invalid builder id; must have 2 slashes")
	}
	dest.Project = parts[0]
	dest.Bucket = parts[1]
	dest.Builder = parts[2]
	return nil
}
