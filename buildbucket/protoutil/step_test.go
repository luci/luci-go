...
func BenchmarkBetterWithMap(b *testing.B) {
	for i := 0; i < b.N; i++ {
		IsWorseStepStatusWithMap(pb.Status_CANCELED, pb.Status_FAILURE)
	}
}

func BenchmarkBetterWithBS(b *testing.B) {
	for i := 0; i < b.N; i++ {
		IsWorseStepStatusWithBS(pb.Status_SUCCESS, pb.Status_FAILURE)
	}
}

func BenchmarkBetterWithLinear(b *testing.B) {
	for i := 0; i < b.N; i++ {
		IsWorseStepStatusWithLinear(pb.Status_SUCCESS, pb.Status_FAILURE)
	}
}
