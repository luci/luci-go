...
func (r *buildRunner) Run(ctx context.Context, args *pb.RunBuildArgs) (*pb.Build, error) {
	...
	if build == nil {
		return nil, errors.Reason("user executable did not return a build").Err()
	}
...
func normalizeArgs(a *pb.RunBuildArgs) error {
	...
	case a.CacheDir == "":
		return errors.Reason("cache_dir is required").Err()
	}
