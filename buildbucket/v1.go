...
// Thils file implements v1->v2 converters.

// BuildAddress returns V1 build address string for a V2 build.
// If b.Number is positive, the address is "<bucket>/<builder>/<number>".
// Otherwise it is "<id>".
//
// See also "go.chromium.org/luci/common/api/buildbucket/v1".FormatBuildAddress.
func BuildAddress(b *buildbucketpb.Build) string {
	return v1.FormatBuildAddress(b.Id, b.Builder.GetBucket(), b.Builder.GetBucket(), int(b.Number))
}
...
// the returned build won't include it either.
//
...
func BuildToV2(msg *v1.ApiCommonBuildMessage) (b *buildbucketpb.Build, err error) {
	...
	if err != nil {
		return nil, errors.Annotate(err, "invalid status").Tag(MalformedBuild).Err()
	}
...
func toStringPair(s string) (buildbucketpb.StringPair, bool) {
	parts := strings.SplitN(s, ":", 2)
	if len(parts) != 2 {
		return buildbucketpb.StringPair{}, false
	}
	return buildbucketpb.StringPair{Key: parts[0], Value: parts[1]}, true
}

func toStringPairs(tags []string) []*buildbucketpb.StringPair {
	ret := make([]*buildbucketpb.StringPair, 0, len(tags))
	for _, t := range tags {
		if p, ok := toStringPair(t); ok {
			ret = append(ret, &p)
		}
	}
	return ret
