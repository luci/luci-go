...
func builderToV2(msg *v1.ApiCommonBuildMessage, tags strpair.Map, params *v1Params) (ret *buildbucketpb.Builder_ID, err error) {
	...
	case err != nil:
		err = errors.Annotate(err, "parsing bucket name").Tag(MalformedBuild).Err()
	case ret.Project != msg.Project:
		err = errors.Reason(
			"message project %s does not match bucket project %s", msg.Project, ret.Project).Err()
	}
