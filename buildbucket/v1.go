...
func StatusToV2(build *v1.ApiCommonBuildMessage) (bp.Status, error) {
	switch build.Status {
	case "":
		return bp.Status_STATUS_UNSPECIFIED, nil

	case "SCHEDULED":
		return bp.Status_SCHEDULED, nil

	case "STARTED":
		return bp.Status_STARTED, nil

	case "COMPLETED":
		switch build.Result {
		case "SUCCESS":
			return bp.Status_SUCCESS, nil

		case "FAILURE":
			switch build.FailureReason {
			case "", "BUILD_FAILURE":
				return bp.Status_FAILURE, nil
			case "INFRA_FAILURE", "BUILDBUCKET_FAILURE", "INVALID_BUILD_DEFINITION":
				return bp.Status_INFRA_FAILURE, nil
			default:
...
func BuildToV2(msg *v1.ApiCommonBuildMessage) (b *bp.Build, err error) {
	...
	b = &bp.Build{
		...
		Input: &bp.Build_Input{
			Experimental: msg.Experimental,
		},
		Output: &bp.Build_Output{},

		Infra: &bp.BuildInfra{
			Buildbucket: &bp.BuildInfra_Buildbucket{
				Canary: msg.Canary,
			},
			Swarming: &bp.BuildInfra_Swarming{
	...
	b.Infra.Swarming.BotDimensions = make([]*bp.StringPair, 0, len(resultDetails.TaskResult.BotDimensions))
	for _, d := range resultDetails.TaskResult.BotDimensions {
		for _, v := range d.Value {
			b.Infra.Swarming.BotDimensions = append(b.Infra.Swarming.BotDimensions, &bp.StringPair{
				Key:   d.Key,
				Value: v,
			})
		}
...
func tagsToV2(dest *bp.Build, tags []string) error {
	...
	for _, t := range toStringPairs(tags) {
		...
		case "swarming_dimension":
			if d := toStringPair(t.Value); d != nil {
				dest.Infra.Swarming.TaskDimensions = append(dest.Infra.Swarming.TaskDimensions, &bp.RequestedDimension{
					Key:   d.Key,
					Value: d.Value,
				})
			}
...
func builderToV2(msg *v1.ApiCommonBuildMessage, tags strpair.Map, params *v1Params) (ret *bp.BuilderID, err error) {
	ret = &bp.BuilderID{Builder: params.Builder}
	if ret.Builder == "" {
...
func toStringPair(s string) *bp.StringPair {
	parts := strings.SplitN(s, ":", 2)
	if len(parts) != 2 {
		return nil
	}
	return &bp.StringPair{Key: parts[0], Value: parts[1]}
}

func toStringPairs(tags []string) []*bp.StringPair {
	ret := make([]*bp.StringPair, 0, len(tags))
	for _, t := range tags {
