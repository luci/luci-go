...
func (r *collectRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	endedBuilds := make([]*buildbucketpb.Build, 0, len(r.buildIDs))
	for {
		req := &buildbucketpb.GetBuildRequest{Id: r.buildIDs[0]}
		req.Fields = &field_mask.FieldMask{Paths: []string{
			// Include all output properties, which are omitted by default.
			"output",

			// Since we add output props above, we must list all default fields too.
			"builder",
			"cancel_reason",
			"create_time",
			"created_by",
		...
		fmt.Printf("Checking status of the build %d... ", r.buildIDs[0])
		...
		if build.Status&buildbucketpb.Status_ENDED_MASK != 0 {
			endedBuilds = append(endedBuilds, build)
			r.buildIDs = r.buildIDs[1:]
		} else {
