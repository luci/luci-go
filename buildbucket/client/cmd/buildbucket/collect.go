...
func collectBuildDetails(ctx context.Context, client buildbucketpb.BuildsClient, buildIDs []int64, interval time.Duration) ([]*buildbucketpb.Build, error) {
	unfinishedBuilds := buildIDs
	endedBuildDetails := make([]*buildbucketpb.Build, 0, len(buildIDs))
	for {
...
func writeBuildDetails(endedBuilds []*buildbucketpb.Build, outputPath string) error {
	if outputPath == "" {
		return nil
	}
	...
	encodedMessages := make([]json.RawMessage, 0, len(endedBuilds))
	m := &jsonpb.Marshaler{}
	for _, build := range endedBuilds {
...
func (r *collectRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	endedBuilds, err := collectBuildDetails(
		ctx, client, r.buildIDs, r.intervalArg)
	if err != nil {
		return r.done(ctx, err)
	}

	if err := writeBuildDetails(endedBuilds, r.outputArg); err != nil {
		return r.done(ctx, err)
	}
