...
func (r *collectRun) collectBuildDetails(ctx context.Context) ([]*buildbucketpb.Build, error) {
...
func (r *collectRun) writeBuildDetails(endedBuilds []*buildbucketpb.Build, outputPath string) error {
	if r.outputArg == "" {
		return nil
	}
...
func (r *collectRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	endedBuilds, err := r.collectBuildDetails(ctx)
	if err != nil {
		return r.done(ctx, err)
	}

	if err := r.writeBuildDetails(endedBuilds, r.outputArg); err != nil {
		return r.done(ctx, err)
	}
