...
func TestRetrieveBuildIDs(t *testing.T) {
	...
	Convey("RetrieveBuildIDs", t, func() {
		Convey("Basic", func() {
			...
			var actualReq *bp.BatchRequest
			ids, err := retrieveBuildIDs(builds, func(req *bp.BatchRequest) (*bp.BatchResponse, error) {
				actualReq = req
				return textpb(&bp.BatchResponse{}, `
					responses { get_build {id: 2} }
					responses { get_build {id: 3} }
				`).(*bp.BatchResponse), nil
			})
			So(err, ShouldBeNil)
