...
func (f *clsFlag) retrieveCLs(ctx context.Context, httpClient *http.Client) ([]*bp.GerritChange, error) {
	ret := make([]*bp.GerritChange, len(f.cls))
...
func (f *clsFlag) retrieveCL(ctx context.Context, cl string, httpClient *http.Client) (*bp.GerritChange, error) {
	ret, err := parseCL(cl)
	switch {
	case err != nil:
		return nil, err
...
func parseCL(s string) (*bp.GerritChange, error) {
