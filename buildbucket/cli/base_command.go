...
func (r *baseCommandRun) initClients(ctx context.Context) error {
	...
	r.client = bp.NewBuildsPRPCClient(&prpc.Client{
		C:       r.httpClient,
		Host:    r.host,
		Options: rpcOpts,
	})
...
func (r *baseCommandRun) batchAndDone(ctx context.Context, req *bp.BatchRequest) int {
	...
	for i, res := range res.Responses {
		...
		case *bp.BatchResponse_Response_Error:
			...
			if len(req.Requests) > 1 {
				...
				case *bp.BatchRequest_Request_CancelBuild:
		...
		case *bp.BatchResponse_Response_GetBuild:
			build = res.GetBuild
		case *bp.BatchResponse_Response_CancelBuild:
			build = res.CancelBuild
		case *bp.BatchResponse_Response_ScheduleBuild:
...
func (r *baseCommandRun) retrieveBuildIDs(ctx context.Context, builds []string) (buildIDs []int64, err error) {
	return retrieveBuildIDs(builds, func(req *bp.BatchRequest) (*bp.BatchResponse, error) {
		return r.client.Batch(ctx, req)
	})
}

func retrieveBuildIDs(builds []string, callBatch func(*bp.BatchRequest) (*bp.BatchResponse, error)) (buildIDs []int64, err error) {
	buildIDs = make([]int64, len(builds))
	batchReq := &bp.BatchRequest{
		Requests: make([]*bp.BatchRequest_Request, 0, len(builds)),
	}
	...
	for i, b := range builds {
		...
		if getBuild.Builder == nil {
			...
			batchReq.Requests = append(batchReq.Requests, &bp.BatchRequest_Request{
				Request: &bp.BatchRequest_Request_GetBuild{GetBuild: getBuild},
			})
