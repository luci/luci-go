...
func (f *commitFlag) retrieveCommit() (*bp.GitilesCommit, error) {
	if f.commit == "" {
		return nil, nil
	}
...
func parseCommit(s string) (commit *bp.GitilesCommit, mustConfirmRef bool, err error) {
	m := regexCommit.FindStringSubmatch(s)
	if m == nil {
		return nil, false, fmt.Errorf("does not match regexp %q", regexCommit)
	}
	commit = &bp.GitilesCommit{
		Host:    m[1],
		Project: m[2],
		Id:      m[4],
		Ref:     m[5],
