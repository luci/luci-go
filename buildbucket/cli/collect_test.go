...
func TestCollect(t *testing.T) {
	...
	Convey("Collect", t, func() {
		Convey("writeBuildDetails", func() {
			...
			writeBuildDetails([]int64{123, 456}, map[int64]*bp.Build{
				123: {Id: 123},
				456: {Id: 456},
			}, file.Name())
		...
		Convey("collectBuildDetails", func() {
			...
			sleepCalls := 0
			buildsMock := bp.NewMockBuildsClient(gomock.NewController(t))

			batchMock := buildsMock.EXPECT().Batch(
				gomock.Any(),
				&bp.BatchRequest{
					Requests: []*bp.BatchRequest_Request{
						{
							Request: &bp.BatchRequest_Request_GetBuild{
								GetBuild: &bp.GetBuildRequest{
									Id:     123,
			...
			Convey("all builds are ended from the start", func() {
				batchMock.Return(&bp.BatchResponse{
					Responses: []*bp.BatchResponse_Response{
						...
						{
							Response: &bp.BatchResponse_Response_GetBuild{
								GetBuild: &bp.Build{
									Id:     456,
									Status: bp.Status_SUCCESS,
								},
							},
						},
						{
			...
			Convey("one build ended on a second request", func() {
				...
				).Return(&bp.BatchResponse{
					Responses: []*bp.BatchResponse_Response{
						{
							Response: &bp.BatchResponse_Response_GetBuild{
								GetBuild: &bp.Build{
									Id:     123,
									Status: bp.Status_INFRA_FAILURE,
								},
							},
						},
					},
