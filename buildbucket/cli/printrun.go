...
func (r *printRun) RegisterFieldFlags() {
	...
	r.Flags.StringVar(&r.fields, "fields", "", `
		Print only provided fields. Fields should be specified in a comma separated string according to the JSON encoding schema for FieldMask (https://developers.google.com/protocol-buffers/docs/proto3#json).
		This flag is mutually exclusive with -A, -p, -steps and -id.

		Example: Request fields id and status from the build
...
func (r *printRun) FieldMask() (*field_mask.FieldMask, error) {
	if r.fields != "" {
		...
		// TODO: Use Unmarshal feature in JSONPB when protobuf v2 API is released
		//  Currently, there's an existing issue in Go JSONPB implementation which
		//  results in serialization and deserialization of FieldMask not working
		//  as expected. See: https://github.com/golang/protobuf/issues/745
