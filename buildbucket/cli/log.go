...
func (r *logRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	// Find the logs.
	logByName := map[string]*bp.Step_Log{}
	for _, l := range step.Logs {
		logByName[l.Name] = l
	}
	var logs []*bp.Step_Log
	if len(r.logs) == 0 {
		logs = r.defaultLogs(logByName)
	} else {
		logs = make([]*bp.Step_Log, 0, len(r.logs))
...
func (r *logRun) defaultLogs(available map[string]*bp.Step_Log) []*bp.Step_Log {
	logs := make([]*bp.Step_Log, 0, 2)
...
func (r *logRun) getSteps(ctx context.Context) ([]*bp.Step, error) {
...
func (r *logRun) printLogs(ctx context.Context, logs []*bp.Step_Log) error {
	if len(logs) == 0 {
		return nil
	}
