...
func collectBuildDetails(ctx context.Context, client bp.BuildsClient, buildIDs []int64, sleep func()) (map[int64]*bp.Build, error) {
	buildIDs = dedupInt64s(buildIDs)
	ended := make(map[int64]*bp.Build, len(buildIDs))
	for {
		breq := &bp.BatchRequest{}
		for _, id := range buildIDs {
			if _, ok := ended[id]; ok {
				continue
			}

			getReq := &bp.GetBuildRequest{
				Id:     id,
				Fields: getRequestFieldMask,
			}
			breq.Requests = append(breq.Requests, &bp.BatchRequest_Request{
		...
		for _, resp := range bresp.Responses {
			...
			case build.Status&bp.Status_ENDED_MASK != 0:
...
func writeBuildDetails(buildIDs []int64, buildDetails map[int64]*bp.Build, outputPath string) error {
