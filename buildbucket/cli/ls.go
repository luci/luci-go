...
// requests.
func (r *lsRun) parseSearchRequests(ctx context.Context, args []string) (*bp.BatchRequest, error) {
	...
	for _, path := range args {
		searchBuilds := proto.Clone(baseReq).(*bp.SearchBuildsRequest)
		var err error
		if searchBuilds.Predicate.Builder, err = r.parsePath(path); err != nil {
			return nil, fmt.Errorf("invalid path %q: %s", path, err)
		}
		ret.Requests = append(ret.Requests, &bp.BatchRequest_Request{
			Request: &bp.BatchRequest_Request_SearchBuilds{SearchBuilds: searchBuilds},
		})
	}
	...
	if len(ret.Requests) == 0 {
		ret.Requests = append(ret.Requests, &bp.BatchRequest_Request{
			Request: &bp.BatchRequest_Request_SearchBuilds{SearchBuilds: baseReq},
		})
	}
...
func (r *lsRun) parseBaseRequest(ctx context.Context) (*bp.SearchBuildsRequest, error) {
	ret := &bp.SearchBuildsRequest{
		Predicate: &bp.BuildPredicate{
			Tags:                r.Tags(),
...
func (r *lsRun) parsePath(path string) (*bp.BuilderID, error) {
	bid := &bp.BuilderID{}
