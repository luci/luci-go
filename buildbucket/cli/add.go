...
func (r *addRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	req := &bp.BatchRequest{}
	for i, a := range args {
		schedReq := proto.Clone(baseReq).(*bp.ScheduleBuildRequest)
		...
		req.Requests = append(req.Requests, &bp.BatchRequest_Request{
			Request: &bp.BatchRequest_Request_ScheduleBuild{ScheduleBuild: schedReq},
		})
	}
...
func (r *addRun) prepareBaseRequest(ctx context.Context) (*bp.ScheduleBuildRequest, error) {
	ret := &bp.ScheduleBuildRequest{
	...
	if r.experimental {
		ret.Experimental = bp.Trinary_YES
	}
