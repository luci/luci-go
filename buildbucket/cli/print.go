...
var (
	...
	ansiStatus         = map[bp.Status]string{
		bp.Status_SCHEDULED:     ansi.LightWhite,
		bp.Status_STARTED:       ansi.Yellow,
		bp.Status_SUCCESS:       ansi.Green,
		bp.Status_FAILURE:       ansi.Red,
		bp.Status_INFRA_FAILURE: ansi.Magenta,
	}
)
...
// Build prints b.
func (p *printer) Build(b *bp.Build) {
	defer p.tab.Flush()
...
// commit prints c.
func (p *printer) commit(c *bp.GitilesCommit) {
	if c.Id == "" {
...
// change prints cl.
func (p *printer) change(cl *bp.GerritChange) {
	p.linkf("https://%s/c/%s/+/%d/%d", cl.Host, cl.Project, cl.Change, cl.Patchset)
}

// step prints s.
func (p *printer) step(s *bp.Step) {
...
func (p *printer) buildTime(b *bp.Build) {
	now := p.nowFn()
