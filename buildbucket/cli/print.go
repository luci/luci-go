...
func (p *printer) Build(b *pb.Build) {
	...
	// conditions
	if b.Id == 0 {
		panic(fmt.Errorf("Expect non zero value id present in the build"))
	}
	...
	if builder := b.Builder; builder == nil ||
		...
		panic(fmt.Errorf("Expect builder present in the build and all fields under builder should not be zero value. Got: %v", builder))
	}
