...
// Command run_annotations is a LUCI executable that consumes @@@ANNOTATIONS@@@
// on stdin.
package main
...
func sendAnnotations(ctx context.Context, ann *milo.Step) error {
	...
	if err != nil {
		return errors.Annotate(err, "failed to convert annotations proto to Build message").Err()
	}

	props, err := milo.ExtractProperties(ann)
	if err != nil {
		return errors.Annotate(err, "failed to extract properties from an annotation proto").Err()
	}
...
func main() {
	...
	processor := annotee.New(ctx, annotee.Options{
		...
		Execution:              annotation.ProbeExecution(os.Args, env.Sorted(), cwd),
