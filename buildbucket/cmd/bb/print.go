...
func (p *printer) Build(b *buildbucketpb.Build) {
	...
	p.f("%sBuild %d  %s  '%s/%s/%s", ansiStatus[b.Status], b.Id, b.Status, b.Builder.Project, b.Builder.Bucket, b.Builder.Builder)
	if b.Number != 0 {
		p.f("/%d", b.Number)
	}
	p.f("'%s\n", ansi.Reset)
	...
	if b.SummaryMarkdown != "" {
		...
		p.f("%s\n", strings.TrimSpace(b.SummaryMarkdown))
	}
...
func (p *printer) step(s *buildbucketpb.Step) {
	...
	p.indent.Level += 2
	summary := strings.TrimSpace(s.SummaryMarkdown)
	if summary != "" {
		// TODO(nodir): color markdown.
