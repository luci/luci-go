...
func cmdLog(defaultAuthOpts auth.Options) *subcommands.Command {
	return &subcommands.Command{
		...
		ShortDesc: "prints step log(s)",
		LongDesc: `Prints step log(s).

Argument BUILD can be an int64 build id or a string
<project>/<bucket>/<builder>/<build_number>, e.g. chromium/ci/linux-rel/1

Argument STEP is a build step name. Use | as parent-child separator, e.g.
"parent|child".

Argument LOG is one ore more log namees of the STEP. They will be multiplexed
them by time. Defaults to stdout and stderr.
`,
...
func (r *logRun) getStep(ctx context.Context, args *logRunArgs) (*buildbucketpb.Step, error) {
	...
	build, err := client.GetBuild(ctx, args.getBuild)
	if err != nil {
		return nil, err
	}
...
// multiplexLogs receives from log entries and calls f in the order of log
// timestamps.
