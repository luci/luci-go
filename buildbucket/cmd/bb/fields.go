...
func (f *buildFieldFlags) Register(fs *flag.FlagSet) {
	fs.BoolVar(&f.all, "A", false, "display build entirely")
	fs.BoolVar(&f.steps, "steps", false, "display steps")
	fs.BoolVar(&f.inputProperties, "ip", false, "display input properties")
	fs.BoolVar(&f.outputProperties, "op", false, "display output properties")
}
