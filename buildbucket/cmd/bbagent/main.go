...
func mainImpl() int {
	...
	builds, err := host.Run(cctx, opts, func(ctx context.Context, hostOpts host.Options) error {
		...
		deadlineEventCh, dctx, _ := lucictx.AdjustDeadline(ctx, calcReservedDeadline(ctx), 500*time.Millisecond)
...
func calcReservedDeadline(ctx context.Context) time.Duration {
