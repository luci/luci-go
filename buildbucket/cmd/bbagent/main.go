...
func mainImpl() int {
	...
	var shutdownLuciexeFn atomic.Value
	builds, err := host.Run(cctx, opts, func(ctx context.Context, hostOpts host.Options) error {
		...
		shutdownLuciexeFn.Store(shutdown)
	...
	go func() {
		// Monitors the `dispatcherErrCh` and checks for fatal error defined by
		// `fatalPred`. Stops the build shuttling and shutdown the luciexe if
		// a fatal error is received.
		stopped := false
		fatalPred := func(err error) bool {
			// Tolerate the InvalidArgument error at the beginning of the build where
			// build status hasn't switched to STARTED yet.
		...
		for {
			...
			case err := <-dispatcherErrCh:
				if !stopped && fatalPred(err) {
					atomic.StoreInt32(&stopShuttling, 1)
					if shutdown := shutdownLuciexeFn.Load(); shutdown != nil {
						shutdown.(func())()
					}
					stopped = true
					fatalErr.Store(err)
	...
	for build := range builds {
		if atomic.LoadInt32(&stopShuttling) == 0 {
			buildsCh.C <- build
			finalBuild = build
		}
	}
	...
	reportErrToBB := func(err error) {
		...
		if bbErr != nil {
			logging.Errorf(ctx, "Failed to report error %s to Buildbucket due to %s", err, bbErr)
		}
	}
