...
// OAuthScopes defines OAuth scopes used by the Bootstrapper.
//
...
//
// Note that bootstrapper subprocesses (git, user executable) are still free to
// request whatever scopes they need (though LUCI_CONTEXT protocol). The scopes
// here are only for parts of bootstrapper (LogDog client, Devshell proxy, etc).
//
...
//
// Such context can be used by the bootstrapper or its subprocesses
//  (if they are launched with environ populated via ExportIntoEnv).
//
// There are two such contexts: a system context and a user context.
//
// The system auth context is used for running logdog .
// On Swarming all these actions will use bot-associated account (specified in
// Swarming bot config), whose logical name (usually "system") is provided via
// "-luci-system-account" command-line flag.
//
// The user auth context is used for actually running the user executable. It is
// the context the bootstrapper starts with by default. On Swarming this will be
// the context associated with service account specified in the Swarming task
// definition.
//
...
type AuthContext struct {
	...
	// has very low quota. Bootstrapper needs to know all such hostnames in
	// advance to be able to force authenticated access to them.
	...
	authenticator *auth.Authenticator // used by bootstrapper itself
	...
	gsutilState string         // path to a bootstrap-managed state directory
...
func (ac *AuthContext) Launch(ctx context.Context, tempDir string) (err error) {
	...
	// Construct authentication with default set of scopes to be used through
	// out the bootstrapper
...
// Authenticator returns an authenticator that can be used by the bootstrapper.
//
