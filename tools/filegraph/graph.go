...
// relevant to file A.
//
// It is implemented as a trie with edges connecting trie leafs, where trie
// reflects the directory structure.
// A directory with subdirs/files is represented by a node struct with children.
...
type graph struct {
	...
	// The nodes in the map key must be leafs.
	//
