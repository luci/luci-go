...
type graph struct {
	...
	//
	// The map value is the non-normalized edge weight.
	// To normalize it, it must be divided by key.from.weight. See node.weight.
	//
	// Note: using one giant hash table with float64 values uses significantly
	// less memory that a hashtable in each node.
...
// ensureNode returns a node for the path.
