...
//
// If exclude is non-empty, commits reachable from exclude are be
// excluded. This is similar to "exclude.." revision range in git-log.
//
...
func readCommits(ctx context.Context, dir, exclude string, fn func(c commit) error) (err error) {
	...
	if err := cmd.Start(); err != nil {
		return errors.Reason("git failed to start").Err()
	}
	defer func() {
		cancel()
		werr := cmd.Wait()
