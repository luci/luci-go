...
type logParser struct {
	...
	// the separator character. NUL by default, customized in tests.
	sep byte
}

// newLogReader initializes a log parser.
// r must be the output of git-log.
...
func (r *logParser) readRawBlock(dest *fileChange) (err error) {
	...
	// Read status
	var status string
	if status, err = r.read(r.sep); err != nil {
		return
	}
...
func (r *logParser) ReadCommits(fn func(commit) error) error {
	for {
		...
		// It should be either EOF or a specific byte.
		switch err := r.expect(r.sep); {
		case err == io.EOF:
			return nil
		case err != nil:
			return err
