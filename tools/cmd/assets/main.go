...
// GetAssetChecksum returns the asset checksum (sha256). Ruturns nil if no such
// asset.
func GetAssetChecksum(name string) []byte {
...
// Checksums returns a map of all asset checksums (sha256).
func Checksums() map[string][]byte {
	cpy := make(map[string][]byte, len(files))
	for k, v := range fileSha256s {
		bCpy := make([]byte, len(v))
		copy(bCpy, v)
		cpy[k] = bCpy
	}
	return cpy
...
func run(dir string, extraExts []string) error {
	exts := defaultExts.Union(stringset.NewFromSlice(extraExts...))
...
func findAssets(pkgDir string, exts stringset.Set) (assetMap, error) {
	assets := assetMap{}
...
// `assetExts` glob.
func isAssetFile(path string, assetExts stringset.Set) (ok bool) {
	base := filepath.Base(path)
	assetExts.Iter(func(pattern string) (keepGoing bool) {
...
func generate(t *template.Template, pkg *build.Package, assets assetMap, assetExts stringset.Set, path string) error {
