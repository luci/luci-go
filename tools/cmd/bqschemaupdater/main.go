...
func updateFromTableDef(ctx context.Context, ts tableStore, td tableDef) error {
	...
	case isNotFound(err):
		...
		log.Println("Creating a new table... please update the documentation in https://chromium.googlesource.com/infra/infra/+/master/doc/bigquery_tables.md or the internal equivalent.")
		return nil
	case err != nil:
		return err
	}
...
func run(ctx context.Context) error {
	...
	td.Description = fmt.Sprintf("Proto: https://cs.chromium.org/%s\nTable Description: %s", url.PathEscape(fmt.Sprintf("%s file:%s", flags.messageName, file.GetName())), td.Description)
