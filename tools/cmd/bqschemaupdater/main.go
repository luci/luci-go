...
func parseFlags() (*flags, error) {
	...
	flag.StringVar(&f.protoDir, "proto-dir", ".", "path to directory with the schema .proto file")

	const protoPathsUsage = "path to directory with the imported .proto file; can be specified multiple times"
	flag.Var(&f.protoPaths, "proto-path", protoPathsUsage)
	// protoc error message talks about --proto-path, not -proto-path, so support both.
	flag.Var(&f.protoPaths, "-proto-path", protoPathsUsage)
	...
	var err error
	f.protoDir, err = filepath.Abs(f.protoDir)
	if err != nil {
		return nil, errors.Annotate(err, "could not make path %q absolute", f.protoDir).Err()
	}
...
func loadProtoDescription(dir string, protoPaths []string) (*descriptor.FileDescriptorSet, error) {
	...
	// If in Go mode, include all $GOPATH/src directories because we like
	...
	for _, p := range goPaths() {
		...
		goPathArgs = append(goPathArgs, "--proto_path="+src)
	...
	case !inGopath:
		...
		// Below we pass absolute paths to .proto files, so unless
		// we pass a directory containing them as a proto path, protoc
		// will complain. Do that dor the user.
		args = append(args, "--proto_path="+ dir)

		// And then pass user-defined proto paths.
		for _, p := range protoPaths {
			args = append(args, "--proto_path="+p)
		}

	case len(protoPaths) > 0:
