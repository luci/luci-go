...
func (m *GoogleOAuth2Method) Authenticate(c context.Context, r *http.Request) (user *auth.User, err error) {
	...
	if pc == nil {
		u, _, err := m.authenticateAgainstGoogle(c, accessToken)
		return u, err
	}

	cacheKey := tokenCheckCache(accessToken)
	userIface, err := pc.GetOrCreate(c, cacheKey, func() (interface{}, time.Duration, error) {
		return m.authenticateAgainstGoogle(c, accessToken)
	})
	if err != nil {
		return nil, err
	}
	return userIface.(*auth.User), nil
}
