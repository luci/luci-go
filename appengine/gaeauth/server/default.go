...
//
// By default on the dev server it is based on dev server cookies, in prod it is
// based on OpenID. Works only if appropriate handlers have been installed into
// the router. See InstallHandlers.
//
// It is allowed to assigned to CookieAuth (e.g. to install a tweaked auth
// method) before InstallHandlers is called.
