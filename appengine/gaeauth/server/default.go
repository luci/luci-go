...
//
// This method is DEPRECATED since it depends on Users API not available
// outside of GAE first-gen runtime.
//
// Use go.chromium.org/luci/server/encryptedcookies instead. To facilitate the
// migration you can switch to the encrypted cookies while still running on
// the GAE first-gen by calling SwitchToEncryptedCookies() early during the
