...
var errNotConfigured = errors.New(
	"Auth Service URL is not configured, you MUST configure it for apps used " +
		"in production, visit /auth/admin/settings to do so.")

// GetAuthDB fetches AuthDB snapshot from the datastore and returns auth.DB
...
//
// If auth_service URL is not configured, returns special kind of auth.DB that
// implements some default authorization rules (allow everything on dev server,
// forbid everything and emit errors on real GAE).
func GetAuthDB(c context.Context, prev auth.DB) (auth.DB, error) {
	// Grab revision number of most recent snapshot.
	latest, err := authdb.GetLatestSnapshotInfo(c)
	if err != nil {
		return nil, err
	}
	...
	if latest == nil {
		...
		return auth.ErroringDB{Error: errNotConfigured}, nil
	}
	...
	// assertion.
	if prevDB, _ := prev.(*auth.SnapshotDB); prevDB != nil {
	...
	start := clock.Now(c)
	proto, err := authdb.GetAuthDBSnapshot(c, latest.GetSnapshotID())
	if err != nil {
		return nil, err
	}
...
// devServerDB implements auth.DB by allowing everything.
//
