...
type appSettings struct {
	// ConfigServiceURL is URL of config service to fetch configs from.
...
func fetchAppSettings(c context.Context) (appSettings, error) {
	cfg := appSettings{}
	switch err := settings.Get(c, "helloworld", &cfg); err {
	case nil:
		return cfg, nil
...
func storeAppSettings(c context.Context, cfg *appSettings, who, why string) error {
	return settings.Set(c, "helloworld", cfg, who, why)
}
