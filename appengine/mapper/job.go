...
func (j *Job) FetchInfo(c context.Context) (*JobInfo, error) {
	...
	updated := j.Updated // will be max(ETA of each shard)
	...
	// since it will also sum rate estimation errors too.
	info.Updated = google.NewTimestamp(updated)
	info.EntitiesPerSec = float32(float64(info.ProcessedEntities) / updated.Sub(j.Created).Seconds())
...
// ToInfo returns a proto message with information about the shard.
func (s *shard) ToInfo() *ShardInfo {
	var rate float64
	var eta *timestamp.Timestamp

	if s.ProcessedCount > 0 {
		runtime := s.Updated.Sub(s.Created)
	...
	return &ShardInfo{
		...
		Eta:               eta, // may be nil if unknown
		ProcessedEntities: s.ProcessedCount,
		TotalEntities:     s.ExpectedCount, // may be -1 if unknown
		EntitiesPerSec:    float32(rate),
	}
}
