...
type Progress struct {
	...
	ETA       time.Duration // expected time until completion or -1 if unknown
}

func (p *Progress) calcRateAndETA() {
	...
	if left := p.Total - p.Processed; left > 0 {
		p.ETA = time.Duration(float64(left) * float64(time.Second) / p.Rate)
	} else {
		p.ETA = -1
	}
}
...
func (j *Job) FetchProgress(c context.Context) (*JobProgress, error) {
	...
	jobP.calcRateAndETA() // ... but mostly rate
	...
	if jobP.Total != -1 {
		...
		for _, s := range jobP.Shards {
			if s.ETA != -1 && (jobP.ETA == -1 || s.ETA > jobP.ETA) {
				jobP.ETA = s.ETA
			}
		}
	}
