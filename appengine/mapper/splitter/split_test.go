...
func TestSplitIntoRanges(t *testing.T) {
	Convey("Works", t, func() {
		...
		getRanges := func(q *datastore.Query, shards int, oversampling float64, max int) []intRange {
		...
		getShardSizes := func(q *datastore.Query, shards int, oversampling float64) []int {
			ranges, err := SplitIntoRanges(c, q, Params{
				Shards:             shards,
				OversamplingFactor: oversampling,
			})
		...
		Convey("With evenly distributed int keys", func() {
			...
			// closer to 512).
			So(getRanges(q, 2, 1.0, maxID), ShouldResemble, []intRange{
				{-1, 399, 399},
				{399, -1, 625},
			})
			So(getRanges(q, 2, 32.0, maxID), ShouldResemble, []intRange{
				{-1, 476, 476},
				{476, -1, 548},
			})
			So(getRanges(q, 2, 128.0, maxID), ShouldResemble, []intRange{
				{-1, 489, 489},
				{489, -1, 535},
			})
			...
			So(getRanges(q, 3, 1.0, maxID), ShouldResemble, []intRange{
				{-1, 265, 265},
				{265, 399, 134},
				{399, -1, 625},
			})
			So(getRanges(q, 3, 32.0, maxID), ShouldResemble, []intRange{
				{-1, 285, 285},
				{285, 696, 411},
				{696, -1, 328},
			})
			So(getRanges(q, 3, 128.0, maxID), ShouldResemble, []intRange{
				{-1, 327, 327},
				{327, 658, 331},
				{658, -1, 366},
			})
		...
		Convey("With normally distributed int keys", func() {
			...
			So(getShardSizes(q, 1, 1.0), ShouldResemble, []int{len(seen)})

			// Shards have ~ even sizes (in terms of number of points there).
			shards := getShardSizes(q, 10, 32.0)
			...
			So(getRanges(q, 10, 32.0, 0), ShouldResemble, []intRange{
				{-1, 47124, 47124},
				{47124, 48158, 1034},
				{48158, 48925, 767},
				{48925, 49363, 438},
		...
		Convey("Handles ancestor filter", func() {
			...
			So(getShardSizes(q, 4, 32.0), ShouldResemble, []int{
				2, 2, 4, 4,
			})

			// With ancestor query, discovers only entities that match the query.
			So(getShardSizes(q.Ancestor(root1), 4, 32.0), ShouldResemble, []int{
				1, 1, 2, 0, // 4 total
			})
			So(getShardSizes(q.Ancestor(root2), 4, 32.0), ShouldResemble, []int{
				4, 1, 2, 1, // 8 total
			})
		})
		...
		Convey("Handles arbitrary keys", func() {
			...
			So(getShardSizes(q, 8, 32.0), ShouldResemble, []int{
