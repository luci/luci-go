...
// MapperFactory knows how to construct instances of Mapper.
//
// MapperFactory is supplied by the users of the library and registered in the
// controller via RegisterMapperFactory call.
//
...
type MapperFactory func(c context.Context, j *Job, shardIdx int) (Mapper, error)
...
// RegisterMapperFactory adds the given mapper factory to the internal registry.
//
...
func (ctl *Controller) RegisterMapperFactory(id ID, m MapperFactory) {
	ctl.m.Lock()
	defer ctl.m.Unlock()
	...
	if ctl.mappers == nil {
		ctl.mappers = make(map[ID]MapperFactory, 1)
	}
	ctl.mappers[id] = m
}

// getMapperFactory returns a registered mapper factory or an error.
//
// Grabs the reader lock inside. Can return only fatal errors.
func (ctl *Controller) getMapperFactory(id ID) (MapperFactory, error) {
	ctl.m.RLock()
	defer ctl.m.RUnlock()
...
func (ctl *Controller) initMapper(c context.Context, j *Job, shardIdx int) (Mapper, error) {
	f, err := ctl.getMapperFactory(j.Config.Mapper)
	if err != nil {
...
func (ctl *Controller) LaunchJob(c context.Context, j *JobConfig) (JobID, error) {
	...
	if _, err := ctl.getMapperFactory(j.Mapper); err != nil {
		return 0, errors.Annotate(err, "bad job config").Err()
	}
