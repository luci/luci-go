...
var procCache = caching.RegisterProcessCache(100)
...
func (s *storeImpl) GetSecret(k secrets.Key) (secrets.Secret, error) {
	secret, err := procCache.LRU(s.ctx).GetOrCreate(s.ctx, s.cfg.Prefix+":"+string(k), func() (interface{}, time.Duration, error) {
