...
func TestTestable(t *testing.T) {
	...
	Convey("With dispatcher", t, func() {
		...
		Convey("GetScheduledTasks works", func() {
			...
			// Have them.
			popped := tst.GetScheduledTasks()
			So(len(popped), ShouldEqual, 4)

			// Correct order. First to execute are in front.
			So(popped[0].Payload, ShouldResemble, &duration.Duration{Seconds: 3})
			So(popped[1].Payload, ShouldResemble, &duration.Duration{Seconds: 4})
			So(popped[2].Payload, ShouldResemble, &duration.Duration{Seconds: 2})
			So(popped[3].Payload, ShouldResemble, &duration.Duration{Seconds: 1})
		})
...
func TestRunSimulation(t *testing.T) {
	...
	Convey("Setup", t, func() {
		...
		addTask := func(c context.Context, idx int64, delay time.Duration, dedup string) {
			d.AddTask(c, &tq.Task{
				Payload:          &duration.Duration{Seconds: idx},
		...
		toIndexes := func(arr []Task) []int64 {
			...
			for _, task := range arr {
				out = append(out, task.Payload.(*duration.Duration).Seconds)
			}
			return out
		}
		...
		errorOnIdx := -1
		handler := func(c context.Context, payload proto.Message) error {
			switch payload.(*duration.Duration).Seconds {
		...
		Convey("Happy path", func() {
			...
			// Task executed in correct sequence and deduplicated task is skipped.
		...
		Convey("Error", func() {
			...
			So(toIndexes(pending), ShouldResemble, []int64{})
			// Only two executed. Last one errored.
			So(toIndexes(executed), ShouldResemble, []int64{1, 2})
			// The clock matches last executed task.
			So(clock.Now(ctx).Sub(epoch), ShouldEqual, time.Second)
		})
