...
type Testable interface {
	...
	//
	// Returned tasks are sorted by ETA (oldest first) and Name.
	...
	//
	// Panics if there's no test clock in the context.
	//
	// It stops whenever any of the following happens:
	//   * The queue of pending tasks is empty.
	//   * ETA of the next task is past deadline.
...
//
// BEWARE: There's are no compile time type checks here. If you add a method or
...
func (l tasksList) Less(i, j int) bool {
	return sortby.Chain{
		func(i, j int) bool { return l[i].Task.ETA.Before(l[j].Task.ETA) },
		func(i, j int) bool { return l[i].Task.Name < l[j].Task.Name },
	}.Use(i, j)
}
...
loop:
	for {
		oldest, queue := t.pickOldestETA()
		switch {
		case oldest == nil:
			break loop // no more tasks
		case !deadline.IsZero() && oldest.Task.ETA.After(deadline):
			break loop // deadline reached
		}

		if err = taskqueue.Delete(c, queue, oldest.Task); err != nil {
			panic("impossible, the task must be in the queue")
		}
		executed = append(executed, *oldest)

		tc.Set(oldest.Task.ETA)
		if err = t.ExecuteTask(c, *oldest, nil); err != nil {
			break
		}
	}
...
func (t *testableImpl) pickOldestETA() (oldest *Task, queue string) {
	...
	// should be modified to return oldest task directly (it can pick it more
	...
	for q, tasks := range t.tqt.GetScheduledTasks() {
		for _, task := range tasks {
			...
			if oldest == nil || (tasksList{tt, *oldest}).Less(0, 1) {
				oldest = &tt
				queue = q
			}
		}
