...
type State struct {
	...
	//
	// It will be mapped to some free task number via TaskNumAllocator.
...
// NotifyTaskIsAlive), its task number will be reused by some other (new) task.
...
const (
	// noFlushErrorTreshold defines when we start to complain in error log that
	// the last successful flush (if ever) was too long ago.
	noFlushErrorTreshold = 5 * time.Minute
...
func (s *State) enableTsMon(c context.Context) {
	t := s.Target(c)
	t.TaskNum = -1 // will be assigned later via TaskNumberMapper
	...
	// invalid logging that the last flash was long time ago.
...
func (s *State) flushIfNeeded(c context.Context, req *http.Request, state *tsmon.State, settings *tsmonSettings) {
	...
	if err != nil {
		...
		if sinceLastFlush := now.Sub(lastFlush); sinceLastFlush > noFlushErrorTreshold {
...
func (s *State) ensureTaskNumAndFlush(c context.Context, taskID string, state *tsmon.State, settings *tsmonSettings) error {
	...
	// Notify the task number assigner that we are still alive and grab the
	// TaskNum assigned to the currently running process.
