...
func TestScan(t *testing.T) {
	...
	Convey("Scan works", t, func() {
		...
		Convey("Normal operation", func() {
			...
			savedReminders := []*Reminder{
				&Reminder{
					Id:         "01",
					FreshUntil: fresh,
					Payload:    genReminder(ctx).Payload,
				},
				&Reminder{
					Id:         "02",
					FreshUntil: stale,
		...
		Convey("Abnormal operation", func() {
			...
			Convey("Level-3 not allowd", func() {
			...
			// TODO(tandrii): find a way to expire ctx withing FetchReminderMeta for a
			// realistic test.
			...
			Convey("Timeout without anything fetched", func() {
				...
				db.EXPECT().FetchReminderMeta(ctx, gomock.Any(), gomock.Any(), gomock.Any()).
