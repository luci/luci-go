...
func TestMakeReminder(t *testing.T) {
	...
	Convey("makeReminder", t, func() {
		...
		req := taskspb.CreateTaskRequest{
			...
			Task: &taskspb.Task{
				MessageType: &taskspb.Task_HttpRequest{HttpRequest: &taskspb.HttpRequest{
					...
					Url:        "https://exaple.com/user/exec",
				}},
		...
		Convey("typical request with deadline", func() {
			reqCtx, cancel := clock.WithTimeout(ctx, 5*time.Second)
			defer cancel()
			...
			So(r.FreshUntil, ShouldEqual, testclock.TestRecentTimeLocal.Add(5*time.Second).UTC())
	...
	Convey("AddTask works", t, func() {
		...
		Convey("with deadline", func() {
			...
			Convey("no time to complete happy path", func() {
				...
				So(len(db.reminders), ShouldEqual, 1)
				So(latestOf(metricPostProcessedAttempts, "OK", "happy", "fake"), ShouldBeNil)
			})
			...
			Convey("happy path", func() {
				...
				So(latestOf(metricPostProcessedAttempts, "OK", "happy", "fake"), ShouldEqual, 1)
				So(latestOf(metricPostProcessedDurationsMS, "OK", "happy", "fake").(*distribution.Distribution).Count(), ShouldEqual, 1)
			...
			Convey("happy path transient task creation error", func() {
				...
				pp(ctx)
				So(len(db.reminders), ShouldEqual, 1)
				So(latestOf(metricPostProcessedAttempts, "fail-task", "happy", "fake"), ShouldEqual, 1)
			})
			Convey("happy path permanent task creation error", func() {
				fakeCT.errs = []error{status.Errorf(codes.InvalidArgument, "forever")}
				pp(ctx)
				So(latestOf(metricPostProcessedAttempts, "ignored-bad-task", "happy", "fake"), ShouldEqual, 1)
...
func (_ *ramDB) Kind() string { return "fake" }
