...
func main() {
	...
	server.Main(nil, modules, func(srv *server.Server) error {
		...
		d := ttq.NewDatastore(qTTQ, host+"/handle/ttq", ctc)

		srv.Routes.POST("/hit-me/:opt", router.MiddlewareChain{}, func(ctx *router.Context) {
			metricTaskExecuted.Add(ctx.Context, 1, ctx.Params.ByName("opt"), ttq.DbDatastore)
		...
		srv.Routes.GET("/trans/:opt", router.MiddlewareChain{}, func(rctx *router.Context) {
			...
			metricTaskCreated.Add(rctx.Context, 1, opt, ttq.DbDatastore)
			if opt == "unhappy" {
