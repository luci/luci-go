...
type Database interface {
	// Kind is identifies this particular database implementation.
	//
	// Among other things it is used in Cloud Tasks messages used to implement
	// distributed sweeping. Changes should be done carefully.
	Kind() string
...
// Impl knows how instantiate Database instances.
type Impl struct {
	// Kind identifies this particular database implementation.
	//
	// Must match Kind() of the produces Database instances.
	Kind string

	// ProbeForTxn "probes" a context for an active transaction, returning a DB of
	// the corresponding kind that can be used to transactionally submit
	// reminders.
	...
	// transactions.
	//
	// This is used by sweeper to enumerate reminders.
