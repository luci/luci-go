...
// SweepOptions configures the internal sweeping process.
//
...
//
// Sweeping processes ensures all transactionally committed tasks will have
...
// database.
//
// Default values are chosen s.t. sweeping can keep up temporary hiccups
// resulting in 5000 QPS creation rate of stale user tasks, which implies these
// constraints:
...
//
// Experiments show:
//  (a, b) handling TasksPerSweepShard=2048 takes ~6s.
...
type SweepOptions struct {
	...
	// The queue must already exist with throughput of at least 10 QPS.
	//
...
func (s SweepOptions) applyDefaults() (SweepOptions, error) {
	r := s // copy
	if s.Queue == "" {
		return r, errors.New("Queue is required")
	}

	switch {
	case s.ScanInterval == 0:
		r.ScanInterval = time.Minute
	case s.ScanInterval < time.Minute:
		return r, errors.New("ScanInterval must be at least 1 Minute")
	default:
		r.ScanInterval = s.ScanInterval.Truncate(sweepScanIntervalGranularity)
	}
	switch {
	case s.Shards == 0:
		...
		r.Shards = 16
	case s.Shards > 100:
		// >128 is a sign of desperate attempt to process backlog w/o
		// undrestanding. Chances are that the real problem is somewhere else, say a
		// misconfigured helper Queue.
		return r, errors.New("Shards must be in [0..100] range")
	}
	if s.TasksPerSweepShard == 0 {
		r.TasksPerSweepShard = 2048
	}
	return r, nil
}
