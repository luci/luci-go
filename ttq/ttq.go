...
type Database interface {
	...
	FetchReminderKeys(ctx context.Context, low, high string) ([]*internal.Reminder, error)
	FetchRemainingReminders(context.Context, []*internal.Reminder) ([]*internal.Reminder, error)
	...
	ReturnLease(context.Context, *internal.Lease, int) error
...
func (s *TTQ) sweepItems(ctx context.Context, shardId, level int, items []*internal.Reminder) error {
	desired := internal.PartitionSpanning(items)
...
func (s *TTQ) sweepOneBatch(ctx context.Context, batch []*internal.Reminder) error {
	now := clock.Now(ctx)
	batch, err := s.db.FetchRemainingReminders(ctx, batch)
...
func (s *TTQ) withLease(ctx context.Context, desired *internal.Partition, shardId int,
	...
	default:
		...
		defer func() {
			if err := s.db.ReturnLease(ctx, l, shardId); err != nil {
...
type Lease struct {
	...
	Parts     []string  `gae:",noindex"` // each el is a internal.Partition.String()
