...
//
// It can also run locally, but it still needs real Cloud Datastore, so you need
// to provide some Cloud Project name. Note that it will create real entities
// there which may interfere with production server/tq deployment in this
...
func (c *CountDownChain) Register(disp *tq.Dispatcher) {
	...
	c.dispatcher.RegisterTaskClass(tq.TaskClass{
		...
		// This is used for deserilization and also for discovery of what ID to use
		// when submitting tasks. Changing it safe as along as JSONPB representation
		// of in-flight task still matches the new proto.
		...
		Queue: "tq-countdown-tasks",

		// Handler will be called to handle the previously submitted task. It
		// can also be attached later (perhaps even in a different package) via
		// AttachHandler, if necessary.
...
func main() {
	...
	// tasks. Tests usually create their own separate dispatcher, since using
	// the global one makes tests collide with one another. See main_test.go.
