...
func (p *structPLS) save(propMap PropertyMap, prefix string, parentST *structTag, is IndexSetting) (idxCount int, err error) {
	saveProp := func(name string, si IndexSetting, v reflect.Value, st *structTag) (err error) {
		...
		if st.isSlice || (parentST != nil && parentST.isSlice) {
			if st.convertMethod == convertProto {
				return errors.New("slice of proto.Message is not supported")
			}
...
func getStructCodecLocked(t reflect.Type) (c *structCodec) {
	...
	for i := range c.byIndex {
		...
		case ft.Implements(typeofProtoMessage):
			...
			switch opts {
			case "": // default.
				st.protoOption = protoOption("compress")
			case "compress", "nocompress", "legacy":
				st.protoOption = protoOption(opts)
			// This package historically ignored unsupported options.
			// However, it is expected that new variotions of "compress" and possibly
			// "legacy" will be added in the future. Therefore, explicitly disallow
			// all other options.
			case "noindex":
