...
func TestIteratorHeap(t *testing.T) {
	...
	Convey("iteratorHeap", t, func() {
		...
		heap.Init(h)
		heap.Push(h, &iteratorEntry{order: "bb", iterator: &queryIterator{}})
		heap.Push(h, &iteratorEntry{order: "aa", iterator: &queryIterator{}})
		So(h.Len(), ShouldEqual, 2)

		var res []*iteratorEntry
		for h.Len() > 0 {
			res = append(res, heap.Pop(h).(*iteratorEntry))
		}
		So(res, ShouldResemble, []*iteratorEntry{
			{order: "aa", iterator: &queryIterator{}},
			{order: "bb", iterator: &queryIterator{}},
		})
	})
}
	...
	Convey("Test RunMulti", t, func() {
		...
		Convey("ok", func() {
			Convey("default - key ascending", func() {
				...
				var foos []*Foo
				err := RunMulti(c, queries, func(foo *Foo, getCursor CursorCB) error {
