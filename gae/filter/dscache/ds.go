...
func (d *dsCache) GetMulti(keys []*ds.Key, metas ds.MultiMetaGetter, cb ds.GetMultiCB) error {
	...
	lockItems, err := d.impl.TryLockAndFetch(d.c, itemKeys, nonce)
	if err != nil {
...
func (d *dsCache) RunInTransaction(f func(context.Context) error, opts *ds.TransactionOptions) error {
	...
	// TODO(vadimsh): Should we delete locks even on txn failure? There is
	// almost no harm in deleting them. But keeping them for long has cost on
	// Get performance.
	if err == nil {
