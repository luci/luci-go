	...
	if !p.empty() {
		...
		err := d.RawInterface.GetMulti(p.toGet, p.toGetMeta, func(j int, pm ds.PropertyMap, err error) {
			...
			if pm == nil {
				...
				toSave.PromoteToData(d.impl.AllocBuffer(0), expiry)
			} else {
				// Serialize and compress the PropertyMap, bail if too large.
				buf, err := encodeItemValue(pm, d.impl.AllocBuffer)
				if err == nil && buf.Len() > internalValueSizeLimit {
					err = errors.Reason("encoded entity too big (%d > %d)", buf.Len(), internalValueSizeLimit).Err()
				}
				if err == nil {
