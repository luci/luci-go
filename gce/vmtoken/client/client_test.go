...
// handler implements http.Handler to handle *http.Requests.
type handler struct {
	// Handler is called by ServeHTTP.
	Handler func(req *http.Request)
}

// ServeHTTP handles an *http.Request. Panics on error. Implements http.Handler.
		...
		meta := metadata.NewClient(&http.Client{
			Transport: &roundtripper.StringRoundTripper{
				Handler: func(req *http.Request) (int, string) {
					...
					So(err, ShouldBeNil)
					So(req.URL.Query().Get("audience"), ShouldEqual, url.Host)
