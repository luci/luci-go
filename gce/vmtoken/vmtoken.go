...
// the token (as it was supplied to the GCE metadata endpoint when generating
// the token).
type Payload struct {
...
func Verify(c context.Context, jwt string) (*Payload, error) {
	...
	// are already cached in the process memory.
...
func verifyImpl(c context.Context, jwt string, certs signatureChecker) (*Payload, error) {
	...
	if err = certs.CheckSignature(hdr.Kid, []byte(chunks[0]+"."+chunks[1]), sig); err != nil {
		return nil, fmt.Errorf("bad JWT - bad signature - %s", err)
	}
	...
	var payload struct {
		...
		Exp           int64  `json:"exp"`            // "expiry", as unixtimestamp
		Iat           int64  `json:"iat"`            // "issued at", as unix timestampa
	...
	if err = unmarshalB64JSON(chunks[1], &payload); err != nil {
		return nil, fmt.Errorf("bad JWT payload - %s", err)
	}
	...
	if payload.Google.ComputeEngine.ProjectID == "" {
		return nil, fmt.Errorf("no google.compute_engine in the GCE VM token, use 'full' format")
	}
	...
	case now < payload.Iat-allowedDriftSec:
		return nil, fmt.Errorf("bad JWT: too early (now %d < iat %d)", now, payload.Iat)
	case now > payload.Exp+allowedDriftSec:
		return nil, fmt.Errorf("bad JWT: expired (now %d > exp %d)", now, payload.Exp)
	}
