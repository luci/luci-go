...
// pythonDefaults is a map of the default python that runs the Swarming bot process.
var pythonDefaults = map[string]string{
	"linux":   "/usr/bin/python",
	"windows": "C:\\tools\\python\\bin\\python.exe",
}
...
func newConnectCmd() *subcommands.Command {
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			cmd.Flags.StringVar(&cmd.user, "python", pythonDefaults[runtime.GOOS], "Path to the python to start the Swarming bot process.")
			return cmd
		},
	}
}
