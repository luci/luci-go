...
func fetch(c context.Context) (*Config, error) {
	...
	vms := &gce.Configs{}
	switch cfg, err := cli.GetConfig(c, set, vmsFile, false); {
	...
	kinds := &gce.Kinds{}
	switch cfg, err := cli.GetConfig(c, set, kindsFile, false); {
	case err == config.ErrNoConfig:
	...
	default:
		...
		if rev != "" && cfg.Revision != rev {
			// If the VMs config was fetched at some revision, kinds must not
			// be fetched at a different revision because VMs may depend on
			// kinds. The exception is if kinds is not found at all. Then VMs'
			// dependence on kinds is easily checked during validation.
		...
		for i, m := range v.GetAttributes().Metadata {
			if m.GetFromFile() != "" {
				...
				if _, ok := fileMap[file]; !ok {
					...
					if cfg.Revision != rev {
						return errors.Reason("config revision mismatch %q", cfg.Revision).Err()
					}
