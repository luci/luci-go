...
The deadline is how long an instance has to live. An instance's deadline is
derived from the lifetime in the Config and the instance creation time. Once the
deadline is up, the backend package will attempt to replace the instance.
Replacing the instance is how changes to VMs are picked up, since instances are
immutable.
