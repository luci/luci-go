...
type InstanceCount struct {
	...
	Counts []instanceCount `gae:"counts"`
}
...
func UpdateInstances(c context.Context) {
	now := clock.Now(c)
	if err := datastore.Run(c, datastore.NewQuery("InstanceCount"), func(n *InstanceCount) {
		for _, count := range n.Counts {
			if now.Sub(n.Computed) > 10*time.Minute {
				...
				if err := datastore.Delete(c, n); err != nil {
					// Log the error, but continue sending the rest of the metrics.
					// The deletion will be retried next time this is called.
					errors.Log(c, errors.Annotate(err, "failed to delete count %q", n.Prefix).Err())
				}
				continue
			}
