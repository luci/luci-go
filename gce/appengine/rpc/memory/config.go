...
// Package memory contains implementations of servers in its parent package backed by in-memory storage.
package memory
...
func (srv *Config) EnsureVMs(c context.Context, req *config.EnsureVMsRequest) (*config.Block, error) {
	if srv.vms == nil {
		srv.vms = make(map[string]*config.Block)
	}
	srv.vms[req.GetId()] = req.GetVms()
