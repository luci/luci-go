	...
	Convey("ToTime", t, func() {
		Convey("invalid", func() {
			Convey("location", func() {
				...
				_, err := t.ToTime()
				So(err, ShouldErrLike, "location must be a valid IANA location")
			})
		...
		Convey("invalid", func() {
			...
			Convey("day", func() {
				...
				Convey("unspecified", func() {
					...
					t.Validate(c)
					errs := c.Finalize().(*validation.Error).Errors
					So(errs, ShouldContainErr, "day must not be unspecified")
				})
			})
			...
			Convey("location", func() {
				...
				t.Validate(c)
				errs := c.Finalize().(*validation.Error).Errors
				So(errs, ShouldContainErr, "location must be a valid IANA location")
			})
		})
