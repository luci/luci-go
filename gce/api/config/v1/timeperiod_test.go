...
func TestTimePeriod(t *testing.T) {
	...
	Convey("parseDuration", t, func() {
		Convey("invalid", func() {
			Convey("mismatch", func() {
				_, err := parseDuration("1yr")
				So(err, ShouldErrLike, "duration did not match regex")
			})

			Convey("overflow", func() {
				_, err := parseDuration("9223372036854775808s")
				So(err, ShouldErrLike, "invalid int64")
			})
		})
		...
		Convey("invalid", func() {
			...
			Convey("duration", func() {
				Convey("invalid", func() {
					...
					tp.Validate(c)
					errs := c.Finalize().(*validation.Error).Errors
					So(errs, ShouldErrLike, "invalid duration")
				})
				...
				Convey("negative", func() {
					...
					tp.Validate(c)
					errs := c.Finalize().(*validation.Error).Errors
					So(errs, ShouldErrLike, "duration must be positive")
				})
			})
