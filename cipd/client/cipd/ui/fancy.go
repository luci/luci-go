...
func (a *fancyActivity) renderLine(advance bool, width int) (line string, active bool) {
	...
	if logMessage != "" {
		...
		// Pad it with spaces on the right to align progress bars.
		if strLen < 34 {
			writeString(strings.Repeat(" ", 34-strLen))
		}
		...
		// Render percents as numbers too. Mostly to have something dynamic on
