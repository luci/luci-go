...
// Sort orders the entries according to the canonical order.
...
func (d *ClientDigestsFile) Serialize(w io.Writer, version, versionFile string) error {
	_, err := iotools.WriteTracker(w, func(w io.Writer) error {
		...
		fmt.Fprintf(w, "#   cipd selfupdate -version-file %q -generate-digests\n", versionFile)
		fmt.Fprintf(w, "# For the CIPD client version:\n")
		fmt.Fprintf(w, "#   %s\n", version)
		fmt.Fprintf(w, "# Do not modify manually. All changes will be overwritten.\n")
		fmt.Fprintf(w, "# To change, modify %q and regenerated this file.\n\n", versionFile)
		for _, e := range d.entries {
			fmt.Fprintf(w, "%s\t%s\t%s\n",
				e.plat,
...
//
// Unrecognized algorithms are silently skipped.
