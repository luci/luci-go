...
// file. Its presence of correctness is not checked.
func LoadEnsureFile(path string) (*File, error) {
	...
	parsedFile, err := ParseFile(f)
	if err != nil {
		return nil, err
	}

	if parsedFile.ResolvedVersions != "" {
		parsedFile.ResolvedVersions = filepath.Join(basePath,
			filepath.FromSlash(parsedFile.ResolvedVersions))
	}
