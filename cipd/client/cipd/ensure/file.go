...
// versions of these.
//
// The given expander (usually template.DefaultExpander()) is used to verify
// all referenced template variables exist.
func ParseFile(r io.Reader, expander template.Expander) (*File, error) {
...
// resolver, if you like). Returning a Pin with empty InstanceID causes
// File.Resolve to silently omit the package from the resolved file.
//
...
func (f *File) Resolve(rslv VersionResolver, expander template.Expander) (*ResolvedFile, error) {
	...
	for _, p := range resolved {
		...
		// InstanceID may be "" if the resolver asked us to skip the package.
		if p.pin.InstanceID != "" {
