...
type Host struct {
	testServeErr error // non-nil if test to simulate srv.Serve error
...
func (h *Host) LaunchPlugin(ctx context.Context, args []string, ctrl *Controller) (*Process, error) {
	...
	// Wait for the server to start, since we need its port now.
	<-serverReady
...
func (h *Host) Close(ctx context.Context) {
	...
	h.closed = true
	h.srvErr = errors.Reason("the plugin host is already closed").Err()
	if h.srv != nil {
		h.srv.Stop()
		h.srv = nil
	}
...
func (h *Host) pluginForRPC(ctx context.Context) (*Process, error) {
	...
	ticket := md.Get(ticketMetadataKey)
	if len(ticket) == 0 {
		return nil, status.Errorf(codes.Unauthenticated, "no ticket in the request")
	}
	h.m.Lock()
	defer h.m.Unlock()
	if plugin, _ := h.plugins[ticket[0]]; plugin != nil {
		return plugin, nil
	}
...
// Terminate tries to gracefully terminate the plugin, killing it with SIGKILL
// on the context timeout or after 5 sec.
//
// Returns the same value as Err().
