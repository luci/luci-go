...
// makeExclusionFilter produces a predicate that checks an relative file path
...
func makeExclusionFilter(patterns []string) (fs.ScanFilter, error) {
	...
	// Compile regular expressions. Note that we want to validate that each
	// individual pattern is a valid regexp, and for that reason we don't just
	// concatenate them in a single uber-regexp.
	exps := []*regexp.Regexp{}
