...
// `Realize` on it concurrently to get the cached file. Only one will actually
// do the work, the rest will just wait for the first one to finish.
...
//
// If the caller is the first one to call `Realize`, the supplied `write`
// callback will be used to actually populate the file. If there's another
// concurrent `Realize` call elsewhere, the caller will just block until the
// first `Realize` operation completes. If `Realize` was completed already,
// the caller will just get the cached result (either a pkg.Source or an error).
func (a *Allocation) Realize(ctx context.Context, write func(*os.File) error) (pkg.Source, error) {
	a.m.Lock()
	owner := !a.realized
	a.realized = true
	a.m.Unlock()
	...
	var err error
	if owner {
		err = a.resolve(a.cache.openOrFetch(ctx, a.pin, write))
	} else {
		err = a.wait(ctx)
	}
...
type cacheFile struct {
	closed int32       // >0 if was already closed
...
func (f *cacheFile) Close(ctx context.Context, corrupt bool) error {
	if atomic.AddInt32(&f.closed, 1) == 1 {
...
func (c *InstanceCache) openOrFetch(ctx context.Context, pin common.Pin, write func(*os.File) error) (*os.File, error) {
