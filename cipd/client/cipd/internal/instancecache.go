...
type InstanceRequest struct {
	...
	Open    bool            // true to return it as a pkg.Instance
...
type InstanceCache struct {
	...
	fetchQueue   chan func()
	fetchWG      sync.WaitGroup
	fetchPending int32
	fetchResults chan *InstanceResult
...
func (c *InstanceCache) Launch(ctx context.Context) {
	...
	c.fetchQueue = make(chan func(), 10000)
	c.fetchResults = make(chan *InstanceResult, 10)

	c.fetchWG.Add(1)
	go func() {
		defer c.fetchWG.Done()
		for f := range c.fetchQueue {
			f()
		}
	}()
}
