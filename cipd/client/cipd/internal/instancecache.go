...
func (c *InstanceCache) Launch(ctx context.Context) {
	...
	logging.Infof(ctx, "Using%s instance cache at %q", tmp, c.FS.Root())

	// This is an effectively unlimited buffer. At very least it should be large
	// enough to hold all instances submitted by RequestInstances(...) before
...
func (c *InstanceCache) Close(ctx context.Context) {
	...
	if c.Tmp {
		if err := os.RemoveAll(c.FS.Root()); err != nil {
			logging.Warningf(ctx, "Leaking temp directory %q: %s", c.FS.Root(), err)
		}
	}
}
...
func (c *InstanceCache) openOrFetch(ctx context.Context, pin common.Pin) (*os.File, error) {
	...
	for {
		...
		case err != nil:
			...
			logging.Warningf(ctx, "Could not get %q from the cache: %s", pin, err)
		default:
			logging.Infof(ctx, "Instance cache hit for %q", pin)
		...
		case os.IsNotExist(err) && attempt == 1:
			logging.Infof(ctx, "Pin %q is still not in the cache, retrying...", pin)
		case os.IsNotExist(err) && attempt > 1:
			logging.Errorf(ctx, "Pin %q is unexpectedly missing from the cache", pin)
			return nil, err
		case err != nil:
			logging.Errorf(ctx, "Failed to open the instance file %q: %s", pin, err)
			return nil, err
		default:
