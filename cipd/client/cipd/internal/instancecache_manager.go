...
type InstanceRequest struct {
	...
	ServiceURL string             // the service URL to fetch from
...
type InstanceResult struct {
	...
	ServiceURL string          // the original service URL from the InstanceRequest
...
func (c *ManagedInstanceCache) handleRequest(req *InstanceRequest) *InstanceResult {
	...
	for i, cache := range c.Caches {
		switch src, fetched, err := cache.OpenAsSource(ctx, req.ServiceURL, pin); {
		case err != nil:
