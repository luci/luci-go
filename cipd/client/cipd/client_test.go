...
func TestMaybeUpdateClient(t *testing.T) {
	...
	Convey("With temp dir", t, func(c C) {
		...
		writeClientBin := func(body string, algo api.HashAlgo) *api.ObjectRef {
			So(ioutil.WriteFile(clientBin, []byte(body), 0777), ShouldBeNil)
			return caclObjRef(body, algo)
		}

		readClientBin := func() string {
			body, err := ioutil.ReadFile(clientBin)
			So(err, ShouldBeNil)
			return string(body)
		}
		...
		Convey("MaybeUpdateClient updates outdated client, warming caches", func() {
			...
			upToDateRef := caclObjRef("up-to-date", api.HashAlgo_SHA256)

			writeClientBin("outdated", api.HashAlgo_SHA256)
			...
			// Yep, updated.
			So(readClientBin(), ShouldEqual, "up-to-date")
