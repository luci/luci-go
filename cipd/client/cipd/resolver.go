...
// actually exist.
//
// The instance of Resolver is stateful. It caches result of resolution and
// verification, so subsequent attempts to resolve/verify same pins are fast.
//
...
func (r *Resolver) ResolvePackage(ctx context.Context, pkg, ver string) (common.Pin, error) {
	...
	// If the version already looks like an instance ID just verify it exists.
	if common.ValidateInstanceID(ver, common.AnyHash) == nil {
		pin := common.Pin{PackageName: pkg, InstanceID: ver}
		if err := r.verifyPin(ctx, pin); err != nil {
			return common.Pin{}, err
		}
		return pin, nil
	}

	// Otherwise resolve it into the IID. Note that ResolveVersion verifies that
	// the resolved pin exists, so skip 'verifyPin' step for such resolved pins.
	return r.resolveVersion(ctx, pkg, ver)
}
...
func (r *Resolver) ResolveEnsureFile(ctx context.Context, file *ensure.File, expander template.Expander) (*ensure.ResolvedFile, error) {
	...
	ret, err := file.Resolve(func(pkg, ver string, def ensure.PackageDef) (common.Pin, error) {
		...
		if err != nil {
			mu.Lock()
			fails = append(fails, fail{def, err})
			mu.Unlock()
...
func (r *Resolver) resolveVersion(ctx context.Context, pkg, ver string) (common.Pin, error) {
	...
	promise := r.resolving.Get(ctx, unresolved, func(ctx context.Context) (interface{}, error) {
		...
		if err != nil {
			logging.Errorf(ctx, "Failed to resolve package %s: %s", unresolved, err)
		} else {
...
func (r *Resolver) verifyPin(ctx context.Context, pin common.Pin) error {
	promise := r.verifying.Get(ctx, pin, func(ctx context.Context) (interface{}, error) {
		...
		if err != nil {
			logging.Errorf(ctx, "Failed to resolve instance info for %s: %s", pin, err)
		}
		return nil, err
	})
