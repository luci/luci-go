...
type ScanOptions struct {
	...
	// SiteServiceDir is a name of a directory with CIPD guts to skip
	//
	// Usually .cipd.
	SiteServiceDir string
}

// siteServiceDir is an absolute path to SiteServiceDir or "" if unset.
func (s *ScanOptions) siteServiceDir(root string) string {
	if s.SiteServiceDir == "" {
		return ""
	}
	return filepath.Join(root, s.SiteServiceDir)
}
...
func ScanFileSystem(dir string, root string, exclude ScanFilter, scanOpts ScanOptions) ([]File, error) {
	...
	svcDir := scanOpts.siteServiceDir(root)
...
func WrapFile(abs string, root string, fileInfo *os.FileInfo, scanOpts ScanOptions) (File, error) {
	...
	if info.Mode()&os.ModeSymlink != 0 {
		...
		if svcDir := scanOpts.siteServiceDir(root); svcDir != "" && IsSubpath(targetAbs, svcDir) {
			abs = targetAbs
