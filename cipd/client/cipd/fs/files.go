...
//
// It receives a  path of the file being judged and returns true if it should be
// skipped. The path is a filepath relative to the starting scanning directory
// (i.e. 'dir' in ScanFileSystem).
...
//
// It skips files and directories for which 'exclude(path relative to dir)'
...
// applying), except when 'dir == root' (lexicographically, as paths). In that
// case the symlink is followed and whatever it points to is used as the new
// 'dir' and 'root'. This is completely transparent in the output though, since
// File uses only relative paths. Without this exception using a symlink as
// a package root leads to very surprising errors.
func ScanFileSystem(dir string, root string, exclude ScanFilter, scanOpts ScanOptions) ([]File, error) {
	...
	// If we are scanning 'root' directly, it doesn't matter if it is a symlink.
	// We should never try to package it as such, it is useless.
	if dir == root {
