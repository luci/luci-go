...
type FileSystem interface {
	...
	// CaseSensitive returns true if the file system FileSystem operates in is
	// case-sensitive.
...
func (f *fsImpl) CaseSensitive() (bool, error) {
	f.once.Do(func() {
		f.caseSens, f.caseSensErr = func() (bool, error) {
			...
			default:
				return false, fmt.Errorf("cannot stat file when case-sensitivity of %q - %s", f.root, err)
			}
		}()
	})
