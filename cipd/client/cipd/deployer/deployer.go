...
func (d *deployerImpl) addToSiteRoot(ctx context.Context, subdir string, files []pkg.FileInfo, installMode pkg.InstallMode, pkgDir, srcDir string) (*pathTree, error) {
	// Build a set of all added files (including intermediary directories). It
	// will be used to ensure all intermediary files system nodes are directories,
	// not symlinks (more about this below).
	touched := &pathTree{}
	touched.init(len(files))
	for _, f := range files {
	...
	touched.visitIntermediates(func(rootRel string) bool {
...
// pathTree is a tree of native path relative to the deployer root.
//
// Think of it as a tree where each node is a path and leafs additionally
// represent infinite subtrees (rooted at these leafs).
//
// All methods assume paths have been properly cleaned already and do not have
// "." or "..". 'nil' value is valid for read methods. It denotes an empty set.
type pathTree struct {
	nodes map[string]struct{} // intermediate tree nodes
	leafs map[string]struct{} // leafs (also roots of their infinite subtrees)
}

// init initializes the path tree allocating the given capacity.
func (p *pathTree) init(capacity int) {
	p.nodes = make(map[string]struct{}, capacity) // overestimated
	p.leafs = make(map[string]struct{}, capacity) // exact
}
...
func (p *pathTree) add(rel string) {
	p.leafs[rel] = struct{}{}
	forParentDirs(rel, func(par string) bool {
		p.nodes[par] = struct{}{}
		return true
	})
}
...
func (p *pathTree) has(rel string) bool {
	...
	if _, ok := p.leafs[rel]; ok {
		return true
	}
	// Was added as a parent of some entry?
	if _, ok := p.nodes[rel]; ok {
		return true
	}
...
// forParentDirs takes a native path "a/b/c/d" and calls 'cb' with "a", "a/b"
// and "a/b/c".
//
// Purely lexicographical operation.
//
// Stops if the callback returns false. Note that it doesn't visit 'rel' itself.
func forParentDirs(rel string, cb func(p string) bool) {
	for i, r := range rel {
