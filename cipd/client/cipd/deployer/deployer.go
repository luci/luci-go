...
func (d *deployerImpl) packagePath(ctx context.Context, subdir, pkg string, allocate bool) (string, error) {
	...
	waiter := pauseUntil(ctx, "Collision when picking an index", time.Minute)
	for {
		...
		// this directory. Maybe it created if for the same package as we want?
		...
		// another slot a bit later, to given a chance for the concurrent writers to
		// proceed too.
...
// pauseUntil sleeps 1-5 sec on each invocation until reaching a deadline.
func pauseUntil(c context.Context, why string, d time.Duration) func() error {
	var attempt int32
