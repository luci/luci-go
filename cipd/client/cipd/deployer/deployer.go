...
func (d *deployerImpl) CheckDeployed(ctx context.Context, subdir, pkgname string, par ParanoidMode, m pkg.ManifestMode) (out *DeployedPackage, err error) {
	...
	case out.packagePath == "":
		logging.Errorf(ctx, "Failed to figure out packagePath of %q in %q: no matching .cipd/pkgs/*/package.json", pkgname, subdir)
	...
	case err != nil:
		...
		return
	case current == "":
		logging.Errorf(ctx, "Failed to figure out installed instance ID of %q in %q: missing _current symlink", pkgname, subdir)
		return // not deployed
	default:
