...
func (d *deployerImpl) packagePath(ctx context.Context, subdir, pkg string, allocate bool) (string, error) {
	...
	waiter := pauseToRetryUntil(ctx, "Collision when picking an index", time.Minute)
	for {
...
// pauseToRetryUntil sleeps a bit on each invocation until reaching a deadline.
func pauseToRetryUntil(c context.Context, why string, d time.Duration) func() error {
	var attempt int32
