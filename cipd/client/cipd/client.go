...
func (client *clientImpl) maybeUpdateClient(ctx context.Context, fs local.FileSystem,
	...
	// Grab the expected hash of the new binary from 'digests' file, if there.
	var clientRef *api.ObjectRef
	if digests != nil {
		plat := platform.CurrentPlatform()
		if clientRef = digests.ClientRef(plat); clientRef == nil {
			return common.Pin{}, fmt.Errorf("there's no hash for %q in *.digests file", plat)
		}
	}

	// Resolve the version to a pin, to be able to later grab URL to the binary.
	...
	// Name of the client binary inside the client CIPD package. Acts only as a
	// key for the extracted refs cache, nothing more, so can technically be
	// arbitrary.
	clientFileName := "cipd"
	...
	// If not using pinned digests file, lookup the hash corresponding to the pin
	// in the extracted refs cache. See rememberClientRef calls below for where it
	// is stored initially.
	if cache := client.getTagCache(); clientRef == nil && cache != nil {
	...
	// Compare the hash of the running binary to the expected one. Here clientRef
	// is nil only if not using pinned digests and there's not entry for the pin
	...
	// Grab the signed URL to the client binary and its server-supplied hash.
	info, err := client.DescribeClient(ctx, pin)
	if err != nil {
		return common.Pin{}, err
	}

	// If there's no locally supplied *.digests file, trust what backend says.
	if digests == nil {
		clientRef = info.Digest
	}

	// Check the local binary hash again. We may end up here if we had a cache
	// miss in the extracted refs cache and didn't know expected clientRef yet.
	switch yep, err := currentHashMatches(clientRef); {
	case err != nil:
		return common.Pin{}, err // can't read clientExe
	case yep:
	...
	err = client.installClient(
		...
		clientRef.HexDigest)
	if err != nil {
		return common.Pin{}, err // either a download error or hash mismatch
	}
