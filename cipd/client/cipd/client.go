...
func (client *clientImpl) maybeUpdateClient(ctx context.Context, fs local.FileSystem,
	...
	// If using pinned client digests, make sure the pin there matches the hash
	// reported by the backend. In most cases a mismatch means the pinned digests
	// file is just stale. The mismatch can also happen if the backend is
	// compromised or the client package was forcefully replaced (this should be
	// very rare...).
	if digests != nil {
		...
		case !digests.Contains(plat, clientRef):
			return common.Pin{}, fmt.Errorf(
				"the CIPD client hash reported by the backend (%s) is not in *.digests file, "+
					"most likely the file is stale", clientRef.HexDigest)
		default:
	...
	case yep:
		...
		// eliminate unnecessary cache writes).
		if info != nil {
	...
	// Grab the signed URL to the client binary if we haven't done so already.
	if info == nil {
		if info, err = client.DescribeClient(ctx, pin); err != nil {
			return common.Pin{}, err
		}
