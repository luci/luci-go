...
func (client *clientImpl) maybeUpdateClient(ctx context.Context, fs local.FileSystem, targetVersion, clientExe string) (pin common.Pin, err error) {
	...
	// Grab the signed URL to the client binary and its ObjectRef.
	...
	if cache != nil {
		err = cache.AddExtractedObjectRef(ctx, pin, clientFileName, info.ClientRef)
		if err != nil {
			return
		}
		client.doBatchAwareOp(ctx, batchAwareOpSaveTagCache)
	...
	var yep bool
	if yep, err = currentHashMatches(info.ClientRef); err != nil || yep {
	...
	err = client.installClient(
		ctx, fs,
		common.MustNewHash(info.ClientRef.HashAlgo),
		info.ClientBinary.SignedUrl,
		clientExe,
		info.ClientRef.HexDigest)
	return
}
...
// retryUntilReady calls the callback and retries on FailedPrecontion errors.
func retryUntilReady(ctx context.Context, timeout time.Duration, cb func(context.Context) error) error {
	...
	for {
		...
		case grpc.Code(err) == codes.FailedPrecondition: // the instance is not ready
			...
			clock.Sleep(clock.Tag(ctx, "cipd-sleeping"), 5*time.Second)
