...
func (client *clientImpl) ensurePackagesImpl(ctx context.Context, allPins common.PinSliceBySubdir, paranoia ParanoidMode, maxThreads int, dryRun, silent bool) (aMap ActionMap, err error) {
	...
	for cache.HasPendingFetches() {
		...
		if err == nil && client.pluginAdmission != nil {
			admErr := client.pluginAdmission.CheckAdmission(state.pin).Wait(ctx)
			if admErr != nil {
				err = errors.Annotate(admErr, "not admitted for deployment").Err()
			}
		}

		// Keep installing stuff as long as it keeps working (no errors).
		actionIdx := 0
		for err == nil && actionIdx < len(state.updates) {
			switch a := state.updates[actionIdx]; a.action {
			case ActionInstall:
				_, err = client.deployer.DeployInstance(ctx, a.subdir, res.Instance, maxThreads)
			case ActionRepair:
				err = client.deployer.RepairDeployed(ctx, a.subdir, state.pin, maxThreads, deployer.RepairParams{
		...
		corruption := reader.IsCorruptionError(err)
		if res.Instance != nil {
		...
		if corruption && state.attempts < 1 {
			logging.Warningf(ctx, "cipd: refetching %s after failing to unpack it: %s", state.pin, err)
			cache.RequestInstances([]*internal.InstanceRequest{
				{
					Context: ctx,
					Pin:     state.pin,
