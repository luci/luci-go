...
const (
	...
	// CheckPresence indicates that EnsurePackages should verify all files
	// that are supposed to be installed into site root are indeed present there,
	// and reinstall ones that are missing.
	//
	// Note that it will not check file's content though. Only presence.
...
type Client interface {
	...
	// desired one.
	//
	// Depending on the value of paranoia, will optionally verify that all
	// installed packages are installed correctly and will attempt to fix ones
	// that are not. See the enum for more info.
	//
...
func (client *clientImpl) EnsurePackages(ctx context.Context, allPins common.PinSliceBySubdir, paranoia ParanoiaMode, dryRun bool) (aMap ActionMap, err error) {
	...
	// installed package.
	var needsRepair repairCB
	switch paranoia {
	case NotParanoid:
		needsRepair = func(string, common.Pin) *RepairPlan {
			return nil
		}
	case CheckPresence:
		needsRepair = func(subdir string, pin common.Pin) *RepairPlan {
			...
			case err != nil:
				// This error is probably non-recoverable, but we'll try and properly
				// fail later.
				return &RepairPlan{
					NeedsReinstall:  true,
					ReinstallReason: fmt.Sprintf("failed to check package state - %s", err),
				}
			case !state.Deployed:
				// This should generally not be happening. Can probably happen if two
				// clients are messing with same .cipd/* directory concurrently.
				return &RepairPlan{
					NeedsReinstall:  true,
			...
			case len(state.ToRedeploy) != 0:
				// Have some corrupted files that needs to be restored.
				return &RepairPlan{
