...
type PackageDef struct {
	...
	// PreserveTimestamps instructs CIPD to preserve the mtime of the files.
...
func (def *PackageDef) FindFiles(cwd string) ([]File, error) {
	...
	for _, chunk := range def.Data {
		...
		// Individual file.
		if chunk.File != "" {
			file, err := WrapFile(makeAbs(chunk.File), root, nil, fileOpts)
			if err != nil {
				return nil, err
			}
			add(file)
		...
		if chunk.Dir != "" {
			...
			// Run the scan.
			files, err := ScanFileSystem(startDir, root, exclude, fileOpts)
			if err != nil {
				return nil, err
			}
			for _, f := range files {
