...
func TestScanFileSystem(t *testing.T) {
	Convey("Given a temp directory", t, func() {
		...
		Convey("Scan empty dir works", func() {
			files, err := ScanFileSystem(tempDir, tempDir, nil, FileOptions{})
		...
		Convey("Non root start path works", func() {
			...
			writeFile(tempDir, "1/b", "", 0666)
			writeFile(tempDir, "1/2/a", "", 0666)
			files, err := ScanFileSystem(filepath.Join(tempDir, "1"), tempDir, nil, FileOptions{})
			So(err, ShouldBeNil)
			names := make([]string, len(files))
			for i, f := range files {
				names[i] = f.Name()
		...
		Convey("Start path must be under root", func() {
			_, err := ScanFileSystem(filepath.Dir(tempDir), tempDir, nil, FileOptions{})
		...
		Convey("Exclude filter works", func() {
			...
			files, err := ScanFileSystem(tempDir, tempDir, excluder, FileOptions{})
			So(err, ShouldBeNil)
			So(len(files), ShouldEqual, 1)
		...
		if runtime.GOOS != "windows" {
			...
			Convey("Relative symlink to outside of package cause error", func() {
				...
				_, err := ScanFileSystem(tempDir, tempDir, nil, FileOptions{})
				So(err, ShouldNotBeNil)
			})
		}
	})
...
func TestWrapFile(t *testing.T) {
	Convey("Given a temp directory", t, func() {
		...
		Convey("WrapFile simple file works", func() {
			...
			out, err := WrapFile(filepath.Join(tempDir, "dir", "a", "b"), tempDir, nil, FileOptions{})
			So(err, ShouldBeNil)
		...
		Convey("WrapFile directory fails", func() {
			mkDir(tempDir, "dir")
			_, err := WrapFile(filepath.Join(tempDir, "dir"), tempDir, nil, FileOptions{})
			So(err, ShouldNotBeNil)
		})
		...
		Convey("WrapFile outside of root fails", func() {
			...
			_, err := WrapFile(filepath.Join(tempDir, "b"), filepath.Join(tempDir, "a"), nil, FileOptions{})
			So(err, ShouldNotBeNil)
		})
		...
		Convey("WrapFile outside of root fails (tricky path)", func() {
			...
			_, err := WrapFile(filepath.Join(tempDir, "abc"), filepath.Join(tempDir, "a"), nil, FileOptions{})
			So(err, ShouldNotBeNil)
		})
		...
		if runtime.GOOS != "windows" {
			Convey("WrapFile executable file works", func() {
				writeFile(tempDir, "single_file", "12345", 0766)
				out, err := WrapFile(filepath.Join(tempDir, "single_file"), tempDir, nil, FileOptions{})
				So(err, ShouldBeNil)
			...
			Convey("WrapFile rel symlink in root", func() {
				...
				mkDir(tempDir, "d")
				out, err := WrapFile(filepath.Join(tempDir, "a", "b", "c"), tempDir, nil, FileOptions{})
				So(err, ShouldBeNil)
			...
			Convey("WrapFile .cipd symlink", func() {
				...
				out, err := WrapFile(filepath.Join(tempDir, "some_executable"), tempDir, nil, FileOptions{})
				So(err, ShouldBeNil)
				if runtime.GOOS != "windows" {
					So(out.Executable(), ShouldBeTrue)
				}
			...
			Convey("WrapFile rel symlink outside root", func() {
				writeSymlink(tempDir, "a/b/c", filepath.FromSlash("../../../d"))
				_, err := WrapFile(filepath.Join(tempDir, "a", "b", "c"), tempDir, nil, FileOptions{})
				So(err, ShouldNotBeNil)
			})
...
func TestFileSystemDestination(t *testing.T) {
	...
	Convey("Given a temp directory", t, func() {
		...
		Convey("Empty success write works", func() {
			...
			files, err := ScanFileSystem(destDir, destDir, nil, FileOptions{})
			So(err, ShouldBeNil)
		...
		Convey("Committing bunch of files works", func() {
			...
			writeFileToDest("wr", CreateFileOptions{Writable: true}, "exe data")
			if runtime.GOOS != "windows" {
			...
			files, err := ScanFileSystem(destDir, destDir, nil, FileOptions{PreserveModTime: true, PreserveWritable: true})
			So(err, ShouldBeNil)
			names := make([]string, len(files))
			...
			allFiles, err := ScanFileSystem(tempDir, tempDir, nil, FileOptions{})
