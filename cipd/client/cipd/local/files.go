...
func (d *fsDest) CreateFile(ctx context.Context, name string, opts CreateFileOptions) (io.WriteCloser, error) {
	...
	// in non-atomic mode we just directly write into 'path'.
	writeTo := path
	if d.atomic {
		writeTo = tempFileName(path)
	}
	file, err := os.OpenFile(writeTo, os.O_CREATE|os.O_WRONLY|os.O_EXCL, mode)
	if err != nil {
		return nil, err
	}
...
type txnFSDest struct {
	...
	// Root temporary directory where files are located between Begin/End.
	tempDir string
	// The underlying FS destination configured to extract files into a
	// subdirectory of 'tempDir' which will be renamed into 'dest' in End(true).
	fsDest *fsDest
}
...
func (d *txnFSDest) Begin(ctx context.Context) error {
	...
	// that tempDir is totally owned by txnFSDest, and it ensures there are no
...
func (d *txnFSDest) End(ctx context.Context, success bool) error {
	...
	// Clean up the temp dir and the state no matter what.
	defer func() {
