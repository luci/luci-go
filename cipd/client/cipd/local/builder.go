...
func zipInputFiles(ctx context.Context, files []File, w io.Writer, level int, preserveWritable bool) error {
	...
	for i, in := range files {
		...
		// Legacy packages always set the user-writable bit (0400) and then ignore
		// it when unpacking. Unless we are asked to preserve the writable bit,
		// stick with the old behavior for backward compatibility.
