...
type DeployedPackage struct {
	...
	// Fields below are only of interest to the deployer itself, thus they are
	// private.
...
func (d *deployerImpl) RepairDeployed(ctx context.Context, subdir string, pin common.Pin, params RepairParams) error {
	...
	// manifest is all files that should be present in a healthy deployment.
	...
	for _, name := range broken {
		if f := repairableFiles[name]; f != nil {
			...
			logging.Warningf(ctx, "Can't repair %q, the source is not available", name)
		}
	}
	if len(repair) != 0 {
	...
	add := func(name string) {
		if info, ok := manifest[name]; ok {
			...
			logging.Warningf(ctx, "Can't relink %q, unknown file", name)
		}
	}
