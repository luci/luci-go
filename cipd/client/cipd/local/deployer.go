...
func (d *deployerImpl) checkIntegrity(ctx context.Context, subdir string, pin common.Pin, pkgDir string) (redeploy, relink []string, err error) {
	...
	for _, f := range manifest.Files {
		...
		default:
			...
			//  3. Both CIPD-managed symlinks exist and the external file itself is
			//     missing. Such symlink is NOT broken. If we try to "repair" it, we
			//     end up in the same state anyway: absolute symlinks point to file
