...
func ExtractFiles(ctx context.Context, files []fs.File, dest fs.Destination, maxThreads int, withManifest pkg.ManifestMode) (extracted []pkg.FileInfo, err error) {
	...
	err = parallel.WorkPool(workerCount, func(errorChan chan<- func() error) {
		for {
			if err = ctx.Err(); err != nil {
				return
			}
	...
	manifest.Files = filterFiles(extracted)
...
// filterFiles returns all the files with non-empty names.
func filterFiles(files []pkg.FileInfo) []pkg.FileInfo {
	filtered := make([]pkg.FileInfo, 0, len(files))
