		...
		CommandRun: func() subcommands.CommandRun {
			...
			c.Flags.StringVar(&c.ensureFile, "ensure-file", "<path>",
				(`The "ensure" file to verify. See syntax described here: ` +
					`https://godoc.org/go.chromium.org/luci/cipd/client/cipd/ensure.` +
					` Providing '-' will read from stdin.`))
			c.Flags.Var(&c.platforms, "platform",
				`Manually specify the platform to verify against. This can be provided multiple times `+
					`to verify against multiple patforms. Platforms specified in this manner override `+
					`any $VerifiedPlatform directives in the manifest.`)
			c.Flags.BoolVar(&c.currentPlatform, "current-platform", false,
				`Verify against the current CIPD platform. This is cumulative with "-platform".`)
			return c
		},
	}
}
...
func (c *ensureFileVerifyRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	if c.currentPlatform {
		c.platforms.Set(common.CurrentPlatform().String())
	}
	err := verifyEnsureFile(ctx, c.ensureFile, c.platforms.ToSlice(), c.clientOptions)
	return c.done(nil, err)
}

func verifyEnsureFile(ctx context.Context, desiredStateFile string, platforms []string,
	...
	// Determine which platforms to verify against.
	switch {
	case len(platforms) > 0:
		if len(ensureFile.VerifyPlatforms) > 0 {
			logging.Infof(ctx, "Using command-line platforms instead of %d declared in the ensure file.",
				len(ensureFile.VerifyPlatforms))
		}

		verifyPlatforms := make([]common.TemplatePlatform, len(platforms))
		for i, plat := range platforms {
			var err error
			if verifyPlatforms[i], err = common.ParseTemplatePlatform(plat); err != nil {
				return errors.Annotate(err, "invalid platform string %q", plat).Err()
			}
		}
		ensureFile.VerifyPlatforms = verifyPlatforms

	case len(ensureFile.VerifyPlatforms) > 0:
		logging.Debugf(ctx, "Verifying against %d platform(s) in the ensure file.", len(ensureFile.VerifyPlatforms))

	default:
		logging.Errorf(ctx,
			"Verification platforms must be specified in the ensure file ($VerifiedPlatform) or command-line (-platform).")
