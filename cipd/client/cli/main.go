...
func (opts *ensureFileOptions) loadEnsureFile(ctx context.Context, clientOpts *clientOptions, verifying verifyingEnsureFile, vers versionFileOpt) (*ensure.File, error) {
	...
	if parseVersionsFile && parsedFile.ResolvedVersions != "" {
		...
		logging.Debugf(ctx, "Using the versions from the resolved version file %q", filepath.Base(parsedFile.ResolvedVersions))
	}
...
func (c *ensureFileResolveRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	if c.checkOnly {
		existing, err = loadVersionsFile(ef.ResolvedVersions, c.ensureFile)
		if err != nil {
			return c.done(nil, err)
		}
	}
...
func loadVersionsFile(path, ensureFile string) (ensure.VersionsFile, error) {
	...
	ef, err := ensure.ParseVersionsFile(f)
