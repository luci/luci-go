...
func (c *ensureFileVerifyRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	ctx := cli.GetContext(a, c, env)
	err := verifyEnsureFile(ctx, c.ensureFile, c.defaultPlatforms, c.clientOptions)
	return c.done(nil, err)
}

func verifyEnsureFile(ctx context.Context, desiredStateFile string, defaultPlatforms []string, clientOpts clientOptions) error {
	ensureFile, err := loadAndValidateEnsureFile(ctx, desiredStateFile, &clientOpts)
	if err != nil {
		return err
	}
	...
	if result.HasErrors() {
		for _, pin := range result.InvalidPins {
			plats := result.PinPackageMap[pin]
			if len(plats) > 0 {
				platNames := make([]string, len(plats))
				for i, plat := range plats {
					platNames[i] = plat.String()
				}

				logging.Errorf(ctx, "Failed to verify pin %s from %s.", pin, strings.Join(platNames, ", "))
			} else {
		...
		for _, pkg := range result.InvalidPackages {
			logging.Errorf(ctx, "Failed to resolve package %s.", pkg)
		}
