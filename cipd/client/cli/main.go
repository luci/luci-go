...
func (c *selfupdateRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	var digests *ensure.ClientDigestsFile
...
func loadClientDigests(path string) (*ensure.ClientDigestsFile, error) {
	switch f, err := os.Open(path); {
	case os.IsNotExist(err):
		base := filepath.Base(path)
	...
	default:
		defer f.Close()
		return ensure.ParseClientDigestsFile(f)
	}
}

// assembleClientDigests produces the digests file by making backend RPCs.
func assembleClientDigests(ctx context.Context, c cipd.Client, version string) (*ensure.ClientDigestsFile, []pinInfo, error) {
