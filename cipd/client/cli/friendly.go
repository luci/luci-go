...
func (site *installationSite) installPackage(ctx context.Context, pkgName, version string, maxThreads int) (*pinInfo, error) {
	if site.client == nil {
	...
	actions, err := site.client.EnsurePackages(ctx, deployed, cipd.NotParanoid, maxThreads, false)
	if err != nil {
		return nil, err
	}
...
func cmdInstall(params Parameters) *subcommands.Command {
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			_ = c.Flags.Bool("force", false, "Not used anymore and ignored.")
			return c
		},
	}
}
...
func (c *installRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	return c.done(site.installPackage(ctx, pkgName, version, c.maxThreads))
}
