...
`/<os>-<arch>`, for example `infra/tools/cipd/linux-amd64`. The `os` part can
be `linux`, `mac`, or `windows`, and `arch` can be `386`, `amd64` or `armv6l`.
See [the ensure package docs][ensure-docs] for accepted os and arch values.
