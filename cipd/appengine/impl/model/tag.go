...
func ListInstanceTags(c context.Context, inst *Instance) (out []*Tag, err error) {
	...
	// entity and setting up a composite index (key, -modified_ts). This requires
	// migration of existing entities. We punt on this for now and sort in memory
	// instead (just like we did on the client before). The most heavily tagged
	// instances in our datastore have few hundred tags at most, so this is
	// bearable.
