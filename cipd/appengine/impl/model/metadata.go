...
type InstanceMetadata struct {
	...
	ContentType string `gae:"content_type,noindex"` // a content type, if known

	AttachedBy string    `gae:"attached_by"` // who added this metadata
	AttachedTs time.Time `gae:"attached_ts"` // when it was added
}

// AttachInstanceMetadata transactionally attaches metadata to an instance.
//
// Mutates `md` in place by calculating fingerprints.
//
...
func AttachInstanceMetadata(ctx context.Context, inst *Instance, md []*api.InstanceMetadata) error {
	now := clock.Now(ctx).UTC()
	who := string(auth.CurrentIdentity(ctx))

	// Calculate fingerprints before the transaction, it is relatively slow.
	for _, m := range md {
		m.Fingerprint = common.InstanceMetadataFingerprint(m.Key, m.Value)
	}

	return Txn(ctx, "AttachInstanceMetadata", func(ctx context.Context) error {
		...
		// etc. and skip emitting INSTANCE_METADATA_ATTACHED event log entry.
