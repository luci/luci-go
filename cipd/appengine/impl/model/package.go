...
//
// Lists all packages recursively. The result is sorted by the package name.
func ListPackages(c context.Context, prefix string, showHidden bool) (out []string, err error) {
	...
	err = datastore.Run(c, q, func(p *Package) error {
		...
		if !p.Hidden || showHidden {
			out = append(out, p.Name)
		}
		return nil
	})
	if err != nil {
		return nil, errors.Annotate(err, "failed to queries the list of packages").Err()
	}
	return out, nil
}
