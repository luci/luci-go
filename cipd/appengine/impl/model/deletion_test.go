	...
	Convey("Works", t, func() {
		...
		entitiesCount := func(root *datastore.Key) (count int64) {
			...
			So(datastore.Run(ctx, q, func(k *datastore.Key) {
				...
				// except we can't disable them, since we needs them for transactions to
				// work.
		...
		// Create a bunch of stuff to be deleted.
		...
		// After the deletion we left only with another-pkg.
