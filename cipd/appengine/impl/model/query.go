...
func filterByTag(c context.Context, page []*datastore.Key, tag *api.Tag) ([]*datastore.Key, error) {
	...
	for i, inst := range page {
		tagKeys[i] = datastore.NewKey(c, "InstanceTag", tagID, 0, inst)
	}
	// TODO(vadimsh): Need to actually fetch entity bodies and double check that
	// Tag field matches 'tag' as a precaution against SHA1 collision on TagID.
