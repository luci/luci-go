...
func QueryInstances(c context.Context, pkg string, tags []*api.Tag, pageSize int32, cursor datastore.Cursor) (out []*Instance, nextCur datastore.Cursor, err error) {
	...
	for pageSize == 0 || int32(len(out)) < pageSize {
		...
		page, cursor, err = queryByTag(c, PackageKey(c, pkg), mainTag, cursor, want)
		if err != nil {
			return nil, nil, err
		}
...
func queryByTag(c context.Context, root *datastore.Key, tag string, cursor datastore.Cursor, pageSize int32) (
	out []*datastore.Key,
