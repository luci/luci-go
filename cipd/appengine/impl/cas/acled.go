...
var perMethodACL = map[string]struct {
	...
	// so usage of 'force_ref' field is forbidden.
	"BeginUpload":  {"administrators", nil},
	"FinishUpload": {"*", denyForceRef},
}

func denyForceRef(c context.Context, req proto.Message) error {
	if req.(*api.FinishUploadRequest).ForceRef != nil {
		return status.Errorf(codes.PermissionDenied, "usage of 'force_ref' is forbidden")
	}
	return nil
}
