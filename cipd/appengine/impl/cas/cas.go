...
// non-transient error, logs and forgets it, since retrying won't help.
func (s *storageImpl) cleanupUploadTask(c context.Context, task *tasks.CleanupUpload) (err error) {
	return s.maybeDelete(c, s.getGS(c), task.PathToCleanup)
}

// maybeDelete is called to delete temporary file when finishing or cleaning up
// an upload.
//
...
// so it is fine.
//
// When finalizing upload, If Delete fails fatally, we are in a tough position,
// since we did publish the file already, so the upload operation is technically
// successful and marking it as failed is a lie. So we log and ignore fatal
// Delete errors. They should not happen anyway.
//
