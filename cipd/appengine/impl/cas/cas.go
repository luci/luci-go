...
func (s *storageImpl) FinishUpload(c context.Context, r *api.FinishUploadRequest) (resp *api.UploadOperation, err error) {
	...
	if r.ForceHash != nil {
		return s.finishAndForcedHash(c, r, &op)
	}
...
// finishAndForcedHash is called to finalize uploads that use ForceHash field.
//
// It publishes the object immediately, skipping the verification.
func (s *storageImpl) finishAndForcedHash(c context.Context, r *api.FinishUploadRequest, op *upload.Operation) (*api.UploadOperation, error) {
	gs := s.getGS(c)
	cfg, err := s.settings(c)
	if err != nil {
		return nil, err
	...
	pubErr := gs.Publish(c, cfg.ObjectPath(r.ForceHash), op.TempGSPath, -1)
	...
	// or not.
	mutated, err := op.Advance(c, func(_ context.Context, op *upload.Operation) error {
		if pubErr != nil {
...
func (s *storageImpl) verifyUploadTask(c context.Context, task *tasks.VerifyUpload) (err error) {
	...
	defer func() {
		...
		// Update the status of the operation based on 'err'. If this change fails
		// itself, return a transient error to make sure 'verifyUploadTask' is
		// retried. We have no other choice.
		...
		if opErr != nil {
			err = errors.Annotate(opErr, "failed to update the operation").Tag(transient.Tag).Err()
			return
		}
		...
		// directory doesn't hurt. Just log the error.
	...
	reader, err := gs.Reader(c, op.TempGSPath, 0)
	if err != nil {
	...
	bufSize := 4 * 1024 * 1024
	if size := reader.Size(); size < int64(bufSize) {
		bufSize = int(size)
	}
	...
	_, err = io.CopyBuffer(
		hasher,
		io.NewSectionReader(reader, 0, reader.Size()),
		make([]byte, bufSize))
	if err != nil {
		return errors.Annotate(err, "failed to read google storage file").Err()
	}
	verifiedHexDigest = hex.EncodeToString(hasher.Sum(nil))

	// If we know the expected hash, verify it matches what we have calculated.
	if op.HexDigest != "" && op.HexDigest != verifiedHexDigest {
		return errors.Reason("expected %s digest to be %s, got %s", op.HashAlgo, op.HexDigest, verifiedHexDigest).Err()
	}
	...
	}), op.TempGSPath, reader.Generation())
	if err != nil {
...
//
// If this fails transiently, we ask the client (task queue) to retry the
// corresponding RPC (by returning transient errors), to delete the file
...
// file already, so the upload operation is technically successful. We log and
// ignore fatal Delete errors. They should not happen anyway.
//
// Thus, returns either nil or a transient error.
