...
//
// Larger values means less Google Storage RPC calls, but more memory usage.
...
func (s *storageImpl) verifyUploadTask(c context.Context, task *tasks.VerifyUpload) (err error) {
	...
	if op.HexDigest != "" && op.HexDigest != verifiedHexDigest {
		return errors.Reason("expected %s be %s, got %s", op.HashAlgo, op.HexDigest, verifiedHexDigest).Err()
	}
