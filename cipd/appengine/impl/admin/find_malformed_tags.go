...
func findMalformedTagsMapper(c context.Context, job mapper.JobID, _ *api.JobConfig, keys []*datastore.Key) error {
	return visitAndMarkTags(c, job, keys, func(t *model.Tag) *markedTag {
		err := common.ValidateInstanceTag(t.Tag)
		if err != nil {
			return nil // skip good tags
		}
		return &markedTag{Why: err.Error()}
	})
}
