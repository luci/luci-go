...
func (impl *adminImpl) LaunchJob(c context.Context, cfg *api.JobConfig) (*api.JobID, error) {
	...
	if err := launchCfg.Params.SetProto(jobCfgKey, cfg); err != nil {
		return nil, status.Errorf(codes.Internal, "failed to prepare job params - %s", err)
	}
...
func (impl *adminImpl) GetJobState(c context.Context, id *api.JobID) (*api.JobState, error) {
	...
	cfg := &api.JobConfig{}
	if err := job.Config.Params.GetProto(jobCfgKey, cfg); err != nil {
		return nil, toStatus(err)
	}
