...
func TestVisitAndMarkTags(t *testing.T) {
	...
	Convey("With a bunch of instances", t, func() {
		...
		Convey("Happy path", func() {
			...
			// Mark all k:1 tags.
			err := visitAndMarkTags(ctx, 1, keys, func(t *model.Tag) *markedTag {
				if t.Tag == "k:1" {
					return &markedTag{Why: "why not?"}
				}
				return nil
			})
			So(err, ShouldBeNil)
			...
			err = visitAndMarkTags(ctx, 2, keys, func(t *model.Tag) *markedTag {
				return &markedTag{Why: "all"}
			})
			So(err, ShouldBeNil)
