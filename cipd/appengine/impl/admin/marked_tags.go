...
// the callback, which may choose to mark a tag by returning non-nil markedTag.
//
// Such marked tags are then stored in the datastore and later can be queried.
//
// visitAndMarkTags populates all fields in markedTag struct returned by
// a callback, except 'Why'.
func visitAndMarkTags(c context.Context, job mapper.JobID, keys []*datastore.Key, cb func(*model.Tag) *markedTag) error {
