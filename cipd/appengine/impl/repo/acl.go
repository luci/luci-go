...
const superGroup = "administrators"
...
// allRoles is a set of all possible roles (as granted to superGroup).
...
//
// Returns only transient errors.
func rolesInPrefix(c context.Context, metas []*api.PrefixMetadata) (*api.RolesInPrefix, error) {
	// Administrators group has implicit permissions to do everything everywhere.
	switch yes, err := auth.IsMember(c, superGroup); {
	case err != nil:
		return nil, errors.Annotate(err, "failed to check group memberships when checking ACLs").Err()
	case yes:
		return makeRolesInPrefix(allRoles), nil
	}
	...
	return makeRolesInPrefix(roles), nil
}
...
// makeRolesInPrefix constructs *api.RolesInPrefix from a set of Roles.
//
// Arrange the response in the order of Role enum definition.
func makeRolesInPrefix(roles map[api.Role]struct{}) *api.RolesInPrefix {
	resp := &api.RolesInPrefix{
		Roles: make([]*api.RolesInPrefix_RoleInPrefix, 0, len(roles)),
	}
	for r := api.Role_READER; r <= api.Role_OWNER; r++ {
		if _, ok := roles[r]; ok {
			resp.Roles = append(resp.Roles, &api.RolesInPrefix_RoleInPrefix{
				Role: r,
			})
		}
	}
