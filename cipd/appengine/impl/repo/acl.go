...
//
// It understand role inheritance defined by impliedRoles map.
//
...
func hasRole(c context.Context, metas []*api.PrefixMetadata, role api.Role) (bool, error) {
	...
	for _, meta := range metas {
		for _, acl := range meta.Acls {
			...
			for _, p := range acl.Principals {
				...
				if chunks[0] != "group" {
					logging.Warningf(c, "Unrecognized principal kind in PrefixMetadata for prefix %q: %q", meta.Prefix, p)
					continue
				}
				groups.Add(chunks[1])
			}
	...
	if err != nil {
		return false, errors.Annotate(err, "failed to check group memberships when checking role %s", role).Err()
	}
	return yes, nil
}
