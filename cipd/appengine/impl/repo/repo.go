...
// paginatedQuery is a common part of ListInstances and QueryInstances.
...
// QueryInstances implements the corresponding RPC method, see the proto doc.
func (impl *repoImpl) QueryInstances(c context.Context, r *api.QueryInstancesRequest) (resp *api.QueryInstancesResponse, err error) {
	defer func() { err = grpcutil.GRPCifyAndLogErr(c, err) }()
	...
	result, nextPage, err := impl.paginatedQuery(c, paginatedQueryOpts{
		...
		Handler: func(cur datastore.Cursor, pageSize int32) ([]*model.Instance, datastore.Cursor, error) {
			return model.QueryInstances(c, r.Package, r.Tags, pageSize, cur)
		},
	})
	if err != nil {
		return nil, err
	...
	return &api.QueryInstancesResponse{
		Instances:     result,
		NextPageToken: nextPage,
	}, nil
}
