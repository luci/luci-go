...
func adaptGrpcErr(h func(*router.Context) error) router.Handler {
	return func(ctx *router.Context) {
		...
		if code := grpc.Code(err); code != codes.OK {
			http.Error(ctx.Writer, err.Error(), grpcutil.CodeStatus(code))
		}
	}
}
...
func (impl *repoImpl) handleClientBootstrap(ctx *router.Context) error {
	...
	// Validate syntax of everything before doing any heavy calls.
	...
	// Get a name of the cipd client package, e.g 'infra/tools/cipd/linux-amd64'.
	pkg, err := processing.GetClientPackage(platform)
	if err != nil {
		return status.Errorf(codes.InvalidArgument, "bad platform name")
	}

	// Resolve the version into a concrete instance. This also does argument
	// validation, ACL checks and verifies the instance exists.
	...
	// Grab the location of the extracted CIPD client.
