...
func (impl *repoImpl) ListPrefix(c context.Context, r *api.ListPrefixRequest) (resp *api.ListPrefixResponse, err error) {
	...
	// One of 'visibleRoots' => its full recursive listing.
	perVisibleRoot := map[string][]string{}
	...
	// package named r.Prefix in the result (if any), so we skip this check for
	// f.Prefix itself.
	suspects := make([]string, 0, len(visibleRoots))
	for _, pfx := range visibleRoots {
		if pfx != r.Prefix {
			suspects = append(suspects, pfx)
		}
	}
	rootPkgs, err := model.CheckPackages(c, suspects, r.IncludeHidden)
	if err != nil {
