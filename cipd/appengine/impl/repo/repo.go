...
func (impl *repoImpl) AttachMetadata(c context.Context, r *api.AttachMetadataRequest) (resp *empty.Empty, err error) {
	...
	for _, m := range r.Metadata {
		...
		if err := common.ValidateInstanceMetadataLen(len(m.Value)); err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "bad value for metadata key %q - %s", m.Key, err)
		}
		if err := common.ValidateContentType(m.ContentType); err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "bad content type for metadata key %q - %s", m.Key, err)
		}
	}
	...
	if err := model.AttachInstanceMetadata(c, inst, r.Metadata); err != nil {
		return nil, err
	}
	return &empty.Empty{}, nil
}
