...
func TestHasRole(t *testing.T) {
	...
	Convey("Works", t, func() {
		...
		metas = addPrefixACLs(metas, "root", map[api.Role][]string{
			api.Role_OWNER:  []string{"user:direct-owner@example.com", "group:top-owners"},
			api.Role_WRITER: []string{"group:top-writers"},
			api.Role_READER: []string{"group:top-readers"},
		})
		metas = addPrefixACLs(metas, "root/something/else", map[api.Role][]string{
			api.Role_OWNER:  []string{"group:inner-owners"},
			api.Role_WRITER: []string{"group:inner-writers"},
			api.Role_READER: []string{"group:inner-readers"},
		})
