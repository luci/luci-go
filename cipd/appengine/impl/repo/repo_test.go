	...
	Convey("With fakes", t, func() {
		...
		Convey("Legacy", func() {
			Convey("Happy path", func() {
				...
				So(body, ShouldEqual,
					`{"status":"SUCCESS","instance":{"package_name":"infra/tools/cipd/linux-amd64",`+
						`"instance_id":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"},`+
						`"client_binary":{"file_name":"cipd","sha1":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",`+
						`"fetch_url":"http://fake/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb?d=cipd",`+
			...
			Convey("Missing instance", func() {
				...
				So(code, ShouldEqual, http.StatusOK)
				So(body, ShouldEqual,
					`{"status":"INSTANCE_NOT_FOUND","error_message":"no such instance"}`)
			})
			...
			Convey("Not extracted yet", func() {
				...
				So(code, ShouldEqual, http.StatusOK)
				So(body, ShouldEqual,
					`{"status":"NOT_EXTRACTED_YET","error_message":"the client binary is not extracted yet, try later"}`)
			})
			...
			Convey("Fatal error during extraction", func() {
				...
				So(code, ShouldEqual, http.StatusOK)
				So(body, ShouldEqual,
					`{"status":"ERROR","error_message":"the client binary is not available - client extraction failed - BOOM"}`)
			})
		})
	})
}
	...
	Convey("With fakes", t, func() {
		...
		Convey("handleLegacyInstance works", func() {
			...
			Convey("Happy path", func() {
				...
				So(code, ShouldEqual, http.StatusOK)
				So(body, ShouldEqual,
					`{"status":"SUCCESS","fetch_url":"http://fake/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",`+
						`"instance":{"package_name":"a/b","instance_id":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",`+
						`"registered_by":"user:reg@example.com","registered_ts":"1454472306000000"}}`)
			})
			...
			Convey("Missing pkg", func() {
				...
				So(code, ShouldEqual, http.StatusOK)
				So(body, ShouldEqual,
					`{"status":"INSTANCE_NOT_FOUND","error_message":"no such package"}`)
			})
		})
		...
		Convey("handleLegacyResolve works", func() {
			...
			Convey("Happy path", func() {
				...
				So(code, ShouldEqual, http.StatusOK)
				So(body, ShouldEqual,
					`{"status":"SUCCESS","instance_id":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"}`)
			})
			...
			Convey("Ambiguous version", func() {
				...
				So(code, ShouldEqual, http.StatusOK)
				So(body, ShouldEqual,
					`{"status":"AMBIGUOUS_VERSION","error_message":"ambiguity when resolving the tag, more than one instance has it"}`)
			})
		})
	})
}
