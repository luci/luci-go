...
func TestUpdateProcessors(t *testing.T) {
	...
	Convey("With mocks", t, func() {
		...
		Convey("Noop updateProcessors", func() {
			...
			So(impl.updateProcessors(ctx, inst, nil), ShouldBeNil)
		...
		Convey("Updates processors successfully", func() {
			...
			So(impl.updateProcessors(ctx, inst, map[string]processing.Result{
				"ok":   processing.Result{Result: "OK"},
				"fail": processing.Result{Err: fmt.Errorf("failed")},
			}), ShouldBeNil)
		...
		Convey("Missing entity", func() {
			So(impl.updateProcessors(ctx, inst, nil), ShouldErrLike, "the entity is unexpectedly gone")
		})
	})
}
