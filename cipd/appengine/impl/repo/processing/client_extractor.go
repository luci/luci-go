...
type ClientExtractorResult struct {
	ClientBinary struct {
		...
		// Hex digests of the extracted file calculated using all algos known to
		// the server at the time the file was uploaded.
		//
...
func (r *ClientExtractorResult) ObjectRefAliases() []*api.ObjectRef {
	...
	for algo := int32(0); api.HashAlgo_name[algo] != ""; algo++ {
...
func (e *ClientExtractor) Run(ctx context.Context, inst *model.Instance, pkg *PackageReader) (res Result, err error) {
	...
	// We also always calculate all other hashes we know about in parallel, for
	// old bootstrap scripts that may not understand the most recent hash algo.
