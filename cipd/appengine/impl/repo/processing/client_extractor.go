...
// ClientExtractorResult is stored in JSON form as result of ClientExtractor
// execution.
//
...
// ClientExtractor is a processor that extract CIPD client binary from CIPD
...
func (e *ClientExtractor) Run(ctx context.Context, inst *model.Instance, pkg *PackageReader) (res Result, err error) {
	...
	// Prepare the HTTP transport used for uploads. Authentication is handled
	// through the tokens in upload session URL.
	tr, err := auth.GetRPCTransport(ctx, auth.NoAuth)
	if err != nil {
		err = errors.Annotate(err, "failed to get the RPC transport").Err()
		return
	}
	...
	if err != nil {
		err = errors.Annotate(err, "failed to open CAS upload session").Err()
		return
	}
	uploader := gs.Uploader{
		Context:   ctx,
	...
	// Copy in 2MB chunks, calculating SHA1 on the fly. We use fullReader to make
	// sure we send full 2MB chunks to GS. Otherwise 'reader' uses 32KB buffers
	// (remember, in Go io.Reader can choose to read less than asked and zip
	// readers really like 32KB buffers).
	...
	// If asked to rewind to a far away offset (should be rare), just restart the
	...
	switch {
	case err != nil:
		err = errors.Annotate(err, "failed to finalize the upload").Err()
		return
