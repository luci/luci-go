...
//
// Return grpc-annotated Internal error if something is wrong.
func Get(ctx context.Context) (*Settings, error) {
	c := &Settings{}
	if err := settings.Get(ctx, settingsKey, c); err != nil && err != settings.ErrNoSettings {
		return nil, errors.Annotate(err, "failed to fetch settings").
			Tag(grpcutil.InternalTag, transient.Tag).Err()
	}
	if c.StorageGSPath == "" || c.TempGSPath == "" {
