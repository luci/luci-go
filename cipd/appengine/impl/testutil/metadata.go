...
func (s *MetadataStore) VisitMetadata(c context.Context, prefix string, cb metadata.Visitor) error {
	...
	return s.asGraph(prefix).traverse(func(n *node) (cont bool, err error) {
		...
		// to it, just recurse deeper until we find some metadata.
		if !n.hasMeta {
			return true, nil
		}
		...
		// And ask the callback whether we should to proceed.
		return cb(clean, md)
	})
}
...
// addChilds adds and returns a child node if it didn't exist before.
