...
// Visiter is a callback passed to VisitMetadata.
//
// It decides whether to continue exploring this metadata subtree or not.
type Visiter func(prefix string, md []*api.PrefixMetadata) (cont bool, err error)

// Storage knows how to store, fetch and update prefix metadata, as well as
// how to calculate its fingerprint.
//
// The metadata is organizes in a forest-like structure, where each node is
...
//
// Doesn't try to understand what metadata means, just fingerprints, stores and
// enumerates it.
//
...
type Storage interface {
	...
	// VisitMetadata enumerates the metadata in breadth-first order.
	//
	// Can be used to fetch all metadata items at and under the given prefix.
	//
	// The callback is called for each visited node (starting from 'prefix' itself
	// if metadata for it exists), receiving same metadata list as if GetMetadata
	// was used to fetch it. The callback can decide whether to proceed with the
	// enumeration of the corresponding subtree or skip it (by returning either
	// true or false).
	//
	// Abort the traversal on a first error from the callback.
	//
	// Returns either a transient error if fetching failed, or whatever error the
	// callback returned.
	VisitMetadata(c context.Context, prefix string, cb Visiter) error
