...
type Storage interface {
	// GetMetadata fetches metadata associated with the given prefix
	// and all parent prefixes.
	//
	// Does not check permissions (it's the responsibility of the caller).
	//
	...
	// This happens if metadata for prefix doesn't exist.
	//
	...
	// prefix.
	//
	// Does not check permissions (it's the responsibility of the caller). Does
	// not check format of the updated metadata.
	//
	...
	// callback doesn't return an error, the metadata's fingerprint is updated and
	// the metadata is saved to the storage and returned to the caller.
	//
