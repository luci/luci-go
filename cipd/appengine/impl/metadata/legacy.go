...
//
// The processes of constructing and deconstructing PrefixMetadata is not
// perfectly reversible:
//   * Order of Acls entries in PrefixMetadata is not preserved.
//   * Order of prinicipals in is also not preserved.
//   * Empty Acls entries are removed from PrefixMetadata.
//
...
func (legacyStorageImpl) UpdateMetadata(c context.Context, prefix string, cb func(m *api.PrefixMetadata) error) (*api.PrefixMetadata, error) {
	...
	err = datastore.RunInTransaction(c, func(c context.Context) error {
		...
		// Don't let the callback to mess with prefix or fingerprint.
	...
	switch {
	case cbErr != nil:
		// The callback failed, need to return the error as is, as promised.
		return nil, cbErr
	case err != nil:
