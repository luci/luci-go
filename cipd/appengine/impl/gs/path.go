...
func ValidatePath(p string) error {
	...
	if len(chunks) < 3 || chunks[0] != "" || chunks[len(chunks)-1] == "" {
...
func SplitPath(p string) (bucket, path string) {
	...
	sep := strings.IndexByte(p[1:], '/')
	if sep == -1 {
		panic(fmt.Errorf("%q doesn't name a file within the bucket, only the bucket itself", p))
	}
	return p[1 : sep+1], p[sep+2:]
}
