...
func TestGoogleStorage(t *testing.T) {
	...
	Convey("With mocked service", t, func(c C) {
		...
		Convey("Copy unconditional", func() {
			...
			So(gs.Copy(ctx, "/src_bucket/src_obj", "/dst_bucket/dst_obj", -1, -1), ShouldBeNil)
		})
		...
		Convey("Copy conditional", func() {
			...
			So(gs.Copy(ctx, "/src_bucket/src_obj", "/dst_bucket/dst_obj", 1, 2), ShouldBeNil)
		})
		...
		Convey("Copy error", func() {
			...
			err := gs.Copy(ctx, "/src_bucket/src_obj", "/dst_bucket/dst_obj", -1, -1)
			So(StatusCode(err), ShouldEqual, 403)
		})
		...
		Convey("Publish success", func() {
			...
			So(gs.Publish(ctx, "/src_bucket/src_obj", "/dst_bucket/dst_obj", 1), ShouldBeNil)
		})
		...
		Convey("Publish bad precondition", func() {
			...
			err := gs.Publish(ctx, "/src_bucket/src_obj", "/dst_bucket/dst_obj", 1)
