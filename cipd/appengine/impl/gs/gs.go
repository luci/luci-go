...
func (gs *impl) Publish(c context.Context, dst, src string, srcGen int64) error {
	...
	// longer matches srcGen generation, if we check it), or 'dst' already exists
	// (its generation is no longer 0).
	//
	...
	// operation is successful overall).
	if srcGen < 0 && code == http.StatusPreconditionFailed {
		return nil
	}
