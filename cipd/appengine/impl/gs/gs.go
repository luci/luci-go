...
func (gs *GoogleStorage) Reader(c context.Context, path string, gen int64) (*Reader, error) {
	...
	// Fetch object metadata, including its size and the generation number (which
	// is useful when 'gen' is <= 0).
	call := gs.srv.Objects.Get(SplitPath(path)).Context(c)
	if gen > 0 {
		call.Generation(gen)
	}
...
func (r *Reader) ReadAt(p []byte, off int64) (n int, err error) {
	...
	logging.Infof(r.c, "gs: ReadAt(path=%q, offset=%d, length=%d, gen=%d)", r.path, off, toRead, r.gen)
