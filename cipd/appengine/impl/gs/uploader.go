...
func (u *Uploader) Write(p []byte) (n int, err error) {
	...
	for u.Offset != bufEnd && err == nil {
		...
		err = withRetry(u.Context, func() error {
			// When resuming, we upload 0-byte chunk to grab the last known offset.
			// Otherwise we just carry on uploading stuff.
			var chunk []byte
			if !resuming {
			...
			// On transient errors, try to resume right away.
			...
			case err != nil:
				...
				// trying to resume. Let 'withRetry' handle it by backing off.
				return err
...
func (u *Uploader) uploadChunk(p []byte) (int64, error) {
	...
	// Since it is not really a redirect, we just use 308 below.
	switch {
	...
	case resp.StatusCode != 308:
		...
		return 0, fmt.Errorf("unexpected status code %d, expecting 308", resp.StatusCode)
	}
