...
func TestValidatePrefixMetadata(t *testing.T) {
	t.Parallel()
	...
	Convey("Happy path", t, func() {
		...
		So(ValidatePrefixMetadata(m), ShouldBeNil)
	...
	Convey("Validates prefix", t, func() {
		So(ValidatePrefixMetadata(&api.PrefixMetadata{Prefix: "//"}),
			ShouldErrLike, "invalid package prefix")
	})

	Convey("No role", t, func() {
		So(ValidatePrefixMetadata(&api.PrefixMetadata{
			Prefix: "abc",
			Acls: []*api.PrefixMetadata_ACL{
				{},
			},
