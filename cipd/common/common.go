...
var (
	...
	instanceTagKeyRe = regexp.MustCompile(`^[a-z0-9_\-]+$`)

	// instanceTagValRe is a regular expression for tag values.
	//
	// Arbitrary printable ASCII symbols except whitespace.
	instanceTagValRe = regexp.MustCompile(`^[[:graph:]]+$`)
...
func ParseInstanceTag(t string) (*api.Tag, error) {
	...
	case !instanceTagKeyRe.MatchString(chunks[0]):
		return nil, fmt.Errorf("invalid tag key in %q (should be a lowercase word)", t)
	case !instanceTagValRe.MatchString(chunks[1]):
		return nil, fmt.Errorf("invalid tag key in %q (should not have whitespace)", t)
	default:
		return &api.Tag{
			Key:   chunks[0],
			Value: chunks[1],
