...
type RepositoryClient interface {
	...
	//
	// An instance metadata entry is a key-value pair, where the key is an
	// identifier-like string and the value can be an arbitrary blob up to 512 Kb
	// in size.
	//
	...
	// attached metadata is noop. Thus all metadata of an instance can be thought
	// of as a multimap, where a key maps to a set of unique values.
	//
	...
	// for example, to point to a entry to detach in DetachMetadata.
	//
	// Returns:
	//   PERMISSION_DENIED if the caller is not a WRITER for the prefix.
	//   INVALID_ARGUMENT if the request is malformed.
	...
	// (see InstaceMetadata message for details), or just taken from some existing
	// InstanceMetadata message (e.g. from inside a ListMetadataResponse).
	//
	// Detaching metadata that doesn't exist is noop.
	//
	// Returns:
	//   PERMISSION_DENIED if the caller is not an OWNER for the prefix.
	//   INVALID_ARGUMENT if the request is malformed.
	//   NOT_FOUND if the package or the instance doesn't exist.
	DetachMetadata(ctx context.Context, in *AttachMetadataRequest, opts ...grpc.CallOption) (*empty.Empty, error)
	// Lists metadata entries attached to an instance.
	//
	// Either returns all metadata or only entries with some given keys. Results
	// are sorted by the registration time of metadata entries (the most recent
	// first).
	//
	// Returns:
	//   PERMISSION_DENIED if the caller is not a READER for the prefix.
...
func (c *repositoryClient) DetachMetadata(ctx context.Context, in *AttachMetadataRequest, opts ...grpc.CallOption) (*empty.Empty, error) {
	out := new(empty.Empty)
...
type RepositoryServer interface {
	...
	//   INVALID_ARGUMENT if the request is malformed.
	//   NOT_FOUND if the package or the instance doesn't exist.
	DetachMetadata(context.Context, *AttachMetadataRequest) (*empty.Empty, error)
	// Lists metadata entries attached to an instance.
	//
	// Either returns all metadata or only entries with some given keys. Results
	// are sorted by the registration time of metadata entries (the most recent
...
func (UnimplementedRepositoryServer) DetachMetadata(context.Context, *AttachMetadataRequest) (*empty.Empty, error) {
...
func _Repository_DetachMetadata_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	...
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(RepositoryServer).DetachMetadata(ctx, req.(*AttachMetadataRequest))
	}
	return interceptor(ctx, in, info, handler)
}
