...
A LUCI project can define email templates in a project config directory.
...
where `<template_name>` must match regexp `^[a-z][a-z0-9\_]*$`.
Example:
...
* [Build](https://godoc.org/go.chromium.org/luci/buildbucket/proto#Build):
  the build for which the notification is being generated.
...
with status {{.Build.Status}} on `{{.Build.EndTime | time}}`.
...
```html
This file defines subtemplates used by otheer files.
