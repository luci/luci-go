...
func main() {
	...
	flag.StringVar(&f.TemplateRootDir, "template-root-dir", "", text.Doc(`
		Path to the email tempalte dir.
		Defaults to the parent directory of the tempalte file
	`))
	...
	if err := run(ctx, buildPath, templatePath, f); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
}

func run(ctx context.Context, buildPath, templateFile string, f parsedFlags) error {
	build, err := readBuild(buildPath)
	if err != nil {
		return fmt.Errorf("failed to read build: %s", err)
	}
