...
// stored in the datastore as a marshalled proto into the struct's Settings field.
func (s *Settings) Load(props datastore.PropertyMap) error {
	if pdata, ok := props["Settings"]; ok {
		configs := pdata.Slice()
		if len(configs) != 1 {
			return fmt.Errorf("property `Settings` for entity of kind `Settings` is invalid")
		}
		configBytes := configs[0].Value().([]byte)
		if err := proto.Unmarshal(configBytes, &s.Settings); err != nil {
			return err
		}
...
// field as a proto binary and stores it in the Settings property.
func (s *Settings) Save(withMeta bool) (datastore.PropertyMap, error) {
	props, err := datastore.GetPLS(s).Save(withMeta)
	if err != nil {
		return nil, err
	}
	bytes, err := proto.Marshal(&s.Settings)
	if err != nil {
		return nil, err
	}
	props["Settings"] = datastore.MkProperty(bytes)
	return props, nil
}
