...
func updateProject(c context.Context, cs *parsedProjectConfigSet) error {
	return datastore.RunInTransaction(c, func(c context.Context) error {
		...
		oldBuilders := make([]interface{}, 0, len(cs.ProjectConfig.Notifiers))
		...
		for _, cfgNotifier := range cs.ProjectConfig.Notifiers {
			for _, cfgBuilder := range cfgNotifier.Builders {
				...
				newBuilder := &Builder{
					...
					NotifyBlamelist: cfgNotifier.NotifyBlamelist == nil,
				}
				if newBuilder.NotifyBlamelist {
					newBuilder.BlamelistNotification = *cfgNotifier.NotifyBlamelist
				}
		...
		for i := range oldBuilders {
			oldBuilder := oldBuilders[i].(*Builder)
			newBuilder := newBuilders[i]
			if oldBuilder.Repository == newBuilder.Repository && oldBuilder.Ref == newBuilder.Ref {
