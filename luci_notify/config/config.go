...
// which are not in a given "known" set, and which are associated with a
// specific project via parent key.
func clearDeadNotifiers(c context.Context, parent *datastore.Key, knownNotifiers stringset.Set) error {
...
func updateProject(c context.Context, projectName string, projectCfg *notifyConfig.ProjectConfig, cfg *configInterface.Config) error {
	...
	err := datastore.Get(c, &existingProject)
	switch err {
	...
	err = datastore.RunInTransaction(c, func(c context.Context) error {
		...
		knownNotifiers := stringset.New(len(projectCfg.Notifiers))
		...
		if err := datastore.Put(c, notifiers); err != nil {
			return errors.Annotate(err, "saving notifiers").Err()
		}
		newProject := NewProject(projectName, cfg)
		if err := datastore.Put(c, newProject); err != nil {
			return errors.Annotate(err, "saving %s", projectName).Err()
		}
		return clearDeadNotifiers(c, parentKey, knownNotifiers)
	}, nil)
	return err
}

// clearDeadProjects deletes all Notifier structures from the datastore
// associated with dead projects. It will leave the Project structure,
// however, because it doesn't take up much space.
func clearDeadProjects(c context.Context, knownProjects stringset.Set) error {
	toDelete := []*datastore.Key{}
...
func updateNotifiers(c context.Context) error {
	...
	for _, cfg := range configs {
		...
		// Extract the config from the luci-config message.
