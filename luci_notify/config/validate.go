...
func validateNotification(c *validation.Context, cfgNotification *notifypb.Notification) {
	...
	validateRegex(c, "failed_step_regexp", cfgNotification.FailedStepRegexp)
	validateRegex(c, "failed_step_regexp_exclude", cfgNotification.FailedStepRegexpExclude)
}

func validateRegex(c *validation.Context, fieldName, regex string) {
