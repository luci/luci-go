...
func (c ProdCloudTasksClient) CreateTask(ctx context.Context, queue string, task *taskspb.Task) (*taskspb.Task, error) {
	req := &taskspb.CreateTaskRequest{
		Parent: fmt.Sprintf("projects/%s/locations/%s/queues/%s", c.appID, c.region, queue),
		Task:   task,
	}
	return c.ct.CreateTask(ctx, req)
}

func newProdCloudTasksClient(ctx context.Context, appID string, region string) (*ProdCloudTasksClient, error) {
...
func main() {
	...
	ct, err := newProdCloudTasksClient(context.Background(), appID, "us-central1")
	if err != nil {
