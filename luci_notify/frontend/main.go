...
// The context key, so that we can embed the http.Request object into the context.
var requestKey = "http.request"

// withRequest returns a context with the http.Request object in it.
...
func init() {
	...
	basemw := standard.Base().Extend(
		auth.Authenticate(authServer.CookieAuth),
		withRequestMiddleware,
	)

	// Cron endpoint.
	r.GET("/admin/update", basemw /*.Extend(gaemiddleware.RequireCron)*/, config.UpdateHandler)
