...
type Builder struct {
	...
	// It is computed as either the commit time of the revision associated
	// with the that build.
	StatusTime time.Time

	// StatusRevision can be used to decide whether a Status should be updated.
...
func LookupBuilder(c context.Context, id string, build *buildbucket.Build, commit *gitiles.Commit) (*Builder, error) {
	...
	err := datastore.RunInTransaction(c, func(c context.Context) error {
		...
		case commit.Commit == prev.StatusRevision:
			return nil

		// ...or if it's an older revision.
		case commit.Commit != prev.StatusRevision && prev.StatusTime.After(time.Time(commit.Committer.Time)):
