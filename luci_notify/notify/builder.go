...
type Builder struct {
	...
	// UpdateTime is the timestamp of the last time this Builder was updated.
	UpdateTime time.Time

	// Status is current status of the builder.
	// It is updated every time a new build has a new status.
...
// WithBuilder Gets a Builder from the datastore and executes a function with
// that Builder as a parameter.
//
...
func WithBuilder(c context.Context, id string, f func(context.Context, *Builder) error) error {
	prev := &Builder{ID: id}
