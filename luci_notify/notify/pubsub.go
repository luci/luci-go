...
func handleBuild(c context.Context, build *buildbucket.Build) error {
	...
	gClient, err := NewGitilesClient(c, gCommit.RepoURL())
	if err != nil {
		return err
	}

	// commits contains a list of commits from gCommit.Revision..builder.StatusRevision (newest..oldest).
	commits, err := gClient.History(c, builder.StatusRevision, gCommit.Revision)
	if err != nil {
		return err
	}
	if len(commits) == 0 {
	...
	err = datastore.RunInTransaction(c, func(c context.Context) error {
		...
		case index < 0:
			logging.Debugf(c, "Found build with old commit during transaction, ignoring...")
			builder.Status = StatusUnknown
			return nil
...
func BuildbucketPubSubHandler(ctx *router.Context) {
	...
	default:
		c = WithGitilesClientFactory(c, GitilesClientFactoryImpl)
		if err := handleBuild(c, build); err != nil {
