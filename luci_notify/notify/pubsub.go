...
func handleBuild(c context.Context, d *tq.Dispatcher, build *Build, history HistoryFunc) error {
	...
	if builderRepoHost != gCommit.Host || builderRepoProject != gCommit.Project || builder.Ref != gCommit.Ref {
		logging.Infof(c, "Builder %s triggered by commit to https://%s/%s/+/%s"+
			"instead of known https://%s/+/%s, ignoring...",
			builderID, gCommit.Host, gCommit.Project, gCommit.Ref, builder.Repository, builder.Ref)
		return nil
	}
	...
	err = datastore.RunInTransaction(c, func(c context.Context) error {
		...
		if firstDiff != nil && builder.NotifyBlamelist {
			diff := builder.StatusSourceManifest.Diff(firstDiff.Old)
			blamelist = blamelistFromDiff(c, diff, firstDiff)
		}
