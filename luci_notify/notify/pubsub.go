...
func handleBuild(c context.Context, d *tq.Dispatcher, build *Build, history HistoryFunc) error {
	...
	err = datastore.RunInTransaction(c, func(c context.Context) error {
		...
		case err == datastore.ErrNoSuchEntity:
			...
			return Notify(c, d, nil, &templateParams)
		case err != nil:
	...
	err = datastore.RunInTransaction(c, func(c context.Context) error {
		...
		if builder.BlamelistNotification != nil {
			if len(builder.BlamelistNotification.GetRepositoryWhitelist()) != 0 {
				blamelistLogs = blamelistLogs.Trim(NewCheckout(builder.StatusGitilesCommits))
			}
			recipients = append(recipients, blamelistLogs.Blamelist(builder.BlamelistNotification.Template)...)
		}
