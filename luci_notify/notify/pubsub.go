...
// for testing.
func handleBuild(c context.Context, ct CloudTasksClient, appId string, build *Build, getCheckout CheckoutFunc, history HistoryFunc) error {
	...
	notifyNoBlame := func(c context.Context, b config.Builder, oldStatus buildbucketpb.Status) error {
		...
		return Notify(c, ct, appId, recipients, templateInput)
	}
	...
	err = datastore.RunInTransaction(c, func(c context.Context) error {
		...
		return parallel.FanOutIn(func(ch chan<- func() error) {
			ch <- func() error { return Notify(c, ct, appId, recipients, templateInput) }
...
func BuildbucketPubSubHandler(ctx *router.Context, ct CloudTasksClient, appId string) error {
	c := ctx.Context
	...
	default:
		return handleBuild(c, ct, appId, build, srcmanCheckout, gitilesHistory)
	}
}
