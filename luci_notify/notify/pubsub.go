...
func handleBuild(c context.Context, d *tq.Dispatcher, build *Build, history HistoryFunc) error {
	...
	if builder.BlamelistNotification != nil {
		filterRepositories(buildCommits, builder.BlamelistNotification.GetRepositoryWhitelist())
	...
	err = datastore.RunInTransaction(c, func(c context.Context) error {
		...
		if outOfOrder {
			...
			return Notify(c, d, notifications, buildbucketpb.Status_STATUS_UNSPECIFIED, build, blamelistNotification, blamelist)
		}
