...
func pubsubDummyBuild(builder string, status buildbucketpb.Status, creationTime time.Time, revision string, notifyEmails ...[]EmailNotify) *Build {
	var build Build
	build.Bucket = *testutil.TestBuild("test", "hello", builder, status)
	build.Bucket.Input = &buildbucketpb.Build_Input{
	...
	build.Bucket.CreateTime, _ = ptypes.TimestampProto(creationTime)
...
func TestHandleBuild(t *testing.T) {
	...
	Convey(`Test Environment for handleBuild`, t, func() {
		...
		verifyBuilder := func(build *Build, revision string) {
			...
			So(builder.Status, ShouldEqual, build.Bucket.Status)
		}
		...
		Convey(`no revision`, func() {
			build := &Build{Bucket: *testutil.TestBuild("test", "hello", "test-builder-1", buildbucketpb.Status_SUCCESS)}
			testSuccess(build)
			grepLog("revision")
		})
