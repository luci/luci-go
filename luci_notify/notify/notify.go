...
func createEmailTask(c context.Context, recipients []EmailNotify, oldStatus buildbucketpb.Status, build *Build, lucicfg configInterface.Interface) ([]*tq.Task, error) {
	templateContext := map[string]interface{}{
		"OldStatus": oldStatus.String(),
		"Build":     build,
	}
	tasks := []*tq.Task{}
	templates, err := emailTemplates(c, build, lucicfg)
	if err != nil {
...
func emailTemplates(c context.Context, build *Build, lucicfg configInterface.Interface) ([]emailMap, error) {
	templateMap := []emailMap{}
	//project := config.Set().Project("projects/" + build.Builder.Project)
	templates, err := lucicfg.ListFiles(c, configInterface.ProjectSet(build.Builder.Project))
	if err != nil {
...
func Notify(c context.Context, d *tq.Dispatcher, notifiers []*notifyConfig.Notifier, oldStatus buildbucketpb.Status, build *Build, lucicfg configInterface.Interface) error {
	...
	tasks, err := createEmailTask(c, recipients, oldStatus, build, lucicfg)
	if err != nil {
