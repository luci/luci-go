...
func taskName(task *internal.EmailTask, dedupKey string) string {
	// Mostly copied from luci/appengine/tq/tq.go.
...
func Notify(c context.Context, ct CloudTasksClient, appId string, recipients []EmailNotify, templateParams *notifypb.TemplateInput) error {
	...
	for emailKey, task := range tasks {
		...
		req := &taskspb.CreateTaskRequest{
			Parent: fmt.Sprintf("projects/%s/locations/us-central1/queues/email", appId),
			Task: &taskspb.Task{
		...
		if _, err := ct.CreateTask(c, req); err != nil {
			return err
		}
	}
	return nil
