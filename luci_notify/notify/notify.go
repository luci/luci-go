...
func createEmailTask(c context.Context, recipients []string, build *buildbucket.Build, builder *Builder) (*tq.Task, error) {
...
func Notify(c context.Context, d *tq.Dispatcher, notifiers []*config.Notifier, build *buildbucket.Build, builder *Builder) error {
	...
	task, err := createEmailTask(c, recipientSet.ToSlice(), build, builder)
	if err != nil {
		return errors.Annotate(, "failed to create email task").Tag(transient.Tag).Err()
	}
	d.AddTask(task)
	return nil
}
