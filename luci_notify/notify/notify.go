...
// ComputeRecipients computes the set of recipients given a set of notifications, and
// potentially static and dynamic blamelists.
//
// A static blamelist is computed from the input commit, and a dynamic blamelist is derived
// from output commits.
func ComputeRecipients(notifications notifypb.Notifications, staticBlame []*gitpb.Commit, dynamicBlame Logs) []EmailNotify {
	...
	for _, notification := range n {
		...
		if len(whitelist) == 0 {
			recipients = append(recipients, commitsBlamelist(staticBlame, notification.Template)...)
			continue
		}

		// If the whitelist is non-empty, use the dynamic blamelist.
		whiteset := stringset.NewFromSlice(whitelist...)
		recipients = append(recipients, dynamicBlame.Filter(whiteset).Blamelist(notification.Template)...)
	}
	return recipients
}
