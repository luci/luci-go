...
func UpdateTreeClosers(c context.Context, build *buildbucketpb.Build) error {
	...
	parentBuilder := &config.Builder{
		...
		ID:         fmt.Sprintf("%s/%s", build.Builder.Bucket, build.Builder.Builder),
	}
	...
	err := datastore.Run(c, q, func(tc *config.TreeCloser) {
		...
		if tc.Status != newStatus {
			...
			err := datastore.Put(c, tc)
			if err != nil {
				// Don't just return this error, as doing so would cause us to stop and
				// not try to update any other TreeClosers watching this builder.
	...
	if len(putErrors) > 0 {
		return errors.NewMultiError(putErrors...)
	}
	return nil
}
