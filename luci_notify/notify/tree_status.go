...
func updateTrees(c context.Context, ts treeStatusClient) error {
	...
	// Guards access to both treeClosers and closingEnabledForProject.
	mtx := sync.Mutex{}
	var treeClosers []*config.TreeCloser
	closingEnabledForProject := map[string]bool{}
		...
		for _, project := range projects {
			...
			ch <- func() error {
				return datastore.RunInTransaction(c, func(c context.Context) error {
					...
					closingEnabledForProject[project.Name] = project.TreeClosingEnabled
	...
	return parallel.WorkPool(32, func(ch chan<- func() error) {
		for host, treeClosers := range groupTreeClosers(treeClosers) {
			...
			ch <- func() error { return updateHost(c, ts, host, treeClosers, &closingEnabledForProject) }
		}
	})
}
...
func updateHost(c context.Context, ts treeStatusClient, host string, treeClosers []*config.TreeCloser, closingEnabledForProject *map[string]bool) error {
	...
	anyEnabled := false
	for _, tc := range treeClosers {
		enabled, has := (*closingEnabledForProject)[tcProject(tc)]
		anyEnabled = anyEnabled || (has && enabled)
	}
	...
	for _, tc := range treeClosers {
		...
		if anyEnabled && !(*closingEnabledForProject)[tcProject(tc)] {
			continue
		}
