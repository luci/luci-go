...
// NOTE: If you want to add new emoji, do so in Gatekeeper, not here. The full
// list will be copied over before Gatekeeper is deleted.
var emoji = []string{
	"(｡>﹏<｡)",
	"☃",
	"☀ Tree is open ☀",
...
func randomEmoji() string {
	emoji := emoji[rng.Intn(len(emoji))]
	if emoji[len(emoji)-1] == ')' {
		return emoji + " "
	}
	return emoji
}
