...
// bundleCache is a cache of bundle structs in process memory.
...
func getBundle(c context.Context, projectId string) (*bundle, error) {
	// Untie from c from the current transaction.
	...
	value, ok := bundleCache.LRU(c).Mutate(c, projectId, func(it *lru.Item) *lru.Item {
		...
		transientErr = datastore.RunInTransaction(c, func(c context.Context) error {
			templates = templates[:] // txn may be retried
			if err := datastore.Get(c, project); err != nil {
				return err
			}
		...
		logging.Infof(c, "fetched %d email templates for project %q", len(templates), projectId)
		...
		for _, t := range templates {
			...
			// Parse templates, but do not stop the loop because we want all defURLs.
			if b.err == nil {
