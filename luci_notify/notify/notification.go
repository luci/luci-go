...
luci-notify has detected a new {{ .Build.Result }} on builder "{{ .BuilderName }}".
...
<table>
  ...
  <tr>
    <td>Created at:</td>
    <td>{{ .GetCreatedTime.String }}</td>
  </tr>
  <tr>
    <td>Finished at:</td>
    <td>{{ .GetCompletedTime.String }}</td>
  </tr>
  {{ if eq .Build.Result "FAILURE" }}
  <tr>
    <td>Failure reason:</td>
    <td>{{ .Build.FailureReason }}</td>
  </tr>
  {{ end }}
</table>
...
func (n *Notification) Email() (*Email, error) {
	...
	return BaseEmail().
		...
		Subject("[%s] build %s in %s on %s",
			newStatus,
			id,
			oldStatus,
			newStatus,
		).
		HTMLBody(emailTemplate, n.Build)
}
