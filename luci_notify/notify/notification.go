...
type Notification struct {
	...
	// State is the Builder before Build was seen.
	//
	// This is used primarily for constructing the content of the
	// final notification for each communication channel.
	State *Builder
...
func shouldNotify(n *config.NotificationConfig, build *buildbucket.BuildInfo, state *Builder) bool {
	return (n.OnSuccess && build.Build.Result == "SUCCESS") ||
		(n.OnFailure && build.Build.Result == "FAILURE") ||
		(n.OnChange && state.LastBuildResult != "UNKNOWN" &&
			state.LastBuildResult != build.Build.Result)
}
...
func CreateNotification(notifiers []*config.Notifier, build *buildbucket.BuildInfo, state *Builder) *Notification {
