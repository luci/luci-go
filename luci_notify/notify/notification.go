...
<table>
  ...
  <tr>
    <td>Created at:</td>
    <td>{{ .Build.GetCreatedTime.String }}</td>
  </tr>
  <tr>
    <td>Finished at:</td>
    <td>{{ .Build.GetCompletedTime.String }}</td>
  </tr>
...
func (n *Notification) email() (*Email, error) {
	return BaseEmail().
		To(n.EmailRecipients...).
		Subject(`[Build %s] Builder %s on %s`,
			n.Build.Build.Result,
			n.Build.Parameters.BuilderName,
			n.Build.Build.Bucket,
		).
		HTMLBody(emailTemplate, n)
}
...
func CreateNotification(notifiers []*config.Notifier, build *buildbucket.BuildInfo, state *BuilderState) *Notification {
	if build.Build.CreatedTs <= state.LastBuildTime {
		// TODO(mknyszek): There must be something better than just ignoring it.
		//
		// This case is logged when looking up/updating builder state.
		return nil
	}
...
func (n *Notification) Dispatch(c context.Context) error {
	e, err := n.email()
	if err != nil {
		return errors.Annotate(err, "failed to build email").Err()
	}
