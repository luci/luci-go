...
func NewCheckout(commits notifypb.GitilesCommits) {
	results := make(Checkout)
	for _, gitilesCommit := range g.GetCommits() {
		repo := fmt.Sprintf("https://%s/%s", gitilesCommit.Host, gitilesCommit.Project)
		results[repo] = gitilesCommit.Id
	}
	return results
}
...
func (c Checkout) ToGitilesCommits() notifypb.GitilesCommits {
	...
	sort.Slice(result.Commits, func(i, j *buildbucketpb.GitilesCommit) bool {
		hostResult := strings.Compare(i.Host, j.Host)
		if hostResult == 0 {
			return strings.Compare(i.Project, j.Project) < 0
		}
		return hostResult < 0
	})
	return result
...
func (c Checkout) Filter(whitelist []string) Checkout {
...
// Trim uses a Checkout to cut out information from the Logs which
// do not pertain to the checkout, and return a new trimmed Logs.
//
// More specifically, Trim will remove repositories which are not
// found in the checkout, or if the checkout contains a revision which
// is unavailable to the Logs. If the Logs has the relevant repository
// as well as the commit from the Checkout, all commits older than that
// from the Logs will be trimmed.
func (l Logs) Trim(checkout Checkout) Logs {
	newLogs := make(Logs)
	for repo, log := range l {
		commit, ok := checkout[repo]
		if !ok {
			continue
		}
