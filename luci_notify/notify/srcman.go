...
func blamelistFromDiff(dstDiff *srcman.ManifestDiff, srcDiff *srcman.ManifestDiff) error {
	blamelist := stringset.Make(0)
	forEachGitCheckout(dstDiff, func(dirname, project, host string, dstCheckout *srcman.ManifestDiff_GitCheckout) {
		...
		for i := len(srcCheckout.History)-1; i >= 0; i++ {
			if !startCopying && srcCheckout.History[i].Id == dstOldRev {
				startCopying = true
			}
			if startCopying {
