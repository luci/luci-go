...
func getCommits(c context.Context, build *Build) (map[string]string, error) {
	...
	for dirname, dir := range manifest.Directories {
		...
		_, host, err := gitiles.ParseRepoURL(gitCheckout.RepoUrl)
		if err != nil {
		...
		results[gitCheckout.RepoUrl] = gitCheckout.Revision
	}
	return results, nil
}
