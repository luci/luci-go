...
// FakeTreeStatusApp simulates the behaviour of a real tree status instance, but
// locally, in-memory.
type FakeTreeStatusApp struct {
	statusForHosts map[string]TreeStatus
	nextKey        int
	mtx            sync.Mutex
}

func (ts *FakeTreeStatusApp) getStatus(host string) (*TreeStatus, error) {
	ts.mtx.Lock()
	defer ts.mtx.Unlock()
...
func (ts *FakeTreeStatusApp) putStatus(host, message, prevKey string) error {
	ts.mtx.Lock()
	defer ts.mtx.Unlock()
	...
	ts.statusForHosts[host] = TreeStatus{
		"buildbot@chromium.org", message, strconv.Itoa(key), status,
	}
	return nil
}
...
func TestUpdateTrees(t *testing.T) {
	Convey("Test environment", t, func() {
		...
		testUpdateTrees := func(initialTreeStatus, builder1Status, builder2Status, expectedStatus config.TreeCloserStatus) {
			...
			status, err := ts.getStatus("chromium-status.appspot.com")
			So(err, ShouldBeNil)
		...
		Convey("Multiple trees", func() {
			...
			status, err = ts.getStatus("v8-status.appspot.com")
			So(err, ShouldBeNil)
			So(status.status, ShouldEqual, config.Closed)
		})
	})
