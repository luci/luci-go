...
// FromProperty implements datastore.PropertyConverter. It parses a '[]byte'
// into an embedded 'Notifications' when used with the "go.chromium.org/gae" library.
...
var _ datastore.PropertyConverter = (*OptionalBlamelistNotification)(nil)

// ToProperty implements datastore.PropertyConverter. It causes an embedded
// 'OptionalBlamelistNotification' to serialize to an unindexed '[]byte' when used with the
// "go.chromium.org/gae" library.
func (p *OptionalBlamelistNotification) ToProperty() (prop datastore.Property, err error) {
	data, err := proto.Marshal(p)
	if err == nil {
		prop.SetValue(data, datastore.NoIndex)
	}
...
// FromProperty implements datastore.PropertyConverter. It parses a '[]byte'
// into an embedded 'OptionalBlamelistNotification' when used with the "go.chromium.org/gae" library.
func (p *OptionalBlamelistNotification) FromProperty(prop datastore.Property) error {
	data, err := prop.Project(datastore.PTBytes)
	if err != nil {
		return err
	}
