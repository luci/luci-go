...
func (n *Notification) ShouldNotify(oldStatus buildbucketpb.Status, newBuild *buildbucketpb.Build) bool {
	...
	if n.FailedStepRegexp != "" {
		// We should never get an invalid regex here, as our validation
		// should catch this. If we do, posRegex will be nil and hence
		// ignored below.
	...
	for _, step := range newBuild.Steps {
		if step.Status == buildbucketpb.Status_FAILURE {
			if (posRegex == nil || posRegex.MatchString(step.Name)) && (negRegex == nil || !negRegex.MatchString(step.Name)) {
				return true
			}
		}
	}
