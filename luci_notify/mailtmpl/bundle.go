...
func (b *Bundle) GenerateStatusMessage(templateName string, input *config.TemplateInput) (message string) {
	var err error
...
func generateDefaultStatusMessage(input *config.TemplateInput) string {
	const template = "{{ stepNames .MatchingFailedSteps }} on {{ buildUrl . }} {{ .Build.Builder.Builder }} from {{ .Build.Output.GitilesCommit.Id }}"

	t, err := text.New("").Funcs(Funcs).Parse(template)
	if err != nil {
		panic(errors.Annotate(err, "parsing of the default status message template has failed").Err())
	}
