...
func buildInputSpec(opts *isolate.ArchiveOptions) (string, *command.InputSpec, error) {
	...
	fmt.Printf("execRoot=%s inputSpec=%+v\n", execRoot, inputSpec)
	if len(inputSpec.InputExclusions) > 0 {
		fmt.Printf("inputExclusions=%+v\n", inputSpec.InputExclusions[0])
		reg := inputSpec.InputExclusions[0].Regex
		for _, p := range inputPaths {
			absPath := filepath.Clean(filepath.Join(execRoot, p))
			m, _ := regexp.MatchString(reg, absPath)
			fmt.Printf("regex=%s absPath=%s matched?=%v\n", reg, absPath, m)
		}
	}
