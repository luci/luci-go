...
func NewUploader(cl *rexec.Client) *Uploader {
	return &Uploader{
		cl:      cl,
...
func (up *Uploader) Upload(ctx context.Context, opts ...*isolate.ArchiveOptions) ([]digest.Digest, error) {
	...
	for _, opt := range opts {
		...
		rootDg, chks, _, err := tree.ComputeMerkleTree(execRoot, inputSpec, chunker.DefaultChunkSize, up.cl.FileMetadataCache)
		if err != nil {
	...
	if err := up.cl.GrpcClient.UploadIfMissing(ctx, chunkers...); err != nil {
