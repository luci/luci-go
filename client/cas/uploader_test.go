...
func TestUploader(t *testing.T) {
	...
	Convey(`Filter files`, t, testfs.MustWithTempDir(t, "", func(tmpDir string) {
		...
		dgs, err := up.Upload(context.Background(), &isolate.ArchiveOptions{
			Isolate:             isol1Path,
			IgnoredPathFilterRe: filepath.Join("filtered", "foo"),
		})
		So(err, ShouldBeNil)
		So(dgs, ShouldHaveLength, 1)

		// `foo*` files are filtered away
		filteredDirPb := &repb.Directory{Files: []*repb.FileNode{
			{Name: "foo", Digest: fooDg.ToProto()},
			{Name: "foo2", Digest: fooDg.ToProto()},
		}}
		filteredDirDg, err := digest.NewFromMessage(filteredDirPb)
		So(err, ShouldBeNil)
		isol1Dir := &repb.Directory{
			Files: []*repb.FileNode{
				{Name: "bar", Digest: barDg.ToProto()},
			},
			Directories: []*repb.DirectoryNode{
				{Name: "filtered", Digest: filteredDirDg.ToProto()},
			},
		}
		cas := e.Server.CAS
		blob, ok := cas.Get(dgs[0])
		So(ok, ShouldBeTrue)
		fmt.Printf("marshaled isol1Dir=%+v\nblob=%+v\n", mustMarshal(isol1Dir), blob)
		// So(blob, ShouldResemble, mustMarshal(isol1Dir))

		So(cas.BlobWrites(fooDg), ShouldEqual, 0)
		So(cas.BlobWrites(barDg), ShouldEqual, 1)
		// Fail on purpose to see what's going on
