...
const testChunkerSize = chunker.DefaultChunkSize

func TestChunkerDeduper(t *testing.T) {
	Convey(`Basic`, t, func() {
		fooCh := chunker.NewFromBlob([]byte("foo"), testChunkerSize)
		barCh := chunker.NewFromBlob([]byte("bar"), testChunkerSize)
		bazCh := chunker.NewFromBlob([]byte("baz"), testChunkerSize)
...
func chunkerFromFile(t *testing.T, path string) *chunker.Chunker {
	...
	So(err, ShouldBeNil)
	return chunker.NewFromFile(path, dg, testChunkerSize)
}
