...
func (c *contextRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	la, err := srv.Initialize(ctx)
	if err != nil {
		logging.WithError(err).Errorf(ctx, "Failed to bind the local auth server")
		return ExitCodeInternalError
	}
	defer srv.Close(ctx) // close the server no matter what

	// Start the serving loop.
	if err := srv.Start(); err != nil {
		logging.WithError(err).Errorf(ctx, "Failed to start the local auth serving loop")
		return ExitCodeInternalError
	}
