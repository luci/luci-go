...
func FetchAndMap(ctx context.Context, isolatedHash isolated.HexDigest, c *isolatedclient.Client, cache cache.Cache, outDir string) (map[isolated.HexDigest]*isolated.Isolated, Stats, error) {
	start := time.Now()
	...
	added := append([]int64{}, cache.GetAdded()...)
	used := append([]int64{}, cache.GetUsed()...)
