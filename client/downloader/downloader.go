...
func (d *Downloader) doFileJob(name string, details *isolated.File, outputDir string) {
	...
	// This process needs to be serialized to avoid an error when more than one
	// thread attempts to make the same directory.

	// Create a context that makes locking safer.
	err := func() error {
		dir := path.Dir(filename)

		// Fast path: if the cache has the directory, keep going.
		d.muCache.RLock()
		if d.dirCache.Has(dir) {
			d.muCache.RUnlock()
			return nil
		}
		d.muCache.RUnlock()

		// Slow path: create the directory recursively.
		// After that, add the directory and all parent directories to the cache.

		// Call MkdirAll first, so that when we update the cache, we're positive
		// that the dir exists.
		//
		// Note, calling MkdirAll without a lock is safe because MkdirAll will
		// not error if it finds that any part of the directory structure it is
		// trying to create already exists (that is, it double-checks).
		...
		// Double check, since another thread may have already added it.
		if d.dirCache.Has(dir) {
			return nil
		}
		d.dirCache.Add(dir)
		iterDir := path.Dir(dir)
		for iterDir != outputDir {
			if !d.dirCache.Has(iterDir) {
				d.dirCache.Add(iterDir)
			}
			iterDir = path.Dir(iterDir)
		}
		return nil
	}()
	if err != nil {
