...
func New(ctx context.Context, c *isolatedclient.Client, hash isolated.HexDigest,
	...
	if options.MaxFileStatsInterval == 0 {
		options.MaxFileStatsInterval = time.Second * 5
	}

	interval := 100 * time.Millisecond
	if interval > options.MaxFileStatsInterval {
		interval = options.MaxFileStatsInterval
	}
