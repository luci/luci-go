...
func TestDownloaderFetchIsolated(t *testing.T) {
	...
	Convey(`A downloader should be able to download the isolated.`, t, func() {
		...
		d := New(ctx, client, 3)
		d.FetchIsolated(isolated2hash, func(name string, _ *isolated.File) (io.WriteCloser, error) {
			var buf dummyFile
			registry[name] = &buf
			return &buf, nil
		})
	...
	Convey(`A downloader should be able to be cancelled.`, t, func() {
		registry := map[string]*dummyFile{}
		d := New(ctx, client, 6)
		d.FetchIsolated(isolated2hash, func(name string, _ *isolated.File) (io.WriteCloser, error) {
			d.Cancel(errors.Reason("failed to create file").Err())
			var buf dummyFile
			registry[name] = &buf
			return &buf, nil
		})
		So(d.Close(), ShouldNotBeNil)
	})
}
