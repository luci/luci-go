...
func TestDownloaderFetchIsolated(t *testing.T) {
	...
	Convey(`A downloader should create intermediate subdirs as needed.`, t, testfs.MustWithTempDir(t, "isolated", func(tmpDir string) {
		tmpDir += "/temp"
		var mu sync.Mutex
		var files []string
	...
	Convey(`A downloader should accept paths with trailing slashes`, t, testfs.MustWithTempDir(t, "isolated", func(tmpDir string) {
		tmpDir += "/"
		var mu sync.Mutex
		var files []string
		d := New(ctx, client, isolated2hash, tmpDir, &Options{
