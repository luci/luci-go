...
func printChange(c *gerrit.Change) {
	fmt.Printf("ID %s\n", c.ID)
	...
	printAccountInfo("Owner", &c.Owner)
	printAccountInfo("Submitter", &c.Submitter)
	for _, i := range c.Reviewers.CC {
		printAccountInfo("CC", &i)
	}
	for _, i := range c.Reviewers.Reviewer {
		printAccountInfo("Reviewer", &i)
	}
	for _, i := range c.Reviewers.Removed {
		printAccountInfo("Removed", &i)
	}
}

func printAccountInfo(title string, i *gerrit.AccountInfo) {
	if i.Name != "" && i.Email != "" {
		fmt.Printf("%s: %s <%s>\n", title, i.Name, i.Email)
	} else if i.Email != "" {
		fmt.Printf("%s: %s <%s>\n", title, i.Email, i.Email)
	}
}
