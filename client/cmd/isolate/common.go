...
func (c *isolateFlags) Init(f *flag.FlagSet) {
	...
	f.Var(&c.Blacklist, "blacklist", "List of globs to use as blacklist filter when uploading directories. Prefer \"-ignored-path-filter-re\", and this will be deprecated soon.")
	...
	f.BoolVar(&c.AllowMissingFileDir, "allow-missing-file-dir", true, "If this flag is true, invalid entries in the isolated file are only logged, but won't stop it from being processed.")
}
