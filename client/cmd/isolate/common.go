...
func recreateTree(outDir string, rootDir string, deps []string) error {
	if err := filesystem.MakeDirs(outDir); err != nil {
		return lucierrors.Annotate(err, "failed to create directory: %s", outDir).Err()
	}
	...
	for _, dep := range deps {
		...
		if _, ok := createdDirs[dstDir]; !ok {
			if err := filesystem.MakeDirs(dstDir); err != nil {
				return lucierrors.Annotate(err, "failed to call MakeDirs(%s)", dstDir).Err()
			}
		...
		if err != nil {
			return lucierrors.Annotate(err, "failed to call HardlinkRecursively(%s, %s)", dep, dst).Err()
		}
	}
	return nil
}
