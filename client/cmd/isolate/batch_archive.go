...
isolate. Format of files is:
{
  ...
  "args": [list of command line arguments for single 'archive' command]
		...
		CommandRun: func() subcommands.CommandRun {
			...
			c.Flags.BoolVar(&c.allowCommandAndRelativeCWD, "allow-command-and-relative-cwd", true, "This flag specifies whether client allows to use command and relative_cwd filed in isolate file. See crbug.com/1069704 for detail.")
...
type batchArchiveRun struct {
	...
	dumpJSON                   string
	allowCommandAndRelativeCWD bool
...
func (c *batchArchiveRun) main(a subcommands.Application, args []string) error {
	...
	return batchArchive(ctx, client, al, c.dumpJSON, c.maxConcurrentChecks, c.maxConcurrentUploads, args, blacklistStrings, c.allowCommandAndRelativeCWD)
}

// batchArchive archives a series of isolates specified by genJSONPaths.
func batchArchive(ctx context.Context, client *isolatedclient.Client, al archiveLogger, dumpJSONPath string, concurrentChecks, concurrentUploads int, genJSONPaths []string, blacklistStrings []string, allowCommandAndRelativeCWD bool) error {
	...
	for _, genJSONPath := range genJSONPaths {
		...
		opts.AllowCommandAndRelativeCWD = allowCommandAndRelativeCWD
		if err != nil {
			return err
		}
		...
		isolSummary, err := a.Archive(deps, rootDir, opts.Blacklist, opts.Isolated, isol)
