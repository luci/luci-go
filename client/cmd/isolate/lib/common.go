...
type baseCommandRun struct {
	...
	rbeClientFactory func(ctx context.Context, instance string, opts auth.Options, readOnly bool) (*client.Client, error)
...
func (c *baseCommandRun) newRBEClient(ctx context.Context, instance string, opts auth.Options, readOnly bool) (*client.Client, error) {
	factory := c.rbeClientFactory
	if factory == nil {
		factory = chromecas.NewRBEClient
	}
	return factory(ctx, instance, opts, readOnly)
}

func (c *baseCommandRun) newCASClient(ctx context.Context, instance string, opts auth.Options, readOnly bool) (*cas.Client, error) {
	factory := c.casClientFactory
	if factory == nil {
		factory = chromecas.NewCASClient
	}
	return factory(ctx, instance, opts, readOnly)
}
...
func (r *baseCommandRun) uploadToCAS(ctx context.Context, dumpJSON string, authOpts auth.Options, fl *chromecas.Flags, al *archiveLogger, opts ...*isolate.ArchiveOptions) ([]digest.Digest, error) {
...
func (r *baseCommandRun) uploadToCASNew(ctx context.Context, authOpts auth.Options, fl *chromecas.Flags, al *archiveLogger, opts ...*isolate.ArchiveOptions) ([]digest.Digest, error) {
...
// TODO(crbug.com/1193375): remove this func after migrating to RBE's cas package.
func (r *baseCommandRun) uploadToCASLegacy(ctx context.Context, authOpts auth.Options, fl *chromecas.Flags, al *archiveLogger, opts ...*isolate.ArchiveOptions) ([]digest.Digest, error) {

	// To cancel |uploadEg| when there is error in |digestEg|.
	ctx, cancel := context.WithCancel(ctx)
	defer cancel()

	cl, err := r.newRBEClient(ctx, fl.Instance, authOpts, false)
	if err != nil {
		return nil, err
	}
	defer cl.Close()
