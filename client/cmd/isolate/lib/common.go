	...
	eg.Go(func() error {
		...
		for i, o := range opts {
			...
			if o.IgnoredPathFilterRe != "" {
				pattern := o.IgnoredPathFilterRe
				if runtime.GOOS == "windows" {
					// CAS's regexp filter is OS-agnostic, while Isolate' s filter is
					// OS-specific. Convert to OS-agnostic.
					pattern = strings.Replace(pattern, "\\", "/", -1)
				}
				if in.Exclude, err = regexps.Compile(pattern); err != nil {
