...
func (c *downloadRun) main(a subcommands.Application, args []string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to call FetchIsolated(): %v", err)
	}
	if c.outputFiles != "" {
		...
		if err := ioutil.WriteFile(c.outputFiles, []byte(filesData), 0664); err != nil {
			return fmt.Errorf("failed to call WriteFile(%s, ...); %v", c.outputFiles, err)
		}
	}
	return nil
}
...
func (c *downloadRun) Run(a subcommands.Application, args []string, _ subcommands.Env) int {
	...
	if err := c.main(a, args); err != nil {
		fmt.Fprintf(a.GetErr(), "%s: failed to call main(): %v\n", a.GetName(), err)
		return 1
	}
	return 0
}
