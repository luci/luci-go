...
func (s *swarmingServiceImpl) GetTaskOutputs(c context.Context, taskID, outputDir string, ref *swarming.SwarmingRpcsFilesRef) ([]string, error) {
	...
	dl := downloader.New(c, isolatedClient, isolated.HexDigest(ref.Isolated), dir)
	dl.SetMaxConcurrentJobs(s.worker)
	var filesMu sync.Mutex
	var files []string
	dl.SetFileCallback(func(name string, _ isolated.File) {
		filesMu.Lock()
