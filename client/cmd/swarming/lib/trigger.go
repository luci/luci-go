...
type optionalDimensionFlag struct {
	kv         *swarming.SwarmingRpcsStringPair
	expiration int64
}

var _ = flag.Value(new(optionalDimensionFlag))

// String implements the flag.Value interface.
func (f *optionalDimensionFlag) String() string {
	if f == nil || f.isEmpty() {
		return ""
	}
...
func (f *optionalDimensionFlag) Set(s string) error {
	if s == "" {
		return nil
	}
	...
	if colon == -1 {
		return errors.Reason("cannot find key in -optional-dimension: %q", s).Err()
	}
	k := s[:colon]
	valExp := s[colon+1:]
	colon = strings.LastIndexByte(valExp, ':')
	if colon == -1 {
		return errors.Reason("cannot find sep between value and expiration in -optional-dimension: %q", valExp).Err()
	}
	exp, err := strconv.ParseInt(valExp[colon+1:], 10, 64)
	if err != nil {
		return errors.Reason("cannot parse the expiration in -optional-dimension: %q", valExp).Err()
	}
...
func (f *optionalDimensionFlag) isEmpty() bool {
	return f.kv == nil
}
...
type triggerRun struct {
	...
	optionalDimension         optionalDimensionFlag
