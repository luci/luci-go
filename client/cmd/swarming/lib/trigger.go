...
func (c *triggerRun) Init(defaultAuthOpts auth.Options) {
	...
	// Other.
	var rawCmd bool
	c.Flags.BoolVar(&rawCmd, "raw-cmd", true, "DEPRECATED (no meaning): When set, the command after -- is run on the bot. Note that this overrides any command in the .isolated file.")
