...
// CmdTerminate returns an object for the `terminate` subcommand.
...
func (t *terminateRun) pollTask(ctx context.Context, taskID string, service swarmingService) (*swarming.SwarmingRpcsTaskResult, error) {
	...
	for {
		...
		state, statErr := parseTaskState(res.State)
		if statErr != nil {
			return res, statErr
		}
		if !state.Alive() {
...
func (t *terminateRun) terminateBot(ctx context.Context, botID string, service swarmingService) error {
	...
	if t.wait {
		taskres, taskerr := t.pollTask(ctx, res.TaskId, service)
		if taskerr != nil {
			fmt.Printf("Failed to terminate %s\n", botID)
			return taskerr
		}
