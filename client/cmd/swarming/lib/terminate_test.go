...
func TestTerminateBots(t *testing.T) {
	...
	Convey(`Test terminate`, t, func() {
		...
		Convey(`other than status "complete" returned when terminating bot with wait`, func() {
			...
			So(err, ShouldErrLike, "couldn't complete")
			So(givenBotID, ShouldResemble, statusNotCompletedBotID)
			So(givenTaskID, ShouldResemble, statusNotCompletedTaskID)
		})
