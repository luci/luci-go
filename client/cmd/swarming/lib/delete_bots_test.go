...
func TestDeleteBots(t *testing.T) {
	...
	Convey(`Test delete with no error`, t, func() {
	...
	Convey(`Test when delete finish with error`, t, func() {
		...
		Convey(`Test when bot wasn't deleted`, func() {
			givenbotID = []string{}
			b.force = true
			botNotDeletedservice := &testService{
				deleteBots: func(ctx context.Context, botID string) (*swarming.SwarmingRpcsDeletedResponse, error) {
					givenbotID = append(givenbotID, botID)
			...
			err := b.deleteBotsInList(ctx, []string{"testbot123", "cannotdeletebotID", "testbot456", "testbot789"}, botNotDeletedservice)
