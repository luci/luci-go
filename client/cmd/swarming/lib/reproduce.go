...
func (c *reproduceRun) prepareTaskRequestEnvironment(ctx context.Context, taskID string, service swarmingService) (*exec.Cmd, error) {
	...
	for _, prefix := range properties.EnvPrefixes {
		...
		for _, value := range prefix.Value {
			absPath, err := filepath.Abs(filepath.Join(c.work, value))
			if err != nil {
				return nil, errors.Annotate(err, "absPath PROBLEM").Err()
			}
			//absPath := filepath.Clean(filepath.Join(c.work, value))
	...
	//cmd := exec.CommandContext(ctx, fmt.Sprintf("/usr/local/google/home/jojwang/cr/infra/go/src/go.chromium.org/luci/client/cmd/swarming/lib/work/out/Release/.task_template_packages/%s", processedCmds[0]), processedCmds[1:]...)
