...
func (c *reproduceRun) main(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	if err = c.reproduceTaskRequestCommand(ctx, args[0], service, nil); err != nil {
		return errors.Annotate(err, "failed to complete command").Err()
	}
	return nil
}

func (c *reproduceRun) reproduceTaskRequestCommand(ctx context.Context, taskID string, service swarmingService, w io.Writer) error {
	cmd, err := c.createTaskRequestCommand(ctx, taskID, service)
	if err != nil {
...
func (c *reproduceRun) createTaskRequestCommand(ctx context.Context, taskID string, service swarmingService) (*exec.Cmd, error) {
	...
	if err != nil {
		return nil, errors.Annotate(err, "failed to get task request").Err()
	}
