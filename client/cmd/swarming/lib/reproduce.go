...
// matchInvalidInvocationIDChars is used to find chars that are invalid for an invocation ID.
var matchInvalidInvocationIDChars = regexp.MustCompile(`[^a-z0-9_\-:.]`)

// TODO(crbug.com/1188381): use the correct instance (e.g."staging.results.api.cr.dev")
// depending on the swarming instance.
// resultsHost is the ResultDB instance to use.
var resultsHost = chromeinfra.RessultDBHost
...
type reproduceRun struct {
	...
	createInvocation func(context.Context, *http.Client, string) (*resultpb.Invocation, metadata.MD, error)
}
...
func (c *reproduceRun) prepareTaskRequestEnvironment(ctx context.Context, taskID string, service swarmingService) (*exec.Cmd, lucictx.Exported, error) {
	...
	if tr.Resultdb != nil && tr.Resultdb.Enable {
		...
		invocation, med, err := c.createInvocation(ctx, authcli, tr.Realm)
		if err != nil {
...
func createInvocation(ctx context.Context, authcli *http.Client, realm string) (*resultpb.Invocation, metadata.MD, error) {
