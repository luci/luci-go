...
type reproduceRun struct {
	...
	// createInvocation is sued in testing to insert a mock method.
	createInvocation func(context.Context, *http.Client, string, metadata.MD) (*resultpb.Invocation, error)
}
...
func (c *reproduceRun) prepareTaskRequestEnvironment(ctx context.Context, taskID string, service swarmingService) (*exec.Cmd, lucictx.Exported, error) {
	...
	if tr.Resultdb != nil && tr.Resultdb.Enable {
		...
		med := metadata.MD{}
		invocation, err := c.createInvocation(ctx, authcli, tr.Realm, med)
		//invocation, err := c.createInvocation(ctx, authcli, "chromium:public", med)
		if err != nil {
...
func createInvocation(ctx context.Context, authcli *http.Client, realm string, med metadata.MD) (*resultpb.Invocation, error) {
