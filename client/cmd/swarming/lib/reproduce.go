...
type reproduceRun struct {
	...
	createInvocation func(context.Context, *http.Client, string, string) (*resultpb.Invocation, metadata.MD, error)
	resultsHost      string
}
...
func (c *reproduceRun) prepareTaskRequestEnvironment(ctx context.Context, taskID string, service swarmingService) (*exec.Cmd, lucictx.Exported, error) {
	...
	if tr.Resultdb != nil && tr.Resultdb.Enable {
		...
		invocation, med, err := c.createInvocation(ctx, authcli, tr.Realm, c.resultsHost)
		if err != nil {
...
func createInvocation(ctx context.Context, authcli *http.Client, realm string, resultsHost string) (*resultpb.Invocation, metadata.MD, error) {
