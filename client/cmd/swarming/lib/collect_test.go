...
func TestCollectPollForTaskResult(t *testing.T) {
	...
	Convey(`Test bot finished with outputs on Isolate server`, t, func() {
		...
		service := &testService{
			...
			getTaskFilesFromIsolate: func(c context.Context, output string, ref *swarming.SwarmingRpcsFilesRef) ([]string, error) {
				writtenTo = output
	...
	Convey(`Test bot finished with outputs on CAS`, t, func() {
		...
		service := &testService{
			...
			getTaskFilesFromCAS: func(c context.Context, outdir string, _ *rbeclient.Client, casRef *swarming.SwarmingRpcsCASReference) ([]string, error) {
				writtenTo = outdir
