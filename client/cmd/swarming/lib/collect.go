	...
	err := retry.Retry(ctx, transient.Only(retry.Default), func() error {
		...
		if c.outputDir != "" {
			...
			if result.OutputsRef != nil {
				outputs, err = service.GetTaskFilessFromIsolate(ctx, outdir, result.OutputsRef)
				if err != nil {
					return tagTransientGoogleAPIError(err)
				}
			}
			if result.CasOutputRoot != nil {
				...
				outputs, err = service.GetTaskFilesFromCAS(ctx, outdir, cascli, result.CasOutputRoot)
				if err != nil {
					return tagTransientGoogleAPIError(err)
				}
			}
