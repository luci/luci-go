...
func TestPrepareTaskRequestEnvironment(t *testing.T) {
	...
	Convey(`Make sure we can create the correct Cmd from a fetched task's properties.`, t, func() {
		...
		c.createInvocation = func(_ context.Context, _ *http.Client, realm string, _ string) (*resultpb.Invocation, metadata.MD, error) {
			actualRealm = realm
			med := metadata.Pairs("update-token", "token")
			return &resultpb.Invocation{Name: "TestName"}, med, nil
		}
...
func TestPrepareTaskRequestEnvironment_NoInvocationUpdateToken(t *testing.T) {
	...
	Convey(`Make sure we check for an update-token`, t, func() {
		c := reproduceRun{}
		c.init(&testAuthFlags{})
		c.createInvocation = func(_ context.Context, _ *http.Client, _ string, _ string) (*resultpb.Invocation, metadata.MD, error) {
			return &resultpb.Invocation{}, metadata.MD{}, nil
		}
		service := &testService{
			getTaskRequest: func(_ context.Context, _ string) (*swarming.SwarmingRpcsTaskRequest, error) {
				return &swarming.SwarmingRpcsTaskRequest{
