...
func (c *triggerRun) main(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	result, err := createNewTask(client, c.commonFlags.serverURL, request)
	if err != nil {
		return err
	}
...
func createNewTask(client *http.Client, serverURL string, request *swarming.SwarmingRpcsNewTaskRequest) (*swarming.SwarmingRpcsTaskRequestMetadata, error) {
	s, err := swarming.New(client)
	if err != nil {
		return nil, err
	}
	s.BasePath = serverURL + swarmingAPISuffix
	return s.Tasks.New(request).Fields("task_result").Do()
}
