...
func (c *spawnTasksRun) main(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	if merr == nil && c.cancelExtraTasks {
		err = cancelExtraTasks(ctx, service, invocationTag, results)
		if err != nil {
...
func addInvocationUUIDTags(requests []*swarming.SwarmingRpcsNewTaskRequest) ([]*swarming.SwarmingRpcsNewTaskRequest, string, error) {
	// The same InvocationUUID tag is used for all requests.
...
func addRPCUUIDTags(requests []*swarming.SwarmingRpcsNewTaskRequest) ([]*swarming.SwarmingRpcsNewTaskRequest, []string, error) {
	var rpcTags []string
	for _, request := range requests {
		// RPCUUID is a unique tag for each request, to find if a task is ever run twice due to a transient RPC error.
		randomUUID, err := uuid.NewRandom()
		if err != nil {
