...
func (c *spawnTasksRun) main(a subcommands.Application, args []string, env subcommands.Env) error {
	...
	requests, err := processTasksFile(c.jsonInput)
	if err != nil {
		return err
	}
	...
	results, merr := createNewTasks(client, c.commonFlags.serverURL, requests)
...
func createNewTasks(client *http.Client, serverURL string, requests []*swarming.SwarmingRpcsNewTaskRequest) ([]*swarming.SwarmingRpcsTaskRequestMetadata, error) {
	var mu sync.Mutex
	results := make([]*swarming.SwarmingRpcsTaskRequestMetadata, 0, len(requests))
	return results, parallel.WorkPool(8, func(gen chan<- func() error) {
		for _, request := range requests {
			 gen <- func() error {
				result, err := createNewTask(client, serverURL, request)
				if err != nil {
					return err
				}
				mu.Lock()
