...
func (t *tasksRun) Init(defaultAuthOpts auth.Options) {
	...
	t.Flags.Int64Var(&t.lim, "limit", 200, "Maximum number of tasks to retrieve.")
	t.Flags.BoolVar(&t.success, "success", false, "Restrict results to successfully completed tasks.")
...
func (t *tasksRun) Parse() error {
	...
	if t.lim < 1 {
		return errors.Reason("-limit must be positive").Err()
	}
	if t.outfile != "" {
...
func (t *tasksRun) main(a subcommands.Application) error {
	...
	call := s.Tasks.List().Limit(t.lim).Tags(t.tags...)
	if t.success {
		call = call.State("COMPLETED_SUCCESS")
	}
	...
	for {
		...
		if result.Cursor == "" || int64(len(tasks)) >= t.lim || len(result.Items) == 0 {
			break
		}
