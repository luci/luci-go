...
func (c *collectRun) Init(defaultAuthOpts auth.Options) {
	...
	c.Flags.StringVar(&c.taskOutputDir, "task-output-dir", "", "Where to download isolated output to.")
...
func (c *collectRun) fetchTaskResults(ctx context.Context, taskID string, service swarmingService) taskResult {
	...
	if len(c.taskOutputDir) > 0 && result.OutputsRef != nil {
		dir := filepath.Clean(c.taskOutputDir)
		if err := service.GetTaskIsolated(ctx, result.OutputsRef, dir); err != nil {
			return taskResult{taskID: taskID, err: err}
		}
	}
