...
func (c *collectRun) Parse(args []string) error {
	...
	if c.timeout < 0 {
		return fmt.Errorf("invalid timeout: %s", c.timeout.String())
	}

	// Validate arguments.
	if c.jsonInput != "" && len(args) != 0 {
		return errors.New("cannot specify both --json and task ids")
	} else if len(args) != 0 {
		...
		return errors.New("must specify either --json or at least one task id")
	}
	return err
}
...
func (c *collectRun) summarizeResults(results []TaskResult) ([]byte, error) {
	...
	return json.Marshal(map[string]interface{}{"shards": jsonShardResults})
}
