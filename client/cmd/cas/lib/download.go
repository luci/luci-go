...
// Does the download by uploading to isolate-server.
func (r *downloadRun) doDownload(ctx context.Context, args []string) error {
	...
	for path, output := range outputs {
		...
		log.Printf("start %s %v", path, *output)
		...
		eg.Go(func() (err error) {
			...
			if err != nil {
				return errors.Annotate(err, "failed to open file", err).Err()
			}
			defer func() {
			...
			log.Printf("finish %s %v", path, *output)
			return nil
		})
	}
