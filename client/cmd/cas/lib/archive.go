...
func CmdArchive() *subcommands.Command {
	return &subcommands.Command{
		...
		LongDesc: `Given a list of files and directories, creates a CAS digest file and uploads the
tree to to a CAS server.

When specifying directories and files, you must also specify a current working
directory for that file or directory. The current working directory will not be
...
func (c *archiveRun) doArchive(ctx context.Context, args []string) error {
	...
	if dj := c.casFlags.DigestJSON; dj != "" {
		rootDg.ToProto()
		s, err := (&jsonpb.Marshaler{}).MarshalToString(rootDg.ToProto())
		if err != nil {
