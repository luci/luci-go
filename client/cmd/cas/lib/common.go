...
type AuthFlags interface {
	...
	// NewClient creates an authorised RBE Client.
	NewClient(ctx context.Context, instance string, readOnly bool) (*client.Client, error)

	// NewCASClient creates an authorised RBE-CAS Client.
	NewCASClient(ctx context.Context, instance string, readOnly bool) (*cas.Client, error)
}
