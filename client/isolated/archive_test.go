...
func TestArchiveFiles(t *testing.T) {
	...
	Convey("ArchiveFiles", t, testfs.MustWithTempDir(t, "", func(dir string) {
		...
		So(os.Chmod(filepath.Join(dir, "a"), 0666), ShouldBeNil)
		So(os.Chmod(filepath.Join(dir, "b", "c"), 0666), ShouldBeNil)
		So(os.Chmod(filepath.Join(dir, "b", "d"), 0666), ShouldBeNil)
		...
		for nc, src := range contents {
			...
			So(src, ShouldResemble, map[isolated.HexDigest][]byte{
				// referenced items.
				"e744cd78300392c35b84c59a195b3f5207181239": []byte(`{"algo":"sha-1","includes":["de702d0c3eb0f40a4c8ad6852a05fbdf26bb2d00"],"version":"1.4"}
`),
				"de82dc5dfc9e95fcb1401fab3d2f72ed319f8937": []byte(`{"algo":"sha-1","files":{"a":{"h":"86f7e437faa5a7fce15d1ddcb9eaeaea377667b8","m":438,"s":1}},"version":"1.4"}
`),

				// directory structure.
				"de702d0c3eb0f40a4c8ad6852a05fbdf26bb2d00": []byte(`{"algo":"sha-1","files":{"b/c":{"h":"5b2505039ac5af9e197f5dad04113906a9cf9a2a","m":438,"s":2},"b/d":{"h":"e3f284cae2097a55ddbaf92f45b91ea8506b49bf","m":438,"s":2}},"version":"1.4"}
`),
		...
		So(items[0].Digest(), ShouldEqual, "de82dc5dfc9e95fcb1401fab3d2f72ed319f8937")
		So(items[1].DisplayName, ShouldEqual, "")
		So(items[1].Digest(), ShouldEqual, "e744cd78300392c35b84c59a195b3f5207181239")
	}))
}
