...
func TestArchiveDirs(t *testing.T) {
	...
	Convey(`Tests directory with symlinks.`, t, func() {
		...
		h := isolated.GetHash(namespace)
		baseIsolated := filesArchiveExpect(h, tmpDir, filepath.Join("base", "foo"), filepath.Join("base", "foolink"), filepath.Join("base", "sub", "bar"), filepath.Join("base", "sub", "barlink")).Isolated
		topIsolated := isolated.New(h)
		for k, v := range baseIsolated.Files {
			topIsolated.Files[k] = v
		}
		...
		if !isWindows() {
			// In-tree symlink that points to an absolute path, which is converted to relative path.
			topIsolated.Files[filepath.Join("base", "sub", "foolink")] = isolated.SymLink(filepath.Join("..", "foo"))
			// Out-of-tree symlink, which gets converted to a ordinary files.
			...
			topIsolated.Files[filepath.Join("base", "sub", "foolink")] = isolated.BasicFile(isolated.HashBytes(h, winLinkData), mode, int64(len(winLinkData)))
		...
		if !isWindows() {
			// On Windows, no file/link refers to |rootFooData|. Instead, they are all |winLinkData|.
			expected[namespace][isolated.HashBytes(h, rootFooData)] = string(rootFooData)
		}
		if isWindows() {
			expected[namespace][isolated.HashBytes(h, winLinkData)] = string(winLinkData)
		}
