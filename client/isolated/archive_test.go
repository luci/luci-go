...
func TestArchiveFail(t *testing.T) {
	...
	Convey(`Fails to archive a file`, t, func() {
		...
		Convey(`File missing`, func() {
			...
			// Checking after clsoing to ensure there's no race condition.
			So(item1.Error(), ShouldResemble, fmt.Errorf("source(foo) failed: open %s%cnonexistent: no such file or directory", tmpDir, filepath.Separator))
