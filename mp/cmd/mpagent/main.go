...
func (agent *Agent) poll(ctx context.Context) error {
	...
	mp, err := getClient(ctx, client, server)
	if err != nil {
		return err
	}
...
func (agent *Agent) handle(ctx context.Context, instruction *machine.ComponentsMachineProviderRpcMessagesPollResponse, mp *MachineProvider, hostname string, serviceAccount string) error {
