    ...
    protected render() {
        ...
        while (entries.length === 1) {
            ...
            if (subbranch === '') {
                ...
                >
                    <span class="one-line-content branch-name" style="grid-column: 1/3;">&nbsp;&nbsp;${this.branchName + this.testResults[0].testId.slice(this.pathName.length)}</span>
                </div>
                `
            }

            if (this.branchName.length + branchExt.length + subbranch.length > 40 - this.depth * 2) {
                break;
            }
        ...
        if (this.depth === -1) {
            return html`
            <div id="bar">${this.branchName + branchExt}</div>
            <div id="body">
                <div id="content">
