...
func TestCancelTasks(t *testing.T) {
	...
	ftt.Run("With mocks", t, func(t *ftt.Test) {
		...
		tasksManager := &tasks.MockedManager{
			EnqueueBatchCancelMock: func(_ context.Context, tasks []string, killing bool, purpose string, retried int32) error {
				pendingCancel = append(pendingCancel, slices.Sorted(slices.Values(tasks)))
				return nil
			},
		}
		...
		t.Run("Limit is checked", func(t *ftt.Test) {
		...
		t.Run("Cancelling", func(t *ftt.Test) {
			endRange := endTS
			...
			cancel := func(killRunning bool, tags []string, expected []string) {
				...
				assert.Loosely(t, pendingCancel, should.Match([][]string{expected}))
				pendingCancel = nil
				...
				assert.Loosely(t, pendingCancel, should.HaveLength(len(expected)/2))
				pendingCancel = nil
			}
