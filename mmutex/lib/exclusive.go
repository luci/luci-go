...
func RemoveDrainFile(ctx context.Context, drainFilePath string) {
	if err := os.Remove(drainFilePath); err != nil {
		// Fail to remove drain file. It is possibly removed after lock is acquired.
		if e, ok := err.(*os.PathError); ok {
			logging.Infof(ctx, "[mmutex][exclusive] Secure check found drain file already removed. File path: %s", drainFilePath)
		} else {
			logging.Errorf(ctx, "[mmutex][exclusive] Failed to remove drain file when lock is not acquired. File path: %s. Error: %s", drainFilePath, e)
		}
	}
}
