...
func RemoveDrainFile(ctx context.Context, drainFilePath string) {
	if err := os.Remove(drainFilePath); err != nil {
		...
		if e, ok := err.(*os.PathError); ok {
			logging.Infof(ctx, "[mmutex][exclusive] Secure check found drain file already removed: %s", drainFilePath)
		} else {
			logging.Errorf(ctx, "[mmutex][exclusive] Failed to remove drain file when lock is not acquired: %s", drainFilePath)
		}
	}
}
