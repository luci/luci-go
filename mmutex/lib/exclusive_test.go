...
func TestExclusive(t *testing.T) {
	...
	Convey("RunExclusive", t, func() {
		...
		Convey("removes drain file immediately after acquiring the lock", func() {
			blockRunExclusive := make(chan bool)
			defer func() {
				blockRunExclusive <- true
			}()

			var runExclusiveErr error
			go func() {
			...
			// Sleep for a millisecond to allow time for RunExclusive to acquire the lock and
			// block on our channel.
			clock.Sleep(ctx, time.Millisecond)
