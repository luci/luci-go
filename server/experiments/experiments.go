...
//
// The only way to get an ID is to call Register.
type ID struct {
	name string
}
...
// CLI flag.
//
// In tests an experiment can be enabled via Enabled(ctx, id).
func (id ID) Enabled(ctx context.Context) bool {
	cur, _ := ctx.Value(&ctxKey).(stringset.Set)
	return cur.Has(id.name)
}

// Register is usually called during init() time to declare some available
// experiment.
