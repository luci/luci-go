...
type ModuleOptions struct {
	// RootSecretPath points to the root secret key used to derive all other
	// secrets.
	//
	...
	// the following structure (see Secret):
	//
	//   {
	...
	RootSecretPath string

	// Source produces the root secret to use by the module.
	//
	// If given, overrides RootSecretPath. This is useful when the secrets module
	// is initialized programatically rather than through flags.
	//
...
func (m *serverModule) Initialize(ctx context.Context, host module.Host, opts module.HostOptions) (context.Context, error) {
	// Prepare a Source based on RootSecretPath.
	source := m.opts.Source
	if source == nil {
		if m.opts.RootSecretPath == "" {
			return ctx, nil // secrets are not configured, this is fine
		}
		var err error
		source, err = initSource(ctx, m.opts.RootSecretPath, opts.Prod)
		if err != nil {
...
// initSource initializes a correct Source based on rootSecretPath.
func initSource(ctx context.Context, rootSecretPath string, prod bool) (Source, error) {
	switch {
	case rootSecretPath == ":dev":
		if prod {
	...
	case strings.HasPrefix(rootSecretPath, "sm://"):
		...
		return NewSecretManagerSource(ctx, rootSecretPath, ts)

	default:
		return &FileSource{Path: rootSecretPath}, nil
	}
}
