...
type updateHandler struct {
	name string // e.g. "sm://<project>/<secret>"
	cb   func(context.Context, Secret)
}
...
func (sm *SecretManagerStore) LoadRootSecret(ctx context.Context, rootSecret string) error {
	...
	sm.RegisterUpdateHandler(secret.name, func(_ context.Context, secret Secret) {
...
// RegisterUpdateHandler registers a callback which is called when the stored
// secret is updated.
//
// The handler is called from an internal goroutine and receives a context
// passed to MaintenanceLoop.
func (sm *SecretManagerStore) RegisterUpdateHandler(name string, cb func(context.Context, Secret)) error {
	if !strings.HasPrefix(name, "sm://") {
		return nil // no updates for static secrets
	}

	project, secret, err := sm.parseSMSecret(name)
	if err != nil {
		return nil
	}
	...
	sm.handlers = append(sm.handlers, updateHandler{name: name, cb: cb})
	return nil
}
...
func (sm *SecretManagerStore) readSecret(ctx context.Context, name string, withPrevVersion bool) (*trackedSecret, error) {
	...
	case strings.HasPrefix(name, "sm://"):
		...
		return sm.readSecretFromGSM(ctx, project, secret, withPrevVersion)
...
func (sm *SecretManagerStore) parseSMSecret(name string) (project, secret string, err error) {
	...
	case len(parts) == 1:
		...
		return sm.CloudProject, parts[0], nil
	case len(parts) == 2:
...
func (sm *SecretManagerStore) notifyUpdateHandlers(ctx context.Context, name string, val Secret) {
	var cbs []func(context.Context, Secret)
