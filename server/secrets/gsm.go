...
func (sm *SecretManagerStore) MaintenanceLoop(ctx context.Context) {
	...
	if rootSecret.nextReload.IsZero() {
...
func (sm *SecretManagerStore) readSecret(ctx context.Context, name string, withPrevVersion bool) (trackedSecret, error) {
	...
	case strings.HasPrefix(name, "devsecret://"):
		...
		if err != nil {
			return trackedSecret{}, errors.Annotate(err, "bad devsecret://, not base64 encoding").Err()
		}
...
func (sm *SecretManagerStore) readSecretFromGSM(ctx context.Context, name string, withPrevVersion bool) (trackedSecret, error) {
	...
	if err != nil {
		return trackedSecret{}, errors.Annotate(err, "failed to get the latest version of the secret %s", name).Err()
	}
	...
	if idx == -1 {
		return trackedSecret{}, errors.Reason("unexpected version name format %q", latest.Name).Err()
	}
	...
	default:
		return trackedSecret{}, errors.Annotate(err, "failed to read the previous version %d of the secret %s", prevVersion, name).Err()
	}
}
