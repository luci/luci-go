...
func TestSecretManagerSource(t *testing.T) {
	...
	Convey("With SecretManagerStore", t, func() {
		...
		Convey("readSecret unknown", func() {
			_, err := sm.readSecret(ctx, "something", false)
		...
		Convey("readSecret sm://<secret>", func() {
			gsm.createVersion("fake-project", "secret", "zzz")

			s, err := sm.readSecret(ctx, "sm://secret", false)
			So(err, ShouldBeNil)
			So(s.name, ShouldEqual, "sm://fake-project/secret")
		...
		Convey("readSecret sm://<project>/<secret>", func() {
			gsm.createVersion("another-project", "secret", "zzz")

			s, err := sm.readSecret(ctx, "sm://another-project/secret", false)
			So(err, ShouldBeNil)
			So(s.name, ShouldEqual, "sm://another-project/secret")
			So(s.value, ShouldResemble, Secret{Current: []byte("zzz")})
			So(s.versions, ShouldEqual, [2]int64{1, 0})
			So(s.nextReload.IsZero(), ShouldBeFalse)
		})
		...
		Convey("readSecret with prev version", func() {
			gsm.createVersion("fake-project", "secret", "old")
			gsm.createVersion("fake-project", "secret", "new")

			s, err := sm.readSecret(ctx, "sm://secret", true)
			So(err, ShouldBeNil)
		...
		Convey("readSecret with prev version disabled", func() {
			ref := gsm.createVersion("fake-project", "secret", "old")
			gsm.createVersion("fake-project", "secret", "new")
		...
		Convey("Derived secrets work", func() {
			...
			sm.RegisterUpdateHandler("sm://project/secret", func(context.Context, Secret) {
				close(rotated)
			})
