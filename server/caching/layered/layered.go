...
// ErrCanSatisfyMinTTL is returned by GetOrCreate if the factory function
// produces an item that expires sooner than the requested MinTTL.
var ErrCanSatisfyMinTTL = errors.New("new cache item has insufficient TTL")
...
// be forcefully refreshed. If the refreshed item also expires sooner than
// the requested min TTL, GetOrCreate will return ErrCanSatisfyMinTTL.
...
// goroutines.
//
// On each cache access if the remaining TTL the cached item is less than
// 'threshold', it may randomly be considered already expired (with probability
// increasing when item nears its true expiration).
//
// This is useful to avoid a situation when many concurrent consumers all
// discover at the same time that the item has expired, and then all proceed
// waiting for a refresh. With randomized early expiration only the most unlucky
...
func (c *Cache) GetOrCreate(ctx context.Context, key string, fn lru.Maker, opts ...Option) (interface{}, error) {
	...
	// Check the item is in the local cache, its TTL is acceptable and we don't
	// want to randomly prematurely expire it (see WithRandomizedExpiration).
	...
	item, err := lru.Create(ctx, key, func() (interface{}, time.Duration, error) {
		// Now that we have the lock, recheck that the item still needs a refresh.
		if v, ok := lru.Get(ctx, key); ok {
			if item := v.(*itemWithExp); item.isAcceptableTTL(now, o.minTTL) {
		...
		if !item.isAcceptableTTL(now, o.minTTL) {
			return nil, 0, ErrCanSatisfyMinTTL
		}
	...
	return item.(*itemWithExp).val, nil
}
...
// return nil. Logs errors inside.
//
// Does not double check item expiration time.
...
//
// It is a user-generated value plus its expiration time (or null time if it
// doesn't expire).
...
// isAcceptableTTL returns true if item's TTL is within requested range.
func (i *itemWithExp) isAcceptableTTL(now time.Time, minTTL time.Duration) bool {
	if i.exp.IsZero() {
		return true // the item never expires, satisfies any requested TTL
	}
	// Note: '>=' must not be used here, since minTTL may be 0, and we don't want
	// to return true on zero expiration, since it means "never expires".
	return i.exp.Sub(now) > minTTL
}

// randomlyExpired return true if the item must be considered already expired.
//
// See WithRandomizedExpiration for rationale. The context is used only to grab
// RNG.
func (i *itemWithExp) randomlyExpired(ctx context.Context, now time.Time, threshold time.Duration) bool {
	...
	ttl := i.exp.Sub(now)
	if ttl > threshold {
		return false // far from expiration, no need to enable randomization yet
	}

	// TODO(vadimsh): The choice of distribution here was made arbitrary. Some
	// literature suggests to use exponential distribution instead. In practice
	// what we do here seems good enough. On each check we randomly expire the
	// item with probability p = (threshold - ttl) / threshold. So closer the
	// item to its true expiration (ttl is smaller), higher the probability.
	rnd := time.Duration(mathrand.Int63n(ctx, int64(threshold)))
	return rnd > ttl
}

// expiration returns expiration time used when storing this item in the cache.
//
// Zero value means "does not expire". Panics if calculated expiration is
// negative. Use isAcceptableTTL to detect this case beforehand.
