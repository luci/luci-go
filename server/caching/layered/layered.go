...
//
// This is useful to avoid a situation when many concurrent consumers at the
// same time discover that the item has expired, and then all proceed waiting
...
//
// Zero value means "does not expire" (as understood by both LRU and Global
// caches). Panics if the calculated expiration is negative. Use isAcceptableTTL
// to detect this case beforehand.
