...
func RegisterLRUCache(capacity int) LRUHandle {
	if atomic.LoadUint32(&registrationForbidden) == 1 {
		...
		panic("can't call RegisterLRUCache after NewProcessCacheData is called")
	}
...
//
// The value itself lives inside the context. See WithProcessCacheData.
type SlotHandle int

// Fetch returns the cached data, if it is available and fresh, or attempts to
// refresh it by calling given callback.
//
// If the context doesn't have ProcessCacheData installed, this will panic.
func (h SlotHandle) Fetch(c context.Context) (interface{}, error) {
	return nil, nil
	//return c.Value(&processCacheKey).(*ProcessCacheData).caches[int(h)]
}
...
func RegisterCacheSlot() SlotHandle {
	return SlotHandle(atomic.AddUint32(&nextAvailableSlot, 1) - 1)
}
...
func NewProcessCacheData() *ProcessCacheData {
	...
	// registering new caches. All RegisterLRUCache calls should happen during
	// module init time.
