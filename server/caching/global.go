...
// all).
//
// This interface provides a smallest sufficient API to support needs of
...
// GlobalCacheProvider returns a BlobCache instance targeting a namespace.
type GlobalCacheProvider func(namespace string) BlobCache

// WithGlobalCache installs a global cache implementation into the supplied
// context.
func WithGlobalCache(c context.Context, provider GlobalCacheProvider) context.Context {
...
// gracefully (e.g by falling back to using only process cache).
func GlobalCache(c context.Context, namespace string) BlobCache {
	if p, _ := c.Value(&globalCacheKey).(GlobalCacheProvider); p != nil {
		return p(namespace)
	}
	return nil
}
