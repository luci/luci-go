...
const (
	...
	// token that has `aud` set to the supplied value. WithScopes can't be used in
	// this case and supplying it will cause an error.
	//
	...
	// option and follows the same rules as outlines in AsSelf comment.
	//
	...
	// WithIDTokenAudience option and follows the same rules as outlines in AsSelf
	// comment.
	//
...
// since they are only ones that exclusively use only Authorization header.
//
...
func asSelfIDTokenHeaders(ctx context.Context, uri string, opts *rpcOptions) (*oauth2.Token, map[string]string, error) {
	...
	//
	// The method below works almost everywhere (GAE, Flex, Cloud Run, GKE with
	// Workload Identity), but it requires the service account to have
	// iam.serviceAccountTokenCreator role on itself, which is a bit weird and
	// not default. On GAE/Flex/Cloud Run we should use GCE metadata server
	// instead.
