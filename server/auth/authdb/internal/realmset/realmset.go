...
const ExpectedAPIVersion = int64(1)
...
//
// It can be passed to QueryAuthorized. Return (0, false) if there's no such
...
// HasRealm returns true if the given realm exists.
//
// Doesn't try to validate its name.
...
func Build(r *protocol.Realms, qg *graph.QueryableGraph) (*Realms, error) {
	...
	// footprint at the end. It (just like `realms`) uses a composite key
	...
	// identity sets we just keep using stringset.Set we've build as is, assuming
	// using identities directly in Realm ACLs is rare and not worth optimizing
	...
	for key, sets := range realmsToBe {
		var sorted graph.SortedNodeSet
		if len(sets.groups) > 0 {
			sorted = dedupper.Dedup(sets.groups)
		}
		var idents stringset.Set
		if sets.idents.Len() > 0 {
			idents = sets.idents
		}
		realms[key] = groupsAndIdents{groups: sorted, idents: idents}
	}
...
// interns returns an interned copy of 's'.
...
// categorizePrincipals splits a list of principals in a Binding into a list
// of groups (identified by their indexes in a QueryableGraph) and list of
// identity names.
//
