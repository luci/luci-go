...
//
// Consider only direct mentions of the identity (i.e. ignores globs).
func (g *Graph) buildMembershipsMap() map[identity.Identity]SortedNodeSet {
	...
	for idx, node := range g.Nodes {
		// For each member of 'node', mark 'node' and all its parent groups as a
	...
	for ident, nodeSet := range sets {
		...
		if existing, ok := seen[key]; ok {
			sorted = existing // reuse identical existing slice
		} else {
			seen[key] = sorted
		}
...
// QueryableGraph is a processed Graph optimized for IsMember queries and lower
// memory consumption.
//
