...
func TestGlobSet(t *testing.T) {
	...
	Convey("Edge cases in Has", t, func() {
		b := NewBuilder()
		b.Add("user:*@example.com")
		b.Add("user:*@other.example.com")
		gs, _ := b.Build()

		So(gs.Has("a@example.com"), ShouldBeFalse)                // no "user:" prefix
		So(gs.Has("service:a@example.com"), ShouldBeFalse)        // wrong prefix
		So(gs.Has("user:a@example.com\nsneaky"), ShouldBeFalse)   // sneaky '/n'
		So(gs.Has("user:a@example.com-and-stuff"), ShouldBeFalse) // '$' is checked
	})
}
