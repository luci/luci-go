...
func TestFetcher(t *testing.T) {
	...
	Convey("With mocks", t, func(c C) {
		...
		serverSignedID := "auth-service-id" // ID used to sign blob by the server
			...
			case "/bucket/prefix/latest.db":
				if !serverHasAccessNow {
					...
					w.Write(genSignedAuthDB(ctx, signer,
						serverSignedID, serverSignature, serverLatestRev, serverLatestVal))
				}
			default:
		...
		f := Fetcher{
			...
			AuthServiceAccount: serverSignedID,
