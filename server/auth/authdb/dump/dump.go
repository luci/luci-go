...
type Fetcher struct {
	...
	storageClient *http.Client // client to access Google Storage, mocked in tests
}
...
func (f *Fetcher) FetchAuthDB(c context.Context, cur *authdb.SnapshotDB) (fresh *authdb.SnapshotDB, err error) {
	...
	err = retry.Retry(c, transient.Only(indefiniteRetry), func() (err error) {
		fresh, err = f.doFetchAttempt(c, cur, client)
		return err
...
func (f *Fetcher) doFetchAttempt(c context.Context, cur *authdb.SnapshotDB, client *http.Client) (*authdb.SnapshotDB, error) {
	// Fetch a tiny latest.json. In most cases this is the only RPC we'll do.
	latestRev, needAccess, err := f.fetchLatestRev(c, client)
	if err != nil {
		return nil, err
	}
	...
	if needAccess {
		...
		switch latestRev, needAccess, err = f.fetchLatestRev(c, client); {
		case err != nil:
			return nil, err
	...
	signed, err := f.fetchSignedAuthDB(c, client)
	if err != nil {
		return nil, err
	}
...
// transient.
func (f *Fetcher) fetchLatestRev(c context.Context, client *http.Client) (rev int64, needAccess bool, err error) {
	switch code, resp, err := f.fetchFromGCS(c, client, "latest.json"); {
	case err != nil:
	...
	case code == http.StatusForbidden || code == http.StatusNotFound:
		logging.Errorf(c, "Permission errors when fetching latest.json")
		return 0, true, nil
	default:
...
func (f *Fetcher) fetchSignedAuthDB(c context.Context, client *http.Client) (*protocol.SignedAuthDB, error) {
	code, resp, err := f.fetchFromGCS(c, client, "latest.db")
	switch {
	case err != nil:
...
func (f *Fetcher) fetchFromGCS(c context.Context, client *http.Client, rel string) (statusCode int, body []byte, err error) {
	storageURL := "https://storage.googleapis.com"
	if f.storageURL != "" {
		storageURL = f.storageURL
	}
	url := fmt.Sprintf("%s/%s/%s", storageURL, f.StorageDumpPath, rel)
	logging.Debugf(c, "GET %s", url)
