...
type SnapshotDB struct {
	...
	groups         *graph.QueryableGraph  // representation of all known groups
...
func NewSnapshotDB(authDB *protocol.AuthDB, authServiceURL string, rev int64, validate bool) (*SnapshotDB, error) {
	...
	groupsGraph, err := graph.Build(authDB.Groups)
	if err != nil {
		return nil, errors.Annotate(err, "failed to build Auth groups graph").Err()
	}
	queryableGroups, err := groupsGraph.ToQueryable()
	if err != nil {
		return nil, errors.Annotate(err, "failed to convert groups graph to queryable form").Err()
	}
