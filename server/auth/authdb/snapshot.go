...
func (db *SnapshotDB) GetCertificates(c context.Context, signerID identity.Identity) (*signing.PublicCertificates, error) {
	...
	// Fetch the certificates and the service account ID directly from the token
	// server. Note that FetchCertificatesFromLUCIService has an in-process cache
	// inside, so its OK to call it often.
	...
	// If `signerID` is not the token server, return (nil, nil) to indicate that
	// it should not be trusted (per GetCertificates contract).
