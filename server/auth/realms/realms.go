...
const (
	...
	// bindings.
	//
	// HasPermission() automatically falls back to root realms if any of the
	// realms it receives do not exist. You still can pass a root realm to
	// HasPermission() explicitly if you specifically want to check the root
	// realm permissions.
	Root = "@root"
...
// ValidateRealmName returns an error if a real name is not valid.
//
// If `scope` is GlobalScope, `realm` is expected to have form
// "<project>:<realm>". If `scope` is ProjectScope, `realm` is expected to have
// form "<realm>". Any other values of `scope` case panics.
//
