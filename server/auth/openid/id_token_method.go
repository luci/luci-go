...
//     it is later checked against a whitelist of client IDs by LUCI auth stack.
...
type GoogleIDTokenAuthMethod struct {
	// Audience lists allowed token audience values for tokens that identify
...
func (m *GoogleIDTokenAuthMethod) Authenticate(ctx context.Context, r *http.Request) (*auth.User, error) {
	...
	// Grab (usually locally cached) discovery document.
	...
	// Validate the signature, expiration. Extract preliminary user info from it.
	...
	// auth stack check it against a whitelist (same way it does for OAuth2 access
	// tokens).
...
func (m *GoogleIDTokenAuthMethod) Warmup(ctx context.Context) error {
	_, err := fetchDiscoveryDoc(ctx, googleDiscoveryURL)
	return err
}
