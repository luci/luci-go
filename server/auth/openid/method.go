...
func (m *AuthMethod) InstallHandlers(r *router.Router, base router.MiddlewareChain) {
	...
	r.GET(clientIDURL, base, m.handleClientID)
}
...
// handleClientID returns the client ID, which can be used by browser components
// to perform login with implicit flow.
func (m *AuthMethod) handleClientID(ctx *router.Context) {
