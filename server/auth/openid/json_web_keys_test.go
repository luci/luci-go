...
func TestParseJSONWebKeySet(t *testing.T) {
	...
	Convey("Happy path", t, func() {
		keys, err := ParseJSONWebKeySet([]byte(`{
			"keys": [
				{
					"kty": "RSA",
					"alg": "RS256",
	...
	Convey("Bad json", t, func() {
		_, err := ParseJSONWebKeySet([]byte("not json"))
		So(err, ShouldErrLike, "failed to parse JSON web key")
	})

	Convey("No key ID", t, func() {
		_, err := ParseJSONWebKeySet([]byte(`{
			"keys": [
				{
					"kty": "RSA",
					"alg": "RS256",
					"use": "sig",
					"n": "sJ46htmsgvo99FyibDKBh6zHGr2hldxfHt",
					"e": "AQAB"
				}
			]
		}`))
