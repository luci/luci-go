...
// of resources ("<subject>", usually plural), managed by some particular kind
// of LUCI service ("<service>").
//
...
//   - buildbucket.builds.create
//   - swarming.pools.listBots
//   - swarming.tasks.cancel
//
...
//
// Permission checks done by a service may be accompanied by a map of attributes
// that describe a context of the call (like what resource is being accessed).
// Such attributes can be used to make role bindings conditional. Attribute keys
// have form "<service>.<noun>[.<field>]+" (for example, "scheduler.job.name").
// Each permission predeclares what attributes are supplied when it is being
// checked. The service must consistently supply values of all such declared
// attributes every time it checks the permission.
//
// Each individual LUCI service should document what permissions it checks, when
// and with what attributes. It becomes a part of service's public API. Usually
// services should check only permissions of resources they own (e.g.
// "<service>.<subject>.*"), but in exceptional cases they may also check
// permissions intended for other services. This is primarily useful for
// services that somehow "proxy" access to resources.
//
...
type Permission struct {
	...
	Attributes    []string               `protobuf:"bytes,3,rep,name=attributes,proto3" json:"attributes,omitempty"` // the list of recognized attributes
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Permission) GetAttributes() []string {
	if x != nil {
		return x.Attributes
	}
	return nil
...
// the permission being checked and it is part of the permission configuration.
// The service should also document it in its API documentation.
...
// Realm is a named map {<permission> -> [(<principalsSet>, <conditions>)]}.
//
// A principal is said to have a permission in a realm with a specific
// context (a set of attributes) if there's a (<principalsSet>, <conditions>)
// pair in this permission's list, such that the principal is in <principalsSet>
// and the context matches <conditions>.
//
...
//
// The actual permissions mapping is defined via a list of bindings, where each
// binding basically says "all these principals have all these permissions if
...
// Binding assigns all specified permissions to all specified principals if
// the specified conditions are satisfied based on value of attributes passed
...
type RealmData struct {
	...
	ProjectScopedAccount string `protobuf:"bytes,2,opt,name=project_scoped_account,json=projectScopedAccount,proto3" json:"project_scoped_account,omitempty"`
	// A Cloud Project ID number to bill calls to, if any.
	//
	// Set only on the "@root" realm.
	BillingCloudProjectId uint64 `protobuf:"varint,3,opt,name=billing_cloud_project_id,json=billingCloudProjectId,proto3" json:"billing_cloud_project_id,omitempty"`
	unknownFields         protoimpl.UnknownFields
	sizeCache             protoimpl.SizeCache
}
...
func (x *RealmData) GetProjectScopedAccount() string {
	if x != nil {
...
func (x *RealmData) GetBillingCloudProjectId() uint64 {
	if x != nil {
...
const file_go_chromium_org_luci_server_auth_service_protocol_components_auth_proto_realms_proto_rawDesc = "" +
	"\n" +
	"Tgo.chromium.org/luci/server/auth/service/protocol/components/auth/proto/realms.proto\x12\x16components.auth.realms\"\xe9\x01\n" +
	"\x06Realms\x12\x1f\n" +
	"\vapi_version\x18\x01 \x01(\x03R\n" +
	"apiVersion\x12D\n" +
	"\vpermissions\x18\x02 \x03(\v2\".components.auth.realms.PermissionR\vpermissions\x12A\n" +
	"\n" +
	"conditions\x18\x04 \x03(\v2!.components.auth.realms.ConditionR\n" +
	"conditions\x125\n" +
	"\x06realms\x18\x03 \x03(\v2\x1d.components.auth.realms.RealmR\x06realms\"\\\n" +
	"\n" +
	"Permission\x12\x12\n" +
	...
	"attributes\"\xb5\x01\n" +
	"\tCondition\x12T\n" +
	"\brestrict\x18\x01 \x01(\v26.components.auth.realms.Condition.AttributeRestrictionH\x00R\brestrict\x1aL\n" +
	"\x14AttributeRestriction\x12\x1c\n" +
	"\tattribute\x18\x01 \x01(\tR\tattribute\x12\x16\n" +
	"\x06values\x18\x02 \x03(\tR\x06valuesB\x04\n" +
	"\x02op\"\x8f\x01\n" +
	"\x05Realm\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12;\n" +
	"\bbindings\x18\x02 \x03(\v2\x1f.components.auth.realms.BindingR\bbindings\x125\n" +
	"\x04data\x18\x03 \x01(\v2!.components.auth.realms.RealmDataR\x04data\"k\n" +
	"\aBinding\x12 \n" +
	"\vpermissions\x18\x01 \x03(\rR\vpermissions\x12\x1e\n" +
	"\n" +
	"principals\x18\x02 \x03(\tR\n" +
	"principals\x12\x1e\n" +
	"\n" +
	"conditions\x18\x03 \x03(\rR\n" +
	"conditions\"\xa8\x01\n" +
	"\tRealmData\x12,\n" +
	"\x12enforce_in_service\x18\x01 \x03(\tR\x10enforceInService\x124\n" +
	"\x16project_scoped_account\x18\x02 \x01(\tR\x14projectScopedAccount\x127\n" +
	"\x18billing_cloud_project_id\x18\x03 \x01(\x04R\x15billingCloudProjectIdB<Z:go.chromium.org/luci/server/auth/service/protocol;protocolb\x06proto3"
