...
// GetOrCreateScopedServiceAccount allows on-demand creation of project scoped service accounts.
//
...
func GetOrCreateScopedServiceAccount(ctx context.Context, project, accountId string) (created bool, err error) {
	created = false
	err = nil
...
func MintAccessTokenForServiceAccount(ctx context.Context, params MintAccessTokenParams) (*oauth2.Token, error) {
	...
	if params.Project != "" {
		if _, err := GetOrCreateScopedServiceAccount(ctx, params.Project, params.ServiceAccount); err != nil {
			return nil, err
		}
	}
