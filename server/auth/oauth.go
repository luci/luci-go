...
//
// Note that it is currently uses /tokeninfo endpoint which "has no SLA and
// is not intended for production use". The closes alternative is /userinfo
// endpoint, but it doesn't return the token expiration time (so we can't cache
// the result of the check) nor the list of OAuth scopes the token has.
//
...
// authentication instead (they are locally verifiable JWTs).
//
...
// support two tokens kinds and know which one to use when.
//
// There's no solution currently that preserves correctness, performance,
...
//     pick some arbitrary expiration time which will be generally incorrect).
//   * Using OpenID tokens scarifies usability.
