...
func (a *IAPAuthMethod) Authenticate(ctx context.Context, r *http.Request) (*auth.User, error) {
	logging.Infof(ctx, "iap auth settings: %+v", *a)
	iapJwt, ok := r.Header[IAPJWTAssertionHeader]
	if !ok {
		logging.Debugf(ctx, "No IAP JWT assertion present in request.")
		return nil, ErrMissingJWTAssertionHeader
	}

	aud := fmt.Sprintf("/projects/%s/apps/%s", a.NumericProjectID, a.AppID)
	validateFunc := a.Validator
	if validateFunc == nil {
		validateFunc = idtoken.Validate
	}

	jwtPayload, err := validateFunc(ctx, iapJwt[0], aud)
	if err != nil {
		logging.Errorf(ctx, "couldn't validate jwt payload for aud: %q", aud)
		return nil, err
	}

	email, ok := jwtPayload.Claims["email"]
	if !ok {
		return nil, fmt.Errorf("No email claim")
	}
	...
	logging.Debugf(ctx, "auth.User: %+v", ret)
	return ret, nil
}
