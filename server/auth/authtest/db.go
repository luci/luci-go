...
//
// Panics if `realm` is not a valid globally scoped realm, i.e. doesn't look
// like "<project>:<realm>".
...
// MockError modifies db to make its methods to return this error.
//
// 'err' may be nil, in which case the previously set mock error is removed.
...
// NewFakeDB creates an empty FakeDB.
//
// Populate it using Mock* calls.
...
// AddMocks applies a bunch of mocks to the state in db.
...
// need this method. Modify FakeDB in the FakeState instead.
...
// mockedForID returns mockedForID[id], initializing it if necessary.
//
