...
func (a *Authenticator) Authenticate(ctx context.Context, r *http.Request) (_ context.Context, err error) {
	...
	stage = "IP_WHITELIST"
	if s.peerIP, err = checkPeerIP(ctx, cfg, s.db, r, s.peerIdent); err != nil {
		return nil, err
	}
	...
	if token := r.Header.Get(delegation.HTTPHeaderName); token != "" {
		...
		if s.user, err = checkDelegationToken(ctx, cfg, s.db, token, s.peerIdent); err != nil {
			return nil, err
		}
...
func checkClientIDWhitelist(ctx context.Context, cfg *Config, db authdb.DB, email, clientID string) error {
	...
	logging.Warningf(ctx, "auth: %q is using client_id %q not in the whitelist", email, clientID)
