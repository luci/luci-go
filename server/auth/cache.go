...
// tokenCacheConfig contains configuration of a token cache for a some single
// token kind.
...
type cachedToken struct {
	...
	// OAuth2Token is used caching an OAuth2 token, otherwise nil.
	OAuth2Token *cachedOAuth2Token `json:"oauth2_token,omitempty"`
	// DelegationToken is used when caching a delegation token, otherwise nil.
...
func (tc *tokenCache) fetchOrMintToken(ctx context.Context, op *fetchOrMintTokenOp) (tok *cachedToken, err error, label string) {
	...
	fetched, err := tc.lc.GetOrCreate(ctx, cacheKey, func() (val interface{}, ttl time.Duration, err error) {
		...
		// Note: we set 'mintedToken' and 'label' from the outer scope here.
		var tok *cachedToken
	...
	if err != nil {
		...
		case err == layered.ErrCantSatisfyMinTTL:
			// This happens if op.Mint failed to produce token that lives longer than
			// MinTTL.
			label = "ERROR_INSUFFICIENT_MINTED_TTL"
		case label == "":
	...
	// handle it sloppily and just return an error (still better than accidentally
	// using wrong token).
	tok = fetched.(*cachedToken)
