...
//  * 'standalones/' contain all standalone templates what will be loaded without templates from 'includes/'
// Only templates from 'pages/' and 'standlones/' are included in the output map.
func AssetsLoader(assets map[string]string) Loader {
	return func(c context.Context, funcMap template.FuncMap) (map[string]*template.Template, error) {
		...
		for name, body := range assets {
			if strings.HasPrefix(name, "standalones/") {
				t := template.New(name).Funcs(funcMap)
				if _, err := t.Parse(body); err != nil {
					return nil, err
				}
...
func FileSystemLoader(path string) Loader {
	return func(c context.Context, funcMap template.FuncMap) (map[string]*template.Template, error) {
		...
		if err = readFilesInDir(filepath.Join(abs, "standalones"), files); err != nil {
			return nil, err
		}
