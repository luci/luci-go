...
// Modules is a simple framework for extending server.Server with optional
...
type Module interface {
	...
	// requests.
	//
	// The module may use the given 'srv' interface to register itself in various
	// server guts. It must not retain 'srv': the object it points to becomes
	// invalid after Initialize finishes (using it leads to panics). This is
	// intentional.
	//
...
// their initialization to inject themselves into the server.
type Host interface {
	...
	// TODO:
	//  * Allow getting an oauth2.TokenSource to make authenticated calls.
	//  * Allow adding a middleware to the default middleware chain.
	//  * Allow adding an interceptor to pRPC server interceptor list.
}
