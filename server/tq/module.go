...
type ModuleOptions struct {
	...
	// SweepMode defines how to perform transaction tasks reminders sweeps.
	//
	...
	// indicates an intent do submit a Cloud Task. If the subsequent Cloud Tasks
	...
	// SweepInitiationEndpoint is an URL path that can be hit to initiate a sweep.
	//
	// GET requests to this endpoint (if they have proper authentication headers)
	// will launch sweep initiation inside this process.
	//
	// On GAE default is "/internal/tasks/c/sweep". On non-GAE it is "-", meaning
	// the endpoint is not exposed. Instead there should be some single process
	// somewhere that calls Sweep() to periodically initiate sweeps.
	...
	//
	// The queue should be configured to allows to least 10 QPS.
	//
	// Default is "default".
	SweepTaskQueue string

	// SweepTaskPrefix is an URL prefix to use for sweep subtasks with running
...
func (m *tqModule) initDispatching(ctx context.Context, host module.Host, opts module.HostOptions) error {
	...
	if opts.Prod {
		...
		disp.Submitter = scheduler
		if disp.CloudProject == "" {
			disp.CloudProject = "tq-project"
		}
		if disp.CloudRegion == "" {
