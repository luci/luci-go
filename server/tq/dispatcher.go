...
// TaskClass defines how to serialize and route tasks of a specific type.
type TaskClass struct {
	// ID is unique identifier of this class of tasks.
	//
	// It is used to decide how to deserialize and route the task. Change IDs of
	// existing task classes is disruptive operation, make sure the queue is
	// drained first.
	ID string

	// Prototype identifier a proto message type of a task payload.
	//
	...
	// a previous type can be decoded using the new type.
	Prototype proto.Message

	// Handler will be called by the dispatcher to execute the tasks.
	//
	// The handler will receive the task's payload of the exact same type as
	// type of Prototype.
	Handler Handler
...
type Task struct {
	...
	//
	// Its type will be used to find a matching TaskClass which defines how to
	// route and handle the task.
...
// InstallRoutes installs HTTP route for "/internal/tq/*" paths.
//
...
// add now. This works only outside of transactions. Using DeduplicationKey with
