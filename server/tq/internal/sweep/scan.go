...
func Scan(ctx context.Context, p ScanParams) ([]*reminder.Reminder, partition.SortedPartitions, error) {
	...
	if needMoreScans {
		if len(rs) == 0 {
			...
			// We fetched something but the hit a limit or timed out. Divide the range
			// after the last fetched Reminder.
...
//
// There's a high chance they will be processed by on AddTask happy path, we
// shouldn't interfere.
//
