...
func (d *Distributed) ExecSweepTask(ctx context.Context, task *sweeppb.SweepTask) error {
	...
	// Use the configuration passed with the task. Note that this call may return
	// both results and an error.
	...
	if err != nil {
		if len(reminders) == 0 && len(followUp) == 0 {
			...
			return errors.Reason("scan failed without returning any results").Err()
		}
	...
	go func() {
		defer wg.Done()
		count, err := d.processReminders(ctx, lessor, task.LeaseSectionId, db, reminders)
		logging.Infof(ctx, "Successfully processed %d reminders", count)
		lerr.Assign(1, err)
	}()
	wg.Wait()
		...
		for _, part := range parts {
			...
			work <- func() error {
				if err := d.EnqueueSweepTask(ctx, task); err != nil {
					logging.Errorf(ctx, "Failed to enqueue follow up task %q: %s", task.Partition, err)
					return err
				}
				return nil
			}
...
// there.
//
// Logs errors inside. Returns number of successfully processed reminders.
func (d *Distributed) processReminders(ctx context.Context, lessor internals.Lessor, sectionID string, db databases.Database, reminders []*reminder.Reminder) (int, error) {
	l := len(reminders)
	if l == 0 {
		return 0, nil
	}
