...
func TestEventuallyExecutes(t *testing.T) {
	// This test ensures that transactionally submitted tasks eventually execute,
	...
	Convey("Distributed sweeping", t, func() {
		...
		const sweepCronSleep = "sweep cron"
		...
		tc.SetTimerCallback(func(d time.Duration, t clock.Timer) {
			...
			if testclock.HasTags(t, sweepCronSleep) {
				tc.Add(d)
			}
		})
		...
		// Use a single RND for all flaky.Errors(...) instances. Otherwise their
		...
		disp.Sweeper = tq.NewDistributedSweeper(disp, tq.TaskClass{
			ID:    "sweep",
			Queue: "default",
		})
				...
				for i := 1; i <= totalTasks; i++ {
					...
					work <- func() error {
						return datastore.RunInTransaction(ctx, func(ctx context.Context) error {
							disp.AddTask(ctx, &tq.Task{Payload: &durationpb.Duration{Seconds: int64(i)}})
		...
		for {
			...
			logging.Infof(ctx, "%s: %d reminders left", clock.Now(ctx).Sub(epoch), reminders)
			if reminders == 0 {
				break // no pending tasks and no reminders in the datastore, we are done
			}

			// Blow up if it takes so much time to converge.
			if clock.Now(ctx).Sub(start) > 30*time.Minute {
				panic("Looks like the test is stuck")
			}

			// Launch the next sweep a bit later.
			clock.Sleep(clock.Tag(ctx, sweepCronSleep), 30*time.Second)
		}
