...
// will idempotently create a Cloud Task and delete record in the database.
//
// SweeperOptions tune some of parameters of this process. Roughly:
//   1. Sweep() call in Dispatcher creates SweepShards subtasks that each scan
//      a portion of the database for old reminders.
//   2. Each such subtask is allowed to process no more than TasksPerScan
//      reminder records. This caps its runtime and memory usage. TasksPerScan
//      should be small enough so that all sweeping subtasks finish before next
//      Sweep() call, but large enough so that it makes meaningful progress.
//   3. If the sweeping subtask detects there's more than TasksPerScan items
//      it needs to cover, it launches SecondaryScanShards follow-up sweeping
//      subtasks that cover the remaining items. This should be happening in
//      rare circumstances only if the database is slow or has a large backlog.
type SweeperOptions struct {
	...
	// TasksPerScan caps maximum number of tasks that a sweep subtask will
	// process.
	//
	// Defaults to 2048.
	TasksPerScan int
	...
	// LessorID identifiers an implementation of a system that manages leases on
	// subranges of the database.
	//
	// Default is the same ID as the database implementation ID.
	LessorID string

	// SweepTaskQueue is a Cloud Tasks queue name to use for sweep subtasks.
	//
	...
	// SweepTaskPrefix is a URL prefix to use for sweep subtasks.
	//
	...
	// TaskHost is a hostname to dispatch sweep subtasks to.
	//
...
func NewDistributedSweeper(disp *Dispatcher, opts SweeperOptions) Sweeper {
	...
	// sweep subtasks via `disp`, which will eventually result in ExecSweepTask.
...
//
// This is useful in tests to make sure changes to disp.Submitter affecting
// the sweeper too.
