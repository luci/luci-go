...
// NewDistributedSweeper creates a sweeper that distributes sweeping tasks
// through the given dispatcher using the given TaskClass.
//
...
func NewDistributedSweeper(disp *Dispatcher, tc TaskClass) Sweeper {
	...
	tc.Handler = func(ctx context.Context, msg proto.Message) error {
		return distr.ExecSweepTask(ctx, msg.(*sweeppb.SweepTask))
	}
