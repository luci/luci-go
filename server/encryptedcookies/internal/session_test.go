...
func TestSession(t *testing.T) {
	...
	Convey("Works", t, func() {
		...
		enc, err := sessionAEAD.Encrypt([]byte("blah"), nil)
		So(err, ShouldBeNil)

		req, _ := http.NewRequest("GET", "http://example.com", nil)
		req.Header.Add("Cookie", httpCookie.String())

		httpCookieFromReq := EncryptedSessionCookie(req)
		So(httpCookieFromReq, ShouldNotBeNil)

		cookieFromReq, err := DecryptSessionCookie(primaryAEAD, httpCookieFromReq)
		So(err, ShouldBeNil)

		sessionAEADFromReq, err := PerSessionAEAD(cookieFromReq)
		So(err, ShouldBeNil)

		dec, err := sessionAEADFromReq.Decrypt(enc, nil)
		So(err, ShouldBeNil)
		So(string(dec), ShouldEqual, "blah")
	})
}
