...
func (m *AuthMethod) Authenticate(ctx context.Context, r *http.Request) (*auth.User, auth.Session, error) {
	...
	// If the fake session user is not matching server's email, uses a fake
	// profile and install an erroring session that asks the caller to log in as
	// a developer. We can't generate real tokens for fake users.
...
//
// In most cases local dev server run under the developer account.
...
func (serverSelfSession) IDToken(ctx context.Context) (*oauth2.Token, error) {
	...
	// fake is ignored when running locally using a token establishes with
