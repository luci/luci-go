...
func hitEndpoint(ctx context.Context, endpoint string, params map[string]string) ([]byte, error) {
	...
	resp, err := client.Post(endpoint, "application/x-www-form-urlencoded", body)
	if resp != nil {
		defer func() {
	...
	if err != nil {
		return nil, errors.Annotate(err, "failed to read response from %q", endpoint).Tag(transient.Tag).Err()
	}
	resp.Body.Close()

	if resp.StatusCode != 200 {
		err := errors.Reason("got HTTP %d from %q with body %q", resp.StatusCode, endpoint, blob)
