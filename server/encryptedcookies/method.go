...
import (
	...
	"go.chromium.org/luci/server/encryptedcookies/sessions"
...
type handler func(ctx context.Context, r *http.Request, rw http.ResponseWriter, cfg *OpenIDConfig) error
...
func (m *AuthMethod) handler(ctx *router.Context, cb handler) {
	...
	if err == nil {
		err = cb(ctx.Context, ctx.Request, ctx.Writer, cfg)
	}
	if err != nil {
...
func (m *AuthMethod) loginHandler(ctx *router.Context) {
	m.handler(ctx, func(ctx context.Context, r *http.Request, rw http.ResponseWriter, cfg *OpenIDConfig) error {
		return fmt.Errorf("not implemented")
	})
}
