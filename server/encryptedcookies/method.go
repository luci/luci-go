...
func (m *AuthMethod) Authenticate(ctx context.Context, r *http.Request) (*auth.User, error) {
	encryptedCookie := internal.EncryptedSessionCookie(r)
	if encryptedCookie == nil {
		return nil, nil // the method is not applicable, skip it
	}
	...
	if aead == nil {
		return nil, errors.Reason("the encryption key is not configured").Err()
	}
	...
	//
	// TODO(vadimsh): Remove the cookie if the session is no longer valid. This is
	// not trivial, since we don't have http.ResponseWriter here. It is also not
	// clear how clearing the cookie would work with Javascript Fetch API calls.
	session, err := m.Sessions.FetchSession(ctx, sid)
	switch {
	case err != nil:
		logging.Warningf(ctx, "Failed to fetch session %q: %s", sid, err)
		return nil, errors.Reason("failed to fetch the session").Tag(transient.Tag).Err()
	case session == nil:
	...
	// Check if we need to refresh short-lived tokens stored in the session.
	...
	if internal.ShouldRefreshSession(ctx, ttl) {
		...
		case err != nil:
			logging.Errorf(ctx, "Failed to refresh the session: %s", err)
			return nil, errors.Reason("failed to refresh the session, see server logs").Tag(transient.Tag).Err()
		case session == nil:
			// TODO(vadimsh): Remove the cookie. See the TODO above.
			return nil, nil // the session is no longer valid, just ignore it
		}
	}
...
var (
	// errCodeReuse is returned of the authorization code is reused.
...
func (m *AuthMethod) handler(ctx *router.Context, cb handler) {
	...
	if err != nil {
		...
		logging.Errorf(ctx.Context, "HTTP %d: %s", code, err)
...
func (m *AuthMethod) callbackHandler(ctx *router.Context) {
	m.handler(ctx, func(ctx context.Context, r *http.Request, rw http.ResponseWriter, cfg *OpenIDConfig, discovery *openid.DiscoveryDoc) error {
		...
		err = m.Sessions.UpdateSession(ctx, statepb.SessionId, func(s *sessionpb.Session) error {
			...
			s.LastRefresh = s.Created
			s.NextRefresh = timestamppb.New(exp) // refresh when short-lived tokens expire
			s.Sub = tok.Sub
			s.Email = tok.Email
			s.Name = tok.Name
...
// refreshSession refreshes short-lived tokens stored in the session, thus
// checking that the refresh token is still valid.
//
// Returns:
//   (new session, nil) - if the session was successfully refreshed.
//   (nil, nil) - if the refresh token was revoked and the session is closed.
...
func (m *AuthMethod) refreshSession(ctx context.Context, cookie *encryptedcookiespb.SessionCookie, session *sessionpb.Session) (*sessionpb.Session, error) {
	...
	sessionAEAD, err := internal.PerSessionAEAD(cookie)
	if err != nil {
		return nil, errors.Annotate(err, "failed to instantiate the per-session AEAD").Err()
	}
	private, err := internal.DecryptPrivate(sessionAEAD, session.EncryptedPrivate)
	if err != nil {
		return nil, errors.Annotate(err, "failed to decrypt the private part of the session").Err()
	}
	...
	if err != nil {
		...
		logging.Errorf(ctx, "Refresh failed: %s", err)
	}
	stillGood := err == nil
	...
	if stillGood {
		// Grab updated user info from the ID token.
		if tok, _, err = openid.UserFromIDToken(ctx, tokens.IdToken, discovery); err != nil {
			return nil, errors.Annotate(err, "failed to check ID token").Err()
		}
		// Make sure we've also got the access token
		if tokens.AccessToken == "" {
			return nil, errors.Reason("the ID provider didn't produce access token").Err()
		}
	...
	err = m.Sessions.UpdateSession(ctx, cookie.SessionId, func(s *sessionpb.Session) error {
		...
		// be harmless, but logging won't hurt.
		if s.Generation != session.Generation {
			logging.Warningf(ctx,
				"The session was updated already by another handler (%d != %d). Overwriting...",
		...
		if stillGood {
			...
			s.State = sessionpb.State_STATE_BROKE
			s.NextRefresh = nil
