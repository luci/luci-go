...
func (m *AuthMethod) loginHandler(ctx *router.Context) {
	m.handler(ctx, func(ctx context.Context, r *http.Request, rw http.ResponseWriter, cfg *OpenIDConfig, discovery *openid.DiscoveryDoc) error {
		...
		// Encrypt it using AEAD, since we are going to expose it.
		aead := auth.GetAEAD(ctx)
		if aead == nil {
		...
		// Prepare parameters for the authorization endpoint.
		v := url.Values{
			"response_type":         {"code"},
		...
		// Finally, redirect to the identity provider's authorization endpoint.
		// It will eventually call us back, see callbackHandler.
...
func (m *AuthMethod) callbackHandler(ctx *router.Context) {
	m.handler(ctx, func(ctx context.Context, r *http.Request, rw http.ResponseWriter, cfg *OpenIDConfig, discovery *openid.DiscoveryDoc) error {
		...
		// (so that it can set the cookie on a non-default version domain).
		...
		// used already and we should reject this second attempt.
		switch err, session := m.Sessions.FetchSession(ctx, statepb.SessionId); {
		case err != nil:
		...
		// Encrypt sensitive tokens using the per-session keys.
		...
		// Actually create the new session in the session store.
