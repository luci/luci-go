...
func (m *AuthMethod) closeSession(ctx context.Context, aead tink.AEAD, encryptedCookie *http.Cookie, cfg *OpenIDConfig, discovery *openid.DiscoveryDoc) error {
	...
	if err != nil {
		...
		// Refresh tokens are usually observable through ID provider UI. There's a
		// list of "Such and such services have information about your profile".
		// The expectation is that if the user finishes the logout flow, the service
		// will disappear from the list of authorized services. To do that, we must
		// successfully revoke the token.
		//
		// Since the logout is used *specifically* to achieve this, it is better to
		// fail it if the revocation fails, unlike the best-effort revocation in
		// /login (where the primary purpose is to get a new refresh token, not to
