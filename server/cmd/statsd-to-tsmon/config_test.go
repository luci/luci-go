...
func TestConfig(t *testing.T) {
	...
	Convey("Errors", t, func() {
		...
		Convey("Not a static suffix", func() {
			...
			}), ShouldErrLike, `must end in a static suffix`)
		})
...
func TestPattern(t *testing.T) {
	...
	Convey("All static", t, func() {
		...
		So(p.suffix(), ShouldEqual, "def")
	})

	Convey("All var", t, func() {
		p, err := parsePattern("${abc}.${def}")
		So(err, ShouldBeNil)
		So(p, ShouldResemble, &pattern{
			str:  "${abc}.${def}",
			len:  2,
			vars: map[string]int{"abc": 0, "def": 1},
		})
		So(p.suffix(), ShouldEqual, "")
	})
