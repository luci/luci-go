...
// suffix returns the last name component if it is static or "" otherwise.
...
func loadRule(metric types.Metric, spec *config.Rule) (sfx string, rule *Rule, err error) {
	...
	for idx, f := range tsmonFields {
		...
		vr, err := parseVar(val)
		if err != nil {
			return "", nil, errors.Reason("field %q has bad value %q", f.Name, val).Err()
		}
		if vr != "" {
	...
	// We require suffixes to be static and unique to simplify FindMatchingRule.
	if sfx = pat.suffix(); sfx == "" {
		return "", nil, errors.Reason(" must end in a static suffix").Err()
	}
...
// into its components.
//
// Each var appearance must be unique.
