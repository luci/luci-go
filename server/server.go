...
//
// Interceptors are chained in order they are registered. The first registered
// interceptor becomes the outermost.
//
// An interceptor provided via server.PRPC.UnaryServerInterceptor by the
// server.Server end user (if any) is always the innermost.
//
// Should be called before ListenAndServe (panics otherwise).
