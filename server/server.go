...
func (o *Options) Register(f *flag.FlagSet) {
	...
	f.StringVar(
		...
		"ts-mon-account",
		o.TsMonAccount,
		"Collect and flush tsmon metrics using this account for auth (disable tsmon if not set)",
	)
	f.StringVar(
		&o.TsMonServiceName,
		"ts-mon-service-name",
		o.TsMonServiceName,
		"Service name of tsmon target (disable tsmon if not set)",
	)
	f.StringVar(
		&o.TsMonJobName,
		"ts-mon-job-name",
		o.TsMonJobName,
		"Job name of tsmon target (disable tsmon if not set)",
	)
}
...
func (s *Server) ListenAndServe() error {
	...
	// Start periodically flushing metrics.
	if s.tsmon != nil {
