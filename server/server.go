...
func (s *Server) ListenAndServe() error {
	...
	s.PRPC.UnaryServerInterceptor = grpcmon.NewUnaryServerInterceptor(
		grpcutil.NewUnaryServerPanicCatcher(
			limiter.NewUnaryServerInterceptor(
				s.rpcLimiter,
...
func (s *Server) initRPCLimiter() error {
	...
	// We want limiter metrics to be reported before every flush (so the flushed
	// values are as fresh as possible) and also once a second (to make the most
	// current limiter state observable through /admin/portal/tsmon/metrics debug
	// UI).
