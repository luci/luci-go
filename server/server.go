...
// server managed to star successfully but can't re-read the secret later (e.g.
...
func (s *Server) readRootSecret() (secrets.Secret, error) {
	if s.opts.RootSecretPath == "" {
		...
		return secrets.Secret{}, fmt.Errorf("-root-secret-path is required when running in prod mode")
	}
	...
	err = json.NewDecoder(f).Decode(&secret)
	return secret, errors.Annotate(err, "not a valid JSON").Err()
}
