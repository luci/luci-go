...
//
// Requests that match some registered virtual host router won't reach the
// default router (server.Routes), even if the host-specific router doesn't have
// a route for them. Such requests finish with HTTP 404.
//
// Also the router created by VirtualHost is initially completely empty: the
// server and its modules don't install anything into it (there's no mechanism
// to do this). For that reason VirtualHost should never by used to register
// a router for the "main" domain name: it will make the default server.Routes
// (and all handlers installed there by server modules) useless, probably
// breaking the server. Populate server.Routes instead.
//
// Should be called before ListenAndServe (panics otherwise).
