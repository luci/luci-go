...
func (s *Server) rootMiddleware(c *router.Context, next router.Handler) {
	...
	// (where we know the response status code).
...
func (s *Server) startRequestSpan(ctx context.Context, r *http.Request, skipSampling bool) (context.Context, *trace.Span) {
	...
	span.AddAttributes(
		// Server info, for queries.
		trace.StringAttribute("cr.dev/image", s.Options.ContainerImageID),
		trace.StringAttribute("cr.dev/service", s.Options.TsMonServiceName),
		trace.StringAttribute("cr.dev/job", s.Options.TsMonJobName),
		trace.StringAttribute("cr.dev/host", s.hostname),
