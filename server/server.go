...
//
// • go.chromium.org/luci/common/trace: Ttacing via Google Cloud Trace and
...
// when submitting task as Cloud Datastore or Cloud Spanner transactions.
//
...
// pRPC services
//
// The server.PRPC is the primary grpc.ServiceRegistrar that should be used
// to expose server's public gRPC/pRPC APIs. It is pre-configured with a set of
...
// balancer (or a series of load balancers) that terminate TLS and sets
...
// trusted, by the server assumes the load balancer at least sanitizes the
...
// flag specifies how many layers of load balancers to skip to get to the
// original IP.
package server
