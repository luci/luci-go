...
func New(ctx context.Context, opts Options, mods []module.Module) (srv *Server, err error) {
	...
	if err := srv.initEarlyAuth(); err != nil {
...
// initEarlyAuth initializes the core auth system by preparing the context
// and verifying auth tokens can actually be minted (i.e. supplied credentials
// are valid).
//
// It is called before tsmon monitoring is initialized: tsmon needs auth.
// The rest of the auth initialization (the part that needs tsmon) happens in
// initAuth after tsmon is initialized.
func (s *Server) initEarlyAuth() error {
	...
	// initialized in initAuth once we have a working auth context that can
	// call IAM.
...
// initAuth finishes auth system initialization.
//
