...
authentication allows to expose pprof endpoints directly.</p>

<p>Usage:</p>
<ul>
<li>Hit "Generate pprof token" button below.</li>
<li>Copy the token it produces.</li>
<li>Pass it as <code>?tok=...</code> URL parameters to pprof endpoints.</li>
</ul>

<p>Remember that in most cases requests are routed to some random backend
process, so hitting same profile page multiple times may result in completely
