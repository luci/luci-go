...
//
// The handler can be invoked via GET requests to "<serving-prefix>/<id>", e.g.
// usually "/internal/cron/<id>". This URL path should be used when configuring
// Cloud Scheduler (e.g. use in cron.yaml when running on Appengine).
//
...
// InstallRoutes installs HTTP routes that handle requests from Cloud Scheduler.
func (d *Dispatcher) InstallRoutes(r *router.Router, prefix string) {
	...
	r.GET(prefix, mw, func(c *router.Context) {
		id := c.Params.ByName("handler")
		switch err := d.executeHandlerByID(c.Context, id); {
		case err == nil:
			c.Writer.Write([]byte("OK"))
		case transient.Tag.In(err):
			logging.Errorf(c.Context, "Transient error in cron handler %q: %s", id, err)
			http.Error(c.Writer, fmt.Sprintf("transient error: %s", err), 500)
		default:
			logging.Errorf(c.Context, "Fatal error in cron handler %q: %s", id, err)
			http.Error(c.Writer, fmt.Sprintf("fatal error: %s", err), 202)
		}
	})
}
