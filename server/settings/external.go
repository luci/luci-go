...
//
// The bundle has expiration time of 1 sec, so that Settings{...} implementation
// doesn't go through slower cache-miss code path all the time a setting is
// read (which happens multiple times per request). In practice it means changes
// loaded via Load() apply at most 1 sec later.
func (s *ExternalStorage) FetchAllSettings(context.Context) (*Bundle, time.Duration, error) {
	...
	return &Bundle{Values: values}, time.Second, nil
}
