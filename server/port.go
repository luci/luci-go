...
// PortOptions describes options of a single serving HTTP port.
//
...
type Port struct {
	...
	// registered per-host router (see VirtualHost). Normally, there are no
	...
	m       sync.Mutex
	srv     *http.Server              // initialized in server(), nil before that
...
// nameForLog returns a string to identify this port in server logs.
func (p *Port) nameForLog() string {
	if p.opts.Name != "" {
		return fmt.Sprintf("[%s] http://%s", p.opts.Name, p.opts.ListenAddr)
	}
...
// initHandlerLocked initializes the top router for the server.
func (p *Port) initHandlerLocked() http.Handler {
	mapping := p.perHost
	fallback := p.Routes

	if len(mapping) == 0 {
		return fallback // no need for an extra layer of per-host routing
	}
