...
// AsTime returns a time.Time from this *Timestamp.
//
// If t is nil, this returns a "zero" time.
...
// NewTimestamp constructs and returns a Timestamp proto message.
//
// If t is Zero, this returns nil.
...
func AdjustDeadline(ctx context.Context, atLeastCleanup time.Duration) (newCtx context.Context, cleanup <-chan struct{}, cancel func()) {
	...
	curGrace := t.GracePeriod.AsDuration()
	newGrace := curGrace - atLeastCleanup
	if newGrace < 0 {
		newGrace = 0
	}
	t.GracePeriod = durationpb.New(newGrace)

	curDeadline, _ := ctx.Deadline()
	luciDeadline := t.Deadline.AsTime()
	...
	if !adjustedDeadline.IsZero() {
		...
		t.Deadline = NewTimestamp(adjustedDeadline)
	}
