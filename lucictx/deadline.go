...
//    }
//
//    func ScopedFunction(newCtx context.Context, cleanup <-chan struct{}) {
...
func runDeadlineMonitor(ctx context.Context, cancel func(), adjustedSoftDeadline time.Time, gracePeriod time.Duration) (<-chan DeadlineEvent, func()) {
	...
	go func() {
		...
		if evt == InterruptEvent {
			// note: clock.After also wakes up on ctx.Done.
			<-clock.After(ctx, gracePeriod)
		} else {
			<-ctx.Done()
		}
		cancel() // noop if context is already Done
	}()
