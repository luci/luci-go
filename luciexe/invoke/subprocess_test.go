...
func TestSubprocess(t *testing.T) {
	Convey(`Subprocess`, t, func() {
		...
		Convey(`prep initial build`, func() {
			o.CollectOutput = true
			...
			inputBuild := &bbpb.Build{
				...
				EndTime:         timestamppb.New(time.Date(2020, time.January, 2, 3, 4, 5, 3, time.UTC)),
				UpdateTime:      timestamppb.New(time.Date(2020, time.January, 2, 3, 4, 5, 4, time.UTC)),
