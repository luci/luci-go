...
func TestSubprocess(t *testing.T) {
	Convey(`Subprocess`, t, func() {
		...
		Convey(`deadline`, func() {
			...
			So(err, ShouldBeNil)
			// ensure subprocess is ready to intercept the signal.
			<-time.After(100 * time.Millisecond)
			Convey(`interrupt`, func() {
				deadlineEventCh <- lucictx.InterruptEvent
				_, err = sp.Wait()
				So(err, ShouldErrLike, "waiting for luciexe")
			...
			Convey(`timeout`, func() {
				deadlineEventCh <- lucictx.TimeoutEvent
				_, err = sp.Wait()
				So(err, ShouldErrLike, "waiting for luciexe")
				So(sp.cmd.ProcessState.ExitCode(), ShouldEqual, terminateExitCode)
			})
			Convey(`closure`, func() {
				deadlineEventCh <- lucictx.ClosureEvent
				_, err = sp.Wait()
				So(err, ShouldErrLike, "waiting for luciexe")
				So(sp.cmd.ProcessState.ExitCode(), ShouldEqual, -1)
			})
		})
