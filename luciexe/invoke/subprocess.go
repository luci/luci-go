...
// conditions are met:
//  * Fail to read the luciexe output (e.g. missing or in invalid format)
//  * The final output build has non-terminal status
//
	...
	s.waitOnce.Do(func() {
		...
		if s.collectPath != "" {
			if s.build, s.err = luciexe.ReadBuildFile(s.collectPath); s.err != nil {
				...
				s.err = errors.Reason("expected terminal status from final build; got %s", s.build.GetStatus()).Err()
				failStep(s.Step, s.err)
			}
		}
	})
