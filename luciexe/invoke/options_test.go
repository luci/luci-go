...
func TestOptionsNamespace(t *testing.T) {
	Convey(`test Options.Namespace`, t, func() {
		...
		Convey(`nested`, func() {
			...
			lo, _, err := o.rationalize(ctx)
			So(err, ShouldBeNil)
			So(lo.env.GetEmpty(bootstrap.EnvNamespace), ShouldResemble, "u/sub")
			So(lo.step, ShouldResemble, &bbpb.Step{
				Name:      "sub", // host application will swizzle this
				StartTime: nowP,
				Status:    bbpb.Status_STARTED,
				Logs: []*bbpb.Log{
					{Name: "$build.proto", Url: "u/sub/build.proto"},
					{Name: "stdout", Url: "u/sub/stdout"},
					{Name: "stderr", Url: "u/sub/stderr"},
				},
			})
		})
		...
		Convey(`deeply nested`, func() {
			o.Env.Set(bootstrap.EnvNamespace, "u")
			o.Namespace = "step|sub"
			lo, _, err := o.rationalize(ctx)
			So(err, ShouldBeNil)
			So(lo.env.GetEmpty(bootstrap.EnvNamespace), ShouldResemble, "u/step/sub")
			So(lo.step, ShouldResemble, &bbpb.Step{
				Name:      "step|sub", // host application will swizzle this
				StartTime: nowP,
				Status:    bbpb.Status_STARTED,
				Logs: []*bbpb.Log{
					{Name: "$build.proto", Url: "u/step/sub/build.proto"},
					{Name: "stdout", Url: "u/step/sub/stdout"},
					{Name: "stderr", Url: "u/step/sub/stderr"},
				},
			})
		})
	})
	...
	Convey(`Options.CacheDir`, t, func() {
		...
		Convey(`errors`, func() {
			...
			Convey(`bad override (doesn't exist)`, func() {
				o.CacheDir = filepath.Join(tdir, "cache")
				_, _, err := o.rationalize(ctx)
				So(err, ShouldErrLike, "no such file or directory")
			})
