...
func (o *Options) prepNamespace(ctx context.Context, lo *launchOptions) error {
	...
	if curLogdogNamespace != "" {
		bits = append(bits, curLogdogNamespace)
	}
	bits = append(bits, strings.Split(o.Namespace, "|")...)
	ldNS, _ := types.MakeStreamName("_", bits...)
...
func (o *Options) mkdirs() (ret dirs, err error) {
	...
	maybeMkdir := func(out *string, dirname, name string) {
		if err == nil {
			*out = filepath.Join(base, dirname)
			err = errors.Annotate(os.Mkdir(*out, 0777), "preparing %q", name).Err()
		}
	}
...
func (lo *launchOptions) prepStdio(ctx context.Context) error {
	...
	openStream := func(targ *io.WriteCloser, name string) (err error) {
		*targ, err = bs.Client.NewTextStream(
			ctx, types.StreamName(name), streamclient.ForProcess())
		return errors.Annotate(err, "opening %q", name).Err()
	}
