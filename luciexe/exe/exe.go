...
func mkBuildStream(ctx context.Context, build *bbpb.Build) (BuildSender, func() error) {
...
func Run(main func(context.Context, *bbpb.Build, BuildSender) error) {
	runCtx(gologger.StdConfig.Use(context.Background()), main)
}
...
func runCtx(ctx context.Context, main func(context.Context, *bbpb.Build, BuildSender) error) {
	retcode := func() int {
		build := &bbpb.Build{}
		if handleFn := mkOutputHandler(&osArgs, build); handleFn != nil {
			defer handleFn()
		}

		buildFrom(os.Stdin, build)
		sendBuild, closer := mkBuildStream(ctx, build)
		defer func() {
		...
		// User code; This function should convert all user code errors/panic's into
		// non-panicing state.
		//
		// Defers above here may, however, panic again, which will report the error to
		// stderr in the normal way by crashing the program.
		return func() (retcode int) {
			defer func() {
	...
	// Our outer management function completed successfully, so exit the binary.
	osExit(retcode)
}
