...
//  - Context: The context will be canceled when the program receives the os.
//   Interrupt or SIGTERM (on unix) signal. The context also has standard go
//   logging setup.
//  - *Build: The initial Build state, as read from stdin. The *Build is not
//   protected by a mutex of any sort, so the `main` function is responsible
//   for protecting it if it can be modified from multiple goroutines.
//  - userArgs: All command line arguments supplied after `ArgsDelim`.
