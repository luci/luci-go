...
func TestExe(t *testing.T) {
	Convey(`test exe`, t, func() {
		...
		osArgs = []string{"fake_test_executable"}

		ctx := context.Background()

		Convey(`basic`, func() {
			Convey(`success`, func() {
				runCtx(ctx, func(ctx context.Context, build *bbpb.Build, bs BuildSender) error {
					return nil
				})
		...
		Convey(`output`, func() {
			...
			Convey(`binary`, func() {
				...
				osArgs = append(osArgs, luciexe.OutputCLIArg, outFile)
				runCtx(ctx, func(ctx context.Context, build *bbpb.Build, bs BuildSender) error {
					build.SummaryMarkdown = "Hi."
					return nil
				})
				So(exitCode, ShouldEqual, 0)
				So(osArgs, ShouldResemble, []string{"fake_test_executable"})
				So(lastBuild(), ShouldResembleProto, &bbpb.Build{
					Status:          bbpb.Status_SUCCESS,
					SummaryMarkdown: "Hi.",
				})
