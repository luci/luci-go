...
//
// If constructLogURL is True, all returned step will have fully-qualified
// log url (i.e. logdog://host/prefix/+/log_name) and the viewer url will also
// be populated. Otherwise, the log url will simply be the log stream name and
// it is caller's responsibility to calculate the full URL. Values of
// defaultLogdogHost and defaultLogdogPrefix are only meaningful when
