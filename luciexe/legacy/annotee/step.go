...
func (p *stepConverter) convertSteps(c context.Context, bbSteps *[]*pb.Step, ann *annotpb.Step, stepPrefix string) (*pb.Step, error) {
	...
	logs, summary := p.calcLogsAndSummary(c, ann)
	bb.Logs = logs
	bb.SummaryMarkdown = summary
	...
	*bbSteps = append(*bbSteps, bb)
	...
	start, end, err := fixupStartAndEndTime(bb.StartTime, bb.EndTime, bbSubsteps)
	if err != nil {
	...
	status, err := determineStatus(bb.StartTime, bb.EndTime, ann, bbSubsteps)
	if err != nil {
		return nil, errors.Annotate(err, "Step %s: ", bb.Name).Err()
	}
	bb.Status = status
...
func determineStatus(startTime, endTime *timestamp.Timestamp, annStep *annotpb.Step, subSteps []*pb.Step) (pb.Status, error) {
	var ret pb.Status
	// Determine status.
	switch {
