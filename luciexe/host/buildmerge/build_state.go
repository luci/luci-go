...
func (t *buildStateTracker) parseAndSend(data *buffer.Batch) error {
	...
	if state.closed {
		// close new work after we update state, but before we return.
		defer t.closeWorkChan()
	} else {
...
func (t *buildStateTracker) closeWorkChan() {
	t.workCloseMu.Lock()
	defer t.workCloseMu.Unlock()
	t.work.Close()
}
