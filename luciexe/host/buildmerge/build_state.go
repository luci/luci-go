...
func newBuildState(ctx context.Context, merger *agent, namespace string, err error) *buildStateTracker {
	...
	workCh, err = dispatcher.NewChannel(ctx, &dispatcher.Options{
		...
		state := *ret.latestState.Load().(*buildState)
		// already closed
		if state.closed {
			return nil
		}
...
func (bs *buildStateTracker) GetLatest() *buildState {
	retCopy := *bs.latestState.Load().(*buildState)
	if retCopy.build == nil {
		retCopy.build = &bbpb.Build{
...
// GetFinal waits for the build to 'close' then returns the final state of the
// Build, as well as a boolean indicating that the Build's internal structure is
// safe to evaluate for merge steps.
//
// This always returns a non-nil Build to make the calling code simpler.
func (bs *buildStateTracker) GetFinal() *buildState {
	<-bs.work.DrainC
	return bs.GetLatest()
}
...
func (bs *buildStateTracker) parse(entry *logpb.LogEntry) {
	...
	// entry == nil tells us to close, so if we haven't done that yet, do it now.
	if entry == nil && !bs.shouldClose {
		bs.shouldClose = true
		bs.work.C <- nil
		bs.work.Close()
		return
