...
func TestAgent(t *testing.T) {
	...
	Convey(`buildState`, t, func() {
		...
		Convey(`bad stream type`, func() {
			...
			tracker := merger.states["url://u/build.proto"]
		...
		Convey(`can emit changes for merge steps`, func() {
			...
			rootTrack := merger.states["url://u/build.proto"]
			subTrack := merger.states["url://u/sub/build.proto"]
			So(rootTrack, ShouldNotBeNil)
			So(subTrack, ShouldNotBeNil)
			...
			Convey(`can handle recursive merge steps`, func() {
				...
				superTrack := merger.states["url://u/sub/super_deep/build.proto"]
				So(superTrack, ShouldNotBeNil)
