...
func spyOn(ctx context.Context, b *butler.Butler, base *bbpb.Build) *spy {
	curNamespace := os.Getenv(luciexe.LogdogNamespaceEnv)
	...
	if curNamespace != "" {
		userNamespace = fmt.Sprintf("%s/u", curNamespace)
	} else {
		userNamespace = "u"
	}
	if err := os.Setenv(luciexe.LogdogNamespaceEnv, userNamespace); err != nil {
		panic(err)
	}
	userNamespaceSlash := userNamespace + "/"
	builds := buildmerge.New(ctx, userNamespaceSlash, base, mkURLCalcFn())
...
func mkURLCalcFn() buildmerge.CalcURLFn {
	...
	if strings.HasPrefix(host, "file://") {
		...
		return func(nsSlash, streamName string) (url string, viewURL string) {
			fullStreamName := nsSlash + streamName
