...
// stdin.
//
// After the luciexe has finished its initialization and is 'running', it MUST
// close stdin.
//
...
//   - The absolute path of the cache directory is available in LUCI_CONTEXT as
//     LUCI_CONTEXT["run_build"]["cache_dir"]. This directory MAY be on the same
//     filesystem as CWD (but is not guaranteed to be). The available caches are
//     described in Buildbucket as CacheEntry messages.
//
// See also:
//   * LUCI_CONTEXT: https://chromium.googlesource.com/infra/luci/luci-py/+/HEAD/client/LUCI_CONTEXT.md
//
...
// implement the LUCI Executable protocol using their "run_build" subcommand.
//
// LUCI Executables on Buildbucket
//
// Buildbucket accepts LUCI Executables as CIPD packages containing the
// luciexe to run with the fixed name of "run_build".
//
// On Windows, this may be named "run_build.exe" or "run_build.bat". Buildbucket
