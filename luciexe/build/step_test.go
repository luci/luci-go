...
func TestStepLog(t *testing.T) {
	Convey(`Step logging`, t, func() {
		...
		Convey(`logging redirects`, func() {
			...
			So(lc.GetFakeData()["step/0/log/0"].GetStreamData(), ShouldContainSubstring, "set status: SUCCESS")
			So(lc.GetFakeData()["step/0/log/0"].GetStreamData(), ShouldContainSubstring, "hi there!")
		})
		...
		Convey(`can open logs`, func() {
			...
			So(lc.GetFakeData()["step/0/log/1"].GetStreamData(), ShouldContainSubstring, "here's some stuff")
		})
		...
		Convey(`can open datagram logs`, func() {
			...
			So(lc.GetFakeData()["step/0/log/1"].GetDatagrams(), ShouldContain, "here's some stuff")
		})

		Convey(`sets LOGDOG_NAMESPACE`, func() {
			ctx = (environ.Env{}).SetInCtx(ctx) // blank out environ to avoid environmental effects
			So(environ.FromCtx(ctx).GetEmpty(luciexe.LogdogNamespaceEnv), ShouldEqual, "")

			_, subCtx := StartStep(ctx, "some step")
			So(environ.FromCtx(subCtx).GetEmpty(luciexe.LogdogNamespaceEnv), ShouldEqual, "step/0/u")

			_, subSubCtx := StartStep(ctx, "some sub step")
			So(environ.FromCtx(subSubCtx).GetEmpty(luciexe.LogdogNamespaceEnv), ShouldEqual, "step/1/u")
		})
	})
}
