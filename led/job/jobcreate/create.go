...
func FromNewTaskRequest(r *swarming.SwarmingRpcsNewTaskRequest, name, swarmingHost string) (ret *job.Definition, err error) {
	...
	case "bbagent":
		...
		err = jobDefinitionFromBuildbucket(bb, r)

	case "kitchen":
		bb := &job.Buildbucket{LegacyKitchen: true, Name: name}
		ret.JobType = &job.Definition_Buildbucket{Buildbucket: bb}
		err = jobDefinitionFromBuildbucketLegacy(bb, r)
