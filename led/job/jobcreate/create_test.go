...
func readTestFixture(fixtureBaseName string) *job.Definition {
	...
	jd, err := FromNewTaskRequest(req, "test_name", "swarming.example.com")
	So(err, ShouldBeNil)
...
func TestCreateKitchen(t *testing.T) {
	...
	Convey(`consume kitchen buildbucket swarming task`, t, func() {
		jd := readTestFixture("kitchen")

		So(jd.GetBuildbucket(), ShouldNotBeNil)
		So(jd.Info().SwarmingHostname(), ShouldEqual, "chromium-swarm.appspot.com")
		So(jd.Info().TaskName(), ShouldEqual, "led: test_name")
	})
}
