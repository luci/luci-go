...
func Apply(ctx context.Context, b *bbpb.Build, task *swarmingpb.TaskRequest) error {
	for _, name := range b.GetInput().GetExperiments() {
		if exp, ok := knownExperiments[name]; ok {
			...
			logging.Warningf(ctx, "Experiment %q is unknown to led: not interpreting it. "+
				"Note that bbagent will see the experiment, it still may interpret it somehow", name)
		}
	}
	return nil
}
