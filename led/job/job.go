...
type expiringDims struct {
	...
	// key -> values
	dimesions map[string]stringset.Set
}

func (ed *expiringDims) addDimVals(key string, values ...string) {
	if ed.dimesions == nil {
		ed.dimesions = map[string]stringset.Set{}
	}
	if set, ok := ed.dimesions[key]; !ok {
		ed.dimesions[key] = stringset.NewFromSlice(values...)
	} else {
		set.AddAll(values)
	}
}
...
func (ed *expiringDims) updateFrom(other *expiringDims) {
	for key, values := range other.dimesions {
...
func (ed *expiringDims) createWith(template *swarmingpb.TaskProperties) *swarmingpb.TaskProperties {
	...
	newDims := make([]*swarmingpb.StringListPair, 0, len(ed.dimesions))
	for _, key := range keysOf(ed.dimesions) {
		newDims = append(newDims, &swarmingpb.StringListPair{
			Key: key, Values: ed.dimesions[key].ToSortedSlice()})
	}
...
func (jd *Definition) makeExpiringSliceData() (ret []*expiringDims, err error) {
	...
	// Cache and dimention expiration have opposite defaults for 0 or negative
	// times.
	//
...
func (jd *Definition) FlattenToSwarming(ctx context.Context, uid string, ks KitchenSupport) error {
	...
	baseProperties.Env = make([]*swarmingpb.StringPair, len(bb.EnvVars))
	copy(baseProperties.Env, bb.EnvVars)
	expEnvValue := "FALSE"
	if bb.BbagentArgs.Build.Input.Experimental {
		expEnvValue = "TRUE"
	}
	baseProperties.Env = append(baseProperties.Env, &swarmingpb.StringPair{
