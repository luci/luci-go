...
func (jd *Definition) addLedProperties(ctx context.Context, uid string) error {
	...
	// TODO(iannucci): change logdog project to something reserved to 'led'.
...
func (ed *expiringData) createWith(template *swarmingpb.TaskProperties) *swarmingpb.TaskProperties {
	...
	for _, dim := range ret.Dimensions {
		addDims(dim.Key, dim.Values...)
	}
...
func (jd *Definition) generateCommand(ctx context.Context) ([]string, error) {
	bb := jd.GetBuildbucket()
	...
	if bb.LegacyKitchen {
		...
		propStruct.Fields["$recipe_engine/buildbucket"] = &structpb.Value{
			Kind: &structpb.Value_StringValue{StringValue: buildStr}}

		kitchenArgs.RecipeName = propStruct.Fields["recipe"].GetStringValue()
		delete(propStruct.Fields, "recipe")

		var jsonBuf bytes.Buffer
		err = (&jsonpb.Marshaler{}).Marshal(&jsonBuf, bb.BbagentArgs.Build.Input.Properties)
		if err != nil {
...
func (jd *Definition) FlattenToSwarming(ctx context.Context, uid string) error {
	...
	baseProperties.Command, err = jd.generateCommand(ctx)
	if err != nil {
