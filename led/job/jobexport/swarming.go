...
// panic.
func ToSwarmingNewTask(ctx context.Context, jd *job.Definition, uid string) (*swarming.SwarmingRpcsNewTaskRequest, error) {
	if err := jd.FlattenToSwarming(ctx, uid); err != nil {
	...
	ret := &swarming.SwarmingRpcsNewTaskRequest{
		BotPingToleranceSecs: task.BotPingTolerance.Seconds,
		Name:                 task.Name,
		User:                 uid,
	...
	for _, slice := range task.TaskSlices {
		...
		toAdd := &swarming.SwarmingRpcsTaskSlice{
			...
			Properties: &swarming.SwarmingRpcsTaskProperties{
				...
				ExecutionTimeoutSecs: props.ExecutionTimeout.Seconds,
				GracePeriodSecs:      props.GracePeriod.Seconds,
				IoTimeoutSecs:        props.IoTimeout.Seconds,
