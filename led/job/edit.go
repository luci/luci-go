...
type HighLevelEditor interface {
	...
	// is obtained from this location; it will be installed to `pathInTask`.
	//
	// If cipdPkg and cipdVers are omitted, they indicate that the task payload is
	// obtained from the UserPayload isolate source. The job will assume that it
	// can find the payload at `pathInTask`.
	TaskPayload(cipdPkg, cipdVers string, pathInTask string)

	// AddGerritChange ensures the GerritChange is in the set of inputs.
	AddGerritChange(cl *bbpb.GerritChange)

	// RemoveGerritChange ensures the GerritChange is not in the set of inputs.
