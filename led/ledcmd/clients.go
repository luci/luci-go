...
var ledUserAgent = "led"

func init() {
	if info, err := version.GetCurrentVersion(); err != nil {
		ledUserAgent = fmt.Sprintf("led, %s @ %s", info.PackageName, info.InstanceID)
	} else {
		ledUserAgent = "led, <unknown version>"
	}
}
