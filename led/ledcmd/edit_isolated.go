...
func EditIsolated(ctx context.Context, authClient *http.Client, jd *job.Definition, xform IsolatedTransformer, authOpts auth.Options) error {
...
func uploadToCas(ctx context.Context, client *client.Client, dir string) (*digest.Digest, error) {
	...
	rootDg, chunkers, _, err := client.ComputeMerkleTree(dir, &is, chunker.DefaultChunkSize, filemetadata.NewNoopCache())
	if err != nil {
		return nil, errors.Annotate(err, "failed to compute Merkle Tree").Err()
	}

	_, err = client.UploadIfMissing(ctx, chunkers...)
	if err != nil {
