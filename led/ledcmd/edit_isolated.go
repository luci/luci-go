...
// IsolatedTransformer is a function which recieves a directory on the local
...
func EditIsolated(ctx context.Context, authClient *http.Client, jd *job.Definition, xform IsolatedTransformer) error {
	...
	if current.Digest != "" {
		...
		dl := downloader.New(ctx, mkIsoClient(authClient, current), isolated.HexDigest(current.Digest), tdir, &downloader.Options{
	...
	hash, err := isolateDirectory(ctx, mkIsoClient(authClient, jd.UserPayload), tdir)
	if err != nil {
		return err
	}
