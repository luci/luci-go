...
func FetchOutputJSON(ctx context.Context, swarmingHost, taskID string) ([]byte, error) {
	// Set up swarming service for getting task info.
	swarmSvc, err := swarmingAPI.NewService(ctx)
	if err != nil {
		return nil, err
	}
	swarmSvc.BasePath = fmt.Sprintf("%s/%s", swarmingHost, swarmingAPIEndpoint)
	...
	// Fetch the isolate.
	logging.Infof(
		ctx, "Fetching %s for isolated outs of task %s in %s", ref.Isolated, taskID, swarmingHost)
	buf := &bytes.Buffer{}
	...
	if !ok {
		return nil, errors.Reason(
			...
			outputJSONFileName, taskID, swarmingHost).Err()
	}

	// Now fetch (from the same server and namespace) the output file by digest.
	logging.Infof(
		ctx,
		"Fetching %s for output of task %s in %s",
		string(outputFile.Digest), taskID, swarmingHost,
	)
	buf = &bytes.Buffer{}
