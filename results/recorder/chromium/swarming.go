...
// IsolatedRef describes the fields needed to fetch an isolated object.
type IsolatedRef struct {
	// Server is the isolate server where the object lives.
	Server string

	// Namespace describes the namespace where the object lives.
	Namespace string

	// Digest describes the hash (digest) of the object.
	Digest string
}
...
type SwarmingService interface {
	...
	GetTaskResult(taskID string) (*swarming.SwarmingRpcsTaskResult, error)
}
...
func FetchOutputJSON(ctx context.Context, swarmSvc SwarmingService, id string) (*bytes.Buffer, error) {
	...
	// Fetch the isolate.
	logging.Infof(
		ctx, "Fetching %s for isolated outs of task %s in %s", string(ref.Digest), id, host)
	buf := &bytes.Buffer{}
	if err := isoClient.Fetch(ctx, isolated.HexDigest(ref.Digest), buf); err != nil {
		return nil, err
	}
...
func GetOutputsRef(ctx context.Context, swarmSvc SwarmingService, id string) (*IsolatedRef, error) {
	taskResult, err := swarmSvc.GetTaskResult(id)
	if err != nil {
		return nil, err
	}
	...
	return &IsolatedRef{
		taskResult.OutputsRef.Isolatedserver,
		taskResult.OutputsRef.Namespace,
		taskResult.OutputsRef.Isolated,
	}, nil
}
...
func GetProdSwarmingService(ctx context.Context, host string) (*ProdSwarmingService, error) {
	...
	service.BasePath = host
	return &ProdSwarmingService{service}, nil
}

// GetHost returns basepath for this prod swarming service.
func (s *ProdSwarmingService) GetHost() string {
	return s.Service.BasePath
}

// GetTaskResult gets the task with given ID.
func (s *ProdSwarmingService) GetTaskResult(taskID string) (*swarming.SwarmingRpcsTaskResult, error) {
	return s.Service.Task.Result(taskID).Do()
}

// getProdIsolatedClient gets the correct prod isolateserver for the given object ref.
// TODO: handle the authClient correctly.
func getProdIsolatedClient(ctx context.Context, ref *IsolatedRef) (*isolatedclient.Client, error) {
