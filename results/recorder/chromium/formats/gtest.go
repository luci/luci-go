...
// TestIteration represents the per_iteration_data as described in
// https://cs.chromium.org/chromium/src/base/test/launcher/test_results_tracker.h?l=83&rcl=96020cfd447cb285acfa1a96c37a67ed22fa2499
// (base::TestResultsTracker::SaveSummaryAsJSON)
//
// Fields not used by Test Results are omitted.
...
func convertFromJSONToGTest(ctx context.Context, reader io.Reader) (*GTestResults, error) {
	results := &GTestResults{}
	if err := json.NewDecoder(reader).Decode(results); err != nil {
		return nil, err
	}

	if len(results.AllTests) == 0 {
		return nil, errors.Annotate(ErrBadFormat, `missing "all_tests" in output.json`).Err()
	}
