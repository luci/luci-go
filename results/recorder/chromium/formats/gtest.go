...
//
// This can error in one of two ways:
// 1) failure to marshal, which is unrecoverable for any format, and
// 2) ErrBadFormat, which indicates successful marshalling but mismatched fields
//    for GTest results.
func (r *GTestResults) ConvertFromJSON(ctx context.Context, reader io.Reader) error {
	...
	if len(r.AllTests) == 0 {
		return errors.Annotate(ErrBadFormat, `missing "all_tests" field in JSON`).Err()
	}
