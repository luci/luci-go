		...
		for name, results := range data {
			...
			if err := variantDefMap.Validate(); err != nil {
				return nil, errors.Annotate(
					err,
					"Iteration %d of test %s in task %s on %s failed to validate variant def",
					i, name, req.Task.Id, req.Task.Hostname).Err()
			}
...
func toGTestStatus(s string) (resultspb.Status, error) {
	...
	default:
		...
		return resultspb.Status_STATUS_UNSPECIFIED, errors.Reason("unknown GTest status %s", s).Err()
	}
}
...
func (r *GTestResults) convertRunResult(ctx context.Context, name string, result *GTestRunResult) *resultspb.Result {
	...
	if err != nil {
		logging.Errorf(ctx, "Failed to convert GTest Status %s", result.Status)
	}
