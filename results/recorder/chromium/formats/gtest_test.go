...
func TestGTestConversions(t *testing.T) {
	...
	Convey(`To Invocation works`, t, func() {
		results := &GTestResults{
			...
			StartTime: float64(1565478949.087654321),
		}
		...
		expectedTestTags := map[string][]*resultspb.StringPair{
			"prefix/test1": {
				&resultspb.StringPair{Key: "lossless_snippet", Value: "true"},
				&resultspb.StringPair{Key: "file", Value: "fileA.cc"},
				&resultspb.StringPair{Key: "line", Value: "12"},
			},
			"prefix/test2": {
				&resultspb.StringPair{Key: "file", Value: "fileA.cc"},
				&resultspb.StringPair{Key: "line", Value: "24"},
			},
			"prefix/test3": {
				&resultspb.StringPair{Key: "file", Value: "fileB.cc"},
				&resultspb.StringPair{Key: "line", Value: "36"},
			},
		}
		for _, tpb := range inv.Tests {
			...
			for i, res := range tpb.Variants[0].Results {
				...
				So(res.EndTime.Seconds-res.StartTime.Seconds, ShouldEqual, 0)
				So(
					res.EndTime.Nanos-res.StartTime.Nanos,
					ShouldAlmostEqual,
					expectedTestDurationsNs[tpb.Name][i],
					100,
				)
