...
func TestGTestJsonConversion(t *testing.T) {
	...
	Convey(`Works`, t, func() {
		results, err := convertFromJSONToGTest(ctx, bytes.NewReader(buf))
		So(err, ShouldBeNil)
		...
		So(results.PerIterationData, ShouldResemble, []map[string][]*GTestRunResult{
			map[string][]*GTestRunResult{
				"FooTest.TestDoBar": []*GTestRunResult{
					&GTestRunResult{
						Status: "CRASH",
						ElapsedTimeMs: 1837,
				...
				"FooTest.TestDoBaz": []*GTestRunResult{
					&GTestRunResult{
						Status: "SUCCESS",
						ElapsedTimeMs: 837,
		...
		So(results.TestLocations, ShouldResemble, map[string]*Location{
			"FooTest.TestDoBar": &Location{File: "../../chrome/browser/foo/test.cc", Line: 287},
			"FooTest.TestDoBaz": &Location{File: "../../chrome/browser/foo/test.cc", Line: 293},
		})
	})
}
