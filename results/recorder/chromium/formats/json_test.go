	...
	Convey(`To Invocation works`, t, func() {
		...
		So(
			...
			[]*resultspb.StringPair{
				{Key: "json_format_tag", Value: "desktop"},
				{Key: "json_format_tag", Value: "linux"},
				{Key: "json_format_tag", Value: "ubuntu"},
				{Key: "test_framework", Value: "json"},
			},
		)
		...
		Convey(`durations captured correctly`, func() {
			...
			Convey(`fails with mismatched runs and durations`, func() {
				...
				So(err, ShouldErrLike, "had 4 durations populated but has 3 test statuses")
			})
		})
