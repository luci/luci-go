...
// Parse parses []*resultspb.TestResult from r and updates inv in-place
// accordingly.
// If an error is returned, inv is left intact.
//
// Does not populate TestResult.Name.
func (r *JSONTestResults) Parse(ctx context.Context, req *resultspb.DeriveInvocationRequest, inv *resultspb.Invocation) ([]*resultspb.TestResult, error) {
	if r.Version != 3 {
...
func ofJSONStatus(s string) (resultspb.TestStatus, error) {
	switch s {
	case "CRASH":
...
func (f *TestFields) toProtos(dest *[]*resultspb.TestResult, testPath string) error {
	...
	if len(f.Times) > 0 {
		durations = f.Times
	} else if f.Time != 0 { // Do set duratoin if it is unknown.
