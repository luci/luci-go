...
func (r *JSONTestResults) convertTests(curPath string, curNode json.RawMessage) error {
	...
	for key, value := range maybeNode {
		...
		// Errors may be because field names are actually used as names in the trie nodes, so ignore
		// them. If the trie cannot be converted, curNode will eventually not be a map when it should be
		// and we will fail at the respective convertTests call.
