...
func (results *JSONTestResults) convertTests(curPath string, curNode json.RawMessage) error {
	...
	for key, value := range maybeNode {
		...
		delim := "/"
		testPath := key
		if results.PathDelimiter != "" {
			delim = results.PathDelimiter
		}
		if curPath != "" {
		...
		if err := results.convertTests(testPath, value); err != nil {
