...
func ConvertFromJSON(ctx context.Context, reader io.Reader) (*JSONTestResults, error) {
	...
	// Convert Tests.
	if err := convertTests(ctx, "", results.TestsRaw, results); err != nil {
		return nil, err
	}
	/*for key, value := range results.TestsRaw {
		if err := convertTests(ctx, key, value, results); err != nil {
			return nil, err
		}
	}*/
...
func convertTests(ctx context.Context, curPath string, curNode json.RawMessage, results *JSONTestResults) error {
	...
	for key, value := range maybeNode {
		...
		// No error might still mean that the RawMessage was only a dict, not a TestFields.
		if maybeFields.Actual != "" && maybeFields.Expected != "" {
			if results.Tests == nil {
				results.Tests = make(map[string]*TestFields)
			}
			results.Tests[testPath] = maybeFields
			continue
		}

		// Otherwise, try to process it as an intermediate node.
		if err := convertTests(ctx, testPath, value, results); err != nil {
