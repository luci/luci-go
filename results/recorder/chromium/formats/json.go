		...
		if curPath != "" {
			...
			if prefixJson, ok := r.Metadata[testNamePrefixKey]; ok {
				...
				if err := json.Unmarshal(prefixJson, &prefix); err != nil {
					return errors.Annotate(err, "%s not string, got %v", testNamePrefixKey, prefixJson).Err()
				}
				testPath = fmt.Sprintf("%s%s", prefix, key)
			}
		}
...
func (f *TestFields) toProtos(name string) ([]*resultspb.TestResult, error) {
	...
	if len(durations) > 0 && len(statuses) != len(durations) {
