			...
			Items: &genai.Schema{
				...
				Properties: map[string]*genai.Schema{
					"path": {
						...
						Description: "The relative path to the modified file in the culprit CL",
					},
					"edits": {
						Type: genai.TypeArray,
						Items: &genai.Schema{
...
Please generate the fix CL to fix the compile failure. You must output search/replace blocks for ONLY files modified in the Culprit CL. Do NOT edit neighbor files or any files outside the Culprit CL. You must output search/replace blocks for ONLY the exact snippets you wish to modify.
...
func GenerateFixforwardCL(ctx context.Context, genaiClient llm.Client, gerritClient *gerrit.Client, culpritCommit string, failureLog string, repoUrl string, revertURL string) error {
	...
	neighborFiles := make(map[string]string)
	...
	if totalInfoSize < maxTotalSize {
		...
		for dir := range directories {
			...
			// Append neighbor files in this directory for read-only LLM context
			for _, file := range files {
				fullPath := dir + "/" + file
				// Skip if we already fetched it as modified file or neighbor file
				if _, ok := modifiedFiles[fullPath]; ok {
					continue
				}
				if _, ok := neighborFiles[fullPath]; ok {
					continue
				}
	...
	// 6. Validate and pre-apply edits strictly to culprit modified files
	editedFiles := make(map[string]string)
	for _, f := range llmParsed.Files {
		content, ok := modifiedFiles[f.Path]
		if !ok {
			logging.Warningf(ctx, "LLM returned edits for file %s which was not modified in culprit CL %s; skipping edit", f.Path, culpritCommit)
			continue
		}
		if prev, ok := editedFiles[f.Path]; ok {
			content = prev
		}
		fileModified := false
		...
		if fileModified {
			editedFiles[f.Path] = content
		}
	}

	if len(editedFiles) == 0 {
		return errors.Reason("no valid edits to culprit modified files generated by LLM for culprit %s", culpritCommit)
	}
	...
	// 8. Apply edits
	for filePath, content := range editedFiles {
		...
		if err := gerritClient.ChangeEditFileContent(ctx, editReq); err != nil {
			logging.Errorf(ctx, "Failed to edit file %s on CL %d: %v", filePath, change.Number, err)
		}
	}
