...
func TestUpdateAnalysisProgress(t *testing.T) {
	...
	Convey("UpdateAnalysisProgress NthSection", t, func() {
		...
		Convey("Schedule run for next commit", func() {
			...
			So(singleRerun.Status, ShouldEqual, gfipb.RerunStatus_RERUN_STATUS_FAILED)
			So(res, ShouldResemble, &gfipb.UpdateAnalysisProgressResponse{})
		...
		Convey("Nthsection couldn't find suspect", func() {
			...
			req := &gfipb.UpdateAnalysisProgressRequest{
				...
				RerunResult: &gfipb.RerunResult{
					RerunStatus: gfipb.RerunStatus_RERUN_STATUS_INFRA_FAILED,
				},
				BotId: "abc",
			}
		...
		Convey("Nthsection error", func() {
			...
			req := &gfipb.UpdateAnalysisProgressRequest{
				...
				RerunResult: &gfipb.RerunResult{
					RerunStatus: gfipb.RerunStatus_RERUN_STATUS_PASSED, // This would result in a conflict, since commit 9 failed
				},
				BotId: "abc",
			}
			...
			So(singleRerun2.Status, ShouldEqual, gfipb.RerunStatus_RERUN_STATUS_PASSED)
