...
func TestGetMessageType(t *testing.T) {
	...
	Convey("Works", t, func() {
		...
		msg := &testprotos.Complex{
			...
			OneofVal: &testprotos.Complex_Simple{
				Simple: simple,
			},
		}
		...
		desc = mt.fields["simple"]
		So(desc.typ, ShouldEqual, reflect.TypeOf(&testprotos.Simple{}))
		So(desc.onProtoReflection(val).Interface().(*testprotos.Simple), ShouldEqual, simple)

		// Grabbing another oneof alternative "switches" the wrapper to point to it.
