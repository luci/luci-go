...
type fieldDesc struct {
	...
	// onChanged is called each time the field is assigned to by the skylark code.
	//
	...
	onChanged func(dict skylark.StringDict)

	// onProtoReflection is called when the skylark message representation is
...
func GetMessageType(typ reflect.Type) (*MessageType, error) {
	...
	//     So we'll build a reverse map {7 -> ['a', 'b']} for this.
...
type oneofAlternative struct {
	name     string       // e.g. "a" for the example above
...
func (g *oneofGroup) onAlternativePicked(alt *oneofAlternative, dict skylark.StringDict) {
...
// onReflectingAlternative is called when writing skylark value into the proto
// message.
//
...
func (g *oneofGroup) onReflectingAlternative(alt *oneofAlternative, msg reflect.Value) reflect.Value {
	// Is the wrapping field already set to some value? Look at the type of this
	// value, maybe it is already what we want. Note that 'wrap' itself is an
	// interface (isMsg_Wrapper), so we need to look deeper at the type pointed to
	// by the interface.
	wrap := msg.Field(g.fieldIdx)
	if !wrap.IsNil() && wrap.Elem().Type() == alt.outerTyp {
		// Need to "dereference" the interface to to set fields inside the value.
		wrap = wrap.Elem() // ~ wrap = wrap.(*testprotos.Msg_FieldA)
	} else {
		// Need to replace the wrapper value with the correctly typed one.
		wrap = reflect.New(alt.outerTyp.Elem()) // ~ wrap = &testprotos.Msg_FieldA{}
		msg.Field(g.fieldIdx).Set(wrap)         // ~ msg.Wrapper = wrap
	}
	return wrap.Elem().Field(0) // ~ return ref to wrap.A, to be set to
}
