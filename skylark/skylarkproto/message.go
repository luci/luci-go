...
func (m *Message) ToProto() (proto.Message, error) {
	...
	for name, val := range m.fields {
		...
		assigner, err := getAssigner(fd.typ, val)
		if err == nil {
			err = assigner(fd.value(msg))
		}
		if err != nil {
...
func (m *Message) Type() string { return m.typ.name }
...
func (m *Message) Attr(name string) (skylark.Value, error) {
	// A method?
	if builtin := messageMethods[name]; builtin != nil {
		return builtin.BindReceiver(m), nil
	}
...
func (m *Message) AttrNames() []string {
	// Note: AttrNames is called only by dir(...), it is OK that it is slow.
	out := make([]string, 0, len(m.typ.fields)+len(messageMethods))
	for k := range m.typ.fields {
		out = append(out, k)
	}
	for k := range messageMethods { // see methods.go
		out = append(out, k)
	}
	sort.Strings(out)
	return out
...
func (m *Message) SetField(name string, val skylark.Value) error {
	// Overriding methods is not allowed.
	if messageMethods[name] != nil {
		return fmt.Errorf("can't overwrite method field %q in message %q", name, m.Type())
	}
	...
	// Instantiate an assigner to do type checking, but do not actually assign
	// anything, since at this point we do not yet have Go values to assign to,
	// everything is represented by Skylark values stored in 'fields'.
	if _, err := getAssigner(fd.typ, val); err != nil {
