...
func TestAll(t *testing.T) {
	...
	// Loads protos from test.proto, grab a reference to the module dict.
	dict, err := LoadModule("go.chromium.org/luci/lucisky/skylarkproto/testprotos/test.proto")
	if err != nil {
		t.Fatalf("failed to load testprotos/test.proto - %s", err)
	}
	predeclared["testprotos"] = dict["testprotos"]

	// Load skylark assertion helpers.
	dict, err = skylarktest.LoadAssertModule()
	if err != nil {
		t.Fatalf("failed to load assertion module - %s", err)
	}
	predeclared["assert"] = dict["assert"]
...
func runSingleTest(t *testing.T, script string, predeclared skylark.StringDict) {
	// Hook up prints and asserts inside *.sky to 't'.
