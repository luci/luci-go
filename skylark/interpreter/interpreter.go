...
type Interpreter struct {
	...
	// Logger is called by skylarks' built in print(...) function.
	//
	...
	globals skylark.StringDict       // builtins + symbols from init.sky
}
...
func (intr *Interpreter) Init() error {
	...
	// Add all user defined builtins.
	for k, v := range intr.Builtins {
		intr.globals[k] = v
	}
...
func (intr *Interpreter) loadIfMissing(key string, loader func() (skylark.StringDict, error)) (skylark.StringDict, error) {
	...
	// This will be overwritten to something nice on non-panic.
	m := &loadedModule{
		err: fmt.Errorf("panic when loading %q", key),
	}
...
func (intr *Interpreter) loadFileModule(module string) (skylark.StringDict, error) {
	src, err := ioutil.ReadFile(module)
	if err != nil {
		return nil, errors.Annotate(err, "failed to read %q", module).Err()
	}
	return skylark.ExecFile(intr.thread(), intr.rootRel(module), src, intr.globals)
}
