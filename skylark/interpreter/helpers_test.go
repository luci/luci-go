...
func deindend(s string) string {
	...
	for _, line := range lines {
		...
		if idx == -1 {
			continue // either an empty line or entirely whitespace
		}
		indent = line[:idx]
	}
...
type intrParams struct {
	...
	// from the file system). If there's main.sky script, it will be executed via
	// ExecFile and its global dict keys returned.
	scripts map[string]string

	// stdlib contains stdlib source code, as path => body mapping. In particular,
	// init.sky will be auto-loaded by the interpreter.
...
//
// Returns keys of the dict of the main.sky script (if any), and a list of
...
func runIntr(p intrParams) (keys []string, logs []string, err error) {
	...
	if _, ok := p.scripts["main.sky"]; ok {
		var dict skylark.StringDict
		dict, err = intr.ExecFile("main.sky")
		if err == nil {
...
// runScript runs a single init.sky script through the interpreter.
func runScript(body string) error {
	_, _, err := runIntr(intrParams{
		stdlib: map[string]string{"init.sky": body},
	})
	return err
}
