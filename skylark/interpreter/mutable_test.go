...
func TestMutable(t *testing.T) {
	...
	Convey("Works", t, func() {
		...
		So(err, ShouldBeNil)
		So(logs, ShouldResemble, []string{"[builtin:init.sky:5] [1, 2]"})
	})
		...
		So(runScript(`
			def test():
				...
				if s.get() != 0:
					fail("should be 0")
				if not s:
					fail("should be truish")  # holds a value, even if it's 0!

				s.set(None)
				if s.get() != None:
					fail("should be None")
				if s:
					fail("should be falseish")  # doesn't hold a value anymore
