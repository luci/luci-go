...
//
// It serializes JSON-ish skylark value to string. Doesn't support integers
...
// visitingSet is a set of containers we currently has recursed into.
//
// Used to detect cycles.
type visitingSet map[interface{}]struct{}

func (v visitingSet) push(container interface{}) error {
...
func (v visitingSet) pop(container interface{}) {
	delete(v, container)
}
...
func toGoNative(v skylark.Value, visiting visitingSet) (interface{}, error) {
	// Push containers to 'visiting' set right away. Note that Tuples are special,
	// since they are not hashable (being a slice). We push a pointer instead.
	...
	if container != nil {
		if err := visiting.push(container); err != nil {
			return nil, err
		}
		defer visiting.pop(container)
	}
