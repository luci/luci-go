...
// List is a Starlark value that looks like a list, but implements type checks
// and implicit conversions when assigning elements.
//
// Differences from regular list:
//   * Not comparable. go.starlark.net has no way to express that typed lists
//     should be comparable to typed lists AND to regular lists. And without
//     this feature comparisons become very confusing. Callers can convert typed
//     lists to regular lists first if they need to compare them.
type List struct {
	item Converter      // defines the type of elements
...
//
// All subsequently added elements will be converter the 'item' converter too.
...
// Converter returns the type converted used by this list.
func (l *List) Converter() Converter { return l.item }

// Binary implement 'l <op> y' (or 'y <op> l', depending on 'side').
//
...
func (l *List) Binary(op syntax.Token, y starlark.Value, side starlark.Side) (starlark.Value, error) {
	...
	case syntax.PLUS:
		...
		if yl, ok := y.(*starlark.List); ok {
			...
			asTyped, err := NewList(l.item, vals)
			if err != nil {
				return nil, err
			}
		...
		// When adding typed lists types must match.
		if yl, ok := y.(*List); ok {
			if yl.item != l.item {
				goto unsupported // adding typed list of different types
			}
...
func (l *List) asTypedList(v starlark.Value, err error) (starlark.Value, error) {
	if err == nil {
		if asL, ok := v.(*starlark.List); ok {
			return &List{item: l.item, list: asL}, nil
		}
	}
	return v, err
}
...
func (l *List) AttrNames() []string {
	...
	// list methods that we might need to "wrap" in Attr(...).
	return []string{
		"append",
		"clear",
		"extend",
...
func (l *List) Type() string {
	return fmt.Sprintf("list<%s>", l.item.Type())
}

func (l *List) String() string {
	// Note: if we don't override this some error messages looks very confusing,
	// e.g. 'Error: ["1", "2"] != ["1", "2"]' when comparing typed list to a
	// regular list.
	return fmt.Sprintf("list<%s>(%s)", l.item.Type(), l.list.String())
}
...
func (l *List) wrappedExtend(orig *starlark.Builtin) *starlark.Builtin {
	return starlark.NewBuiltin("extend", func(th *starlark.Thread, _ *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
		...
		// be a noop anyway.
		if it, ok := iterable.(*List); ok && it.item == l.item {
