...
m.b = True
assert.eq(m.b, True)
assert.eq(proto.to_pbtext(m), 'b: true\n')
m.b = False
assert.eq(m.b, False)
assert.eq(proto.to_pbtext(m), '')  # 'false' is default!
...
def set_bad():
  m2.b = [1, 2, 3]
assert.fails(set_bad, 'can\'t assign list to a value of kind "bool"')

# We don't support implicit conversions to bool. Callers should use bool(...)
# cast explicitly.
def set_int():
  m2.b = 0
assert.fails(set_int, 'can\'t assign integer to a value of kind "bool"')

# Assiging bool to non-bool field fails.
def set_bool_to_int():
  m2.i64 = False
assert.fails(set_bool_to_int, 'can\'t assign boolean to a value of kind "int64"')
