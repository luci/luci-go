...
// interfaces.
//
// TODO(vadimsh): Currently not safe to cross-goroutine use without external
...
func (m *Message) FromProto(p proto.Message) error {
	...
	for name, fd := range m.typ.fields {
		...
		// oneof alternatives this is an invalid zero value, we skip then right
...
// field.
//
// It convert dicts or Nones to *Message of corresponding type using NewMessage
...
func maybeMakeMessages(typ reflect.Type, val starlark.Value) (starlark.Value, error) {
	if dict, ok := val.(*starlark.Dict); ok && isProtoTyp(typ) {
	...
	if seq, ok := val.(starlark.Sequence); ok && typ.Kind() == reflect.Slice && isProtoTyp(typ.Elem()) && shouldMakeMessages(seq) {
...
func isProtoTyp(typ reflect.Type) bool {
