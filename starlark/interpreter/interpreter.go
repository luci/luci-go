...
// 'exec' in a way to support loading Starlark programs that consist of many
// files that reference each other, thus allowing decomposing code into small
// logical chunks and enabling code reuse.
//
//
...
// the loaded code always executes only once. Interpreter MAY load modules in
// parallel in the future, libraries must not rely on their loading order and
...
// of Interpreter can also supply arbitrary "predeclared" symbols they want to
// be available in the global scope of all modules.
package interpreter
