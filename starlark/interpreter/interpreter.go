...
// it is not idempotent: calling 'exec' on a module twice actually executes it
// twice.
//
...
type Interpreter struct {
	...
	execing map[moduleKey]struct{}      // a set of currently exec'ing modules
...
// package, to use is if '@pkg' part in 'ref' is omitted.
...
func (intr *Interpreter) Init(ctx context.Context) error {
	...
	intr.execing = map[moduleKey]struct{}{}
...
func (intr *Interpreter) LoadModule(ctx context.Context, pkg, path string) (starlark.StringDict, error) {
	...
	// This module has been 'exec'-ed previous? Then it is not allowed to be
	// loaded. Modules are either 'library-like' or 'script-like', not both.
...
func (intr *Interpreter) ExecModule(ctx context.Context, pkg, path string) (starlark.StringDict, error) {
	...
	// This module has been 'load'-ed previous? Then it is not allowed to be
	// executed. Modules are either 'library-like' or 'script-like', not both.
	if _, yes := intr.modules[key]; yes {
		return nil, errors.New("the module has been loaded before and therefore is not executable")
	}

	// Mark this module as not loadable anymore.
	intr.execed[key] = struct{}{}

	// Forbid recursive loads.
	if _, yes := intr.execing[key]; yes {
		return nil, errors.New("cycle while executing module")
	}
	intr.execing[key] = struct{}{}
	defer func() { delete(intr.execing, key) }()
...
func (intr *Interpreter) execBuiltin() *starlark.Builtin {
	return starlark.NewBuiltin("exec", func(th *starlark.Thread, fn *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
		...
		// The thread was produced by LoadModule or ExecModule? Custom threads (e.g.
		// as used for running Starlark callbacks from native code) do not have
		// enough context to perform 'exec' safely.
		...
		if err != nil {
			return nil, fmt.Errorf("cannot load %s: %s", module.GoString(), err)
		}
