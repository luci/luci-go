...
// Built-in packages
//
// There are two special packages recognized natively by the interpreter:
// '__main__' and 'stdlib'.
//
...
// "@__main__//<path>" are presented as simply "//<path>". That's it, no other
// special handling.
//
// Module '@stdlib//init.star' (if present) is loaded before any other code
// and all its exported symbols that do not start with '_' are made available
// in the global namespace of all other modules (except ones loaded by
// '@stdlib//init.star' itself).
//
//
// Built-in symbols
//
// Embedders of the interpreter can supply arbitrary "predeclared" symbols they
// want to be available in the global scope of all loaded modules.
//
// Additionally, all modules have access to:
//
//
//  def struct(**kwargs):
...
// Init initializes the interpreter and loads '@stdlib//init.star'.
//
// Registers most basic built-in symbols first (like 'struct' and 'fail'), then
// whatever was passed via Predeclared. Then loads '@stdlib//init.star', which
// may define more symbols or override already defined ones.
//
// Whatever symbols end up in the global dict of '@stdlib//init.star' module
// will become available as global symbols in all modules.
func (intr *Interpreter) Init() error {
	...
	top, err := intr.LoadModule(StdlibPkg, "init.star")
...
func (intr *Interpreter) execModule(pkg, path string) (starlark.StringDict, error) {
	...
	th := &starlark.Thread{
		Load: func(th *starlark.Thread, loadPath string) (starlark.StringDict, error) {
			...
			// Modules within a package refer to sibling files in the same package
			// when not specifying a package alias in load(...).
			if key.pkg == "" {
				key.pkg = pkg
			}
	...
	// Construct a full module name for error messages and stack traces. Omit a
