...
// Exported functions:
//
//    def loader(*raw_descriptor_sets):
//      """Returns a new proto descriptor loader."""
//
//    def to_textpb(msg):
...
func ProtoLib() starlark.StringDict {
	return starlark.StringDict{
		"proto": starlarkstruct.FromStringDict(starlark.String("proto"), starlark.StringDict{
			"loader":           starlark.NewBuiltin("loader", loader),
...
// loader constructs new Loader, seeding it with the given descriptor sets.
func loader(_ *starlark.Thread, fn *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	if len(kwargs) > 0 {
