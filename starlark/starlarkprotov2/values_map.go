...
// key is key in protoreflect.Map, together with its Starlark representation.
type key struct {
...
func toStarlarkDict(l *Loader, fd protoreflect.FieldDescriptor, m protoreflect.Map) (*starlark.Dict, error) {
	...
	// Construct starlark.Dict from sorted keys and values taken from proto map.
...
func assignProtoMap(m protoreflect.Message, fd protoreflect.FieldDescriptor, iter starlark.IterableMapping) error {
	...
	for it.Next(&sk) {
		...
		if !ok {
			continue // should not really be happening for well-behaved types
		}
