...
func prepareRHS(l *Loader, fd protoreflect.FieldDescriptor, v starlark.Value) (starlark.Value, error) {
	...
	case fd.IsList():
		...
		// it is some other list<T'>, T'!=T. Otherwise make a copy.
		myT := converter(l, fd)
		if tpd, ok := v.(*typed.List); ok {
			if tpd.Converter() == myT {
				return v, nil // exact same type, reuse by reference
			}
			return nil, fmt.Errorf("use explicit list(...) conversion")
		}
		return typed.AsTypedList(myT, v) // make a copy

	case fd.IsMap():
		// Reuse dict<K,V> by reference if it has matching type. Reject it right
		// away it is some other dict<K',V'>, K'!=K || V'!=V. Otherwise make a copy.
		...
		if tpd, ok := v.(*typed.Dict); ok {
			...
			return nil, fmt.Errorf("use explicit dict(...) conversion")
		}
		return typed.AsTypedDict(myK, myV, v) // make a copy
