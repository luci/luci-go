...
// MessageFromProto creates a new message of this type and populates it based on
// values in the given proto.Message of the matching type.
//
// Type of 'p' must match 't', panics otherwise. To check, compare
// p.ProtoReflect().Descriptor() to t.Descriptor().
func (t *MessageType) MessageFromProto(p proto.Message) *Message {
	...
	refl.Range(func(fd protoreflect.FieldDescriptor, v protoreflect.Value) bool {
		...
		if err != nil {
			panic(fmt.Errorf("field %q: %s", fd.Name(), err))
		}
		return true
	})
	return m
