...
def test():
  match, groups = matches('a (\d+) (\d+)', 'a 123 456 tail')
  assert(match, True)
  assert(groups, ("a 123 456", "123", "456"))

  match, groups = matches('a (\d+) (\d+)', 'a huh 456')
  assert(match, False)
  assert(groups, ())

  match, groups = matches('.*', 'a 123 456 tail')
  assert(match, True)
...
func TestRegexp(t *testing.T) {
	...
	Convey("Fails", t, func() {
		So(runScript(`matches('(((', '')`), ShouldErrLike, "error parsing regexp")
	})
}
