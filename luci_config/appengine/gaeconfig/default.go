...
type CacheConfig struct {
	// ServiceCache, if not nil, installs a cache layer immediately after the
	// remote configuration service and caches its responses.
	ServiceCache func(c context.Context, b backend.B, s *Settings) backend.B
...
func installCacheCronHandlerImpl(r *router.Router, base router.MiddlewareChain, be backend.B) {
	base = base.Extend(func(c *router.Context, next router.Handler) {
		...
		c.Context = installConfigBackend(c.Context, mustFetchCachedSettings(c.Context), be, CacheConfig{
			...
			ServiceCache: func(c context.Context, be backend.B, s *Settings) backend.B {
...
func UseFlex(c context.Context, cache *lru.Cache) context.Context {
	...
	if cache != nil {
		ccfg.ServiceCache = func(c context.Context, be backend.B, s *Settings) backend.B {
			lruBE := caching.LRU{
				Cache:      cache,
...
func installConfigBackend(c context.Context, s *Settings, be backend.B, ccfg CacheConfig) context.Context {
	...
	if exp := s.CacheExpiration(); exp > 0 {
		...
		if fn := ccfg.ServiceCache; fn != nil {
			be = fn(c, be, s)
		}
		...
		if fn := ccfg.With; fn != nil {
			c = fn(c, be, s)
		}

		if fn := ccfg.DataCache; fn != nil {
			be = fn(c, be, s)
		}

		// Install in-memory cache (LRU).
		be = caching.LRUBackend(be, processCacheLRUSize, exp)
	}
