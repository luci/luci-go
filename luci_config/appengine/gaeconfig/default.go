...
// It requires that the hostname and the email of config service have been
// defined in the appengine app settings page before the installed endpoints are
// called. If the caller is not the config service, it also requires that the
// name of the trusted group has been defined in the appengine app settings.
func InstallValidationHandlers(r *router.Router, base router.MiddlewareChain, validator *validation.Validator) {
	...
	base = base.Extend(a.GetMiddleware(), func(c *router.Context, next router.Handler) {
		...
		if s.ConfigServiceHost == "" {
			errStatus(w, http.StatusInternalServerError, "ConfigServiceHost has not been defined in settings")
			return
		}
		if s.ConfigServiceEmail == "" {
			errStatus(w, http.StatusInternalServerError, "ConfigServiceEmail has not been defined in settings")
			return
		}
		...
		if s.AdministratorsGroup == "" {
			errStatus(w, http.StatusInternalServerError, "AdministratorsGroup has not been defined in settings")
			return
		}
		switch isAdmin, err := auth.IsMember(cc, s.AdministratorsGroup); {
		case err != nil:
			errStatus(w, http.StatusInternalServerError, "Unable to authenticate")
		case isAdmin:
			next(c)
		default:
			errStatus(w, http.StatusForbidden, "Insufficient authority for validation")
		}
	})
