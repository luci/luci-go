...
func StartEmulator(ctx context.Context) (*Emulator, error) {
	...
	if emulator.cfgDir, err = emulator.createSpannerEmulatorConfig(); err != nil {
		return nil, err
	}
...
func (emulator *Emulator) opts() []option.ClientOption {
	return []option.ClientOption{
		option.WithEndpoint(emulator.hostport),
...
func (emulator *Emulator) Stop() error {
	...
	if emulator.cfgDir != "" {
		switch removeErr := filesystem.RemoveAll(emulator.cfgDir); {
		case removeErr == nil:
			emulator.cfgDir = ""
			fmt.Println("gcloud config removed")
		default:
			return errors.Annotate(removeErr, "failed to remove the temporary config directory").Err()
		}
	}
	return nil
}
