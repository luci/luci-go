...
// Package spantest implements creation/destruction of temporary Spanner .
package spantest
...
type TempDBConfig struct {
	// InstanceName specifies the Spannner instance where to create the
	// temporary database.
	...
	//
	// In leu of a proper DDL parser, it is split using regexes. Therefore
	// the script MUST:
...
func NewTempDB(ctx context.Context, cfg TempDBConfig) (*TempDB, error) {
	...
	dbName := fmt.Sprintf("db_%d", time.Now().Unix()%1000)
