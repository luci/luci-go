...
func NewTempInstance(ctx context.Context, projectName string) (string, error) {
	...
	if emulatorAddr == "" {
		return "", errors.Annotate(fmt.Errorf("cloud spanner emulator host not found"), errAnnotation).Err()
	}
...
func StartSpannerEmulator(ctx context.Context) (*Emulator, error) {
	...
	emulator := &Emulator{
		Host:     fmt.Sprintf("localhost:%d", mathrand.Intn(ctx, 500)+9000),
		RestPort: mathrand.Intn(ctx, 500) + 9000,
	}

	fmt.Println(fmt.Sprintf("Starting Cloud Spanner Emulator at host: %s, restport: %d", emulator.Host, emulator.RestPort))
	cmd := exec.Command("gcloud", "beta", "emulators", "spanner", "start", "--host-port", emulator.Host, "--rest-port", fmt.Sprintf("%d", emulator.RestPort))
...
func (emulator *Emulator) CreateSpannerEmulatorConfig() (string, error) {
	...
	os.Setenv("SPANNER_EMULATOR_HOST", emulator.Host)
	cmd := exec.Command(
		"/bin/sh",
		"-c",
		fmt.Sprintf("gcloud config configurations create spanner-emulator;"+
			...
			" gcloud config set api_endpoint_overrides/spanner http://localhost:%d/;", emulator.RestPort))
	return tdir, cmd.Run()
}
