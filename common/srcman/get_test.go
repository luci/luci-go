...
import (
	...
	"go.chromium.org/luci/srcman/buildsource/rawpresentation"
)
...
func TestGet(t *testing.T) {
	...
	Convey(`Test srcman.Get`, t, func() {
		...
		demoData := &srcman.Manifest{
			Directories: map[string]*srcman.Manifest_Directory{
				"something": {
					GitCheckout: &srcman.Manifest_GitCheckout{
		...
		Convey(`cache`, func() {
			Convey(`read OK`, func() {
				putDemo()
				m, hsh, err := Get(ctx, &srcman.ManifestLink{Sha256: demoID})
				So(err, ShouldErrLike, nil)
			...
			Convey(`bad Read`, func() {
				...
				_, _, err := Get(ctx, &srcman.ManifestLink{Sha256: demoID, Url: "banana://fweep"})
		...
		Convey(`uncached`, func() {
			...
			manifest, hsh, err := Get(ctx, &srcman.ManifestLink{
			...
			Convey(`cached after logdog read`, func() {
				manifest2, hsh2, err := Get(ctx, &srcman.ManifestLink{
					Sha256: hsh,
					Url:    "monkeyspit",
				})
				So(err, ShouldErrLike)
