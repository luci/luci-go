...
// from logdog.
func GetMulti(ctx context.Context, links ...*srcman.ManifestLink) ([]*srcman.Manifest, [][]byte, error) {
	retLock := sync.Mutex{}
	retManifests := make([]*srcman.Manifest, len(links))
...
// from logdog.
func Get(ctx context.Context, a *srcman.ManifestLink) (*srcman.Manifest, []byte, error) {
