...
func WithContext(ctx context.Context, srv *Server, cb func(context.Context, *net.TCPAddr) error) error {
	// Bind to the port.
	addr, err := srv.Initialize(ctx)
	if err != nil {
	...
	logging.Debugf(ctx, "The Devshell server is at http://127.0.0.1:%d", addr.Port)
