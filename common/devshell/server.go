...
func (s *Server) serve() (err error) {
	...
	for {
		...
		if err != nil {
			select {
			case <-s.getDoneChan():
				logging.WithError(err).Errorf(s.ctx, "server closed")
			default:
			}
			return
		}
...
func (c *client) handle() error {
	...
	if err := c.readRequest(); err != nil {
		if err := c.respondWithError(err); err != nil {
			return fmt.Errorf("failed to send error: %v", err)
		}
		return nil
	}

	buf, err := c.generateResponse()
	if err != nil {
		if err := c.respondWithError(fmt.Errorf("internal error")); err != nil {
			return fmt.Errorf("failed to send error: %v", err)
		}
		return err
	}
...
func (c *client) generateResponse() ([]byte, error) {
	// Get the token.
	...
	response := []interface{}{c.email, nil, t.AccessToken, expiry}
...
func (c *client) respondWithError(err error) error {
	var buf bytes.Buffer
	buf.WriteString(fmt.Sprintf("%d\n", len(err.Error())))
