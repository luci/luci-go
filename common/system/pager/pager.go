...
// creates less subprocess, directs its stdout to os.Stdout and calls fn with
// out set to less stdin. fn's context is canceled if the user quits less.
//
// If fn returns non-zero exit code before less exits, Main returns the exit
// code. Otherwise Main returns less's exit code.
...
func Main(ctx context.Context, fn func(ctx context.Context, out io.WriteCloser) int) int {
	...
	go func() {
		...
		// Let less know that this is the end.
		stdin.Close()
	}()
