...
type myCustomComparisonObj struct{}

func (myCustomComparisonObj) cmp(actual any, expected ...any) string {
	if len(expected) != 1 {
...
func TestCustomComparisonSelectorTest(t *testing.T) {
	t.Parallel()

	var custom myCustomComparisonObj

	ftt.Run("something", t, func(t *ftt.Test) {
		assert.Loosely(t, "cheese", convey.Adapt(custom.cmp)("cheese"))
	})
}
