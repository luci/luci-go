...
type Channel struct {
	...
	// ClosedC will unblock when this Channel's processor has ACK'd that C is
	// closed.
	ClosedC <-chan struct{}
...
// already closed).
//
// This blocks until ClosedC is unblocked.
...
// IsClosed returns true iff the Channel is closed.
...
func NewChannel(ctx context.Context, opts *Options, send SendFn) (Channel, error) {
	...
	return Channel{C: itemCh, ClosedC: closedCh, DrainC: drainCh}, nil
}
