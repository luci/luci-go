...
func NewChannel(ctx context.Context, opts *Options, send SendFn) (ret Channel, err error) {
	...
	cstate := coordinatorState{
		...
		ctx:   ctx,
		isDbg: logging.GetLevel(ctx) <= logging.Debug,
		timer: clock.NewTimer(clock.Tag(ctx, "coordinator")),
	}

	if cstate.isDbg {
		cstate.dbg = logging.Get(logging.SetField(ctx, "dispatcher.coordinator", true)).Debugf
	} else {
	...
	ret = Channel{C: itemCh, DrainC: drainCh}
	return
}
