...
// SafeSend attempts to send the item `itm` to the Channel, which may be closed.
//
// On successful send, returns true, otherwise false.
func (c Channel) SafeSend(itm interface{}) (ok bool) {
	defer func() { recover() }()
	c.C <- itm
	return true
}
