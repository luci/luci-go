	...
	Convey(`test good option groups`, t, func() {
		...
		for _, options := range goodOptions {
			Convey(options.name, func() {
				...
				if expect.ItemSizerFunc == nil {
					So(myOptions.ItemSizerFunc, ShouldBeNil)
				} else {
					So(myOptions.ItemSizerFunc, ShouldEqual, expect.ItemSizerFunc)
					expect.ItemSizerFunc = nil
				}
				myOptions.ItemSizerFunc = nil
...
func TestOptionValidationBad(t *testing.T) {
	Convey(`bad option validation`, t, func() {
		...
		Convey(`ItemSizerFunc == nil`, func() {
			...
			Convey(`NeedsItemSize`, func() {
				opts := Options{Buffer: buffer.Defaults}
				opts.Buffer.FullBehavior = &buffer.DropOldestBatch{MaxLiveBytes: 71}
				So(opts.normalize(ctx), ShouldErrLike, "Buffer.FullBehavior requires size info")
			})
		})
	})
}
