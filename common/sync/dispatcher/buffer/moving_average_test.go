...
func TestMovingAverage(t *testing.T) {
	Convey(`movingAverage`, t, func() {
		...
		Convey(`construction`, func() {
			...
			Convey(`panics on bad initialGuess`, func() {
				So(func() {
					newMovingAverage(100, -1)
				}, ShouldPanicLike, "initialGuess must be")
			})
		})
		...
		Convey(`usage`, func() {
			...
			Convey(`avg matches initialGuess`, func() {
			...
			Convey(`get() always returns > 0`, func() {
				for i := 0; i < 100; i++ {
					ma.record(0)
				}
				So(ma.get(), ShouldEqual, 1)
			})
		})
