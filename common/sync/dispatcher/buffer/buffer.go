...
//   * newBatch shoudl be true if this is a batch which has never been inserted
...
func (buf *bufferImpl) maybePruneLocked(id uint64, newBatch bool) bool {
	if buf.lenLocked() < buf.opts.MaxItems {
		return true
	}
	...
	case DropOldestBatch:
		dropped := buf.heap.DropOneBatchOlderThan(id)
		if dropped != nil {
			buf.itemsInHeap -= dropped.countedSize
		}
...
func (buf *bufferImpl) maybeCutBatchLocked(ctx context.Context) {
	...
	shouldCut := buf.opts.BatchSize > 0 && len(batch.Data) >= buf.opts.BatchSize
	if !shouldCut {
...
func (buf *bufferImpl) AddNoBlock(ctx context.Context, item interface{}) {
	...
	if buf.currentBatch == nil {
		buf.currentBatch = &Batch{
			Data:     make([]interface{}, 0, buf.batchSizeGuess.get()),
...
func (buf *bufferImpl) lenLocked() int {
	ret := buf.itemsInHeap
	if buf.currentBatch != nil {
