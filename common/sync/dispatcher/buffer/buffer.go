...
func (buf *Buffer) LeaseOne(now time.Time) (leased *Batch) {
	// Maybe flush the current batch if it's got data and has been waiting long
	// enough.
	//
	// NOTE: !(now < nextSend) == (now >= nextSend)
	if cur := buf.currentBatch; cur != nil {
		if len(cur.Data) > 0 && !now.Before(cur.nextSend) {
			buf.Flush(now)
		}
	}

	switch {
	case len(buf.knownLeases) >= int(buf.opts.MaxLeases):
		// not allowed to make another lease
	case len(buf.unleased) == 0:
		// no batches to lease out
