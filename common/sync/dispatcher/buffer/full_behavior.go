...
type BlockNewItems struct {
	...
	// The maximum* number of bytes that this Buffer is allowed to house (including
	// both leased and unleased items).
	//
	...
	// a BatchSizeMax of 100, and you insert items worth 999 bytes, this will
	// still allow the addition of another item (of up to 100 bytes) before
...
type DropOldestBatch struct {
	...
	//
	// Default: -1 if BatchItemsMax == -1 else max(1000, BatchItemsMax)
	// Required: Must be >= BatchItemsMax or -1 (only MaxLiveBytes applies)
	MaxLiveItems int

	// The maximum number of leased and unleased bytes that the Buffer may have
	// before dropping data.
	//
	// Once a batch is dropped, it no longer counts against MaxLiveBytes, but it
	...
	// time is:
	//
	//     MaxLiveBytes + (BatchSizeMax * MaxLeases)
	//
	// NOTE: This may only be set if BatchSizeMax is > 0; Otherwise buffer will
...
func (d *DropOldestBatch) ComputeState(stats Stats) (okToInsert, dropBatch bool) {
	...
	sizeDrop := d.MaxLiveBytes > -1 && stats.TotalSize() >= d.MaxLiveBytes
...
func (d *DropOldestBatch) Check(opts Options) (err error) {
	...
	if d.MaxLiveBytes == 0 {
		d.MaxLiveBytes = negOneOrMult(opts.BatchSizeMax, 5)
	}
	...
	switch {
	case d.MaxLiveBytes == -1:
	case d.MaxLiveBytes < opts.BatchSizeMax:
		return errors.Reason("DropOldestBatch.MaxLiveBytes must be >= BatchSizeMax[%d]: got %d",
			opts.BatchSizeMax, d.MaxLiveBytes).Err()
	case opts.BatchSizeMax == -1:
		return errors.Reason("DropOldestBatch.MaxLiveBytes may only be set with BatchSizeMax[%d] > 0",
			opts.BatchSizeMax).Err()
	}

	if d.MaxLiveItems == -1 && d.MaxLiveBytes == -1 {
		return errors.Reason("DropOldestBatch must have one of MaxLiveItems or MaxLiveBytes > 0").Err()
	}
