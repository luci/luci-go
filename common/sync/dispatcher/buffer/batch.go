...
func (b LeasedBatch) NACK(ctx context.Context, err error) {
	...
	buf.mu.Lock()
	heapDiff := 0
	if canInsert := buf.maybePruneLocked(b.batchID, false); canInsert {
		heapDiff = b.countedSize - b.leasedSize
		heap.Push(&buf.heap, b.Batch)
	} else {
		heapDiff = b.leasedSize
	}
	buf.itemsInHeap -= heapDiff
	buf.mu.Unlock()
}
