	...
	var goodOptions = []struct {
...

		{
			...
			options: Options{
				...
				FullBehavior:  &DropOldestBatch{MaxLiveItems: 400, MaxLiveBytes: -1},
				Retry:         retry.None,
			},
			expected: Options{
				MaxLeases:     2,
...

		{
			...
			expected: Options{
				...
				BatchSizeMax:  Defaults.BatchSizeMax,
				BatchAgeMax:   Defaults.BatchAgeMax,
				FullBehavior:  &DropOldestBatch{MaxLiveItems: 1000, MaxLiveBytes: -1},
				Retry:         Defaults.Retry,
			},
		},

		{
			...
			expected: Options{
				...
				BatchSizeMax:  Defaults.BatchSizeMax,
				BatchAgeMax:   Defaults.BatchAgeMax,
				FullBehavior:  &DropOldestBatch{MaxLiveItems: 2000, MaxLiveBytes: -1},
				Retry:         Defaults.Retry,
			},
		},
...
func TestOptionValidationBad(t *testing.T) {
	var badOptions = []struct {
		...
		{
			"DropOldestBatch.MaxLiveBytes < -1",
			Options{
				FullBehavior: &DropOldestBatch{MaxLiveBytes: -2},
			},
			"DropOldestBatch.MaxLiveBytes must be",
		},

		{
			"DropOldestBatch.MaxLiveItems == DropOldestBatch.MaxLiveBytes == -1",
			Options{
				FullBehavior: &DropOldestBatch{MaxLiveItems: -1, MaxLiveBytes: -1},
			},
			"DropOldestBatch must have one of",
		},

		{
			"DropOldestBatch.MaxLiveBytes > 0 && BatchSizeMax == -1",
			Options{
				FullBehavior: &DropOldestBatch{MaxLiveBytes: 100},
			},
			"DropOldestBatch.MaxLiveBytes may only be set",
		},
		...
		{
			"BlockNewItems.MaxLiveBytes > 0 && BatchSizeMax == -1",
			Options{
