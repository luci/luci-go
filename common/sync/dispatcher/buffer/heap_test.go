		...
		Convey(`ordering`, func() {
			...
			Convey(`(nextSend, id)`, func() {
				...
				Convey(`test push-pop yields sorted`, func() {
					...
					for {
						// shuffle until it's not the same as the original; it's possible to
						// get unlucky and shuffle it right into the original order :p
						rand.Shuffle(shuffled.Len(), shuffled.Swap)
						if ShouldNotResemble(shuffled, h) == "" {
							break
						}
					}
