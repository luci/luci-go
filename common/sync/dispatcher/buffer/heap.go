...
// Oldest finds and returns the oldest batch id and index.
//
// If not found, the returned idx will be -1
func (h *batchHeap) Oldest() (id uint64, idx int) {
	idx = -1
	for i, batch := range *h {
		if id == 0 || batch.id < id {
			id, idx = batch.id, i
...
// PopAt pops the *Batch at the given index in the heap. Used in conjunction
// with OldestID.
func (h *batchHeap) PopAt(idx int) *Batch {
	return heap.Remove(h, idx).(*Batch)
}
