...
type Stats struct {
	...
	//
	// UnleasedItemSizeBytes is the size in bytes of the same items.
	...
	// leases.
	//
	// LeasedItemSizeBytes is the size in bytes of the same items.
	LeasedItemCount int
	...
	//
	// DroppedLeasedItemSizeBytes is the size in bytes of the same items.
...
func (s *Stats) addOneUnleased() {
	s.update(1, actionAdd, categoryUnleased)
}

func (s *Stats) add(b *Batch, to category) {
	s.update(b.countedSize, actionAdd, to)
}

func (s *Stats) del(b *Batch, to category) {
	s.update(b.countedSize, actionDel, to)
}

func (s *Stats) mv(b *Batch, from, to category) {
	s.update(b.countedSize, actionMove, from, to)
}

func (s *Stats) update(items int, act action, from category, to ...category) {
	switch {
	case act == actionAdd:
		count := s.getCategoryVars(from)
		*count += items

	case act == actionDel || act == actionMove:
		count := s.getCategoryVars(from)
		*count -= items

		if act == actionMove {
			count = s.getCategoryVars(to[0])
			*count += items
		}

	default:
		panic(fmt.Errorf("unknown action %d", act))
	}
}
