...
func TestPriorityQueue(t *testing.T) {
	...
	// This prevents new work to start.
	blockAllWorkers := func(workers int, work chan<- PriorityWork) func() {
		// first, block all workers, so that new work is not scheduled
	...
	Convey(`PriorityQueue schedules in the priority order`, t, func() {
		...
		err := PriorityQueue(1, func(work chan<- PriorityWork) bool {
			releaseWorkers := blockAllWorkers(1, work)

			// add work in the reverse order
			for i := 0; i < 5; i++ {
				i := i
				work <- PriorityWork{
					Priority: i, // high priority work
					F: func() error {
						orderLock.Lock()
			...
			// release workers. Let the prioritization kick in and decide which work to do, which should be
	...
	Convey(`PriorityQueue stops scheduling new work when work channel is closed`, t, func() {
		highPriorityExecuted := int32(0)
		const workers = 10
		err := PriorityQueue(workers, func(work chan<- PriorityWork) bool {
			releaseWorkers := blockAllWorkers(workers, work)
			...
			for i := 0; i < workers; i++ {
				...
				work <- PriorityWork{
					...
					F: func() error {
						// this work should not be scheduled
						return errors.New("low priority work was scheduled")
					},
				}
			}
			...
			// Wait till high priority group starts. If there is a bug, this test will hang :(
			highPriorityStarting.Wait()

			// HACK: we need to stop scheduling (by returning false), and THEN release high priority workers.
			// Strictly speaking, doing it without a race is not possible with this API, so we are cheating here.
