...
func TestBuffer(t *testing.T) {
	...
	Convey(`A task Buffer`, t, func() {
		...
		Convey(`Has proper task dispatch order`, func() {
			...
			worker_started := make(chan int, iters+3) // tasks push here.
			wait := make(chan int)                    // first task will wait until it is closed.
			gen := func(taskC chan<- func() error) {
				...
				// Ensure 1 task has actually execution. Note that task is still running
				// because it's blocked on `wait` channel.
				<-worker_started
