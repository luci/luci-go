...
// PriorityQueue works like WorkPool, but schedules work in the order of
// priority.
//
// PriorityQueue consumes items from the work channel as soon as they are sent
// and puts them to a priority queue. As soon as there is an available worker,
// starts the highest priority item, if any.
//
	...
	for {
		...
		case c, ok := <-candidateC:
			...
			case atomic.LoadInt32(&stop) == 1:
				// Stop scheduling new work, but wait till the
				// already-scheduled-work is done.
				close(chosen)
