...
func (up *UnbufferedPublisher) Publish(c context.Context, msgs ...*pubsub.Message) ([]string, error) {
	...
	if err != nil {
		...
		if up.RecreateClient != nil {
			ierr := up.RecreateClient()
			if ierr == nil {
				logging.Debugf(c, "Recreatng a new PubSub client")
			} else {
				logging.WithError(err).Warningf(c, "Failed to create a new PubSub client")
			}
		}
		return nil, err
	}
