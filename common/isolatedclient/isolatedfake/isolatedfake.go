...
func New() IsolatedFake {
	...
	s.mux.HandleFunc("/", func(w http.ResponseWriter, req *http.Request) {
		s.Fail(fmt.Errorf("unknown endpoint %s", req.URL))
	})
	return s
}
...
func (s *isolatedFake) Contents() map[string]map[isolated.HexDigest][]byte {
	...
	for namespace, c := range s.contents {
		x := map[isolated.HexDigest][]byte{}
		for k, v := range c {
			x[k] = v
		}
		out[namespace] = x
...
func (s *isolatedFake) preupload(r *http.Request) interface{} {
	...
	for i, d := range data.Items {
		if _, ok := s.contents[data.Namespace.Namespace]; !ok {
			s.contents[data.Namespace.Namespace] = map[isolated.HexDigest][]byte{}
		}
		if _, ok := s.contents[data.Namespace.Namespace][isolated.HexDigest(d.Digest)]; !ok {
			// Simulate a write to Cloud Storage for larger writes.
			ticket := "ticket:" + data.Namespace.Namespace + "," + string(d.Digest)
...
func (s *isolatedFake) fakeCloudStorageUpload(w http.ResponseWriter, r *http.Request) {
	...
	if r.Header.Get("Content-Type") != "application/octet-stream" {
		w.WriteHeader(400)
		s.Fail(fmt.Errorf("invalid content type: %s", r.Header.Get("Content-Type")))
		return
	}
	if r.Method != "PUT" {
		w.WriteHeader(405)
		s.Fail(fmt.Errorf("invalid method: %s", r.Method))
		return
	}
...
func (s *isolatedFake) fakeCloudStorageDownload(w http.ResponseWriter, r *http.Request) {
	...
	namespace := r.URL.Query().Get("namespace")
	n, ok := s.contents[namespace]
	if !ok {
		w.WriteHeader(404)
		s.Fail(fmt.Errorf("namspace not found: %s", namespace))
		return
	}
	digest := isolated.HexDigest(r.URL.Query().Get("digest"))
	data, ok := n[digest]
	if !ok {
		w.WriteHeader(404)
		s.Fail(fmt.Errorf("file not found: %s", digest))
		return
	}
...
func (s *isolatedFake) retrieve(r *http.Request) interface{} {
	...
	rawContent, ok := s.contents[data.Namespace.Namespace][digest]
	if !ok {
