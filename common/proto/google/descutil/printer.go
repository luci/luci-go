...
// openNL prints a string, followed by " {\n" and increases indentation level.
// Returns a function that decreases indentation level and closes the brace
// followed by a newline.
// Usage: defer openNL("package x")()
func (p *Printer) openNL(format string, a ...interface{}) func() {
	p.Printf(format, a...)
	p.Printf(" {\n")
	p.Out.Level++
	return func() {
...
func (p *Printer) Service(service *descriptorpb.ServiceDescriptorProto, methodIndex int) {
	...
	defer p.openNL("service %s", service.GetName())()
...
func (p *Printer) optionValue(fd protoreflect.FieldDescriptor, v protoreflect.Value) {
	...
	case protoreflect.EnumNumber:
		p.Printf(" %s", fd.Enum().Values().ByNumber(x).Name())
...
func collectOptions(m protoreflect.ProtoMessage) optFields {
	toPrint := optFields{}

	m.ProtoReflect().Range(func(fd protoreflect.FieldDescriptor, v protoreflect.Value) bool {
		toPrint = append(toPrint, optField{fd, v})
		return true
	})
	sort.Slice(toPrint, func(i, j int) bool {
		return toPrint[i].fd.Number() < toPrint[j].fd.Number()
	})
	return toPrint
}
...
func (of optFields) write(p *Printer, prefix string, afterEach func(last bool)) {
	for i, f := range of {
		if f.fd.IsExtension() {
			p.Printf("%s(%s) =", prefix, p.shorten(string(f.fd.FullName())))
		} else {
			p.Printf("%s%s =", prefix, f.fd.TextName())
		}
		p.optionValue(f.fd, f.v)
		afterEach(i == len(of)-1)
	}
}

func (p *Printer) fieldOptions(field *descriptorpb.FieldDescriptorProto) {
	needJSONName := field.GetJsonName() != camel(field.GetName())
	toPrint := collectOptions(field.Options)

	if !needJSONName && len(toPrint) == 0 {
		return
	}
	...
	if needJSONName {
		p.Printf("json_name = %q", field.GetJsonName())
		nl(len(toPrint) > 0)
	}
...
func (p *Printer) Message(msg *descriptorpb.DescriptorProto) {
	...
	defer p.openNL("message %s", msg.GetName())()

	collectOptions(msg.Options).write(p, "option ", func(last bool) {
		p.Printf(";\n")
	})
...
func (p *Printer) OneOf(msg *descriptorpb.DescriptorProto, oneOfIndex int) {
	...
	defer p.openNL("oneof %s", of.GetName())()
...
func (p *Printer) Enum(enum *descriptorpb.EnumDescriptorProto) {
	...
	defer p.openNL("enum %s", enum.GetName())()
