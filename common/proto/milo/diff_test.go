	...
	Convey(`test Diff`, t, func() {
			...
			Convey(`directory mod`, func() {
				...
				So(d.Directories, ShouldResemble, map[string]*ManifestDiff_Directory{
					"foo": {
						...
						GitCheckout: ManifestDiff_MODIFIED,
					},
				})
			})
		...
		Convey(`cipd packages`, func() {
			a := &Manifest{
				Directories: map[string]*Manifest_Directory{
					"foo": {
						CipdServerUrl: "https://some.server.example.com",
			...
			Convey(`url diff`, func() {
				b.Directories["foo"].CipdServerUrl = "https://nope@nope.example.com/nope.nope"
				d := a.Diff(b)
				So(d.Overall, ShouldEqual, ManifestDiff_MODIFIED)
				So(d.Directories, ShouldResemble, map[string]*ManifestDiff_Directory{
					"foo": {
						Overall:       ManifestDiff_MODIFIED,
						CipdServerUrl: ManifestDiff_MODIFIED,
					},
				})
			})
		...
		Convey(`isolated items`, func() {
			a := &Manifest{
				Directories: map[string]*Manifest_Directory{
					"foo": {
						IsolatedServerUrl: "https://some.server.example.com",
			...
			Convey(`server url`, func() {
				b.Directories["foo"].IsolatedServerUrl = "https://nurbs.example.com"
				d := a.Diff(b)
				So(d.Overall, ShouldEqual, ManifestDiff_MODIFIED)
				So(d.Directories, ShouldResemble, map[string]*ManifestDiff_Directory{
					"foo": {
						Overall:           ManifestDiff_MODIFIED,
						IsolatedServerUrl: ManifestDiff_MODIFIED,
					},
				})
			})
