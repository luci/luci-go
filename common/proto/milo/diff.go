...
func (old *Manifest_GitCheckout) Diff(new *Manifest_GitCheckout) ManifestDiff_Stat {
...
//
// The two objects should be ordered so that `old` is the older of the two.
...
func (old *Manifest_Directory) Diff(new *Manifest_Directory) *ManifestDiff_Directory {
	...
	ret.Overall = zeroCmpTwo(old, new, func() ManifestDiff_Stat {
		...
		ret.GitCheckout = dirChanged.add(old.GitCheckout.Diff(new.GitCheckout))

		ret.CipdServerUrl = dirChanged.add(zeroCmpTwo(old.CipdServerUrl, new.CipdServerUrl, nil))
		if ret.CipdServerUrl == ManifestDiff_EQUAL && new.CipdServerUrl != "" {
		...
		ret.IsolatedServerUrl = dirChanged.add(zeroCmpTwo(old.IsolatedServerUrl, new.IsolatedServerUrl, nil))
