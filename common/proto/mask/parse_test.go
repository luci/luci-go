...
func TestParsePath(t *testing.T) {
	Convey("Expect path parsing", t, func() {
		...
		Convey("fails for invalid delimiter", func() {
			tryParsePath("str@").andExpectError(fmt.Errorf("expect delimiter: .; got @"))
		})
		...
		Convey("fails when parsing repeated field and path ends with index", func() {
			tryParsePath("strs.1").andExpectError(fmt.Errorf("expect a star following repeated field; got token: 1"))
			tryParsePath("nums.2").andExpectError(fmt.Errorf("expect a star following repeated field; got token: 2"))
			tryParsePath("msgs.a").andExpectError(fmt.Errorf("expect a star following repeated field; got token: a"))
		})
		...
		Convey("fails when parsing map field with incompatible key type", func() {
			tryParsePath("map_num_str.a").andExpectError(fmt.Errorf("expect map key kind int32; got token: a"))
			tryParsePath("map_str_num.1").andExpectError(fmt.Errorf("expect map key kind string; got token: 1"))
			tryParsePath("map_bool_str.not_a_bool").andExpectError(fmt.Errorf("expect map key kind bool; got token: not_a_bool"))
		})
		...
		Convey("fails when parsing message field and given a non-string field name", func() {
			tryParsePath("msg.123").andExpectError(fmt.Errorf("expect a field name of type string; got token: 123"))
		})
		Convey("fails when parsing message field and star is not the last token", func() {
			tryParsePath("msg.*.str").andExpectError(fmt.Errorf("expect end of string; got token: str"))
		})
		...
		Convey("fails when multiple delimiters", func() {
			tryParsePath("msg..str").andExpectError(fmt.Errorf("unknown token: ."))
			tryParsePath("msg..").andExpectError(fmt.Errorf("unknown token: ."))
		})
...
// Helper struct to enable fluent descriptive test for path parsing
