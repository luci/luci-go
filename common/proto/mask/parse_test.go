...
func TestParsePath(t *testing.T) {
	Convey("Expect path parsing", t, func() {
		...
		Convey("fails when given subfield for a scalar field", func() {
			tryParsePath("str.str").andExpectError(fmt.Errorf("scalar field cannot have subfield: \"str\""))
		})
		Convey("fails for invalid delimiter", func() {
			tryParsePath("str@").andExpectError(fmt.Errorf("expected delimiter: .; got @"))
		})
		...
		Convey("fails when parsing repeated field and path ends with index", func() {
			tryParsePath("strs.1").andExpectError(fmt.Errorf("expected a star following a repeated field; got token: \"1\""))
			tryParsePath("nums.2").andExpectError(fmt.Errorf("expected a star following a repeated field; got token: \"2\""))
			tryParsePath("msgs.a").andExpectError(fmt.Errorf("expected a star following a repeated field; got token: \"a\""))
		})
		...
		Convey("fails when parsing map field with incompatible key type", func() {
			tryParsePath("map_num_str.a").andExpectError(fmt.Errorf("expected map key kind int32; got token: \"a\""))
			tryParsePath("map_str_num.1").andExpectError(fmt.Errorf("expected map key kind string; got token: \"1\""))
			tryParsePath("map_bool_str.not_a_bool").andExpectError(fmt.Errorf("expected map key kind bool; got token: \"not_a_bool\""))
		})
		...
		Convey("fails when parsing message field and given a non-string field name", func() {
			tryParsePath("msg.123").andExpectError(fmt.Errorf("expected a field name of type string; got token: \"123\""))
		})
		Convey("fails when parsing message field and star is not the last token", func() {
			tryParsePath("msg.*.str").andExpectError(fmt.Errorf("expected end of string; got token: \"str\""))
		})
		Convey("fails when parsing message field with unknown subfield", func() {
			tryParsePath("msg.unknown_field").andExpectError(fmt.Errorf("field \"unknown_field\" does not exist in message %s", testMsgDescriptor.Name()))
			tryParsePath("msg.msg.unknown_field").andExpectError(fmt.Errorf("field \"unknown_field\" does not exist in message %s", testMsgDescriptor.Name()))
		})
		...
		Convey("fails when ends delimiter", func() {
			tryParsePath("msg.").andExpectError(fmt.Errorf("path can't end with delimiter: ."))
		})
		Convey("fails when quoted string is not closed", func() {
			tryParsePath("`quoted``str").andExpectError(fmt.Errorf("a quoted string is never closed; got: \"quoted`str\""))
		})
		Convey("fails when an integer literal has only minus sign", func() {
			tryParsePath("map_num_str.-").andExpectError(fmt.Errorf("expected digit following minus sign for negative numbers; got minus sign only"))
		})
		Convey("fails when multiple delimiters", func() {
			tryParsePath("msg..str").andExpectError(fmt.Errorf("unexpected token: ."))
			tryParsePath("msg..").andExpectError(fmt.Errorf("unexpected token: ."))
		})
...
func (res parseResult) andExpectError(expected error) {
	So(res.err, ShouldBeError, expected)
}
