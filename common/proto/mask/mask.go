...
func (m Mask) mergeImpl(src, dest protoreflect.Message) {
	for seg, submask := range m.children {
		...
		case fieldDesc.IsList():
			...
			srcList, newDestList := srcVal.List(), newField.List()
			for i := 0; i < srcList.Len(); i++ {
				newDestList.Append(cloneValue(srcList.Get(i), kind))
			}
...
// input value for all the other kinds (i.e. scalar). List and map value is not
