...
func (m Mask) trimImpl(reflectMsg protoreflect.Message) {
	reflectMsg.Range(func(fieldDesc protoreflect.FieldDescriptor, fieldVal protoreflect.Value) bool {
		...
		switch incl, _ := m.includesImpl(path{fieldName}, 0); incl {
		case Exclude:
...
func (m Mask) trimMap(protoMap protoreflect.Map, valueKind protoreflect.Kind) {
	protoMap.Range(func(k protoreflect.MapKey, v protoreflect.Value) bool {
		keyString := k.String()
		switch incl, _ := m.includesImpl(path{keyString}, 0); incl {
		case Exclude:
			protoMap.Clear(k)
		case IncludePartially:
			if valueKind != protoreflect.MessageKind {
				...
				// partially include the key because of star child. So, check
				// for both key child and star child.
...
func (m Mask) Includes(path string) (Inclusiveness, error) {
	...
	incl, _ := m.includesImpl(parsedPath, 0)
	return incl, nil
}
...
func (m Mask) includesImpl(p path, startAt int) (Inclusiveness, Mask) {
	if len(m.children) == 0 {
	...
	for _, seg := range []string{p[startAt], "*"} {
		if child, ok := m.children[seg]; ok {
			if cIncl, cInclMask := child.includesImpl(p, startAt+1); cIncl > incl {
...
func (m Mask) mergeImpl(src, dest protoreflect.Message) {
	for seg, submask := range m.children {
		// Star field is not possible for update mask so this won't be nil
		...
		case fieldDesc.IsList():
			...
			for i, srcList, newList := 0, srcVal.List(), newField.List(); i < srcList.Len(); i++ {
				newList.Append(cloneValue(srcList.Get(i), kind))
			}
...
// cloneValue returns a cloned value if value kind is message and returns
...
func (m Mask) Submask(path string) (Mask, error) {
	...
	switch incl, inclMask := m.includesImpl(parsedPath, 0); incl {
