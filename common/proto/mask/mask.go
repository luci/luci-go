...
func (m Mask) trimImpl(reflectMsg protoreflect.Message) {
	reflectMsg.Range(func(fieldDesc protoreflect.FieldDescriptor, fieldVal protoreflect.Value) bool {
		...
		case IncludePartially:
			// Child for this field must exist because the path is PartiallyInclude.
			...
			case fieldDesc.Kind() != protoreflect.MessageKind:
				...
				// it entirely. Skip it because scalars do not have subfields. Note
				// that FromFieldMask would fail on such a mask because a scalar
				// field cannot be followed by other fields.
				reflectMsg.Clear(fieldDesc)
			default:
