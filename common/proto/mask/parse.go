...
func (t *tokenizer) nextToken() (token, error) {
	...
	case r == '`':
		...
		for {
			nextBacktickRel := strings.IndexRune(string(t.path[t.pos:]), '`')
			if nextBacktickRel == -1 {
				sb.WriteString(string(t.path[t.pos:]))
				return token{}, fmt.Errorf("a quoted string is never closed; got: %q", sb.String())
			}
			nextBacktickAbs := t.pos + nextBacktickRel
			sb.WriteString(string(t.path[t.pos:nextBacktickAbs]))
	...
	case r == '-' || unicode.IsDigit(r):
		t.pos++ // swallow the minus sign or first digit
		n := readWhile(t.path, unicode.IsDigit, t.pos)
		if r == '-' && n == 0 {
		...
		return token{
			typ:   intLiteral,
			value: string(t.path[t.pos-n-1 : t.pos]),
		}, nil

	case r == '_' || unicode.IsLetter(r):
		t.pos++ // swallow the underscore or first letter
		n := readWhile(t.path, stringPredicate, t.pos)
		t.pos += n
		val := string(t.path[t.pos-n-1 : t.pos])
		typ := strLiteral
	...
	default:
		return token{}, fmt.Errorf("unexpected token: %c", r)
	}
}

// stringPredicate returns true if the given unicode code point matches
// the string literal grammar.
func stringPredicate(r rune) bool {
	return unicode.IsLetter(r) || unicode.IsDigit(r) || r == '_'
}

// readWhile reads a slice of unicode code points starting from the given
// position and stops when the provided condition is not matched or end of
// slice is reached. Returns the number of code points read. 0 will be
// returned if the starting position is out of bounds.
func readWhile(rs []rune, cond func(r rune) bool, start int) int {
