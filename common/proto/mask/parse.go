...
func (t *tokenizer) nextToken() (token, error) {
	...
	case b == '`':
		...
		sb := &strings.Builder{}
		for {
			nextBacktickRel := strings.IndexByte(t.path[t.pos:], '`')
	...
	case b == '-' || isDigit(b):
		...
		n := readWhile(t.path[t.pos:], isDigit)
		if b == '-' && n == 0 {
	...
	case b == '_' || isLetter(b):
		...
		n := readWhile(t.path[t.pos:], isValidStringChar)
		t.pos += n
...
// isDigit tells whether the given byte represents a digit, i.e. [0-9].
func isDigit(b byte) bool {
	return b >= '0' && b <= '9'
}

// isLetter tells whether the given byte represents a letter, i.e. [a-zA-Z].
func isLetter(b byte) bool {
	return (b >= 'a' && b <= 'z') || (b >= 'A' && b <= 'Z')
}

// isValidStringChar tells whether the given byte represents a valid
// character in a string literal according to the grammar.
func isValidStringChar(b byte) bool {
	return isLetter(b) || isDigit(b) || b == '_'
}

// readWhile reads the given string from the beginning and stops when the
// provided condition is not matched or end of string is reached. Returns
// the number of bytes read.
func readWhile(s string, cond func(b byte) bool) int {
	for i := 0; i < len(s); i++ {
		if !cond(s[i]) {
			return i
		}
