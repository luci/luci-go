...
func (p *userAuthTokenProvider) RefreshToken(ctx context.Context, prev, base *Token) (*Token, error) {
	...
	case err == nil:
		...
		// "upgrade" existing cached tokens to include email. Note that if the token
		// is not associated with an email at all, we'll have prev.Email == NoEmail,
		// which will skip grabEmail call below.
		email := prev.Email
		if email == "" {
			var err error
