...
type Server struct {
	// Name identifies this server in logs.
	Name string

	// Port is a local TCP port to bind to or 0 to allow the OS to pick one.
	Port int

	// Serve is called from internal goroutine to run the server loop.
	//
	...
	//
	// If 'Serve' returns after the listener is closed, the returned error is
	// ignored (it is most likely caused by the closed listener).
	Serve func(c context.Context, l net.Listener, wg *sync.WaitGroup) error
...
func (s *Server) serve() error {
	...
	err := s.Serve(ctx, listener, &s.wg) // blocks until killServe() is called
