...
func (s *Server) Start(ctx context.Context) (*lucictx.LocalAuth, error) {
	...
	s.srv.Name = "local_auth"
	s.srv.Port = s.Port
	s.srv.Serve = func(c context.Context, l net.Listener, wg *sync.WaitGroup) error {
		return s.serve(c, l, wg, la)
	}

	addr, err := s.srv.Start(ctx)
	if err != nil {
...
func (s *Server) serve(ctx context.Context, l net.Listener, wg *sync.WaitGroup, la *lucictx.LocalAuth) error {
