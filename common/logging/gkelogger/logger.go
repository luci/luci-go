...
type LogEntry struct {
	...
	// OperationID is used to group log lines from a single request together.
	OperationID string `json:"logging.googleapis.com/operation,omitempty"`
...
// Sink using the LogEntry (if any) as a prototype for log entries.
//
...
// Sink. This allows to "prepopulate" fields like TraceID or OperationID in all
// log entries emitted by logging.Logger.
//
// Such factory can be installed in the context via logging.SetFactory.
func Factory(sink *Sink, prototype LogEntry) func(context.Context) logging.Logger {
...
func (l *jsonLogger) LogCall(lv logging.Level, calldepth int, format string, args []interface{}) {
	ts := clock.Now(l.ctx).UnixNano()

	e := l.prototype
	e.Severity = lv.String()
	e.Message = fmt.Sprintf(format, args...)
	e.Time = fmt.Sprintf("%d.%d", ts/1e9, ts%1e9)
