...
func StartTrace(c context.Context, request *http.Request) context.Context {
	return SetTraceID(c, request, randString(c, TraceSize))
}

// SetTrace is a convenience function to set a traceID in the logging context.
func SetTraceID(c context.Context, request *http.Request, traceID string) context.Context {
	req := &Request{
...
func EndTrace(c context.Context, response *http.Response) {
	...
	if response.StatusCode >= 200 && response.StatusCode <= 299 {
		logging.Infof(c, "Request completed OK")
	} else {
		logging.Errorf(c, "Request completed with error")
	}
}
