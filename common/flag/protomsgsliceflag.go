...
type messageSliceFlag struct {
	// msgSliceVal is a slice of pointer to concrete proto message struct.
...
func MessageSliceFlag(msgSlicePtr interface{}) flag.Getter {
	...
	if !sliceElemType.Implements(protoMsgBaseType) {
		panic(fmt.Sprintf("Expect element of the slice to implement interface: %s"+
			". Got type: %s", protoMsgBaseType, sliceElemType))
	}
...
// String returns all messages serailzed in JSON and seprated by a new line.
