...
func getCommandEnv(ctx context.Context, tmpDir string, cipdInfo *cipdInfo, runDir string, env environ.Env, envPrefixes map[string][]string, outDir, botFile string) (environ.Env, error) {
	...
	if cipdInfo != nil {
		binDirs := make([]string, 0, len(cipdInfo.binaryPaths)+1)
		for _, binpath := range cipdInfo.binaryPaths {
			binDirs = append(binDirs, filepath.Dir(binpath))
		}
		binDirs = append(binDirs, out.GetEmpty("PATH"))
		out.Set("PATH", strings.Join(binDirs, string(filepath.ListSeparator)))
