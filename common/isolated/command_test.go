...
func TestRunCommand(t *testing.T) {
	...
	Convey("TestRunCommand", t, func() {
		...
		Convey("timeout", func() {
			exitCode, timeout, err := runCommand(ctx, []string{"sleep", "30000"}, ".", environ.Env{}, 0, time.Minute)
		...
		Convey("context timeout", func() {
			...
			defer cancel()
			exitCode, timeout, err := runCommand(ctx, []string{"sleep", "30000"}, ".", environ.Env{}, time.Minute, time.Minute)
