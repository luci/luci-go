...
// returnBuf is called when the buffer is no longer needed to put it for reuse.
...
func (r *bufferingReaderAt) readBlock(offset int64) (data []byte, err error) {
	...
	// pool and them immediately grab this buffer.
	r.lru.prepareForAdd()
	data = r.grabBuf()
...
func (r *bufferingReaderAt) ReadAt(p []byte, offset int64) (read int, err error) {
	...
	for blockOff := min; blockOff < max; blockOff += bs {
		...
		if err != nil {
			if read == len(p) {
				// We managed to read everything we wanted, so the error is irrelevant.
				return read, nil
			}
			return read, err
		}
