...
type blocksLRU struct {
	max     int         // how many blocks we want to cache
...
func (l *blocksLRU) init() {
	l.blocks = map[int64]*list.Element{}
	l.ll.Init()
}
...
//
// Does nothing if its not yet full.
...
type bufferingReaderAt struct {
	...
	pool [][]byte // available buffers of blockSize len
}
...
// significantly (100x), in exchange for modest amount of RAM.
//
// This reader is safe to user concurrently (just like any ReaderAt), but beware
...
func NewBufferingReaderAt(r io.ReaderAt, blockSize int, lruSize int) io.ReaderAt {
	reader := &bufferingReaderAt{
		r:         r,
		blockSize: blockSize,
		lru:       blocksLRU{max: lruSize},
	}
	reader.lru.evicted = func(b block) { reader.returnBuf(b.data) }
	reader.lru.init()
	return reader
}
...
// returnBuf is called when the buffer is not longer needed.
func (r *bufferingReaderAt) returnBuf(b []byte) {
	b = b[:cap(b)]
...
func (r *bufferingReaderAt) readBlock(offset int64) (data []byte, err error) {
	...
	// Kick out the oldest block (if any) to move buffer to the free buffers pool
	// and grab the buffer from the pool.
	r.lru.prepareForAdd()
	data = r.grabBuf()
...
func (r *bufferingReaderAt) ReadAt(p []byte, offset int64) (read int, err error) {
	...
	min := int64((offset / bs) * bs) // block-aligned offset
	max := offset + int64(len(p))    // how far we need to read

	// Read all blocks that intersect with the requested segment.
	for blockOff := min; blockOff < max; blockOff += bs {
		...
		// 'offset'.
		if offset > blockOff {
			p := offset - blockOff // position inside the block to read from
			if p < int64(len(data)) {
				data = data[p:] // grab the tail of the block
			} else {
				data = nil // we probably hit EOF before than the requested offset
			}
		}

		// Copy copies min of len(data) and whatever space is left in 'p', so this
		// is always safe. The last block may be copied from partially.
		read += copy(p[read:], data)

		// Finish reading on errors (including EOF).
		if err != nil {
			if read == len(p) {
				// We managed to read all we wanted, so the error is irrelevant.
				return read, nil
			}
			return read, err
		}
