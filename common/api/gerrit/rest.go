...
func (c *client) ChangeEditFileContent(ctx context.Context, req *gerritpb.ChangeEditFileContentRequest, opts ...grpc.CallOption) (*empty.Empty, error) {
	path := fmt.Sprintf("/changes/%s/edit/%s", uniqueGerritID(req.Project, req.Number), req.FilePath)
...
func (c *client) ChangeEditPublish(ctx context.Context, req *gerritpb.ChangeEditPublishRequest, opts ...grpc.CallOption) (*empty.Empty, error) {
	path := fmt.Sprintf("/changes/%s/edit:publish", uniqueGerritID(req.Project, req.Number))
...
func (c *client) SubmitChange(ctx context.Context, req *gerritpb.SubmitChangeRequest, opts ...grpc.CallOption) (*gerritpb.ChangeInfo, error) {
	var resp changeInfo
	path := fmt.Sprintf("/changes/%s/submit", uniqueGerritID(req.Project, req.Number))
	var data struct{}
...
func (c *client) callRaw(ctx context.Context, method, urlPath string, params url.Values, data []byte, expectedHTTPCodes ...int) (int, []byte, error) {
	...
	logging.Debugf(ctx, "gerrit API call: %+v", req)
...
func branchToRef(branch string) string {
	return fmt.Sprintf("%s%s", refsPrefix, branch)
}

func refToBranch(ref string) (string, error) {
	if !strings.HasPrefix(ref, refsPrefix) {
		return "", fmt.Errorf("%s is not a branch ref", ref)
	}
	return strings.TrimPrefix(ref, refsPrefix), nil
}

func uniqueGerritID(project string, number int64) string {
