...
func (c *client) GetChange(ctx context.Context, req *gerritpb.GetChangeRequest, opts ...grpc.CallOption) (
	*gerritpb.ChangeInfo, error) {
	...
	path := fmt.Sprintf("/changes/%s", url.QueryEscape(req.Id))
	if _, err := c.call(ctx, "GET", path, nil, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
...
func (c *client) CheckAccess(ctx context.Context, req *gerritpb.CheckAccessRequest, opts ...grpc.CallOption) (
	*gerritpb.CheckAccessResponse, error) {
	...
	if _, err := c.call(ctx, "POST", path, &input, &resp); err != nil {
		return nil, err
	}
...
// will be -1.
func (c *client) call(ctx context.Context, method, urlPath string, data, dest interface{}, expectedHTTPCodes ...int) (int, error) {
	...
	req, err := http.NewRequest(method, c.BaseURL+urlPath, &buffer)
	if err != nil {
