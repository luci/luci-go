	...
	if recipients := in.NotifyDetails.GetRecipients(); len(recipients) > 0 {
		...
		for _, recipient := range recipients {
			...
			if ni, ok := data.NotifyDetails[rts]; !ok {
				ni = &notifyInfo{
					Accounts: make([]int64, len(recipient.NotifyInfo.GetAccounts())),
				}
				for i, aid := range recipient.NotifyInfo.GetAccounts() {
					ni.Accounts[i] = aid
				}
				data.NotifyDetails[rts] = ni
			} else {
				ni.Accounts = append(ni.Accounts, recipient.NotifyInfo.GetAccounts()...)
			}
		}
