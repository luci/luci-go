...
// client implements gerritpb.GitilesClient.
type client struct {
	Client *http.Client
...
func (c *client) CheckAccess(ctx context.Context, req *gerritpb.CheckAccessRequest, opts ...grpc.CallOption) (
	*gerritpb.CheckAccessResponse, error) {
	...
	}{req.Account, req.Ref, req.Permission}
	var resp struct {
	...
	default:
		logging.Errorf(ctx, "gerrit: unexpected AccessCheckInfo.Status %d response", resp.Status)
		return nil, status.Errorf(codes.Internal, "unexpected AccessCheckInfo.Status %d response", resp.Status)
	}
	return res, nil
}

// post executs POST request to Gerrit REST API with JSON input/output.
//
// post returns HTTP status code and error. If error happened before HTTP status
// code was determined, HTTP status code will be -1.
func (c *client) post(ctx context.Context, urlPath string, data interface{}, dest interface{}, expectedHTTPCodes ...int) (int, error) {
	...
	if err != nil {
		return r.StatusCode, errors.Annotate(err, "could not read response body").Err()
	}
	...
	for _, s := range expectedHTTPCodes {
		if r.StatusCode == s {
			...
			return r.StatusCode, json.Unmarshal(body, dest)
		}
	}
