...
func (c *client) post(ctx context.Context, urlPath string, data, dest interface{}, expectedHTTPCodes ...int) (int, error) {
	...
	if err := json.NewEncoder(&buffer).Encode(data); err != nil {
		return -1, status.Errorf(codes.Internal, "failed to serialize request message: %s", err.Error())
	}
	r, err := ctxhttp.Post(ctx, c.Client, c.BaseURL+urlPath, contentType, &buffer)
	if err != nil {
		return -1, status.Errorf(codes.Internal, "failed to execute Post HTTP request: %s", err.Error())
	}
	defer r.Body.Close()

	body, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return r.StatusCode, status.Errorf(codes.Internal, "failed to read response: %s", err.Error())
	}
	...
	for _, s := range expectedHTTPCodes {
		if r.StatusCode == s {
			...
			if err = json.Unmarshal(body, dest); err != nil {
				return r.StatusCode, status.Errorf(codes.Internal, "failed to desirealize response: %s", err.Error())
			}
