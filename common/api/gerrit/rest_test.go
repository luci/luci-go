...
func TestRestSetReview(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	...
	Convey("Set Review", t, func() {
		...
		actualBody := reviewInput{}
		So(json.Unmarshal(actualRawBody, &actualBody), ShouldBeNil)
		So(actualBody, ShouldResemble, reviewInput{
			Message: "This is a message",
			Labels: map[string]int32{
				"Code-Review": -1,
			},
			Tag:    "autogenerated:cq",
			Notify: "OWNER",
			NotifyDetails: map[string]*notifyInfo{
				"TO":  {Accounts: []int64{1, 2, 3, 4, 5}},
				"BCC": {Accounts: []int64{1, 6}},
			},
			OnBehalfOf: 10001,
		})
...
func TestGerritError(t *testing.T) {
	...
	Convey("Gerrit returns", t, func() {
		// All APIs share the same error handling code path, so use GetChange as
		// an example
