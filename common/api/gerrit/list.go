...
func (p *listChangesPager) fetch(ctx context.Context) (*gerritpb.ListChangesResponse, error) {
	switch {
	case p.opts.Limit <= 0:
	...
	changes, more, err := p.fetchInner(ctx)
...
func (p *listChangesPager) fetchInner(ctx context.Context) ([]*gerritpb.ChangeInfo, bool, error) {
	...
	// RPCs. This is take care of at the end.
	...
	for len(changes) <= p.opts.Limit && more {
		...
		case more && len(newChanges) == len(changes):
			// No progress, likely because there >opts.PageSize Gerrit changes with
			// the same .updated timestamp.
...
// verifyCanonicalOrder returns possibly modified slice of changes in
// canonical order.
//
// Expects to receive already canonical order.
// If not, warns to log and fixes the order.
