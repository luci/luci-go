...
func TestQuery(t *testing.T) {
	...
	Convey("Query with bad URL", t, func() {
		srv, c := newMockClient(func(w http.ResponseWriter, r *http.Request) {})
		defer srv.Close()
		_, _, err := c.Query(ctx, "bad://url/", QueryRequest{Query: "mockhash"})
		So(err, ShouldNotBeNil)
	})
	...
	Convey("Query", t, func() {
		...
		Convey("Basic", func() {
			cls, more, err := c.Query(ctx, "https://c.googlesource.com/",
				QueryRequest{
					Query: "some_query",
				})
			So(err, ShouldBeNil)
...
func newMockClient(handler func(w http.ResponseWriter, r *http.Request)) (*httptest.Server, *Client) {
	...
	return srv, &Client{Client: http.DefaultClient, mockGerritURL: srv.URL}
}
