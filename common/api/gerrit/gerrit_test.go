...
func TestNewProdClient(t *testing.T) {
	t.Parallel()
	Convey("Malformed", t, func() {
		f := func(arg string) {
			_, err := NewProdClient(http.DefaultClient, arg)
			So(err, ShouldNotBeNil)
		}
		f("badurl")
...
func TestGetDetails(t *testing.T) {
	...
	Convey("Details", t, func() {
		...
		Convey("WithOptions", func() {
			cl, err := c.GetDetails(ctx, "629279", []string{"CURRENT_REVISION"})
			So(err, ShouldBeNil)
...
func newMockClient(handler func(w http.ResponseWriter, r *http.Request)) (*httptest.Server, *Client) {
	...
	// Note that we use ProdClient instead of implementing the Client
	// interface in a custom mock client struct because we have access to the
	// private member gerritURL and can thus bypass the checks made by
	// NewProdClient.
	c := &Client{ProdClient{http.DefaultClient, *pu}}
	return srv, c
}
