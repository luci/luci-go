...
// to reach gerrit.
//
// For testing purposes the insecure flag allows to bypass protocol and host
// domain checks.
func NewClient(c *http.Client, gerritURL string, insecure bool) (*Client, error) {
	u, err := NormalizeGerritURL(gerritURL, insecure)
	if err != nil {
		return nil, err
	}
	pu, err := url.Parse(u)
	...
	return &Client{c, *pu, insecure}, nil
}
...
func (c *Client) get(ctx context.Context, path string, query url.Values, result interface{}) error {
	...
	defer r.Body.Close()
	if r.StatusCode != 200 {
		err = errors.Reason("failed to fetch %s, status code %d", u.String(), r.StatusCode).Err()
		if r.StatusCode >= 500 {
