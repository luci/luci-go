...
//
// RefsSet groups refs by prefix and issues 1 refs RPC per prefix. This is more
...
type RefSet interface {
	...
	Resolve(ctx context.Context, client gitiles.GitilesClient, project string) (map[string]string, error)
}

// NewRefSet creates an instance of the RefSet from a collection of ref specs.
//
// Refs spec can be either
//   * a fully-qualified ref with at least 2 slashes, e.g. `refs/heads/master`,
//     `refs/tags/v1.2.3`, or
//   * a regular expression with "regexp:" prefix to match multipl refs, e.g.
...
//
// See also ValidateRefSpecs function.
func NewRefSet(refSpecs []string) RefSet {
	w := refSet{map[string]*refSetPrefix{}}
	nsRegexps := map[string][]string{}
	for _, refSpec := range refSpecs {
		prefix, suffix, regexp := parseRefSpec(refSpec)
	...
	for prefix, regexps := range nsRegexps {
		var err error
		w.byPrefix[prefix].descendantRegexp, err = regexp.Compile(
...
// ValidateRefSpecs validates passed set of ref specs.
//
// It ensures that passed strings match the requirements for the ref specs as
// described in the documentation for the NewRefSet function.
func ValidateRefSpecs(c *validation.Context, refSpecs []string) {
	for _, ref := range refSpecs {
		...
		if strings.Count(ref, "/") < 2 {
			c.Errorf(`fewer than 2 slashes in a ref %s`, ref)
		}
	}
}
...
// Resolve returns map from watched ref to its resolved tip.
//
// Refs which don't exist or are not visible to requester, won't be included
// the in the map.
func (w *refSet) Resolve(c context.Context, client gitiles.GitilesClient, project string) (refTips map[string]string, err error) {
		...
		for prefix := range w.byPrefix {
			...
			work <- func() error {
				resp, err := client.Refs(
					c, &gitiles.RefsRequest{Project: project, RefsPath: prefix})
				if err != nil {
					return err
				}
				lock.Lock()
...
func parseRefSpec(refSpec string) (prefix, literalSuffix, regexpSuffix string) {
	if strings.HasPrefix(refSpec, "regexp:") {
		refRegexp := strings.TrimPrefix(refSpec, "regexp:")
	...
	lastSlash := strings.LastIndex(refSpec, "/")
	prefix, literalSuffix = refSpec[:lastSlash], refSpec[lastSlash+1:]
