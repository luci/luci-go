...
func TestUserProto(t *testing.T) {
	...
	Convey(`Test User.Proto`, t, func() {
		u := &User{
			...
			Time:  time.Date(2016, 3, 9, 3, 46, 18, 0, time.UTC),
		}
...
func TestCommitProto(t *testing.T) {
	...
	Convey(`Test Commit.Proto`, t, func() {
		c := &Commit{
			...
			Author:    User{"author", "author@example.com", time.Date(2016, 3, 9, 3, 46, 18, 0, time.UTC)},
			Committer: User{"committer", "committer@example.com", time.Date(2016, 3, 9, 3, 46, 18, 0, time.UTC)},
			Message:   "I am\na\nbanana",
		}
