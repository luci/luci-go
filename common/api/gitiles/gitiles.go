...
//
// If authenticated is true, the returned URL has "a/" path prefix.
func NormalizeRepoURL(repoURL string, authenticated bool) (string, error) {
...
type Client struct {
	Client *http.Client
	// Auth, if true, indicates that HTTP requests must include includes
	// /a/ URL path prefix.
	// Client.Transport must include access token.
	Auth bool
