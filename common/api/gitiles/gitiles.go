...
func (c *Client) rawLog(ctx context.Context, repoURL, treeish string, lowerLimit int, nextCursor string) (*logResponse, error) {
	...
	combinedLog := []Commit{}
	nextPath := subPath + fmt.Sprintf("&n=%d", pageSize)
	for {
...
// paging back until it reaches r1 and will return the last page in
// reverse order (r1, r2, r3, r4, ..., r~PageSize).
//
// This function will call rawLog as many times as necessary until it reaches
// the specified revision. It will then return at least one full page worth
// of commits if the log contains multiple pages. To customize the size of the
