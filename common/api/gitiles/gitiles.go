...
func (c *Client) rawLog(ctx context.Context, repoURL, treeish string, lowerLimit int, nextCursor string) (*logResponse, error) {
	...
	if lowerLimit < 1 {
		return nil, fmt.Errorf("limit must be at least 1, but %d provided", lowerLimit)
	}
	...
	resp := &logResponse{}
	for {
		// Clear .Next each time because c.get will not do it for us.
		resp.Next = ""
		if nextCursor != "" {
...
//
// The response for rawLog will always include (for r0..rx) the immediate
// ancestors of rx (rx-1, rx-2, ..., rx-n) but may not reach r1 necessarily
// if the delta between the commits reachable from rx and those
// reachable from r0 is larger than the a given limit; LogForward will keep
// paging back until it reaches r1 and will return the last page or two in
// reverse order (r1, r2, r3, r4, ..., r~PageSize).
//
...
//
// WARNING: This api pages back from the rx until r0 (or any commit reachable
// from r0), and it makes a request to gitiles for each page. Using too small a
// page size will make this api generate an *excessive* amount of requests.
