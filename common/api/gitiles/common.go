...
func ParseRepoURL(repoURL string) (host, project string, err error) {
	...
	case strings.Contains(u.Path, "+"):
		err = errors.New("repo URL path cannot contain +")
	case !strings.HasPrefix(u.Path, "/"):
		err = errors.New("repo URL path must start with slash")
	case u.Path == "/":
		err = errors.New("repo URL path must be not empty and not just slash")
	case u.RawQuery != "":
		err = errors.New("repo URL cannot contain query")
	case u.Fragment != "":
		err = errors.New("repo URL cannot contain fragment")
	default:
		host = u.Host
