...
func TestRefs(t *testing.T) {
	...
	Convey("WatchedRefs work", t, func() {
		wr := NewWatchedRefs([]string{
			`refs/heads/master`,
			`regexp:refs/branch-heads/\d+\.\d+`,
		})

		Convey("explicit refs work", func() {
			So(wr.HasRef("refs/heads/master"), ShouldBeTrue)
			So(wr.HasRef("refs/heads/foo"), ShouldBeFalse)
		})

		Convey("regexp refs work", func() {
			So(wr.HasRef("refs/branch-heads/1.12"), ShouldBeTrue)
			So(wr.HasRef("refs/branch-heads/1.12.123"), ShouldBeFalse)
		})
