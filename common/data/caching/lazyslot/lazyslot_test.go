...
func TestLazySlot(t *testing.T) {
	Convey("Blocking mode works", t, func() {
		...
		fetcher := func(c context.Context, prev interface{}) (interface{}, time.Duration, error) {
			lock.Lock()
			defer lock.Unlock()
			counter++
	...
	Convey("Initial failed fetch causes errors", t, func() {
		...
		_, err := s.Get(c, func(c context.Context, prev interface{}) (interface{}, time.Duration, error) {
			return nil, 0, failErr
		})
		So(err, ShouldEqual, failErr)

		// Subsequence successful fetch.
		val, err := s.Get(c, func(c context.Context, prev interface{}) (interface{}, time.Duration, error) {
			return 1, 0, nil
		})
		So(err, ShouldBeNil)
	...
	Convey("Returns stale copy while fetching", t, func(conv C) {
		...
		// Put initial value.
		s := Slot{}
		v, err := s.Get(c, func(c context.Context, prev interface{}) (interface{}, time.Duration, error) {
	...
	Convey("Recovers from panic", t, func() {
		...
		clk.Add(5 * time.Second)
		So(func() {
			s.Get(c, func(c context.Context, prev interface{}) (interface{}, time.Duration, error) {
				panic("omg")
			})
		}, ShouldPanicWith, "omg")
