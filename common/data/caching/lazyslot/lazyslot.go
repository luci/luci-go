...
//
// It receives the previously known value when refreshing it. If the returned
// expiration  duration is zero, the returned value never expires.
...
func (s *Slot) Get(c context.Context, fetcher Fetcher) (value interface{}, err error) {
	...
	value, exp, err = fetcher(c, prevValue)
...
// fetched value.
//
// 'completed' is false if the fetch paniced.
