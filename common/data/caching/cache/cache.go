...
func (d *disk) Hardlink(digest isolated.HexDigest, dest string, perm os.FileMode) error {
	...
	if err := os.Link(src, dest); err != nil {
		debugInfo := fmt.Sprintf("Stats:\n*  src: %s\n*  dest: %s\n*  destDir: %s\n%s\ndigestHardlinkRecords: %s", statsStr(src), statsStr(dest), statsStr(filepath.Dir(dest)), uidGidStr(), d.hardlinkRecordsToStr())
...
func uidGidStr() string {
	uid := os.Getuid()
	gid := os.Getgid()
	var s string
	if uid == -1 || gid == -1 {
		s = "UID/GUI unavailable"
	} else {
		s = fmt.Sprintf("UID=%d GID=%d", uid, gid)
	}
	return s
}
