...
func (p *Policies) isUnbounded(s units.Size) bool {
...
func (d *Cache) add(digest isolated.HexDigest, src io.Reader, cb func() error) error {
	...
	if !d.policies.isUnbounded(units.Size(size)) {
		_ = os.Remove(fname)
...
func (d *Cache) respectPolicies() error {
	...
	for {
		...
		if d.lru.length() <= d.policies.MaxItems && d.policies.isUnbounded(d.lru.sum) && freeSpace >= minFreeSpaceWanted {
			break
		}
		if d.lru.length() == 0 {
