...
func (d *Cache) add(digest isolated.HexDigest, src io.Reader, cb func() error) error {
	...
	if d.policies.MaxSize != 0 && units.Size(size) > d.policies.MaxSize {
		_ = os.Remove(fname)
...
func (d *Cache) respectPolicies() error {
	...
	for {
		...
		log.Println(d.lru.length(), " ", d.policies.MaxItems, " ", d.lru.sum)

		if d.lru.length() <= d.policies.MaxItems && (d.policies.MaxSize == 0 || d.lru.sum <= d.policies.MaxSize) && freeSpace >= minFreeSpaceWanted {
			break
		}
		if d.lru.length() == 0 {
