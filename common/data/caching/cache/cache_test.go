...
func hash(i int) string {
	return hex.EncodeToString(crypto.SHA1.New().Sum([]byte(strconv.Itoa(i))))
}
...
func BenchmarkDisk(b *testing.B) {
	...
	for _, n := range []int{16, 256, 65536, 65536 * 4} {
		...
		b.Run(fmt.Sprintf("with %d files", n), func(b *testing.B) {
			...
			for j := 0; j < n; j++ {
				if err := ioutil.WriteFile(filepath.Join(src, hashes[j]), nil, 0600); err != nil {
			...
			for i := 0; i < b.N; i++ {
				...
				for j := 0; j < n; j++ {
					h := hashes[j]
					if err := os.Link(filepath.Join(src, h), filepath.Join(cache, h)); err != nil {
						b.Errorf("failed to write file: %v", err)
					}
				}
