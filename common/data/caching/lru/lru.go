...
const (
	// Never is a sentinel Duration indicating that a cache entry does not have
	// an expiration time.
...
// expiration time. If there is no current value, the expiration will be <0
// (Remove).
//
...
// (Never), the returned Item will never expire. Otherwise, the Item will
...
// evicted.
//
// Cache isn't inherently safe for concurrent access. In order to ensure that
// this is the case, it should be configured with an appropriate Locker
// instance.
//
...
type Cache struct {
	...
	// initOnce is used to ensure that the Cache is initialized at most once.
	initOnce sync.Once
...
// initializse must be called prior to any mutation.
func (c *Cache) initialize() { c.initOnce.Do(c.Reset) }
...
func (c *Cache) Peek(ctx context.Context, key interface{}) (cur interface{}, has bool) {
	now := clock.Now(ctx)
	c.withRLock(func() {
...
func (c *Cache) Get(ctx context.Context, key interface{}) (ret interface{}, has bool) {
	c.initialize()
...
//
// If the supplied expiration is <0 (Remove), Put will function as a Remove
// operation.
//
...
func (c *Cache) Mutate(ctx context.Context, key interface{}, gen Generator) (value interface{}, ok bool) {
	...
	c.withLock(func() {
		...
		if ent == nil {
			ent = &cacheEntry{key, value, time.Time{}}
		} else {
			ent.v = value
		}
...
func (c *Cache) withRLock(fn func()) {
	if c.Locker != nil {
		c.Locker.RLock()
