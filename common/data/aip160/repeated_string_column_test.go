...
func TestRepeatedStringColumn(t *testing.T) {
	ftt.Run("RepeatedStringColumn", t, func(t *ftt.Test) {
		table := NewDatabaseTable().WithFields(
			NewField().WithFieldPath("array").WithBackend(NewRepeatedStringColumn("db_array")).Filterable().Build(),
		).Build()
