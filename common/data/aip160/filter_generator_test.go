	...
	ftt.Run("WhereClause", t, func(t *ftt.Test) {
		...
		t.Run("Simple filter", func(t *ftt.Test) {
			...
			t.Run("string array key value contains operator", func(t *ftt.Test) {
				...
				assert.Loosely(t, result, should.Equal("(EXISTS (SELECT 1 FROM UNNEST(db_sakv) as v WHERE v LIKE @p_0))"))
			})
			t.Run("string array key value equal operator", func(t *ftt.Test) {
				...
				assert.Loosely(t, result, should.Equal("(@p_0 IN UNNEST(db_sakv))"))
			})
			t.Run("string array key value not equal operator", func(t *ftt.Test) {
				...
				assert.Loosely(t, result, should.Equal("(EXISTS (SELECT 1 FROM UNNEST(db_sakv) as v WHERE STARTS_WITH(v, @p_1) AND NOT v = @p_0))"))
			})
