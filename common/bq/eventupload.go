...
// as opposed to one fieldinfo per oneof memeber.
...
func mapFromMessage(m proto.Message, path []string) (map[string]bigquery.Value, error) {
	...
	for _, fi := range infos {
		...
		case len(fi.oneOfFields) != 0:
			...
			bqValue, err = getValue(rawValue, path, oof.Properties)
			switch {
			case err != nil:
		...
		default:
			bqField = fi.OrigName
			bqValue, err = getValue(s.FieldByIndex(fi.structIndex).Interface(), path, fi.Properties)
			switch {
			case err != nil:
				return nil, errors.Annotate(err, "%s", fi.OrigName).Err()
			case bqValue == nil:
