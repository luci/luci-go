...
func mapFromMessage(m proto.Message, path []string) (map[string]bigquery.Value, error) {
	...
	case sPtr.Kind() != reflect.Ptr:
		return nil, fmt.Errorf("a type implementing proto.Message is not a pointer")
	case sPtr.IsNil():
		return nil, nil
	}
	...
	if s.Kind() != reflect.Struct {
		return nil, fmt.Errorf("a type implementing proto.Message is not a pointer to a struct")
	}
	...
	var rowMap map[string]bigquery.Value // keep it nil unless there are values
