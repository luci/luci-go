...
func run(ctx context.Context, inputDir string) error {
	...
	for _, protoFile := range protoFiles {
		...
		name := path.Base(fileDesc.GetName())
		goFile := filepath.Join(
			inputs.outputDir,
			filepath.FromSlash(goPackage),
			strings.TrimSuffix(name, ".proto")+".pb.go",
		)
		...
		if strings.HasSuffix(name, "_test.proto") {
	...
	if !*disableGRPC && *withDiscovery {
		// We support generating discovery file only when all generated *.pb.go end
		// up in the same Go package. Otherwise it's not clear what package to put
		// the pb.discovery.go into.
		if goPackages.Len() != 1 {
			return errors.Reason(
				"cannot generated pb.discovery.go: generated *.pb.go files are in multiple packages %v",
...
func stageInputs(ctx context.Context, inputDir string, mods, protoImportPaths []string) (inputs *stagedInputs, err error) {
	// Try to fine the main module (if running in modules mode). We'll put
