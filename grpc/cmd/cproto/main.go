...
func compile(c context.Context, gopath, protoFiles []string, dir, descSetOut string) (outDir string, err error) {
	...
	for _, p := range gopath {
		...
		// If the dir is under $GOPATH/src, generate .go files near
		// .proto files.
...
func run(c context.Context, goPath []string, dir string) error {
	...
	outDir, err := compile(c, goPath, protoFiles, dir, descPath)
	if err != nil {
		return err
	}
