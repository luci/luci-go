...
func TestEncoding(t *testing.T) {
	...
	Convey("writeError", t, func() {
		...
		Convey("client error", func() {
			writeError(c, rec, status.Error(codes.NotFound, "not found"), 0)
		...
		Convey("internal error", func() {
			writeError(c, rec, status.Error(codes.Internal, "errmsg"), 0)
		...
		Convey("unknown error", func() {
			writeError(c, rec, status.Error(codes.Unknown, "errmsg"), 0)
			So(rec.Code, ShouldEqual, http.StatusInternalServerError)
