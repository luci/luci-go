...
func (s *Server) authenticate() router.Middleware {
	...
	return func(c *router.Context, next router.Handler) {
		...
		case transient.Tag.In(err):
			writeError(c.Context, c.Writer, withCode(err, codes.Internal), 0)
		case err != nil:
			writeError(c.Context, c.Writer, withCode(err, codes.Unauthenticated), 0)
		default:
			c.Context = ctx
			next(c)
		}
...
func (s *Server) handlePOST(c *router.Context) {
	...
	switch perr, _ := err.(*protocolError); {
	case perr != nil:
		writeError(c.Context, c.Writer, withCode(perr.err, codes.InvalidArgument), perr.status)
	case err != nil:
		writeError(c.Context, c.Writer, err, 0)
	default:
