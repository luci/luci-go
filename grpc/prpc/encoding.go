...
// \n.
func marshalMessage(msg proto.Message, format Format, pbuf *proto.Buffer, wrap bool) ([]byte, error) {
	pbuf.Reset() // reset unconditionally
	...
	case FormatBinary:
		...
		return pbuf.Bytes()[:proto.Size(msg)], nil
...
func writeMessage(c context.Context, w http.ResponseWriter, msg proto.Message, format Format) {
	...
	body, err := marshalMessage(msg, format, &proto.Buffer{}, true)
	if err != nil {
...
func statusDetailsToHeaderValues(details []*any.Any, format Format) ([]string, error) {
	...
	for i, det := range details {
		msgBytes, err := marshalMessage(det, format, buf, false)
		if err != nil {
			return nil, err
		}
