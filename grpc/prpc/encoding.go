...
func writeMessage(c context.Context, w http.ResponseWriter, msg proto.Message, format Format) {
	...
	if err != nil {
		writeError(c, w, withCode(err, codes.Internal), 0)
		return
	}
...
// writeError writes err to w and potentially to the log in c.
// If status is 0, derives it from err's code.
func writeError(c context.Context, w http.ResponseWriter, err error, status int) {
	...
	var body string
	if status >= 500 {
		logging.Errorf(c, "prpc: responding with %s error: %s", code, desc)
		body = http.StatusText(status)
	} else {
		logging.Warningf(c, "prpc: responding with %s error: %s", code, desc)
		body = desc
	}
