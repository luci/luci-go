...
func writeMessage(c context.Context, w http.ResponseWriter, msg proto.Message, format Format) {
	...
	if err != nil {
		writeError(c, w, codes.Internal, 0, "%s", err)
		return
	}
...
// writeError writes an error to w and potentially to the log in c.
func writeError(c context.Context, w http.ResponseWriter, code codes.Code, status int, format string, a ...interface{}) {
