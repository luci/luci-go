...
//
// Encodes metadata entries with keys that end in "-bin".
func metaIntoHeaders(md metadata.MD, h http.Header) error {
	for k, vs := range md {
		...
		bin := strings.HasSuffix(canon, "-Bin")
		for _, v := range vs {
			if bin {
				v = base64.StdEncoding.EncodeToString([]byte(v))
			}
			h[canon] = append(h[canon], v)
		}
	}
	return nil
}
...
//
// Decodes entries with keys that end in "-bin". See also readStatusDetails
