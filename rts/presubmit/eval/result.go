...
func (r *Result) TresholdScores(t Threshold) Scores {
	...
	if r.TotalFailures > 0 {
		s.TestRecall = float64(t.PreservedTestFailures) / float64(r.TotalFailures)
	}
	if r.TotalDuration > 0 {
		saved := r.TotalDuration - t.TestDuration
		s.Savings = float64(saved) / float64(r.TotalDuration)
	}
	return s
}
...
func (r *Result) Print(w io.Writer, minChangeRecall float64) error {
	...
	type entry struct {
		Threshold
		Scores
		ExpectedChangeRecall float64
	}
	entries := map[float64]entry{}
	if minChangeRecall <= 1.0 {
		start := 0
		if minChangeRecall > 0 {
			start = int(minChangeRecall * 100)
		}
		for row := 0; row < 100; row++ {
			for col := 0; col < 100; col++ {
				...
				p.printf(
					...
					row+1,
					col+1,
					scoreString(s.ChangeRecall),
					scoreString(s.Savings),
					scoreString(s.TestRecall),
					t.Value.Distance,
					t.Value.Rank,
				)
				if e, ok := entries[s.ChangeRecall]; !ok || s.Savings > e.Savings {
					entries[s.ChangeRecall] = entry{Threshold: t, Scores: s}
				}
			}
		}
	}
	...
	p.printf("ChangeRecall | Savings | TestRecall | Distance | Rank\n")
	p.printf("-----------------------------------------------------\n")
