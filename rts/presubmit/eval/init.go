...
func (r *evalRun) Init(ctx context.Context) error {
	switch {
	case r.WindowsDays <= 0:
		return errors.New("-window must be positive")
	case r.Concurrency < 0:
		return errors.New("-j must be positive")
	case r.GerritQPSLimit <= 0:
		return errors.New("-gerrit-qps-limit must be positive")
	case r.RejectedPatchSetProvider == nil:
		return errors.New("RejectedPatchSets is not initialized")
	}
...
func (r *evalRun) newGerritClient(authenticator *auth.Authenticator) (*gerritClient, error) {
	...
	return &gerritClient{
		...
		//evalRun: run,
		clCache: cache{
			dir:        filepath.Join(r.CacheDir, "gerrit-cl"),
			memory:     lru.New(256),
