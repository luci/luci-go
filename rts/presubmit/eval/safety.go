...
func (e *safetyEval) eval(ctx context.Context) error {
	...
	for {
		...
		case rej, ok := <-e.rejectionC:
			...
			bestAf := Affectedness{Distance: math.Inf(1)}
			for _, af := range e.out.TestVariantAffectedness {
				if bestAf.Distance > af.Distance {
					bestAf.Distance = af.Distance
				}
				if bestAf.Rank == 0 || bestAf.Rank > af.Rank {
					bestAf.Rank = af.Rank
				}
			}
			preservedRejection := e.shouldRun(bestAf)
