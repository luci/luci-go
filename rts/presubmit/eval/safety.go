...
func (r *ChangeRecall) preservedRejections() int {
	return r.EligibleRejections - len(r.LostRejections)
}

// Score returns the fraction of eligible rejectinos that were preserved.
// May return NaN.
func (r *ChangeRecall) Score() float64 {
	if r.TotalRejections == 0 || r.EligibleRejections == 0 {
		return math.NaN()
	}
	return float64(100*r.preservedRejections()) / float64(r.EligibleRejections)
}
