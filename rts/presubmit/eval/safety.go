	...
	for {
		...
		case rej, ok := <-e.rejectionC:
			...
			for i, af := range e.out.TestVariantAffectedness {
				closestAf := e.out.TestVariantAffectedness[closest]
				if closestAf.Distance > af.Distance {
					closest = i
				} else if af.Rank != 0 && closestAf.Rank != 0 && af.Rank < closestAf.Rank {
					if af.Distance > closestAf.Distance {
						return errors.Reason(
							"%s is further than %s (%f > %f), but its rank less (%d < %d)",
							rej.FailedTestVariants[i], rej.FailedTestVariants[closest],
							af.Distance, closestAf.Distance,
							af.Rank, closestAf.Rank,
						).Err()
					}
					closest = i
				}
