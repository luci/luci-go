...
func (e *Eval) evaluateSafety(ctx context.Context, dest *Result) error {
	...
	if len(changeAffectedness) == 0 {
		return errors.New("no change rejections")
	}
	...
	for row := 0; row < 100; row++ {
		for col := 0; col < 100; col++ {
			t := &dest.Thresholds[row][col]
			t.PreservedRejections = dest.TotalRejections - int(lostRejections[row+1][col+1])
			t.PreservedTestFailures = dest.TotalTestFailures - int(lostFailures[row+1][col+1])
			t.ChangeRecall = float64(t.PreservedRejections) / float64(dest.TotalRejections)
			t.DistanceChangeRecall = float64(row+1) / 100
			t.RankChangeRecall = float64(col+1) / 100
			t.TestRecall = float64(t.PreservedTestFailures) / float64(dest.TotalTestFailures)
		}
	}
	return nil
}
...
func (e *Eval) evaluateEfficiency(ctx context.Context, dest *Result) error {
	...
	for row := 0; row < 100; row++ {
		for col := 0; col < 100; col++ {
			...
			t.Savings = float64(t.SavedDuration) / float64(dest.TotalDuration)
		}
	}
	return nil
}
