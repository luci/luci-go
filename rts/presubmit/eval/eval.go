...
func (r *evalRun) evaluateSafety(ctx context.Context) error {
	...
	eg.Go(func() error {
		err := r.Rejections.PlaybackRejections(ctx)
		return errors.Annotate(err, "failed to playback history").Err()
	})

	r.parallelize(eg, func() error {
		return analyzeRejections(ctx, r.Rejections.RejectionC, r.Strategy, func(ctx context.Context, rej analyzedRejection) error {
			mu.Lock()
	...
	distancePercentiles, rankPercentiles := r.res.Thresholds.init(changeAffectedness)
	...
	bucketize := func(afs AffectednessSlice) *bucketGrid {
		...
		for _, af := range afs {
			buckets.inc(&r.res.Thresholds, af, 1)
		}
		buckets.makeCumulative()
		return &buckets
	}

	r.res.TotalRejections = len(changeAffectedness)
	lostRejections := bucketize(changeAffectedness)

	r.res.TotalTestFailures = len(testAffectedness)
...
func (r *evalRun) evaluateEfficiency(ctx context.Context) error {
	...
	eg.Go(func() error {
		err := r.Durations.PlaybackDurations(ctx)
		return errors.Annotate(err, "failed to playback history").Err()
	})
	...
	r.parallelize(eg, func() error {
		...
		for td := range r.Durations.DurationC {
			// Invoke the strategy.
