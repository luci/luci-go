...
func (e *Eval) evaluateSafety(ctx context.Context, res *Result) (*thresholdGrid, error) {
	...
	maybeAddToHeap := func(item rejectionHeapItem) {
		switch {
		case cap(furthest) == 0:
			return

		// If the heap has a free slot, just add the rejection.
		case len(furthest) < cap(furthest):
			heap.Push(&furthest, item)

		// Otherwise, if the rejection is further than heap's closest one, then
		// replace the latter.
		// is closer than this one.
		case furthest[0].MostAffected.Distance < item.MostAffected.Distance:
			furthest[0] = item
			heap.Fix(&furthest, 0)
		}
	}
	...
	e.goMany(eg, func() error {
		for rej := range e.Rejections.RejectionC {
			...
			maybeAddToHeap(rejectionHeapItem{Rejection: rej, MostAffected: mostAffected})
			mu.Unlock()
		}
		return nil
	})
...
func (h *rejectionHeap) LogAndClear(ctx context.Context) {
	...
	for len(*h) > 0 {
		...
		p.rejectionAndAffectedness(r.Rejection, r.MostAffected)
	}
	p.Level--
