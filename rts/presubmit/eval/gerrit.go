...
type gerritClient struct {
	// gerritGetChangeRPC makes a request to Gerrit fetch a change info.
	// Mockable.
...
//
// If readCache is false, ignores the cache when reading, but caches the
// fetched change.
...
// fetchChange fetches a Gerrit change.
...
func (c *gerritClient) fetchChangeWithQuotaErrorsRetries(ctx context.Context, host string, req *gerritpb.GetChangeRequest) (*gerritpb.ChangeInfo, error) {
	...
	err := retry.Retry(ctx, iter, func() (err error) {
		...
		ret, err = c.gerritGetChangeRPC(ctx, host, req)
		return
	}, nil)
	return ret, err
}
