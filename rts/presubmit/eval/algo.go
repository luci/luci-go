...
func (s AffectednessSlice) closest() (int, error) {
	...
	// The rejection is preserved if at least one test ran, so find the
	// closest test.
	closest := 0
	for i, af := range s {
		closestAf := s[closest]
...
// checkConsistency returns a non-nil error if a and b are consistent,
// e.g. a is closer, but its rank is greater.
func checkConsistency(a, b Affectedness) error {
	...
	if (a.Distance < b.Distance && a.Rank >= b.Rank) || (a.Distance > b.Distance && a.Rank <= b.Rank) {
		return errors.Reason("rank and distance are inconsistent: %#v and %#v", a, b).Err()
	}
	return nil
}
