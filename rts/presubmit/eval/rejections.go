...
type analyzedRejection struct {
	*evalpb.Rejection
	Eligible bool
	// same order as Rejection.TestVariants.
	Affectedness AffectednessSlice
	Closest      Affectedness
}

func analyzeRejections(ctx context.Context, rejC <-chan *evalpb.Rejection, strategy Strategy, callback func(analyzedRejection) error) error {
	var out Output
	for {
		select {
		case <-ctx.Done():
		...
		case rej, ok := <-rejC:
			...
			if cap(out.TestVariantAffectedness) < len(in.TestVariants) {
				out.TestVariantAffectedness = make([]Affectedness, len(in.TestVariants))
			} else {
				out.TestVariantAffectedness = out.TestVariantAffectedness[:len(in.TestVariants)]
				for i := range out.TestVariantAffectedness {
					out.TestVariantAffectedness[i] = Affectedness{}
				}
			}
			...
			err = callback(analyzedRejection{
				Rejection:    rej,
