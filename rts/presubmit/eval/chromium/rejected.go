...
// RejectedPatchSets implements eval.RejectedPatchSetProvider for Chromium
// based on CQ and ResultDB BigQuery tables.
	...
	WITH
		...
		flat AS (
			SELECT
				...
				ps.change,
				ps.patchset,
				MIN(ps_approx_timestamp) timestamp,
				test_id,
				ANY_VALUE(file_name) as file_name,
			FROM builds b
			JOIN failed_test_variants t ON b.id = t.build_id
			GROUP BY ps.host, ps.change, ps.patchset, test_id
			-- TODO(crbug.com/1112125): consider moving filtering "only failed"
			-- after joining. This will increase the query cost, but might improve
