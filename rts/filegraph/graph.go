...
type Graph interface {
	...
	// See also Node.Name().
	//
	// This function is deterministic: g.Node(name) == g.Node(name).
...
type Node interface {
	...
	// It is a relative, forward-slash separated path, e.g. "foo/bar.cc".
	// Special name "." means root.
	Name() string
	...
	// If outgoingEdges is false, then iterates over incoming edges.
	// If callback returns false, then iteration stops.
	Adjacent(outgoingEdges bool, callback func(other Node, distance float64) bool)
}
