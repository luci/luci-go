...
func TestWrite(t *testing.T) {
	...
	Convey(`Write`, t, func() {
		...
		Convey(`Two direct children`, func() {
			...
			foo.edges = []edge{{to: bar, probSum: probMultiplier}}
			bar.edges = []edge{{to: foo, probSum: probMultiplier}}
			g := &Graph{
				Commit: "deadbeef",
				root: node{
			...
			test(g,
				...
				"2",        // index of foo
				"16777216", // probSumMultiplied for bar->foo

				"1",        // number of foo edges
				"1",        // index of bar
				"16777216", // probSumMultiplied for foo->bar
			)
		})
	})
}
