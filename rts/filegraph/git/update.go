...
func (g *Graph) apply(fileChanges []fileChange) error {
	...
	// any other file.
	// Encode it same way as edge.probSumMultiplied.
	p := probSumMultiplier / int64(len(files)-1)
	...
	for _, file := range files {
		...
		for i, e := range file.edges {
			if _, ok := fileSet[e.to]; ok {
				...
				// Add the probability of this file being selected from this comment,
				// unless it is an alias edge.
				if e.probSumMultiplied != 0 {
					file.edges[i].probSumMultiplied += p
				}
			}
		}
		...
		for _, to := range files {
			if to != file {
				if _, ok := updated[to]; !ok {
					...
					file.edges = append(file.edges, edge{to: to, probSumMultiplied: p})
				}
			}
		}
	}
