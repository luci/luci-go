...
func (u *Updater) Update(ctx context.Context, g *Graph) error {
	...
	pendingParents := stringset.New(2)
	return readLog(ctx, u.RepoDir, g.Commit, rev, func(c commit) error {
		...
		pendingParents.DelAll(c.ParentHashes)
		if len(pendingParents) > 0 {
			// This means we are currently processing a non-first parent and it
			// is not safe stopping point.
			// For example, in the following graph the commit 5 has two parents
			// (2 and 4), and the commit numbers represent the order of commits
			...
			//    0
			//
			// Commit 2 is a safe stopping point because all commits reachable from 2
			// are exactly the changes incorporated into the graph.
			// However commit 3 is not sa safe stopping point because the graph
			// already incroporated changes 1 and 2, which are not reachable from 3.
			//
			// Instead, wait until we reach 5. We will discover that point when
			// both 2 and 4 are marked as processed.
		} else {
			g.Commit = c.Hash
			if u.Callback != nil {
