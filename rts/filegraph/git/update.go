...
func (u *Updater) apply(g *Graph, fileChanges []fileChange) error {
	...
	for _, c := range fileChanges {
		...
		case c.Status == 'R':
			...
			if oldFile := g.node("//" + c.Path); oldFile != nil {
		...
		case c.Path2 != "":
			return errors.Reason("unexpected path2 %q for file status %c", c.Path2, c.Status).Err()

		default:
			files = append(files, g.ensureNode("//"+c.Path))
		}
	}
