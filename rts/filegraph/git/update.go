...
// Updater synchronizes a graph with a git repository.
...
func (s *Updater) Update(ctx context.Context, g *Graph) error {
	g.ensureInitialized()
...
func (s *Updater) apply(g *Graph, fileChanges []fileChange) error {
	...
	for _, c := range fileChanges {
		...
		case 'R':
			...
			newFile := g.ensureNode("//" + c.Path2)
			if oldFile := g.node(name); oldFile != nil {
		...
		case 'C':
			// The file was renamed.
			newFile := g.ensureNode(name)
			if oldFile := g.node("//" + c.Path2); oldFile != nil {
				oldFile.ensureAlias(newFile)
				newFile.ensureAlias(oldFile)
			}
			files = append(files, newFile)
