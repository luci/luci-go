...
func (g *Graph) apply(fileChanges []fileChange) error {
	...
	probMultiplied := probSumMultiplier / int64(len(files)-1)
		...
		for i, e := range file.edges {
			if _, ok := fileSet[e.to]; ok {
				...
				if e.probSumMultiplied != 0 {
					file.edges[i].probSumMultiplied += probMultiplied
				}
			}
		}
		...
		for _, to := range files {
			if to != file {
				if _, ok := updated[to]; !ok {
					...
					file.edges = append(file.edges, edge{to: to, probSumMultiplied: probMultiplied})
				}
			}
		}
	}
