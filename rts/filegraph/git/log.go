...
func (r *logReader) readParentHashes() (hashes []string, err error) {
	// If the next byte is a separator or \n, then there are no parent hashes.
	switch b, err := r.r.Peek(1); {
	case err != nil:
		return nil, err
	case b[0] == r.sep:
		return nil, nil // This is an orphan commit with no modified files.
	case b[0] == '\n':
		return nil, nil // This is an orphan commit with some modified files.
	}
