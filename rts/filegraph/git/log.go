...
func (r *logReader) readParentHashes() (hashes []string, err error) {
	// If the next byte is a separator, then there are no parent commits.
...
func (r *logReader) readFileChanges() (changes []fileChange, err error) {
	// If the next byte is '\n', then file changes follow.
	if hasFileChanges, err := r.readIf('\n'); err != nil || !hasFileChanges {
		return nil, err
	}

	// There is at least one file change and each one starts with ":".
	for {
		if nextFollows, err := r.readIf(':'); err != nil || !nextFollows {
			return changes, err
		}
