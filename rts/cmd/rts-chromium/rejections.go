...
func (r *presubmitHistoryRun) rejections(ctx context.Context, f func(*evalpb.RejectionFragment) error) error {
	...
	// The BigQuery rows are ordered by change and patchset numbers,
	// so are rejection fragments.
	// Keep track if the current patchset to detect patchset boundaries.
	...
	for {
		...
		if newPS {
			// Report the patchset in the first fragment.
