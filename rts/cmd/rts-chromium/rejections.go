...
func (r *presubmitHistoryRun) rejections(ctx context.Context, f func(*evalpb.Rejection) error) error {
	...
	// Each result row represents a failed test.
	...
	for {
		...
		curRej.FailedTests = append(curRej.FailedTests, row.FailedTests.proto())
	}
}
...
//
// This query intentionally does not use ARRAY_AGG for failed tests because it
