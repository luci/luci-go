...
func (r *selectRun) selectTests(excludedTestFiles io.Writer) (err error) {
	q := prepareFileGraphQuery(r.fg, r.changedFiles.ToSlice())
	if q == nil {
		return nil
	}
	...
	q.Run(func(sp *filegraph.ShortestPath) (keepGoing bool) {
		...
		rank++
		if rank > r.threhsold.MaxRank && sp.Distance > r.threhsold.MaxDistance && r.existingTestFiles.Has(sp.Node.Name()) {
			if _, err = fmt.Fprintln(excludedTestFiles, sp.Node.Name()); err != nil {
				return false
			}
		}
		return true
