...
func (r *selectRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	r.gitModel.Threshold = r.chooseThreshold().Value
	logging.Infof(ctx, "chosen threhsold: %#v", r.gitModel.Threshold)
...
func (r *selectRun) loadInput(ctx context.Context) error {
	...
	eg.Go(func() error {
		err := r.loadEvalResult(filepath.Join(gitGraphModel, "eval-result.json"))
...
// loadEvalResult loads eval.Results, including threhsolds.
