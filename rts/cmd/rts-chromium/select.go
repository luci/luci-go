	...
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			r.Flags.Float64Var(&r.targetChangeRecall, "target-change-recall", 0.99, text.Doc(`
				...
				A value in (0.0, 1.0).
			`))
			r.Flags.StringVar(&r.excludedTestFilesPath, "excluded-test-files", "", text.Doc(`
				Path to the file where to write excluded test files.
...
type selectRun struct {
	...
	excludedTestFilesPath string

	threshold    threshold
	changedFiles stringset.Set
	predict      git.RTSPredict
}
...
func (r *selectRun) run(ctx context.Context) error {
	...
	f, err := os.Create(r.excludedTestFilesPath)
	if err != nil {
		return errors.Annotate(err, "failed to create exclude file at %q", r.excludedTestFilesPath).Err()
	}
	defer f.Close()
	return r.selectTests(func(excludedFile string) error {
		_, err := fmt.Fprintln(f, excludedFile)
		return err
	})
}
...
func (r *selectRun) loadGraph() error {
	...
	r.predict.Graph = &git.Graph{}
	return r.predict.Graph.Read(bufio.NewReader(f))
}

// loadTestFileList loads the list of existing test files from the model.
func (r *selectRun) loadTestFileList() error {
	fileName := filepath.Join(r.modelDir, "test-file-list.txt")
	var err error
	r.predict.TestFiles, err = loadStringSet(fileName)
	return err
}
