...
func (r *selectRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	r.gitModel.Threshold = rts.Affectedness{Distance: 14, Rank: 100000}
...
func (r *selectRun) loadInput(ctx context.Context) error {
	...
	gitGraphModel := filepath.Join(r.modelDir, "git-file-graph")
	eg.Go(func() error {
		err := r.loadGraph(filepath.Join(gitGraphModel, "graph.fg"))
		return errors.Annotate(err, "failed to load file graph").Err()
	})
	eg.Go(func() error {
		err := r.loadThresholds(filepath.Join(gitGraphModel, "thresholds.json"))
...
// loadThresholds loads the mapping {changeRecallScore -> thresholds} from a
// file.
func (r *selectRun) loadThresholds(fileName string) error {
...
func (r *selectRun) loadGraph(fileName string) error {
	...
	// There might have been such change in the repo that the chosen threhsold,
	// the model or both are no longer good.
