...
func (r *createModelRun) run(ctx context.Context) error {
	var err error
...
func (r *createModelRun) evaluate(ctx context.Context) (*eval.Result, error) {
	ev := eval.Eval{
		Strategy:   r.graph.EvalStrategy,
	...
	eg.Go(func() error {
		...
		synth := history.RejectionSynth{}
		return rejQ.run(ctx, r.bqClient, func(frag *evalpb.RejectionFragment) error {
			if rej := synth.Next(frag); rej != nil {
				select {
				case <-ctx.Done():
					return ctx.Err()
				case rejC <- rej:
