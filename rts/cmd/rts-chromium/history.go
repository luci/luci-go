...
func (r *baseHistoryRun) RegisterBaseFlags(fs *flag.FlagSet) {
	fs.StringVar(&r.out, "out", "", "Path to the output file")
	fs.Var(luciflag.Date(&r.startTime), "from", "Fetch results starting from this date; format: 2020-01-02")
	fs.Var(luciflag.Date(&r.endTime), "to", "Fetch results until this date; format: 2020-01-02")
...
func (r *baseHistoryRun) fetchResults(ctx context.Context, table *bigquery.Table) error {
	...
	// 2. Download the prepared files and places them on the file system.
	...
	extractJob, err := table.ExtractorTo(gcsRef).Run(ctx)
	if err != nil {
		return err
	}
	if err := waitForSuccess(ctx, extractJob); err != nil {
		return errors.Annotate(err, "extract job %q failed", extractJob.ID()).Err()
	}
