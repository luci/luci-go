...
func cmdFetchDurations(authOpt *auth.Options) *subcommands.Command {
	return &subcommands.Command{
		...
		LongDesc:  "Fetch test duration data, suitable for selection strategy evaluation",
	...
	affected_files AS (
		...
		WHERE create_time BETWEEN @startTime and TIMESTAMP_ADD(@endTime, INTERVAL 1 DAY)
			...
			AND NOT EXISTS (
				...
				-- Must start with '"src/"', otherwise it is not an src.git change.
				-- The leading quote is there because it is a JSON string.
				WHERE f NOT LIKE "\"src/%"
			)
	),
	ps_affected_files AS (
		-- Choose the longest file list.
		-- Lists for the same patchset can be different if the parent CL landed
		-- between bot_updates of different tryjobs.
		SELECT change, patchset, ARRAY_AGG(af ORDER BY ARRAY_LENGTH(files) DESC)[OFFSET(0)].files
	...
	test_results AS (
		...
		WHERE partition_time BETWEEN @startTime and @endTime
		  AND RAND() <= @frac
			...
			-- TODO(nodir): replace with simple "not empty" check after crbug.com/1130425 is fixed.
			AND REGEXP_CONTAINS(test_location.file_name, r'(?i)\.(cc|html|m|c|cpp)$')
	)

-- Join all tables and produce the rows in the TestDurationRecord protojson format.
SELECT
	[STRUCT(
		STRUCT(
			"chromium-review.googlsource.com" AS host,
			"src" AS project,
			ps.change AS number
		) AS change,
		ps.patchset,
		ANY_VALUE(ARRAY(
			SELECT AS STRUCT
				"https://chromium.googlsource.com/src" AS repo,
				f as path
	...
	ARRAY_AGG(STRUCT(
		STRUCT(
			test_id AS id,
			ARRAY((SELECT FORMAT("%s:%s", key, value) FROM tr.variant)) as variant,
...
JOIN ps_affected_files af ON ps.change = af.change AND ps.patchset = af.patchset
