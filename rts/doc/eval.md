...
A safe RTS algorithm doesn't let bad code into the repo.
...
one test that caused the rejection. For example, if 10 tests failed out of
1000, it is sufficient to select just 2 to preserve the rejection.
...
*   `total_duration` is the sum of test durations in the historical records.
...
This fetches historical records of 2020-10-01. This example is purely
...
* [rts-random](../cmd/rts-random) flips a coin to decide whether to run a
  test. It is a Hello World of RTS algorithms. Its performance is poor.
...
* It is often much quicker, so you can iterate faster.
* The results of safety evaluation are actionable, unlike efficiency.
  This is because safety evaluation prints the CLs where the algorithm
  failed to select affected tests.
* Data for efficiency evaluation is larger and takes much longer to fetch.

This is achieved by instructing `presubmit-history` subcommand to omit fetching
test durations used for efficiency evaluation:
* Pass `-duration-data-frac 0` to `rts-chromium presubmit-history`.
* As a convention, use `.safety.hist` extension for history files.

Omitting test durations makes it practical to use much wider time window.
The following examples fetches all rejections from September 2020:
...
only the last day of the time window that used for safety evaluation.
...
* `-duration-data-frac` is the fraction of test durations to fetch,
  defaulting to 0.001 (0.1%).
* `-min-duration` is the minimum duration to fetch. In practice,
  quick tests are dwarfed by slow tests. The default is 1s.

### Scope to one builder

To iterate quicker, narrow the evaluation down to one builder using `-builder`
flag of `presubmit-history`:
...
Another reason to do this is that some builders, such as linux-rel, are used
by developers as representative of other builders.
In such case, it is important to *preserve rejections* at the builder level,
in addition to CQ at large.

### Do not perfect safety

Ideally the algorithm preserves all rejections, but it is often not practical
because sometimes test flakes creep in. The recommended strategy of fine tuning
an algorithm is to preserve most rejections and manually analyze the rest.
Some of them will turn out to be "OK", and others may provide insight into
further tuning.
