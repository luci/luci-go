...
func main() {
	impl.Main(func(srv *server.Server, services *impl.Services) error {
		...
		srv.Routes.POST("/internal/pubsub/bigquery-log", oidcMW, func(c *router.Context) {
			// Authentication is done oidcMW, do authorization now.
			// bigquery-log-pubsub@ is a part of the PubSub Push subscription config.
			got := auth.CurrentIdentity(c.Context)
			want := identity.Identity(fmt.Sprintf("user:bigquery-log-pubsub@%s.iam.gserviceaccount.com", srv.Options.CloudProject))
			if got != want {
				logging.Errorf(c.Context, "Expecting ID token of %q, got %q", want, got)
