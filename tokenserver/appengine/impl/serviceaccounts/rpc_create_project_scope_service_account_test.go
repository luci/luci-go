...
func testServiceAccountCreator(ctx context.Context, identity *projectscope.ScopedIdentity) (*iam.ServiceAccount, bool, error) {
...
func TestCreateProjectScopedServiceAccount(t *testing.T) {
	gcpProjectResolver := newTestGcpProjectResolver()
	gcpProjectResolver.Add("svc", "bar")
	gcpProjectResolver.Add("svc3", "baz")

	ctx := gaetesting.TestingContext()
	rpc := CreateProjectScopedServiceAccountRPC{
		ScopedIdentities:     projectscope.GetScopedIdentities(),
		GcpProjectResolver:   gcpProjectResolver,
