...
func TestCreateProjectScopedServiceAccount(t *testing.T) {
	...
	generatedAccountId, err := projectscope.GenerateAccountId(req.Service, req.Project)
	...
	Convey("Explicit override for account which already exists", t, func() {
		req.Service = "svc2"
		req.OverrideAccountId = generatedAccountId
		_, err = rpc.CreateProjectScopedServiceAccount(ctx, req)
		So(err, ShouldNotBeNil)
		So(err, ShouldResemble, status.Errorf(codes.AlreadyExists, "service account id already taken"))
	})
