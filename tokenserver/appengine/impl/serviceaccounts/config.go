...
func prepareRules(cfg policy.ConfigBundle, revision string) (policy.Queryable, error) {
	...
	def := admin.ServiceAccountRuleDefaults{}
	if parsed.Defaults != nil {
		def = *parsed.Defaults
	}
	if def.MaxGrantValidityDuration == 0 {
		def.MaxGrantValidityDuration = defaultMaxGrantValidityDuration
	}
	...
	validateDefaults("defaults", &def, &v)
	...
	for _, ruleProto := range parsed.Rules {
		r, err := makeRule(ruleProto, &def, revision)
		if err != nil {
			return nil, err
		}
...
func makeRule(ruleProto *admin.ServiceAccountRule, def *admin.ServiceAccountRuleDefaults, rev string) (*Rule, error) {
	v := validation.Context{}
	...
	allowedScopes := stringset.New(len(ruleProto.AllowedScope) + len(def.AllowedScope))
	for _, scope := range ruleProto.AllowedScope {
		allowedScopes.Add(scope)
	}
	for _, scope := range def.AllowedScope {
		allowedScopes.Add(scope)
	}
	...
	if ruleProto.MaxGrantValidityDuration == 0 {
		ruleProto.MaxGrantValidityDuration = def.MaxGrantValidityDuration
	}
