...
func (r *Rules) Check(c context.Context, query *RulesQuery) (*Rule, error) {
	...
	isProxy := false
	if isProxy, err = rule.Proxies.IsMember(c, query.Proxy); err != nil {
	...
	if isTrustedProxy, err = rule.TrustedProxies.IsMember(c, query.Proxy); err != nil {
		logging.WithError(err).Errorf(c, "Failed to check membership of caller %q", query.Proxy)
		return nil, grpc.Errorf(codes.Internal, "membership check failed")
	}
	...
	// it themselves. So do the end user check only for non-trusted proxies.
	if !isTrustedProxy {
