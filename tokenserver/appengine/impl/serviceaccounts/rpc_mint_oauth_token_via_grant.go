...
func (r *MintOAuthTokenViaGrantRPC) MintOAuthTokenViaGrant(c context.Context, req *minter.MintOAuthTokenViaGrantRequest) (*minter.MintOAuthTokenViaGrantResponse, error) {
	...
	var grantBody *tokenserver.OAuthTokenGrantBody
	var rule *Rule
	validator := func() (err error) {
		if grantBody, rule, err = r.validateRequest(c, req, callerID); err != nil {
			return err
		}
		return nil
	}
	if err := utils.ValidateAndNormalizeRequest(c, req.OauthScope, &req.MinValidityDuration, req.AuditTags, validator); err != nil {
