...
func (r *MintOAuthTokenViaGrantRPC) MintOAuthTokenViaGrant(c context.Context, req *minter.MintOAuthTokenViaGrantRequest) (*minter.MintOAuthTokenViaGrantResponse, error) {
	...
	for _, scope := range req.OauthScope {
		if !rule.AllowedScopes.Has(scope) {
			notAllowed = append(notAllowed, scope)
		}
	}
	if len(notAllowed) != 0 {
		logging.Errorf(c, "Requesting a token with disallowed scopes: %q", notAllowed)
		return nil, grpc.Errorf(codes.PermissionDenied, "scopes are not allowed by the rule %q - %q", rule.Rule.Name, notAllowed)
	}

	// At this point everything is validated and we are good to go.
