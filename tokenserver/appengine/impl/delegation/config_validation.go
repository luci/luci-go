...
func validateRule(ctx *validation.Context, title string, r *admin.DelegationRule) {
	...
	case r.MaxValidityDuration == 0:
		ctx.Errorf("%s", `"max_validity_duration" is required`)
	case r.MaxValidityDuration < 0:
		ctx.Errorf("%s", `"max_validity_duration" must be positive`)
	case r.MaxValidityDuration > 24*3600:
		ctx.Errorf("%s", `"max_validity_duration" must be smaller than 86401`)
	}
}
