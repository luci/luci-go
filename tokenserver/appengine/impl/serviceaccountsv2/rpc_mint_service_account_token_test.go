	...
	Convey("Request validation", t, func() {
		...
		Convey("Bad access token request", func() {
			So(call(&minter.MintServiceAccountTokenRequest{
				...
				ServiceAccount: testAccount.Email(),
				Realm:          testRealm,
			}), ShouldErrLike, "bad oauth_scope: required")
			...
			}), ShouldErrLike, "bad oauth_scope: empty string")
			...
			So(call(&minter.MintServiceAccountTokenRequest{
				...
				OauthScope:      []string{"zzz"},
				IdTokenAudience: "aud",
			}), ShouldErrLike, "bad id_token_audience: must not be used")
		})
		...
		Convey("Bad ID token request", func() {
			So(call(&minter.MintServiceAccountTokenRequest{
				...
				ServiceAccount: testAccount.Email(),
				Realm:          testRealm,
			}), ShouldErrLike, "bad id_token_audience: required")
			...
			So(call(&minter.MintServiceAccountTokenRequest{
				...
				OauthScope:      []string{"zzz"},
				IdTokenAudience: "aud",
			}), ShouldErrLike, "bad oauth_scope: must not be used")
		})
