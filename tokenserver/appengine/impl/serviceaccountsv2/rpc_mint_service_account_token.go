...
// callEnv groups a bunch of argument to simplify passing them to functions.
//
...
func (r *MintServiceAccountTokenRPC) MintServiceAccountToken(ctx context.Context, req *minter.MintServiceAccountTokenRequest) (*minter.MintServiceAccountTokenResponse, error) {
	...
	resp := &minter.MintServiceAccountTokenResponse{
		Token:          tok.Token,
		Expiry:         google.NewTimestamp(tok.Expiry.Truncate(time.Second)),
...
func (r *MintServiceAccountTokenRPC) validateRequest(req *minter.MintServiceAccountTokenRequest) (*validatedRequest, error) {
	...
	if req.TokenKind == minter.ServiceAccountTokenKind_SERVICE_ACCOUNT_TOKEN_ACCESS_TOKEN {
		if len(req.OauthScope) == 0 {
			return nil, fmt.Errorf("bad oauth_scope: required when token_kind is %s", req.TokenKind)
		}
		for _, scope := range req.OauthScope {
			if scope == "" {
				return nil, fmt.Errorf("bad oauth_scope: empty string")
			}
		}
		oauthScopes = stringset.NewFromSlice(req.OauthScope...)
	} else {
		if len(req.OauthScope) != 0 {
			return nil, fmt.Errorf("bad oauth_scope: must not be used when token_kind is %s", req.TokenKind)
		}
	}
	...
	if req.TokenKind == minter.ServiceAccountTokenKind_SERVICE_ACCOUNT_TOKEN_ID_TOKEN {
		if req.IdTokenAudience == "" {
			return nil, fmt.Errorf("bad id_token_audience: required when token_kind is %s", req.TokenKind)
		}
	} else {
		if req.IdTokenAudience != "" {
			return nil, fmt.Errorf("bad id_token_audience: must not be used when token_kind is %s", req.TokenKind)
		}
	}
...
func (r *MintServiceAccountTokenRPC) checkACLs(ctx context.Context, env *callEnv, req *validatedRequest) error {
	...
	switch yes, err := env.db.HasPermission(ctx, env.caller, permMintToken, []string{req.realm}); {
	case err != nil:
	...
	switch yes, err := env.db.HasPermission(ctx, accountID, permExistInRealm, []string{req.realm}); {
	case err != nil:
