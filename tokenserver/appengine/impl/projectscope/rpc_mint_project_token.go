...
func (r *MintProjectTokenRPC) MintProjectToken(c context.Context, req *minter.MintProjectTokenRequest) (*minter.MintProjectTokenResponse, error) {
	...
	// Member: Continue
	member, err := auth.IsMember(c, authorizedGroups...)
	switch {
	case err != nil:
		logging.WithError(err).Errorf(c, "unable to perform group check of member %s and groups %v", callerID, authorizedGroups)
