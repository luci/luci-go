...
func validateProjectScopedServiceAccountsCfg(ctx *validation.Context, cfg *admin.ProjectScopedServiceAccounts) {
	...
	for _, scopedService := range cfg.Services {
		if projectIDs, found := association[scopedService.Service]; found {
			for _, projectID := range projectIDs {
				if projectID == scopedService.CloudProjectId {
					ctx.Errorf("two associations with identical values %v", scopedService)
				}
			}
		}
...
func validateServiceName(ctx *validation.Context, service string) {
	if service == "" {
		ctx.Errorf("service cannot be empty")
	}
}

func validateProjectID(ctx *validation.Context, projectID string) {
	if projectID == "" {
		ctx.Errorf("cloud_project_id name cannot be empty")
	}
}
