...
// MintServiceOAuthTokenRPC implements TokenMinter.MintServiceOAuthToken.
// method.
type MintServiceOAuthTokenRPC struct {
	...
	// ScopedIdentities manages project scoped identities.
	//
	// In  prod it is projectscope.persistentIdentityManager
	ScopedIdentities func(context.Context) projectscope.ScopedIdentityManager
...
func (r *MintServiceOAuthTokenRPC) NormalizeValidityDuration(ctx context.Context, req *minter.MintServiceOAuthTokenRequest, rule *Rule) {
	if req.MinValidityDuration == 0 || req.MinValidityDuration < 0 || req.MinValidityDuration > rule.Rule.MaxValidityDuration {
		logging.Debugf(ctx, "Normalized MinValidityDuration from %d to %d", req.MinValidityDuration, rule.Rule.MaxValidityDuration)
		req.MinValidityDuration = rule.Rule.MaxValidityDuration
	}
}

// MintServiceOAuthToken mints a project-scoped service account token.
//
...
func (r *MintServiceOAuthTokenRPC) MintServiceOAuthToken(c context.Context, req *minter.MintServiceOAuthTokenRequest) (*minter.MintServiceOAuthTokenResponse, error) {
	...
	r.NormalizeValidityDuration(c, req, rule)

	// Scoped identity should not exist yet, and we'll try to eagerly create the IAM service account
	scopedIdentity, created, err := r.ScopedIdentities(c).GetOrCreate(c, string(callerID), req.LuciProject, rule.CloudProjectID, r.CreateServiceAccount)
	if err != nil {
	...
	// Minting token might fail because of the Cloud IAM is not ready to issue a token
	// right after service account creation. If this is the case, a transient error is indicated
	// to the client which is encouraged to retry.
	// All other errors are non-transient.
	if err != nil {
		logging.WithError(err).Errorf(c, "Failed to mint scoped oauth token for service %q in project %q, created=%t", callerID, req.LuciProject, created)
		msg := "failed to mint scoped oauth token"
		if created {
			code := codes.ResourceExhausted
			return nil, transient.Tag.Apply(status.Errorf(code, msg))
		}
		code := codes.Internal
	...
	if err != nil && created {
		logging.WithError(err).Errorf(c, "Failed to mint scoped oauth token for service %q in project %q, ")
	}
	if err != nil {
