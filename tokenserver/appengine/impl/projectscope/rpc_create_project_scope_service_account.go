...
// CreateProjectScopedServiceAccount creates a new project scoped service account.
func (r *CreateProjectScopedServiceAccountRPC) CreateProjectScopedServiceAccount(c context.Context, req *admin.CreateProjectScopedServiceAccountRequest) (*admin.CreateProjectScopedServiceAccountResponse, error) {
	...
	scopedIdentity, created, err := r.ScopedIdentities(c).GetOrCreate(c, req.Service, req.Project, rule.CloudProjectID, r.CreateServiceAccount)
	if err != nil {
		logging.WithError(err)
		return nil, status.Errorf(codes.Internal, err.Error())
...
func (r *CreateProjectScopedServiceAccountRPC) validateRequest(c context.Context, req *admin.CreateProjectScopedServiceAccountRequest) error {
	r.logRequest(c, req)

	if req.Project == "" {
		return status.Errorf(codes.InvalidArgument, "project field must be populated")
	}
	if req.Service == "" {
		return status.Errorf(codes.InvalidArgument, "service field must be populated")
	}
	return nil
}

// logRequest logs the body of the request.
func (r *CreateProjectScopedServiceAccountRPC) logRequest(c context.Context, req *admin.CreateProjectScopedServiceAccountRequest) {
	if !logging.IsLogging(c, logging.Debug) {
		return
	}
	m := jsonpb.Marshaler{Indent: "  "}
	dump, _ := m.MarshalToString(req)
	logging.Debugf(c, "CreateProjectScopedServiceAccount:\n%s", dump)
}
