...
func newAdminServer(tc testclock.TestClock, getIdentities func(ctx context.Context) projectscope.ScopedIdentityManager, rules func(ctx context.Context) (*Rules, error)) admin.AdminServer {
	return &adminServerImpl{
		CreateProjectScopedServiceAccountRPC: CreateProjectScopedServiceAccountRPC{
			Rules:            rules,
			ScopedIdentities: getIdentities,
		},
...
func newMinterServer(tc testclock.TestClock, signer signing.Signer, rules func(ctx context.Context) (*Rules, error), getIdentities func(ctx context.Context) projectscope.ScopedIdentityManager) minter.TokenMinterServer {
	...
	createSa := func(ctx context.Context, gcpProject string, identity *projectscope.ScopedIdentity, client projectscope.ServiceAccountClient) (*iam.ServiceAccount, bool, error) {
...
func TestProjectScopedServiceAccounts(t *testing.T) {
	...
	Convey("Happy path / positive cases", t, func() {
		...
		Convey("SA created explicitly, then token minting", func() {
			...
			So(createSaResp, ShouldResemble, &admin.CreateProjectScopedServiceAccountResponse{
				AccountEmail: projectscope.GenerateAccountID(scopedServiceName, scopedServiceLuciProject),
			})
		...
		Convey("Fail to lookup, create account, succeed to lookup", func() {
			_, err := adminServer.LookupProjectScopedServiceAccount(adminCtx, &admin.LookupProjectScopedServiceAccountRequest{
				AccountEmail: projectscope.GenerateAccountID(forLookupServiceName, forLookupServiceLuciProject),
			})
	...
	Convey("Error cases / negative cases", t, func() {
		...
		Convey("Create SA twice", func() {
			...
			So(err, ShouldBeNil)
			So(createSaResp, ShouldResemble, &admin.CreateProjectScopedServiceAccountResponse{
				AccountEmail: projectscope.GenerateAccountID(doubleCreateServiceName, doubleCreateServiceLuciProject),
			})
