...
// CustomRequestValidator can be used by an RPC to add its own request validation.
type CustomRequestValidator func() error
...
func ValidateAndNormalizeRequest(c context.Context, oauthScope []string, durationSecs *int64, auditTags []string, customValidation ...CustomRequestValidator) error {
	...
	// Run custom RPC validator functions
	for _, validator := range customValidation {
