...
func TestCreateServiceAccount(t *testing.T) {
	Convey("Setup test environment", t, func() {
		...
		client := &testServiceAccountClient{
			...
			getServiceAccountImpl: func(ctx context.Context, gcpProject, accountID string) (account *iam.ServiceAccount, e error) {
		...
		Convey("Test service account field population upon creation", func() {
			...
			So(err, ShouldBeNil)
			serviceAccount, err := CreateServiceAccount(ctx, gcpProject, identity, client)
			So(err, ShouldBeNil)
		...
		Convey("createClient returning an error", func() {
			_, err := CreateServiceAccount(ctx, "", &ScopedIdentity{}, nil)
			So(err, ShouldNotBeNil)
		})
	})
}
