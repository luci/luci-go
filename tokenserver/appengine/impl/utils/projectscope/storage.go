...
func (s *persistentIdentityManager) lookup(c context.Context, identity *ProjectIdentity) (*ProjectIdentity, error) {
	tmp := *identity
	if err := s.lookupInternal(c, &tmp); err != nil {
		return nil, err
	}
	return &tmp, nil
}
...
func (s *persistentIdentityManager) Update(c context.Context, identity *ProjectIdentity) (*ProjectIdentity, error) {
	tmp, err := s.LookupByProject(c, identity.Project)
	switch {
	case err == nil && *tmp == *identity: // Doesn't need update
		return identity, nil
	case err == nil && *tmp != *identity: // Needs update
		goto update
	case err == ErrNotFound: // Not found, attempt to create
		goto update
	default:
		return nil, err
	}
