...
func TestMAC48(t *testing.T) {
	...
	Convey("String", t, func() {
		Convey("eui64", func() {
			_, err := (MaxMAC48 + 1).String()
			So(err, ShouldErrLike, "invalid MAC-48 address")
		})

		Convey("ok", func() {
			mac, err := MAC48(1108152157446).String()
			So(err, ShouldBeNil)
			So(mac, ShouldEqual, "01:02:03:04:05:06")
		})

		Convey("min", func() {
			mac, err := MAC48(0).String()
			So(err, ShouldBeNil)
			So(mac, ShouldEqual, "00:00:00:00:00:00")
		})

		Convey("max", func() {
			mac, err := MaxMAC48.String()
			So(err, ShouldBeNil)
			So(mac, ShouldEqual, "ff:ff:ff:ff:ff:ff")
		})
	})
}
