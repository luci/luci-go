...
func TestIPv4Range(t *testing.T) {
	...
	Convey("max length", t, func() {
		start, length, err := IPv4Range("127.0.0.1/0")
		So(err, ShouldBeNil)
		So(start, ShouldEqual, 0)
		So(length, ShouldEqual, 4294967296)
	})
}

func TestParseIPv4(t *testing.T) {
	Convey("ok", t, func() {
		So(ParseIPv4(uint32(0)).String(), ShouldEqual, "0.0.0.0")
		So(ParseIPv4(uint32(2130706433)).String(), ShouldEqual, "127.0.0.1")
		So(ParseIPv4(uint32(4294967295)).String(), ShouldEqual, "255.255.255.255")
	})
}
