...
func (s *DecoratedCrimson) ListIPs(c context.Context, req *ListIPsRequest) (rsp *ListIPsResponse, err error) {
	var newCtx context.Context
	if s.Prelude != nil {
		newCtx, err = s.Prelude(c, "ListIPs", req)
	}
	if err == nil {
		c = newCtx
		rsp, err = s.Service.ListIPs(c, req)
	}
	if s.Postlude != nil {
		err = s.Postlude(c, "ListIPs", rsp, err)
	}
	return
}
