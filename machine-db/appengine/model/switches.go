...
type SwitchesTable struct {
	// racks is a map of switch name to ID in the database.
...
func (t *SwitchesTable) fetch(c context.Context) error {
	db := database.Get(c)
	rows, err := db.QueryContext(c, "SELECT id, name, description, ports, rack_id FROM switches")
	if err != nil {
...
func (t *SwitchesTable) computeChanges(c context.Context, datacenters []*config.DatacenterConfig) error {
	...
	for _, dc := range datacenters {
		for _, rack := range dc.Rack {
			for _, cfg := range rack.Switch {
				...
				if !ok {
					return errors.Reason("failed to determine rack ID for switch: %s: unknown rack: %s", cfg.Name, rack.Name).Err()
				}
	...
	for _, s := range t.current {
		if cfg, ok := cfgs[s.Name]; ok {
			...
			if s.Ports != cfg.Ports || s.Description != cfg.Description || s.RackId != cfg.RackId {
			...
			t.removals = append(t.removals, s.Name)
		}
	}

	// Switches remaining in the map are present in the config but not the database.
	// Iterate deterministically over the slice to determine which switches need to be added.
	for _, dc := range datacenters {
		for _, rack := range dc.Rack {
			for _, cfg := range rack.Switch {
...
func (t *SwitchesTable) add(c context.Context) error {
	...
	db := database.Get(c)
	stmt, err := db.PrepareContext(c, "INSERT INTO switches (name, description, ports, rack_id) VALUES (?, ?, ?, ?)")
	if err != nil {
	...
	for len(t.additions) > 0 {
		...
		if err != nil {
			return errors.Annotate(err, "failed to add switch: %s", s.Name).Err()
		}
		t.current = append(t.current, s)
		t.additions = t.additions[1:]
		logging.Infof(c, "Added switch: %s", s.Name)
		s.Id, err = result.LastInsertId()
		if err != nil {
			return errors.Annotate(err, "failed to get switch ID: %s", s.Name).Err()
		}
	}
	return nil
}
...
func (t *SwitchesTable) remove(c context.Context) error {
	...
	db := database.Get(c)
	stmt, err := db.PrepareContext(c, "DELETE FROM switches WHERE name = ?")
	if err != nil {
		return errors.Annotate(err, "failed to prepare statement").Err()
	}
	defer stmt.Close()
	...
	for len(t.removals) > 0 {
		s := t.removals[0]
		if _, err := stmt.ExecContext(c, s); err != nil {
			// Defer ensures the slice of switches is updated even if we exit early.
			return errors.Annotate(err, "failed to remove switch: %s", s).Err()
		}
		removed.Add(s)
		t.removals = t.removals[1:]
		logging.Infof(c, "Removed switch: %s", s)
	}
	return nil
}
...
func (t *SwitchesTable) update(c context.Context) error {
	...
	db := database.Get(c)
	stmt, err := db.PrepareContext(c, "UPDATE switches SET description = ?, ports = ?, rack_id = ? WHERE id = ?")
	if err != nil {
		return errors.Annotate(err, "failed to prepare statement").Err()
	}
	defer stmt.Close()
	...
	updated := make(map[string]*Switch, len(t.updates))
	defer func() {
		for _, s := range t.current {
			if _, ok := updated[s.Name]; ok {
				s.Description = updated[s.Name].Description
				s.Ports = updated[s.Name].Ports
				s.RackId = updated[s.Name].RackId
			}
		}
	}()
	for len(t.updates) > 0 {
		...
		if _, err := stmt.ExecContext(c, s.Description, s.Ports, s.RackId, s.Id); err != nil {
			return errors.Annotate(err, "failed to update switch: %s", s.Name).Err()
		}
		updated[s.Name] = s
		t.updates = t.updates[1:]
		logging.Infof(c, "Updated switch: %s", s.Name)
	}
	return nil
}
...
func EnsureSwitches(c context.Context, cfgs []*config.DatacenterConfig, rackIds map[string]int64) error {
	...
	if err := s.computeChanges(c, cfgs); err != nil {
