...
func EnsureDatacenters(c context.Context, datacenters map[string]*config.DatacenterConfig) error {
	db := database.Get(c)
...
func (d *DatacentersServer) GetDatacenters(c context.Context, req *crimson.DatacentersRequest) (*crimson.DatacentersResponse, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			...
			return nil, grpc.Errorf(codes.Internal, "Failed to fetch datacenter")
		}
