...
func addDatacenters(c context.Context, datacenters []*DatacenterDiff) error {
	...
	for _, datacenter := range datacenters {
		if _, err := stmt.ExecContext(c, datacenter.Config.Name, datacenter.Config.Description); err != nil {
			return errors.Annotate(err, "failed to add datacenter: %s", datacenter.Config.Name).Err()
		}
		logging.Infof(c, "Added datacenter: %s", datacenter.Config.Name)
	}
	return nil
}
...
func updateDatacenters(c context.Context, datacenters []*DatacenterDiff) error {
	...
	defer stmt.Close()
	for _, datacenter := range datacenters {
		if _, err := stmt.ExecContext(c, datacenter.Config.Description, datacenter.Id); err != nil {
			return errors.Annotate(err, "failed to update datacenter: %s", datacenter.Config.Name).Err()
		}
		logging.Infof(c, "Updated datacenter: %s", datacenter.Config.Name)
	}
	return nil
}
...
func deleteDatacenters(c context.Context, datacenters []*DatacenterDiff) error {
	...
	for _, datacenter := range datacenters {
		if _, err := stmt.ExecContext(c, datacenter.Id); err != nil {
			return errors.Annotate(err, "failed to delete datacenter: %s", datacenter.Database.Name).Err()
		}
		logging.Infof(c, "Deleted datacenter: %s", datacenter.Database.Name)
	}
	return nil
}
