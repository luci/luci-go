...
func (t *KVMsTable) remove(c context.Context) error {
	...
	// Remove each KVM from the database. It's more efficient to update the slice of
...
func (t *KVMsTable) update(c context.Context) error {
	...
	// Update each KVM in the database. It's more efficient to update the slice of
	...
	for len(t.updates) > 0 {
		kvm := t.updates[0]
		if _, err := stmt.ExecContext(c, kvm.Description, kvm.MacAddress, kvm.State, kvm.RackId, kvm.PlatformId, kvm.Id); err != nil {
