...
func (t *IPsTable) add(c context.Context) error {
	...
	for len(t.additions) > 0 {
		...
		if err != nil {
			return errors.Annotate(err, "failed to add IP address %q", common.Uint32ToIPv4(ip.IPv4)).Err()
		}
		t.current = append(t.current, ip)
		t.additions = t.additions[1:]
		logging.Infof(c, "Added IP address %q", common.Uint32ToIPv4(ip.IPv4))
		ip.Id, err = result.LastInsertId()
		if err != nil {
			return errors.Annotate(err, "failed to get IP address ID %q", common.Uint32ToIPv4(ip.IPv4)).Err()
		}
	}
	return nil
}
...
func (t *IPsTable) remove(c context.Context) error {
	...
	for len(t.removals) > 0 {
		...
		if _, err := stmt.ExecContext(c, ip.Id); err != nil {
			...
			return errors.Annotate(err, "failed to remove IP address %q", common.Uint32ToIPv4(ip.IPv4)).Err()
		}
		removed[ip.Id] = struct{}{}
		t.removals = t.removals[1:]
		logging.Infof(c, "Removed IP address %q", common.Uint32ToIPv4(ip.IPv4))
	}
	return nil
}
...
func (t *IPsTable) update(c context.Context) error {
	...
	for len(t.updates) > 0 {
		...
		if _, err := stmt.ExecContext(c, ip.VLANId, ip.Id); err != nil {
			return errors.Annotate(err, "failed to update IP address %q", common.Uint32ToIPv4(ip.IPv4)).Err()
		}
		updated[ip.Id] = ip
		t.updates = t.updates[1:]
		logging.Infof(c, "Updated IP address %q", common.Uint32ToIPv4(ip.IPv4))
	}
	return nil
}
