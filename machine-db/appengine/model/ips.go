...
import (
	...
	"go.chromium.org/luci/machine-db/common/utils"
)
...
func (t *IPsTable) computeChanges(c context.Context, vlans []*config.VLAN) error {
	...
	for _, vlan := range vlans {
		for _, block := range vlan.CidrBlock {
			ipv4, length, err := utils.Range(block)
			if err != nil {
...
func (t *IPsTable) add(c context.Context) error {
	...
	for len(t.additions) > 0 {
		...
		if err != nil {
			return errors.Annotate(err, "failed to add IP address %d", ip.Ipv4).Err()
		}
		t.current = append(t.current, ip)
		t.additions = t.additions[1:]
		logging.Infof(c, "Added IP address %d", ip.Ipv4)
		ip.Id, err = result.LastInsertId()
		if err != nil {
			return errors.Annotate(err, "failed to get IP address ID %d", ip.Ipv4).Err()
		}
	}
	return nil
}
...
func (t *IPsTable) remove(c context.Context) error {
	...
	for len(t.removals) > 0 {
		...
		if _, err := stmt.ExecContext(c, ip.Id); err != nil {
			...
			return errors.Annotate(err, "failed to remove IP address %d", ip.Ipv4).Err()
		}
...
func (t *IPsTable) update(c context.Context) error {
	...
	for len(t.updates) > 0 {
		...
		if _, err := stmt.ExecContext(c, ip.VlanId, ip.Id); err != nil {
			return errors.Annotate(err, "failed to update IP address %d", ip.Ipv4).Err()
		}
		updated[ip.Id] = ip
