...
func (r *RacksTable) fetch(c context.Context) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rack.Id, &rack.Name, &rack.Description, &rack.Datacenter_id); err != nil {
...
func (r *RacksTable) computeChanges(c context.Context, datacenters []*config.DatacenterConfig) error {
	...
	for _, dc := range datacenters {
		for _, cfg := range dc.Rack {
			...
			if !ok {
				return errors.Annotate(fmt.Errorf("unknown datacenter: %s", dc.Name), "failed to determine datacenter ID for rack: %s", cfg.Name).Err()
			}
			cfgs[cfg.Name] = &Rack{
				RackConfig:    cfg,
				Datacenter_id: id,
	...
	for _, rack := range r.racks {
		if cfg, ok := cfgs[rack.Name]; ok {
			...
			if rack.Description != cfg.Description || rack.Datacenter_id != cfg.Datacenter_id {
...
func (r *RacksTable) add(c context.Context) error {
	...
	for len(r.additions) > 0 {
		rack := r.additions[0]
		result, err := stmt.ExecContext(c, rack.Name, rack.Description, rack.Datacenter_id)
		if err != nil {
	...
	defer func() {
		for _, rack := range r.racks {
			if _, ok := updated[rack.Name]; ok {
				...
				rack.Datacenter_id = updated[rack.Name].Datacenter_id
			}
		}
	}()
	for len(r.updates) > 0 {
		rack := r.updates[0]
		if _, err := stmt.ExecContext(c, rack.Description, rack.Datacenter_id, rack.Id); err != nil {
...
func (r *RacksServer) GetRacks(c context.Context, req *crimson.RacksRequest) (*crimson.RacksResponse, error) {
	racks, err := r.getRacks(c, stringset.NewFromSlice(req.Names...), stringset.NewFromSlice(req.Datacenters...))
	if err != nil {
...
func (r *RacksServer) getRacks(c context.Context, names, datacenters stringset.Set) ([]*crimson.Rack, error) {
	db := database.Get(c)
