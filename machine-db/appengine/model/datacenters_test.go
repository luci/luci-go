...
func TestDatacenters(t *testing.T) {
	Convey("fetch", t, func() {
		...
		Convey("query failed", func() {
			...
			So(d.datacenters, ShouldHaveLength, 0)
		...
		Convey("ok", func() {
			...
			m.ExpectQuery(selectStmt).WillReturnRows(rows)
			So(d.fetch(c), ShouldBeNil)
			So(d.datacenters, ShouldResemble, []*Datacenter{
				{
	...
	Convey("computeChanges", t, func() {
		...
		Convey("empty", func() {
			...
			So(d.additions, ShouldHaveLength, 0)
			So(d.updates, ShouldHaveLength, 0)
			So(d.removals, ShouldHaveLength, 0)
		})
	...
	Convey("add", t, func() {
		...
		Convey("ok", func() {
			...
			So(d.add(c), ShouldBeNil)
			So(d.additions, ShouldHaveLength, 0)
			So(d.datacenters, ShouldHaveLength, 1)
			So(m.ExpectationsWereMet(), ShouldBeNil)
		})
	})
	...
	Convey("update", t, func() {
		...
		Convey("ok", func() {
			...
			So(d.updates, ShouldHaveLength, 0)
			So(d.datacenters, ShouldHaveLength, 1)
			So(d.datacenters[0].Description, ShouldEqual, "new description")
			So(m.ExpectationsWereMet(), ShouldBeNil)
		})
	})
}
