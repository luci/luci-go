...
func TestDatacenters(t *testing.T) {
	...
	Convey("computeChanges", t, func() {
		...
		Convey("addition", func() {
			...
			So(table.additions, ShouldResemble, []*config.DatacenterConfig{
				{
					Name:        "datacenter 1",
					Description: "description 1",
				},
		...
		Convey("removal", func() {
			...
			So(table.additions, ShouldBeEmpty)
			So(table.updates, ShouldBeEmpty)
			So(table.removals, ShouldResemble, []string{
				table.current[0].Name,
			})
		})
	})
	...
	Convey("add", t, func() {
		...
		Convey("prepare failed", func() {
			table.additions = append(table.additions, &config.DatacenterConfig{
				Name: "datacenter",
			})
