...
func (*DatacentersTable) needsUpdate(row *Datacenter, cfg *config.DatacenterConfig) bool {
...
func (t *DatacentersTable) add(c context.Context) error {
	...
	stmt, err := db.PrepareContext(c, `
		INSERT INTO datacenters (name, description)
		VALUES (?, ?)
	`)
	if err != nil {
	...
	for len(t.additions) > 0 {
		...
		if err != nil {
			return errors.Annotate(err, "failed to add datacenter %q", cfg.Name).Err()
		}
		dc := &Datacenter{
			DatacenterConfig: config.DatacenterConfig{
				Name:        cfg.Name,
		...
		if err != nil {
			return errors.Annotate(err, "failed to get datacenter ID %q", cfg.Name).Err()
		}
	}
	return nil
}
