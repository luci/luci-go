...
// platformsCfg is the name of the config file enumerating platforms.
const platformsCfg = "platforms.cfg"
...
func importPlatforms(c context.Context, configSet cfgtypes.ConfigSet) error {
	...
	if err := cfgclient.Get(c, cfgclient.AsService, configSet, platformsCfg, textproto.Message(platform), metadata); err != nil {
		return errors.Annotate(err, "failed to load %s", platformsCfg).Err()
	}
	logging.Infof(c, "Found %s revision %q", platformsCfg, metadata.Revision)

	ctx := &validation.Context{Context: c}
	ctx.SetFile(platformsCfg)
