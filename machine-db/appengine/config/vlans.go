...
// vlansCfg is the name of the config file enumerating vlans.
const vlansCfg = "vlans.cfg"
...
func importVLANs(c context.Context, configSet cfgtypes.ConfigSet) error {
	...
	if err := cfgclient.Get(c, cfgclient.AsService, configSet, vlansCfg, textproto.Message(vlan), metadata); err != nil {
		return errors.Annotate(err, "failed to load %s", vlansCfg).Err()
	}
	logging.Infof(c, "Found %s revision %q", vlansCfg, metadata.Revision)

	ctx := &validation.Context{Context: c}
	ctx.SetFile(vlansCfg)
...
func validateVLANs(c *validation.Context, cfg *config.VLANs) {
	...
	for _, vlan := range cfg.Vlan {
		...
		case len(vlan.CidrBlock) < 1:
			c.Errorf("vlan %d must have at least one cidr block", vlan.Id)
		case len(vlan.CidrBlock) > vlanMaxCidrBlocks:
			c.Errorf("vlan %d must have at most %d cidr blocks", vlan.Id, vlanMaxCidrBlocks)
		}
		for _, block := range vlan.CidrBlock {
			_, _, err := net.ParseCIDR(block)
			if err != nil {
				c.Errorf("vlan %d has invalid cidr block %q", vlan.Id, vlan.CidrBlock)
			}
		}
