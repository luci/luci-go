...
// datacentersCfg is the name of the config file enumerating datacenter files.
const datacentersCfg = "datacenters.cfg"
...
func importDatacenters(c context.Context, configSet cfgtypes.ConfigSet) error {
	...
	if err := cfgclient.Get(c, cfgclient.AsService, configSet, datacentersCfg, textproto.Message(cfg), metadata); err != nil {
		return errors.Annotate(err, "failed to load %s", datacentersCfg).Err()
	}
	logging.Infof(c, "Found %s revision %q", datacentersCfg, metadata.Revision)

	ctx := &validation.Context{Context: c}
	ctx.SetFile(datacentersCfg)
	...
	for filename, dc := range datacenters {
		...
		for _, rack := range dc.Rack {
			...
			for _, s := range rack.Switch {
				...
				switch {
				case s.Ports < 1:
					c.Errorf("switch %q must have at least one port", s.Name)
				case s.Ports > switchMaxPorts:
					c.Errorf("switch %q must have at most %d ports", s.Name, switchMaxPorts)
				}
				c.Exit()
			}
			c.Exit()
