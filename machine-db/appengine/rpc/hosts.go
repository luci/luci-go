...
func assignHostnameAndIP(c context.Context, tx *sql.Tx, hostname string, ipv4 common.IPv4) (int64, error) {
	// Look up IP address to determine VLAN.
	ip, err := getIP(c, tx, ipv4)
	switch {
	case err != nil:
		return 0, err
	case ip.Hostname != "":
		return 0, status.Errorf(codes.AlreadyExists, "duplicate physical host for IP address %q", ip.Ipv4)
	}
	...
	if err != nil {
		...
		case e.Number == mysqlerr.ER_DUP_ENTRY && strings.Contains(e.Message, "'name'"):
			...
			return 0, status.Errorf(codes.AlreadyExists, "duplicate hostname %q for VLAN %d", hostname, ip.Vlan)
		}
