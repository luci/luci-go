...
func (*Service) ListNICs(c context.Context, req *crimson.ListNICsRequest) (*crimson.ListNICsResponse, error) {
	nics, err := listNICs(c, database.Get(c), stringset.NewFromSlice(req.Names...), stringset.NewFromSlice(req.Machines...))
	if err != nil {
...
func listNICs(c context.Context, q database.QueryerContext, names, machines stringset.Set) ([]*crimson.NIC, error) {
	...
	if names.Len() > 0 {
		args := make([]interface{}, names.Len())
		for i, n := range names.ToSlice() {
			args[i] = n
		}
		stmt = stmt.Where("n.name IN ("+squirrel.Placeholders(len(args))+")", args...)
	}
	if machines.Len() > 0 {
		args := make([]interface{}, machines.Len())
		for i, m := range machines.ToSlice() {
			args[i] = m
		}
