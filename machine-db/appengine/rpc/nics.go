...
func createNIC(c context.Context, n *crimson.NIC) error {
	...
	mac, err := common.MAC48StringToUint64(n.MacAddress)
	if err != nil {
...
func listNICs(c context.Context, names stringset.Set, machines stringset.Set) ([]*crimson.NIC, error) {
	...
	for rows.Next() {
		n := &crimson.NIC{}
		var mac uint64
		if err = rows.Scan(&n.Name, &n.Machine, &mac, &n.Switch, &n.Switchport); err != nil {
		...
		if matches(n.Name, names) && matches(n.Machine, machines) {
			mac48, err := common.Uint64ToMAC48(mac)
			if err != nil {
				// Assuming we validated MAC addresses going into the database, this shouldn't happen.
				return nil, err
			}
