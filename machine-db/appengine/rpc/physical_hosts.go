...
func createPhysicalHost(c context.Context, h *crimson.PhysicalHost) error {
	...
	// By setting hostnames.vlan_id NOT NULL when setting up the database, we can avoid checking if the given VLAN is valid.
	// MySQL will turn up NULL for its column value which will be rejected as an error. We don't need to look up VLAN because
	// the ID is given in the request, but we look it up anyway to ensure it exists in the database.
	stmt, err := tx.PrepareContext(c, `
		INSERT INTO hostnames (name, vlan_id)
		VALUES (?, (SELECT id FROM vlans WHERE id = ?))
	`)
	if err != nil {
		// TODO(smut): Refactor to avoid having to explicitly call tx.Rollback() on every error.
	...
	_, err = stmt.ExecContext(c, h.Name, h.Vlan)
	stmt.Close()
	if err != nil {
		if err := tx.Rollback(); err != nil {
			errors.Log(c, errors.Annotate(err, "failed to roll back transaction").Err())
	...
	if err != nil {
		...
		case !ok:
			// Type assertion failed.
		case e.Number == mysqlerr.ER_BAD_NULL_ERROR && strings.Contains(e.Message, "'hostname_id'"):
			// e.g. "Error 1048: Column 'hostname_id' cannot be null".
			return status.Errorf(codes.NotFound, "unknown hostname %q for VLAN %d", h.Name, h.Vlan)
