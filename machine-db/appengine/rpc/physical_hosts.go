...
func createPhysicalHost(c context.Context, h *crimson.PhysicalHost) error {
	...
	// By setting hosts.vlan_id, hosts.nic_id, and hosts.os_id NOT NULL when setting up the database, we can avoid
	// checking if the given VLAN, NIC, and OS are valid. MySQL will turn up NULL for its column values which will be
	// rejected as an error. We don't need to look up VLAN because the ID is given in the request, but we look it up
	// anyway to ensure it exists in the database. In order to look up the NIC we need to look up the machine's ID.
	...
	_, err = stmt.ExecContext(c, h.Name, h.Vlan, h.Nic, h.Machine, h.Os, h.VmSlots, h.Description, h.DeploymentTicket)
	if err != nil {
		...
		case e.Number == mysqlerr.ER_DUP_ENTRY && strings.Contains(e.Message, "'nic_id'"):
			// e.g. "Error 1062: Duplicate entry '1' for key 'nic_id'".
			return status.Errorf(codes.AlreadyExists, "duplicate host for NIC %q on machine %q", h.Nic, h.Machine)
		...
		case e.Number == mysqlerr.ER_BAD_NULL_ERROR && strings.Contains(e.Message, "'nic_id'"):
			...
			// The same error message indicates that either machine or NIC does not exist.
			return status.Errorf(codes.NotFound, "unknown NIC %q for machine %q", h.Nic, h.Machine)
...
func listPhysicalHosts(c context.Context, names stringset.Set, vlans map[int64]struct{}) ([]*crimson.PhysicalHost, error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&h.Name, &h.Vlan, &h.Machine, &h.Nic, &h.Os, &h.VmSlots, &h.Description, &h.DeploymentTicket); err != nil {
