...
func (*Service) ListMachines(c context.Context, req *crimson.ListMachinesRequest) (*crimson.ListMachinesResponse, error) {
	machines, err := listMachines(c, database.Get(c), req.Names, req.Platforms, req.Racks, req.States)
	if err != nil {
...
func listMachines(c context.Context, q database.QueryerContext, names, platforms, racks []string, states []common.State) ([]*crimson.Machine, error) {
	...
	stmt = selectInString(stmt, "p.name", platforms)
	stmt = selectInString(stmt, "r.name", racks)
	stmt = selectInState(stmt, "m.state", states)
