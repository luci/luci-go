...
func (*Service) CreateMachine(c context.Context, req *crimson.CreateMachineRequest) (*crimson.CreateMachineResponse, error) {
	if err := validateCreateMachineRequest(req.Machine); err != nil {
		return nil, err
	}
	if err := createMachine(c, req.Machine); err != nil {
		return nil, err
	}
	return &crimson.CreateMachineResponse{
		Machine: req.Machine,
	}, nil
}
...
func createMachine(c context.Context, machine *crimson.Machine) error {
	...
	_, err = stmt.ExecContext(c, machine.Name, machine.Platform, machine.Rack, machine.Description, machine.AssetTag, machine.ServiceTag, machine.DeploymentTicket)
	if err != nil {
		...
		case e.Number == mysqlerr.ER_DUP_ENTRY:
			...
			return status.Errorf(codes.AlreadyExists, "duplicate machine %q", machine.Name)
		case e.Number == mysqlerr.ER_BAD_NULL_ERROR && strings.Contains(e.Message, "'platform_id'"):
			...
			return status.Errorf(codes.NotFound, "unknown platform %q", machine.Platform)
		case e.Number == mysqlerr.ER_BAD_NULL_ERROR && strings.Contains(e.Message, "'rack_id'"):
			// e.g. "Error 1048: Column 'rack_id' cannot be null".
			return status.Errorf(codes.NotFound, "unknown rack %q", machine.Rack)
		}
...
// validateCreateMachineRequest validates a machine for creation.
func validateCreateMachineRequest(machine *crimson.Machine) error {
	switch {
	case machine.Name == "":
		return status.Error(codes.InvalidArgument, "machine name is required and must be non-empty")
	case machine.Platform == "":
