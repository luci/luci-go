...
func TestGetDatacenters(t *testing.T) {
	Convey("getDatacenters", t, func() {
		...
		selectStmt := `^SELECT name, description FROM datacenters$`
...
func TestGetSwitches(t *testing.T) {
	Convey("getSwitches", t, func() {
		...
		defer db.Close()
		c := database.With(context.Background(), db)
		selectStmt := `^SELECT switches.name, switches.description, switches.ports, racks.name, datacenters.name FROM switches, racks, datacenters WHERE switches.rack_id = racks.id AND racks.datacenter_id = datacenters.id$`
		columns := []string{"switches.name", "switches.description", "switches.ports", "racks.name", "datacenters.name"}
		rows := sqlmock.NewRows(columns)
