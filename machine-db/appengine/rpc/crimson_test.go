...
func TestGetRacks(t *testing.T) {
	Convey("getRacks", t, func() {
		...
		Convey("query failed", func() {
			names := stringset.NewFromSlice([]string{"rack"}...)
			dcs := stringset.NewFromSlice([]string{"datacenter"}...)
			m.ExpectQuery(selectStmt).WillReturnError(fmt.Errorf("error"))
		...
		Convey("matches", func() {
			names := stringset.NewFromSlice([]string{"rack 2", "rack 3"}...)
			dcs := stringset.NewFromSlice([]string{"datacenter 2", "datacenter 3"}...)
			rows.AddRow("rack 1", "description 1", "datacenter 1")
			rows.AddRow("rack 2", "description 2", "datacenter 2")
		...
		Convey("ok", func() {
			names := stringset.NewFromSlice([]string{}...)
			dcs := stringset.NewFromSlice([]string{}...)
			rows.AddRow("rack 1", "description 1", "datacenter 1")
			rows.AddRow("rack 2", "description 2", "datacenter 2")
			m.ExpectQuery(selectStmt).WillReturnRows(rows)
			racks, err := getRacks(c, names, dcs)
