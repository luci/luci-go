...
func TestGetDatacenters(t *testing.T) {
	Convey("getDatacenters", t, func() {
		...
		selectStmt := `^SELECT d.name, d.description FROM datacenters d$`
...
func TestGetRacks(t *testing.T) {
	Convey("getRacks", t, func() {
		...
		defer db.Close()
		c := database.With(context.Background(), db)
		selectStmt := `^SELECT r.name, r.description, d.name FROM racks r, datacenters d WHERE r.datacenter_id = d.id$`
...
func TestGetSwitches(t *testing.T) {
	Convey("getSwitches", t, func() {
		...
		defer db.Close()
		c := database.With(context.Background(), db)
		selectStmt := `^SELECT s.name, s.description, s.ports, r.name, d.name FROM switches s, racks r, datacenters d WHERE s.rack_id = r.id AND r.datacenter_id = d.id$`
