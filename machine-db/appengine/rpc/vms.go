...
func createVM(c context.Context, v *crimson.VM) (err error) {
	...
	res, err := stmt.ExecContext(c, v.Name, v.Vlan)
	stmt.Close()
	if err != nil {
	...
	_, err = stmt.ExecContext(c, hostnameId, v.Host, v.Vlan, v.Os, v.Description, v.DeploymentTicket)
	stmt.Close()
	if err != nil {
		switch e, ok := err.(*mysql.MySQLError); {
		case !ok:
...
func listVMs(c context.Context, names stringset.Set, vlans map[int64]struct{}) ([]*crimson.VM, error) {
	...
	rows, err := db.QueryContext(c, `
		...
		WHERE vm.hostname_id = h1.id
			AND h1.vlan_id = v.id
			AND vm.physical_host_id = p.id
			AND p.hostname_id = h2.id
			AND vm.os_id = o.id
	`)
