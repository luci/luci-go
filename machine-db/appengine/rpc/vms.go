...
func updateVM(c context.Context, v *crimson.VM, mask *field_mask.FieldMask) (_ *crimson.VM, err error) {
	...
	vms, err := listVMs(c, tx, []string{v.Name}, []int64{v.Vlan})
	switch {
	case err != nil:
...
func validateVMForUpdate(v *crimson.VM, mask *field_mask.FieldMask) error {
	...
	for _, path := range mask.Paths {
		...
		case "host":
			// If hostname is specified, require host VLAN as well. Both are required to uniquely identify a host.
			if v.Host == "" {
				return status.Error(codes.InvalidArgument, "physical hostname is required and must be non-empty")
			}
			if v.HostVlan < 1 {
		...
		case "host_vlan":
			// If host VLAN is specified, require hostname as well. Both are required to uniquely identify a host.
			if v.Host == "" {
				return status.Error(codes.InvalidArgument, "physical hostname is required and must be non-empty")
			}
			if v.HostVlan < 1 {
