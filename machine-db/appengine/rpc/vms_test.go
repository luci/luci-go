...
func TestCreateVM(t *testing.T) {
	Convey("createVM", t, func() {
		...
		insertVmStmt := `
			...
			VALUES \(\?, \(SELECT p.id FROM physical_hosts p, hostnames h WHERE p.hostname_id = h.id AND h.name = \? AND h.vlan_id = \?\), \(SELECT id FROM oses WHERE name = \?\), \?, \?\)
		`
		...
		Convey("prepare failed", func() {
			...
			m.ExpectPrepare(insertVmStmt).WillReturnError(fmt.Errorf("error"))
			m.ExpectRollback()
			So(createVM(c, vm), ShouldErrLike, "Internal server error")
		})
...
func TestListVMs(t *testing.T) {
	Convey("listVMs", t, func() {
		...
		c := database.With(context.Background(), db)
		selectStmt := `
			^SELECT h1.name, v.id, h2.name, o.name, vm.description, vm.deployment_ticket
			FROM vms vm, hostnames h1, physical_hosts p, hostnames h2, vlans v, oses o
			WHERE vm.hostname_id = h1.id AND h1.vlan_id = v.id AND vm.physical_host_id = p.id AND p.hostname_id = h2.id AND vm.os_id = o.id$
		`
		columns := []string{"h1.name", "v.id", "h2.name", "o.name", "vm.description", "vm.deployment_ticket"}
