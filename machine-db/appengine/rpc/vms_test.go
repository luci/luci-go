...
func TestCreateVM(t *testing.T) {
	Convey("createVM", t, func() {
		...
		Convey("begin failed", func() {
			...
			rows.AddRow(2130706433, 1, nil)
			m.ExpectBegin().WillReturnError(fmt.Errorf("error"))
			m.ExpectQuery(selectIPStmt).WithArgs(2130706433).WillReturnRows(rows)
		...
		Convey("invalid IP", func() {
			...
			m.ExpectBegin()
			m.ExpectQuery(selectIPStmt).WithArgs(2130706433).WillReturnRows(rows)
			m.ExpectExec(insertNameStmt).WithArgs(host.Name, 1).WillReturnError(&mysql.MySQLError{Number: mysqlerr.ER_DUP_ENTRY, Message: "'name'"})
			m.ExpectRollback()
			So(createPhysicalHost(c, host), ShouldErrLike, "unknown IPv4 address")
		})

		Convey("duplicate IP", func() {
			host := &crimson.PhysicalHost{
				Name:    "host 1",
				Machine: "machine",
				Os:      "os",
			...
			rows.AddRow(2130706433, 1, "host 2")
			m.ExpectBegin()
			m.ExpectQuery(selectIPStmt).WithArgs(2130706433).WillReturnRows(rows)
			m.ExpectExec(insertNameStmt).WithArgs(host.Name, 1).WillReturnError(&mysql.MySQLError{Number: mysqlerr.ER_DUP_ENTRY, Message: "'name'"})
			m.ExpectRollback()
			So(createPhysicalHost(c, host), ShouldErrLike, "duplicate physical host for IP address")
		})
		...
		Convey("duplicate hostname/VLAN", func() {
			...
			m.ExpectBegin()
			m.ExpectQuery(selectIPStmt).WithArgs(2130706433).WillReturnRows(rows)
			m.ExpectExec(insertNameStmt).WithArgs(vm.Name, 1).WillReturnError(&mysql.MySQLError{Number: mysqlerr.ER_DUP_ENTRY, Message: "'name'"})
			m.ExpectRollback()
		...
		Convey("query failed", func() {
			...
			m.ExpectBegin()
			m.ExpectQuery(selectIPStmt).WithArgs(2130706433).WillReturnRows(rows)
			m.ExpectExec(insertNameStmt).WithArgs(vm.Name, 1).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(updateIPStmt).WithArgs(1, 2130706433, 1).WillReturnResult(sqlmock.NewResult(1, 1))
