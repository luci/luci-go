...
func TestCreateHost(t *testing.T) {
	Convey("createHost", t, func() {
		...
		hSelectStmt := `
			...
			WHERE name = \? AND vlan_id = \? AND host_id IS NULL$
		`
		...
		Convey("prepare failed", func() {
			...
			mRows.AddRow("1")
			m.ExpectBegin()
			m.ExpectQuery(mSelectStmt).WithArgs("machine").WillReturnRows(mRows)
		...
		Convey("ok", func() {
			...
			hRows.AddRow("1")
			m.ExpectBegin()
			m.ExpectQuery(hSelectStmt).WithArgs("host", 1).WillReturnRows(hRows)
			m.ExpectPrepare(insertStmt)
...
func TestListHosts(t *testing.T) {
	Convey("listHosts", t, func() {
		...
		Convey("matches", func() {
			...
			hosts, err := listHosts(c, names, vlans)
			So(err, ShouldBeNil)
			So(hosts, ShouldResemble, []*crimson.Host{
				{
					Name:    "host 1",
					Vlan:    1,
					Machine: "machine 1",
...
func TestValidateHostForCreation(t *testing.T) {
	t.Parallel()

	Convey("host unspecified", t, func() {
		err := validateHostForCreation(nil)
