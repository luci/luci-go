...
func TestCreateDRAC(t *testing.T) {
	Convey("createDRAC", t, func() {
		...
		`
		selectStmt := `
			^SELECT h.name, h.vlan_id, m.name, i.ipv4
			FROM dracs d, hostnames h, machines m, ips i
			WHERE d.hostname_id = h.id AND d.machine_id = m.id AND i.hostname_id = h.id
				AND h.name IN \(\?\) AND i.ipv4 IN \(\?\)$
		`
		columns := []string{"h.name", "m.name", "i.ipv4", "h.vlan_id"}
		...
		Convey("duplicate machine", func() {
			...
			m.ExpectExec(insertNameStmt).WithArgs(drac.Name, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(updateIPStmt).WithArgs(1, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(insertDRACStmt).WithArgs(1, drac.Machine).WillReturnError(&mysql.MySQLError{Number: mysqlerr.ER_DUP_ENTRY, Message: "'machine_id'"})
			m.ExpectRollback()
			res, err := createDRAC(c, drac)
		...
		Convey("invalid machine", func() {
			...
			m.ExpectExec(insertNameStmt).WithArgs(drac.Name, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(updateIPStmt).WithArgs(1, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(insertDRACStmt).WithArgs(1, drac.Machine).WillReturnError(&mysql.MySQLError{Number: mysqlerr.ER_BAD_NULL_ERROR, Message: "'machine_id' is null"})
			m.ExpectRollback()
			res, err := createDRAC(c, drac)
		...
		Convey("unexpected invalid", func() {
			...
			m.ExpectExec(insertNameStmt).WithArgs(drac.Name, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(updateIPStmt).WithArgs(1, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(insertDRACStmt).WithArgs(1, drac.Machine).WillReturnError(&mysql.MySQLError{Number: mysqlerr.ER_BAD_NULL_ERROR, Message: "error"})
			m.ExpectRollback()
			res, err := createDRAC(c, drac)
			So(err, ShouldErrLike, "Internal server error")
			So(res, ShouldBeNil)
		...
		Convey("unexpected error", func() {
			...
			m.ExpectExec(insertNameStmt).WithArgs(drac.Name, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(updateIPStmt).WithArgs(1, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(insertDRACStmt).WithArgs(1, drac.Machine).WillReturnError(&mysql.MySQLError{Number: mysqlerr.ER_NO, Message: "name vlan_id"})
			m.ExpectRollback()
			res, err := createDRAC(c, drac)
			So(err, ShouldErrLike, "Internal server error")
			So(res, ShouldBeNil)
		...
		Convey("commit failed", func() {
			...
			m.ExpectExec(insertNameStmt).WithArgs(drac.Name, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(updateIPStmt).WithArgs(1, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(insertDRACStmt).WithArgs(1, drac.Machine).WillReturnResult(sqlmock.NewResult(1, 1))
		...
		Convey("ok", func() {
			...
			rows.AddRow(drac.Name, 1, drac.Machine, 2130706433)
			m.ExpectBegin()
			m.ExpectExec(insertNameStmt).WithArgs(drac.Name, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(updateIPStmt).WithArgs(1, 2130706433).WillReturnResult(sqlmock.NewResult(1, 1))
			m.ExpectExec(insertDRACStmt).WithArgs(1, drac.Machine).WillReturnResult(sqlmock.NewResult(1, 1))
	...
	Convey("listDRACs", t, func() {
		...
		Convey("name", func() {
			selectStmt := `
				^SELECT h.name, h.vlan_id, m.name, i.ipv4
				FROM dracs d, hostnames h, machines m, ips i
				WHERE d.hostname_id = h.id AND d.machine_id = m.id AND i.hostname_id = h.id AND h.name IN \(\?\)$
			`
			req := &crimson.ListDRACsRequest{
				Names: []string{"drac"},
			}
		...
		Convey("machine", func() {
			selectStmt := `
				^SELECT h.name, h.vlan_id, m.name, i.ipv4
				FROM dracs d, hostnames h, machines m, ips i
				WHERE d.hostname_id = h.id AND d.machine_id = m.id AND i.hostname_id = h.id AND m.name IN \(\?\)$
			`
			req := &crimson.ListDRACsRequest{
		...
		Convey("IPv4", func() {
			selectStmt := `
				^SELECT h.name, h.vlan_id, m.name, i.ipv4
				FROM dracs d, hostnames h, machines m, ips i
				WHERE d.hostname_id = h.id AND d.machine_id = m.id AND i.hostname_id = h.id AND i.ipv4 IN \(\?\)$
			`
			req := &crimson.ListDRACsRequest{
		...
		Convey("VLAN", func() {
			selectStmt := `
				^SELECT h.name, h.vlan_id, m.name, i.ipv4
				FROM dracs d, hostnames h, machines m, ips i
				WHERE d.hostname_id = h.id AND d.machine_id = m.id AND i.hostname_id = h.id AND h.vlan_id IN \(\?\)$
			`
			req := &crimson.ListDRACsRequest{
				Vlans: []int64{0},
			}
		...
		Convey("non-empty", func() {
			...
			rows.AddRow(req.Names[0], req.Vlans[0], req.Machines[0], 1)
			rows.AddRow(req.Names[0], req.Vlans[0], req.Machines[0], 2)
		...
		Convey("ok", func() {
			...
			`
			req := &crimson.ListDRACsRequest{}
			rows.AddRow("drac", 0, "machine 1", 1)
			rows.AddRow("drac", 0, "machine 2", 2)
