...
var crimsonCtxKey = "crimson.CrimsonServer context key"
...
func InstallHandlers(r *router.Router, base router.MiddlewareChain, srv crimson.CrimsonServer, tmplPath string) {
	m := base.Extend(func(c *router.Context, next router.Handler) {
		c.Context = context.WithValue(c.Context, &crimsonCtxKey, srv)
		next(c)
	})
	m = m.Extend(
...
// impl returns crimson.CrimsonServer as passed to InstallHandlers.
//
// It is used to pull all data necessary to render pages.
func impl(c context.Context) crimson.CrimsonServer {
	s, _ := c.Value(&crimsonCtxKey).(crimson.CrimsonServer)
	if s == nil {
		panic("impossible, &crimsonCtxKey is not set")
	}
	return s
}
