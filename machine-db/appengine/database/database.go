...
// GetDatabase returns the database pointer embedded in the current context.
func GetDatabase(c context.Context) *sql.DB {
	return c.Value(dbKey).(*sql.DB)
}

// WithDatabase installs a database pointer into the given context. It can be retrieved using dbKey.
// Intended to be used as part of a MiddlewareChain.
func WithDatabase(c *router.Context, next router.Handler) {
	database, err := getDatabaseConnection(c.Context)
	if err != nil {
		fmt.Errorf("Failed to retrieve a database connection: %s", err.Error())
		c.Writer.Header().Set("Content-Type", "text/plain")
		c.Writer.WriteHeader(http.StatusInternalServerError)
		return
	}
	c.Context = context.WithValue(c.Context, dbKey, database)
	next(c)
}
