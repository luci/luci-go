...
func addDRACCmd() *subcommands.Command {
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			cmd.Flags.StringVar(&cmd.drac.Ipv4, "ip", "", "The ip address assigned to this DRAC. Required and must be a free IP address returned by get-ips.")
			cmd.f.Register(cmd)
			return cmd
		},
	}
...
func getDRACsCmd() *subcommands.Command {
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			cmd.Flags.Var(flag.StringSlice(&cmd.req.Ipv4S), "ip", "ip address to filter by. Can be specified multiple times.")
