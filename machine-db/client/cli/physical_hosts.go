...
func printPhysicalHosts(showNoHeaders bool, hosts ...*crimson.PhysicalHost) {
	if len(hosts) > 0 {
		p := newStdoutPrinter()
		defer p.Flush()
		if showNoHeaders == false {
...
func (c *AddPhysicalHostCmd) Run(app subcommands.Application, args []string, env subcommands.Env) int {
	...
	printPhysicalHosts(c.f.showNoHeaders, resp)
	return 0
}
...
func (c *GetPhysicalHostsCmd) Run(app subcommands.Application, args []string, env subcommands.Env) int {
	...
	printPhysicalHosts(c.f.showNoHeaders, resp.Hosts...)
	return 0
}
