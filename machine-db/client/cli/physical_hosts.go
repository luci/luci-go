...
func addPhysicalHostCmd() *subcommands.Command {
	return &subcommands.Command{
		UsageLine: "add-host -name <name> -vlan <id> -machine <machine> -os <os> -ipv4 <ip address> -state <state> [-slots <vm slots>] [-desc <description>] [-tick <deployment ticket>]",
		...
		CommandRun: func() subcommands.CommandRun {
			...
			cmd.Flags.Var(StateFlag(&cmd.host.State), "state", "The state of this host. Required and must be the name of a state returned by get-states.")
