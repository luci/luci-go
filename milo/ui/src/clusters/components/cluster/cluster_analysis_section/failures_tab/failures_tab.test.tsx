...
describe('Test FailureTable component', () => {
  ...
  it('when filtering by failure type then should display matching groups', async () => {
    ...
    await fireEvent.change(screen.getByTestId('failure_filter_input'), {
      target: { value: 'critical-failures-exonerated' },
    });
  ...
  it('when filtering with impact then should recalculate impact', async () => {
    ...
    await fireEvent.change(screen.getByTestId('impact_filter_input'), {
      target: { value: 'without-retries' },
    });

    let presubmitRejects = screen.getByTestId(
      'failure_table_group_presubmitrejects',
    );
    ...
    presubmitRejects = screen.getByTestId(
      'failure_table_group_presubmitrejects',
    );
  ...
  it('when grouping by variants then should modify displayed tree', async () => {
    ...
    await screen.findByRole('table');
    await fireEvent.change(screen.getByTestId('group_by_input'), {
      target: { value: 'v1' },
    });
