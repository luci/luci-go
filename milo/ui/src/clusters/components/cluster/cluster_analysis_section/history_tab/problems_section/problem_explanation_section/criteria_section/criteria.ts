...
export const criteriaForPolicy = (
  ...
  policy.metrics.forEach((m) => {
    const metricDefinition = metricDefinitions.find(
      (d) => d.metricId === m.metricId,
    );
    const threshold = activationCriteria
      ? m.activationThreshold
      : m.deactivationThreshold;
    const currentTimewiseCounts: Cluster_TimewiseCounts | undefined =
      metricValues ? metricValues[m.metricId] : undefined;

    const metricName = metricDefinition?.humanReadableName || m.metricId;
    const metricDescription =
      metricDefinition?.description || '(metric description unavailable)';
    ...
    if (threshold?.oneDay !== undefined) {
      ...
      result.push({
        ...
        satisfied: isCriteriumSatisfied(
          currentValue,
          threshold.oneDay,
          activationCriteria,
        ),
