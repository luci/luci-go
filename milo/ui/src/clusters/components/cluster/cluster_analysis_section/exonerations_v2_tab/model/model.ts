...
const isFlakyCriteriaAlmostMet = (
  criteria: TestStabilityCriteria,
  tvb: ExoneratedTestVariantBranch,
): boolean => {
  ...
  const runFlakyRate =
    tvb.flakeRate.runFlakyVerdicts / tvb.flakeRate.totalVerdicts;
  return (
    runFlakyRate >= criteria.flakeRate.flakeRateThreshold / 2.0 &&
    tvb.flakeRate.runFlakyVerdicts >= criteria.flakeRate.flakeThreshold / 2.0
  );
};
...
const isFailureCriteriaAlmostMet = (
  criteria: TestStabilityCriteria,
  tvb: ExoneratedTestVariantBranch,
): boolean => {
  ...
  return (
    (tvb.failureRate.consecutiveUnexpectedTestRuns > 0 &&
      tvb.failureRate.consecutiveUnexpectedTestRuns >=
        criteria.failureRate.consecutiveFailureThreshold / 2) ||
    (tvb.failureRate.unexpectedTestRuns > 0 &&
      tvb.failureRate.unexpectedTestRuns >=
        criteria.failureRate.failureThreshold / 2)
  );
};
...
export const flakyCriteriaMetIndicator = (
  criteria: TestStabilityCriteria,
  tvb: ExoneratedTestVariantBranch,
) => {
  if (isFlakyCriteriaMet(tvb)) {
...
export const failureCriteriaMetIndicator = (
  criteria: TestStabilityCriteria,
  tvb: ExoneratedTestVariantBranch,
) => {
  if (isFailureCriteriaMet(tvb)) {
...
export const anyCriteriaMetIndicator = (
  criteria: TestStabilityCriteria,
  tvb: ExoneratedTestVariantBranch,
  ...
  if (isFlakyCriteriaMet(tvb) || isFailureCriteriaMet(tvb)) {
    ...
    isFlakyCriteriaAlmostMet(criteria, tvb) ||
    isFailureCriteriaAlmostMet(criteria, tvb)
  ) {
    return CriteriaMetIndicator.AlmostMet;
  }
  return CriteriaMetIndicator.NotMet;
...
export const sortTestVariantBranches = (
  response: ExoneratedTestVariantBranches,
  field: SortableField,
  ascending: boolean,
): ExoneratedTestVariantBranch[] => {
  const cloneTVBs = [...response.testVariantBranches];
  const compare = (
    a: ExoneratedTestVariantBranch,
    b: ExoneratedTestVariantBranch,
  ): number => {
    let result = 0;
    switch (field) {
      case 'testId':
      ...
      case 'beingExonerated':
        result =
          anyCriteriaMetIndicator(response.criteria, a) -
          anyCriteriaMetIndicator(response.criteria, b);
        break;
      ...
      case 'lastExoneration':
        result =
          dayjs(a.lastExoneration).unix() - dayjs(b.lastExoneration).unix();
        break;
      default:
