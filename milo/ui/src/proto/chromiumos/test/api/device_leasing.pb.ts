    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 4: {
          ...
          const entry4 = ExtendLeaseRequest_UserPayloadEntry.decode(reader, reader.uint32());
          if (entry4.value !== undefined) {
            message.userPayload[entry4.key] = entry4.value;
          }
          continue;
        }
      }
...
function createBaseExtendLeaseRequest_UserPayloadEntry(): ExtendLeaseRequest_UserPayloadEntry {
  return { key: "", value: "" };
}

export const ExtendLeaseRequest_UserPayloadEntry: MessageFns<ExtendLeaseRequest_UserPayloadEntry> = {
  encode(message: ExtendLeaseRequest_UserPayloadEntry, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.key !== "") {
      writer.uint32(10).string(message.key);
    }
    if (message.value !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): ExtendLeaseRequest_UserPayloadEntry {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseExtendLeaseRequest_UserPayloadEntry() as any;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  fromJSON(object: any): ExtendLeaseRequest_UserPayloadEntry {
    return {
      key: isSet(object.key) ? globalThis.String(object.key) : "",
  ...
  toJSON(message: ExtendLeaseRequest_UserPayloadEntry): unknown {
    const obj: any = {};
    if (message.key !== "") {
      obj.key = message.key;
    }
  ...
  create(base?: DeepPartial<ExtendLeaseRequest_UserPayloadEntry>): ExtendLeaseRequest_UserPayloadEntry {
    return ExtendLeaseRequest_UserPayloadEntry.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<ExtendLeaseRequest_UserPayloadEntry>): ExtendLeaseRequest_UserPayloadEntry {
    const message = createBaseExtendLeaseRequest_UserPayloadEntry() as any;
    message.key = object.key ?? "";
