...
export const Artifact: MessageFns<Artifact> = {
  encode(message: Artifact, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.testIdStructured !== undefined) {
      TestIdentifier.encode(message.testIdStructured, writer.uint32(98).fork()).join();
    }
    if (message.testId !== "") {
    ...
    if (message.rbeUri !== "") {
      writer.uint32(122).string(message.rbeUri);
    }
    if (message.artifactType !== "") {
      writer.uint32(130).string(message.artifactType);
    }
    return writer;
  },
  ...
  fromJSON(object: any): Artifact {
    return {
      ...
      rbeUri: isSet(object.rbeUri) ? globalThis.String(object.rbeUri) : "",
