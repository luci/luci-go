...
export const QueryTestVariantFailureRateRequest: MessageFns<QueryTestVariantFailureRateRequest> = {
  encode(message: QueryTestVariantFailureRateRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    for (const v of message.testVariants) {
      TestVariantIdentifier.encode(v!, writer.uint32(18).fork()).join();
    }
    return writer;
  },
...
export const TestVariantIdentifier: MessageFns<TestVariantIdentifier> = {
  encode(message: TestVariantIdentifier, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.testId !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): TestVariantIdentifier {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
  ...
  create(base?: DeepPartial<TestVariantIdentifier>): TestVariantIdentifier {
    return TestVariantIdentifier.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<TestVariantIdentifier>): TestVariantIdentifier {
    const message = createBaseTestVariantIdentifier() as any;
