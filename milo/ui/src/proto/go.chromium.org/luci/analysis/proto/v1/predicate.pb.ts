...
export const TestVerdictPredicate = {
  encode(message: TestVerdictPredicate, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.subRealm !== "") {
      writer.uint32(10).string(message.subRealm);
    }
    if (message.variantPredicate !== undefined) {
      VariantPredicate.encode(message.variantPredicate, writer.uint32(18).fork()).ldelim();
    }
    if (message.submittedFilter !== 0) {
      writer.uint32(24).int32(message.submittedFilter);
    }
    if (message.partitionTimeRange !== undefined) {
      TimeRange.encode(message.partitionTimeRange, writer.uint32(34).fork()).ldelim();
    }
    if (message.includeBisectionResults === true) {
      writer.uint32(40).bool(message.includeBisectionResults);
    }
    return writer;
  },

  decode(input: _m0.Reader | Uint8Array, length?: number): TestVerdictPredicate {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
