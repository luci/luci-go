...
export interface Drac {
  ...
  /** State of the drac */
  readonly resourceState: State;
  /** This field only used by Shivas. */
  readonly vlan: string;
  /**
   * This field only used by Shivas.
   * It refers to which ip the Drac is assigned to.
   */
  readonly ip: string;
}

/**
 * Peripherals for Android hosts
 *
 * Next Tag: 5
 */
export interface AndroidHostPeripheral {
  readonly name: string;
  readonly serialNumber: string;
  readonly peripheralType: AndroidHostPeripheral_PeripheralType;
  readonly resourceState: State;
}

export enum AndroidHostPeripheral_PeripheralType {
  PERIPHERAL_TYPE_UNDEFINED = 0,
  PERIPHERAL_TYPE_THUNDERSYNC = 1,
  PERIPHERAL_TYPE_MONSOON = 2,
}

export function androidHostPeripheral_PeripheralTypeFromJSON(object: any): AndroidHostPeripheral_PeripheralType {
  switch (object) {
    case 0:
    case "PERIPHERAL_TYPE_UNDEFINED":
      return AndroidHostPeripheral_PeripheralType.PERIPHERAL_TYPE_UNDEFINED;
    case 1:
    case "PERIPHERAL_TYPE_THUNDERSYNC":
      return AndroidHostPeripheral_PeripheralType.PERIPHERAL_TYPE_THUNDERSYNC;
    case 2:
    case "PERIPHERAL_TYPE_MONSOON":
      return AndroidHostPeripheral_PeripheralType.PERIPHERAL_TYPE_MONSOON;
    default:
      throw new globalThis.Error(
        "Unrecognized enum value " + object + " for enum AndroidHostPeripheral_PeripheralType",
      );
  }
}

export function androidHostPeripheral_PeripheralTypeToJSON(object: AndroidHostPeripheral_PeripheralType): string {
  switch (object) {
    case AndroidHostPeripheral_PeripheralType.PERIPHERAL_TYPE_UNDEFINED:
      return "PERIPHERAL_TYPE_UNDEFINED";
    case AndroidHostPeripheral_PeripheralType.PERIPHERAL_TYPE_THUNDERSYNC:
      return "PERIPHERAL_TYPE_THUNDERSYNC";
    case AndroidHostPeripheral_PeripheralType.PERIPHERAL_TYPE_MONSOON:
      return "PERIPHERAL_TYPE_MONSOON";
    default:
      throw new globalThis.Error(
        "Unrecognized enum value " + object + " for enum AndroidHostPeripheral_PeripheralType",
      );
...
export const Drac: MessageFns<Drac> = {
  encode(message: Drac, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.vlan !== "") {
      writer.uint32(114).string(message.vlan);
    }
    if (message.ip !== "") {
...
function createBaseAndroidHostPeripheral(): AndroidHostPeripheral {
  return { name: "", serialNumber: "", peripheralType: 0, resourceState: 0 };
}

export const AndroidHostPeripheral: MessageFns<AndroidHostPeripheral> = {
  encode(message: AndroidHostPeripheral, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.name !== "") {
      writer.uint32(10).string(message.name);
    }
    ...
    if (message.peripheralType !== 0) {
      writer.uint32(24).int32(message.peripheralType);
    }
    if (message.resourceState !== 0) {
      writer.uint32(32).int32(message.resourceState);
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): AndroidHostPeripheral {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseAndroidHostPeripheral() as any;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
  ...
  fromJSON(object: any): AndroidHostPeripheral {
    return {
      name: isSet(object.name) ? globalThis.String(object.name) : "",
      serialNumber: isSet(object.serialNumber) ? globalThis.String(object.serialNumber) : "",
      peripheralType: isSet(object.peripheralType)
        ? androidHostPeripheral_PeripheralTypeFromJSON(object.peripheralType)
        : 0,
      resourceState: isSet(object.resourceState) ? stateFromJSON(object.resourceState) : 0,
    };
  },

  toJSON(message: AndroidHostPeripheral): unknown {
    const obj: any = {};
    if (message.name !== "") {
      obj.name = message.name;
    }
    ...
    if (message.peripheralType !== 0) {
      obj.peripheralType = androidHostPeripheral_PeripheralTypeToJSON(message.peripheralType);
    }
    if (message.resourceState !== 0) {
      obj.resourceState = stateToJSON(message.resourceState);
    }
  ...
  create(base?: DeepPartial<AndroidHostPeripheral>): AndroidHostPeripheral {
    return AndroidHostPeripheral.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<AndroidHostPeripheral>): AndroidHostPeripheral {
    const message = createBaseAndroidHostPeripheral() as any;
    message.name = object.name ?? "";
