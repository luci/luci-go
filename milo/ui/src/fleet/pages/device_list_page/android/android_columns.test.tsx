...
describe('Android Columns', () => {
  it('should generate proper order_by_fields for base dimensions mapped as labels', () => {
    const columns = getAndroidColumns(['model', 'id', 'state'], 'Android');
  ...
  it('should generate proper order_by_fields for average utilization columns', () => {
    const columns = getAndroidColumns(['average_7d', 'average_30d'], 'Android');
  ...
  it('should generate proper order_by_fields for Nlyte sync/update columns', () => {
    const columns = getAndroidColumns(
      ['ufs.nlyte_update_time', 'ufs.nlyte_last_sync'],
      'Android',
    );
  ...
  it('should generate proper order_by_fields and header for location_tag column', () => {
    const columns = getAndroidColumns(['location_tag'], 'Android');

    const locationTagCol = columns.find(
      (c) => (c.accessorKey || c.id) === 'location_tag',
    ) as AndroidColumnDef;
    ...
    expect(locationTagCol.meta?.infoTooltip).toBe(
      'Location tag / location group tag from UFS',
    );
  });
  ...
  it('should render celsius indicator for battery temperature column', () => {
    const columns = getAndroidColumns(['battery_temperature'], 'Android');
