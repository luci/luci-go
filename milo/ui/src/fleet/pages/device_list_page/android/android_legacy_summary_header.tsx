...
export interface AndroidLegacySummaryHeaderProps {
  aip160: string;
...
export function AndroidLegacySummaryHeader({
  aip160,
  setFiltersBatch,
  ...
  const legacyData = countQuery.data?.androidCount;
  ...
  const isError =
    countQuery.isError || onlineCountQuery.isError || offlineCountQuery.isError;
  const error =
    countQuery.error || onlineCountQuery.error || offlineCountQuery.error;

  const totalDevices = legacyData?.totalDevices || 0;
  const totalHosts = legacyData?.totalHosts || 0;
  const hostsRunning = legacyData?.labRunningHosts || 0;
  const hostsMissing = legacyData?.labMissingHosts || 0;
          ...
          >
            ...
            {showAvgUtilization &&
              renderUtilizationSection(
                legacyData?.average7d,
                legacyData?.average30d,
              )}
          </Grid>
