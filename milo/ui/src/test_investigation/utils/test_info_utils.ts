...
export function constructBlamelistCommitLink(
  ...
  const baseUrl = getBlamelistUrlFromVariantBranch(testVariantBranch);

  return baseUrl
    ? `${baseUrl}?expand=CP-${commitPosition}#CP-${commitPosition}`
    : undefined;
}
