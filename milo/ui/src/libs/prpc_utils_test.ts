...
describe('genCacheKeyForPrpcRequest', () => {
  it('should generate identical keys for identical requests', async () => {
    ...
    const key1 = await genCacheKeyForPrpcRequest('prefix', new Request(...fetchStub.mock.calls[0]));
    const key2 = await genCacheKeyForPrpcRequest('prefix', new Request(...fetchStub.mock.calls[1]));
  ...
  it('should generate different keys for requests with different critical headers', async () => {
    ...
    const key1 = await genCacheKeyForPrpcRequest('prefix', new Request(...fetchStub.mock.calls[0]), ['header-key']);
    const key2 = await genCacheKeyForPrpcRequest('prefix', new Request(...fetchStub.mock.calls[1]), ['header-key']);
    expect(key1).not.toStrictEqual(key2);
  });
  ...
  it('should generate different keys for requests with different default critical headers', async () => {
    ...
    const key1 = await genCacheKeyForPrpcRequest('prefix', new Request(...fetchStub.mock.calls[0]));
    const key2 = await genCacheKeyForPrpcRequest('prefix', new Request(...fetchStub.mock.calls[1]));
    const key3 = await genCacheKeyForPrpcRequest('prefix', new Request(...fetchStub.mock.calls[2]));
    const key4 = await genCacheKeyForPrpcRequest('prefix', new Request(...fetchStub.mock.calls[3]));
    expect(key1).not.toStrictEqual(key2);
  ...
  it('should generate different keys when prefix are different', async () => {
    ...
    const key1 = await genCacheKeyForPrpcRequest('prefix1', new Request(...fetchStub.mock.calls[0]));
    const key2 = await genCacheKeyForPrpcRequest('prefix2', new Request(...fetchStub.mock.calls[1]));
    expect(key1).not.toStrictEqual(key2);
  });
