...
describe('Build Page', () => {
  it('should navigate to the default tab', () => {
    ...
    cy.location('pathname').should(
      'equal',
      '/ui/p/chromium/builders/ci/linux-rel-swarming/15252/overview'
    );
  });

  it('should initiate the signin flow if the page is 404 and the user is not logged in', () => {
    cy.visit(
      '/p/not-bound-project/builders/not-bound-bucket/not-found-builder/12479'
    );
  ...
  it('should compute invocation ID from buildNum in URL', () => {
    ...
    cy.get('milo-build-page')
      ...
      .should(
        'eq',
        'build-70535a5a746775ce83281f4e4e318b2b7b239d1e7eb7c8f790bf570a14cf61fe-15252'
      );
  });
  ...
  it('should not break browser back button after internal redirection', () => {
    ...
    cy.location('pathname').should(
      'equal',
      '/ui/p/chromium/builders/ci/linux-rel-swarming/15253/overview'
    );
    cy.get('#summary-html'); // Ensure the overview tab is loaded.
    ...
    cy.location('pathname').should(
      'equal',
      '/ui/p/chromium/builders/ci/linux-rel-swarming/15253/test-results'
    );
