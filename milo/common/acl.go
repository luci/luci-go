...
// NewAccessClient returns a new AccessClient for talking to this milo instance's buildbucket instance.
// The only pRPC service exposed by buildbucket currently is the access API.
func NewAccessClient(c context.Context) (accessProto.AccessClient, error) {
	settings := GetSettings(c)
	if settings.Buildbucket.GetHost() == "" {
		return nil, errors.Reason("no buildbucket done found").Err()
	}
	t, err := auth.GetRPCTransport(c, auth.AsUser)
	if err != nil {
		return nil, errors.Annotate(err, "getting RPC Transport").Err()
	}
	return bbAccess.NewClient(settings.Buildbucket.Host, &http.Client{Transport: t}), nil
}

// BucketPermissions gets permissions for the current identity for all given buckets.
func BucketPermissions(c context.Context, client accessProto.AccessClient, buckets ...string) (bbAccess.Permissions, error) {
	perms := make(bbAccess.Permissions, len(buckets))

	// Set the namespace in the context for memcache.
	// TODO(mknyszek): Derive this namespace from the buildbucket host we're calling to.
	c, err := info.Namespace(c, "buildbucket-access")
	if err != nil {
		return nil, err
	}
		...
		if merr, ok := err.(errors.MultiError); ok {
			for i, err := range merr {
				if err != nil && err != memcache.ErrCacheMiss {
					logging.WithError(err).Warningf(c, "memcache get %s", entries[i])
				}
			}
		} else {
