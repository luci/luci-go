...
func GetAllConsoles(c context.Context, builderName string) ([]*Console, error) {
	itm, err := caching.RequestCache(c).GetOrCreate(c, consolesCacheKey(builderName), func() (interface{}, time.Duration, error) {
		...
		return con, 0, errors.
			Annotate(err, "getting consoles for %q", builderName).
			Tag(transient.Tag).
			Err()
	})
	con, _ := itm.([]*Console)
