...
func hasPermission(ctx context.Context, permission realms.Permission, realm string) (bool, error) {
	...
	auth.HasPermissionDryRun{
		ExpectedResult: true,
		TrackingBug:    "crbugs.com/1110314",
	}.Execute(ctx, permission, realm)

	// If the user has access to the project, by default they have access to all
	// consoles, until actual realms.cfg is added.
...
func filterConsoles(c context.Context, consoles []*Console, visibleProjects stringset.Set) ([]*Console, error) {
	...
	for _, con := range consoles {
		...
		allowed, err := hasPermission(c, permGetConsoles, con.Def.Realm)
		if err != nil {
			return nil, err
		}
		if !allowed {
...
func GetConsole(c context.Context, proj, id string) (*Console, error) {
	...
	case nil:
		switch allowed, err := hasPermission(c, permGetConsoles, con.Def.Realm); {
		case err != nil:
			return nil, err
		case !allowed:
			return nil, errors.Reason("caller does not have permission %s in realm of console %s", permGetConsoles, con.Def.Realm).Tag(grpcutil.PermissionDeniedTag).Err()
		default:
			return &con, nil
		}
	default:
