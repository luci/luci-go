...
type Project struct {
	...
	BusyBuilderIDs   []string

	// Tolerate unknown fields when fetching entities.
	_ datastore.PropertyMap `gae:"-,extra"`
}

// BuilderIsBusy checks if the builder is marked as busy in this project.
func (p *Project) BuilderIsBusy(builderID *buildbucketpb.BuilderID) bool {
	builderIDStr := bbProtoutil.FormatBuilderID(builderID)
	if p == nil {
		return false
	}
	for _, bid := range p.BusyBuilderIDs {
		if bid == builderIDStr {
			return true
		}
	}
...
func updateProject(c context.Context, cfg *configInterface.Config) (stringset.Set, error) {
	...
	project.BusyBuilderIDs = miloCfg.BusyBuilderIds
