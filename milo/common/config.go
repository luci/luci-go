...
var (
	permGetConsole = realms.RegisterPermission("milo.console.get")
)
...
func filterConsoles(c context.Context, consoles []*Console, visibleProjects stringset.Set) ([]*Console, error) {
	...
	for _, con := range consoles {
		...
		allowed, err := hasPermission(c, permGetConsole, con.Realm)
		if err != nil {
			return nil, err
		}
		if !allowed {
...
func GetConsole(c context.Context, proj, id string) (*Console, error) {
	...
	case nil:
		switch allowed, err := hasPermission(c, permGetConsole, con.Realm); {
		case err != nil:
			return nil, err
		case !allowed:
			return nil, errors.Reason("caller does not have permission %s in realm of console %s", permGetConsole, con.Realm).Tag(grpcutil.PermissionDeniedTag).Err()
		default:
			return &con, nil
		}
	default:
