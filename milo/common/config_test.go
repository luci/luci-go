...
func TestConfig(t *testing.T) {
	...
	Convey("Test Environment", t, func() {
		...
		Convey("Send update", func() {
			...
			Convey("Check user can't list consoles in realms they don't have access to", func() {
				cUser := auth.WithState(c, &authtest.FakeState{Identity: "user:e@example.com"})
				...
				So(err, ShouldBeNil)
				So(cons, ShouldHaveLength, 1)
				So(cons[0].ID, ShouldEqual, "default")
			})

			Convey("Check user can list consoles in realms they have access to", func() {
				cUser := auth.WithState(c, &authtest.FakeState{
					Identity: "user:e@example.com",
					IdentityPermissions: []authtest.RealmPermission{
						{Realm: "foo:fake_realm", Permission: permGetConsoles},
				...
				So(cons[0].ID, ShouldEqual, "default")
				So(cons[1].ID, ShouldEqual, "realm_test_console")
			})
