...
1. Create a CL and add release notes to the description, as follows:
    a. Get the commit hashes from the old and new versions. E.g. in channels.json,
    ...
    a. Navigate to the DEPS file in the corresponding commits on Gitiles, e.g.
    ...
    a. Check the pinned commit of [infra/luci/luci-go] in the DEPS file at these
    ...
    a. Run git log between these two commits:
        ```
        git log 2bdb75fedc32..a4f26ffd812e --date=short --first-parent --format='%ad %ae %s'
        ```
    a. Add the resulting command line and output to the CL description. Example:
