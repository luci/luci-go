...
func (s *MiloInternalService) QueryBlamelist(ctx context.Context, req *milopb.QueryBlamelistRequest) (_ *milopb.QueryBlamelistResponse, err error) {
	...
	startCommitish, err := prepareQueryBlamelistRequest(req)
	if err != nil {
	...
	commits, err := git.Get(ctx).Log(ctx, req.GitilesCommit.Host, req.GitilesCommit.Project, startCommitish, opts)
	if err != nil {
		return nil, err
	}
...
//  * extracts start startCommitish from page token or gittles commit.
func prepareQueryBlamelistRequest(req *milopb.QueryBlamelistRequest) (startCommitish string, err error) {
	switch {
	case req.PageSize < 0:
