...
func (s *MiloInternalService) QueryBlamelist(ctx context.Context, req *milopb.QueryBlamelistRequest) (*milopb.QueryBlamelistResponse, error) {
	...
	err = parallel.WorkPool(8, func(c chan<- func() error) {
		for i, commit := range commits[1:] { // skip the first commit... it's us!
			newBlameSize := int32(i + 1) // +1 since we skipped the first one

			// We have already found a build before this commit, no point looking
			// further.
			if newBlameSize >= atomic.LoadInt32(&blameLengthInt32) {
				break
			}
			...
			c <- func() error {
				...
				if len(builds) > 0 {
					m.Lock()
					if newBlameSize < blameLengthInt32 {
						blameLengthInt32 = newBlameSize
					}
					m.Unlock()
				}
				return nil
