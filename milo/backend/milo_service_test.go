...
func TestPrepareQueryBlamelistRequest(t *testing.T) {
	...
	Convey(`TestPrepareQueryBlamelistRequest`, t, func() {
		Convey(`extract commit ID correctly`, func() {
			startCommitID, _, err := prepareQueryBlamelistRequest(&milopb.QueryBlamelistRequest{
		...
		Convey(`extract commit ID correctly when there's a page token`, func() {
			commitID, hash, err := prepareQueryBlamelistRequest(&milopb.QueryBlamelistRequest{
				GitilesCommit: &buildbucketpb.GitilesCommit{
					Host:    "host",
					Project: "project/src",
					Id:      "commit-id-1",
			...
			nextCommitID, _, err := prepareQueryBlamelistRequest(&milopb.QueryBlamelistRequest{
				GitilesCommit: &buildbucketpb.GitilesCommit{
					Host:    "host",
					Project: "project/src",
					Id:      "commit-id-1",
		...
		Convey(`coerce page_size to 1000 if it's greater than 1000`, func() {
			...
			_, _, err := prepareQueryBlamelistRequest(req)
			So(err, ShouldBeNil)
			So(req.PageSize, ShouldEqual, 1000)
		})

		Convey(`set page_size to 100 if it's not set`, func() {
			req := &milopb.QueryBlamelistRequest{
				GitilesCommit: &buildbucketpb.GitilesCommit{
					Host:    "host",
					Project: "project/src",
			...
			_, _, err := prepareQueryBlamelistRequest(req)
			So(err, ShouldBeNil)
			So(req.PageSize, ShouldEqual, 100)
		})
	})
}
...
func TestQueryBlamelist(t *testing.T) {
	...
	Convey(`TestQueryBlamelist`, t, func() {
		...
		createFakeBuild := func(builder *buildbucketpb.BuilderID, buildNum int, commitID string) *model.BuildSummary {
			builderID := buildbucket.LegacyBuilderIDString(builder)
