...
type Console struct {
	...
	// literal prefix "refs/release-" only contains one slash. The regexp must
	// match full ref name and ^ and $ will be added automatically if not present.
	// Eg. refs/heads/master, regexp:refs/branch-heads/\d+.\d+
	Ref string `protobuf:"bytes,4,opt,name=ref" json:"ref,omitempty"`
	// ExcludeRef is a ref, commits from which are to be ignored even when they
	// are reachable from the ref specified above. This uses the same format as
	// ref field above including the ability to specify a regular expression via
	// with "regexp:" prefix.
	//
	// E.g. the config below allows to track commits from all release
