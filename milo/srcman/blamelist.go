...
type historyWorkItem struct {
	...
	toUpdate *milo.ManifestDiff_GitCheckout
	ok       bool
}

func (i *historyWorkItem) process() error {
	opts := []gitiles.LogOption{gitiles.Limit(100)}
	if i.withFiles {
		opts = append(opts, gitiles.WithTreeDiff)
	}
	log, err := i.client.Log(i.c, i.toUpdate.RepoUrl, i.treeish, opts...)
	if err != nil {
		logging.WithError(err).Warningf(i.c, "fetching log - %q", i.dirname)
		return nil
	}

	i.toUpdate.History, err = gitiles.LogProto(log)
	if err != nil {
		logging.WithError(err).Warningf(i.c, "protoizing log - %q", i.dirname)
		return nil
	}
...
func PopulateHistory(c context.Context, diff *milo.ManifestDiff, withFiles bool) (ok bool) {
	...
	switch len(workList) {
	case 0:
		logging.Infof(c, "no work items to process")
	case 1:
		logging.Infof(c, "one work item to process")
		_ = workList[0].process() // always returns nil, logs all errors itself
	default:
		logging.Infof(c, "%d work items to process", len(workList))
		limit := 8
		...
		// nil.
		_ = parallel.RunMulti(c, limit, func(mr parallel.MultiRunner) error {
			return mr.RunMulti(func(ch chan<- func() error) {
				for _, wi := range workList {
					ch <- wi.process
				}
			})
		})
