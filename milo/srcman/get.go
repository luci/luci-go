...
// from logdog.
func Get(ctx context.Context, a *milo.Step_ManifestLink) (*source_manifest.Manifest, []byte, error) {
	if len(a.Sha256) == sha256.Size {
		// cached, mebbeh?
		entry := &sourceManifest{ID: base128.EncodeToString(a.Sha256)}
		...
		case nil:
			...
			// TODO(iannucci): I'm not sure if this is a good idea or not, but leaning
			// towards the side of deleting bad data from datastore.
			logging.WithError(err).Warningf(ctx, "bad sourceManifest(%x), deleting...", a.Sha256)
			if err := datastore.Delete(ctx, entry); err != nil {
				logging.WithError(err).Warningf(ctx, "failed to delete bad cache entry")
			}

		default:
			return nil, nil, errors.Annotate(err, "reading sourceManifest %x", a.Sha256).Tag(transient.Tag).Err()
	...
	entry := &sourceManifest{ID: base128.EncodeToString(dataSha[:]), Data: data}
	ret, err := entry.Read()
	if err != nil {
