...
var (
	...
	getHistoryCounter = metric.NewCounter(
		"luci/milo/git/GetHistory/cache_hit",
...
func GetHistory(c context.Context, url, commitish string, limit int) (*milo.ConsoleGitInfo, error) {
	...
	if err := getHistoryCounter.Add(c, 1, cacheHit, url, ref); err != nil {
		logging.WithError(err).Errorf(c, "failed to increment getHistoryCounter")
	}
	return ret, err
}
