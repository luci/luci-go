...
func loadCacheFromDS(c context.Context, host, project, excludeRef string, limit int, refTips map[string]string) (cachedLogs map[string][]*gitpb.Commit, missingRefs []string) {
	...
	for _, item := range items {
		if _, ok := cachedLogs[item.ref]; !ok {
			missingRefs = append(missingRefs, item.ref)
		}
	}
...
func (impl *implementation) loadLogsForRefs(c context.Context, host, project, excludeRef string, limit int, refTips map[string]string) (logs [][]*gitpb.Commit, err error) {
	cachedLogs, missingRefs := loadCacheFromDS(c, host, project, excludeRef, limit, refTips)
