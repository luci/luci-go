...
type logCache struct {
	Key    string   `gae:"$id"`
	Commit string   `gae:"commit"`
	Log    [][]byte `gae:"commits"`
}
	...
	if err = parallel.FanOutIn(func(ch chan<- func() error) {
		for ref, commit := range refTips {
			...
			ch <- func() error {
				...
				if err := datastore.Get(c, &lc); err != datastore.ErrNoSuchEntity && lc.Commit == commit {
					// Parse cached result from datastore.
					log = make([]*gitpb.Commit, 0, len(lc.Log))
					for _, commitData := range lc.Log {
					...
					lc.Commit = commit
					lc.Log = make([][]byte, 0, len(log))
					for _, commit := range log {
						marshalled, err := proto.Marshal(commit)
						if err != nil {
							return err
						}
						lc.Log = append(lc.Log, marshalled)
					}
					datastore.Put(c, &lc)
				}
