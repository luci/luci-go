...
type Client interface {
	...
	// LatestCommits returns latest commits reachable from several refs.
	//
	// Returns up a slice of up to a limit (defaults to 50 when negative) of
	// commits such that:
	//  * for each source ref, subsequence of returned slice formed by
	//    its corresponding commits will be topological.
	//  * across refs, commits will be ordered by timestamp.
	//
