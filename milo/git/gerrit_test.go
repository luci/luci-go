...
func TestFetchGerritChangeEmail(t *testing.T) {
	...
	Convey("FetchGerritChangeEmail", t, func() {
		...
		impl := implementation{
			...
			acls: &ACLs{
				hosts: map[string]*hostACLs{
					"host": &hostACLs{itemACLs: itemACLs{readers: []string{"user:allowed@example.com"}}},
				},
			},
		}
		c = Use(c, &impl)
		...
		_, err := impl.FetchGerritChangeEmail(cDenied, "host", 123)
		...
		Convey("Happy cached path", func() {
			email, err := impl.FetchGerritChangeEmail(cAllowed, "host", 123)
			So(err, ShouldBeNil)
