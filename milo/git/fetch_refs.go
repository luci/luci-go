...
func (p *implementation) FetchRefs(c context.Context, host, project, excludeRef string, refConfigs []string, limit int) (commits []*gitpb.Commit, err error) {
	...
	if err = parallel.FanOutIn(func(work chan<- func() error) {
		wr.ForEachNamespace(func(refsName string) {
			work <- func() error {
				return schedulerGitiles.FetchCurrentTips(c, func() (*gitilespb.RefsResponse, error) {
					req := gitilespb.RefsRequest{Project: project, RefsPath: refsName}
