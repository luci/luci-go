...
func TestTagError(t *testing.T) {
	...
	Convey("tagError Works", t, func() {
		...
		So(grpcutil.Code(tagError(cAnon, errGRPCNotFound)), ShouldEqual, codes.Unauthenticated)
		So(grpcutil.Code(tagError(cUser, errGRPCNotFound)), ShouldEqual, codes.NotFound)
	})
}
