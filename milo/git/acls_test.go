...
func TestACLsWork(t *testing.T) {
	...
	Convey("ACLs work", t, func() {
		...
		Convey("ReadGranted works", func() {
			acls := load(
				&config.Settings_SourceAcls{
					Hosts:    []string{"all.googlesource.com"},
					Projects: []string{"b.googlesource.com/all"},
					Readers:  []string{"group:all"},
				},
				&config.Settings_SourceAcls{
					Hosts:    []string{"b.googlesource.com"},
					Projects: []string{"c.googlesource.com/only"},
					Readers:  []string{"group:some", "they@example.com"},
				},
			)
			granted := func(ctx context.Context, host, project string) bool {
				r, err := acls.ReadGranted(ctx, host, project)
				if err != nil {
					panic(err)
				}
				return r
			...
			So(granted(cAnon, "all.googlesource.com", "any"), ShouldBeTrue)
			So(granted(cAnon, "b.googlesource.com", "all"), ShouldBeTrue)
			So(granted(cAnon, "b.googlesource.com", "any"), ShouldBeFalse)

			cThey := auth.WithState(c, &authtest.FakeState{
				Identity:       "user:they@example.com",
				IdentityGroups: []string{"all"},
			})
			So(granted(cThey, "b.googlesource.com", "all"), ShouldBeTrue)
			So(granted(cThey, "b.googlesource.com", "any"), ShouldBeTrue)
			So(granted(cThey, "c.googlesource.com", "only"), ShouldBeTrue)
			...
			So(granted(cSome, "b.googlesource.com", "any"), ShouldBeTrue)
			So(granted(cSome, "c.googlesource.com", "only"), ShouldBeTrue)
