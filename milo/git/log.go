...
//
// The first byte of a cache value specifies how to interpret the rest:
...
//           The first byte indicates the distance between the two commits.
...
func (l *logReq) readCache(c context.Context) (cacheResult string, commits []*gitpb.Commit, ok bool) {
	...
	for {
		...
		case dist+e.Value()[0] <= maxDist:
			// first byte is distance, and the referenced cache is sufficient for reuse
			dist += e.Value()[0]
			descendant := hex.EncodeToString(e.Value()[1:])
			logging.Debugf(c, "recursing into cache %s with %d distance", descendant, e.Value()[0])
