...
// Log makes returns commits from the given repository
// host (e.g. "chromium.googlesource.scom"), project (e.g. "chromium/src") and
// commitish (e.g. "refs/heads/master").
//
...
func Log(c context.Context, host, project, commitish string, limit int) ([]*gitpb.Commit, error) {
	...
	add := func(page []*gitpb.Commit) {
		...
		case len(commits) == 0:
			commits = page
		case len(page) == 0:
		case page[0].Id == commits[len(commits)-1].Id:
	...
	for remaining > 100 {
		...
		case err != nil:
			return commits, err
		case len(page) < 100:
			// This can only happen if there are no more commits
			add(page)
			return commits, nil
		case len(page) > 100:
			panic("impossible") // log never returns >100 commits
		default:
	...
	// last page. One logReq can handle all of it.
	req.min = remaining
	page, err := req.call(c)
	if err != nil {
		return commits, err
...
func (l *logReq) readCache(c context.Context) (cacheResult string, commits []*gitpb.Commit, ok bool) {
	...
	for {
		...
		default:
			...
			default:
				// TODO(nodir): if we need commits C20..C10, but we have only
				// C25..C15, instead of discarding C20..C15 and loading C20..C10
				// reuse C20..C15 and fetch C15..C5.
