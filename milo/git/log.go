...
func Log(c context.Context, host, project, commitish string, min int) ([]*gitpb.Commit, error) {
	...
	case min <= 0:
		min = 50
	case min > 100:
		panic(fmt.Sprintf("min cannot be > 100"))
	}
...
// logReq implements Log().
//
// Cache implementation detail:
// This function attempts to cache commits not only keyed by the requested committish,
// but also ancestors, for example if received 100 ancestor
// commits C1, C2, C4, ..., C100 from a commit C100,
// cache with C1, C2, ..., C99 as keys too.
//
// The first byte of a cache value specifies how to intepret the rest:
//  0:       the rest if protobuf-marshalled gitilespb.LogResponse message
//  1-99:    the rest is byte id of a a child commit that
//           the commit-in-cache-key is reachable from.
//           The first byte indicates the distance between two commits.
...
func (l *logReq) readCache(c context.Context) (cacheResult string, commits []*gitpb.Commit, ok bool) {
	...
	for {
		...
		case e.Value()[0] == 0:
			...
			if err := proto.Unmarshal(e.Value()[1:], &decoded); err != nil {
		...
		case e.Value()[0] >= 100:
			logging.WithError(err).Errorf(c, "unexpected first byte %d in cache value at key %q", e.Value()[0], e.Key())
			cacheResult = decodingFailure

		case dist+e.Value()[0] <= maxDist: // first byte is distance, and the referenced cache is sufficient for reuse
			dist += e.Value()[0]
			descendant := hex.EncodeToString(e.Value()[1:])
			logging.Debugf(c, "recursing into cache with %d distance: %q", e.Value()[0], descendant)
...
func (l *logReq) writeCache(c context.Context, res *gitilespb.LogResponse) {
	...
	if err != nil {
		logging.WithError(err).Errorf(c, "failed to marshal gitiles log\n%#v", res.Log)
		return
	}

	cacheValue := make([]byte, len(marshalled)+1)
	copy(cacheValue[1:], marshalled)
	l.commitishEntry.SetValue(cacheValue)
	...
	if !l.commitishIsHash && len(res.Log) > 0 {
		...
		hashCache := l.mkCache(c, hex.EncodeToString(res.Log[0].Id))
		hashCache.SetValue(l.commitishEntry.Value())
		caches = append(caches, hashCache)
	}

	if len(res.Log) > 1 {
		// Also potentially cache with ancestor as cache keys.
		ancestorCaches := make([]memcache.Item, len(res.Log)-1)
		for i := 1; i < len(res.Log); i++ {
			ancestorCaches[i-1] = l.mkCache(c, hex.EncodeToString(res.Log[i].Id))
		}
