...
// FetchGerritChangeEmail implements Source interface.
func (p *implementation) FetchGerritChangeEmail(c context.Context, host string, changeNumber int64) (string, error) {
	...
	case err != nil:
		return "", err
	case allowed:
		return changeInfo.GetOwner().GetEmail(), err
	default:
		return "", status.Errorf(codes.NotFound, "https://%s/%d not found or no access", host, changeNumber)
	}
}

// changeEmailAndProject fetches and caches Gerrit's change owner email and project.
//
// Cache doesn't expire because we expect change's owner and project to be
// immutable after change is created.
//
