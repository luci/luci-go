...
func TestFetchRefs(t *testing.T) {
	...
	Convey("FetchRefs", t, func() {
		...
		Convey("refSuffix must be required", func() {
			_, err := impl.FetchRefs(
				cAllowed, host, "project", "refs/heads/master", "refs/branch-heads/", nil, 50)
		...
		suffixRegexp, err := regexp.Compile(`\d+\.\d+`)
		So(err, ShouldBeNil)

		Convey("ACLs respected", func() {
			_, err := impl.FetchRefs(
				cDenied, host, "project", "refs/heads/master", "refs/branch-heads/",
				suffixRegexp, 50)
		...
		Convey("one ref matches", func() {
			...
			commits, err := impl.FetchRefs(
				cAllowed, host, "project", "refs/heads/master", "refs/branch-heads/",
				suffixRegexp, 50)
			So(err, ShouldBeNil)
