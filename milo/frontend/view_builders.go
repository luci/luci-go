...
func getHistory(c context.Context, builderID buildsource.BuilderID, project string, limit int) (*builderHistory, error) {
	...
	return &hist, parallel.FanOutIn(func(fetch chan<- func() error) {
		...
		// fail to find it). If no BuilderSummary is found, fall back on passed
		// project.
		q := datastore.NewQuery("BuilderSummary").Eq("BuilderID", builderID)
		var matchingBuilder model.BuilderSummary
