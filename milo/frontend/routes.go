...
func Run(templatePath string) {
	...
	r.GET("/p/:project/builders/:bucket/:builder/:number", htmlMW, func(c *router.Context) {
		BuildHandler(c, &buildbucket.BuildID{
			Project: c.Params.ByName("project"),
			Address: fmt.Sprintf("%s/%s/%s", c.Params.ByName("bucket"), c.Params.ByName("builder"), c.Params.ByName("number")),
		})
	})

	// Console
	r.GET("/p/:project/consoles/:name", htmlMW, ConsoleHandler)
	r.GET("/console/:project/:name", htmlMW, movedPermanently("/p/:project/consoles/:name"))
	r.GET("/p/:project/consoles", htmlMW, func(c *router.Context) {
		ConsolesHandler(c, c.Params.ByName("project"))
	})
	r.GET("/console/:project", htmlMW, movedPermanently("/p/:project/consoles/"))
	...
	r.GET("/p/:project/builders/:bucket/:builder", htmlMW, func(c *router.Context) {
		...
		// Requires deleting the redirect for /buildbucket/:bucket/:builder
	...
	r.GET("/buildbucket/:bucket/:builder", htmlMW, movedPermanently("/p/chromium/builders/:bucket/:builder"))
