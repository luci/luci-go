...
async function prefetchArtifact(reqUrl: URL) {
  ...
  const [invocationId, , testId, resultId, artifactId] = match.slice(2, 7).map((v) => decodeURIComponent(v || ''));
  ...
  prefetchResultDBService
    .getArtifact(
      { name: constructArtifactName({ invocationId, testId, resultId, artifactId }) },
      // Bypass the service cache but trigger the cachedFetch cache.
