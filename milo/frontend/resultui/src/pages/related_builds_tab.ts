...
export class RelatedBuildsTabElement extends MobxLitElement {
  ...
  private generateBuildLink(build: Build) {
    const href = `/ui/p/${build.builder.project}/${build.builder.bucket}/${build.builder.builder}/b${build.id}`;
  ...
  private displayTimestamp(t: Timestamp): string {
    var d = new Date(t.seconds * 1000);
    return d.toString();
  }

  // TODO (crbug.com/1112224): format and extract this to a library
  private displayDuration(beginTime: Timestamp, endTime: Timestamp): string {
    return (endTime.seconds - beginTime.seconds) + " secs";
  }
