...
export class TestResultsTabElement extends MobxLitElement {
  ...
  private get hasSingleVariant() {
    // the complexity should be low since the iterator is executed only when there's only one test.
    ...
    return html`
      ...
      <div id="main">
        ...
        <div id="test-result-view">
          ${repeat(iter.withPrev(iter.flat(iter.map(state.selectedNode.tests(), (t) => t.variants))), ([v]) => `${v.testId} ${v.variantKey}`, ([v, prev]) => html`
          <tr-variant-entry
            .variant=${v}
