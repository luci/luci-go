...
export class BlamelistTabElement extends MobxLitElement {
  ...
  private get latestCommitId() {
    return this.buildState.selectedBlamelistPin?.id;
  }
  ...
  private get gitilesLink() {
    if (this.revisionRange && this.buildState.selectedBlamelistPinRepoURL) {
      return `${this.buildState.selectedBlamelistPinRepoURL}/+log/${this.revisionRange}`;
    }
    return '';
  }
  ...
  protected render() {
    if (this.buildState.build && !this.buildState.selectedBlamelistPin) {
      return html`
        <div id="no-blamelist">
    ...
    return html`
      <div id="header">
        <div id="repo-selector">
          ...
          <select
            id="repo-select"
            @input=${(e: InputEvent) => this.buildState.selectedBlamelistPinIndex = Number((e.target as HTMLOptionElement).value)}
          >
            ${this.buildState.build?.blamelistPins.map((pin, i) => html`
            <option value=${i} ?selected=${this.buildState.selectedBlamelistPinIndex === i}>${getGitilesRepoURL(pin)}</option>
            `)}
          </select>
        </div>
      ...
      <div id="main">
        ...
        <milo-commit-entry
          .number=${i + 1}
          .repoUrl=${this.buildState.selectedBlamelistPinRepoURL}
          .commit=${commit}
          .expanded=${false}
        ></milo-commit-entry>
        `)}
