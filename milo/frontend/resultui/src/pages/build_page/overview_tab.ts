    ...
    switch (bpd.status) {
      case BuildStatus.Scheduled:
        return html`
          ...
          <span class="status ${STATUS_CLASS_MAP.get(bpd.status)}">
            ${STATUS_DISPLAY_MAP.get(bpd.status)}
          </span>
          since ${displayTimestamp(bpd.create_time)}
        `;
      case BuildStatus.Started:
        return html`
          ...
          since ${displayTimestamp(bpd.start_time)}
        `;
      case BuildStatus.Canceled:
        return html`
          ...
          after ${displayDuration(bpd.start_time, bpd.end_time)}
          by ${bpd.canceled_by}
        `;
        ...
        <table>
          ${input.gitiles_commit ? html`
            <tr>
              <td>Revision:</td>
              <td>
                <a href=${getURLForGitilesCommit(input.gitiles_commit)}>${input.gitiles_commit.id}</a>
  ...
  private renderInfra() {
    ...
    return html`
      <div>
        <h3>Infra</h3>
        <table>
          <tr><td>Buildbucket ID:</td><td><tr-link .link=${bpd.buildbucket_link}></td></tr>
          ...
          <tr>
            <td>Bot:</td>
            <td>${botLink ? html`<tr-link .link=${botLink}></tr-link>` : 'N/A'}</td>
          </tr>
          ` : ''}
          <tr><td>Recipe:</td><td><tr-link .link=${bpd.recipe_link}></tr-link></td></tr>
        </table>
      </div>
    `;
  }
