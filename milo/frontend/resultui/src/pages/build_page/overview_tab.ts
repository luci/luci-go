  ...
  private renderTiming() {
    ...
    return html`
      <div>
        <h3>Timing</h3>
        <table>
          <tr><td>Created:</td><td>${build.createTime.toFormat(LONG_TIME_FORMAT)}</td></tr>
          <tr><td>Started:</td><td>${build.startTime?.toFormat(LONG_TIME_FORMAT) || 'N/A'}</td></tr>
          <tr><td>Ended:</td><td>${build.endTime?.toFormat(LONG_TIME_FORMAT) || 'N/A'}</td></tr>
