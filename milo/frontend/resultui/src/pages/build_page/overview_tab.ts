...
export class OverviewTabElement extends MobxLitElement {
  ...
  private renderActionButtons() {
    ...
    const canRetry = this.buildState.canPerform('ADD_BUILD');
    const canCancel = this.buildState.canPerform('CANCEL_BUILD');
    ...
    if (build.endTime) {
      return html`
        ...
        <div title=${canRetry ? '' : 'You have no permission to retry this build.'}>
          <mwc-button
            dense unelevated
            @click=${() => this.showRetryDialog = true}
            ?disabled=${!canRetry}
          >Retry Build</mwc-button>
        </div>
      `;
    }
    ...
    return html`
      <h3>Actions</h3>
      <div title=${canRetry ? '' : 'You have no permission to cancel this build.'}>
        <mwc-button
          dense unelevated
          @click=${() => this.showCancelDialog = true}
          ?disabled=${!canCancel}
        >Cancel Build</mwc-button>
      </div>
    `;
  }
