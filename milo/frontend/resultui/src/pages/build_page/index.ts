...
export class BuildPageElement extends MobxLitElement implements BeforeEnterObserver {
  ...
  // When buildIdParam is set, the page is visited with a short link.
  private buildIdParam!: string;
  private urlSuffix = '';
  ...
  onBeforeEnter(location: RouterLocation, cmd: PreventAndRedirectCommands) {
    ...
    if (typeof buildId === 'string' && path instanceof Array) {
      this.buildIdParam = buildId as string;
  ...
  @computed private get documentTitle() {
    ...
    if (this.buildIdParam) {
      return `${statusDisplay} - ${this.buildIdParam}`;
    } else {
  ...
  connectedCallback() {
    ...
    this.disposers.push(reaction(
      ...
      () => {
        ...
        if (this.buildIdParam) {
          this.buildState.buildId = this.buildIdParam;
        } else if (this.buildNumOrIdParam.startsWith('b')) {
          this.buildState.buildId = this.buildNumOrIdParam.slice(1);
        } else {
          this.buildState.buildNum = Number(this.buildNumOrIdParam);
        }
        ...
        () => {
          ...
          const buildUrl = router.urlForName(
            'build',
            {
              ...
              build_num_or_id: `b${this.buildIdParam}`,
            },
          );
  ...
  protected render() {
    if (this.buildIdParam || this.prerender) {
      return html``;
    }
