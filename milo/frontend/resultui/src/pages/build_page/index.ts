  ...
  protected render() {
    return html`
      ...
      <mwc-dialog
        ...
        @closed=${() => {
          const noFeedbackEle = this.shadowRoot!.getElementById('no-feedback')! as HTMLInputElement;
          if (noFeedbackEle.checked) {
            this.configsStore.userConfigs.showFeedbackDialog = false;
            this.configsStore.save();
          }
      ...
      >
        <div>
          ...
          Take a moment to give us feedback before switching back to the old build page.
        </div>
        <br>
        <input type="checkbox" id="no-feedback">
        <label for="no-feedback">Don't show again</label>
        <mwc-button
          slot="primaryAction"
          dense
          unelevated
      ...
      <div id="build-summary">
        ...
        <a
          @click=${(e: Event) => {
            ...
            if (this.configsStore.userConfigs.showFeedbackDialog) {
