...
export class TimelineTabElement extends MiloBaseElement {
  ...
  private renderSidePanel() {
    ...
    for (const [i, listNum, step] of traverseStepList(build.steps)) {
      ...
      this.bindStepInfoTooltip(rect, step);
    ...
    for (const [i, listNum, step] of traverseStepList(build.steps)) {
      ...
      window.setTimeout(() => {
        ...
        stepText.each(function () {
          ...
          timelineTab.bindStepInfoTooltip(eventTargetRect, step);
        });
      }, 10);
    }
   ...
   */
  private bindStepInfoTooltip<T extends BaseType>(ele: Selection<T, unknown, null, undefined>, step: StepExt) {
