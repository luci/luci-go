...
export class ArtifactPageElement extends MobxLitElement implements BeforeEnterObserver {
  ...
  @computed private get invocationId() {
    return decodeURIComponent(this.artifactName.match(/invocations\/(?<inv_id>.*?)\//)!.groups!['inv_id']);
  }
  ...
  protected render() {
    return html`
      ...
      ></tr-status-bar>
      <div id="details">
        <a href=${router.urlForName('invocation', {'invocation_id': this.invocationId})}>View Invocation</a><br>
