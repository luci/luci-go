...
export class TestResultsTabElement extends MobxLitElement implements BeforeEnterObserver {
  ...
  connectedCallback() {
    ...
    if (searchParams.has('clean')) {
      this.invocationState.showEmptySections = false;
    }
    ...
    this.disposers.push(reaction(
      () => {
        const newSearchParams = new URLSearchParams({
          ...
          ...this.invocationState.showEmptySections ? {} : {'clean': ''},
        });
    ...
    return html`
      <div id="header">
        <div class="filters-container">
          <input
            id="empty-sections-checkbox"
            type="checkbox"
            ?checked=${this.invocationState.showEmptySections}
            @change=${(e: MouseEvent) => {
              this.invocationState.showEmptySections = (e.target as HTMLInputElement).checked;
              this.configsStore.save();
            }}
          >
          <label
            for="empty-sections-checkbox"
            title="Show status sections with no tests."
          >
            Empty Sections
      ...
      <div id="main" class=${classMap({'show-empty-sections': this.invocationState.showEmptySections})}>${this.renderMain()}</div>
    `;
  }
  ...
  static styles = css`
    ...
    .section-header.expanded:not(.empty) {
      border-bottom: 1px solid var(--divider-color);
    }
    .section-header.empty {
      display: none;
    }
    .show-empty-sections .section-header.empty {
      display: block;
    }
