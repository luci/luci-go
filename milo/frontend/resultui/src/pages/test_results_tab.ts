...
export class TestResultsTabElement extends MobxLitElement implements BeforeEnterObserver {
   ...
   */
  private async loadNextPage(status?: TestVariantStatus) {
    try {
      if (status) {
  ...
  private renderAllVariants() {
    ...
    return html`
      ...
      ${multiSections ? '' : html`
      <hr class="divider" style=${styleMap({'display': this.totalDisplayedVariantCount === 0 ? 'none' : ''})}>
      <div id="variant-list-tail">
        Showing ${this.totalDisplayedVariantCount} tests.
        <span
          class="active-text"
