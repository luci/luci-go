...
export class TestResultsTabElement extends MobxLitElement implements BeforeEnterObserver {
  ...
  async loadNextPage(untilStatus?: TestVariantStatus) {
    try {
      ...
      if (this.shouldForceLoadingTest && this.invocationState.totalDisplayedVariantCount === 0) {
        this.loadNextPage();
      }
    } catch (e) {
  ...
  connectedCallback() {
    ...
    if (searchParams.has('force-test')) {
    ...
    this.disposers.push(reaction(
      () => {
        const newSearchParams = new URLSearchParams({
          ...
          ...this.invocationState.forcedLoadingTest ? {'force-test': ''} : {},
        });
