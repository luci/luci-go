...
export class TestResultsTabElement extends MobxLitElement implements BeforeEnterObserver {
  ...
  private renderAllVariants() {
    ...
    const groupingKeys = this.invocationState.groupingKeys;
    const groupingKeyGetters = this.invocationState.groupingKeyGetters;
    return html`
      ${(testLoader?.groupedNonExpectedVariants || []).map((group) =>
        this.renderVariantGroup(
          groupingKeys.map((k, i) => [k, groupingKeyGetters[i](group[0])]),
          group
        )
      )}
  ...
  private renderVariantGroup(groupDef: [string, unknown][], variants: TestVariant[]) {
    const groupKey = JSON.stringify(groupDef);
    const expanded = computed(() => !this.collapsedVariantGroups.has(groupKey)).get();
    return html`
      <div
        class=${classMap({
          expanded,
        ...
        @click=${() => {
          if (expanded) {
            this.collapsedVariantGroups.add(groupKey);
          } else {
            this.collapsedVariantGroups.delete(groupKey);
          }
        }}
      >
