      ...
      >
        <table>
          <tr>
            ...
            <td>
              <input
                value=${this.uncommittedColumnKeys.join(', ')}
                placeholder="A list of comma separated property keys (e.g. v.test_suite,v.gpu)."
                @input=${(e: InputEvent) =>
                  (this.uncommittedColumnKeys = (e.target as HTMLInputElement).value
                    .split(',')
                    .map((k) => k.trim())
                    .filter((k) => k !== ''))}
              />
          ...
          <tr>
            ...
            <td>
              <input
                value=${this.uncommittedSortingKeys.join(', ')}
                placeholder="A list of comma separated property keys (e.g. v.test_suite,v.gpu)."
                @input=${(e: InputEvent) =>
                  (this.uncommittedSortingKeys = (e.target as HTMLInputElement).value
                    .split(',')
                    .map((k) => k.trim())
                    .filter((k) => k !== ''))}
              />
          ...
          <tr>
            ...
            <td>
              <input
                value=${this.uncommittedGroupingKeys.join(', ')}
                placeholder="A list of comma separated property keys (e.g. v.test_suite,v.gpu)."
                @input=${(e: InputEvent) =>
                  (this.uncommittedGroupingKeys = (e.target as HTMLInputElement).value
                    .split(',')
                    .map((k) => k.trim())
                    .filter((k) => k !== ''))}
              />
