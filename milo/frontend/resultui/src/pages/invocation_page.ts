...
export class InvocationPageElement extends MobxLitElement implements BeforeEnterObserver {
  ...
  connectedCallback() {
    ...
    this.disposers.push(reaction(
      () => {
        if (this.selectedNode.finalized || this.testLoader.isLoading) {
          return 0;
        }
        return Math.max(this.pageLength - this.selectedNode.allTests.length, 0);
      },
      ...
      <div id="main" class=${classMap({'show-left-panel': this.leftPanelExpanded})}>
        ...
        <div id="test-result-view">
          ...
          <div id="test-result-content">
            ...
            `)}
            <div id="list-tail">
              <span>Showing ${Math.min(this.selectedNode.allTests.length, this.pageLength)}/${this.selectedNode.allTests.length}${this.selectedNode.finalized ? '' : '+'} tests.</span>
              <span
                id="load-more"
                style=${styleMap({'display': this.pageLength >= this.selectedNode.allTests.length && this.selectedNode.finalized ? 'none' : ''})}
