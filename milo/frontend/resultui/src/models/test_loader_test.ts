...
import { deepStripNotIn } from '../libs/test_utils';
...
describe('test_loader', () => {
  ...
  const testResult2 = {testId: 'a', resultId: '2', variant: {def: {'key1': 'val1'}}} as Partial<TestResult> as TestResult;
  // Result with the same test ID and a different variant.
  const testResult3 = {testId: 'a', resultId: '3', variant: {def: {'key1': 'val2'}}} as Partial<TestResult> as TestResult;

  // Result with a different test ID and the same variant.
  const testResult4 = {testId: 'b', resultId: '1', variant: {def: {'key1': 'val2'}}} as Partial<TestResult> as TestResult;
  // Result with multiple variant keys.
  const testResult5 = {testId: 'c', resultId: '3', variant: {def: {'key2': 'val1', 'key1': 'val2'}}} as Partial<TestResult> as TestResult;
  // Result with the same variant but variant keys are in different order.
  const testResult6 = {testId: 'c', resultId: '4', variant: {def: {'key1': 'val2', 'key2': 'val1'}}} as Partial<TestResult> as TestResult;
  // Result with partially different variant.
  const testResult7 = {testId: 'c', resultId: '4', variant: {def: {'key1': 'val2', 'key2': 'val2'}}} as Partial<TestResult> as TestResult;
  ...
  describe('streamTestResult', () => {
    it('should stream test results from multiple pages', async () => {
      ...
      const testResults = [testResult1, testResult2, testResult3, testResult4, testResult5, testResult6, testResult7];
  ...
  describe('streamTestExoneration', () => {
    it('should stream test exonerations from multiple pages', async () => {
      ...
      const stub = sinon.stub();
      const testExonerations = [testExoneration1, testExoneration2, testExoneration3, testExoneration4, testExoneration5];
  ...
  describe('streamTest', () => {
    it('can group test results and exonerations into tests correctly', async () => {
      ...
      // assert.deepInclude expects array elements in 'actual' to deepEqual
      // (rather than deepInclude) their counter parts in 'expected'.
      // For example, assert.deepInclude({a: [{b: 0, _b: 0}]}, {a: [{b: 0}]})
      // will fail because 'a[0]._b' is not present in 'expected'.
      // Strip out properties we don't want to test first, so private properties
      // in 'actual' won't cause the test to fail.
      assert.deepInclude(deepStripNotIn(tests[0], test1), test1);
      assert.deepInclude(deepStripNotIn(tests[1], test2), test2);
      assert.deepInclude(deepStripNotIn(tests[2], test3), test3);
      assert.deepInclude(deepStripNotIn(tests[3], test4), test4);
      assert.deepInclude(deepStripNotIn(tests[4], test5), test5);
    });
  });
});
