...
describe('test_loader', () => {
  ...
  describe('TestLoader', () => {
    ...
    let finalizeSpy = sinon.spy();
    let testLoader: TestLoader;
    beforeEach(() => {
      addTestSpy = sinon.spy();
      finalizeSpy = sinon.spy();
      testLoader = new TestLoader(
        {addTest: addTestSpy, finalize: finalizeSpy} as Partial<TestNode> as TestNode,
        (async function* () {
          yield test1;
          yield test2;
          yield test3;
    ...
    it('should finalize node when all tests are loaded', async () => {
      assert.isTrue(finalizeSpy.notCalled);
