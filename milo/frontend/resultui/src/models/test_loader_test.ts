...
describe('test_loader', () => {
  ...
  describe('TestLoader', () => {
    ...
    beforeEach(() => {
      ...
      testLoader = new TestLoader(
        ...
        (async function* (): AsyncIterableIterator<[ReadonlyTest[], boolean]> {
          yield [[test1, test2], false];
          yield [[test3, test4], false];
          yield [[test5], true];
        })(),
      );
    });
  ...
  describe('streamTestResultBatches', () => {
    it('should stream test results from multiple pages', async () => {
      ...
      const expectedTestResultBatches = [
        [[testResult1, testResult2, testResult3, testResult4, testResult5], false],
        [[testResult6, testResult7], true],
      ];
      let i = 0;
  ...
  describe('streamTestExoneration', () => {
    it('should stream test exonerations from multiple pages', async () => {
      ...
      const expectedTestExonerationBatches = [
        [[testExoneration1, testExoneration2, testExoneration3], false],
        [[testExoneration4, testExoneration5], true],
      ];
      let i = 0;
  ...
  describe('streamTestBatches', () => {
    it('can group test results and exonerations into tests correctly', async () => {
      const resultIter = (async function*(): AsyncIterableIterator<[TestResult[], boolean]> {
        yield [[testResult1, testResult2, testResult3], false];
        yield [[testResult4, testResult5, testResult6], false];
        yield [[testResult7, testResult8], true];
      })();
      const exonerationIter = (async function*(): AsyncIterableIterator<[TestExoneration[], boolean]> {
        yield [[testExoneration1, testExoneration2, testExoneration3], false];
        yield [[testExoneration4, testExoneration5], false];
      })();
