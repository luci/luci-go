...
describe('test_loader', () => {
  ...
  describe('TestLoader', () => {
    ...
    it('should load tests to a given node on request', async () => {
      ...
      await testLoader.loadMore(3);
      const childAddTestSpy = sinon.spy();
      const childFinalizeLoadingSpy = sinon.spy();
      const mockChild = {path: 'c/', addTest: childAddTestSpy, finalizeLoading: childFinalizeLoadingSpy} as Partial<TestNode> as TestNode;

      await testLoader.loadMore(1, mockChild);
      assert.strictEqual(childAddTestSpy.callCount, 1);
      assert.strictEqual(childAddTestSpy.getCall(0).args[0], test4);
      assert.isFalse(childFinalizeLoadingSpy.called);

      await testLoader.loadMore(2, mockChild);
      assert.strictEqual(childAddTestSpy.callCount, 2);
      assert.strictEqual(childAddTestSpy.getCall(1).args[0], test5);
