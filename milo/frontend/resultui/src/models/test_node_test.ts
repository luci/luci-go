...
describe('TestNode', () => {
  it('should elide test id segments with no branches', async () => {
    ...
    assert.equal(node.elidedName, 'a/b/c/');
    ...
    assert.equal(node.elidedChildren[0].elidedName, 'd');
    ...
    assert.equal(node.elidedChildren[1].elidedName, 'e');
  ...
  it('should un-elide test id segments after a new branch is added', async () => {
    ...
    node.addTest(test3);
    node.branchUnbranchedTests();
    assert.equal(node.elidedName, 'a/b/');
    assert.equal(node.elidedChildren.length, 2);

    assert.equal(node.elidedChildren[0].elidedName, 'c/');
    assert.equal(node.elidedChildren[0].elidedChildren.length, 2);
    assert.equal(node.elidedChildren[0].elidedChildren[0].elidedName, 'd');
    assert.equal(node.elidedChildren[0].elidedChildren[1].elidedName, 'e');
    assert.equal(node.elidedChildren[1].elidedName, 'f');
  });

  it('should not elide test ids when one is the prefix of another', async () => {
    const node = TestNode.newRoot();
    // test1 should not be treated as the parent of test2. All tests should be treated as leaves.
    const test1 = {id: 'a/b/c/d/', variants: []};
    const test2 = {id: 'a/b/c/d/e', variants: []};
    node.addTest(test1);
    node.addTest(test2);
    node.branchUnbranchedTests();
    assert.equal(node.elidedName, 'a/b/c/d/');
    assert.equal(node.elidedChildren.length, 2);
    assert.equal(node.elidedChildren[0].elidedName, '');
    assert.equal(node.elidedChildren[1].elidedName, 'e');
  });
});
