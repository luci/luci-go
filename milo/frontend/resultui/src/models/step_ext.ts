...
/**
 * Extends the Step object with helper methods and properties.
 */
export class StepExt {
  ...
  readonly now: IObservableValue<DateTime>;

  constructor(step: Step, now?: IObservableValue<DateTime>) {
    if (!step.name) {
    ...
    this.now = now || observable.box(DateTime.local());
  }

  /**
   * true if and only if the step and all of descendants succeeded.
   */
  ...
  @computed get duration() {
    return (this.endTime || this.now.get()).diff(this.startTime);
  }
}
