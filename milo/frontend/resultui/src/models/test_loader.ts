...
// Use a unexported class to artificially create file-scoped access modifier.
...
// The hash is unique for each variant.
 ...
 */
export async function* streamTestResultOrExoneration(results: AsyncGenerator<TestResult>, exonerations: AsyncGenerator<TestExoneration>) {
...
/**
 * Groups test results or exonerations into Test
 * @param testResultOrExonerations: must be sorted by testId.
 */
export async function* streamTests(testResultOrExonerations: AsyncIterator<TestResultOrExoneration>): AsyncIterableIterator<ReadonlyTest> {
  let test: Test | undefined;
  let nextTestObject = await testResultOrExonerations.next();
  while (!nextTestObject.done) {
    if (nextTestObject.value.testId === test?.id) {
      test.addTestResultOrExoneration(nextTestObject.value);
    } else {
      if (test !== undefined) {
        yield test;
      }
    ...
    nextTestObject = await testResultOrExonerations.next();
  }
  if (test !== undefined) {
    yield test;
  }
