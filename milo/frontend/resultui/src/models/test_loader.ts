...
export class TestLoader {
  ...
  @computed get unexpectedlySkippedVariantCount() {
    const count = this.unexpectedlySkippedTestVariants.length || 0;
    return `${count}${this.stage <= LoadingStage.LoadingUnexpectedlySkipped ? '+' : ''}`;
  }
