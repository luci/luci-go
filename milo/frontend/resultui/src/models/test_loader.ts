...
export class TestLoader {
  ...
  async loadPagesUntilStatus(status: TestVariantStatus) {
    ...
    // If we wanted to load up to Expected, and we know we didn't start at Expected
    // (due to the initial exit condition), that means we haven't loaded any yet
    // (i.e. we're at the point where ResultDB has just returned the final non-expected
    // variants to us). In this case, load one more page.
    if (status === TestVariantStatus.EXPECTED) {
      await this.loadNextPage();
    }
  }
