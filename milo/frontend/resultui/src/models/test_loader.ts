...
export class TestLoader {
  ...
  private async loadMoreInternal(limit: number, node?: TestNode) {
    const targetNode = node || this.root;
    while (limit > 0) {
      const peeked = await this.testIter.peek();
      // Use node instead of targetNode because when loading tests into root, we
      // don't want to match the prefix of root's test ID.
