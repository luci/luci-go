...
// In-memory cached. Can be used to synced access.
let cachedAuthState: AuthState | null = null;

/**
 * Update the auth state in the persisted cache and in-memory cache.
 */
 ...
 * 2. there's no cached auth state, or
...
/**
 * Gets auth state and populate in-memory cache. Returns null if
 * 1. there's no cached auth state, or
