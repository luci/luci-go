...
export class ResultEntryElement extends MobxLitElement {
  ...
  @computed
  private get artifactsReq(): IPromiseBasedObservable<ListArtifactsResponse> {
  ...
  @computed
  private get artifacts() { return this.artifactsReq.state === 'fulfilled' ? this.artifactsReq.value.artifacts || [] : []; }
    ...
    return html`
      ...
      >
        ...
        <div class="one-line-content">
          Artifacts:
          <span class="light">${this.artifactsReq.state === 'fulfilled' ? `${this.artifacts.length} artifact(s)` : 'loading...'}</span>
        </div>
      </div>
