...
export class TestEntryElement extends MobxLitElement {
  ...
  private get commonTestIdPrefix() {
    const prevSegs = this.prevTestId.match(ID_SEG_REGEX) as string[];
    const currentSegs = this.test!.id.match(ID_SEG_REGEX) as string[];
    return _.takeWhile(prevSegs, (seg, i) => currentSegs[i] === seg).join('');
  }
