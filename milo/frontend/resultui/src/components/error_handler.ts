...
const EVENT_TYPE = 'error-' + Math.random().toString();
...
export function reportError<T extends unknown[], V>(
  this: Element,
  fn: (...params: T) => V,
  defaultFn?: (err: unknown, ...params: T) => V
): (...params: T) => V {
  return (...params: T) => {
    try {
      return fn(...params);
...
export function reportErrorAsync<T extends unknown[], V>(
  ...
  defaultFn?: (err: unknown, ...params: T) => Promise<V>
): (...params: T) => Promise<V> {
...
export class ErrorHandlerElement extends LitElement {
  ...
  /**
   * Called before showing error messages. Return true to mark the error as
   * handled.
   */
  intercept = (_e: ErrorEvent) => false;

  private errorHandler = (e: Event) => {
    const event = e as ErrorEvent;
    event.stopPropagation();
    if (this.intercept(event)) {
      return;
    }
  ...
  connectedCallback() {
    ...
    this.addEventListener(EVENT_TYPE, this.errorHandler);
  }

  disconnectedCallback() {
    this.removeEventListener(EVENT_TYPE, this.errorHandler);
