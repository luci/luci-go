...
export class TestVariantsTableColumnHeader extends MobxLitElement {
  ...
  private sortColumn(ascendent: boolean) {
    const newSortingKeys = this.removeKey(this.invocationState.sortingKeys);
    newSortingKeys.unshift((ascendent ? '' : '-') + this.propKey);
  ...
  protected render() {
    return html`
      ...
      <mwc-menu absolute x="0" y="10" ?open=${this.menuIsOpen} @closed=${() => (this.menuIsOpen = false)}>
        <mwc-list-item @click=${() => this.sortColumn(true)}>Sort up</mwc-list-item>
        <mwc-list-item @click=${() => this.sortColumn(false)}>Sort down</mwc-list-item>
