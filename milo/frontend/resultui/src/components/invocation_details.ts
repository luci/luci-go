    ...
    return html`
      ...
      <div id="body">
        <div id="content" style=${styleMap({'display': this.expanded ? '' : 'none'})}>
          <div>Create Time: ${Date.from(this.invocation!.createTime).toLocaleString()}</div>
          <div>Finalize Time: ${Date.from(this.invocation!.finalizeTime).toLocaleDateString()}</div>
          <div>Deadline: ${Date.from(this.invocation!.deadline).toLocaleDateString()}</div>
          <div
