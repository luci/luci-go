...
export class BuildStepEntryElement extends MobxLitElement {
  ...
  @computed private get header() {
    return this.step.summary_markdown?.split('<br>', 1)[0] || '';
  }

  @computed private get summary() {
    return this.step.summary_markdown?.slice(this.header.length + '<br>'.length) || '';
  }
    ...
    return html`
      <milo-expandable-entry>
        ...
        <div slot="content">
          ...
          <ul style=${styleMap({display: this.step.logs?.length ? '' : 'none'})}>
