...
/**
 * Display all test IDs in a folder-tree like sturcutre.
...
export class TestNavTreeElement extends MobxLitElement {
  ...
  protected render() {
    if (this.root!.directUnprocessedTests.length !== 0) {
      this.root!.processTests();
    }

    return html`
      <div id="header" title=${this.root!.branchName}>${this.root!.branchName}</div>
      <div id="body">
        <div id="content">
          ${repeat(this.root!.children, (node) => node.branchName, (node) => html`
          <tr-test-nav-node
            .depth=${0}
            .node=${node}
          >
