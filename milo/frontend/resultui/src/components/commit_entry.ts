...
export class CommitEntryElement extends MobxLitElement {
  ...
  private renderDescriptionLine(line: string) {
    return html`${line}<br>`;
  }
  ...
  protected render() {
    return html`
      ...
      >
        <span slot="header">
          <b>${this.number}. ${this.title}</b> <i>by ${this.commit.author.name} at ${this.commit.committer.time}</i>
        </span>
        <div slot="content">
          <table slot="content" border="0">
            <tr><td>Changed by:</td><td>${this.commit.author.name} - ${this.commit.author.email}</td></tr>
            <tr><td>Changed at:</td><td>${this.commit.committer.time}</td></tr>
