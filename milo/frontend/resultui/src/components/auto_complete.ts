...
export interface Suggestion {
  // An suggestion with `null` value is treated as a header and is not selectable.
...
export class AutoCompleteElement extends MobxLitElement {
  ...
  private renderSuggestion(suggestion: Suggestion, i: number) {
    if (suggestion.value === null) {
      return html`
        <tr class="dropdown-item header">
          <td colspan="2">${suggestion.explanation}</td>
        </tr>
      `;
    }
    return html`
        ...
        @keydown=${(e: KeyboardEvent) => {
          switch (e.code) {
            case 'ArrowDown':
              ...
              break;
            case 'ArrowUp':
              // Select the previous selectable suggestion.
