...
export class ResultEntryElement extends MobxLitElement {
  ...
  artifactContentRes = computedFn(function(fetchUrl: string): IPromiseBasedObservable<string> {
    return fromPromise(fetch(fetchUrl).then((res) => res.text()));
  });

  artifactContent = computedFn(function(this: ResultEntryElement, fetchUrl: string): string {
    var contentRes = this.artifactContentRes(fetchUrl);
