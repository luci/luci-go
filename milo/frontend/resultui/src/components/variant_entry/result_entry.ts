...
export class ResultEntryElement extends MobxLitElement {
  ...
  private get duration() {
    ...
    return Duration.fromMillis(Number(durationStr.slice(0, durationStr.length - 1)) * 1000);
  }
    ...
    return html`
      <milo-expandable-entry .expanded=${this.expanded} .onToggle=${(expanded: boolean) => (this.expanded = expanded)}>
        <span id="header" slot="header">
          ...
          <span>after <i>${displayDuration(this.duration || Duration.fromMillis(0))}</i></span>
          ${this.swarmingTask
            ? html`
                <span>
                  in task:
                  <a
                    href="https://${this.swarmingTask.host}?task=${this.swarmingTask.id}"
                    target="_blank"
  ...
  static styles = [
    ...
    css`
      ...
      #header {
        display: inline-grid;
        grid-template-columns: 55px 145px 90px 1fr;
        font-size: 14px;
