...
export class VariantEntryElement extends MobxLitElement {
  ...
  @observable.ref private variantDefExpanded = false;
  ...
  private renderVariantDef() {
    if (this.variantDef.length === 0) {
      return html``;
    }
    ...
    return html`
      <div
        ...
        @click=${() => this.variantDefExpanded = !this.variantDefExpanded}
      >
        <mwc-icon class="expand-toggle">${this.variantDefExpanded ? 'expand_more' : 'chevron_right'}</mwc-icon>
        <span id="variant-def" class="one-line-content">
          definition:
          <span class="light" style=${styleMap({display: this.variantDefExpanded ? 'none': ''})}>
      ...
      <table id="def-table" border="0" style=${styleMap({display: this.variantDefExpanded ? '': 'none'})}>
        ${this.variantDef.map(([k, v]) => html`
        <tr>
          <td>${k}:</td>
          <td>${v}</td>
  ...
  static styles = css`
    ...
    #status-indicator.exonerated {
      color: #ff33d2;
    }
    #status-indicator.expected {
      color: #33ac71;
    }
    #status-indicator.unexpected {
      color: #d23f31;
    }
    #status-indicator.flaky {
      color: #f5a309;
    }
