...
describe('reportError', () => {
  ...
  it('should not render error message when handleError fn returns true', async () => {
    after(fixtureCleanup);
    const errorHandlerEle = await fixture<ErrorHandlerElement>(html`
      <milo-error-handler .handleError=${() => true}>
        <milo-error-handler-test></milo-error-handler-test>
      </milo-error-handler>
    `);
  ...
  it('should render error message when handleError fn returns false', async () => {
    after(fixtureCleanup);
    const errorHandlerEle = await fixture<ErrorHandlerElement>(html`
      <milo-error-handler .handleError=${() => false}>
        <milo-error-handler-test></milo-error-handler-test>
      </milo-error-handler>
    `);
    assert.strictEqual(errorHandlerEle.errorMsg, 'Error: error msg');
