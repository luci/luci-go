...
export class InvocationState {
    ...
    return iter.teeAsync(streamTestResultBatches(
      {
        ...
        predicate: {
          expectancy: this.userConfigs.testFilters.showExpected ? Expectancy.All : Expectancy.VariantsWithUnexpectedResults,
        },
        readMask: '*',
      },
  ...
  private get testIterFn() {
    ...
    variantBatches = this.userConfigs.testFilters.showExonerated ?
      variantBatches :
    ...
    variantBatches = this.userConfigs.testFilters.showFlaky ?
      variantBatches :
