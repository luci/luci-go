...
/**
 * Provides OAuth2 clientId to be shared across the app.
...
export class ClientIdProviderElement extends LitElement implements BeforeEnterObserver {
  ...
  async onBeforeEnter() {
    ...
    if (clientId === null) {
      ...
      // Refresh the clientId asynchronously so it doesn't block rendering.
      this.refreshClientId();
    }
  }

  /**
   * Calls the client-id endpoint then updates this.clientId and its local
   * cache.
   */
