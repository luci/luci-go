...
export class UserConfigsStoreV1 {
  ...
  init() {
    const storedConfigsStr = window.localStorage.getItem(UserConfigsStoreV1.KEY) || '{}';
    merge(this.userConfigs, JSON.parse(storedConfigsStr));
    this.disposer = reaction(
      () => JSON.stringify(this.userConfigs),
      (configsStr) => window.localStorage.setItem(UserConfigsStoreV1.KEY, configsStr),
    );
  }
