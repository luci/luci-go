...
function tokenize(s: StateInline): boolean {
  const lineDelimeter = s.md.options.breaks ? '\n' : '  \n';

  // Check if it's a new line.
  if (s.pos === 0 || s.src.slice(s.pos - lineDelimeter.length, s.pos) === lineDelimeter) {
  ...
  if (s.tokens.length > 0 && s.src.slice(s.pos, s.pos + lineDelimeter.length) === lineDelimeter && isInBugLine(s.tokens, s.tokens.length - 1)) {
...
/**
 * Support converting bug identifiers `/\b(\w+:)?#?\d+\b/` in bug lines
 * `/^(>| )*(bugs?)[:=]/i` to crbug links.
 */
