...
import { Suggestion } from '../components/auto_suggestion';
...
export function suggestSearchQuery(subQuery: string): readonly Suggestion[] {
  ...
  if (match) {
    ...
    if (match.groups!['neg'] === '-') {
      return [
        {value: `-ID:${match.groups!['substr']}`, explanation: 'Exclude tests with the specified substring in their ID (case insensitive)'},
      ];
    }
    return [
      {value: `ID:${match.groups!['substr']}`, explanation: 'Include only tests with the specified substring in their ID (case insensitive)'},
      {value: `-ID:${match.groups!['substr']}`, explanation: 'Exclude tests with the specified substring in their ID (case insensitive)'},
    ];
  }
  ...
  suggestions.push(
    {value: `ID:${subQuery}`, explanation: 'Include only tests with the specified substring in their ID (case insensitive)'},
    {value: `-ID:${subQuery}`, explanation: 'Exclude only tests with the specified substring in their ID (case insensitive)'},
  );
  return suggestions;
}
