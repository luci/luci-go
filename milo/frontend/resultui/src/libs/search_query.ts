...
const SPECIAL_QUERY_RE = /^(-?)([a-zA-Z_]+):(.+)$/;
...
const QUERY_TYPE_SUGGESTIONS = [
  ...
  {type: '-ExactID:', explanation: 'Exclude tests with the specified ID (case insensitive)'},
...
export function suggestSearchQuery(query: string): readonly Suggestion[] {
  if (query === '') {
    ...
    return [
      ...
      {value: 'ExactID:test-id', explanation: 'Include only tests with the specified test ID'},
