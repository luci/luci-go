...
export function suggestSearchQuery(query: string): readonly Suggestion[] {
  if (query === '') {
    ...
    return [
      ...
      {value: '-Status:EXPECTED', explanation: 'Exclude tests that matches the filter'},
      {value: 'Status:UNEXPECTED,FLAKY,EXONERATED -RStatus:Skipped', explanation: 'Combine multiple filters'},
