    ...
    switch (type.toUpperCase()) {
      ...
      case '': {
        return (v: TestVariant) =>
          // valueUpper can not contain space. Join the test ID and test name
          // with a space so we can perform search on them at the same time.
          negate !== [v.testId, v.testMetadata?.name || ''].join(' ').toUpperCase().includes(valueUpper);
      }
