...
export function suggestSearchQuery(query: string): readonly Suggestion[] {
  if (query === '') {
    ...
    return [
      {value: null, explanation: html`<strong>Filter Syntax</strong>`},
      {value: 'Status:UNEXPECTED', explanation: 'The syntax of a filter is TYPE:VALUE'},
      {value: 'RStatus:Fail,Crash', explanation: 'Some filters support comma separated list'},
      {value: 'test-id-substr', explanation: 'A filter without type is treated as an ID filter'},
      ...
      {value: null, explanation: html`<strong>Filter Types</strong>`},
