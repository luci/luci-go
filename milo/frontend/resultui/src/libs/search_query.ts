...
const QUERY_TYPE_SUGGESTIONS = [
  ...
  {type: 'TestID:', explanation: 'Include only tests with the specified ID (case sensitive)'},
  {type: '-TestID:', explanation: 'Exclude tests with the specified ID (case sensitive)'},
...
export function suggestSearchQuery(query: string): readonly Suggestion[] {
  if (query === '') {
    ...
    return [
      ...
      {value: 'TestID:test-id', explanation: 'Include only tests with the specified test ID (case sensitive)'},
