 ...
 * @observer
 * class LazyLoadingElement extends LitElement implements ElementObserver {
...
export interface ElementNotifier {
  subscribe(ele: ObserverElement): void;
  unsubscribe(ele: ObserverElement): void;
}

export const [provideNotifier, consumeNotifier] = createContextLink<ElementNotifier>();

/**
 * Notifies the elements when they intersect with the root element for the first
 * time.
 */
export class IntersectionNotifier implements ElementNotifier {
...
/**
 * Notifies the elements when they intersect with the root element for the first
 * time. If no elements were notified for the last `batchInterval` ms, picks
 * `batchSize` un-notified elements and notifies them.
 */
export class ProgressiveNotifier implements ElementNotifier {
...
export const INTERSECTION_NOTIFIER: ElementNotifier = new IntersectionNotifier();
...
export function observer<T extends ObserverElement, C extends Constructor<T>>(cls: C) {
  ...
  class EnterViewObserverElement extends (cls as Constructor<LitElement>) {
    ...
    @consumeNotifier
    set [notifierSymbol](newVal: ElementNotifier) {
