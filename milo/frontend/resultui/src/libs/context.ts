 ...
 * @contextProviderMixinBuilder<Context>(['contextKey'])
 ...
 * @contextSubscriberMixinBuilder<Context>(['contextKey'])
 ...
 * }
 *
 * customElement('context-provider')(contextProviderMixinBuilder<Context>(['contextKey']))(ContextProvider)
 * ```
 */
...
export function buildContextProviderMixin<Ctx>(providedContexts: Array<keyof Ctx>) {
  return function providerMixin<T extends LitElement & Ctx, C extends Constructor<T>>(cls: C) {
    ...
    class Provider extends (cls as Constructor<LitElement>) {
      private readonly contextSubscribers = new Map<keyof Ctx, Set<T>>(providedContexts.map((key) => [key, new Set()]));
       ...
       * value.
       */
      private updateSubscriber(subscriber: T, key: keyof Ctx, newValue: T[keyof Ctx]) {
       ...
       */
      private onSubscribeContext = (event: ContextEvent<Ctx, T>) => {
        const subscribers = this.contextSubscribers.get(event.detail.key);
        if (subscribers) {
          const subscriber = event.detail.element;
          const key = event.detail.key;
          const newValue = (this as LitElement as T)[event.detail.key];
       ...
       */
      private onUnsubscribeContext = (event: ContextEvent<Ctx, T>) => {
        const subscribers = this.contextSubscribers.get(event.detail.key);
        if (subscribers) {
...
export function buildContextSubscriberMixin<Ctx>(requiredContexts: Array<keyof Ctx>) {
  return function subscriberMixin<T extends LitElement & Ctx, C extends Constructor<T>>(cls: C) {
    // TypeScript doesn't allow type parameter in extends or implements
    // position. Cast to Constructor<LitElement> to stop tsc complaining.
      ...
      connectedCallback() {
        ...
        for (const key of requiredContexts) {
          this.dispatchEvent(new CustomEvent('tr-subscribe-context', {
            detail: {key, element: this as LitElement as T},
            bubbles: true,
            cancelable: true,
            composed: true,
          }) as ContextEvent<Ctx, T>);
        }
      }
