...
import { cached, CachedFn, CacheOption } from './cached_fn';

describe('cached_fn', () => {
  let cachedFn: CachedFn<[number, string], string>;
  ...
  it('should return cached response when calling with Cached', async () => {
    const res1 = cachedFn(1, 'a');
    const res2 = cachedFn.withOpt(CacheOption.Cached , 1, 'a');
    assert.strictEqual(res1, res2);
    assert.strictEqual(fnSpy.callCount, 1);
  });
  ...
  it('should refresh the cache when calling with ForceRefresh', async () => {
    const res1 = cachedFn(1, 'a');
    const res2 = cachedFn.withOpt(CacheOption.ForceRefresh, 1, 'a');
  ...
  it('should bypass the cache when calling with NoCache', async () => {
    const res1 = cachedFn(1, 'a');
    const res2 = cachedFn.withOpt(CacheOption.NoCache, 1, 'a');
    const res3 = cachedFn(1, 'a');
    assert.strictEqual(res1, '1-a-0');
    assert.strictEqual(res2, '1-a-1');
