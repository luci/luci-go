...
describe('createVariantPropAccessor', () => {
  it('can create a status accessor', async () => {
    const accessor = createVariantPropKeyFn('status');
    const prop = accessor(({ status: TestVariantStatus.EXONERATED } as Partial<TestVariant>) as TestVariant);
    assert.strictEqual(prop, TestVariantStatus.EXONERATED);
  });

  it('can create a name accessor', async () => {
    const accessor = createVariantPropKeyFn('Name');

    const prop1 = accessor(({
      testId: 'test-id',
    ...
    // Fallback to test id.
    const prop2 = accessor(({ testId: 'test-id' } as Partial<TestVariant>) as TestVariant);
    assert.strictEqual(prop2, 'test-id');
  });

  it('can create a variant accessor', async () => {
    const accessor = createVariantPropKeyFn('v.variant_key');
    ...
    const prop2 = accessor(({} as Partial<TestVariant>) as TestVariant);
