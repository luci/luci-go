...
import { ClusterRequest, ClusterResponse, ClustersService } from './weetbix';
...
describe('ClustersService', () => {
  ...
  it('should not batch more than 1000 requests together', async () => {
    ...
    expect(prpcStub.call.callCount).to.equal(2);
    expect(prpcStub.call.getCall(0).args).to.deep.equal(['weetbix.v1.Clusters', 'Cluster', makeReq(0, 800)]);
    expect(prpcStub.call.getCall(1).args).to.deep.equal(['weetbix.v1.Clusters', 'Cluster', makeReq(800, 400)]);
