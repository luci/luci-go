...
function stubRequests(routeMatcher: RouteMatcher, cacheName: string, opt: StubRequestsOption = {}) {
  ...
  cy.readFile(filename).then((content) => {
    ...
    cy.intercept(routeMatcher, (incomingReq) => {
      ...
      // We need to ensure the full request is stored in the cache because
      // the caller may decide to change the matched header.
      const reqForComparison = stripIgnoredHeaders(req);
