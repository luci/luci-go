...
type BuildPage struct {
	...
	// build on the same builder, and the this run.
	Blame []*Commit

	// Errors contains any non-critical errors encountered while rendering the page.
	// TODO(hinoka): Render me on the page somewhere
	Errors []error
...
func (bp *BuildPage) Steps() []*Step {
	...
	for _, step := range bp.Build.Steps {
		...
		stepMap[step.Name] = s
		if nameParts := strings.Split(step.Name, "|"); len(nameParts) == 1 {
			// Root step.
			...
			if !ok {
				...
				bp.Errors = append(bp.Errors, fmt.Errorf("%s is missing a parent", step.Name))
				return nil
			}
...
func (bp *BuildPage) BuildID() *Link {
	...
	num := fmt.Sprintf("%d", bp.Build.Id)
	if bp.Build.Number != 0 {
		num = fmt.Sprintf("%d", bp.Build.Number)
	}
	b := bp.Build.Builder
	return NewLink(
		num,
		fmt.Sprintf("/p/%s/builder/%s/%s/%s", b.Project, b.Bucket, b.Builder, num),
		fmt.Sprintf("Build %s", num))
}
