...
func (bp *BuildPage) BuildID() *Link {
	...
	// No builder associated? That's weird. Just use the build ID link.
	return NewLink(
		fmt.Sprintf("%d", bp.Build.Id),
		fmt.Sprintf("/b/%d", bp.Build.Id),
		fmt.Sprintf("Build number %d", bp.Build.Id),
	)
}
