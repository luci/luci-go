...
func (b *Build) Buildsets() []string {
	...
	for _, tag := range b.Tags {
		// HACK(hinoka): Remove the commit/git/ buildsets because they're redundant
		// with the commit/gitiles/ buildsets.
		if tag.Key == "buildset" && !strings.HasPrefix(tag.Value, "commit/git/") {
...
func (bp *BuildPage) InputProperties() []property {
	...
	return properties(bp.Input.GetProperties())
}
...
func (bp *BuildPage) OutputProperties() []property {
	...
	return properties(bp.Output.GetProperties())
}
