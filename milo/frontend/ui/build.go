...
type BuildPage struct {
	...
	// Error contains any non-critical errors encountered while rendering the page.
	Error error
...
func (bp *BuildPage) Steps() []*Step {
	...
	for _, step := range bp.Build.Steps {
		collpaseGreen := bp.StepDisplayPref == StepDisplayDefault
		s := &Step{
			Step:      step,
			Collapsed: collapseGreen && step.Status == buildbucketpb.Status_SUCCESS,
		}
		nameParts := strings.Split(step.Name, "|")
		stepMap[step.Name] = s
		// Root
		if len(nameParts) == 1 {
			...
			if !ok {
				// This should never happen, it is against the build.Proto contract.
				bp.Error = fmt.Errorf("%s is missing a parent", step.Name)
				return nil
			}
...
func (bp *BuildPage) BuildID() *Link {
	...
	if bp.Build.Number != 0 {
		num = int(bp.Build.Number)
	}
	return NewLink(
		strconv.Itoa(num),
		fmt.Sprintf("/p/%s/builder/%s/%s/%s", b.Project, b.Bucket, b.Builder, num),
		fmt.Sprintf("Build number %d", num))
}
...
func (bp *BuildPage) Banners() (result []LogoBanner) {
	if bp.Infra == nil {
		return
	}
...
/*
// Timeline returns data for a vis timeline as a JSON string.
func (bp *BuildPage) Timeline(build *buildbucketpb.Build) string {
	...
	groups := make([]group, len(bp.Steps))
	items := make([]item, len(bp.Steps))
