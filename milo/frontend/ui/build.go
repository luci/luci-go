...
// ShowPref is the display preference for the steps.
type ShowPref string

const (
	// Collapsed means that all steps visible, nested steps are collapsed.
	Collapsed ShowPref = "collapsed"
	// Expanded means that all steps visible, nested steps are expanded.
...
func (b *MiloBuild) Fix() {
	for _, comp := range b.Components {
		fixComponent(
			comp, b.Summary.ExecutionTime.Finished, "", b.ShowPref == Collapsed)
	}
}
