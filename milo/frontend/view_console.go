...
// setBuilderSummaries extracts the builder summaries for the current console
// out of the console summary map.
func setBuilderSummaries(project, id string, summaries map[common.ConsoleID]ui.ConsoleSummary) map[string]*model.BuilderSummary {
	consoleID := common.ConsoleID{Project: project, ID: id}
	consoleSummary, ok := summaries[consoleID]
	if !ok {
		return nil
	}
...
func setHeaderSummaries(header *ui.ConsoleHeader, def *config.Header, summaries map[common.ConsoleID]ui.ConsoleSummary) {
...
func summaries(c context.Context, project, id string, def *config.Header) (
	...
	return buildsource.GetConsoleDefSummaries(c, ids)
}
...
func console(c context.Context, project, name string, limit int) (*ui.Console, error) {
	...
	if err := parallel.FanOutIn(func(ch chan<- func() error) {
		...
		ch <- func() (err error) {
			...
			consoleSummaries, err = summaries(c, project, name, headerDef)
			return
		}
		ch <- func() (err error) {
	...
	// and also the header.
	builderSummaries := setBuilderSummaries(project, name, consoleSummaries)
	setHeaderSummaries(header, headerDef, consoleSummaries)
...
func consoleHeaderGroupIDs(project string, config []*config.ConsoleSummaryGroup) ([]common.ConsoleID, error) {
	...
	consoleIDSet.Iter(func(id string) bool {
		consoleIDs = append(consoleIDs, common.NewConsoleID(id))
		return true
	})
...
func ConsolesHandler(c *router.Context, projectName string) {
	...
	summaryMap, err := buildsource.GetConsoleSummaries(c.Context, cons)
	if err != nil {
		ErrorHandler(c, err)
		return
	}
