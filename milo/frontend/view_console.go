...
func console(c context.Context, project, name string, limit int) (*resp.Console, error) {
	...
	// TODO(mknyszek): Don't get console data by row, we care about columns now.
	...
	for col, b := range def.Builders {
		...
		// Accrue builds for the same builder together.
		//
		// TODO(mknyszek): Remove this loop once we bring in data from the
		// datastore in columns instead of rows.
		for row := 0; row < len(commitInfo.Commits); row++ {
			if summaries := rows[row].Builds[name]; len(summaries) > 0 {
				builds[buildIndex] = summaries[0]
			}
			buildIndex++
		}
