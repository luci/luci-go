...
func ConsoleHandler(c *router.Context) error {
	...
	if con.Def.Header != nil {
		...
		if err != nil {
			headerConError = errors.Annotate(err, "error getting header consoles").Err()
	...
	result, err := console(c.Context, project, group, limit, con, headerCons, headerConError)
	if err != nil {
		return err
	}
