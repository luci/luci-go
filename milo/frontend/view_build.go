...
func sanitizeLinkSet(linkSet ui.LinkSet) ui.LinkSet {
	for _, link := range linkSet {
		link.Label = html.EscapeString(link.Label)
		link.URL = html.EscapeString(link.URL)
		link.AriaLabel = html.EscapeString(link.AriaLabel)
		link.Img = html.EscapeString(link.Img)
		link.Alt = html.EscapeString(link.Alt)
	}
	return linkSet
}
