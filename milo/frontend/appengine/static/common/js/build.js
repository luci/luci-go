...
$(document).ready(function() {
  ...
  function createTimeline() {
    ...
    function groupTemplater(group, element, data) {
      let content = `<div class="group-title ${group.data.statusClassName}">${group.data.label}<span class="duration">( ${group.data.duration} )</span></div>`;
      ...
      if (links.length > 0) {
        content += `<ul><li>${links.map(link => linkHtml(link)).join('</li><li>')}</li></ul>`;
      } else {
