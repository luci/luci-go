...
func withGerritMiddleware(c *router.Context, next router.Handler) {
	c.Context = common.WithGerritFactory(c, func(host string) (gerritpb.GerritClient, error) {
		t, err := auth.GetRPCTransport(c, auth.AsSelf, auth.WithScopes(gerrit.OAuthScope))
		if err != nil {
			return nil, err
		}
