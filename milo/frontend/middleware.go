...
// A formatChunk is either an already-processed trusted template.HTML, produced
...
func stringToChunks(s string) []formatChunk {
	return []formatChunk{{isHTML: false, str: s}}
}
...
func replaceAllInChunks(chunks []formatChunk, re *regexp.Regexp, repl func(string) template.HTML) []formatChunk {
	...
	for _, chunk := range chunks {
		...
		for len(s) != 0 {
			...
			if loc == nil {
				ret = append(ret, formatChunk{isHTML: false, str: s})
				break
			}
			if loc[0] > 0 {
				ret = append(ret, formatChunk{isHTML: false, str: s[:loc[0]]})
			}
			html := repl(s[loc[0]:loc[1]])
			ret = append(ret, formatChunk{isHTML: true, html: html})
			s = s[loc[1]:]
		}
	}
	return ret
