...
func getSummary(c context.Context, host string, id int64) (*model.BuildSummary, error) {
	...
	buildAddress := fmt.Sprintf("luci.%s.%s/%s/", b.Builder.Project, b.Builder.Bucket, b.Builder.Builder)
	if b.Number != 0 {
		buildAddress += fmt.Sprintf("%d", b.Number)
	} else {
		buildAddress += fmt.Sprintf("%d", b.Id)
	}

	buildKey := datastore.MakeKey(c, "buildbucket.Build", fmt.Sprintf("%s:%s", host, buildAddress))
	b.UpdateTime.GetNanos()
...
func generateSummary(c context.Context, hostname string, build buildbucket.Build) (*model.BuildSummary, error) {
	bs, err := getSummary(c, hostname, build.ID)
	if err != nil {
