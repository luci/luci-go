...
// pure-data).
func generateSummary(c context.Context, hostname, project string, id int64) (*model.BuildSummary, error) {
	bs, err := getSummary(c, hostname, project, id)
	if err != nil {
	...
	return bs, bs.AddManifestKeysFromBuildSets(c)
}
...
func pubSubHandlerImpl(c context.Context, r *http.Request) error {
	...
	bs, err := generateSummary(c, event.Hostname, build.Project, build.ID)
	if err != nil {
		return err
	}
