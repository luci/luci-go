...
func getBuilders(c context.Context, host string) (*swarmbucket.LegacySwarmbucketApiGetBuildersResponseMessage, error) {
	...
	default:
		...
		if err := deserializeBuildersResponse(mc.Value(), &res); err != nil {
...
func deserializeBuildersResponse(serialized []byte, res *swarmbucket.LegacySwarmbucketApiGetBuildersResponseMessage) error {
	...
	jsonMessage, err = ioutil.ReadAll(r)
	if err != nil {
		return err
	}
	err = r.Close()
