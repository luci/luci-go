...
func getBuilders(c context.Context, host string) (*swarmbucket.SwarmingSwarmbucketApiGetBuildersResponseMessage, error) {
	mc := memcache.NewItem(c, fmt.Sprintf("swarmbucket-builders-%s-%s", host, auth.CurrentIdentity(c)))
	switch err := memcache.Get(c, mc); {
	case err == memcache.ErrCacheMiss:
	case err != nil:
		logging.WithError(err).Warningf(c, "memcache.get failure while loading swarmbucket builders")
	default:
