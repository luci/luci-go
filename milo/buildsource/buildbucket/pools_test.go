	...
	Convey(`Test Pool Information`, t, func() {
		...
		Convey(`Unit Tests`, func() {
			...
			Convey(`Parsing Bot Status`, func() {
				...
				Convey(`Died 3 min ago is Busy`, func() {
					bot.LastSeenTs = RefTime.Add(-3 * time.Minute).Format(swarming.SwarmingTimeLayout)
					bot.IsDead = true
					s, err := botStatus(c, bot)
					So(err, ShouldBeNil)
					So(s.Status, ShouldEqual, model.Busy)
				})
				Convey(`Died 6 min ago is Offline`, func() {
					bot.LastSeenTs = RefTime.Add(-6 * time.Minute).Format(swarming.SwarmingTimeLayout)
					bot.IsDead = true
					s, err := botStatus(c, bot)
					So(err, ShouldBeNil)
