...
// thisPageToken as the previous cursor of nextPageToken.
func backPageToken(c context.Context, bid *buildbucketpb.BuilderID, pageSize int, thisPageToken, nextPageToken string) string {
	memcacheKey := func(cursor string) string {
		key := fmt.Sprintf("%s:%d:%s", protoutil.FormatBuilderID(bid), pageSize, cursor)
...
func GetBuilderPage(c context.Context, bid *buildbucketpb.BuilderID, pageSize int, pageToken string) (*ui.BuilderPage, error) {
	...
	fetch := func(status buildbucketpb.Status, pageSize int, pageToken string) (builds []*ui.Build, nextPageToken string, complete bool, err error) {
		...
		start := clock.Now(c)
		var res *buildbucketpb.SearchBuildsResponse
		res, err = client.SearchBuilds(c, &buildbucketpb.SearchBuildsRequest{
			Predicate: &buildbucketpb.BuildPredicate{
				Builder: bid,
				Status:  status,
				// TODO(iannucci): remove this. This was meant to be temporary.
				// Experimental builds are supposed to be hidden.
	...
	err = parallel.FanOutIn(func(work chan<- func() error) {
		...
		work <- func() (err error) {
			result.ScheduledBuilds, _, result.ScheduledBuildsComplete, err = fetch(buildbucketpb.Status_SCHEDULED, 0, "")
			return
		}
		work <- func() (err error) {
			result.StartedBuilds, _, result.StartedBuildsComplete, err = fetch(buildbucketpb.Status_STARTED, 0, "")
			return
		}
		work <- func() (err error) {
...
// idMap returns a map of build ids.
