...
func toMiloBuild(msg *bbapi.ApiCommonBuildMessage) (*resp.BuildSummary, error) {
	...
	// HACK(nodir): use b.Project when
	// https://chromium-review.googlesource.com/c/infra/luci/luci-go/+/734315
	// lands
	project := strpair.ParseMap(b.Tags["swarming_tag"]).Get("luci_project")
	if b.Number != nil {
		numStr := strconv.Itoa(*b.Number)
		result.Link = resp.NewLink(
			numStr,
			fmt.Sprintf("/p/%s/builders/%s/%s/%s", project, b.Bucket, b.Builder, numStr),
			fmt.Sprintf("build #%s", numStr))
	} else {
		idStr := strconv.FormatInt(b.ID, 10)
		result.Link = resp.NewLink(
			idStr,
			fmt.Sprintf("/p/%s/builds/b%s", project, idStr),
