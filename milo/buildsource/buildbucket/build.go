...
func CancelBuildHandler(ctx *router.Context) error {
	...
	if _, err := cancelBuild(ctx.Context, id, reason); err != nil {
		return errors.Annotate(err, "error while handling cancel build request").Err()
	}
...
func parseCancelBuildInput(ctx *router.Context) (int64, string, error) {
	if err := ctx.Request.ParseForm(); err != nil {
		return 0, "", errors.Annotate(err, "unable to parse cancel build input").Err()
	}
	...
	if len(reasonInput) != 1 || strings.TrimSpace(reasonInput[0]) == "" {
		return 0, "", fmt.Errorf("invalid or missing reason. expected a non-tempty string. actual value: %v", reasonInput)
	}
