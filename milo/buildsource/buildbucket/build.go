...
// simplisticBlamelist return the ui.Blame field from the
...
func simplisticBlamelist(c context.Context, build *model.BuildSummary) ([]*ui.Commit, error) {
	...
	result := make([]*ui.Commit, len(commits))
	...
	if len(builds) == 0 {
		return nil, errors.New("blame list capped at 50 commits")
	}

	logging.Infof(c, "Added %d commit blamelist", len(result))
...
func getBuildbucketBuild(c context.Context, address string) (*bbapi.ApiCommonBuildMessage, error) {
	...
	build, err := buildbucket.GetMessageByAddress(c, client, address)
	switch {
	case err != nil:
...
func getStep(c context.Context, bbBuildMessage *bbapi.ApiCommonBuildMessage) (*logDogTypes.StreamAddr, *miloProto.Step, error) {
...
// getBalme fetches blame information from Gitiles to mb.  This requires the
// BuildSummary to be indexed in Milo.
func getBlame(c context.Context, msg *bbapi.ApiCommonBuildMessage) ([]*ui.Commit, error) {
	host, err := getHost(c)
	if err != nil {
		return nil, err
	}
