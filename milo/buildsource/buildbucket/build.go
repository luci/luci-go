...
// getBuildCached returns a buildbucket build message given a build ID
...
// TODO(hinoka): Validate ACLs here via BuildBucket.
func getBuildCached(c context.Context, id int64) (*bucketApi.ApiCommonBuildMessage, error) {
	host, err := getHost(c)
	if err != nil {
		return nil, err
	}
	...
	case nil:
		...
		if build.Status == "SCHEDULED" {
			// This build doesn't contain information about how to fetch the build
			// from swarming, check to see if there is a newer version that is running.
			build = nil
		}
	...
	// We contact the Buildbucket service explicitly instead of relying on
	// our cached entries from PubSub because of the potential PubSub delay.
