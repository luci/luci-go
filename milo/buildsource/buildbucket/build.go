...
func GetSwarmingTaskID(c context.Context, buildAddress string) (host, taskId string, err error) {
	...
	switch {
	case err != nil:
		return "", "", errors.Annotate(err, "could not get build at %q", buildAddress).Err()
	case build == nil:
		return "", "", errors.Reason("build at %q not found", buildAddress).Tag(common.CodeNotFound).Err()
	}

	shost := build.Tags.Get("swarming_hostname")
	sid := build.Tags.Get("swarming_task_id")
	if shost == "" || sid == "" {
		return "", "", errors.New("not a valid LUCI build")
	}
	return shost, sid, nil
	// }}
...
// GetRawBuild fetches a buildbuckt build from buildbucket given an address.
...
func GetBuild(c context.Context, address string, fetchBlame bool, fetchSteps bool) (*ui.MiloBuild, error) {
	bbBuildMessage, err := GetRawBuild(c, address)
	if err != nil {
		return nil, err
	}
	return ToMiloBuild(c, bbBuildMessage, fetchBlame, fetchSteps)
}

// ToMiloBuild converts a raw buildbucket build to a milo build.
//
// Returns an error only failure to reach buildbucket.
...
// TODO(nodir): switch to buildbucket API v2 and stop loading steps from logdog
// crbug.com/850113
func ToMiloBuild(c context.Context, b *bbv1.ApiCommonBuildMessage, fetchBlame bool, fetchSteps bool) (*ui.MiloBuild, error) {
