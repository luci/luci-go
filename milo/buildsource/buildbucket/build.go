...
func toMiloBuild(c context.Context, msg *bbv1.ApiCommonBuildMessage) (*ui.MiloBuild, error) {
	...
	var pending, duration ui.Interval
	if !b.CompletionTime.IsZero() && b.StartTime.IsZero() {
		// If the build finished before it started, there is no duration, just pending time.
		pending = ui.NewInterval(c, b.CreationTime, b.CompletionTime)
	} else {
		pending = ui.NewInterval(c, b.CreationTime, b.StartTime)
	}
	result := &ui.MiloBuild{
