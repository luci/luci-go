...
// getBuildSummary fetches a build summary where the Context URL matches the
// given address.
func GetBuildSummary(c context.Context, buildID string) (*model.BuildSummary, error) {
	// The host is set to prod because buildbot is hardcoded to talk to prod.
	uri := fmt.Sprintf("buildbucket://cr-buildbucket.appspot.com/build/%s", buildID)
	logging.Debugf(c, "querying for %s", uri)
	bs := []*model.BuildSummary{}
	q := datastore.NewQuery("BuildSummary").Eq("ContextURI", uri).Limit(1)
	if err := datastore.GetAll(c, q, &bs); err != nil {
