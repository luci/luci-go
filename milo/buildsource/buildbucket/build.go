...
func BuildAddress(build *buildbucketpb.Build) string {
	...
	num := strconv.Itoa(int(build.Id))
	if build.Number != 0 {
		num = strconv.Itoa(int(build.Number))
	}
...
func getBlame(c context.Context, b *buildbucketpb.Build) ([]*ui.Commit, error) {
	host, err := getHost(c)
	if err != nil {
		return nil, err
	}
	// No build? No commits? No builder? No blamelist.
	if b == nil || b.Input == nil || b.Input.GitilesCommit == nil || b.Builder == nil {
		return nil, nil
	}
	bSet := []string{
		fmt.Sprintf(
			"commit/gitiles/%s/%s/+/%s",
			b.Input.GitilesCommit.Host,
			b.Input.GitilesCommit.Project,
			b.Input.GitilesCommit.Id),
	}
...
func buildbucketClient(c context.Context) (buildbucketpb.BuildsClient, error) {
...
func GetBuild(c context.Context, bid buildbucketpb.GetBuildRequest) (*buildbucketpb.Build, error) {
	client, err := buildbucketClient(c)
	if err != nil {
		return nil, err
	}
...
func GetBuildPage(c context.Context, br buildbucketpb.GetBuildRequest) (*ui.BuildPage, error) {
	br.Fields = &field_mask.FieldMask{
		// TODO(hinoka): statusReason.
		Paths: []string{
			"id", "builder", "number", "created_by", "create_time", "start_time", "end_time",
			"update_time", "status", "input", "output", "steps", "infra",
		},
	}
	b, err := GetBuild(c, br)
	if err != nil {
