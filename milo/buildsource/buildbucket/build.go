...
func toMiloBuild(c context.Context, msg *bbv1.ApiCommonBuildMessage) (*ui.MiloBuild, error) {
	...
	if !b.CompletionTime.IsZero() && b.StartTime.IsZero() {
		// If the build is completed but never started, that means it was cancelled.
		// PendingTime should reflect the entire pending time, and ExecutionTime
		// will have a zero start time.
