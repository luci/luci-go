...
// GetBuildPage fetches information for a Milo build page from Buildbucket.
func GetBuildPage(c context.Context, bid buildbucketpb.GetBuildRequest) (*ui.BuildPage, error) {
	client, err := buildbucketClient(c)
	if err != nil {
		return nil, err
	}
	b, err := client.GetBuild(c, bid)
	if err != nil {
		return nil, err
	}
	blame, err := getBlame(c, bid)
	if err != nil {
		return nil, err
	}
	return &ui.BuildPage{
