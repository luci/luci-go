...
var (
	...
	ErrGitilesSlow = errors.Reason(
		"The blamelist failed to fetch because Gitiles was taking too long. " +
			"Pressing on the blamelist tab will force the page to wait for the blamelist to load.",
	).Err()
)
...
func simplisticBlamelist(c context.Context, build *model.BuildSummary) (result []*ui.Commit, err error) {
	defer func() {
		if err != nil {
			logging.WithError(err).Warningf(c, "Gitiles error")
