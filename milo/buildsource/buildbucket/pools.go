...
// The BuilderPool is tagged with the current time and references a BotPool via PoolKey.
...
// uniqueDescriptors returns all unique set of PoolDescriptors in the BuilderPool.
...
// save into the datastore.  Since Swarming's BotInfo doesn't have an explicit status
// field that matches Milo's abstraction of a Bot, the status is inferred:
// * A bot with TaskID is Busy
// * A bot that died in the last 5 minutes is Busy
