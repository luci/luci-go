...
func getBuild(c context.Context, master, builder string, buildNum int) (*protocol.Build, error) {
	if err := canAccessMaster(c, master); err != nil {
		return nil, err
	}

	result := &protocol.Build{
		Master:      master,
		Buildername: builder,
		Number:      buildNum,
	}
...
func getBanner(c context.Context, b *protocol.Build) *resp.LogoBanner {
...
// BuildComponent
func summary(c context.Context, b *protocol.Build) resp.BuildComponent {
	...
	if b.Currentstep != nil {
		...
		status = protocol.ResultToStatus(b.Results)
	}
...
func components(b *protocol.Build) (result []*resp.BuildComponent) {
	for _, step := range b.Steps {
		...
		if !step.IsStarted {
			...
			if len(step.Results) > 0 {
				...
				bc.Status = protocol.ResultToStatus(&status)
			} else {
...
// property groups.
func properties(b *protocol.Build) (result []*resp.PropertyGroup) {
...
func blame(b *protocol.Build) (result []*resp.Commit) {
...
func sourcestamp(c context.Context, b *protocol.Build) *resp.SourceStamp {
	ss := &resp.SourceStamp{}
	rietveld := ""
	gerrit := ""
	got_revision := ""
...
func renderBuild(c context.Context, b *protocol.Build) *resp.MiloBuild {
...
func updatePostProcessBuild(b *protocol.Build) {
...
func promoteLogDogLinks(s *protocol.Step, isInitialStep bool, linkMap map[string]string) {
	type stepLog struct {
		label string
		url   string
	}
	...
	var newAliases map[string][]*protocol.LinkAlias
	if l := remainingAliases.Len(); l > 0 {
		newAliases = make(map[string][]*protocol.LinkAlias, l)
