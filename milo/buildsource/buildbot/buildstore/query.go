...
type Query struct {
	...
	NumbersOnly bool
	// ExcludeDetails, if true, indicates that Properties and Steps do not have
	// to be loaded. Honored only by LUCI emulation.
	ExcludeDetails bool
}
...
func GetBuilds(c context.Context, q Query) (*QueryResult, error) {
	...
	emOptions := GetEmulationOptions(c, q.Master, q.Builder)
	if emOptions != nil {
	...
	err = parallel.WorkPool(10, func(work chan<- func() error) {
		for i, msg := range msgs {
			...
			work <- func() error {
				...
				b, err := buildFromBuildbucket(c, msg, !q.ExcludeDetails)
				if err != nil {
					return err
				}
