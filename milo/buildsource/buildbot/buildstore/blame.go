...
func fetchChanges(c context.Context, b *buildbot.Build) error {
	...
	// we don't need a blamelist with a length > 20.
	commits, err := git.Log(c, host, project, b.Sourcestamp.Revision, 20)
	...
	case codes.NotFound:
		logging.WithError(err).Warningf(c, "gitiles returned 404 for log of %s", b.Sourcestamp.Revision)
		b.Sourcestamp.Changes = nil
		return nil
