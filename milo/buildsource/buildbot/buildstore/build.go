...
func GetBuild(c context.Context, master, builder string, number int) (*buildbot.Build, error) {
	return getBuild(c, master, builder, number, true)
}

func getBuild(c context.Context, master, builder string, number int, fetchChanges bool) (*buildbot.Build, error) {
	emOptions, err := GetEmulationOptions(c, master, builder)
	if err != nil {
		return nil, err
	}
	if emOptions != nil && number >= int(emOptions.StartFrom) {
		return getEmulatedBuild(c, master, emOptions.Bucket, builder, number, fetchChanges)
	}
	return getDatastoreBuild(c, master, builder, number)
}

// getEmulatedBuild returns a buildbot build derived from a LUCI build.
func getEmulatedBuild(c context.Context, master, bucket, builder string, number int, fetchChanges bool) (*buildbot.Build, error) {
	...
	build, err := buildFromBuildbucket(c, master, msgs[0], true)
	if err != nil {
		return nil, err
	}
