...
// getBuildbucketURL attempts to extract the buildbucket information from the properties
...
func getBuildbucketURL(b *buildbot.Build) (string, bool) {
...
func summarizeBuild(c context.Context, b *buildbot.Build) (*model.BuildSummary, error) {
	...
	if buildbucketURL, ok := getBuildbucketURL(b); ok {
		bs.ContextURI = append(bs.ContextURI, buildbucketURL)
	}
