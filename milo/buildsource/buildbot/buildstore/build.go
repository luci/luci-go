...
// ImportBuild persists the build in the storage.
...
func (b *buildEntity) GetAllMeta() datastore.PropertyMap {
	return datastore.PropertyMap{
		...
		"kind": datastore.MkProperty(buildKind),
	}
}
...
// IsToBig returns true of err indicates that an entity
// was not saved because it was too large to store.
...
func (b *buildEntity) Save(withMeta bool) (datastore.PropertyMap, error) {
	...
	if len(data) > 950000 {
		return nil, errTooBig{
			fmt.Errorf("GetBuild: GetBuild too big to store (%d bytes)", len(data))}
	}
