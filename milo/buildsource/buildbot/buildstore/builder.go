...
// buildQueryBatchSize is the batch size to use when querying buildEntity.
//
// This should be tuned to the observed query timeout for buildEntity loading. Since
// loading is CPU-bound, this will probably be low. If buildEntity queries start
// encountering datastore timeouts, reduce this value.
const buildQueryBatchSize = int32(50)

// Ternary is a type with 3 defined values: either (zero), yes and no.
type Ternary int
...
// QueryResult is a result of running Query.
...
// Builds takes a buildEntity query and returns results.
func Builds(c context.Context, q Query) (*QueryResult, error) {
	switch {
	case q.Cursor == "":
	case q.Master == "":
		return nil, errors.New("cursor requires master")
	case q.Builder == "":
	...
	// CUSTOM CURSOR.
	// This function uses a custom cursor based on bulid numbers.
	// A cursor is a buildEntity number that defines a page boundary.
	...
	case len(res.Builds) > 0:
		...
		lastNum := res.Builds[len(res.Builds)-1].Number
		if len(res.Builds) == q.Limit && lastNum > 0 {
