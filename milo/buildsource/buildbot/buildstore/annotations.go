...
func (c *annotationConverter) addSteps(ctx context.Context, dest *[]buildbot.Step, src []*miloProto.Step_Substep, stepNamePrefix string) error {
...
func (c *annotationConverter) log(src *miloProto.Link) (*buildbot.Log, error) {
	...
	case *miloProto.Link_LogdogStream:
		...
		u := url.URL{
			Scheme: "https",
			Host:   stream.Server,
			Path:   "/v/?s=" + url.QueryEscape(stream.Prefix+"/"+stream.Name),
		}
		log.URL = u.String()
