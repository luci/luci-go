...
// This file implements conversion of annotation steps to buildbot steps.

// fetchAnnotationProto fetches an annotation proto from LogDog.
// If the stream is not found, returns io.EOF.
func fetchAnnotationProto(c context.Context, addr *types.StreamAddr) (*miloProto.Step, error) {
	// The implementation avoids using existing Milo code because it is likely
	// to change and because does things that we don't need here e.g. caching
	// that wouldn't apply anyway, support for mocking, etc. Instead we use
	// LogDog client directly, and thus make it simpler to reason what this
	...
	return &res, proto.Unmarshal(logEntry.GetDatagram().Data, &res)
}

// annotationConverter converts annotation steps to buidlbot steps.
type annotationConverter struct {
	logdogServer, logdogPrefix string
	buildCompletedTime         time.Time
}

// addSteps adds steps in src to dest.
func (c *annotationConverter) addSteps(ctx context.Context, dest *buildbot.Build, src []*miloProto.Step_Substep, stepNamePrefix string) error {
	for _, substep := range src {
		...
		stepDst, err := c.step(ctx, stepSrc)
		if err != nil {
		...
		dest.Steps = append(dest.Steps, *stepDst)
		c.addSteps(ctx, dest, stepSrc.Substep, stepDst.Name+".")
	}
	return nil
}

// step converts an annotation step to a buildbot step.
func (c *annotationConverter) step(ctx context.Context, src *miloProto.Step) (*buildbot.Step, error) {
	...
	case !c.buildCompletedTime.IsZero():
		res.Results.Result = buildbot.Exception
	...
	for _, l := range allLinks {
		log, err := c.log(l)
		if err != nil {
			logging.WithError(err).Errorf(ctx, "could not convert link %q to buildbot", l.Label)
			continue
		}
	...
	if src.Started != nil {
		...
		if src.Ended != nil {
			...
			res.Times.Finish.Time = c.buildCompletedTime
		}
	}

	res.Text = append(res.Text, src.Text...)
	return res, nil
}

func (c *annotationConverter) log(src *miloProto.Link) (*buildbot.Log, error) {
	// This implementation is based on
	...
	case *miloProto.Link_LogdogStream:
		...
		if stream.Server == "" {
			stream.Server = c.logdogServer
		}
		if stream.Prefix == "" {
			stream.Prefix = c.logdogPrefix
		}
		u := url.URL{
			Scheme: "https",
			Host:   stream.Server,
			Path:   fmt.Sprintf("/v/?s=%s/%s", stream.Prefix, stream.Name),
		}
		log.URL = u.String()
