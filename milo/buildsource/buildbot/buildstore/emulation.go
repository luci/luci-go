...
func BucketOf(c context.Context, master, builder string) (string, error) {
	...
	v, err := bucketCache.GetOrCreate(c, key, func() (v interface{}, exp time.Duration, err error) {
		...
		err = retry.Retry(c, retry.Default, func() error {
			var err error
			switch res, err = ctxhttp.Get(c, &http.Client{Transport: transport}, u); {
			case err != nil:
				return err
			...
			logging.WithError(err).Warningf(c, "error while making RPC to migration app; retrying in %s", delay)
		})
		if err != nil {
			return
		}
		...
		if err := json.NewDecoder(res.Body).Decode(&body); err != nil {
			return nil, 0, errors.Reason("could not decode migration app's response body", err).Err()
		}

		// cache this for 10min in process memory and memcache.
		return body.Bucket, 10 * time.Minute, nil
	})
