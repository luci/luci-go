...
// GetMaster fetches the latest known master state and.
//
...
// ImportMaster persists the master in the storage.
func ImportMaster(
	c context.Context, master *buildbot.Master, internal bool, expireCallback ExpireCallback) error {
...
func cleanUpExpiredBuilds(c context.Context, master *buildbot.Master, expiredCallback ExpireCallback) error {
	...
	if err != nil {
		return errors.Annotate(err, "could not load current builds from master %q", master.Name).Err()
	}
	now := clock.Now(c)
	return parallel.WorkPool(10, func(work chan<- func() error) {
		for _, b := range builds {
			...
			if builder, ok := master.Builders[b.Buildername]; !ok {
				...
				if !found && (b.TimeStamp.IsZero() || b.TimeStamp.Time.Add(20*time.Minute).Before(now)) {
					reason = "build stale"
				}
			}
			if reason != "" {
...
func expireBuild(c context.Context, b *buildbot.Build) error {
	...
	if _, err := ImportBuild(c, b); err != nil {
		return err
	}
	return nil
}
