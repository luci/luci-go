...
func getLUCIBuilders(c context.Context, master string) ([]string, error) {
	...
	t := clock.Now(c)
	defer logging.Debugf(c, "getLUCIBuilders took %.2f secs", clock.Now(c).Sub(t).Seconds())
		...
		for _, bucket := range buildersResponse.Buckets {
			for _, builder := range bucket.Builders {
				...
				ch <- func() error {
					...
					logging.Debugf(c, "processing %s/%s (%s)", bucket.Name, builder.Name, prop.Mastername)
