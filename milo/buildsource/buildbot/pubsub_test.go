...
func newCombinedPsBody(bs []*protocol.Build, m *protocol.Master, internal bool) io.ReadCloser {
	bmsg := buildMasterMsg{
		Master: m,
		Builds: bs,
	}
...
func TestPubSub(t *testing.T) {
	Convey(`A test Environment`, t, func() {
		...
		Convey("Remove source changes", func() {
			m := &protocol.Master{
				...
				Builders: map[string]*protocol.Builder{
					"fake builder": {
						PendingBuildStates: []*protocol.Pending{
							{
								Source: protocol.SourceStamp{
									Changes: []protocol.Change{{Comments: "foo"}},
								},
							},
						},
					},
		...
		b := &protocol.Build{
			...
			Number:      1234,
			Currentstep: "this is a string",
			Times:       protocol.TimeRange{Start: unixTime(123)},
			TimeStamp:   ts,
		}

		Convey("Basic master + build pusbsub subscription", func() {
			h := httptest.NewRecorder()
			slaves := map[string]*protocol.Slave{}
			slaves["testslave"] = &protocol.Slave{
				Name:      "testslave",
				Connected: true,
				Runningbuilds: []*protocol.Build{
					{
						Master:      "Fake Master",
						Buildername: "Fake buildername",
						Number:      2222,
						Times:       protocol.TimeRange{Start: unixTime(1234)},
					},
				},
			}
			ms := protocol.Master{
				Name:     "Fake Master",
				Project:  protocol.Project{Title: "some title"},
				Slaves:   slaves,
				Builders: map[string]*protocol.Builder{},
			}

			ms.Builders["Fake buildername"] = &protocol.Builder{
				CurrentBuilds: []int{1234},
			}
			r := &http.Request{
				Body: newCombinedPsBody([]*protocol.Build{b}, &ms, false),
			}
			p := httprouter.Params{}
			...
			Convey("Expire non-existant current build", func() {
				...
				So(loadB.Results, ShouldEqual, protocol.Exception)
			})
		...
		Convey("Internal master + build pusbsub subscription", func() {
			...
			r := &http.Request{
				Body: newCombinedPsBody([]*protocol.Build{b}, &ms, true),
			}
			p := httprouter.Params{}
			PubSubHandler(&router.Context{
				Context: c,
...
func unixTime(sec int64) protocol.Time {
	return protocol.Time{time.Unix(sec, 0).UTC()}
}
