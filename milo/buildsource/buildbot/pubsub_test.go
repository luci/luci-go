...
func TestPubSub(t *testing.T) {
	Convey(`A test Environment`, t, func() {
		...
		//			{Property: "master"},
		//			{Property: "builder"},
		//			{Property: "number", Descending: true},
		//		},
		//	},
		//	&datastore.IndexDefinition{
		//		Kind: "buildbotBuild",
		...
		err = buildstore.ImportMaster(c, &buildbot.Master{Name: "Fake buildbotMasterEntry"}, false)
		So(err, ShouldBeNil)
		...
		Convey("Remove source changes", func() {
			...
			So(buildstore.ImportMaster(c, m, false), ShouldBeNil)
		...
		Convey("Save build entry", func() {
			...
			Convey("Load build entry", func() {
				loadB, err := buildstore.Build(c, "Fake buildbotMasterEntry", "Fake buildername", 1234)
				So(err, ShouldBeNil)
			...
			Convey("Query build entry", func() {
				...
				res, err := buildstore.Builds(c, buildstore.Query{})
				So(err, ShouldBeNil)
				So(len(res.Builds), ShouldEqual, 1)
				So(res.Builds[0].Currentstep.(string), ShouldEqual, "this is a string")

				Convey("Query for finished entries should be 1", func() {
					res, err := buildstore.Builds(c, buildstore.Query{Finished: buildstore.Yes})
					So(err, ShouldBeNil)
					So(len(res.Builds), ShouldEqual, 1)
					So(res.Builds[0].Currentstep.(string), ShouldEqual, "this is a string")
				})
				Convey("Query for unfinished entries should be 0", func() {
					res, err := buildstore.Builds(c, buildstore.Query{Finished: buildstore.No})
					So(err, ShouldBeNil)
		...
		Convey("Basic master + build pusbsub subscription", func() {
			...
			Convey("And stores correctly", func() {
				loadB, err := buildstore.Build(c, b.Master, b.Buildername, b.Number)
				So(err, ShouldBeNil)
				So(loadB.Master, ShouldEqual, "Fake buildbotMasterEntry")
				So(loadB.Currentstep, ShouldEqual, "this is a string")
			...
			Convey("Don't Expire non-existent current build under 20 min", func() {
				...
				So(h.Code, ShouldEqual, 200)
				loadB, err := buildstore.Build(c, "Fake buildbotMasterEntry", "Fake buildername", 1235)
				So(err, ShouldBeNil)
