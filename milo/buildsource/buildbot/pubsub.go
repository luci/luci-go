...
type buildMasterMsg struct {
	Master *protocol.Master  `json:"master"`
	Builds []*protocol.Build `json:"builds"`
}
...
func putDSMasterJSON(
	c context.Context, master *protocol.Master, internal bool) error {
...
func unmarshal(
	c context.Context, msg []byte) ([]*protocol.Build, *protocol.Master, error) {
	bm := buildMasterMsg{}
	if len(msg) == 0 {
...
func getOSInfo(c context.Context, b *protocol.Build, m *protocol.Master) (
...
func expireBuild(c context.Context, b *protocol.Build) error {
	b.Times.Finish = protocol.Time{clock.Now(c)}
	if !b.TimeStamp.IsZero() {
		b.Times.Finish = b.TimeStamp
	}
	b.Finished = true
	b.Results = protocol.Exception
	b.Currentstep = nil
...
func saveBuildSummary(c context.Context, b *protocol.Build) error {
...
func doMaster(c context.Context, master *protocol.Master, internal bool) int {
	...
	err = datastore.RunBatch(c, buildQueryBatchSize, q, func(b *protocol.Build) {
...
func pubSubHandlerImpl(c context.Context, r *http.Request) int {
	...
	for _, build := range builds {
		...
		if err != nil {
			if _, ok := err.(protocol.ErrTooBig); ok {
