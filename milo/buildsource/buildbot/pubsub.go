...
func expireBuild(c context.Context, b *buildbot.Build) error {
	...
	if _, err := buildstore.ImportBuild(c, b); err != nil {
		return err
	}
	return nil
}
...
func doMaster(c context.Context, master *buildbot.Master, internal bool) int {
	...
	err := buildstore.ImportMaster(c, master, internal)
	...
	current, err := buildstore.GetBuilds(c, buildstore.Query{
		Master:   master.Name,
		Finished: buildstore.No,
	})
	if err != nil {
		logging.WithError(err).Errorf(c, "Could not load current builds from master %q",
			master.Name)
		return http.StatusInternalServerError
	}
			...
			if expireReason != "" {
				...
				work <- func() error {
					...
					logging.Infof(c, "Expiring %s/%s/%d due to %s",
						master.Name, b.Buildername, b.Number, expireReason)
					return expireBuild(c, b)
				}
			}
		}
	})
	if err != nil {
		logging.WithError(err).Errorf(c, "could not expire builds")
		return http.StatusInternalServerError
	}
	return 0
}
...
func pubSubHandlerImpl(c context.Context, r *http.Request) int {
	...
	for _, build := range builds {
		...
		jsonish, _ := json.Marshal(build)
		logging.Infof(c, "Recording build %s", jsonish)
		...
		replaced, err := buildstore.ImportBuild(c, build)
		switch {
		case err == buildstore.ErrImportRejected:
			...
			logging.Errorf(c, "import of %s/%s/%d rejected", build.Master, build.Buildername, build.Number)
			continue
		case buildstore.IsTooBig(err):
