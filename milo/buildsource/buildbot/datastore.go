...
func runBuildsQuery(c context.Context, q *datastore.Query) ([]*protocol.Build, datastore.Cursor, error) {
	...
	builds := make([]*protocol.Build, 0, buildsAllocSize)

	var nextCursor datastore.Cursor
	err = datastore.RunBatch(c, buildQueryBatchSize, q, func(build *protocol.Build, getCursor datastore.CursorCB) error {
