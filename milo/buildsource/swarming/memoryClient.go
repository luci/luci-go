...
func parseAnnotations(c *streamclient.Client) (*rawpresentation.Streams, error) {
	fakeData := c.GetFakeData()
	if fakeData == nil {
		panic(errors.New("parseAnnotations can only be used with a `fake` protocol client"))
	}
