...
// console IDs.
func GetConsoleDefSummaries(c context.Context, consoleIDs []common.ConsoleID) (
	...
	return GetConsoleSummaries(c, defs)
}

func GetConsoleSummaries(c context.Context, defs []*common.Console) (map[common.ConsoleID]ui.ConsoleSummary, error) {
	...
	for i, def := range defs {
		...
		summary.Name = ui.NewLink(def.ID, fmt.Sprintf("/p/%s/g/%s/console", def.Project(), def.ID), ariaLabel)
		for j, builderID := range def.Builders {
			if builder, ok := bsMap[builderID]; ok {
				summary.Builders[j] = builder
			} else {
				bs[i] = &model.BuilderSummary{BuilderID: builderID}
			}
		}
