...
func (m *hostModule) Initialize(ctx context.Context, host module.Host, opts module.HostOptions) (context.Context, error) {
	if m.opts.APIHost == "" {
		return nil, errors.New("-luci-analysis-host must be set")
	}
...
var (
	clientContextKey = "go.chromium.org/luci/analysis/internal/hosts:setting"
)
...
// APIHost returns the hostname of the LUCI Analysis pRPC client.
// E.g. "analysis.api.luci.app".
func APIHost(ctx context.Context) (string, error) {
	...
	if strings.Contains(host, "VERSION") {
		...
		if version == "" {
			return "", errors.New("VERSION reference in LUCI Analysis hostname but GAE_VERSION is not set")
		}
