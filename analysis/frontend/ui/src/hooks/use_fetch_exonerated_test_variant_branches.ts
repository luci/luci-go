...
import { GitilesCommit, SourceRef } from '@/proto/go.chromium.org/luci/analysis/proto/v1/sources.pb';
...
const useFetchExoneratedTestVariantBranches = (project: string, clusterAlgorithm: string, clusterId: string) => {
  return useQuery(['exoneratedTestVariantBranches', project, clusterAlgorithm, clusterId], async () => {
    const clusterService = getClustersService();
    const clusterResponse = await clusterService.queryExoneratedTestVariantBranches({
      parent: `projects/${project}/clusters/${clusterAlgorithm}/${clusterId}/exoneratedTestVariantBranches`,
    });
    const clusterExoneratedTestVariantBranches = clusterResponse.testVariantBranches;
    ...
    const exoneratedTVBs = tvResponse.testVariants?.map((analyzedTV, i) => {
...
function testVariantBranchFromAnalysis(ctvb: ClusterExoneratedTestVariantBranch, analysis: TestVariantStabilityAnalysis): ExoneratedTestVariantBranch {
  return {
    testId: ctvb.testId,
