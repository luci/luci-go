...
import { getMockMetricsList, mockFetchMetrics } from '@/testing_tools/mocks/metrics_mock';
...
import { createMockProjectConfig, mockFetchProjectConfig } from '@/testing_tools/mocks/projects_mock';
...
describe('Test ProblemSection component', () => {
  ...
  it('given a rule with active or previously active problems, it should show those problems', async () => {
    ...
    expect(getByText(clsRejectedRow, 'many CL(s) are being rejected')).toBeInTheDocument();
    expect(queryByTestId(clsRejectedRow, 'problem_priority_chip')).toBeNull();
    expect(getByTestId(clsRejectedRow, 'problem_status_chip')).toHaveTextContent('Resolved');

    const exonerationsRow = screen.getByTestId('policy-row-exonerations');
    expect(exonerationsRow).toBeInTheDocument();
    expect(getByText(exonerationsRow, 'test variant(s) are being exonerated (ignored) in presubmit')).toBeInTheDocument();
    expect(getByTestId(exonerationsRow, 'problem_priority_chip')).toHaveTextContent('P2');
    expect(getByTestId(exonerationsRow, 'problem_status_chip')).toHaveTextContent('Active');

    // Open dialog for one problem.
    fireEvent.click(getByText(screen.getByTestId('policy-row-cls-rejected'), 'more info'));
    expect(await screen.findByText('Problem: many CL(s) are being rejected')).toBeInTheDocument();
    expect(await screen.findByText('Many changelists are unable to submit because of failures on this test.')).toBeInTheDocument();

    // Close dialog.
    fireEvent.click(screen.getByText('Close'));
    await waitFor(() => {
      expect(screen.queryByText('Problem: many CL(s) are being rejected')).toBeNull();
    });

    // Open dialog for the other problem.
    fireEvent.click(getByText(screen.getByTestId('policy-row-exonerations'), 'more info'));
    expect(await screen.findByText('Problem: test variant(s) are being exonerated (ignored) in presubmit')).toBeInTheDocument();
    expect(await screen.findByText('Test variant(s) in this cluster are too flaky to gate changes in CQ. As a result, functionality is not protected from breakage.')).toBeInTheDocument();

    // Close dialog.
    fireEvent.click(screen.getByText('Close'));
    await waitFor(() => {
      expect(screen.queryByText('Problem: test variant(s) are being exonerated (ignored) in presubmit')).toBeNull();
    });
  });
  ...
  it('given a rule with no active or previously active problems, it should show placeholder text', async () => {
    ...
    await screen.findByTestId('problem-summary');
    expect(screen.getByText('No problems have been identified.')).toBeInTheDocument();
  });
  ...
  it('given no policies configured in the project, it should show placeholder text', async () => {
    ...
    await screen.findByTestId('problem-summary');
    expect(screen.getByText('Configure bug management policies')).toBeInTheDocument();
  });
});
