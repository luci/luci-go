...
import { ExoneratedTestVariantBranch } from '@/hooks/use_fetch_exonerated_test_variant_branches';
import {
...
const ExonerationExplanationSection = ({
  ...
  const flakyCriteriaMet = flakyCriteriaMetIndicator(criteria, testVariantBranch);
  const failureCriteriaMet = failureCriteriaMetIndicator(criteria, testVariantBranch);
  ...
  return (
    <>
      <Accordion expanded={expanded == 'flaky'} onChange={(_, isExpanded: boolean) => setExpanded(isExpanded ? 'flaky' : '')}>
        <AccordionSummary
        ...
        <AccordionDetails data-testid='flaky_criteria_details'>
          <FlakyCriteriaSection testVariantBranch={testVariantBranch} criteria={criteria} />
        </AccordionDetails>
      </Accordion>
      <Accordion expanded={expanded == 'failure'} onChange={(_, isExpanded: boolean) => setExpanded(isExpanded ? 'failure' : '')}>
        <AccordionSummary
          expandIcon={<ExpandMoreIcon />}
        ...
        <AccordionDetails data-testid='failure_criteria_details'>
          <FailureCriteriaSection testVariantBranch={testVariantBranch} criteria={criteria} />
        </AccordionDetails>
      </Accordion>
    </>
  );
