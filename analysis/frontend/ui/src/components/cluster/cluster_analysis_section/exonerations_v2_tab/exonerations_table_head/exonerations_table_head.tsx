...
import { SortableField } from '@/components/cluster/cluster_analysis_section/exonerations_v2_tab/model/model';
...
const ExonerationsTableHead = ({
  ...
  return (
    <TableHead data-testid="exonerations_table_head">
      <TableRow>
        <TableCell
          sortDirection={sortField === 'testId' ? (isAscending ? 'asc' : 'desc') : false}
          sx={{ cursor: 'pointer' }}>
          <TableSortLabel
        ...
        <TableCell
          sortDirection={sortField === 'sourceRef' ? (isAscending ? 'asc' : 'desc') : false}
          sx={{ cursor: 'pointer' }}>
          <TableSortLabel
        ...
        <TableCell
          ...
          <TableSortLabel
            ...
            onClick={() => toggleSort('beingExonerated')}>
              Is Being Exonerated <HelpTooltip text='Whether the test variant is currently being exonerated (ignored) by presubmit.' />
          </TableSortLabel>
        </TableCell>
        <TableCell
          sortDirection={sortField === 'criticalFailuresExonerated' ? (isAscending ? 'asc' : 'desc') : false}
          sx={{ cursor: 'pointer' }}>
          <TableSortLabel
        ...
        <TableCell
          sortDirection={sortField === 'lastExoneration' ? (isAscending ? 'asc' : 'desc') : false}
          sx={{ cursor: 'pointer' }}>
          <TableSortLabel
