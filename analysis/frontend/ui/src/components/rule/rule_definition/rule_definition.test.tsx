...
describe('Test RuleInfo component', () => {
  ...
  it('given a rule with definition redacted, a message should be displayed', async () => {
    renderWithRouterAndClient(
        <RuleDefinition onEditClicked={() => {
          // Do nothing.
        }} />,
    );

    await screen.findByText('You do not have permission to view the rule definition.');
    expect(screen.getByText('You do not have permission to view the rule definition.')).toBeInTheDocument();
  });
});
