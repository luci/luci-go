...
type TestResultRow struct {
	...
	// Parent contains info of the result's immediate parent work unit (or legacy
	// invocation).
	Parent *TestResultRow_ParentRecord `protobuf:"bytes,7,opt,name=parent,proto3" json:"parent,omitempty"`
...
func (x *TestResultRow) GetParent() *TestResultRow_ParentRecord {
	if x != nil {
		return x.Parent
	}
	return nil
...
type TestResultRow_InvocationRecord struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The ID of the root invocation. Or, if the test result is
	// in a legacy invocation, the ID of the top-level legacy invocation
	// (for which Invocation.is_export_root is true).
	//
	// See `is_root_invocation` to determine whether the test result
	// is in a root invocation or a legacy invocation.
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// The LUCI Realm the invocation exists under.
	// For example, "chromium:try".
	Realm string `protobuf:"bytes,2,opt,name=realm,proto3" json:"realm,omitempty"`
	// Set to true if the test result is in a ResultDB root invocation,
	// instead of a legacy invocation.
	IsRootInvocation bool `protobuf:"varint,3,opt,name=is_root_invocation,json=isRootInvocation,proto3" json:"is_root_invocation,omitempty"`
	unknownFields    protoimpl.UnknownFields
	sizeCache        protoimpl.SizeCache
}
...
func (x *TestResultRow_InvocationRecord) GetIsRootInvocation() bool {
	if x != nil {
		return x.IsRootInvocation
	}
	return false
}

// ParentRecord for a test result is the immediate parent work unit
// or legacy invocation that directly contains the test result.
//
// If this test result is in a root invocation (invocation.is_root_invocation),
// then this is the immediate parent work unit, else this is the immediate parent
// legacy invocation.
type TestResultRow_ParentRecord struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The ID of the parent work unit (or legacy invocation).
	//
	// If this test result is in a root invocation (invocation.is_root_invocation),
	// then this is a work unit ID, else this is a legacy invocation ID.
	// Work unit IDs are unique only within a given root invocation.
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// Tags represents Work Unit or Legacy Invocation-level string key-value pairs.
	// A key can be repeated.
	Tags []*v1.StringPair `protobuf:"bytes,2,rep,name=tags,proto3" json:"tags,omitempty"`
	// The LUCI Realm the work unit or legacy invocation exists under.
	// For example, "chromium:try".
	Realm string `protobuf:"bytes,3,opt,name=realm,proto3" json:"realm,omitempty"`
	// Arbitrary JSON object that contains structured, domain-specific properties
	// of the work unit or legacy invocation. Stored here stringified as this is
	// the only protocol buffer type that maps to the JSON BigQuery type:
	// https://cloud.google.com/bigquery/docs/write-api#data_type_conversions
...
func (x *TestResultRow_ParentRecord) Reset() {
	*x = TestResultRow_ParentRecord{}
	mi := &file_go_chromium_org_luci_analysis_proto_bq_test_result_row_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TestResultRow_ParentRecord) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TestResultRow_ParentRecord) ProtoMessage() {}

func (x *TestResultRow_ParentRecord) ProtoReflect() protoreflect.Message {
	mi := &file_go_chromium_org_luci_analysis_proto_bq_test_result_row_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use TestResultRow_ParentRecord.ProtoReflect.Descriptor instead.
func (*TestResultRow_ParentRecord) Descriptor() ([]byte, []int) {
	return file_go_chromium_org_luci_analysis_proto_bq_test_result_row_proto_rawDescGZIP(), []int{0, 1}
}

func (x *TestResultRow_ParentRecord) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
...
func (x *TestResultRow_ParentRecord) GetTags() []*v1.StringPair {
	if x != nil {
		return x.Tags
	}
	return nil
...
func (x *TestResultRow_ParentRecord) GetRealm() string {
	if x != nil {
		return x.Realm
	}
	return ""
...
func (x *TestResultRow_ParentRecord) GetProperties() string {
	if x != nil {
		return x.Properties
	}
	return ""
...
const file_go_chromium_org_luci_analysis_proto_bq_test_result_row_proto_rawDesc = "" +
	...
	"\fvariant_hash\x18\x04 \x01(\tR\vvariantHash\x12P\n" +
	"\n" +
	"invocation\x18\x05 \x01(\v20.luci.analysis.bq.TestResultRow.InvocationRecordR\n" +
	"invocation\x12A\n" +
	"\x0epartition_time\x18\x06 \x01(\v2\x1a.google.protobuf.TimestampR\rpartitionTime\x12D\n" +
	"\x06parent\x18\a \x01(\v2,.luci.analysis.bq.TestResultRow.ParentRecordR\x06parent\x12\x12\n" +
	"\x04name\x18\b \x01(\tR\x04name\x12\x1b\n" +
	"\tresult_id\x18\t \x01(\tR\bresultId\x12\x1a\n" +
	"\bexpected\x18\n" +
	" \x01(\bR\bexpected\x12:\n" +
	"\x06status\x18\v \x01(\x0e2\".luci.analysis.v1.TestResultStatusR\x06status\x12@\n" +
	"\tstatus_v2\x18\x19 \x01(\x0e2#.luci.analysis.v1.TestResult.StatusR\bstatusV2\x12!\n" +
	"\fsummary_html\x18\f \x01(\tR\vsummaryHtml\x129\n" +
	"\n" +
	"start_time\x18\r \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x12#\n" +
	"\rduration_secs\x18\x0e \x01(\x01R\fdurationSecs\x120\n" +
	"\x04tags\x18\x0f \x03(\v2\x1c.luci.analysis.v1.StringPairR\x04tags\x12F\n" +
	"\x0efailure_reason\x18\x10 \x01(\v2\x1f.luci.resultdb.v1.FailureReasonR\rfailureReason\x12\x1f\n" +
	"\vskip_reason\x18\x11 \x01(\tR\n" +
	"skipReason\x12F\n" +
	"\x0eskipped_reason\x18\x1a \x01(\v2\x1f.luci.resultdb.v1.SkippedReasonR\rskippedReason\x12*\n" +
	"\n" +
	"properties\x18\x12 \x01(\tB\n" +
	"\xe2\xbc$\x06\n" +
	"\x04JSONR\n" +
	"properties\x123\n" +
	...
	"\vinsert_time\x18\x17 \x01(\v2\x1a.google.protobuf.TimestampR\n" +
	"insertTime\x1af\n" +
	"\x10InvocationRecord\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x14\n" +
	"\x05realm\x18\x02 \x01(\tR\x05realm\x12,\n" +
	"\x12is_root_invocation\x18\x03 \x01(\bR\x10isRootInvocation\x1a\x92\x01\n" +
	"\fParentRecord\x12\x0e\n" +
	...
	"\xe2\xbc$\x06\n" +
	"\x04JSONR\n" +
	"propertiesB-Z+go.chromium.org/luci/analysis/proto/bq;bqpbb\x06proto3"
...
var file_go_chromium_org_luci_analysis_proto_bq_test_result_row_proto_goTypes = []any{
	...
	(*TestResultRow_ParentRecord)(nil),     // 2: luci.analysis.bq.TestResultRow.ParentRecord
...
var file_go_chromium_org_luci_analysis_proto_bq_test_result_row_proto_depIdxs = []int32{
	...
	2,  // 3: luci.analysis.bq.TestResultRow.parent:type_name -> luci.analysis.bq.TestResultRow.ParentRecord
