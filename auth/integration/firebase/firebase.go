...
// Package firebase implements a hacky shim that makes firebase-tools use LUCI
// local auth. Firebase thinks it's using OAuth2 flow, while in fact it is
...
func (s *Server) serve(ctx context.Context, l net.Listener, wg *sync.WaitGroup, secret string) error {
	...
	mux.Handle("/firebase/authorization", &handler{ctx, wg, func(rw http.ResponseWriter, r *http.Request) {
		// Authorization URI is normally used during interactive login to eventually
		// generate a refresh token. Since we pass the refresh token  already, it
		// must never be called.
		rw.WriteHeader(http.StatusNotImplemented)
	}})

	mux.Handle("/firebase/token", &handler{ctx, wg, func(rw http.ResponseWriter, r *http.Request) {
