...
type CommandParams struct {
	...
	// UseScopeFlags specifies whether scopes-related flags must be registered.
	//
	// This is primarily used by `luci-auth` executable.
	//
	// UseScopeFlags is *not needed* for command line tools that call to a fixed
...
type Flags struct {
	...
	iamScopes     bool   // value of -iam-scopes
	contextScopes bool   // value of -context-scopes
}
...
func (fl *Flags) Register(f *flag.FlagSet, defaults auth.Options) {
	...
	if len(fl.defaults.Scopes) == 0 {
		fl.defaults.Scopes = []string{auth.OAuthScopeEmail}
	}
...
// registerScopesFlags can be called after Register to add scopes-related flags.
func (fl *Flags) registerScopesFlags(f *flag.FlagSet) {
	...
	f.BoolVar(&fl.iamScopes, "iam-scopes", false,
		"When set, use scopes needed to impersonate accounts via IAM. Overrides -scopes when present.")
	f.BoolVar(&fl.contextScopes, "context-scopes", false,
...
func (fl *Flags) Options() (auth.Options, error) {
	...
	if fl.iamScopes && fl.contextScopes {
		return auth.Options{}, fmt.Errorf("-iam-scopes and -context-scopes can't be used together")
	}

	switch {
	case fl.iamScopes:
		opts.Scopes = []string{auth.OAuthScopeIAM}
	case fl.contextScopes:
		opts.Scopes = append([]string(nil), allContextScopes...)
	default:
...
func (c *commandRunBase) askToLogin(opts auth.Options, forContext bool) {
	...
	if forContext {
		...
		case opts.ActAsServiceAccount != "":
			...
			loginFlags = []string{"-iam-scopes"}
		default:
			// When not acting, need all scopes used by `luci-auth context`.
			loginFlags = []string{"-context-scopes"}
		}
	} else {
		// Ask for custom scopes only if they were actually requested.
		if !isSameScopes(opts.Scopes, c.flags.defaults.Scopes) {
