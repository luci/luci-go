...
type Options struct {
	...
	// used to call `MintServiceAccountToken` LUCI Token Server's RPC. This
	// requires following the LUCI permissions in the realm specified by
	// `ActViaLUCIRealm`:
	...
	//  "<project>".
	//
	// Using this option requires `TokenServerHost` to be provided.
	//
	// Default: none.
...
//
// When it is not actingModeNone, baseToken != authToken.
...
func (a *Authenticator) ensureInitialized() error {
	...
	// the API used to generate main auth tokens. In non-actor mode, the base
	// token is also the main auth token, so scope it to whatever options were
	// requested.
	var scopes []string
	switch a.actingMode() {
	case actingModeNone:
	...
	// bother with fetching 'authToken' from cache. It will be fetched lazily on
	// the first use.
