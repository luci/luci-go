...
var (
	...
	// ErrIDTokensNotSupported is returned by GetAccessToken when UseIDTokens
	// option is true, but the authentication method doesn't actually support
	// ID tokens.
	ErrIDTokensNotSupported = errors.New("ID tokens are not supported by this authenticator")
)
...
type Options struct {
	...
	//
	// Ignored if UseIDTokens is true.
	//
...
func NewAuthenticator(ctx context.Context, loginMode LoginMode, opts Options) *Authenticator {
	// Add default scope, sort scopes. When using ID tokens we need only the email
	// scope.
	if opts.UseIDTokens || len(opts.Scopes) == 0 {
...
func (a *Authenticator) GetAccessToken(lifetime time.Duration) (*oauth2.Token, error) {
	...
	if a.opts.UseIDTokens {
		if tok.IDToken == "" || tok.IDToken == internal.NoIDToken {
			return nil, ErrIDTokensNotSupported
		}
		return &oauth2.Token{
