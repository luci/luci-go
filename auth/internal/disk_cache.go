...
func (c *DiskTokenCache) absPathOld() string {
	return filepath.Join(c.SecretsDir, "creds.json")
}

func (c *DiskTokenCache) absPathNew() string {
...
func (c *DiskTokenCache) readCache() (*cacheFile, time.Time, error) {
	oldCache, err := c.readCacheFile(c.absPathOld())
	if err != nil {
		return nil, time.Time{}, err
	}
	newCache, err := c.readCacheFile(c.absPathNew())
	if err != nil {
		return nil, time.Time{}, err
	}

	// Merge tokens from oldCache into newCache, but don't override anything.
	seen := stringset.New(len(newCache.Cache))
	for _, entry := range newCache.Cache {
		seen.Add(entry.key.ToMapKey())
	}
	for _, entry := range oldCache.Cache {
		if !seen.Has(entry.key.ToMapKey()) {
			newCache.Cache = append(newCache.Cache, entry)
		}
	}

	// If oldCache didn't exist at all, pretend it was touched in distant past.
	// This avoid weird looking "0001-01-01" dates. Seventies were better.
	if oldCache.LastUpdate.IsZero() {
		oldCache.LastUpdate = time.Date(1970, time.January, 01, 0, 0, 0, 0, time.UTC)
	}

	return newCache, oldCache.LastUpdate, nil
}
...
func (c *DiskTokenCache) updateCacheFiles(cb func(*cacheFile, time.Time) bool) error {
	...
	cache, oldLastUpdate, err := c.readCache()
	if err != nil {
		return err
	}
	...
	// creds.json.
	cache.LastUpdate = oldLastUpdate
	if err := c.writeCacheFile(c.absPathOld(), cache); err != nil {
		return err
	}

	// Update tokens.json as usual, updating its `last_update` field.
	cache.LastUpdate = now
	return c.writeCacheFile(c.absPathNew(), cache)
}
