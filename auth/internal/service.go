...
func (p *serviceAccountTokenProvider) CacheKey(ctx context.Context) (*CacheKey, error) {
	...
	// When using ID tokens, put the audience as an fake scope in the cache key.
	// Cache keys are used only for map indexing, their exact content doesn't
	// matter. Adding a separate field (like `Audience`) to the key causes
	// complication with older binaries that don't know about this field, so
	// we abuse `Scopes` field instead.
	scopes := p.scopes
	if p.useIDTokens {
