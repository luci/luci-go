...
func NewGCETokenProvider(ctx context.Context, account string, scopes []string) (TokenProvider, error) {
	...
	// there. It's presence is enough to cover all scopes used in practice. The
	// exception is non-Cloud scopes (like gerritcodereview). To use such scopes,
	// one will have to use impersonation (see ActAsServiceAccount in auth.go).
