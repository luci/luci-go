...
// global mutex to serialize calls to it from within this process. See also
// comments in MintToken for how we do loose form of collaboration across
// processes.
...
func attemptInit(ctx context.Context, account string, scopes []string) (TokenProvider, error) {
	...
	if err != nil {
		...
		// metadata.OnGCE, and it handles slow gke-metadata-server better. The
		// downside is that if we indeed are NOT on GCE, it may take up to 15 sec to
