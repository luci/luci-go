...
type Context struct {
	...
	//
	// TODO(vadimsh): Delete this method if EnableCloudEmulation works everywhere.
	EnableDevShell bool

	// EnableCloudEmulation enables emulation of GCE instance environment.
	//
	...
	// Note: these fields are used in both EnableCloudEmulation and
	// EnableDevShell modes.
...
func (ac *Context) Launch(ctx context.Context, tempDir string) (nc context.Context, err error) {
	// EnableCloudEmulation provides a superset of EnableDevShell features. No
	// need to have both enabled at the same time (they also conflict with each
	// other).
	if ac.EnableCloudEmulation {
	...
	if ac.EnableCloudEmulation {
		if err := ac.setupCloudEmulationAuth(tempDir); err != nil {
			return nil, err
		}
	}
...
func (ac *Context) ExportIntoEnv(env environ.Env) environ.Env {
	...
	if ac.EnableDevShell || ac.EnableCloudEmulation {
...
func (ac *Context) Report() {
	...
	logging.Infof(ac.ctx,
		"%q account is %s (git_auth: %v, devshell: %v, emulate_cloud:%v, docker:%v, firebase: %v)",
		ac.ID, account, ac.EnableGitAuth, ac.EnableDevShell, ac.EnableCloudEmulation,
...
func (ac *Context) setupCloudEmulationAuth(tempDir string) error {
