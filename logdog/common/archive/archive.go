...
func archiveLogs(w io.Writer, d *logpb.LogStreamDescriptor, logC <-chan *logpb.LogEntry, idx *indexBuilder, cloudLogger CLLogger, streamIDHash []byte, logger logging.Logger) error {
	...
	//
	// If there was an error, the buffered line can possibly contain an incomplete line, which was
	// going to be completed in the next LogEntry. If so, skip flushing out the buffered line to
	// prevent the complete version of the line from being deduped on the next retry of
	// the archival task.
	if err == nil && le != nil {
