...
func archiveLogs(w io.Writer, d *logpb.LogStreamDescriptor, logC <-chan *logpb.LogEntry, idx *indexBuilder, cloudLogger CLLogger, streamIDHash []byte, logger logging.Logger) error {
	...
	//
	// If there was an error, skip flushing it out. It can possibly contain a partial line, which
	// was going to be completed by the line in the next LogEntry. If the partial line is flushed
	// out, the complete version of the line will be deduped out on the next retry.
	if err == nil && le != nil {
