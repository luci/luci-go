...
//
// A log message is identified by the stream_index of the message, and a message can contain
...
//
// 512KB is large enough to contain most of a single log message. If a message contains
...
func archiveLogs(w io.Writer, d *logpb.LogStreamDescriptor, logC <-chan *logpb.LogEntry, idx *indexBuilder, cloudLogger CLLogger, streamIDHash []byte, logger logging.Logger) error {
	...
	for le = range logC {
		...
		// If the last line is not a partial line, the line is the last line of the log message.
		// Export the message if so.
