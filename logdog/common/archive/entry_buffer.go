...
func (eb *entryBuffer) append(e *logpb.LogEntry) []*cl.Entry {
	...
	for i, line := range lines {
		...
		if eb.isExceeding(line) || i == len(lines)-1 {
			...
			eb.buf.Write(line.Value)
			// Add the Delimiter only if it's not going to exceed maxPayload with the buffered
			// content. This is to trim out the delimiter of the last line added to the payload
			// in each CloudLogging entry.
			//
			// If the delimiter was added unconditionally, all the CloudLogging entries will
			// have an ending delimiter, such as \n, rendered in the CloudLogging UI.
			// e.g.,
			// Entry1: "line1
			// "
			// Entry1: "line2
