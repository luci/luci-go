...
func (eb *entryBuffer) Append(e *logpb.LogEntry) (entries []*cl.Entry) {
	...
	//
	// Both are sementically complete lines, but (a) is actually a partial line, and they are
	// handled differently, when it is going to exceed the length limit with the buffered lines.
	//
	...
	for i, line := range lines {
		...
		if eb.isExceeding(line) || i == len(lines)-1 {
			// If the last line has an empty value, skip flushing the line.
			if len(line.Value) > 0 {
...
func (eb *entryBuffer) addLine(line *logpb.Text_Line, writeDelimiter bool) {
	...
	if writeDelimiter && writable-toWrite >= len(line.Delimiter) {
