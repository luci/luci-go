...
// ErrIncompleteStream is returned by Fetcher if Options.CompleteStream was true
// and the underlying Stream is still incomplete (i.e. has not yet been
// terminated by the client, or archived).
...
func (f *Fetcher) fetch(c context.Context) {
	...
	for tidx < 0 || lastSentIndex < tidx {
		...
		case resp := <-logFetchC:
			...
			if f.o.CompleteStream && resp.tidx == -1 {
