...
func TestHandleArchive(t *testing.T) {
	...
	Convey(`A testing archive setup`, t, func() {
		...
		// mock project config
		pcfg := map[config.Set]cfgmem.Files{
			"services/${appid}": {
		...
		c = cfgclient.Use(c, cfgmem.New(pcfg))
		...
		Convey(`With a project name, of which config doesn't exitt, will fail and consume the task`, func() {
