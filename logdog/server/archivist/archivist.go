...
type Archivist struct {
	...
	CLClientFactory func(ctx context.Context, luciProject, clProject string, useProjectScope bool) (*cl.Client, error)
}
...
func (a *Archivist) makeStagedArchival(c context.Context, project string,
	...
	var clClient *cl.Client
	if st.CloudLoggingProjectID != "" {
		log.Infof(c, "Log stream will be exported to %s via CloudLogging.", st.CloudLoggingProjectID)
		clClient, err = a.CLClientFactory(c, project, st.CloudLoggingProjectID, st.CloudLoggingWithProjectScope)
		if err != nil {
			log.Fields{
				log.ErrorKey:   err,
				"protoVersion": ls.State.ProtoVersion,
...
func (sa *stagedArchival) Close() error {
	...
	return errors.MultiError{sa.gsclient.Close(), clErr}
}
