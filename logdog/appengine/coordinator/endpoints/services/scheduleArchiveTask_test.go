...
func TestScheduleArchiveTask(t *testing.T) {
	//t.Parallel()
	...
	Convey(`With a testing configuration`, t, func() {
		...
		Convey(`Returns Forbidden error if not a service.`, func() {
			...
			_, err := svr.ScheduleArchiveTask(c, &logdog.ArchiveDispatchTask{})
		...
		tls.WithProjectNamespace(c, func(c context.Context) {
			Convey(`Dispatch an archive task`, func() {
				...
				Convey(`And it makes it to the Archival Publisher`, func() {
					...
					So(tls.State.ArchivalKey, ShouldNotEqual, []byte{})
					// The first archive task does not increment the retry counter.
					So(tls.State.ArchiveRetryCount, ShouldEqual, 0)
					So(tls.State.ArchivalState().Archived(), ShouldEqual, false)
			...
			Convey(`Dispatch second archive`, func() {
				...
				env.IterateTumbleAll(c)
				So(tls.Get(c), ShouldBeNil)
				So(tls.State.ArchiveRetryCount, ShouldEqual, 1)
				So(env.ArchivalPublisher.Hashes(), ShouldResemble, []string{string(tls.Stream.ID)})
			})
		})
	})
