...
func newArchiveTask(project, id string) *logdog.ArchiveTask {
	return &logdog.ArchiveTask{
		Project: project,
		Id:      id,
	}
}
...
func TaskArchival(c context.Context, state *coordinator.LogStreamState, delay time.Duration, experimentPercent uint32) error {
	...
	log.Debugf(c, "%d < %d, using new pipeline with 47hr delay", randNum, experimentPercent)
	...
	project := "luci." + state.Parent.Namespace()
	id := string(state.ID())
	t, err := tqTask(newArchiveTask(project, id))
	if err != nil {
		log.WithError(err).Errorf(c, "could not create archival task")
		return grpcutil.Internal
	}
	t.Delay = 47 * time.Hour
