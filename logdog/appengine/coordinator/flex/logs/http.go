...
func parse(pathStr string) (project types.ProjectName, path types.StreamPath, err error) {
	...
	if len(parts) != 3 {
		err = fmt.Errorf("Invalid path %q", pathStr)
		return
	}
...
func fetch(c context.Context, ch chan<- logResp, st storage.Storage, lst *coordinator.LogStreamState, path types.StreamPath) {
	...
	backoff := time.Second // How long to wait between
	var err error
	for {
		...
		// Do the actual work.
		nextIndex, err := fetchOnce(c, ch, st, path, lst.ArchivalState().Archived(), index)
		if err != nil {
			ch <- logResp{nil, err}
			return
		}
		...
		if 0 <= lst.TerminalIndex && lst.TerminalIndex <= int64(nextIndex) {
			logging.Fields{
...
func fetchOnce(c context.Context, ch chan<- logResp, st storage.Storage, path types.StreamPath, archived bool, index types.MessageIndex) (types.MessageIndex, error) {
	...
	var ierr error
	// Get as many log entries as we can.  The bigtable library performs retries for us.
...
func writeErrHeaders(w http.ResponseWriter, err error) {
	...
	httpCode, ok := grpcutil.HttpCode[code]
	if !ok {
		httpCode = http.StatusInternalServerError
	}
	http.Error(w, "Error: "+err.Error(), httpCode)
}
	...
	for logResp := range ch {
		...
		for _, line := range log.GetText().GetLines() {
			if _, err := fmt.Fprintln(w, line.GetValue()); err != nil {
				return fmt.Errorf("could not write line %s", line.GetValue())
			}
		}
		// Flush the ResponseWriter buffer.  The go ResponseWriter will not flush
		// until it reaches its threshold.
		flusher.Flush()
	}
	return nil
}
...
func GetHandler(ctx *router.Context) {
	...
	if err := serve(data.ch, ctx.Writer); err != nil {
