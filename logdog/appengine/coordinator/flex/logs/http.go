...
<header>
<div>{{ if .Link }}<a href="{{ .Link }}">Link to build</a>{{ end }}</div>
<div>
	{{ if .IsAnonymous }}
...
func resolveOptions(request *http.Request, pathStr string) (options userOptions, err error) {
	// Try to be smart, and detect if the user is a web browser, or a CLI tool (cURL).
...
func writeFooter(ctx *router.Context, start time.Time, err error, format string) {
	...
	// TODO(hinoka): If there is an error, we can signal an error by acquiring the underlying TCP connection
	...
	for logResp := range data.ch {
		...
		for _, line := range log.GetText().GetLines() {
			...
			if data.options.format == formatHTML {
				...
				if err != nil {
					text = fmt.Sprintf("<div class=\"line\">%s</div>", log.GetTimeOffset(), text)
				} else {
