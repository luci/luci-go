...
type durationInfoStruct struct {
	Timestamp int64 // Delta since start of log stream, in Milliseconds.
...
func (d durationInfoStruct) Style() (result string) {
	if d.TextColor != "" {
...
func linearScale(from, to uint8, scale float64) uint8 {
	return from + uint8(float64(to-from)*scale)
}
...
<div class="line">
	{{ if .DurationInfo.Text }}
		<div class="timestamp" id="{{.ID}}" onclick="window.location.hash='{{.ID}}'"
				 data-timestamp="{{.DurationInfo.Timestamp}}" data-delta="{{.DurationInfo.Delta}}"
