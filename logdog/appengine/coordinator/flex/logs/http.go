...
// TODO(hinoka): Include log description.
func startFetch(c context.Context, pathStr string) (<-chan logResp, *logDesc, error) {
	...
	// Perform the ACL checks, this also installs the project name into the context.
	c, err = coordinator.WithAccess(c, project, path, coordinator.NamespaceAccessREAD)
	if err != nil {
		return nil, nil, err
	}
	logging.Debugf(c, "passed access")
...
// through ch.
// The LogStreamState is used purely to test for the Terminal Index.
