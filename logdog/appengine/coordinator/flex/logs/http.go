...
// matchStreams returns streams that are matched by the pattern, and are not purged.
func matchStreams(streams []*coordinator.LogStream, pattern string) []*coordinator.LogStream {
	...
	r := regexp.MustCompile(strings.Join(doubles, ".*"))
...
// resolvePaths takes a path containing a wildcard and resolves it to the list
// of all known paths.  Purge checks are also preformed here.
...
func resolveParams(c context.Context, streams []*coordinator.LogStream) ([]fetchParams, error) {
	...
	// Get the backend storage instances.  This will be closed in fetch.
	for i, stream := range streams {
...
func startFetch(c context.Context, request *http.Request, pathStr string) (data logData, err error) {
	...
	params, err := resolveParams(c, streams)
	if err != nil {
		return
	}
...
// The LogStreamState is used purely for checking Terminal Indicies.
func fetch(c context.Context, ch chan<- logResp, allParams []fetchParams) {
	...
	if len(allParams) > 1 {
		...
		ch <- logResp{nil, nil, fmt.Errorf("Only single stream supported, found %d", len(allParams))}
		return
	}
