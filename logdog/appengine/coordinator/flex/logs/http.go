...
// before we start waiting.  Each request has a max byte size of 16MB,
// as specified in getBytesLimit.
var maxBuffer = 2
...
func fetch(c context.Context, ch chan<- logResp, project, path string) {
	// Get the server instance.  Test instances can be installed in the context,
	// the production instance is defined as the server struct in service.go.
	server := getServer(c)
