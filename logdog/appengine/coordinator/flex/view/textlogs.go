...
func (fs *fetcherSource) LogEntries(c context.Context, lr *fetcher.LogRequest) (
	...
	if lr.Count > 0 {
		logs = make([]*logpb.LogEntry, 0, lr.Count)
	}

	if err := fs.runStorageRequest(c, sreq, logs); err != nil {
		return nil, fs.tidx, err
	}
	return logs, fs.tidx, nil
}
...
func parseProjectAndPath(v string) (project types.ProjectName, path types.StreamPath, err error) {
	...
	if err = project.Validate(); err != nil {
		err = errors.Annotate(err, "invalid project name").Err()
		return
	}
	if err = path.Validate(); err != nil {
		err = errors.Annotate(err, "invalid log stream path").Err()
		return
	}
	return
}
