...
func CheckAdminUser(c context.Context) (bool, error) {
	cfg, err := config.Config(c)
	if err != nil {
		return false, err
	}
	return checkMember(c, "ADMIN", cfg.Coordinator.AdminAuthGroup)
}
...
//
// Logs the outcome inside. The error is non-nil only if the check itself fails.
func CheckServiceUser(c context.Context) (bool, error) {
	cfg, err := config.Config(c)
	if err != nil {
		return false, err
	}
	return checkMember(c, "SERVICE", cfg.Coordinator.ServiceAuthGroup)
}
...
//
// Logs the outcome inside. The error is non-nil only if the check itself fails.
func CheckProjectReader(c context.Context, pcfg *svcconfig.ProjectConfig) (bool, error) {
	return checkMember(c, "READ", pcfg.ReaderAuthGroups...)
}
...
//
// Logs the outcome inside. The error is non-nil only if the check itself fails.
func CheckProjectWriter(c context.Context, pcfg *svcconfig.ProjectConfig) (bool, error) {
	return checkMember(c, "WRITE", pcfg.WriterAuthGroups...)
}
