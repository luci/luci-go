...
var (
	...
	// PermLogsList is a permission required for listing streaming in a prefix.
...
func HasPermission(ctx context.Context, perm realms.Permission, realm string) (bool, error) {
	// Assert no cross-project mix up is happening.
	if realm != "" {
		if proj, _ := realms.Split(realm); proj != Project(ctx) {
			panic(fmt.Sprintf("invalid HasPermission call with realm %q while in project %q", realm, Project(ctx)))
		}
	}
