...
var (
	...
	archivalStateKey = "_ArchivalState"
)
...
func doQueryStat(c context.Context, ns string, stat queryStat) error {
	...
	if err := datastore.RunBatch(c, 128, q, func(state *datastore.PropertyMap) error {
		asRaw, ok := state.GetMeta(archivalStateKey)
		if !ok {
			logging.Errorf(c, "no archive state for %v", state)
		}
		as, ok := asRaw.(coordinator.ArchivalState)
		if !ok {
			logging.Errorf(c, "%v is not an archival state", asRaw)
		}
