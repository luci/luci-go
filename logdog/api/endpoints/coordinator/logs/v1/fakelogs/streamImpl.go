...
type Stream struct {
	...
	srvSrv      services.ServicesServer
...
func (s *Stream) Write(bs []byte) (int, error) {
	...
	entry := &logpb.LogEntry{
		TimeOffset:  google.NewDuration(s.start.Sub(clock.Now(s.ctx))),
	...
	s.storage.PutEntries(s.ctx, coordinatorTest.AllAccessProject, s.pth, entry)
...
func (s *Stream) Close() error {
	_, err := s.srvSrv.TerminateStream(s.ctx, &services.TerminateStreamRequest{
		Project:       coordinatorTest.AllAccessProject,
		Id:            s.streamID,
		Secret:        s.secret,
		TerminalIndex: s.streamIndex,
	})
	return err
}
