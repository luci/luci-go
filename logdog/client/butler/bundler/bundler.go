...
//     - Aggressively pack expired data into a Bundle. Skipped the expired
//       stream if the stream is drained (which will be unregistered later)
//       or the current data in the stream buffer can't be generate a bundle
//       entry (e.g. only partial size header exists in buffer) (This will
//       allow more data coming in when the stream is revisisted in the next
//       bundle round).
