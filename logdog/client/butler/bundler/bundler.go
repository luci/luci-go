...
//     - Aggressively pack expired data into a Bundle. Skip the expired stream
//       if the stream is drained (which will be unregistered later) or can't
//       generate a new bundle entry with the current data in the stream buffer
//       (e.g. only partial size header exists in buffer). This will allow more
//       data coming in when the stream is revisisted in the next bundle round.
