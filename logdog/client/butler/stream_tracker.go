...
func (s *streamTracker) CheckClosed(name types.StreamName) error {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.checkClosedLocked(name)
}

func (s *streamTracker) checkClosedLocked(name types.StreamName) error {
...
func (s *streamTracker) RegisterStream(name types.StreamName) error {
	if err := s.CheckClosed(name); err != nil {
		return errors.Annotate(err, "stream %q", name).Err()
	}

	s.mu.Lock()
	defer s.mu.Unlock()
	if err := s.checkClosedLocked(name); err != nil {
		return errors.Annotate(err, "stream %q", name).Err()
	}
