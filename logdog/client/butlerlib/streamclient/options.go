...
//
// If the implementation doesn't support returning an *os.File, it should return
// some other io.WriteCloser instead (as opposed to failing).
//
// Note that because os/exec has special handling for *os.File, this should be
// used in the following manner (error checks omitted for brevity). The
// important thing to notice is that stdout must be Close'd after Wait (which
// could be accomplised by a defer). Failure to do this will keep the Stream
// open from the butler's point of view, which will prevent the butler from
// closing down.
//
...
// Fake
//
// The "fake" protocol ignores this option.
//
// Null
//
...
// Windows - TCP Sockets
//
// Windows does not currently support this option for "tcp4"/"tcp6" protocol
// streams. Setting this option will have no effect. See `New` and comments in
// `net.go` for details.
//
// Mac and Linux - TCP or Unix Domain Sockets
//
// This stream will be dialed as a regular unix socket, but will be converted to
// an *os.File via `"net".TCPConn.File()` or `"net".UnixConn.File()`,
// respectively, and the original file handle ("Conn") closed.
//
// Accidentally using a `ForProcess` stream in the current process isn't
// extremely harmful (becase Go has a decent event-based IO system for *NIX
// systems). However, you will no longer be able to cast the returned stream to
// a "net".Conn, meaning you will not be able to e.g. SetDeadline, should that
