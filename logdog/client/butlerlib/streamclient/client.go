...
// return an initialized, raw
type dialer interface {
	// if forProcess is true, this should try its best to return an *os.File, if
	// the dialer supports it.
...
// you can then use to stream data to LogDog.
type Client struct {
	dial dialer
...
//
//   tcp4:host:port (All platforms*)
//   tcp6:host:port (All platforms*)
//
// Connects to a TCP server at the given "host:port", which may be any string
// resolvable by `"net".ResolveTCPAddr`.
//
// Although this works on all platforms, the `ForProcess` feature does not work
// on Windows (i.e. setting ForProcess on a tcp stream will have no effect).
// This is because Golang opens all sockets as OVERLAPPED (which it should) and
// Windows doesn't allow an OVERLAPPED HANDLE to be attached to stdout/stderr.
// See `net.go` for details on how to fix this if you need it.
//
// Theoretically we would use tcp for running the butler on the outside of
// a vm or docker container, but as of August 2019 we haven't done so.
//
//   fake (All platforms)
//
// Sinks all connections and writes into a fake data sink. You can retrieve the
// collected data at any time by using `Client.GetFakeData`. Passing an extra
// spec (like `fake:extra`) is an error. Useful for tests.
//
//   null (All platforms)
//
...
// NewDatagramStream returns a new open binary stream to the butler.
//
...
// "fake" protocol. You can retrive this with Client.GetFakeData().
...
// GetFakeData returns the fake testing data for this Client, if it was opened
// with the "fake" protocol.
//
// Otherwise it returns nil.
func (c *Client) GetFakeData() map[types.StreamName]FakeStreamData {
	if d, ok := c.dial.(*fakeDialer); ok {
		return d.dumpFakeData()
	}
	return nil
}

// SetFakeError sets the fake testing error for this Client, if it was opened
// with the "fake" protocol.
//
// Otherwise does nothing.
func (c *Client) SetFakeError(err error) {
	if d, ok := c.dial.(*fakeDialer); ok {
		d.setFakeError(err)
	}
}
