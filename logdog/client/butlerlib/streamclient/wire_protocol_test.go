...
func acceptOn(ctx context.Context, mkListen func() (net.Listener, error)) (<-chan net.Conn, net.Addr) {
	...
	go func() {
		...
		for {
			...
			if err != nil {
				fmt.Printf("stopping listener for %q\n", listener.Addr())
				break
			}
			go func() {
				select {
	...
	Convey(`in-process`, func() {
		...
		if forProcessTest {
			Convey(`subprocess`, func() {
				...
				So(stream.Close(), ShouldBeNil) // otherwise the connection won't get EOF
