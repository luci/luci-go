...
func TestFakeProtocol(t *testing.T) {
	...
	Convey(`"fake" protocol Client`, t, func() {
		...
		Convey(`bad`, func() {
			Convey(`extra parameter`, func() {
				_, err := New("fake:bad_extra", "namespace")
				So(err, ShouldErrLike,
					`opening path "fake:bad_extra": fake protocol does not accept an address`)
			})
