...
func treeHandler(c *router.Context) {
	...
	if err != nil {
		...
		http.Error(c.Writer, "Error: Blob must be Directory", http.StatusBadRequest)
		return
	}
...
func readBlob(c *router.Context) ([]byte, error) {
	...
	if err != nil {
		http.Error(c.Writer, "Error: Invalid blob size", http.StatusBadRequest)
		return nil, err
	}
	...
	if err != nil {
		...
		errMsg := fmt.Sprintf("Error: %s", code.String())
