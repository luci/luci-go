...
func TestHandlers(t *testing.T) {
	...
	Convey("InstallHandlers", t, func() {
		srv, _, _ := setupServerWithFakeCAS(t)
	...
	Convey("treeHandler", t, func() {
		srv, _, cl := setupServerWithFakeCAS(t)
	...
	Convey("getHandler", t, func() {
		srv, casSrv, cl := setupServerWithFakeCAS(t)

		b := []byte{1}
		bd := digest.NewFromBlob([]byte{1})
		resName := fmt.Sprintf("/%s/blobs/%s/%d", testInstance, bd.Hash, bd.Size)

		Convey("OK", func() {
			// Upload blob.
			_, err := cl.WriteBlob(context.Background(), b)
			So(err, ShouldBeNil)

			resp, err := http.Get(srv.URL + resName)
			So(err, ShouldBeNil)
			defer resp.Body.Close()
...
func setupServerWithFakeCAS(t *testing.T) (*httptest.Server, *fakes.Server, *client.Client) {
	r := router.New()
