...
func treeHandler(c *router.Context) {
	cl, bd, err := preprocessBlobRequest(c)
	if err != nil {
		return
	}
...
// preprocessBlobRequest returns Client and Digest for the requested blob.
func preprocessBlobRequest(c *router.Context) (*client.Client, *digest.Digest, error) {
