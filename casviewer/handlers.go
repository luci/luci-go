...
func InstallHandlers(r *router.Router, cc *ClientCache) {
	...
	r.GET("/projects/:proj/instances/:inst/blobs/:hash/:size/tree", blobMW, treeHandler)
	r.GET("/projects/:proj/instances/:inst/blobs/:hash/:size/", blobMW, getHandler)
}
...
func treeHandler(c *router.Context) {
	_, err := GetClient(c.Context, fullInstName(c.Params))
	if err != nil {
