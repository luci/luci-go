...
// Route install CAS Viewer handlers to the router.
func Route(r *router.Router) {
	mw := router.MiddlewareChain{}

	r.GET("/", mw, rootHanlder)
	r.GET("/:instance/blobs/:hash/:size/tree", mw, treeHandler)
	r.GET("/:instance/blobs/:hash/:size", mw, getHandler)
}
