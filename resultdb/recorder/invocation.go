...
// checkAndUpdateInvocation checks if the invocation can be mutated and also
// finalize the invocation if it's deadline is exceeded.
func checkAndUpdateInvocation(ctx context.Context, txn *spanner.ReadWriteTransaction, invID string) error {
	now := clock.Now(ctx)
	err := mayMutateInvocation(ctx, txn, invID, now)
	if grpcutil.Code(err) == codes.DeadlineExceeded {
		// Finalize the invocation since it's deadline is exceeded.
		muts := []*spanner.Mutation{
			spanner.UpdateMap("Invocation", map[string]interface{}{
				"InvocationId": invID,
...
func mayMutateInvocation(ctx context.Context, txn span.Txn, invID string, now time.Time) error {
	...
	case deadline.Before(now):
		return errors.Reason("%q has exceeded deadline", pbutil.InvocationName(invID)).Tag(grpcutil.DeadlineExceededTag).Err()
