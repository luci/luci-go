...
func mayMutateInvocation(ctx context.Context, txn *spanner.ReadWriteTransaction, invID string) error {
	...
	case len(userToken) > 1:
		return errors.Reason("expected exactly one %q metadata value, got %d", updateTokenMetadataKey, len(userToken)).Tag(grpcutil.InvalidArgumentTag).Err()
	}
