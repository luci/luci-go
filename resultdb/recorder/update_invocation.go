...
func (s *recorderServer) UpdateInvocation(ctx context.Context, in *pb.UpdateInvocationRequest) (*pb.Invocation, error) {
	...
	if err := validateUpdateInvocationRequest(in, now); err != nil {
	...
	// TODO(nodir, jchinlee): return the actual invocation
	return &pb.Invocation{}, mutateInvocationBlindly(ctx, invID, spanner.UpdateMap("Invocations", span.ToSpannerMap(values)))
}
