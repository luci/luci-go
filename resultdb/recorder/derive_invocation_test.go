...
func TestDeriveInvocation(t *testing.T) {
	Convey(`TestDeriveInvocation`, t, func() {
		...
		Convey(`calling to shouldSkipWriteInvocation works`, func() {
			Convey(`if we already have the invocation written`, func() {
				skip, err := shouldSkipWriteInvocation(ctx, span.Client(ctx).Single(), "inserted")
				So(err, ShouldBeNil)
				So(skip, ShouldBeTrue)
			})

			Convey(`if we don't yet have the invocation written`, func() {
				skip, err := shouldSkipWriteInvocation(ctx, span.Client(ctx).Single(), "another")
				So(err, ShouldBeNil)
		...
		Convey(`inserts a new invocation`, func() {
			...
			Convey(`with the correct test results`, func() {
				...
				var status, duration int64
				durations := []int64{}
				it := txn.Read(ctx, "TestResults", spanner.AllKeys(),
					[]string{"TestPath", "IsUnexpected", "Status", "RunDurationUsec"})
				defer it.Stop()
				for {
					row, err := it.Next()
					...
					err = row.Columns(&testPath, &isUnexpected, &status, &duration)
					So(err, ShouldBeNil)
					So(testPath, ShouldEqual, "c1/c2/t1.html")
					So(isUnexpected, ShouldResemble, spanner.NullBool{Valid: false})
					So(status, ShouldEqual, int64(pb.TestStatus_PASS))
					durations = append(durations, duration)
				}
				sort.Slice(durations, func(i, j int) bool { return durations[i] < durations[j] })
				So(durations, ShouldResemble, []int64{1e5, 2e5, 3e5})
			})
		})
	})
}
