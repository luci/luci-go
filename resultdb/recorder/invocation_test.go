	...
	Convey("MayMutateInvocation", t, func() {
		...
		Convey("with token", func() {
			...
			Convey(`with finalized invocation`, func() {
				testutil.MustApply(ctx, insertInvocation("inv", pb.Invocation_COMPLETED, token))
				err := mayMutate()
			...
			Convey(`with active invocation and different token`, func() {
				testutil.MustApply(ctx, insertInvocation("inv", pb.Invocation_ACTIVE, "different token"))
				err := mayMutate()
			...
			Convey(`with active invocation and same token`, func() {
				testutil.MustApply(ctx, insertInvocation("inv", pb.Invocation_ACTIVE, token))
				err := mayMutate()
				So(err, ShouldBeNil)
			})
		})
