...
func DeriveInvocation(ctx context.Context, req *pb.DeriveInvocationRequest) (*pb.DeriveInvocationResponse, error) {
	...
	if inv != nil {
		return &pb.DeriveInvocationResponse{Invocation: inv}, nil
	}
	...
	inv, results, err := convertOutputJSON(ctx, req, outputJSON)
	if err != nil {
		return nil, err
	}

	return &pb.DeriveInvocationResponse{Invocation: inv, TestResults: results}, nil
}
...
func convertOutputJSON(ctx context.Context, req *pb.DeriveInvocationRequest, data []byte) (*pb.Invocation, []*pb.TestResult, error) {
	...
	if err := resultdb.VariantDefMap(req.BaseTestVariant.Def).Validate(); err != nil {
		return nil, nil, errors.Annotate(err, "invalid base test variant $q").Err()
	}
