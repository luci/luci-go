...
func TestFetchingOutputJson(t *testing.T) {
	...
	Convey(`Getting output JSON file`, t, func() {
		...
		swarmingFake := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			var resp string
			switch {
			case path == fmt.Sprintf("/%stask/successful-task/result", swarmingAPIEndpoint):
				resp = fmt.Sprintf(
					`{"outputs_ref": {"isolatedserver": "%s", "namespace": "ns", "isolated": "%s"}}`,
					isoServer.URL, outputsDigest)
			case path == fmt.Sprintf("/%stask/no-output-file-task/result", swarmingAPIEndpoint):
				resp = fmt.Sprintf(
					`{"outputs_ref": {"isolatedserver": "%s", "namespace": "ns", "isolated": "%s"}}`,
	...
	Convey(`Getting Invocation ID`, t, func() {
		swarmingFake := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			var resp string
			switch {
			case path == fmt.Sprintf("/%stask/deduped-task/result", swarmingAPIEndpoint):
				resp = `{"deduped_from" : "first-task", "run_id": "123410"}`
			case path == fmt.Sprintf("/%stask/first-task/result", swarmingAPIEndpoint):
				resp = `{"run_id": "abcd12"}`
			default:
				resp = `{}`
			}
		...
		Convey(`for non-deduped task`, func() {
			...
			So(err, ShouldBeNil)
			So(id, ShouldEqual, "8d457b50d0d6b48439f2a32de3004fad081019d3009e43897165d68db73b30c5")
		})
