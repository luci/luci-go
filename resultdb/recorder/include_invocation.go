...
// validateIncludeInvocationRequest returns a non-nil error if req is determined
// to be invalid.
...
func (s *RecorderServer) IncludeInvocation(ctx context.Context, in *pb.IncludeInvocationRequest) (*empty.Empty, error) {
	if err := validateIncludeInvocationRequest(in); err != nil {
	...
	_, err := span.Client(ctx).ReadWriteTransaction(ctx, func(ctx context.Context, txn *spanner.ReadWriteTransaction) error {
		...
		// Ensure the included invocations exist and also read its state
		// to compute inclusion readiness.
		var ready bool
		eg.Go(func() error {
		...
		if overriddenInvID != "" {
			...
			// we had to verify that this inclusion is not already overridden
			// by something else.
