...
// It executes the callback with each row, starting with the most recent,
// as an argument.
func ByTimestamp(ctx context.Context, realm string, timeRange *pb.TimeRange, callback func(row *spanner.Row) error) error {
	var err error
	now := clock.Now(ctx)
	...
	return spanutil.Query(ctx, st, callback)
}
