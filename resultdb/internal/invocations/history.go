...
var queryTmpl = template.Must(template.New("").Parse(`
	...
	{{if .MatchTestIdPrefix}}
		AND (
			...
			REGEXP_CONTAINS(i.CommonTestIDPrefix, @literalPrefix) OR
			i.CommonTestIDPrefix = "" OR  -- invocations without a common test id prefix
			i.CommonTestIDPrefix = NULL -- invocations before CommonTestIDPrefix
		)
	{{end}}
