...
func (q *HistoryQuery) ByTimestamp(ctx context.Context, callback func(inv ID, ts *timestamp.Timestamp) error) error {
	...
	if re := q.Predicate.GetTestIdRegexp(); re != "" && re != ".*" {
		...
		var ok bool
		if literalPrefix, ok = r.LiteralPrefix(); !ok {
	...
	st.Params["testIdRegexp"] = literalPrefix
...
var queryTmpl = template.Must(template.New("").Parse(`
	...
	{{if .MatchTestIdPrefix}}
		AND (
			...
			i.CommonTestIDPrefix = @noCommonPrefix OR  -- invocations without a common test id prefix
			i.CommonTestIDPrefix = "" -- invocations before CommonTestIDPrefix
	{{end}}
