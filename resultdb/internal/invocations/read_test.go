...
func TestReadIncludedBatch(t *testing.T) {
	ftt.Run(`TestReadIncludedBatch`, t, func(t *ftt.Test) {
		ctx := testutil.SpannerTestContext(t)

		testutil.MustApply(ctx, t,
			insertInvocation("a", nil),
			insertInvocation("b", nil),
			insertInvocation("c", nil),
			insertInvocation("d", nil),
			insertInclusion("a", "b"),
			insertInclusion("a", "c"),
			insertInclusion("b", "d"),
		)
		...
		t.Run(`Existing invocations`, func(t *ftt.Test) {
			ids := []ID{"a", "b", "c"}
			results, err := ReadIncludedBatch(ctx, ids)
			assert.Loosely(t, err, should.BeNil)
			assert.That(t, results, should.Match([][]ID{
				{"b", "c"}, // sorted
				{"d"},
				{},
			}))
		...
		t.Run(`Mixed existing and non-existing invocations`, func(t *ftt.Test) {
			ids := []ID{"a", "non-existent", "b"}
			results, err := ReadIncludedBatch(ctx, ids)
			assert.Loosely(t, err, should.BeNil)
			assert.That(t, results, should.Match([][]ID{
				{"b", "c"},
		...
		t.Run(`Duplicate invocations in request`, func(t *ftt.Test) {
			ids := []ID{"a", "a", "b"}
			results, err := ReadIncludedBatch(ctx, ids)
			assert.Loosely(t, err, should.BeNil)
			assert.That(t, results, should.Match([][]ID{
