...
func TestByTimestamp(t *testing.T) {
	Convey(`ByTimestamp`, t, func() {
		...
		So(res[0].ID, ShouldEndWith, "second")
		So(res[1].ID, ShouldEndWith, "first")

		// Some results, excluding the oldest.
		res, err = ByTimestamp(ctx, &pb.TimeRange{Earliest: middlePB, Latest: afterPB}, realm)
		So(err, ShouldBeNil)
		So(res, ShouldHaveLength, 2)
		So(res[0].ID, ShouldEndWith, "third")
		So(res[1].ID, ShouldEndWith, "second")
		...
		So(res[0].ID, ShouldEndWith, "third")
		So(res[1].ID, ShouldEndWith, "second")
		So(res[2].ID, ShouldEndWith, "first")
