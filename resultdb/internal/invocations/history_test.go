...
func TestByTimestamp(t *testing.T) {
	Convey(`ByTimestamp`, t, func() {
		...
		res, err := ByTimestamp(ctx, &pb.TimeRange{Earliest: startPB, Latest: middlePB}, realm)
		So(err, ShouldBeNil)
		...
		// Before first result.
		res, err = ByTimestamp(ctx, &pb.TimeRange{Earliest: beforePB, Latest: justBeforePB}, realm)
		So(err, ShouldBeNil)
