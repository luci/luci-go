...
func TestQuery(t *testing.T) {
	Convey(`Query`, t, func() {
		...
		insInvArt := func(invID span.InvocationID, artifactID string, extraValues map[string]interface{}) *spanner.Mutation {
		...
		insTRArt := func(invID span.InvocationID, testID, resultID, artifactID string) *spanner.Mutation {
			values := map[string]interface{}{
				"InvocationId": invID,
		...
		Convey(`Test ID regexp`, func() {
			testutil.MustApply(ctx,
				insInvArt("inv1", "a", nil),
				insTRArt("inv1", "t00", "r", "a"),
				insTRArt("inv1", "t10", "r", "a"),
				insTRArt("inv1", "t11", "r", "a"),
				insTRArt("inv1", "t20", "r", "a"),
			)
		...
		Convey(`Follow edges`, func() {
			testutil.MustApply(ctx,
				...
				insTRArt("inv1", "t", "r", "a0"),
				insTRArt("inv1", "t", "r", "a1"),
			)
		...
		Convey(`Artifacts of interesting test results`, func() {
			testutil.MustApply(ctx,
				insInvArt("inv1", "a", nil),
				insTRArt("inv1", "t0", "0", "a"),
				insTRArt("inv1", "t1", "0", "a"),
				insTRArt("inv1", "t1", "1", "a"),
				insTRArt("inv1", "t1", "1", "b"),
				insTRArt("inv1", "t2", "0", "a"),
			)
