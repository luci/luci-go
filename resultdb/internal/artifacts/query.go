...
// parsePageToken parses the page token into invocation ID, parentID
// and artifactId.
func parsePageToken(pageToken string) (inv span.InvocationID, parentID string, artifactID string, err error) {
	switch pos, tokErr := pagination.ParseToken(pageToken); {
	case tokErr != nil:
		err = pagination.TokenError(tokErr)

	case pos == nil:

	case len(pos) != 3:
		err = pagination.TokenError(errors.Reason("expected 3 position strings, got %q", pos).Err())

	default:
		inv = span.InvocationID(pos[0])
		parentID = pos[1]
		artifactID = pos[2]
	}
...
func (q *Query) Fetch(ctx context.Context, txn span.Txn) (arts []*pb.Artifact, nextPageToken string, err error) {
	...
	st.Params["afterInvocationId"],
		...
		err = parsePageToken(q.PageToken)
	if err != nil {
		return
	}
