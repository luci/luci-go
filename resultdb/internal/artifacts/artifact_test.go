	...
	Convey(`TestExist`, t, func() {
		...
		Convey(`Exists`, func() {
			...
			Convey(`with a different hash`, func() {
				ex, err := Exist(ctx, invocations.ID("inv"), "tr/t/r", "a", "b321", 54)
				So(ex, ShouldErrLike, codes.AlreadyExists, `exists w/ a different hash or size`)
			})

			Convey(`with a different size`, func() {
				ex, err := Exist(ctx, invocations.ID("inv"), "tr/t/r", "a", "a123", 1024)
				So(ex, ShouldErrLike, codes.AlreadyExists, `exists w/ a different hash or size`)
			})
		})
