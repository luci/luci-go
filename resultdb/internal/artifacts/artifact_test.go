...
func TestExist(t *testing.T) {
	sampleHash := "sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
	...
	Convey(`TestExist`, t, func() {
		...
		Convey(`Exists`, func() {
			testutil.MustApply(ctx, insert.Artifact(invocations.ID("inv"), "tr/t/r", "a", map[string]interface{}{
				"ContentType": "text/plain",
				"RBECASHash":  sampleHash,
				"Size":        54,
			}))

			Convey(`with the same hash and size`, func() {
				ex, err := Exist(ctx, invocations.ID("inv"), "tr/t/r", "a", sampleHash, 54)
				So(err, ShouldBeNil)
				So(ex, ShouldBeTrue)
			})
			...
			Convey(`with a different hash`, func() {
				...
				So(err, ShouldHaveAppStatus, codes.AlreadyExists, `exists w/ a different hash or size`)
			})
