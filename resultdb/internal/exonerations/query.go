...
func (q *Query) Fetch(ctx context.Context, txn *spanner.ReadOnlyTransaction) (tes []*pb.TestExoneration, nextPageToken string, err error) {
	...
	st := spanner.NewStatement(`
		SELECT InvocationId, TestId, ExonerationId, Variant, ExplanationHtml, VariantHash
	...
	err = span.Query(ctx, txn, st, func(row *spanner.Row) error {
		...
		err := b.FromSpanner(row, &invID, &ex.TestId, &ex.ExonerationId, &ex.Variant, &explanationHTML, &ex.VariantHash)
		if err != nil {
			return err
		}
