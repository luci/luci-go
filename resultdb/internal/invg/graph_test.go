...
func TestFetchGraph(t *testing.T) {
	Convey(`TestInclude`, t, func() {
		...
		fetch := func(stableOnly bool, roots ...span.InvocationID) (*Graph, error) {
			txn := span.Client(ctx).ReadOnlyTransaction()
			defer txn.Close()
			return FetchGraph(ctx, txn, stableOnly, roots...)
		}

		mustFetch := func(stableOnly bool, roots ...span.InvocationID) *Graph {
			g, err := fetch(stableOnly, roots...)
			So(err, ShouldBeNil)
			return g
		}
