...
// RBEConn creates a gRPC connection to RBE.
...
// RegisterRBEInstanceNameFlag registers -artifact-rbe-instance flag.
func RegisterRBEInstanceNameFlag(fs *flag.FlagSet, target *string) {
	fs.StringVar(
		target,
...
func (r *contentRequest) handleRBECASContent(c *router.Context, hash string) {
	...
	stream, err := r.RBECASDataClient.Read(c.Context, &bytestream.ReadRequest{
	...
	if err != nil {
		if status.Code(err) == codes.NotFound {
			...
			logging.Warningf(c.Context, "RBE-CAS responded with %s", err)
