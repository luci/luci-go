...
func TestGenerateSignedURL(t *testing.T) {
	Convey(`TestGenerateSignedURL`, t, func(c C) {
		...
		Convey(`Basic case`, func() {
			...
			So(err, ShouldBeNil)
			So(u, ShouldStartWith, "https://results.usercontent.example.com/invocations/inv/artifacts/a?token=")
		})

		Convey(`Escaped test id`, func() {
			u, _, err := s.GenerateSignedURL(ctx, "invocations/inv/tests/t%2Ft/results/r/artifacts/a")
			So(err, ShouldBeNil)
			So(u, ShouldStartWith, "https://results.usercontent.example.com/invocations/inv/tests/t%2Ft/results/r/artifacts/a?token=")
		})
	})
}
...
func TestServeContent(t *testing.T) {
	Convey(`TestServeContent`, t, func(c C) {
		...
		Convey(`Escaped test id`, func() {
			u, exp, err := s.GenerateSignedURL(ctx, "invocations/inv/tests/t%2Ft/results/r/artifacts/a")
			So(err, ShouldBeNil)
