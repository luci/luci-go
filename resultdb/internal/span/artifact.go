...
// tmpQueryArtifact is a template for the SQL expression that queries artifacts.
// See also ArtifactQuery.
var tmpQueryArtifact = template.Must(template.New("artifactQuery").Parse(`
...
WHERE art.InvocationId IN UNNEST(@invIDs)
	...
	AND (
		...
		(ParentId = @afterParentId AND art.InvocationId > @afterInvocationId) OR
		(ParentId = @afterParentId AND art.InvocationId = @afterInvocationId AND ArtifactId > @afterArtifactId)
	)
...
func (q *ArtifactQuery) Fetch(ctx context.Context, txn Txn) (arts []*pb.Artifact, nextPageToken string, err error) {
	...
	sql := &bytes.Buffer{}
	if err = tmpQueryArtifact.Execute(sql, input); err != nil {
		return
	}
	fmt.Println(sql)
