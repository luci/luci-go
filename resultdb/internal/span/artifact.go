...
// MustParseArtifactName extracts invocation, test id and exoneration
// IDs from the name.
// Panics on failure.
...
// ArtifactParentID returns value of Artifacts.ParentId Spanner column.
// Assumes artifactName is valid.
func ArtifactParentID(artifactName string) string {
	_, testID, resultID, _ := MustParseArtifactName(artifactName)
	if testID != "" {
...
// code.
// Does not read the location of the artifact.
func ReadArtifact(ctx context.Context, txn Txn, name string) (*pb.Artifact, error) {
	invIDStr, _, _, artifactID, err := pbutil.ParseArtifactName(name)
	if err != nil {
		return nil, err
	}
	invID := InvocationID(invIDStr)
	...
	err = ReadRow(ctx, txn, "Artifacts", invID.Key(ArtifactParentID(name), artifactID), map[string]interface{}{
