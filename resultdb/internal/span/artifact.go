...
type ArtifactQuery struct {
	...
	TestResultPredicate *pb.TestResultPredicate // Predicate.Invocation must be nil.
...
func QueryArtifacts(ctx context.Context, txn Txn, q ArtifactQuery) (arts []*pb.Artifact, nextPageToken string, err error) {
	...
	err = Query(ctx, txn, st, func(row *spanner.Row) error {
		...
		testID, resultID, err := ParseArtifactParentID(parentID)
		if err != nil {
			return err
		}
		if testID == "" {
	...
	if len(arts) == q.PageSize {
		...
		invID, testID, resultID, artifatID := MustParseArtifactName(last.Name)
		nextPageToken = pagination.Token(ArtifactParentID(testID, resultID), string(invID), artifatID)
	}
	return
}
