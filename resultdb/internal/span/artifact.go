...
func (q *ArtifactQuery) parentIDRegexp() string {
	testIDRegexp := q.TestResultPredicate.GetTestIdRegexp()
	hasTestIDRegexp := testIDRegexp != "" && testIDRegexp != ".*"
	...
	if edges.IncludedInvocations && edges.TestResults && !hasTestIDRegexp {
		// Fast path.
		return ".*"
	}

	// Collect alternatives and then combine them with "|".
	var alts []string // list of regexp patterns for ParentId column.
	...
	if edges.TestResults {
		...
		// "tr/{testID}/{resultID}"
		if hasTestIDRegexp {
			alts = append(alts, fmt.Sprintf("tr/%s/[^/]+", testIDRegexp))
		} else {
			alts = append(alts, "tr/.+")
		}
	}
