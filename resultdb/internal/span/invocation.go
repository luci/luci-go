...
// InvocationID can convert an invocation id to its different formats:
// invocation name and hash-prefixed ID used in Spanner.
type InvocationID string

// MustParseInvocationName converts an invocation name to an InvocationID.
// The name must be already valid.
...
// InvocationNameFromRowID converts a invocation row ID to a name.
func InvocationNameFromRowID(rowID string) string {
	return pbutil.InvocationName(stripHashPrefix(rowID))
}
