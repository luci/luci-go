...
func ReadInvocationsByTag(ctx context.Context, txn Txn, tag *typepb.StringPair, limit int) (map[InvocationID]*pb.Invocation, error) {
	...
	err := txn.Query(ctx, st).Do(func(row *spanner.Row) error {
		...
		var id InvocationID
		inv := &pb.Invocation{}
		if err := FromSpanner(row, &id, &inv.State, &inv.CreateTime, &inv.FinalizeTime, &inv.Deadline, &inv.BaseTestVariant, &inv.Tags); err != nil {
			return err
		}
		ret[id] = inv
		return nil
...
func QueryInvocations(ctx context.Context, txn *spanner.ReadOnlyTransaction, pred *pb.InvocationPredicate, limit int) (map[InvocationID]*pb.Invocation, error) {
	if limit <= 0 {
		return nil, errors.Reason("limit is not positive").Err()
	}

	// Resolve IDs of the root invocations.
	var ret map[InvocationID]*pb.Invocation
	if name := pred.GetName(); name != "" {
		...
		var err error
		if ret, err = ReadInvocationsByTag(ctx, txn, pred.GetTag(), limit); err != nil {
			return nil, err
		}
	}
	...
	return ReadReachableInvocations(ctx, txn, limit, ret)
}
