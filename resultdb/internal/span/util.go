...
// replacePointers returns a new slice replacing the pointers to Spanner-unsupported types with
// pointers to ones Spanner does support.
//
// Note that in the case of supported types, ptrs[i] and subPtrs[i] are both pointers.
// In the case of unsupported types, ptrs[i] is typically a pointer to a variable, which itself is a
// pointer, whereas subPtrs[i] is just the pointer to the value.
...
func replaceValues(ptrs []interface{}, subPtrs []interface{}) {
	...
	for i, subPtr := range subPtrs {
		if ptrs[i] == subPtr {
			continue
		}

		switch pOrig := ptrs[i].(type) {
		case **tspb.Timestamp:
			pSub, ok := subPtr.(*time.Time)
			if !ok {
				panic(fmt.Sprintf(
