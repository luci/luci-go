...
// ValueBuffer can convert a value from a Spanner type to a Go type.
// Supported types:
//   - string
//   - tspb.Timestamp
//   - pb.InvocationState
...
// written.
func (b *ValueBuffer) ReadRow(ctx context.Context, txn Txn, table string, key spanner.Key, ptrMap map[string]interface{}) error {
	...
	return b.FromSpanner(row, ptrs...)
}

// FromSpanner reads values from row to ptrs, converting types from Spanner
// to Go along the way.
func (b *ValueBuffer) FromSpanner(row *spanner.Row, ptrs ...interface{}) error {
...
func (b *ValueBuffer) fromSpanner(row *spanner.Row, index int, goPtr interface{}) error {
	...
	if err := row.Column(index, spanPtr); err != nil {
		return err
	}
	...
	case *[]*pb.Artifact:
		// Reuse TestResult as a container of artifacts.
		container := &Artifacts{}
