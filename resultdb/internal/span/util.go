...
// ColumnReader can read columns values and convert them to Go types.
// For example, it can convert Spanner's int64 to pb.InvocationState.
type ColumnReader struct {
	// pointers to Go values, such as **tspb.Timestamp
	goPtrs []interface{}
	// pointers to Spanner values, such as *time.Time
	spanPtrs []interface{}
}
...
// Read reads columns from row into ptrs specifies in NewColumnReader.
