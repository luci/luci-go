...
func applyTestPagination(query squirrel.SelectBuilder, queryParams map[string]interface{}, pageToken string, resultColumn string) (squirrel.SelectBuilder, error) {
	...
	default:
		query = query.Where(fmt.Sprintf(`(
				(InvocationId > @cursorInvocationID) OR
				(InvocationId = @cursorInvocationID AND TestPath > @cursorTestPath) OR
				(InvocationId = @cursorInvocationID AND TestPath = @cursorTestPath AND %s > @cursorResultID)
			)`,
			resultColumn,
		))
