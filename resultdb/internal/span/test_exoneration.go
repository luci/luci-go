...
// ExoneratedTestVariants set in an error indicates that too many invocations
// matched a condition.
var TooManyExoneratedTestVariantsTag = errors.BoolTag{
	Key: errors.NewTagKey("too many test exonerations matched the condition"),
}
...
func QueryExoneratedTestVariants(ctx context.Context, txn *spanner.ReadOnlyTransaction, q TestExonerationQuery) (map[string]bool, error) {
	...
	if len(tvs) == q.PageSize {
		return nil, errors.Reason("more than %d exonerated test variants", q.PageSize).Tag(TooManyExoneratedTestVariantsTag).Err()
	}
	return tvs, nil
}
