...
func QueryTestResults(ctx context.Context, txn *spanner.ReadOnlyTransaction, q TestResultQuery) (trs []*pb.TestResult, nextPageToken string, err error) {
	...
	if q.Predicate.GetExpectancy() == pb.TestResultPredicate_VARIANTS_WITH_UNEXPECTED_RESULTS {
		// We must return only test results of test variants that has unexpected results.
		//
		...
		// table.
		from = `
			UnexpectedResults ur
			JOIN TestResults tr ON ur.TestPath = tr.TestPath AND ur.VariantHash = tr.VariantHash
		`
	}
	...
	st := spanner.NewStatement(fmt.Sprintf(`
		...
		# query.
		WITH UnexpectedResults AS (
			SELECT DISTINCT AS STRUCT TestPath, VariantHash
