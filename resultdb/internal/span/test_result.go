...
func ReadTestResults(ctx context.Context, txn Txn, name string, includeExpected bool, cursorTok string, pageSize int) (trs []*pb.TestResult, nextCursorTok string, err error) {
	invID := pbutil.MustParseInvocationName(name)

	var (
		table       = "TestResults"
		conditions  = "(InvocationId = @invID)"
	...
	if !includeExpected {
		...
		conditions += " AND IsUnexpected IS NOT NULL"
	}
