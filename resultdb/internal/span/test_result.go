...
func queryTestResults(ctx context.Context, txn *spanner.ReadOnlyTransaction, q TestResultQuery, selectVariantHash bool, f func(tr *pb.TestResult, variantHash string) error) error {
	if q.PageSize < 0 {
		panic("PageSize < 0")
	}

	extraSelect := ""
	if selectVariantHash {
...
func QueryTestResults(ctx context.Context, txn *spanner.ReadOnlyTransaction, q TestResultQuery) (trs []*pb.TestResult, nextPageToken string, err error) {
	...
	err = queryTestResults(ctx, txn, q, false, func(tr *pb.TestResult, variantHash string) error {
		trs = append(trs, tr)
		return nil
	})
	if err != nil {
...
func QueryTestResultsStreaming(ctx context.Context, txn *spanner.ReadOnlyTransaction, q TestResultQuery, f func(tr *pb.TestResult, variantHash string) error) error {
	...
	return queryTestResults(ctx, txn, q, true, f)
}
