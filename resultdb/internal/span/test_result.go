...
func QueryTestResults(ctx context.Context, txn *spanner.ReadOnlyTransaction, q TestResultQuery) (trs []*pb.TestResult, nextPageToken string, err error) {
	...
	st := spanner.NewStatement(fmt.Sprintf(`
		...
		WHERE InvocationId IN UNNEST(@invIDs)
			...
			#    i.e. operation graph leaves do not read test results that do not
			#    match this predicate. In particular, filter (prefix=false, value="A")
			#    does not read all test results that start with A, but only
			#    TestPath="A".
...
func testPathPredToParam(pred *pb.TestPathPredicate) []testPathPredicate {
	...
	ret := make([]testPathPredicate, len(pred.PathPrefixes)+len(pred.Paths))
