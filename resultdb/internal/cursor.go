...
func Cursor(token string) (*internalpb.Cursor, error) {
	...
	if err := buf.DecodeMessage(cursor); err != nil {
		return nil, errors.Annotate(err, "cursor contents %q", buf).Err()
	}
	return cursor, nil
}
...
// ValidateForRequest indicates whether this cursor may be used with the request
// represented by the given bytes.
func ValidateForRequest(c *internalpb.Cursor, req []byte) bool {
	return bytes.Compare(c.Request, req) == 0
}
