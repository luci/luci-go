...
// statusFromError returns a status to return to the user based on the error.
func statusFromError(err error) *status.Status {
	if s, ok := appstatus.FromError(err); ok {
		return s
	}

	switch unwrapped := errors.Unwrap(err); {
	case unwrapped == context.DeadlineExceeded:
		return status.FromContextError(err)

	case unwrapped == context.Canceled:
		return status.FromContextError(err)
...
func verifyAccess(ctx context.Context) error {
	...
	case !allowed:
		return appstatus.Errf(codes.PermissionDenied, "%s is not in %q CIA group", auth.CurrentIdentity(ctx), accessGroup)
