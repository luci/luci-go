...
func TestIsolate(t *testing.T) {
	...
	Convey(`TestIsolate`, t, func(c C) {
		...
		s, err := NewServer(ctx, "resultdb-usercontent.example.com")
		...
		Convey(`URL sanity check`, func() {
			...
			urlStr, err := s.GenerateSignedIsolateURL(ctx, "isolate.example.com", "default-gzip", "deadbeef")
			So(err, ShouldBeNil)
		...
		Convey(`E2E`, func() {
			...
			status, actualContents := fetch(urlStr)
