...
func BenchmarkGetTestResultsHistory(b *testing.B) {
	...
	for _, p.nBuilds = range []int{100, 1000} {
		...
		runQueryBenchmark(ctx, b, fmt.Sprintf("SpecificTest-%d-invocations-%d-results", totalInvocations, totalResults), q)
		...
		runQueryBenchmark(ctx, b, fmt.Sprintf("SpecificTestVariant-%d-invocations-%d-results", totalInvocations, totalResults), q)
	}
}
