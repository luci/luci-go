...
func insertBenchmarkData(ctx context.Context, p benchmarkDataParams) {
		...
		b.Run(name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				...
				if len(r) != expectedResults {
					panic(fmt.Errorf("Expected %d results but got %d", expectedResults, len(r)))
				}
				cancel()
			}
		})
