...
var useRedis = os.Getenv(testutil.RedisTestEnvVar) == "1"

type benchmarkDataParams struct {
	nOSes, nVariants, nBuilds, nSteps, nShards, nTests, nResults int
}

func insertBenchmarkData(ctx context.Context, p benchmarkDataParams, buildOffset int) {
	...
	for os := 0; os < p.nOSes; os++ {
		for v := 0; v < p.nVariants; v++ {
			...
			for build := buildOffset; build < buildOffset+p.nBuilds; build++ {
...
func BenchmarkGetTestResultsHistory(b *testing.B) {
	runQueryBenchmark := func(ctx context.Context, b *testing.B, name string, q *Query) {
		b.Run(name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				...
				if _, _, err := q.Execute(iCtx); err != nil {
					panic(err)
				}
				cancel()
			}
	...
	p := benchmarkDataParams{4, 4, 4, 4, 4, 4, 4}
	ctx := testutil.SpannerTestContext(b)
	lastBuild := 0

	// 16 builders, each with (100, 1000) builds, where each build-level
	// invocation contains 20 sub-invocations and 256 results.
	for _, p.nBuilds = range []int{100, 1000} {

		insertBenchmarkData(ctx, p, lastBuild)
		lastBuild += p.nBuilds
		totalInvocations := p.nOSes * p.nVariants * p.nBuilds * p.nSteps * p.nShards
		totalResults := totalInvocations * p.nTests * p.nResults
		suffix := "-no-redis"
		if useRedis {
			suffix = "-local-redis"
		}

		q.Request.TestIdRegexp = "step0-shard0-step0"
		runQueryBenchmark(ctx, b, fmt.Sprintf("SpecificTest-%d-invocations-%d-results%s", totalInvocations, totalResults, suffix), q)
		...
		runQueryBenchmark(ctx, b, fmt.Sprintf("SpecificTestVariant-%d-invocations-%d-results%s", totalInvocations, totalResults, suffix), q)
	}
}
