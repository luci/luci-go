...
func (q *Query) Execute(ctx context.Context) ([]*pb.GetTestResultHistoryResponse_Entry, string, error) {
	...
	eg.Go(func() error {
		...
		defer close(q.workC)
		err := q.dispatchWorkItems(ctx)
		return errors.Annotate(err, "history.Query.dispatchWorkItems").Err()
	})

	eg.Go(func() error {
		err := q.fetchAll(ctx)
		return errors.Annotate(err, "history.Query.fetchAll").Err()
	})

	eg.Go(func() error {
		err := q.collect(ctx)
		return errors.Annotate(err, "history.Query.collect").Err()
	})
