...
var queryTmpl = template.Must(template.New("").Parse(`
	{{define "testResults"}}
		...
		{{if .filterByTestIdOrVariant}}
			invs AS (
				...
				{{if .params.literalPrefix}}
					...
					OR (i.CommonTestIDPrefix IS NULL AND i.CreateTime < TIMESTAMP(DATE "2021-4-13"))
				{{end}}
				{{if .params.variant}}
					AND (SELECT LOGICAL_AND(kv IN UNNEST(i.TestResultVariantUnion)) FROM UNNEST(@variant) kv)
					-- To make sure the query can still work for the old invocations
					-- that were created without TestResultVariantUnion.
					OR (i.TestResultVariantUnion IS NULL AND i.CreateTime < TIMESTAMP(DATE "2021-4-13"))
				{{end}}
			),
		{{else}}
			invs AS (
		...
		{{else}}
			...
			FROM invs
			JOIN  TestResults tr USING(InvocationId)
			WHERE 1=1 -- placeholder of the fist where clause, so that testIDAndVariantFilter can always start with "AND".
				{{template "testIDAndVariantFilter" .}}
		{{end}}
