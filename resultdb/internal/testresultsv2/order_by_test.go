...
func TestParseOrderBy(t *testing.T) {
	ftt.Run("ParseOrderBy", t, func(t *ftt.Test) {
		t.Run("Default", func(t *ftt.Test) {
			...
			assert.Loosely(t, o, should.Equal(OrderingByPrimaryKey))
		})

		t.Run("test_id_structured", func(t *ftt.Test) {
			o, err := ParseOrderBy("test_id_structured")
			assert.Loosely(t, err, should.BeNil)
			assert.Loosely(t, o, should.Equal(OrderingByTestID))
		})
		...
		t.Run("Invalid", func(t *ftt.Test) {
			...
			t.Run("unknown field", func(t *ftt.Test) {
				...
				assert.Loosely(t, err, should.ErrLike(`unsupported order by clause: "unknown"; supported orders are "test_id_structured" (test ID order) or "" (system-preferred order)`))
			})
		})
	})
}
