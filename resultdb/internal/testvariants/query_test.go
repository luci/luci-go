		...
		Convey(`Query`, func() {
			...
			Convey(`unexpected`, func() {
				...
				Convey(`Unexpected works`, func() {
					...
					for i, tv := range tvs {
						switch {
						case strings.HasPrefix(tvStrings[i], "1"):
							So(tv.Status, ShouldEqual, uipb.TestVariantStatus_UNEXPECTED)
						case strings.HasPrefix(tvStrings[i], "2"):
							So(tv.Status, ShouldEqual, uipb.TestVariantStatus_FLAKY)
						case strings.HasPrefix(tvStrings[i], "3"):
							So(tv.Status, ShouldEqual, uipb.TestVariantStatus_EXONERATED)
						default:
							panic("impossible")
						}
					}
