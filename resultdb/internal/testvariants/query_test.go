		...
		Convey(`unexpected`, func() {
			...
			Convey(`paging works`, func() {
				page := func(token string, expectedTVLen int32, expectedTVStrings []string) string {
					...
					So(len(tvs), ShouldEqual, expectedTVLen)
				...
				nextToken = page(nextToken, 1, []string{"16/T3/e3b0c44298fc1c14"})
				So(nextToken, ShouldEqual, pagination.Token("EXPECTED", "T5", "c467ccce5a16dc72"))
