...
func (q *Query) Fetch(ctx context.Context) (tvs []*uipb.TestVariant, nextPageToken string, err error) {
	...
	q.params = map[string]interface{}{
		...
		"unexpectedly_skipped": int(uipb.TestVariantStatus_UNEXPECTEDLY_SKIPPED),
	...
	testVariantsWithUnexpectedResults AS (
		SELECT
			...
			CASE
				...
				WHEN num_skipped = num_unexpected AND num_skipped = num_total THEN @unexpectedly_skipped -- "UNEXPECTEDLY_SKIPPED"
