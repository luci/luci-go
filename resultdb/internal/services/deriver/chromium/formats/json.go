...
func (r *JSONTestResults) ToProtos(ctx context.Context, testIDPrefix string, inv *pb.Invocation, availableArtifacts stringset.Set) ([]*TestResult, error) {
	...
	trs := make([]*TestResult, 0, len(r.Tests))
	buf := &strings.Builder{}
	for _, name := range testNames {
		testID := testIDPrefix + name

		// Populate protos.
		err := r.Tests[name].toProtos(ctx, &trs, testID, availableArtifacts, buf)
		if err != nil {
...
func (f *TestFields) toProtos(ctx context.Context, trDest *[]*TestResult, testID string, availableArtifacts stringset.Set, buf *strings.Builder) error {
