...
func (r *JSONTestResults) ToProtos(ctx context.Context, testIDPrefix string, inv *pb.Invocation, availableArtifacts stringset.Set) (
	testResults []*pb.TestResult, artifactToTestResult map[string]*pb.TestResult, err error) {

	artifactToTestResult = map[string]*pb.TestResult{}

	if r.Version != 3 {
		return nil, nil, errors.Reason("unknown JSON Test Results version %d", r.Version).Err()
	}
	...
	testResults = make([]*pb.TestResult, 0, len(r.Tests))
	buf := &strings.Builder{}
	for _, name := range testNames {
		testID := testIDPrefix + name

		// Populate protos.
		err := r.Tests[name].toProtos(ctx, &testResults, artifactToTestResult, testID, availableArtifacts, buf)
		if err != nil {
			return nil, nil, errors.Annotate(err, "test %q failed to convert run fields", name).Err()
		}
	}
...
func (f *TestFields) toProtos(ctx context.Context, trDest *[]*pb.TestResult, a2trDest map[string]*pb.TestResult, testID string, availableArtifacts stringset.Set, buf *strings.Builder) error {
...
// findArtifact looks for the given isolated artifact key by path.
