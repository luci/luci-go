...
func DeriveTestResults(ctx context.Context, task *swarmingAPI.SwarmingRpcsTaskResult, req *pb.DeriveChromiumInvocationRequest, inv *pb.Invocation) ([]*pb.TestResult, []*pb.Artifact, error) {
	// Parse swarming tags.
	...
	if ref != nil && ref.Isolated != "" {
		if results, arts, err = processOutputs(ctx, ref, testIDPrefix, inv, req); err != nil {
	...
	// Generate a name and apply the base variant.
	invID := span.MustParseInvocationName(inv.Name)
	for _, r := range results {
		r.Name = pbutil.TestResultName(string(invID), r.TestId, r.ResultId)
...
func processOutputs(ctx context.Context, outputsRef *swarmingAPI.SwarmingRpcsFilesRef, testIDPrefix string, inv *pb.Invocation, req *pb.DeriveChromiumInvocationRequest) (
	[]*pb.TestResult, []*pb.Artifact, error) {
	...
	if err != nil {
		return nil, nil, errors.Annotate(err, "getting isolated outputs").Err()
	}
	...
	if err != nil {
		return nil, nil, errors.Annotate(err, "getting output JSON file").Err()
	}

	// Convert the output JSON.
	results, artifactToTestResult, err := ConvertOutputJSON(ctx, inv, testIDPrefix, outputJSON, availableArtifacts)
	if err != nil {
		return nil, nil, attachInvalidTaskf(err, "invalid output: %s", err)
	}

	// Convert the isolated.File outputs to pb.Artifacts.
	invID := span.MustParseInvocationName(inv.Name)
	arts := make([]*pb.Artifact, 0, len(outputs))
	for key, f := range outputs {
		a := isolatedFileToArtifact(
			outputsRef.Isolatedserver,
			outputsRef.Namespace,
			key,
			f,
			string(invID),
			artifactToTestResult[key],
		)
		if a != nil {
			arts = append(arts, a)
		}
...
func isolatedFileToArtifact(isolateServer, ns, relPath string, f isolated.File, invID string, testResult *pb.TestResult) *pb.Artifact {
	...
	if testResult != nil {
		a.Name = pbutil.TestResultArtifactName(invID, testResult.TestId, testResult.ResultId, a.ArtifactId)
	} else {
		a.Name = pbutil.InvocationArtifactName(invID, a.ArtifactId)
	}
...
func ConvertOutputJSON(ctx context.Context, inv *pb.Invocation, testIDPrefix string, data []byte, availableArtifacts stringset.Set) (
	testResults []*pb.TestResult, artifactToTestResult map[string]*pb.TestResult, err error) {
	...
	if jsonErr == nil {
		testResults, artifactToTestResult, err = jsonFormat.ToProtos(ctx, testIDPrefix, inv, availableArtifacts)
		if err != nil {
			err = errors.Annotate(err, "converting as JSON Test Results Format").Err()
		}
		return
	}
	...
	if gtestErr == nil {
		testResults, err = gtestFormat.ToProtos(ctx, testIDPrefix, inv)
		if err != nil {
			err = errors.Annotate(err, "converting as GTest results format").Err()
		}
		return
	}

	// Conversion with either format failed, but we don't support other formats.
	return nil, nil, errors.NewMultiError(gtestErr, jsonErr)
}
