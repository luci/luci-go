...
func DeriveTestResults(ctx context.Context, task *swarmingAPI.SwarmingRpcsTaskResult, req *pb.DeriveChromiumInvocationRequest, inv *pb.Invocation) ([]*TestResult, error) {
	// Parse swarming tags.
	ret := parseSwarmingTags(task)

	//inv.Tags = append(inv.Tags, pbutil.StringPairs(ret.tags...)...)
	inv.Tags = append(inv.Tags, ret.tags...)
	...
	if ref != nil && ref.Isolated != "" {
		if results, err = processOutputs(ctx, ref, ret.testIDPrefix, inv, req); err != nil {
	...
	if err != nil {
		...
		result, err := convertTaskToResult(ret.testIDPrefix, task, req)
		if err != nil {
	...
	for _, r := range results {
		if len(r.Variant.GetDef()) == 0 {
			r.Variant = ret.baseVariant
			continue
		}

		// Otherwise combine.
		for k, v := range ret.baseVariant.Def {
