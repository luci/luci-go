...
func processOutputs(ctx context.Context, outputsRef *swarmingAPI.SwarmingRpcsFilesRef, testIDPrefix string, inv *pb.Invocation, req *pb.DeriveChromiumInvocationRequest) ([]*TestResult, error) {
	...
	for i, r := range results {
		...
		for name, key := range r.Artifacts {
			sa := isolatedFileToArtifact(outputsRef.Isolatedserver, outputsRef.Namespace, outputs[key], name)
			if a != nil {
...
func isolatedFileToArtifact(isolateServer, ns string, f isolated.File, artifactID string) *pb.Artifact {
