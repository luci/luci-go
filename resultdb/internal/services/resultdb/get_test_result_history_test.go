...
func TestValidateGetTestResultHistoryRequest(t *testing.T) {
	...
	Convey(`ValidateGetTestResultHistoryRequest`, t, func() {
		...
		ctx := clock.Set(context.Background(), testclock.New(testclock.TestRecentTimeUTC.Add(time.Hour)))
		...
		Convey(`valid`, func() {
			...
			So(validateGetTestResultHistoryRequest(ctx, req), ShouldBeNil)
		})
		...
		Convey(`missing realm`, func() {
			...
			So(validateGetTestResultHistoryRequest(ctx, req), ShouldErrLike, "realm is required")
		})
		...
		Convey(`bad pageSize`, func() {
			...
			So(validateGetTestResultHistoryRequest(ctx, req), ShouldErrLike, "page_size, if specified, must be a positive integer")
		})
		...
		Convey(`bad variant`, func() {
			...
			So(validateGetTestResultHistoryRequest(ctx, req), ShouldErrLike, "variant_predicate")
		})
	})
}
...
func TestGetTestResultHistory(t *testing.T) {
	Convey(`TestGetTestResultHistoryRequest`, t, func() {
		ctx := auth.WithState(
			clock.Set(testutil.SpannerTestContext(t), testclock.New(testclock.TestRecentTimeUTC.Add(time.Hour))),
			&authtest.FakeState{
		...
		Convey(`all results`, func() {
			...
			for i := 0; i < 9; i++ {
				So(res.Entries[i].InvocationTimestamp.AsTime().Before(latest.AsTime()), ShouldBeTrue)
			}
			for i := 9; i < 18; i++ {
