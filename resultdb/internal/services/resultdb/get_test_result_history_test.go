...
func TestGetTestResultHistory(t *testing.T) {
	Convey(`TestGetTestResultHistoryRequest`, t, func() {
		...
		muts := make([]*spanner.Mutation, 0, 48)
		muts = insertResultHistoryData(muts, "some-invocation", time.Duration(0), 3, 3)
		muts = insertResultHistoryData(muts, "some-other-invocation", 2*time.Minute, 3, 3)
		muts = insertResultHistoryData(muts, "yet-another-invocation", 2*time.Minute, 3, 3)
...
func insertResultHistoryData(muts []*spanner.Mutation, id string, offset time.Duration, subInvs, results int) []*spanner.Mutation {
	// Insert indexed invocation with subinvocations and results.
	muts = insertOneResultsInv(muts, id, testclock.TestRecentTimeUTC.Add(offset), true, 0)
	for i := 0; i < subInvs; i++ {
		// Insert result-containing invocations contained by the above.
		idSub := fmt.Sprintf("%s-%d", id, i+1)
		muts = insertOneResultsInv(muts, idSub, testclock.TestRecentTimeUTC, false, results)
		muts = append(muts, insert.Inclusion(invocations.ID(id), invocations.ID(idSub)))
	}
	return muts
}
...
func insertOneResultsInv(muts []*spanner.Mutation, id string, ts time.Time, indexed bool, results int) []*spanner.Mutation {
	...
	muts = append(muts, insert.Invocation(invID, pb.Invocation_ACTIVE, extraArgs))
	for i := 0; i < results; i++ {
		muts = append(muts,
