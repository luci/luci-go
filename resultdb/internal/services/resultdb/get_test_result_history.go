...
func validateGetTestResultHistoryRequest(ctx context.Context, in *pb.GetTestResultHistoryRequest) error {
	...
	if err := validateHistoryTimeRange(ctx, tr); err != nil {
		return appstatus.BadRequest(errors.Annotate(err, "time_range").Err())
	}
	if in.GetPageSize() < 0 {
...
func validateHistoryTimeRange(ctx context.Context, tr *pb.TimeRange) error {
	now := clock.Now(ctx)
	cutoff := now.AddDate(-2, 0, 0)

	validTimestamp := func(ts *timestamp.Timestamp) bool {
		if t, err := ptypes.Timestamp(ts); err != nil || t.After(now) || t.Before(cutoff) {
			return false
		}
		return true
	}
	...
	l := tr.GetLatest()
	if l != nil && !validTimestamp(l) {
		return errors.Reason("latest, if specified, must be within the last two years").Err()
	}

	e := tr.GetEarliest()
	if e != nil && !validTimestamp(e) {
		return errors.Reason("earliest, if specified, must be within the last two years").Err()
	}
...
//
// TODO(crbug.com/1074407): Accept multiple invocations that have the same
// ordinal, and sort their contained results together.
