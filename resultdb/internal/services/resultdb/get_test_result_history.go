...
func (s *resultDBServer) GetTestResultHistory(ctx context.Context, in *pb.GetTestResultHistoryRequest) (*pb.GetTestResultHistoryResponse, error) {
	...
	resultsOffset := initHistoryPaging(in)

	ctx, cancelTx := span.ReadOnlyTransaction(ctx)
	defer cancelTx()

	outOfTime := expirationFlag(ctx)
	workerCtx, cancelWorker := context.WithCancel(ctx)
	defer cancelWorker()

	workerErr := make(chan error, 1)
	results := make(chan historyPageItem)
	go func() {
		defer close(results)
		pool := newHistoryStreamerPool(historyWorkers, in, results, workerErr)
		defer pool.wait(ctx)
	...
	for item := range results {
		...
		if item.pageBreak != nil {
			...
			if len(ret.Entries) >= int(in.PageSize) {
				cancelWorker()
				ret.NextPageToken = makeHistoryPageToken(item.pageBreak, int(in.PageSize)-firstUnsortedEntry)
				ret.Entries = ret.Entries[:in.PageSize]
				return ret, nil
			}
			if *outOfTime && firstUnsortedEntry > 0 {
				cancelWorker()
				ret.NextPageToken = makeHistoryPageToken(item.pageBreak, 0)
...
func validateGetTestResultHistoryRequest(in *pb.GetTestResultHistoryRequest) error {
	...
	if in.GetPageToken() != "" {
		...
		if _, err := indexPointFromTokenField(parts[0]); err != nil {
			return errors.Reason("invalid page token - bad index point field %q", parts[0]).Err()
		}
		if _, err := strconv.Atoi(parts[1]); err != nil {
			return errors.Reason("invalid page token - bad offset field %q", parts[1]).Err()
		}
	}
	return nil
}
