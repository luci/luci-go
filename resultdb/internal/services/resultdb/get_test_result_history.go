	...
	go func() {
		...
		for {
			...
			case rInv, ok := <-rInvs:
				...
				results := matchingResultsInInv(ctx, rInv.Name, in, errC)
				moreResults := true
				for moreResults {
					select {
...
func resultInvocationsForRequest(ctx context.Context, in *pb.GetTestResultHistoryRequest, errC chan<- error) <-chan resultsInvocation {
	...
	go func() {
		...
		for _, indexedInv := range idxInvs {
			reachableInvs, err := invocations.Reachable(ctx, invocations.MustParseNames([]string{indexedInv.Name}))
			if err != nil {
			...
			for _, invName := range reachableInvs.Names() {
...
func matchingResultsInInv(ctx context.Context, invName string, in *pb.GetTestResultHistoryRequest, errC chan<- error) <-chan *pb.TestResult {
