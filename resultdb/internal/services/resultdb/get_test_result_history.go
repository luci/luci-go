...
func (s *resultDBServer) GetTestResultHistory(ctx context.Context, in *pb.GetTestResultHistoryRequest) (*pb.GetTestResultHistoryResponse, error) {
	...
	workC := make(chan history.WorkItem, history.NWorkers)
	allResultsC := make(chan chan history.PageItem, history.NWorkers)
	eg.Go(func() error {
		defer close(allResultsC)
		defer close(workC)
		return history.DispatchWorkItems(workerCtx, in, workC, allResultsC)
	})

	eg.Go(func() error {
		return history.Fetch(workerCtx, workC)
	})

	results := make(chan history.PageItem)
	eg.Go(func() error {
		defer close(results)
		return history.Collect(workerCtx, allResultsC, results)
	})
	...
	outOfTime := func(ctx context.Context) bool {
		dl, ok := ctx.Deadline()
		return ok && dl.Before(clock.Now(ctx).Add(5*time.Second))
	}
ResultsLoop:
	for {
		select {
		case r, ok := <-results:
			...
			ret.Entries = append(ret.Entries, r.Entry)
			if outOfTime(ctx) || len(ret.Entries) == int(in.PageSize) {
				cancelWorker()
				ret.NextPageToken = r.PageToken()
				return ret, nil
			}
		case <-ctx.Done():
			return ret, ctx.Err()
