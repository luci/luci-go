		...
		t.Run(`Invalid request`, func(t *ftt.Test) {
			...
			t.Run("Predicate", func(t *ftt.Test) {
				t.Run("Invalid test ID", func(t *ftt.Test) {
					request.Predicate.TestIds = []string{strings.Repeat("a", 7001)}
					_, err := srv.QueryTestMetadata(ctx, request)
					assert.Loosely(t, err, grpccode.ShouldBe(codes.InvalidArgument))
					assert.Loosely(t, err, should.ErrLike(`predicate: test_ids[0]: longer than 7000 bytes`))
				})
				t.Run("Invalid previous test ID", func(t *ftt.Test) {
					request.Predicate.PreviousTestIds = []string{strings.Repeat("a", 7001)}
					_, err := srv.QueryTestMetadata(ctx, request)
					assert.Loosely(t, err, grpccode.ShouldBe(codes.InvalidArgument))
					assert.Loosely(t, err, should.ErrLike(`predicate: previous_test_ids[0]: longer than 7000 bytes`))
				})
