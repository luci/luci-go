...
// when it's done.
func (p *historyStreamerPool) purge(ctx context.Context) <-chan struct{} {
	t := p.pendingTask
...
func (p *historyStreamerPool) tryPutErr(err error) {
	select {
	case p.errC <- err:
	default:
	}
...
func (rst *resultStreamingTask) Run(ctx context.Context) {
	...
	select {
	case rst.pool.resultsC <- historyPageItem{pageBreak: (*tsIndexPoint)(rst.ts)}:
	case <-ctx.Done():
	}
}
