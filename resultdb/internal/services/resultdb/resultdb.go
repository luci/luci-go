...
func InitServer(srv *server.Server, opts Options) error {
	artifactServer, err := artifactcontent.NewServer(srv.Context, opts.InsecureSelfURLs, opts.ContentHostname)
	if err != nil {
		return err
	}

	artifactServer.InstallHandlers(srv.VirtualHost(opts.ContentHostname))

	pb.RegisterResultDBServer(srv.PRPC, &pb.DecoratedResultDB{
		Service: &resultDBServer{
			generateArtifactURL: artifactServer.GenerateSignedURL,
		},
