...
func finalizeInvocation(ctx context.Context, invID invocations.ID) error {
	...
	_, err := span.ReadWriteTransaction(ctx, func(ctx context.Context, txn *spanner.ReadWriteTransaction) error {
		...
		// Read reachable invocations to cache it after the transaction.
		var err error
		reachable, err = invocations.Reachable(ctx, txn, invocations.NewIDSet(invID))
		if err != nil {
			return errors.Annotate(err, "failed to read invocations reachable from %s", invID.Name()).Err()
		}
