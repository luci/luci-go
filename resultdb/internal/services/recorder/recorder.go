...
func InitServer(srv *server.Server, opt Options) error {
	...
	return initArtifactCreationHandler(srv, &opt)
}

func initArtifactCreationHandler(srv *server.Server, opt *Options) error {
	if opt.ArtifactRBEInstance == "" {
		// TODO(crbug.com/1071258): make opt.ArtifactRBEInstance required.
		return nil
	}

	conn, err := newRBEConn(srv.Context, opt.ArtifactRBEInstance)
	if err != nil {
		return err
	}
	...
	// unescaped paths.
	// Issue: https://github.com/julienschmidt/httprouter/issues/208
	srv.Routes.PUT("invocations/*rest", router.MiddlewareChain{}, ach.Handle)
	return nil
}

func newRBEConn(ctx context.Context, rbeInstance string) (*grpc.ClientConn, error) {
