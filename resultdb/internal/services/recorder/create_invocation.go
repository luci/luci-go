...
func verifyCreateInvocationPermissions(ctx context.Context, in *pb.CreateInvocationRequest) error {
	...
	// The value is only set if the invocation's realm is not set, or is set to the corresponding placeholder.
	legacyTrustedCreator := false
	var err error
	if realm == "" || realm == "chromium:public" {
		realm = "chromium:public"
		legacyTrustedCreator, err = auth.IsMember(ctx, "luci-resultdb-trusted-invocation-creators")
		if err != nil {
			return err
		}
	}
	...
	case err != nil:
		return err
	case !allowed && !legacyTrustedCreator:
