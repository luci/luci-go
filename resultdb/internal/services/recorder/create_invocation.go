...
func validateCreateInvocationRequest(req *pb.CreateInvocationRequest, now time.Time, trustedCreator bool) error {
	...
	if !trustedCreator {
		if !strings.HasPrefix(req.InvocationId, "u:") {
			return errors.Reason(`invocation_id: only invocations created by trusted system may have id not starting with "u:"; please generate "u:{GUID}" or reach out to ResultDB owners`).Err()
		}
