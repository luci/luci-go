...
func validateCreateInvocationRequest(req *pb.CreateInvocationRequest, now time.Time) error {
	...
	inv := req.Invocation
	if inv == nil {
		return errors.Annotate(errors.Reason("invocation is required").Err(), "invocation").Err()
	}
...
func verifyCreateInvocationPermissions(ctx context.Context, in *pb.CreateInvocationRequest) error {
	inv := in.Invocation
	if inv == nil {
		return appstatus.BadRequest(errors.Annotate(errors.Reason("invocation is required").Err(), "invocation").Err())
	}

	realm := inv.Realm
	if realm == "" {
		return appstatus.BadRequest(errors.Annotate(errors.Reason("realm is required").Err(), "invocation.realm").Err())
	}
