...
// invalid.
func validateCreateInvocationRequest(req *pb.CreateInvocationRequest, now time.Time, allowCustomID, allowBQExport, allowSetProducer bool) error {
...
func (s *recorderServer) CreateInvocation(ctx context.Context, in *pb.CreateInvocationRequest) (*pb.Invocation, error) {
	...
	perms, err := getPermissions(ctx, in.GetInvocation(), permCreateInvocation, permCreateWithReservedID, permExportToBigQuery, permSetProducerResource)
	if err != nil {
		return nil, err
	}
	if !perms[permCreateInvocation] {
		return nil, appstatus.Errorf(codes.PermissionDenied, `creator does not have permission to create invocations in realm %q`, in.Invocation.Realm)
	}
	if err := validateCreateInvocationRequest(in, now, perms[permCreateWithReservedID], perms[permExportToBigQuery], perms[permSetProducerResource]); err != nil {
