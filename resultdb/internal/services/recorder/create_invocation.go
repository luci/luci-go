...
func (s *recorderServer) CreateInvocation(ctx context.Context, in *pb.CreateInvocationRequest) (*pb.Invocation, error) {
	...
	if err := validateCreateInvocationsPermission(ctx, []*pb.CreateInvocationRequest{in}); err != nil {
		return nil, appstatus.Attachf(err, codes.PermissionDenied, "user does not have permission to create invocations in realm %s", in.GetInvocation().Realm)
	}
	if err := validateSetProducerResourcePermission(ctx, in.Invocation); err != nil {
		return nil, appstatus.Attachf(err, codes.PermissionDenied, "user does not have permission to set producer resource field in invocations of realm %s", in.GetInvocation().Realm)
	}
