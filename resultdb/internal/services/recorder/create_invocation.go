...
func verifyCreateInvocationPermissions(ctx context.Context, in *pb.CreateInvocationRequest) error {
	...
	allowed, err := auth.HasPermission(ctx, permCreateInvocation, []string{realm})
	if err != nil {
		return err
	}
	if !allowed {
	...
	if !strings.HasPrefix(in.InvocationId, "u-") {
		allowed, err := auth.HasPermission(ctx, permCreateWithReservedID, []string{realm})
		if err != nil {
			return err
		}
		if !allowed {
	...
	if len(inv.GetBigqueryExports()) > 0 {
		allowed, err := auth.HasPermission(ctx, permExportToBigQuery, []string{realm})
		if err != nil {
			return err
		}
		if !allowed {
	...
	if inv.GetProducerResource() != "" {
		allowed, err := auth.HasPermission(ctx, permSetProducerResource, []string{realm})
		if err != nil {
			return err
		}
		if !allowed {
