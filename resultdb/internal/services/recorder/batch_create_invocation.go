...
// createInvocations is a shared implementations for CreateInvocation and BatchCreateInvocations RPCs
func (s *recorderServer) createInvocations(ctx context.Context, reqs []*pb.CreateInvocationRequest, requestID string) ([]*pb.Invocation, []string, error) {
	...
	idSet, err := validateMultipleCreateInvocationRequests(now, reqs, requestID, allowCustomID)
	if err != nil {
	...
	_, err = span.ReadWriteTransaction(ctx, func(ctx context.Context, txn *spanner.ReadWriteTransaction) error {
		deduped, err = deduplicateCreateInvocations(ctx, idSet, requestID, txn)
		if err != nil {
			return err
		}
		if !deduped {
...
// are not repeated.
func validateMultipleCreateInvocationRequests(
...
func deduplicateCreateInvocations(ctx context.Context, idSet span.InvocationIDSet, requestID string, txn span.Txn) (bool, error) {
	invCount := 0
