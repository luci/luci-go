...
func validateBatchCreateInvocationsRequest(
	now time.Time, reqs []*pb.CreateInvocationRequest, requestID string, perms []map[realms.Permission]bool) (invocations.IDSet, error) {
	...
	for i, req := range reqs {
		if err := validateCreateInvocationRequest(req, now, perms[i][permCreateWithReservedID], perms[i][permExportToBigQuery], perms[i][permSetProducerResource]); err != nil {
...
func (s *recorderServer) BatchCreateInvocations(ctx context.Context, in *pb.BatchCreateInvocationsRequest) (*pb.BatchCreateInvocationsResponse, error) {
	...
	var err error
	perms := make([]map[realms.Permission]bool, len(in.Requests))
	for i, r := range in.Requests {
		perms[i], err = getPermissions(ctx, r.GetInvocation(), permCreateInvocation, permCreateWithReservedID, permExportToBigQuery, permSetProducerResource)
		if err != nil {
			return nil, err
		}
		if !perms[i][permCreateInvocation] {
