...
func validateBatchCreateInvocationsRequest(
	ctx context.Context, now time.Time, reqs []*pb.CreateInvocationRequest, requestID string) (invocations.IDSet, error) {
	...
	for i, req := range reqs {
		if err := validateCreateInvocationRequest(ctx, req, now); err != nil {
...
func (s *recorderServer) BatchCreateInvocations(ctx context.Context, in *pb.BatchCreateInvocationsRequest) (*pb.BatchCreateInvocationsResponse, error) {
	...
	if err := validateCreateInvocationPermission(ctx, in.Requests...); err != nil {
		return nil, err
	}
	if err := validateSetProducerResourcePermission(ctx, in.Requests...); err != nil {
		return nil, err
	}

	idSet, err := validateBatchCreateInvocationsRequest(ctx, now, in.Requests, in.RequestId)
	if err != nil {
