...
func createArtifactStates(ctx context.Context, invID invocations.ID, arts []*artifactCreationRequest) ([]*artifactCreationRequest, error) {
	...
	logging.Errorf(ctx, "Failed to write artifact to Spanner: %s", err)
...
func uploadArtifactBlobs(ctx context.Context, rbeIns string, casClient repb.ContentAddressableStorageClient, invID invocations.ID, arts []*artifactCreationRequest) error {
	...
	resp, err := casClient.BatchUpdateBlobs(ctx, casReq, maxReqSizeOpt)
	if err != nil {
		...
		return appstatus.Errorf(codes.Internal, "exceeded the maximum size limit of the internal storage")
	}
