...
// newArtifactCreationRequestsFromProto creates artifactionCreationRequests.
// from the BatchCreationArtifactsRequest. The hash of the created requests are unset, and
// will be set compSetAndCheckState(), after the token validation.
//
...
// - the associated invocation is not ACTIVE, or
// - there exists an artifact w/ the same key, but different hash or size.
//
// On success, it returns a list of the given artifactCreationRequests(s) of which artifacts
// don't exist yet.
func verifyBatchArtifactsState(ctx context.Context, arts []*artifactCreationRequest) ([]*artifactCreationRequest, error) {
	if len(arts) == 0 {
		return nil, nil
	}
	inv := arts[0].invID
...
func (s *recorderServer) BatchCreateArtifacts(ctx context.Context, in *pb.BatchCreateArtifactsRequest) (*pb.BatchCreateArtifactsResponse, error) {
	...
	artsToSave, err := verifyBatchArtifactsState(ctx, arts)
	if err != nil {
		return nil, err
	}
	if len(artsToSave) == 0 {
