...
// checkArtStatesBeforeWriting checks the Spanner states of the artifacts
// in a read-only transaction. see checkArtStates for more info.
func checkArtStatesBeforeWriting(ctx context.Context, invID invocations.ID, arts []*artifactCreationRequest) ([]*artifactCreationRequest, error) {
	ctx, cancel := span.ReadOnlyTransaction(ctx)
	defer cancel()
	return checkArtStates(ctx, invID, arts)
}
...
func createArtifactStates(ctx context.Context, invID invocations.ID, arts []*artifactCreationRequest) error {
	...
	_, err := span.ReadWriteTransaction(ctx, func(ctx context.Context) (err error) {
		...
		if len(noStateArts) == 0 {
			logging.Infof(ctx, "The states of all the artifacts already exist.")
		}
	...
	return errors.Annotate(err, "failed to write artifac to Spanner").Err()
}
...
func uploadArtifactBlobs(ctx context.Context, rbeIns string, casClient repb.ContentAddressableStorageClient, invID invocations.ID, arts []*artifactCreationRequest) error {
	...
	for i, r := range resp.GetResponses() {
		...
		if cd != codes.OK {
			...
			// If resource exhausted, the server quota needs to be updated.
			//
			// Either case, it's a server-error, and internal-error should be returned.
...
func (s *recorderServer) BatchCreateArtifacts(ctx context.Context, in *pb.BatchCreateArtifactsRequest) (*pb.BatchCreateArtifactsResponse, error) {
	...
	artsToCreate, err := checkArtStatesBeforeWriting(ctx, invID, arts)
	if err != nil {
		return nil, err
	}
