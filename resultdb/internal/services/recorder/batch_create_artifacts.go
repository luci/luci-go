		...
		for _, a := range arts {
			work <- func() (err error) {
				...
				if err != nil {
					...
					spanutil.ReadRow(ctx, "Artifacts", inv.Key(a.parentID, a.artifactID), map[string]interface{}{
						"RBECASHash": &actualHash,
						"Size":       &actualSize,
					})
					panic(fmt.Sprintf("%s == %s, %d == %d", a.hash, actualHash.StringVal, a.size, actualSize.Int64))
				}
