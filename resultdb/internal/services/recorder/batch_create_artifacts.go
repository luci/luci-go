...
// parseBatchCreateArtifactsRequest parses the request and returns artifactCreators for
// the artifacts. It returns an error, if any of the artifacts were invalid or the total
// size exceeds MaxBatchUploadArtifactSize.
func parseBatchCreateArtifactsRequest(in *pb.BatchCreateArtifactsRequest) ([]*artifactCreator, error) {
	var tSize int64
	ret := make([]*artifactCreator, 0, len(in.Requests))
...
// On success, it returns a list of the given artifactCreator(s) of which artifacts don't
// exist yet.
