...
// newArtifactCreationRequestsFromProto creates a list of artifactionCreationRequest(s)
// from a BatchCreationArtifactsRequest. It returns an error, if
	...
	err := parallel.FanOutIn(func(work chan<- func() error) {
		work <- func() (err error) {
			...
			if err == nil && invState != pb.Invocation_ACTIVE {
				return appstatus.Errorf(codes.FailedPrecondition, "%s is not active", inv)
			}
			return
		}
