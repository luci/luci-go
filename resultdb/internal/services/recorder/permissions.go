...
func hasPermission(ctx context.Context, perm realms.Permission, inv *pb.Invocation) (bool, error) {
	return auth.HasPermission(ctx, perm, []string{inv.GetRealm()})
}
...
func validateCreateInvocationsPermission(ctx context.Context, reqs []*pb.CreateInvocationRequest) error {
	for _, req := range reqs {
		...
		if !allowed {
			return appstatus.Errorf(codes.PermissionDenied, `invocation: realm: creator does not have permission to create invocations in realm %s`, req.GetInvocation().Realm)
		}
	}
	return nil
}
...
func validateSetProducerResourcePermission(ctx context.Context, reqs []*pb.CreateInvocationRequest) error {
	...
	for _, req := range reqs {
		...
		if inv.GetRealm() == "" {
			...
			return appstatus.Errorf(codes.PermissionDenied, `invocation: producer_resource: caller does not have permisison to set the field in the default realm`)
		}
		...
		if !allowed && inv.ProducerResource != "" {
			return appstatus.Errorf(codes.PermissionDenied, `invocation: producer_resource: caller does not have permission to set the field in realm %s`, req.GetInvocation().Realm)
		}
	}
	return nil
}
