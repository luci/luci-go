	...
	Convey(`BatchCreateTestResults`, t, func() {
		...
		createTestResults := func(req *pb.BatchCreateTestResultsRequest, expectedCommonPrefix string) {
			...
			for i, r := range req.Requests {
				...
				err = invocations.ReadColumns(span.Single(ctx), invID, map[string]interface{}{"CommonTestIDPrefix": &invCommonTestIdPrefix})
				So(err, ShouldBeNil)
		...
		Convey("succeeds", func() {
			...
			Convey("with uncommon test id", func() {
				...
				createTestResults(req, invocations.NoCommonPrefix)
			})
		})
