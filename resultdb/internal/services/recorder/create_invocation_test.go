...
func TestValidateCreateInvocationRequest(t *testing.T) {
	...
	Convey(`TestValidateCreateInvocationRequest`, t, func() {
		...
		Convey(`reserved prefix`, func() {
			...
			So(err, ShouldErrLike, `only invocations created by trusted system may have id not starting with "u:"`)
		})
		...
		Convey(`producer_resource disallowed`, func() {
			...
			So(err, ShouldErrLike, `invocation: producer_resource: producer_resource: only trusted systems are allowed`)
		})
...
func TestCreateInvocation(t *testing.T) {
	Convey(`TestCreateInvocation`, t, func() {
		...
		db := authtest.FakeDB{
			"anonymous:anonymous": []string{"luci-resultdb-trusted-invocation-creators"},
		}
		ctx = db.Use(ctx)
