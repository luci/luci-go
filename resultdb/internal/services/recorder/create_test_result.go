...
func insertTestResult(ctx context.Context, invID span.InvocationID, requestID string, body *pb.TestResult) (*pb.TestResult, *spanner.Mutation) {
	// create a shallow copy of the input message with `Name` to be used as a response.
	ret := *body
	ret.Name = pbutil.TestResultName(string(invID), body.TestId, body.ResultId)

	// some columns accept null for the optional message fields.
	isUnexpected := spanner.NullBool{Bool: true, Valid: true}
	if body.Expected == true {
		isUnexpected.Valid = false
	}
