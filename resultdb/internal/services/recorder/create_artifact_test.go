	...
	Convey(`CreateArtifact`, t, func() {
		...
		Convey(`Verify digest`, func() {
			...
			Convey(`Hash`, func() {
				...
				So(rec.Code, ShouldEqual, http.StatusBadRequest)
				So(rec.Body.String(), ShouldEqual, "Content-Hash header value does not match the hash of the request body\n")
			})
			...
			Convey(`Size`, func() {
				...
				So(rec.Code, ShouldEqual, http.StatusBadRequest)
				So(rec.Body.String(), ShouldEqual, "Content-Length header value does not match the length of the request body\n")
			})
		})
	})
}
