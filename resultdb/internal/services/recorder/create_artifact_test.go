...
func TestCreateArtifact(t *testing.T) {
	Convey(`CreateArtifact`, t, func() {
		...
		send := func(artifact, hash string, size int64) *httptest.ResponseRecorder {
		...
		Convey(`Size is negative`, func() {
			...
			So(rec.Code, ShouldEqual, http.StatusBadRequest)
			So(rec.Body.String(), ShouldContainSubstring, "Content-Length header must not be negative")
		})
	})
}
