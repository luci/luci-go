...
func (ac *artifactCreator) handle(c *router.Context) error {
	...
	// This is mandatory because RBE-CAS does not verify the digest of blobs it
	// already has.
	ver := &digestVerifier{
		r:            c.Request.Body,
...
// digestVerifier is an io.Reader that also verifies digest.
...
func (v *digestVerifier) Read(p []byte) (n int, err error) {
	...
	if _, hashErr := v.actualHash.Write(p[:n]); hashErr != nil {
		panic(hashErr)
	}
	return n, err
}
...
func (v *digestVerifier) ReadVerify() error {
	// Read until the end.
	...
	if v.actualSize != v.expectedSize {
		return appstatus.Errorf(codes.InvalidArgument, "Content-Length header value does not match the length of the request body")
	}

	// Verify hash.
	actualHash := hex.EncodeToString(v.actualHash.Sum(nil))
	if actualHash != v.expectedHash {
		return appstatus.Errorf(codes.InvalidArgument, "Content-Hash header value does not match the hash of the request body")
	}
