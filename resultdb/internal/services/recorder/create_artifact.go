...
func (ac *artifactCreator) parseRequest(c *router.Context) error {
	...
	case ac.size < 0 || ac.size > ac.MaxArtifactContentSize:
		return appstatus.Errorf(codes.InvalidArgument, "%s header must be a value between 0 and %d", artifactContentSizeHeaderKey, ac.MaxArtifactContentSize)
	}
