...
//
// Request:
//  - Parameter "artifact" must be a valid artifact name.
//  - The request body MUST be the artifact contents.
//  - The request MUST include an Update-Token header with the value of
//    invocation's update token, i.e. the same token required in all invocation
//    mutation operations.
...
func (ac *artifactCreator) parseRequest(c *router.Context) error {
	...
	case ac.size < 0:
		return appstatus.Errorf(codes.InvalidArgument, "%s header must not be negative", artifactContentSizeHeaderKey)
	}
