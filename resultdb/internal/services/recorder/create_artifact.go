...
func (h *artifactCreationHandler) Handle(c *router.Context) {
	...
	// each has own body(io.Reader), and meta information(name, invID, testID, etc)
	artReqs, err := h.parseRequest(c.Request)
	if err != nil {
		http.Error(c.Context, "bad request", http.StatusBadRequest)
	}

	// This runs a Spanner query to check duplicates for all the artifacts.
	// It will return artReqs without the duplicates.
	artReqs, err := h.verifyBeforeWriting(ctx, artReqs)
	if err != nil {
		http.Error(c.Context, "bad request", http.StatusBadRequest)
	}
	if len(artReqs) == 0 {
	...
	// This writeToCAS() takes (ctx, []*artifactCreationRequest), instead of
	// (ctx, io.Reader).
	//
