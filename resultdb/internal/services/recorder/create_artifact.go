...
func (ac *artifactCreator) writeToCAS(ctx context.Context, r io.Reader) (err error) {
	ctx, overallSpan := trace.StartSpan(ctx, "writeToCAS")
	...
	bytesSent := 0
	for {
		_, readSpan := trace.StartSpan(ctx, "read chunk")
		n, err := r.Read(buf)
		...
		if first {
			first = false
			req.ResourceName, err = ac.genWriteResourceName(ctx)
			if err != nil {
				return err
			}
		}
		...
		// Send the request.
		_, writeSpan := trace.StartSpan(ctx, "write chunk")
...
func (ac *artifactCreator) genWriteResourceName(ctx context.Context) (string, error) {
