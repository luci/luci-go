...
func purgeOneInvocation(ctx context.Context, inv span.InvocationID) error {
	...
	if err := span.ReadInvocation(ctx, txn, inv, ptrs); err != nil {
		return err
	}
	...
	count := 0
	err := testResultsToPurge(ctx, txn, inv, func(testID, resultID string) error {
		count++
		ms = append(ms, spanner.Delete("TestResults", inv.Key(testID, resultID)))
		...
		if len(ms) > 19000 {
			...
			span.IncTestResultCount(ctx, len(ms), span.Deleted)
			ms = ms[:0]
		}
		return nil
	})
	...
	if err := unsetInvocationResultsExpiration(ctx, inv); err != nil {
		return err
	}

	logging.Debugf(ctx, "Deleted %d test results in %s for purging", count, inv.Name())
	return nil
}
