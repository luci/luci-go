...
func (i *testResultRowInput) rows() []*bq.Row {
	tr := i.tr
	...
	return []*bq.Row{&bq.Row{Message: ret}}
}

func (i *testResultRowInput) id() string {
	return i.tr.Name
}
...
func (b *bqExporter) queryTestResults(
	...
	err = q.Run(ctx, func(tr *pb.TestResult) error {
		...
		rows = append(rows, b.generateBQRows(&testResultRowInput{
