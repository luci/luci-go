...
// Row size limit is 5Mib according to
// https://cloud.google.com/bigquery/quotas#streaming_inserts
// Cap the summaryHTML's length to 4M to ensure the row size is under
// limit.
...
func variantToStringPairs(vr *pb.Variant) []*pb.StringPair {
	...
	for i, k := range keys {
		sp[i] = pbutil.StringPair(k, defMap[k])
	}
	return sp
}

func invocationProtoToInvocation(inv *pb.Invocation) *bqpb.InvocationRecord {
...
func (i *rowInput) row() *bqpb.TestResultRow {
	...
	ret := &bqpb.TestResultRow{
		Exported:      invocationProtoToInvocation(i.exported),
		Parent:        invocationProtoToInvocation(i.parent),
		TestId:        tr.TestId,
