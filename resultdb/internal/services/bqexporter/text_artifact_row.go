...
func (b *bqExporter) downloadArtifactContent(ctx context.Context, a *artifact, rowC chan rowInput) error {
	ac := artifactcontent.Reader{
		...
		Hash:        a.RBECASHash,
		Size:        a.Artifact.Artifact.SizeBytes,
	}
...
func (b *bqExporter) queryTextArtifacts(ctx context.Context, exportedID invocations.ID, bqExport *pb.BigQueryExport, artifactC chan *artifact) error {
	...
	return q.Run(ctx, func(a *artifacts.Artifact) error {
		...
		case <-ctx.Done():
			return ctx.Err()
		case artifactC <- &artifact{a, invs[exportedID], invs[invID]}:
		}
		return nil
	})
}
...
func (b *bqExporter) artifactRowInputToBatch(ctx context.Context, rowC chan rowInput, batchC chan []rowInput) error {
	...
	for row := range rowC {
		...
		if len(rows) >= b.MaxBatchRowCount-1 || batchSize+contentLength >= b.MaxBatchSizeApprox {
			select {
			case <-ctx.Done():
				return ctx.Err()
			case batchC <- rows:
...
func (b *bqExporter) artifactsToRowInputs(ctx context.Context, workers *int32, artifactC chan *artifact, rowC chan rowInput) error {
	defer func() {
		// The last worker closes rowC.
	...
	for a := range artifactC {
		err := b.downloadArtifactContent(ctx, a, rowC)
		if err != nil {
			return err
		}
	}
...
func (b *bqExporter) exportTextArtifactsToBigQuery(ctx context.Context, ins inserter, invID invocations.ID, bqExport *pb.BigQueryExport) error {
	...
	workers := int32(artifactWorkers)
	for w := 0; w < artifactWorkers; w++ {
		eg.Go(func() error {
			return b.artifactsToRowInputs(ctx, &workers, artifactC, rowC)
		})
	}
