...
type rowInput interface {
	// rows returns a slice of proto messages for a BigQuery rows.
	rows() []*bq.Row

	// id returns an identifier for the rows.
	id() string
}

// generateBQRows returns a slice of *bq.Row to be inserted into BQ.
