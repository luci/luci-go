...
type Options struct {
	// ForceQueryInterval forces minimum interval in cron jobs.
	...
	MaxBatchSizeBytesApprox int

	// Maximum rate for BigQuery Streaming Inserts.
	RateLimit rate.Limit

	// How much memory to allocate for row batch buffer.
	BatchBufferSizeBytes int
}
...
func DefaultOptions() Options {
	return Options{
		...
		MaxBatchSizeBytesApprox: 6 * 1024 * 1024, // 6 MiB

		BatchBufferSizeBytes: 2 * 1024 * 1024 * 1024, // 2 GiB
...
func InitServer(srv *server.Server, opts Options) {
	b := &bqExporter{
		...
		batchSem:   semaphore.NewWeighted(int64(opts.MaxBatchSizeBytesApprox / opts.BatchBufferSizeBytes)),
	}
	...
	d := tasks.Dispatcher{
		...
		// invocations at the same time and risk no finishing any of them.
...
func (b *bqExporter) queryTestResults(
	...
	err = q.Run(ctx, txn, func(tr span.TestResultQueryItem) error {
		...
		if len(rows) >= b.MaxBatchRowCount || batchSize >= b.MaxBatchSizeBytesApprox {
			select {
			case <-ctx.Done():
				return ctx.Err()
			case batchC <- rows:
