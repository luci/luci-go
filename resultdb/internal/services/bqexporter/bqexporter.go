...
func (b *bqExporter) batchExportRows(ctx context.Context, ins inserter, batchC chan []*bq.Row, errorLogger func(ctx context.Context, err bigquery.PutMultiError, rows []*bq.Row)) error {
...
func (b *bqExporter) insertRowsWithRetries(ctx context.Context, ins inserter, rows []*bq.Row, errorLogger func(ctx context.Context, err bigquery.PutMultiError, rows []*bq.Row)) error {
...
// testResultRowInput is information required to generate a BigQuery row.
type rowInput interface {
	// row returns a proto message for a BigQuery row.
	rows() []*bq.Row
...
// generateBQRows returns a *bq.Row to be inserted into BQ.
func (b *bqExporter) generateBQRows(i interface{}) []*bq.Row {
	input, ok := i.(rowInput)
	if !ok {
		return nil
	}
	...
	for i, row := range ret {
		if b.UseInsertIDs {
			...
			hash := sha512.Sum512([]byte(fmt.Sprintf("%s%d", baseID, i)))
