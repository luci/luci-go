...
func updateTaskStats(ctx context.Context) error {
	...
	err := spanutil.Query(ctx, span.Single(ctx), st, func(row *spanner.Row) error {
		var taskType string
...
func expiredResultStats(ctx context.Context) (oldestResult time.Time, pendingInvocationsCount int64, err error) {
	...
	`)
	err = spanutil.QueryFirstRow(ctx, span.Single(ctx), st, &earliest, &pendingInvocationsCount)
