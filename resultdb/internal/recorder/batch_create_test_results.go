...
func mustBeEmptyOrEqual(name, actual, expected string) error {
	switch actual {
	case "":
	case expected:
	default:
		return errors.Reason(
			"%s must be either empty or equal to %q, but %q", name, expected, actual,
		).Err()
	}
	return nil
}
...
func validateBatchCreateTestResultsRequest(msg *pb.BatchCreateTestResultsRequest, now time.Time) error {
	if err := pbutil.ValidateInvocationID(msg.Invocation); err != nil {
	...
	for i, r := range msg.Requests {
		if err := mustBeEmptyOrEqual("invocation", r.Invocation, msg.Invocation); err != nil {
			return errors.Annotate(err, "requests: %d", i).Err()
		}
		if err := mustBeEmptyOrEqual("request_id", r.RequestId, msg.RequestId); err != nil {
			return errors.Annotate(err, "requests: %d", i).Err()
		}
