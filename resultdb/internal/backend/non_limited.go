...
// nonLimited is an retry.Iterator implementation that only retries errors
// if they are tagged with errTag.
type nonLimited struct {
...
func (it *nonLimited) Next(ctx context.Context, err error) time.Duration {
	if !it.errTag.In(err) {
		return retry.Stop
	}
...
func nonLimitedFactory(errTag errors.BoolTag, delay, maxDelay time.Duration) retry.Factory {
