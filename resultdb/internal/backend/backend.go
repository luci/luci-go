...
//
// It also holds a format string for the heartbeat message and a logging
// level.
//
...
// Beat emits a log item if lastHeartbeat was longer than MinHeartbeatInterval ago.
//
// It uses a format string if set, otherwise it simply concatenates the args with
// fmt.Sprint.
...
func processingLoop(ctx context.Context, activityName string, minInterval, maxSleep time.Duration, f func(context.Context) error) {
	...
	iterationCounter := 0
	tl := ThrottledLogger{}
	tl.Format = fmt.Sprintf("%s has ran %%d iterations", activityName)
