...
func processingLoop(ctx context.Context, minSleep, maxSleep, iterationTimeout time.Duration, f func(context.Context) error) {
	defer func() {
		if ctx.Err() != nil {
	...
	for {
		...
		if min := minSleep - clock.Since(ctx, start); sleep < min {
			sleep = min
		}
		if sleep > maxSleep {
			sleep = maxSleep
