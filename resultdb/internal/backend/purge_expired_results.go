...
var (
	purgedInvocationsMetric = metric.NewCounter(
		...
		nil)
	sampledInvocationsDelay = metric.NewCumulativeDistribution(
		"resultdb/sampled_invocations_delay",
...
func randomExpiredResultsInvocations(ctx context.Context, shardID, sampleSize int) ([]span.InvocationID, error) {
	st := spanner.NewStatement(`
		WITH expiringInvocations AS (
			SELECT InvocationId, ExpectedTestResultsExpirationTime
	...
	err := span.Query(ctx, span.Client(ctx).Single(), st, func(row *spanner.Row) error {
		...
		var exp spanner.NullTime
		if err := span.FromSpanner(row, &res, &exp); err != nil {
			return err
		}
		ret = append(ret, res)
		sampledInvocationsDelay.Add(ctx, time.Since(exp.Time).Seconds())
		return nil
	})
	return ret, err
}
