...
func purgeExpiredResults(ctx context.Context) {
	...
	for i := int64(0); i <= maxShard; i++ {
		...
		processingLoop(ctx, time.Second, 5*time.Second, 10*time.Minute, func(ctx context.Context) error {
			id, err := sampleExpiredResultsInvocations(ctx, i)
			switch err {
...
// sampleExpiredResultsInvocations selects a random invocation that has expired
// test results.
func sampleExpiredResultsInvocations(ctx context.Context, shardId int64) (span.InvocationID, error) {
