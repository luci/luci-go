...
func purgeExpiredResults(ctx context.Context) {
	...
	for i := int64(0); i <= maxShard; i++ {
		i := i
		go processingLoop(ctx, time.Second, 5*time.Second, 10*time.Minute, func(ctx context.Context) error {
...
// test results.
func randomExpiredResultsInvocation(ctx context.Context, shardId int64) (span.InvocationID, error) {
