...
// runTask leases, runs the task and mark the task as done.
func (b *backend) runTask(ctx context.Context, taskType tasks.Type, id string) (ran bool, err error) {
	...
	if err != nil {
		if permanentInvocationTaskErrTag.In(err) {
			...
			return false, tasks.Delete(ctx, taskType, id)
		}
		return false, err
	}
...
func (b *backend) runInvocationTasks(ctx context.Context, taskType tasks.Type) {
	...
	b.cronGroup(ctx, 100, time.Second, func(ctx context.Context, replica int) error {
		...
		for _, id := range ids {
			...
			switch ran, err := b.runTask(ctx, taskType, id); {
			case err != nil:
				return err
			case ran:
