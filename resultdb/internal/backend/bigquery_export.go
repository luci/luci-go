...
const (
	...
	// HTTP request size limit is 10 MB according to
...
func queryTestResultsStreaming(ctx context.Context, txn *spanner.ReadOnlyTransaction, exportedID span.InvocationID, q span.TestResultQuery, exoneratedTestVariants map[testVariantKey]struct{}, maxBatchRowCount int, batchC chan []*bigquery.StructSaver) error {
...
func exportTestResultsToBigQuery(ctx context.Context, ins inserter, invID span.InvocationID, bqExport *pb.BigQueryExport, maxBatchRowCount int) error {
	...
	eg.Go(func() error {
		defer close(batchC)
		return queryTestResultsStreaming(ctx, txn, invID, q, exoneratedTestVariants, maxBatchRowCount, batchC)
	})
...
func exportResultsToBigQuery(ctx context.Context, invID span.InvocationID, payload []byte) error {
	...
	return exportTestResultsToBigQuery(ctx, ins, invID, bqExport, maxBatchRowCount)
}
