...
func (it *internalIter) next(ctx context.Context) time.Duration {
	...
	case delay > it.maxDelay:
		delay = it.maxDelay
	default:
		it.delay = time.Duration(float64(delay) * 2)
	}

	if it.addJitter {
		// +-10% of delay.
		delay = delay - delay/10 + time.Duration(mathrand.Intn(ctx, int(delay/5)))
	}
...
// If when retrying a request we got different errors, for example
// [quotaExceeded, streamClosed, streamClosed, quotaExceeded],
