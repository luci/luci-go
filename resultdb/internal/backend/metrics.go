...
var (
	...
	oldestTaskMetric = metric.NewInt(
		...
		"The creation time of the oldest task, per type.",
...
func recordOldestTaskMetric(ctx context.Context, typ tasks.Type) {
	tsmoncommon.RegisterCallbackIn(ctx, func(ctx context.Context) {
		...
		typStr := string(typ)
		if err != nil {
			logging.Errorf(ctx, "Failed to get the creation time of the oldest task of type %s: %s", typStr, err)
			return
		}

		oldestTaskMetric.Set(ctx, ct.Unix(), typStr)
	})
}
