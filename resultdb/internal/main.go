...
func Main(init func(srv *server.Server) error) {
	...
	resultdbProd := flag.Bool("resultdb-prod", false, "Run ResultDB in production mode")

	server.Main(nil, modules, func(srv *server.Server) error {
		var err error
		if srv.Context, err = withProdSpannerClient(srv.Context, *spannerDB, !*resultdbProd); err != nil {
			return err
		}
