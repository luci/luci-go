...
func withProdSpannerClient(ctx context.Context, dbFlag string) (context.Context, error) {
	...
	spannerClient, err := spanner.NewClient(ctx, dbFlag,
		...
		option.WithTelemetryDisabled(), // see https://github.com/googleapis/google-cloud-go/issues/1691
	)
	if err != nil {
		return ctx, err
	}
	...
	// before we actually serving any requests. On misconfiguration better to fail
	// early.
