...
// leaseDuration is the time allowed for a worker to work on an invocation task.
...
// Sample randomly picks sampleSize of rows in InvocationTasks
...
// ErrConflict is returned by Lease if the task is already leased by someone
// else.
...
func Lease(ctx context.Context, typ Type, id string) (invID span.InvocationID, payload []byte, err error) {
		...
		default:
			return txn.BufferWrite([]*spanner.Mutation{
				span.UpdateMap("InvocationTasks", map[string]interface{}{
					"TaskType":     string(typ),
					"TaskId":       id,
					"ProcessAfter": now.Add(leaseDuration),
				}),
			})
		}
	})
