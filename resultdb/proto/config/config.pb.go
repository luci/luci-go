...
type Config struct {
	...
	// Allowed tests can use up to 350 bytes for coarse name, 6000 bytes for
	// case name, and 7000 bytes for full encoded test ID.
	//
...
type TestIdEntry struct {
	...
	// The case name. Match by regex.
	// E.g. "^DynamicsProcessingTest/DynamicsProcessingTestEngineArchitecture#.*$".
	CaseNamePattern string `protobuf:"bytes,5,opt,name=case_name_pattern,json=caseNamePattern,proto3" json:"case_name_pattern,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}
...
func (x *TestIdEntry) GetCaseNamePattern() string {
	if x != nil {
		return x.CaseNamePattern
	}
	return ""
}
...
const file_go_chromium_org_luci_resultdb_proto_config_config_proto_rawDesc = "" +
	...
	"\x05value\x18\x02 \x01(\v24.luci.resultdb.config.AndroidBuild.ByDataRealmConfigR\x05value:\x028\x01\"\xc8\x01\n" +
	...
	"\x13module_name_pattern\x18\x04 \x01(\tR\x11moduleNamePattern\x12*\n" +
	"\x11case_name_pattern\x18\x05 \x01(\tR\x0fcaseNamePatternB5Z3go.chromium.org/luci/resultdb/proto/config;configpbb\x06proto3"
