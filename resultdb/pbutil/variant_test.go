...
func TestVariantUtils(t *testing.T) {
	...
	Convey(`Conversion from pair strings works`, t, func() {
		Convey(`for valid pairs`, func() {
			def, err := VariantFromStrings([]string{"k1:v1", "key/k2:v2", "key/with/part/k3:v3"})
			So(err, ShouldBeNil)
			So(def, ShouldResembleProto, Variant(
				"k1",               "v1",
				"key/k2",           "v2",
				"key/with/part/k3", "v3",
			))
		...
		Convey(`for empty list returns nil`, func() {
			def, err := VariantFromStrings([]string{})
			So(def, ShouldBeNil)
			So(err, ShouldBeNil)
		})
	})
	...
	Convey(`Key sorting works`, t, func() {
		...
		So(SortedVariantKeys(v), ShouldResemble, []string{"k1", "k2", "k3"})
	})
}
