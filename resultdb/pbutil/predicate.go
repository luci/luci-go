...
func NormalizeTestPathPredicate(pred *pb.TestPathPredicate) *pb.TestPathPredicate {
	...
	// covered returns true if s already covered by ret.PathPrefixes.
	covered := func(s string) bool {
		for _, another := range ret.PathPrefixes {
			if strings.HasPrefix(s, another) {
				return true
			}
		}
		return false
	...
	// Add only paths not covered by prefixes, and only unique paths.
