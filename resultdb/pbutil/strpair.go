...
func ValidateStringPair(p *pb.StringPair) error {
	...
	if len(p.Key) > maxStringPairKeyLength {
		return errors.Reason("key length %d must be less or equal to %d", len(p.Key), maxStringPairKeyLength).Err()
	}
	if len(p.Value) > maxStringPairValueLength {
		return errors.Reason("value length %d must be less or equal to %d", len(p.Value), maxStringPairValueLength).Err()
	}
	return nil
}
