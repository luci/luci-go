...
func ValidateBigQueryExport(bq_export *pb.BigQueryExport) error {
	...
	if bq_export.GetTestResultsInput() != nil {
		if bq_export.TestResultsInput.GetPredicate() == nil {
			return errors.Reason("test_results_input.predicate missing").Err()
		}

		if err := ValidateTestResultPredicate(bq_export.TestResultsInput.Predicate); err != nil {
