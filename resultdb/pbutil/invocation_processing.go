...
func ValidateBigQueryExport(bqExport *pb.BigQueryExport) error {
	...
	if err := validateResultType(bqExport); err != nil {
		return errors.Annotate(err, "result_type").Err()
	}

	return nil
}

func validateResultType(bqExport *pb.BigQueryExport) error {
	...
	case *pb.BigQueryExport_Artifacts_:
		return errors.Annotate(ValidateArtifactPredicate(resultType.Artifacts.GetPredicate()), "artifacts: predicate").Err()
	case nil:
		return unspecified()
	default:
		panic("impossible")
