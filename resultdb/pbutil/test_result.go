...
// VlixdateSinkArtifact returns a non-nil error if art is invalid.
func ValidateSinkArtifact(art *sinkpb.Artifact) error {
	if art.GetFilePath() == "" && art.GetContents() == nil {
		return errors.Reason("body: either file_path or contents must be present").Err()
	}
	// skip `ContentType`
	return nil
}
...
// ValidateSinkTestResults ValidateTestResult returns a non-nil error if msg is invalid.
