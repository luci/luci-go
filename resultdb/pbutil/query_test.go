		...
		Convey(`test variant`, func() {
			...
			Convey(`exact`, func() {
				...
				Convey(`invalid`, func() {
					...
					So(err, ShouldErrLike, `test_variant: exact: "":"": key: does not match`)
				})
			})
			...
			Convey(`superset_of`, func() {
				...
				Convey(`invalid`, func() {
					...
					So(err, ShouldErrLike, `test_variant: superset_of: "":"": key: does not match`)
				})
			})

			Convey(`unspecified`, func() {
				err := validate(&pb.VariantPredicate{})
				So(err, ShouldErrLike, `test_variant: unspecified`)
			})
		})

		Convey(`honorExonerations requires inclusions`, func() {
			err := ValidateTestResultPredicate(&pb.TestResultPredicate{
				Invocation: &pb.InvocationPredicate{
					RootPredicate: &pb.InvocationPredicate_Name{Name: "invocations/inv"},
					Inclusions:    pb.InvocationPredicate_NONE,
				},
				HonorExonerations: true,
			}, false)
			So(err, ShouldErrLike, `honor_exclusions requires invocation.inclusions to be STABLE or ALL`)
		})
	})
}
