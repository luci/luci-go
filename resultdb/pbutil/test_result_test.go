	...
	Convey("ParseTestResultName", t, func() {
		Convey("Parse", func() {
			invID, testPath, resultID, err := ParseTestResultName(
				"invocations/a/tests/ninja%2Ftest%2FBarTest.DoBaz/results/result5")
			So(err, ShouldBeNil)
			So(invID, ShouldEqual, "a")
			So(testPath, ShouldEqual, "ninja/test/BarTest.DoBaz")
		...
		Convey("Format", func() {
			So(TestResultName("a", "ninja/test/BarTest.DoBaz", "result5"),
				ShouldEqual,
				"invocations/a/tests/ninja%2Ftest%2FBarTest.DoBaz/results/result5")
		})
	})
