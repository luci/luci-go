...
func TestValidateTestResult(t *testing.T) {
	...
	Convey("Succeeds", t, func() {
		...
		Convey("with valid summary", func() {
			in := strings.Repeat("1", maxLenSummaryHTML)
			msg.SummaryHtml = in
	...
	Convey("Fails", t, func() {
		...
		Convey("with invalid ResultID", func() {
			...
			for _, in := range badInputs {
				msg.ResultId = in
				So(ValidateResultID(msg.ResultId), ShouldErrLike, doesNotMatch(resultIDRe))
				So(ValidateTestResult(now, msg), ShouldErrLike, "result_id")
			}
		})
		...
		Convey("with too big summary", func() {
			...
			in := strings.Repeat("☕", maxLenSummaryHTML)
			msg.SummaryHtml = in
		...
		Convey("with invalid StartTime and Duration", func() {
			...
			Convey("because duration is < 0", func() {
				...
				msg.Duration = nd
				So(ValidateStartTimeWithDuration(now, msg.StartTime, msg.Duration), ShouldErrLike, "is < 0")
			...
			Convey("because (start_time + duration) is in the future", func() {
				...
				So(ValidateStartTimeWithDuration(now, msg.StartTime, msg.Duration), ShouldErrLike, "cannot be in the future")
				So(ValidateTestResult(now, msg), ShouldErrLike, "duration")
			})
		})
...
func TestValidateArtifacts(t *testing.T) {
	...
	Convey("Fails", t, func() {
		...
		Convey("with duplicate names", func() {
			art2.Name = art1.Name
			in := []*pb.Artifact{art1, art2}
			So(ValidateArtifacts(in), ShouldErrLike, fmt.Sprintf("duplicate name %q", art1.Name))
		})
