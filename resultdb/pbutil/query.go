...
// ValidateTestResultQuery returns a non-nil error if req is determined to be
// invalid.
func ValidateTestResultQuery(req *pb.TestResultQuery) error {
	if err := ValidateTestResultPredicate(req.GetPredicate()); err != nil {
		return errors.Annotate(err, "predicate").Err()
	}

	if req.MaxStaleness != nil {
		if err := ValidateMaxStaleness(req.MaxStaleness); err != nil {
...
func ValidateTestResultPredicate(p *pb.TestResultPredicate) error {
	...
	if p.TestVariant != nil {
		if err := ValidateVariantPredicate(p.TestVariant); err != nil {
			return errors.Annotate(err, "test_variant").Err()
		}
	}
...
func ValidateVariantPredicate(p *pb.VariantPredicate) error {
	...
	case *pb.VariantPredicate_Exact:
		return errors.Annotate(ValidateVariantDef(pr.Exact), "exact").Err()
	case *pb.VariantPredicate_SupersetOf:
		return errors.Annotate(ValidateVariantDef(pr.SupersetOf), "superset_of").Err()
	case nil:
		return unspecified()
	default:
		panic("impossible")
