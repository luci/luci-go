...
// or pb.QueryArtifactsRequest. Messages sent to itemC are *pb.TestResult,
...
// send all of their results to dest.
...
func Query(ctx context.Context, dest chan<- proto.Message, client pb.ResultDBClient, reqs ...proto.Message) error {
	...
	for _, req := range reqs {
		req := req
		eg.Go(func() error {
			return query(ctx, dest, client, req)
		})
	}
	return eg.Wait()
}

// query implements Query for one request.
func query(ctx context.Context, dest chan<- proto.Message, client pb.ResultDBClient, req proto.Message) error {
	// Check req type.
	switch req.(type) {
...
func (q *queryResults) run(ctx context.Context) error {
	// Prepare a channel of responses such that we can make an RPC as soon as we
