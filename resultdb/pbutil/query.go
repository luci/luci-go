...
func ValidateTestResultPredicate(p *pb.TestResultPredicate, requireInvocation bool) error {
	if p.GetInvocation() == nil {
		...
		if p.HonorExonerations && p.Invocation.Inclusions == pb.InvocationPredicate_NONE {
			return errors.Reason("honor_exclusions requires invocation.inclusions to be STABLE or ALL").Err()
		}
	}
	...
	if err := ValidateEnum(int32(p.ExpectedResults), pb.TestResultPredicate_ExpectancyFilter_name); err != nil {
...
func ValidateInvocationPredicate(p *pb.InvocationPredicate) error {
	if err := ValidateEnum(int32(p.Inclusions), pb.InvocationPredicate_Inclusions_name); err != nil {
		return errors.Annotate(err, "inclusions").Err()
	}
...
func ValidateVariantPredicate(p *pb.VariantPredicate) error {
	...
	case *pb.VariantPredicate_SupersetOf:
		return errors.Annotate(ValidateVariant(pr.SupersetOf), "superset_of").Err()
	case nil:
		return unspecified()
	default:
		panic("impossible")
