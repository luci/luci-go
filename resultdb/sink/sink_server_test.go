	...
	Convey("ReportTestResults", t, func() {
		...
		Convey("ReportTestResults", func() {
			...
			Convey("with an inaccesible artifact file", func() {
				...
				Convey("panics if ExitOnFailures is true", func() {
					sink.(*sinkpb.DecoratedSink).Service.(*sinkServer).cfg.ExitOnFailures = true
					So(func() { report(tr) }, ShouldPanic)
				})

				Convey("drops the request with 200 if ExitOnFailures is false", func() {
