...
func TestArtifactChannel(t *testing.T) {
	...
	Convey("schedule", t, func() {
		...
		cfg.ArtifactStreamer.Transport = mockTransport(
...
func TestUploadTask(t *testing.T) {
	...
	Convey("newUploadTask", t, func() {
		...
		Convey("works", func() {
			...
			So(err, ShouldBeNil)
			So(t, ShouldResemble, &uploadTask{art: fArt, name: name, size: int64(len("content"))})
		})
		...
		Convey("fails", func() {
			...
			So(err, ShouldErrLike, "stat "+fArt.GetFilePath())
		})
	})
	...
	Convey("CreateRequest", t, func() {
		...
		Convey("fails", func() {
			...
			So(err, ShouldErrLike, fmt.Sprintf("the size of artifact %q changed", name))
