...
func TestValidateLocationBase(t *testing.T) {
	t.Parallel()
	...
	Convey("empty", t, func() {
		...
		err := config.validateLocationBase()
		So(err, ShouldBeNil)
	})
	...
	Convey("missing leading double slash", t, func() {
		config := ServerConfig{
			LocationBase: "base/",
		}
		err := config.validateLocationBase()
	...
	Convey("missing trailing slash", t, func() {
		config := ServerConfig{
			LocationBase: "//base",
		}
		err := config.validateLocationBase()
