...
func TestNewServer(t *testing.T) {
	...
	Convey("NewServer", t, func() {
		...
		Convey("uses the default max leases, if missing or 0", func() {
			srv, err := NewServer(ctx, cfg)
			So(err, ShouldBeNil)
			So(srv.cfg.ACMaxLeases, ShouldEqual, DefaultACMaxLeases)
			So(srv.cfg.TCMaxLeases, ShouldEqual, DefaultTCMaxLeases)
		})
		Convey("use the custom max leases, if specified", func() {
			cfg.ACMaxLeases, cfg.TCMaxLeases = 123, 456
			srv, err := NewServer(ctx, cfg)
			So(err, ShouldBeNil)
			So(srv.cfg.ACMaxLeases, ShouldEqual, 123)
			So(srv.cfg.TCMaxLeases, ShouldEqual, 456)
