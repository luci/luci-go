...
func Test(t *testing.T) {
	Convey("Test Server", t, func() {
		...
		Convey("readMessage", func() {
			...
			defer cr.Close()
			defer cw.Close()
			cfg := ServerConfig{AuthToken: "hello"}

			Convey("Times out if MaxConnectTimeout is > 0", func() {
				cfg.MaxConnLifetime = 100*time.Millisecond
				server, err := NewServer(ctx, cfg)
				So(err, ShouldBeNil)
				So(server.handleConnection(ctx, cr), ShouldErrLike, context.DeadlineExceeded)
			})

			Convey("Doesn't time out if MaxConnectTimeout is 0", func() {
