...
// NewMockHTTPClient returns a mockHTTPClient that calls a given function in the Do method.
func NewMockHTTPClient(f func(*http.Request) (*http.Response, error)) *mockHTTPClient {
	return &mockHTTPClient{f}
}

func resp200(*http.Request) (*http.Response, error) {
	return &http.Response{
		StatusCode: 200,
		Body:       nopCloser{bytes.NewReader([]byte("success"))},
	}, nil
}
...
func TestNewArtifactRequest(t *testing.T) {
	...
	uploader := NewArtifactUploader(NewMockHTTPClient(resp200), "example.org")
