...
func (rdbc *rdbChannel) init(ctx context.Context, cfg ServerConfig) error {
	...
	ch, err := dispatcher.NewChannel(ctx, recorderDispatcherOpts(), func(b *buffer.Batch) error {
		req := prepareReportTestResultsRequest(ctx, rdbc.cfg.Invocation, b)
