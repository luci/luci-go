...
// each chunk.
//
// Each chunk can have at most 500 artifacts and the total size is capped at maxBatchable.
type uploadTaskSlicer struct {
	batchReq *pb.BatchCreateArtifactsRequest
	pos      int
	tasks    []interface{}

	// maxBatchable is the maximum sum of artifact sizes in a single BatchCreateArtifactsRequest.
	maxBatchable int64
}
