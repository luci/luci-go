...
type ArtifactUploader struct {
	...
	// testDo mocks http.Client.Do.
	testDo (func(*http.Request) (*http.Response, error))
}
...
// NewRequest returns a new upload request for the artifact.
//
// If artifact.Contents is not empty or the artifact file size <= 64k, the body of the
// returned request points to an instance of bytes.Reader with the whole content. Otherwise,
// the body points to an instance of os.File opened for the artifact file.
...
// Upload uploads the contents of the artifact with the HTTP request to ResultDB.
...
// nopCloser is similar to ioutil.nopCloser, but wraps the methods of io.ReadSeeker, instead
// of io.Reader.
