...
func (u *ArtifactUploader) StreamUpload(ctx context.Context, t *uploadTask, updateToken string) error {
	...
	if fp := t.art.GetFilePath(); fp == "" {
		...
		body, err = os.Open(t.art.GetFilePath())
		if err != nil {
			return err
		}
	...
	if err != nil {
		return errors.Annotate(err, "failed to create an HTTP request").Err()
	}

	// Client.Do closes the body on 3xx responses and calls GetBody to a new copy of
	// the body. This simply moves the cursor of the stream to the beginning to avoid
	// file reopenings.
	if fh, ok := body.(*os.File); ok {
		req.Body = ioutil.NopCloser(fh)
		req.GetBody = func() (io.ReadCloser, error) {
			if _, err := body.Seek(0, io.SeekStart); err != nil {
				return nil, err
			}
			return ioutil.NopCloser(fh), nil
		}
		// http.NewRequestWithContext() sets the content length, if body is byte.Reader.
		// hence, we set req.ContentLength only for artifacts w/ files.
		//
		st, err := fh.Stat()
		if err != nil {
			return err
	...
	// the reqeuest body can be re-read by HTTPClient.Do.
	hash, err := calculateHash(body)
	if err != nil {
		return errors.Annotate(err, "calculating artifact-hash").Err()
	}
	if _, err := body.Seek(0, io.SeekStart); err != nil {
		return err
	}
