...
func (u *ArtifactUploader) Upload(ctx context.Context, name, contentType string, input io.ReadSeeker, updateToken string) error {
	...
	if err != nil {
		return errors.Annotate(err, "artifact %q", name).Err()
	}
	if err = u.send(ctx, req); err != nil {
		return errors.Annotate(err, "artifact %q", name).Err()
	}
	return nil
}
...
func (u *ArtifactUploader) UploadFromFile(ctx context.Context, name, contentType, path, updateToken string) error {
	...
	st, err := os.Stat(path)
	if err != nil {
		return errors.Annotate(err, "artifact %q: failed to query the file status", name).Err()
	}
	// small file?
	if st.Size() <= MaxInMemoryFileSize {
		contents, err := ioutil.ReadFile(path)
		if err != nil {
			return errors.Annotate(err, "artifact %q: failed to read file contents", name).Err()
		}
		return u.Upload(ctx, name, contentType, bytes.NewReader(contents), updateToken)
	}

	if input, err = os.Open(path); err != nil {
		return errors.Annotate(err, "artifact %q: failed to open file", name).Err()
	}
	req, err := u.newRequest(ctx, name, contentType, input, updateToken)
	if err != nil {
		return errors.Annotate(err, "artifact %q", name).Err()
	...
	req.GetBody = func() (io.ReadCloser, error) {
		s, err := os.Open(path)
		if err != nil {
			return nil, errors.Annotate(err, "artifact %q: failed to re-open file", name).Err()
		}
		return s, err
	}
...
func (u *ArtifactUploader) send(ctx context.Context, req *http.Request) error {
	return retry.Retry(ctx, transient.Only(retry.Default), func() error {
		...
		return errors.Annotate(hErr.Err(), "a non-transient error response returned").Err()
	}, nil)
}
