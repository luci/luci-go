...
func NewArtifactUploader(c *http.Client, host string) *ArtifactUploader {
	return &ArtifactUploader{Client: c, Host: host, MaxInMemoryFileSize: DefaultMaxInMemoryFileSize}
}
...
func (u *ArtifactUploader) UploadFromFile(ctx context.Context, name, contentType, path, updateToken string) error {
	...
	// small file?
	if st.Size() <= u.MaxInMemoryFileSize {
	...
	// that GetBody can simply move the stream currsor to the beginning without reopening
	// the file object.
