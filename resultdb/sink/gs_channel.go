...
func (gsc *gsChannel) init(ctx context.Context, cfg ServerConfig) error {
	...
	uchOpts := &dispatcher.Options{
		...
		ErrorFn: func(b *buffer.Batch, err error) bool {
			ut := b.Data[0].(*uploadTask)
			info := ut.art.GetFilePath()
			if info == "" {
				info = fmt.Sprintf("Contents with len(%d)", len(ut.art.GetContents()))
			}
			logging.Errorf(ctx, "failed to upload artifact %q (%s): %s", ut.name, info, err)

			// gs_raw.uploadArtifact internally retries requests on transient errors.
			// Therefore, gs_channel just drops a failed batch.
			return false
		},
	}
