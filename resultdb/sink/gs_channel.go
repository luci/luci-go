...
// addUploadBucket adds an upload bucket to gsChannel.
//
// This function MUST be called inside init().
func (gsc *gsChannel) addUploadBucket(ctx context.Context, upperLimit int64, bufOpts buffer.Options) error {
	...
	if len(gsc.buckets) > 0 {
		...
		if ub.upperLimit == upperLimit {
			panic("cannot add an upload bucket with a duplicate upper limit")
		}
	}
