...
func (gsc *gsChannel) init(ctx context.Context, cfg ServerConfig) error {
	...
	go func() {
		...
		parallel.WorkPool(cfg.GSUploadMaxConcurrency, func(workC chan<- func() error) {
			...
			for ut := gsc.pollUploadTask(ctx); ut != nil; ut = gsc.pollUploadTask(ctx) {
				...
				workC <- func() error {
					...
					// capture and log the error here to have upload failure logs with
					// a timestamp close enough to the time of the error occurrence.
					// MultiError from parallel.WorkPool may be too big to print.
					...
					// ignore the error and return nil always so that no errors are returned
					// from parallel.WorkPool. Otherwise, all the errors will be added into
					// to the errors.MultiError and returned all together as a single
					// MutliError when sinkServer terminates. Errors returned from
					// parallel.WorkPool are not handled, anyways.
					return nil
				}
			}
...
func (gsc *gsChannel) pollUploadTask(ctx context.Context) *uploadTask {
	select {
	case <-ctx.Done():
...
func (gsc *gsChannel) upload(ctx context.Context, ut *uploadTask) error {
	...
	if p := ut.art.GetFilePath(); p != "" {
		...
		src = bytes.NewBuffer(ut.art.GetContents())
	}

	var dst io.WriteCloser
	if gsc.testGS == nil {
		dst = ut.obj.NewWriter(ctx)
	} else {
		dst = gsc.testGS.NewWriter(ut.obj)
	}
