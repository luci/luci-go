...
func (gsc *gsChannel) init(ctx context.Context) error {
	// TODO(ddoman): add tsmon metrics to monitor the status of each upload bucket, and
	// tune the options as necessary.
	...
	if err := gsc.addUploadBucket(
		ctx,
		// This bucket is with the largest upper limit, and any artifacts bigger than 256kb
...
func (gsc *gsChannel) searchUploadBucket(size int64) *uploadBucket {
	...
	// use the bucket with the largest upper limit, no bucket has a large enough upper
