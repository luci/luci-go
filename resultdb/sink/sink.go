...
func (s *Server) Close() error {
	if atomic.LoadInt32(&s.started) == 0 || atomic.LoadInt32(&s.stopped) == 1 {
		// hasn't been started OR has stopped already
		return nil
	}
...
func sendErr(c *router.Context, msg string, statusCode int) {
	logging.Errorf(c.Context, "[ResultSink:AuthError] %s", msg)
