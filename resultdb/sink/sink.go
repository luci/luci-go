...
func NewServer(ctx context.Context, cfg ServerConfig) (*Server, error) {
	...
	s.httpSrv = &http.Server{
		Addr:    cfg.Address,
		Handler: http.HandlerFunc(s.routes.ServeHTTP),
	}
	return s, nil
}
...
// send401logs the error message and sends a 401 response with the message.
func send401(c context.Context, rw http.ResponseWriter, msg string) {
...
func authTokenValidator(authToken string) router.Middleware {
	...
	return func(c *router.Context, next router.Handler) {
		...
		if !ok {
			send401(c.Context, c.Writer, "Authorization header is missing")
			return
		}
		...
		send401(c.Context, c.Writer, "no valid auth_token found")
	}
}
