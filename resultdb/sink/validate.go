...
func validateArtifact(n string, ar *sinkpb.Artifact) error {
	...
	// Is it a valid Artifact if both GetFilePath() and GetContext() returns
	// an empty string? Should the content_type be one of the known types?
	// well-known mime types? Or, just arbitrary strings?
	return nil
}
