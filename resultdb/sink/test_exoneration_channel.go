...
type testExonerationChannel struct {
	...
	// 1 indicates that testExonerationChannel started the process of closing and draining
	// the channel. 0, otherwise.
	closed int32
}

func newTestExonerationChannel(ctx context.Context, cfg *ServerConfig) *testExonerationChannel {
	var err error
	c := &testExonerationChannel{cfg: cfg}
...
func (c *testExonerationChannel) closeAndDrain(ctx context.Context) {
...
func (c *testExonerationChannel) schedule(trs ...*sinkpb.TestResult) {
	c.wgActive.Add(1)
...
func (c *testExonerationChannel) report(ctx context.Context, b *buffer.Batch) error {
	if b.Meta == nil {
		...
		for i, d := range b.Data {
			...
			reqs[i] = &pb.CreateTestExonerationRequest{
				TestExoneration: &pb.TestExoneration{
					...
					ExplanationHtml: "Unexpected passes do not cause a build failure",
				},
			}
		}
