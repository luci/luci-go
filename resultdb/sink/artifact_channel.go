...
type artifactChannel struct {
	// batchUploadChannel uploads artifacts via pb.BatchCreateArtifacts().
	//
	// It batches input artifacts and uploads them all at once.
...
func newArtifactChannel(ctx context.Context, cfg *ServerConfig) *artifactChannel {
	...
	stOpts := &dispatcher.Options{
		Buffer: buffer.Options{
			BatchSize:    1, // this MUST be 1, or the processing logic needs to be updated.
			MaxLeases:    int(cfg.ArtChannelMaxLeases),
	...
	c.streamChannel, err = dispatcher.NewChannel(ctx, stOpts, func(b *buffer.Batch) error {
		return c.cfg.ArtifactUploader.StreamUpload(ctx, b.Data[0].(*uploadTask), token)
	})
	if err != nil {
...
func (c *artifactChannel) schedule(tasks []*uploadTask) {
	...
	for _, task := range tasks {
		// TODO(ddoman): send small artifacts to batchUploadChannel
