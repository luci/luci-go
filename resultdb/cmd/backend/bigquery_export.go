...
func batchExportRows(ctx context.Context, up Uploader, maxBatchSize int, resultC chan *bqpb.TestResultRow, errC chan error) {
	for row := range resultC {
		...
		if err := up.Put(ctx, rows...); err != nil {
			// Uploading failed, stops exporting immediately.
			errC <- err
			close(errC)

			<-resultC // Read one more result to unblock resultC.
			return
		}
	}
	close(errC)
...
func exportTestResultsToBigQuery(ctx context.Context, up Uploader, invID span.InvocationID, bqExport *pb.BigQueryExport, maxBatchSize int) error {
	...
	go batchExportRows(ctx, up, maxBatchSize, resultC, errC)
