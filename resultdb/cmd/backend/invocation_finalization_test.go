	...
	Convey(`FinalizeInvocation`, t, func() {
		...
		finalize := func(invID span.InvocationID) {
			_, err := span.Client(ctx).ReadWriteTransaction(ctx, func(ctx context.Context, txn *spanner.ReadWriteTransaction) error {
				return finalizeInvocation(ctx, txn, invID, testclock.TestRecentTimeUTC)
			})
			So(err, ShouldBeNil)
		}
		...
		Convey(`Includes two ACTIVE`, func() {
			...
			`)
			var b span.Buffer
			var nextInvs []span.InvocationID
			err = span.Client(ctx).Single().Query(ctx, st).Do(func(r *spanner.Row) error {
				...
				So(err, ShouldBeNil)
				nextInvs = append(nextInvs, nextInv)
				So(payload, ShouldResembleProtoText, `try_finalize_invocation{}`)
				return nil
			})
			So(err, ShouldBeNil)
			So(nextInvs, ShouldResemble, []span.InvocationID{"finalizing"})
		})
		...
		Convey(`BigQuery tasks`, func() {
			...
			testutil.MustReadRow(ctx, "InvocationTasks", spanner.Key{bqTaskID("inv", 0)}, map[string]interface{}{
				"InvocationID": &invID,
				"ProcessAfter": &processAfter,
			})
			So(invID, ShouldEqual, "inv")
			So(processAfter, ShouldResemble, pbutil.MustTimestampProto(now))
		})
	})
}
