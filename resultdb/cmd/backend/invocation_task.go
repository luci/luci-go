...
var errLeasingFailure = fmt.Errorf("failed to lease a task")

// leaseInvocationTask leases an invocation task if it can.
// If the task could not be leased, returns errLeasingFailure.
...
func dispatchInvocationTasks(ctx context.Context, ids []string) error {
	return parallel.WorkPool(10, func(workC chan<- func() error) {
		for _, taskID := range ids {
			taskID := taskID
			workC <- func() error {
				invID, task, err := leaseInvocationTask(ctx, taskID)
				switch {
				case err == errLeasingFailure:
				...
				if err := exportResultsToBigQuery(ctx, invID, task.BigqueryExport); err != nil {
					if permanentInvocationTaskErrTag.In(err) {
						logging.Errorf(ctx, "permanent failure to process task %s: %s", taskID, err)
						return deleteInvocationTask(ctx, taskID)
					}
					return err
				}
