...
func purgeExpiredResults(ctx context.Context) {
	processingLoop(ctx, 5*time.Second, 5*time.Second, func(ctx context.Context) error {
		...
		if err == nil && len(expiredResultsInvocationIds) > 0 {
			err = dispatchExpiredResultDeletionTasks(ctx, expiredResultsInvocationIds)
		}
		return err
	})
}
...
// random shard that have expired test results.
...
func queryTestVariantsWithUnexpectedResults(ctx context.Context, id span.InvocationID, limit int) ([]testVariantID, error) {
	...
	st.Params["maxTestVariantsToFilter"] = limit
