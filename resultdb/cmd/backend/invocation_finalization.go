...
func finalizeInvocation(ctx context.Context, txn *spanner.ReadWriteTransaction, invID span.InvocationID) error {
	...
	// task.
	//
	// Note that Spanner currently does not support PENDING_COMMIT_TIMESTAMP()
	...
	st := spanner.NewStatement(`
		...
		WHERE IncludedInvocationId = @invID
			...
			-- Do not insert duplicate tasks, otherwise INSERT INTO will fail.
			AND NOT EXISTS(
				SELECT 1
	...
	logging.Infof(ctx, "inserted %d finalizing tasks", count)

	// Read BQExport spec and insert BigQuery export tasks.
	...
	ms := insertBQExportingTasks(invID, clock.Now(ctx), bqExports.BigqueryExports...)
...
func insertBQExportingTasks(invID span.InvocationID, processAfter time.Time, bqExports ...*pb.BigQueryExport) []*spanner.Mutation {
	...
	for i, bqExport := range bqExports {
		taskID := fmt.Sprintf("bq_export:%s:%d", invID, i)
