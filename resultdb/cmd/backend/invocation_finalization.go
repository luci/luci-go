...
func tryFinalizeInvocation(ctx context.Context, invID span.InvocationID) error {
	...
	// "not ready" state.
	switch can, err := canFinalize(ctx, invID); {
	case err != nil:
		return err
	case !can:
		return nil
...
// These are used exclusively inside canFinalize.
var (
...
// canFinalize returns true if the invocation should be finalized.
// An invocation is ready to be finalized if no ACTIVE invocation is reachable
// from it.
func canFinalize(ctx context.Context, invID span.InvocationID) (bool, error) {
...
func finalizeInvocation(ctx context.Context, invID span.InvocationID) error {
	...
	_, err := span.ReadWriteTransaction(ctx, func(ctx context.Context, txn *spanner.ReadWriteTransaction) error {
		...
		// task.
		st := spanner.NewStatement(`
		  INSERT INTO InvocationTasks (InvocationId, TaskId, Payload, ProcessAfter)
			SELECT including.InvocationId, @taskID, @payload, CURRENT_TIMESTAMP()
