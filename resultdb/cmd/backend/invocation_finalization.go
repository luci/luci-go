...
func tryFinalizeInvocation(ctx context.Context, invID span.InvocationID) error {
	_, err := span.ReadWriteTransaction(ctx, func(ctx context.Context, txn *spanner.ReadWriteTransaction) error {
		...
		case !should:
			return nil
		default:
			return finalizeInvocation(ctx, txn, invID, now)
		}
	})
	return err
}
		...
		eg.Go(func() error {
			...
			return txn.Query(ctx, st).Do(func(row *spanner.Row) error {
				...
				case includedState != pb.Invocation_FINALIZING:
					return errors.Reason("%s has unexpected state %s", includedID, includedState).Err()

				default:
					// The included invocation is FINALIZING and MAY include other
					// still-active invocations. We must visit further.
					visit(includedID)
					return nil
				}
			})
...
// a finalization task.
func finalizeInvocation(ctx context.Context, txn *spanner.ReadWriteTransaction, invID span.InvocationID, now time.Time) error {
	// For each FINALIZING invocation that directly includes ours, schedule
	// a task to try to finalize it.
	// Its OK not to schedule a task for active invocations because
	// state transition ACTIVE->FINALIZING includes creating a finalization
	// task.
	st := spanner.NewStatement(`
		INSERT INTO InvocationTasks (TaskId, InvocationId, Payload, ProcessAfter)
		SELECT CONCAT(@taskIDPrefix, including.InvocationId), including.InvocationId, @payload, CURRENT_TIMESTAMP()
		...
		WHERE IncludedInvocationId = @invID
			AND including.State = @finalizing
			-- Do not insert duplicate tasks; INSERT INTO would fail.
			AND NOT EXISTS(
				SELECT 1
				FROM InvocationTasks
				WHERE TaskId = CONCAT(@taskIDPrefix, including.InvocationId)
		  )
	`)
	st.Params = span.ToSpannerMap(map[string]interface{}{
		"taskIDPrefix": fmt.Sprintf("finalize/%s/", invID),
		"invID":        invID,
		"finalizing":   pb.Invocation_FINALIZING,
		"payload": &internalpb.InvocationTask{
			Task: &internalpb.InvocationTask_TryFinalizeInvocation{
				TryFinalizeInvocation: &internalpb.TryFinalizeInvocation{},
			},
		},
	})
	_, err := txn.Update(ctx, st)
	if err != nil {
		return errors.Annotate(err, "failed to insert invocation tasks").Err()
	}

	// Read BQExport spec.
	bqExports := &internalpb.BigQueryExports{}
	err = span.ReadInvocation(ctx, txn, invID, map[string]interface{}{
		"BigQueryExports": &span.CompressedProto{bqExports},
	})
	if err != nil {
		return err
	}

	// Insert BigQuery export tasks.
	ms := insertBQExportingTasks(invID, now, bqExports.BigqueryExports...)

	// Finally update the invocation state.
	ms = append(ms, span.UpdateMap("Invocations", map[string]interface{}{
		"InvocationId": invID,
...
func insertBQExportingTasks(invID span.InvocationID, processAfter time.Time, bqExports ...*pb.BigQueryExport) []*spanner.Mutation {
	ms := make([]*spanner.Mutation, len(bqExports))
	for i, bqExport := range bqExports {
		...
		ms[i] = span.InsertInvocationTask(taskID, invID, task, processAfter)
	}
	return ms
}
