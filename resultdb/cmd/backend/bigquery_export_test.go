...
func TestExportToBigQuery(t *testing.T) {
	Convey(`TestExportTestResultsToBigQuery`, t, func() {
		...
		testutil.MustApply(ctx, testutil.CombineMutations(
			...
			testutil.InsertTestExonerations("a", "A", pbutil.Variant("k1", "v1", "k2", "v2"), 1), // same test variant
			testutil.InsertTestExonerations("a", "B", pbutil.Variant("k1", "v1"), 1),             //different test variant
		)...)
		...
		Convey(`success`, func() {
			...
			for _, m := range i.insertedMessages {
				...
				if testID == "A" && !tr.Result.Expected {
					So(tr.Exoneration.Exonerated, ShouldEqual, true)
				} else {
					So(tr.Exoneration.Exonerated, ShouldEqual, false)
				}
			}
		})
