...
func TestExportToBigQuery(t *testing.T) {
	Convey(`TestExportTestResultsToBigQuery`, t, func() {
		...
		Convey(`success`, func() {
			err := exportTestResultsToBigQuery(ctx, &mockPassInserter{}, "a", bqExport, 2)
			So(err, ShouldBeNil)
			So(len(insertedMessages), ShouldEqual, 5)

			expectedTestIDs := []string{"A", "B", "C"}
			for _, m := range insertedMessages {
				invID, testID, _, _ := pbutil.ParseTestResultName(m.InsertID)
