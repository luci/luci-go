...
type mockPassInserter struct {
	insertedMessages chan []*bq.Row
}

func (i *mockPassInserter) Put(ctx context.Context, src interface{}) error {
	messages := src.([]*bq.Row)
	insertedMessages := make([]*bq.Row, 0, len(messages))
	insertedMessages = append(insertedMessages, messages...)
	i.insertedMessages <- insertedMessages
	return nil
}
...
func TestExportToBigQuery(t *testing.T) {
	Convey(`TestExportTestResultsToBigQuery`, t, func() {
		...
		Convey(`success`, func() {
			i := &mockPassInserter{}
			i.insertedMessages = make(chan []*bq.Row, 3)
			err := exportTestResultsToBigQuery(ctx, i, "a", bqExport, 2)
			So(err, ShouldBeNil)

			close(i.insertedMessages)

			insertedMessages := make([]*bq.Row, 0, 5)
			for messages := range i.insertedMessages {
				insertedMessages = append(insertedMessages, messages...)
			}
			So(insertedMessages, ShouldHaveLength, 5)
