...
type sinkWrapperFlags struct {
	...
	// TODO(ddoman): move theses to sink/flags.go so that the flags become available
...
func newFlags() *sinkWrapperFlags {
	return &sinkWrapperFlags{
		...
		LogConfig: logging.Config{Level: logging.Debug},
	}
}
...
func (f *sinkWrapperFlags) register(fs *flag.FlagSet) {
	...
	fs.Var(luciflag.StringPairs(f.TestBaseVariants), "test-base-variant",
		"Base for variant def specified for test result, of form key:value, "+
	...
	fs.Var(CommaCodeList(&f.CompleteInvocationExitCodes),
		...
		"Comma-separated list of exit codes from the subprocess that mean "+
		"the Invocation should be marked non-interrupted")
	...
	f.LogConfig.AddFlags(fs)
}

func parseFlags() (*sinkWrapperFlags, []string) {
	fs := flag.NewFlagSet("", flag.ExitOnError)
	wrapperFlags := newFlags()
	wrapperFlags.register(fs)

	// this will exit on error with os.Exit(2)
	fs.Parse(os.Args[1:])
	return wrapperFlags, fs.Args()
}
