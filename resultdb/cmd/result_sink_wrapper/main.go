...
func parseArgs() (wrapperArgs, error) {
	args := wrapperArgs{
		...
		testBaseVariant: make(strpair.Map),
	}
	...
	flag.Var(luciflag.StringPairs(args.resultFiles), "result-file",
		"Files to read and upload after running the subprocess, of form format:path, may be set more than once")
	...
	flag.Var(luciflag.StringPairs(args.testBaseVariant), "base-test-variant",
	...
	// TODO(sajjadm): verify that keys in args.testBaseVariant are unique
