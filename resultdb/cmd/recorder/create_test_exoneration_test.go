...
func TestCreateTestExoneration(t *testing.T) {
	Convey(`TestCreateTestExoneration`, t, func() {
		...
		e2eTest := func(withRequestID bool) {
			...
			invID, testPath, exID, err := pbutil.ParseTestExonerationName(res.Name)
			So(err, ShouldBeNil)
			So(invID, ShouldEqual, "inv")
			So(testPath, ShouldEqual, req.TestExoneration.TestVariant.TestPath)
			So(exID, ShouldEqual, res.ExonerationId)
			So(exID, ShouldStartWith, "6408fdc5c36df5df57957e7b230e4b48c55b06ce849303e855d52f1356f2980e:") // hash of the variant

			if withRequestID {
				So(exID, ShouldEqual, "6408fdc5c36df5df57957e7b230e4b48c55b06ce849303e855d52f1356f2980e:d:fcc63e142b2aa8429cde3f4e799f05fa8f179e7c57165658e89305e3f6c647580383f4e48a73ed10a58e1d70246eaad194301f053dd453a7b1a078dc2f2d5ae7")
			}
			...
			row, err := span.ReadTestExonerationFull(ctx, span.Client(ctx).Single(), invID, testPath, exID)
			So(err, ShouldBeNil)
