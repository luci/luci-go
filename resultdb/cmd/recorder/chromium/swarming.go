...
var (
	// Ignore swarming tasks with any of the below names.
	blacklistedTaskNames = stringset.NewFromSlice(
...
func isBlacklisted(task *swarmingAPI.SwarmingRpcsTaskResult) bool {
	var name string
	for _, t := range task.Tags {
		if key, val := strpair.Parse(t); key == "name" {
			name = val
			break
		}
	}

	return blacklistedTaskNames.Has(name)
}
