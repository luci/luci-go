...
func GetSwarmingTask(ctx context.Context, taskID string, swarmSvc *swarmingAPI.Service) (*swarmingAPI.SwarmingRpcsTaskResult, error) {
	...
	ann := errors.Annotate(err, "swarming service")
	if err, ok := err.(*googleapi.Error); ok {
		return nil, annotateErrWithGRPC(ann, err.Code).Err()
	}
	return task, ann.Tag(grpcutil.InternalTag).Err()
}
...
func GetOutputs(ctx context.Context, isoClient *isolatedclient.Client, ref *swarmingAPI.SwarmingRpcsFilesRef) (map[string]isolated.File, error) {
	...
	if err := isoClient.Fetch(ctx, isolated.HexDigest(ref.Isolated), buf); err != nil {
		ann := errors.Annotate(err, "isolate service")
		if status, ok := lhttp.IsHTTPError(err); ok {
			return nil, annotateErrWithGRPC(ann, status).Err()
		}
		return nil, ann.Tag(grpcutil.InternalTag).Err()
	}
...
// annotateErrWithGRPC updates the error annotator with appropriate gRPC tags given the HTTP code.
func annotateErrWithGRPC(ann *errors.Annotator, httpCode int) *errors.Annotator {
	switch {
	case httpCode == http.StatusUnauthorized:
		ann.Tag(grpcutil.UnauthenticatedTag)
	case httpCode == http.StatusForbidden:
		ann.Tag(grpcutil.PermissionDeniedTag)
	case httpCode == http.StatusNotFound:
		ann.Tag(grpcutil.NotFoundTag)
	case httpCode >= 500:
		ann.Tag(transient.Tag, grpcutil.InternalTag)
	}
	return ann
}
