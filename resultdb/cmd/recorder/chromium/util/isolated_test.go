...
func TestIsolatedUtils(t *testing.T) {
	...
	Convey(`Conversion to *pb.Artifact works`, t, func() {
		...
		expectedArt := &pb.Artifact{FetchUrl: "isolate://isosrv/default-zip/400dc0ffee"}

		Convey(`with text type`, func() {
			expectedArt.Name = "a/foo.txt"
			expectedArt.ContentType = "plain/text"
			So(IsolatedFileToArtifact("isosrv", "default-zip", "a/foo.txt", f),
		...
		Convey(`with image type`, func() {
			...
			So(IsolatedFileToArtifact("isosrv", "default-zip", "a/foo.png", f),
				ShouldResembleProto, expectedArt)
		})
		...
		Convey(`with file size`, func() {
			...
			*f.Size = 4096
			...
			So(IsolatedFileToArtifact("isosrv", "default-zip", "", f),
				ShouldResembleProto, expectedArt)
		})
	})
}
