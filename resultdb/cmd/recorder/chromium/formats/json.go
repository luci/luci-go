...
var testRunSubdirRe = regexp.MustCompile("/retry_([0-9][0-9]*)/")
...
// testArtifactsPerRun maps a run index to a map of artifact identifier to slice of
...
// + fail to process and mark them as `unknownArtifacts`
func (f *TestFields) getArtifacts(outputsToProcess map[string]*pb.Artifact) (artifacts testArtifactsPerRun, unknownArtifacts map[string][]string) {
	...
	for name, paths := range f.Artifacts {
		for i, path := range paths {
			...
			if name == "gold_triage_link" {
				// We don't know how to handle more than one gold_triage_link, so note for later processing.
				// TODO(jchinlee): Return as error after ironing out artifact link representations.
				if i > 0 {
					unknownArtifacts[name] = append(unknownArtifacts[name], path)
					continue
				}
