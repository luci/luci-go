...
func TestFinalizeInvocation(t *testing.T) {
	Convey(`TestFinalizeInvocation`, t, func() {
		...
		Convey(`finalize expired invocation failed`, func() {
			testutil.MustApply(ctx,
			...
			inv, err := recorder.FinalizeInvocation(ctx, &pb.FinalizeInvocationRequest{Name: "invocations/inv"})
			So(err, ShouldErrLike, `"invocations/inv" is expired`)
			So(inv, ShouldBeNil)
		})

		Convey(`idempotent`, func() {
			testutil.MustApply(ctx,
				testutil.InsertInvocation("inv", pb.Invocation_COMPLETED, token, ct),
			)
