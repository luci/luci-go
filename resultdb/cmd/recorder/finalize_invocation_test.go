...
func TestFinalizeInvocation(t *testing.T) {
	Convey(`TestFinalizeInvocation`, t, func() {
		...
		Convey(`finalized failed`, func() {
			MustApply(ctx,
				InsertInvocation("inv", pb.Invocation_COMPLETED, token, ct, map[string]interface{}{"Interrupted": true}),
			)
		...
		Convey(`complete expired invocation failed`, func() {
			MustApply(ctx,
				InsertInvocation("inv", pb.Invocation_ACTIVE, token, ct, nil),
			)
		...
		Convey(`finalized`, func() {
			...
			test := func(resetOnFinalize bool, expected *tspb.Timestamp) {
				MustApply(ctx,
					InsertInvocation(invID, pb.Invocation_ACTIVE, token, ct, nil),
