	...
	Convey(`TestFinalizeInvocation`, t, func() {
		...
		Convey(`finalized`, func() {
			...
			testutil.MustApply(ctx,
				...
				insertInvocationTask(invID, 0, invTask1, oriProcessAfter, true),
				// For test purpose only, tasks for BigQuery Export should be reset on finalization.
				insertInvocationTask(invID, 1, invTask2, oriProcessAfter, false),
			)
			inv, err := recorder.FinalizeInvocation(ctx, &pb.FinalizeInvocationRequest{Name: "invocations/inv"})
			So(err, ShouldBeNil)
			So(inv.State, ShouldEqual, pb.Invocation_COMPLETED)
			...
			test := func(taskID int, expProcessAfter *tspb.Timestamp) {
				key := invID.Key(taskID)
