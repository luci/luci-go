...
func TestValidateFinalizeInvocationRequest(t *testing.T) {
	Convey(`TestValidateFinalizeInvocationRequest`, t, func() {
		...
		Convey(`Invalid name`, func() {
			...
			So(err, ShouldErrLike, `name: does not match ^invocations/([a-z][a-z0-9_\-]*)$`)
		})
...
func TestFinalizeInvocation(t *testing.T) {
	Convey(`TestFinalizeInvocation`, t, func() {
		...
		Convey(`idempotent`, func() {
			...
			mut := spanner.InsertMap("Invocations", map[string]interface{}{
				"InvocationId":                      "inv",
				"State":                             int64(pb.Invocation_COMPLETED),
				"Realm":                             "",
				"UpdateToken":                       token,
			...
			inv, err := recorder.FinalizeInvocation(ctx, &pb.FinalizeInvocationRequest{Name: "invocations/inv", RequestId: "request_id"})
			So(err, ShouldBeNil)
