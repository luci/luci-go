...
// Logs an error when derivedInvBQTable is invalid.
func validateDerivedInvBQTableFlag(ctx context.Context) {
	if *derivedInvBQTable == "" {
		logging.Errorf(ctx, "Missing bq table %s.", *derivedInvBQTable)
	}

	if p, d, t := getBqTableInfo(); p == "" || d == "" || t == "" {
		logging.Errorf(ctx, "Invalid bq table %s.", *derivedInvBQTable)
	}
}
...
func getBqTableInfo() (project, dataset, table string) {
	bqTableInfo := strings.Split(*derivedInvBQTable, ".")
...
func (s *recorderServer) DeriveInvocation(ctx context.Context, in *pb.DeriveInvocationRequest) (*pb.Invocation, error) {
	...
	project, dataset, table := getBqTableInfo()
