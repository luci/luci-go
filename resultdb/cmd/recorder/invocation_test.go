	...
	Convey("MayMutateInvocation", t, func() {
		...
		Convey("with token", func() {
			...
			Convey(`with finalized invocation`, func() {
				MustApply(ctx, InsertInvocation("inv", pb.Invocation_COMPLETED, token, ct, nil))
				err := mayMutate()
			...
			Convey(`with active invocation and different token`, func() {
				MustApply(ctx, InsertInvocation("inv", pb.Invocation_ACTIVE, "different token", ct, nil))
				err := mayMutate()
				So(err, ShouldHaveAppStatus, codes.PermissionDenied, `invalid update token`)
			})

			Convey(`with exceeded deadline`, func() {
				MustApply(ctx, InsertInvocation("inv", pb.Invocation_ACTIVE, token, ct, nil))
...
func TestReadInvocation(t *testing.T) {
	Convey(`ReadInvocationFull`, t, func() {
		...
		Convey(`completed`, func() {
			MustApply(ctx, InsertInvocation("inv", pb.Invocation_COMPLETED, "", ct, nil))
			...
			Convey(`with included invocations`, func() {
				MustApply(ctx,
					InsertInvocation("included0", pb.Invocation_COMPLETED, "", ct, nil),
					InsertInvocation("included1", pb.Invocation_COMPLETED, "", ct, nil),
