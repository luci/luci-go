...
func batchTestResults(ctx context.Context, inv *pb.Invocation, trs []*pb.TestResult, batchSize int) []*testResultBatch {
	baseID := span.MustParseInvocationName(inv.Name)
	batches := make([]*testResultBatch, (len(trs)+batchSize-1)/batchSize)
	...
	for i := range batches {
		...
		batches[i] = &testResultBatch{
			BatchInv: &pb.Invocation{
				Name:         span.InvocationID(fmt.Sprintf("%s::batch::%d", baseID, i)).Name(),
				State:        pb.Invocation_COMPLETED,
				CreateTime:   inv.CreateTime,
				FinalizeTime: inv.FinalizeTime,
				Deadline:     inv.Deadline,
			},
			TestResults: trs[i*batchSize : end],
		}
	}
