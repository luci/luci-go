...
func insertBQExportingTasks(invID span.InvocationID, bq_exports []*pb.BigQueryExport, processAfter time.Time) ([]*spanner.Mutation, error) {
	...
	for i, bq_export := range bq_exports {
		...
		var err error
		muts[i], err = insertInvocationTask(invID, invTask, processAfter)
		if err != nil {
			return nil, err
		}
	}
...
func insertInvocationTask(invID span.InvocationID, invTask *internalpb.InvocationTask, processAfter time.Time) (*spanner.Mutation, error) {
