...
func mutateInvocation(ctx context.Context, id span.InvocationID, f func(context.Context, *spanner.ReadWriteTransaction) error) error {
	...
	_, err := span.ReadWriteTransaction(ctx, func(ctx context.Context, txn *spanner.ReadWriteTransaction) error {
		...
		case state != pb.Invocation_ACTIVE:
			return appstatus.Errf(codes.FailedPrecondition, "%s is not active", id.Name())

		case deadline.Before(now):
			retErr = appstatus.Errf(codes.FailedPrecondition, "%s is not active", id.Name())
...
func extractUserUpdateToken(ctx context.Context) (string, error) {
	...
	case len(userToken) == 0:
		return "", appstatus.Errf(codes.Unauthenticated, "missing %q metadata value in the request", updateTokenMetadataKey)

	case len(userToken) > 1:
		return "", appstatus.Errf(codes.InvalidArgument, "expected exactly one %q metadata value, got %d", updateTokenMetadataKey, len(userToken))
...
func validateUserUpdateToken(updateToken spanner.NullString, userToken string) error {
	...
	if userToken != updateToken.StringVal {
		return appstatus.Errf(codes.PermissionDenied, "invalid update token")
	}
