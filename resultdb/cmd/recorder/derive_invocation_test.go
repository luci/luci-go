...
func TestDeriveInvocation(t *testing.T) {
	Convey(`TestDeriveInvocation`, t, func() {
		...
		Convey(`inserts a new invocation`, func() {
			...
			So(trs[0].TestPath, ShouldEqual, "ninja/tests/FooTest.DoesBar")
			...
			So(trs[1].TestPath, ShouldEqual, "ninja/tests/FooTest.TestDoBar")
			So(trs[1].Status, ShouldEqual, pb.TestStatus_CRASH)
			So(trs[2].TestPath, ShouldEqual, "ninja/tests/FooTest.TestDoBar")
