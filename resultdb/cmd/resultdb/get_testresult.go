...
func (s *resultDBServer) GetTestResult(ctx context.Context, in *pb.GetTestResultRequest) (*pb.TestResult, error) {
	...
	switch tr, err := span.ReadTestResult(ctx, txn, in.Name); {
	case spanner.ErrCode(err) == codes.NotFound:
		return nil, errors.Reason("%q not found", in.Name).
			InternalReason("%s", err).
			Tag(grpcutil.NotFoundTag).
			Err()
	case err != nil:
		return nil, errors.Annotate(err, "failed to fetch %q", in.Name).Err()
	default:
		return tr, nil
	}
}
