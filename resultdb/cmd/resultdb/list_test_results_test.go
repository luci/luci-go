...
func TestReadTestResults(t *testing.T) {
	Convey(`ReadTestResults`, t, func() {
		...
		trs := insertTestResults(ctx, "inv", "DoBaz", 0,
			[]pb.TestStatus{pb.TestStatus_PASS, pb.TestStatus_FAIL,
				pb.TestStatus_FAIL, pb.TestStatus_FAIL, pb.TestStatus_PASS,
			})

		Convey(`all results`, func() {
			token := testRead(ctx, "invocations/inv", false, "", 10, trs)
			So(token, ShouldEqual, "")

			Convey(`with pagination`, func() {
				token := testRead(ctx, "invocations/inv", false, "", 1, trs[0:1])
				So(token, ShouldNotEqual, "")

				token = testRead(ctx, "invocations/inv", false, token, 4, trs[1:])
				So(token, ShouldNotEqual, "")

				token = testRead(ctx, "invocations/inv", false, token, 5, nil)
				So(token, ShouldEqual, "")
			})
		})

		Convey(`unexpected results`, func() {
			testRead(ctx, "invocations/inv", true, "", 10, trs[1:4])

			Convey(`with pagination`, func() {
				token := testRead(ctx, "invocations/inv", true, "", 1, trs[1:2])
				So(token, ShouldNotEqual, "")

				token = testRead(ctx, "invocations/inv", true, token, 1, trs[2:3])
				So(token, ShouldNotEqual, "")

				token = testRead(ctx, "invocations/inv", true, token, 10, trs[3:4])
				So(token, ShouldEqual, "")
			})
		})
		...
		Convey(`span test path boundaries`, func() {
			trs = append(trs, insertTestResults(ctx, "inv", "DoQux", 0,
				[]pb.TestStatus{pb.TestStatus_PASS, pb.TestStatus_FAIL})...)
			token := testRead(ctx, "invocations/inv", false, "", 6, trs[:6])
			So(token, ShouldNotEqual, "")

			testRead(ctx, "invocations/inv", false, token, 2, trs[6:])
		})
	})
}
