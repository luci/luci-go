...
func cmdQuery(p Params) *subcommands.Command {
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			r.Flags.StringVar(&r.readMask, "read-mask", "", text.Doc(`
				Fields to include in the response. Fields should be passed as a comma
				separated string to match the JSON encoding schema of FieldMask.
...
func (r *queryRun) queryAndPrint(ctx context.Context, invIDs []string) error {
	...
	if r.readMask != "" {
		if err := protojson.Unmarshal([]byte(fmt.Sprintf(`"%s"`, r.readMask)), mask); err != nil {
			return errors.Annotate(err, "read-mask").Err()
		}
	}
	...
	if r.merge {
		eg.Go(func() error {
			return r.fetchItems(ctx, invIDs, mask, resultItem{}, resultC)
		})
	} else {
		for _, id := range invIDs {
			id := id
			...
			eg.Go(func() error {
				return r.fetchItems(ctx, []string{id}, mask, tmpl, resultC)
			})
		}
	}
...
func (r *queryRun) fetchItems(ctx context.Context, invIDs []string, mask *field_mask.FieldMask, resultItemTemplate resultItem, dest chan<- resultItem) error {
