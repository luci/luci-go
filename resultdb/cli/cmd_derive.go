...
func (r *deriveRun) deriveInvocation(ctx context.Context, taskID string) (*pb.Invocation, error) {
	...
	err := retry.Retry(ctx, newPollingIter, func() error {
		var err error
		inv, err = r.recorder.DeriveInvocation(ctx, req)
		if s, ok := status.FromError(err); ok && s.Code() == codes.FailedPrecondition {
			if hasPreconditionViolationOfType(s.Details(), pb.DeriveInvocationPreconditionFailureType_INCOMPLETE_SWARMING_TASK.String()) {
		...
		if notReady.In(err) {
			...
			logging.Warningf(ctx, "transient error while fetching task %s: %s; will wait for %s", taskID, err, d)
		}
	})
	if err != nil {
		return nil, err
