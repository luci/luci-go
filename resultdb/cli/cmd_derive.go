...
func cmdDerive(p Params) *subcommands.Command {
	return &subcommands.Command{
		...
		LongDesc: text.Doc(`
			Derives Invocation(s) from Chromium Swarming task(s).
			If an invocation already exists for a given task, then reuses it.
			Then prints results, like ls subcommand.

			SWARMING_HOST must be a hostname without a scheme, e.g.
			chromium-swarm.appspot.com.
			The rest of position arguments is one or more task ids.
		`),
		CommandRun: func() subcommands.CommandRun {
			r := &lsRun{}
			r.queryRun.Register(p)
			return r
		},
	}
}
