...
func (r *updateInclusionsRun) registerFlags(p Params) {
	...
	r.Flags.Var(flag.StringSlice(&r.addArg), "add", text.Doc(`
		Name of one invocation to include, use multiple times to
		add multiple invocations.

		E.g.:
		    -add build:1234567 -add build:3456789
	`))
	r.Flags.Var(flag.StringSlice(&r.removeArg), "remove", text.Doc(`
		Name of the invocation to exclude, use multiple times to
		remove multiple invocations.

		E.g.:
		    -remove build:1234567 -remove build:3456789
	`))
}
...
func (r *updateInclusionsRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	if r.addInvocationNames, err = r.convertInvocationIDs(r.addArg); err != nil {
		return r.done(err)
	}

	if r.removeInvocationNames, err = r.convertInvocationIDs(r.removeArg); err != nil {
		return r.done(err)
	}
...
func (r *updateInclusionsRun) updateInclusions(ctx context.Context) error {
	...
	if r.resultdbCtx.CurrentInvocation.Name == "" {
		return errors.Reason("including invocation name missing from LUCI_CONTEXT").Err()
	}
