...
const (
	// StreamExitCodeInvalidInput is the exit code for `stream` failing due to invalid
	// command line input.
	StreamExitCodeInvalidInput = 1001
	// StreamExitCodeInternalError is the exit code for `stream` failing due to internal
	// errors.
	StreamExitCodeInternalError = 1002
)

// streamCommandError is a builder for associating an error from `stream` with an exit code.
	...
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			r.Flags.BoolVar(&r.isNew, "new", false, text.Doc(`
				...
				If false, the current invocation, set in LUCI_CONTEXT, is used.
			`))
			return r
		},
	}
...
func (r *streamRun) Run(a subcommands.Application, args []string, env subcommands.Env) int {
	...
	// if the current invocation is missing in lucictx, create a new one.
...
func (r *streamRun) runTestCmd(ctx context.Context, cmd *exec2.Cmd) (int, error) {
	...
	err := server.Run(ctx, func(ctx context.Context) error {
		...
		err := cmd.Wait()
		logging.Debugf(ctx, "Stopped: %q", cmd.Args)
