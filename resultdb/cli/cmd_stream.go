...
func genInvID(ctx context.Context) (string, error) {
	...
	username = matchSpaces.ReplaceAllString(username, "")
	username = matchInvalidInvocationIDChars.ReplaceAllString(username, "_")

	suffix := strings.ToLower(fmt.Sprintf(
		"%s:%s", time.Now().UTC().Format(time.RFC3339),
		// The padding character,'=', is not a valid invocation ID charset.
	...
	return fmt.Sprintf("u:%.*s:%s", 90-len(suffix), username, suffix), nil
}
