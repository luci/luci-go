	...
	return &subcommands.Command{
		...
		CommandRun: func() subcommands.CommandRun {
			...
			r.Flags.BoolVar(&r.coerceNegativeDuration, "coerce-negative-duration",
				false, text.Doc(`
				If true, ResultSink API will upload a test result without the duration, if the test result was reported with a negative duration.
				If false, ResultSink API will return an error, if a test result was reported with a negative duration.
			`))
			return r
		},
	}
