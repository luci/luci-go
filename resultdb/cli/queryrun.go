...
func (r *queryRun) registerFlags(p Params) {
	...
	`))
	r.Flags.BoolVar(&r.includeExpected, "include-expected", false, text.Doc(`
		Print test results regardless of whether they were expected.

		If false (default), then print only results of test variants that have
		unexpected results.
	...
	r.Flags.Var(luciflag.StringSlice(&r.testPaths), "test-path", text.Doc(`
		Print result of tests with this path.

		If ends with "*", then treated as a prefix, e.g.
...
func (r *queryRun) queryAndPrint(ctx context.Context, inv *pb.InvocationPredicate) error {
	...
	if r.includeExpected {
		req.Predicate.Expectancy = pb.TestResultPredicate_ALL
	}
	for _, p := range r.testPaths {
		if strings.HasSuffix(p, "*") {
			req.Predicate.TestPath.PathPrefixes = append(req.Predicate.TestPath.PathPrefixes, strings.TrimSuffix(p, "*"))
		} else {
			req.Predicate.TestPath.Paths = append(req.Predicate.TestPath.Paths, p)
		}
	}
