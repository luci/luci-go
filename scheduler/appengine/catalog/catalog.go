...
var (
	...
	metricConfiguredJobs = metric.NewInt(
...
func (cat *catalog) GetProjectJobs(c context.Context, projectID string) ([]Definition, error) {
	...
	defer func() {
		logging.Errorf(c, "wtf: %s %s", projectHasConfig, projectIsValid)
		if projectHasConfig {
			logging.Errorf(c, "wtf=> %s", projectIsValid)
			metricConfigs.Set(c, projectIsValid, projectID)
		}
	}()
	...
	metricConfiguredJobs.Set(c, int64(len(out)), projectID, "valid")
	metricConfiguredJobs.Set(c, int64(disabledCount), projectID, "disabled")
	metricConfiguredJobs.Set(c, int64(invalidCount), projectID, "invalid")
	return out, nil
}
