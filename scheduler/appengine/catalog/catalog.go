...
func (cat *catalog) GetProjectJobs(c context.Context, projectID string) ([]Definition, error) {
	...
	defer func() { _ = metricConfigs.Set(c, projectIsValid, projectID) }()
	...
	projectIsValid = true
	_ = metricConfiguredJobs.Set(c, int64(len(out)), projectID, "valid")
	_ = metricConfiguredJobs.Set(c, int64(disabledCount), projectID, "disabled")
	invalidCount := len(cfg.Job) + len(cfg.Trigger) - len(out) - disabledCount
	_ = metricConfiguredJobs.Set(c, int64(invalidCount), projectID, "invalid")
	return out, nil
}
