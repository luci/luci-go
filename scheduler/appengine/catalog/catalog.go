...
func (cat *catalog) GetProjectJobs(c context.Context, projectID string) ([]Definition, error) {
	...
	// Triggering jobs.
	allJobIDs := cat.getAllJobIDs(&cfg)
...
// reporter as a validation error. Otherwise it is logged as a warning, and the
...
//
// Doesn't filter out disable jobs. IDs don't include project prefixes, e.g.
// they are just "job" instead of "project/job".
func (cat *catalog) getAllJobIDs(cfg *messages.ProjectConfig) stringset.Set {
