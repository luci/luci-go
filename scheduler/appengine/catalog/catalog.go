...
type Definition struct {
	...
	// TriggeredJobNames is a list of job names (not IDs) which this job triggers.
	// It's set only for triggering jobs.
	TriggeredJobNames []string
}
...
func (cat *catalog) GetProjectJobs(c context.Context, projectID string) ([]Definition, error) {
	...
	for _, trigger := range cfg.Trigger {
		...
		out = append(out, Definition{
			...
			Schedule:          schedule,
			Task:              packed,
			TriggeredJobNames: cat.normalizeTriggeredJobNames(trigger),
		})
	}
...
// normalizeTriggeredJobNames returns sorted list without duplicates.
func (cat *catalog) normalizeTriggeredJobNames(t *messages.Trigger) []string {
