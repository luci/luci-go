...
func getConfiguredProjectValue(ctx context.Context, tsmonMemStore store.Store, project string) bool {
	metricValue, err := tsmonMemStore.Get(ctx, metricConfigValid, time.Time{}, []interface{}{project})
	if err != nil {
		panic(err)
	}
...
func getConfiguredJobsValue(ctx context.Context, tsmonMemStore store.Store, project, status string) int64 {
	metricValue, err := tsmonMemStore.Get(ctx, metricConfigJobs, time.Time{}, []interface{}{project, status})
	if err != nil {
		panic(err)
	}
