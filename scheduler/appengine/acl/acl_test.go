...
func TestACLsValidation(t *testing.T) {
	...
	Convey("grant validation", t, func() {
		...
		So(call(&messages.Acl{Role: 3}).Error(), ShouldResemble, "in <unspecified file> (acls): invalid role \"3\"")
		So(call(&messages.Acl{Role: messages.Acl_OWNER, GrantedTo: ""}).Error(),
			ShouldResemble, "in <unspecified file> (acls): missing granted_to for role OWNER")
		So(call(&messages.Acl{Role: messages.Acl_OWNER, GrantedTo: "group:"}).Error(),
			ShouldResemble, "in <unspecified file> (acls): invalid granted_to \"group:\" for role OWNER: needs a group name")
		So(call(&messages.Acl{Role: messages.Acl_OWNER, GrantedTo: "not-email"}).Error(),
			ShouldStartWith, "in <unspecified file> (acls): invalid granted_to \"not-email\" for role OWNER: ")
		So(call(&messages.Acl{Role: messages.Acl_OWNER, GrantedTo: "bot:"}).Error(),
			ShouldStartWith, "in <unspecified file> (acls): invalid granted_to \"bot:\" for role OWNER: ")
	})
	...
	Convey("Task Acls", t, func() {
		Convey("READER and OWNER ACLs are required", func() {
			...
			Convey("No OWNER acl set", func() {
				...
				So(ctx.Finalize().Error(), ShouldResemble,
					"in <unspecified file>: Job or Trigger must have OWNER acl set")
			})
			Convey("No READER acl set", func() {
				ValidateTaskACLs(ctx, nil, []string{},
					[]*messages.Acl{{Role: messages.Acl_OWNER, GrantedTo: "group:owners"}})
				So(ctx.Finalize().Error(), ShouldResemble,
					"in <unspecified file>: Job or Trigger must have READER acl set")
			})
		})
		...
		Convey("Many ACLs", func() {
			...
			Convey("1 too many", func() {
				...
				So(ctx.Finalize().Error(), ShouldResemble,
					"in <unspecified file>: Job or Trigger can have at most 32 acls, but 33 given")
			})
		})
		...
		Convey("Bad acl_set reference in a task definition", func() {
			...
			So(valCtx.Finalize().Error(), ShouldStartWith,
				"in <unspecified file> (acl_sets): referencing AclSet 'typo' which doesn't exist")
		})
