...
// as transient).
//
// See runTxn for more explanation. This is use primarily by errUpdateConflict.
var abortTransaction = errors.BoolTag{Key: errors.NewTagKey("this error aborts the transaction")}

// runTxn runs a datastore transaction retrying the body on subset of transient
// errors or when encountering a commit conflict.
//
...
func runTxn(c context.Context, cb func(context.Context) error) error {
	...
	err := datastore.RunInTransaction(c, func(c context.Context) error {
		...
		if attempt != 1 {
			if innerErr != nil {
				logging.Warningf(c, "Retrying the transaction after error: %s", innerErr)
			} else {
				logging.Warningf(c, "Retrying the transaction: failed to commit")
			}
		}
		switch innerErr = cb(c); {
		case transient.Tag.In(innerErr) && !abortTransaction.In(innerErr):
			return datastore.ErrConcurrentTransaction // causes a retry
		case innerErr == nil:
			return nil
		default:
			return innerErr // causes an abort
		}
