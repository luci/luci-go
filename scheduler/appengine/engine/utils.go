...
func triggerToProto(t task.Trigger) *internal.EngineTrigger {
	return &internal.EngineTrigger{
		Id:           t.ID,
		JobId:        t.JobID,
...
// TriggerFromProto returns Trigger from a given EngineTrigger proto struct.
//
// It's the reverse of Trigger.ToProto().
func triggerFromProto(t *internal.EngineTrigger) *task.Trigger {
	return &task.Trigger{
		ID:           t.Id,
		JobID:        t.JobId,
		InvocationID: t.InvocationId,
...
func triggersToProto(triggers []task.Trigger) []*internal.EngineTrigger {
	out := make([]*internal.EngineTrigger, 0, len(triggers))
...
func triggersFromProto(triggers []*internal.EngineTrigger) []task.Trigger {
