...
type EngineInternal interface {
	...
	// GetInternalJobState is used by Admin RPC interface for debugging jobs.
	//
	// It fetches Job entity, pending triggers and pending completion
	// notifications.
	GetInternalJobState(c context.Context, jobID string) (*InternalJobState, error)
}

// InternalJobState contains detailed information about a job.
//
...
//
// Part of the internal interface, doesn't check ACLs.
func (e *engineImpl) GetInternalJobState(c context.Context, jobID string) (*InternalJobState, error) {
	job, err := e.getJob(c, jobID)
	switch {
	case err != nil:
	...
	state := &InternalJobState{Job: job}
