...
func (e *engineImpl) ListInvocations(c context.Context, job *Job, opts ListInvocationsOpts) ([]*Invocation, string, error) {
	...
	lastPage := invsPage{lastScanned: cursor.LastScanned}
	...
	for opts.PageSize > 0 && !lastPage.final {
		var err error
		out, lastPage, err = fetchInvsPage(c, job, opts, lastPage.lastScanned, out)
		if err != nil {
			return nil, "", err
		}
		opts.PageSize -= lastPage.count
	}
	...
	cursorStr, err := encodeInvCursor(&internal.InvocationsCursor{
		LastScanned: lastPage.lastScanned,
	})
	if err != nil {
