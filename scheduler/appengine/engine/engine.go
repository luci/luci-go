...
type Engine interface {
	...
	//
	// For cron jobs it replaces job's schedule with "triggered", effectively
	// preventing it from running automatically (until it is resumed).
	//
	// For triggered jobs, it clears the pending triggers queue, and makes the job
	// ignore all incoming triggers until it is resumed.
	//
...
func (e *engineImpl) execEnqueueTriggersTask(c context.Context, tqTask proto.Message) error {
	...
	if job == nil || !job.Enabled || job.Paused {
		logTriggers("Discarding following triggers since the job is inactive")
		return nil
	}

	logTriggers("Adding following triggers to pending triggers set")
