...
// batchInvocationLaunch allocated a bunch of Invocation entities, adds them to
// ActiveInvocations list of the job and prepares a tq task that kicks off their
// execution.
//
...
//
// Callers are expected to actually enqueue the task inside the transaction. We
// don't do it right away to give callers opportunity to batch multiple tq tasks
// together, if necessary.
func (e *engineImpl) batchInvocationLaunch(c context.Context, job *Job, req []InvocationRequest) ([]*Invocation, *tq.Task, error) {
	assertInTransaction(c)

	// Since each invocation is in its own entity group, we can allocate them
	// in parallel.
	wg := sync.WaitGroup{}
	wg.Add(len(req))
	...
	for i := range req {
		go func(i int) {
			...
			merr.Assign(i, err)
			if err != nil {
				logging.WithError(err).Errorf(c, "Failed to create invocation")
			}
		}(i)
	}
	...
	if err := merr.Get(); err != nil {
		e.cleanupInvocationsAfterFail(c, invs)
	...
	// Prepare a task that eventually calls 'launchInvocationTask' for each new
	// invocation.
	payload := &internal.LaunchInvocationsBatchTask{
		Tasks: make([]*internal.LaunchInvocationTask, 0, len(invs)),
	}
	for _, inv := range invs {
	...
	return invs, task, nil
}

// cleanupInvocationsAfterFail tries to delete given invocations.
//
// It is best effort. It logs errors, but doesn't return them, to indicate that
// there's nothing we can actually do.
//
// 'invs' is allowed to have nils, they are skipped.
func (e *engineImpl) cleanupInvocationsAfterFail(c context.Context, invs []*Invocation) {
	var keysToKill []*ds.Key
	for _, inv := range invs {
		if inv != nil {
			logging.Warningf(c, "Cleaning up inv %d", inv.ID)
			keysToKill = append(keysToKill, ds.KeyForObj(c, inv))
		}
	}
	if len(keysToKill) != 0 {
		if err := ds.Delete(ds.WithoutTransaction(c), keysToKill); err != nil {
...
func (e *engineImpl) allocateInvocation(c context.Context, job *Job, req InvocationRequest) (*Invocation, error) {
	...
	err := runIsolatedTxn(c, func(c context.Context) (err error) {
		...
		if req.DebugLogLine != "" {
			inv.debugLog(c, "%s", req.DebugLogLine)
		}
