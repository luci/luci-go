...
func (e *engineImpl) GetAllJobs(c context.Context) ([]*Job, error) {
	q := datastore.NewQuery("Job").Eq("Enabled", true)
	return e.queryEnabledJobs(c, q)
}

func (e *engineImpl) GetProjectJobs(c context.Context, projectID string) ([]*Job, error) {
	q := datastore.NewQuery("Job").Eq("Enabled", true).Eq("ProjectID", projectID)
	return e.queryEnabledJobs(c, q)
}

func (e *engineImpl) queryEnabledJobs(c context.Context, q *datastore.Query) ([]*Job, error) {
	ds := datastore.Get(c)
	entities := []*Job{}
	if err := ds.GetAll(q, &entities); err != nil {
		return nil, errors.WrapTransient(err)
...
func (e *engineImpl) ListInvocations(c context.Context, jobID string, pageSize int, cursor string) ([]*Invocation, string, error) {
	...
	q := datastore.NewQuery("Invocation").
		Ancestor(ds.NewKey("Job", jobID, 0, nil)).
		Order("__key__")
...
func (e *engineImpl) GetInvocation(c context.Context, jobID string, invID int64) (*Invocation, error) {
	...
	inv := &Invocation{
		ID:     invID,
		JobKey: ds.NewKey("Job", jobID, 0, nil),
	}
