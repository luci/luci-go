...
type Engine interface {
	...
	// ListInvocations returns invocations of a given job.
	//
	// Returns both historical and active invocations, sorted by their creation
	// time (most recent first). If opts.FinishedOnly is true, will skip active
	// invocations.
	//
	// If you need only active invocations, use GetActiveInvocations which is
	// faster (but doesn't support pagination currently).
	//
	...
	// GetActiveInvocations returns active invocations of a given job.
	//
	// Active invocations are the ones that are pending, starting or running.
	//
	// Returns all active invocations at once, sorted by their creation time
	// (most recent first). Returns only transient errors.
	GetActiveInvocations(c context.Context, job *Job) ([]*Invocation, error)
...
func (e *engineImpl) ListInvocations(c context.Context, job *Job, opts ListInvocationsOpts) ([]*Invocation, string, error) {
	// TODO(vadimsh): Implement FinishedOnly.
...
// ListInvocations returns active invocations of a given job, most recent first.
//
// Part of the public interface.
//
// Supports both v1 and v2 invocations.
func (e *engineImpl) GetActiveInvocations(c context.Context, job *Job) ([]*Invocation, error) {
