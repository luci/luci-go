...
func (e *engineImpl) invocationFinishedTask(c context.Context, tqTask proto.Message) error {
	...
	// the whole thing if any of them fail. Everything that happens in this
	// handler is idempotent (including recentlyFinishedSet modification above).
...
func (e *engineImpl) triageJobStateTask(c context.Context, tqTask proto.Message) error {
	...
	op := triageOp{
		...
		enqueueInvocations: e.enqueueInvocations,
	}
...
// triggeringPolicy decides how to convert a set of pending trigger into a bunch
// of new invocations.
//
// Called within a job transaction. Must not do any expensive calls.
func (e *engineImpl) triggeringPolicy(c context.Context, job *Job, triggers []task.Trigger) ([]InvocationRequest, error) {
