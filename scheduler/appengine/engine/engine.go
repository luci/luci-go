...
func (e *engineImpl) AbortJob(c context.Context, job *Job) error {
	...
	err := e.jobTxn(c, jobID, func(c context.Context, job *Job, isNew bool) (err error) {
		...
		invs, err = e.jobController(job).onJobAbort(c, job)
		return err
	})
	if err != nil {
		return err
...
func (e *engineImpl) ForceInvocation(c context.Context, job *Job) (FutureInvocation, error) {
	...
	err := e.jobTxn(c, job.JobID, func(c context.Context, job *Job, isNew bool) (err error) {
		...
		future, err = e.jobController(job).onJobForceInvocation(c, job)
		return err
	})
...
//
// This is expensive call that hits memcache/datastore. If you have *Job or
// *Invocation already, use their IsV2() methods instead.
//
// Return a fatal error is the job is not found or transient error on all other
// datastore/memcache errors.
...
func (e *engineImpl) jobController(job *Job) jobController {
	if job.IsV2() {
...
// jobControllerForInv returns an appropriate implementation of the
// jobController depending on a version of the engine the invocation is using
// (v1 or v2).
func (e *engineImpl) jobControllerFromInv(inv *Invocation) jobController {
	if inv.IsV2() {
		return &jobControllerV2{eng: e}
	}
	return &jobControllerV1{eng: e}
...
func (e *engineImpl) setJobPausedFlag(c context.Context, job *Job, paused bool, who identity.Identity) error {
	return e.jobTxn(c, job.JobID, func(c context.Context, job *Job, isNew bool) error {
		...
		job.Paused = paused
		return e.jobController(job).onJobScheduleChange(c, job)
	})
}
...
func (e *engineImpl) disableJob(c context.Context, jobID string) error {
	return e.jobTxn(c, jobID, func(c context.Context, job *Job, isNew bool) error {
		...
		job.Enabled = false
		return e.jobController(job).onJobDisabled(c, job)
	})
}
...
func (e *engineImpl) execLaunchInvocationTask(c context.Context, tqTask proto.Message) error {
	...
	err = runTxn(c, func(c context.Context) error {
		...
		// StatusFailed state.
		jobCtl := e.jobControllerFromInv(&inv)
		if err := jobCtl.onInvUpdating(c, &inv, &lastInvState, nil, nil); err != nil {
			return err
		}
...
func (e *engineImpl) execCronTickTask(c context.Context, tqTask proto.Message) error {
	...
	return e.jobTxn(c, msg.JobId, func(c context.Context, job *Job, isNew bool) error {
		...
		return e.jobController(job).onJobCronTick(c, job, msg)
	})
}
