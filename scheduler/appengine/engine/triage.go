...
type triageOp struct {
	...
	// triggeringPolicy decides how to convert a set of pending trigger into
	// a bunch of new invocations.
	triggeringPolicy func(c context.Context, job *Job, triggers []task.Trigger) ([]InvocationRequest, error)
...
func (op *triageOp) processTriggers(c context.Context, job *Job) (*dsset.PopOp, error) {
	...
	triggers := make([]task.Trigger, 0, len(op.triggersList.Items))
	for _, item := range op.triggersList.Items {
		...
		triggers = append(triggers, *triggerFromProto(&trigger))
	}
	...
	for _, t := range triggers {
		logging.Infof(c, "  %q submitted %s ago by %q inv %d", t.ID, now.Sub(t.Created), t.JobID, t.InvocationID)
	}
