...
func TestFullTriggeredFlow(t *testing.T) {
	Convey("full triggered flow", t, func() {
		...
		mgr.launchTask = func(ctx context.Context, ctl task.Controller, _ []task.Trigger) error {
			...
			// Trigger something.
			ctl.EmitTrigger(ctx, "2-triggered", task.Trigger{ID: "trg2", Payload: []byte("note the trigger id")})
			So(ctl.Save(ctx), ShouldBeNil)

			// Trigger more stuff.
			ctl.EmitTrigger(ctx, "2-triggered", task.Trigger{ID: "trg2", Payload: []byte("see, same trigger id")})
			ctl.EmitTrigger(ctx, "3-triggered", task.Trigger{ID: "trg3-1st", Payload: []byte("3/1st")})
			ctl.EmitTrigger(ctx, "3-triggered", task.Trigger{ID: "trg3-2nd", Payload: []byte("3/2nd")})
			ctl.EmitTrigger(ctx, "4-triggered", task.Trigger{ID: "trg4", Payload: []byte("payload 4")})
			ctl.EmitTrigger(ctx, "4-triggered", task.Trigger{ID: "trg4", Payload: []byte("dup with another payload")})
			// Job 5 doesn't exist, but it's totally fine.
			ctl.EmitTrigger(ctx, "5-triggered", task.Trigger{ID: "trg5", Payload: []byte("payload 5")})
		...
		So(inv.DebugLog, ShouldContainSubstring, "[22:42:05.000] Emitting a trigger trg2 for job 2-triggered") // twice.
		So(inv.DebugLog, ShouldContainSubstring, "[22:42:05.000] Emitting a trigger trg3-1st for job 3-triggered")
		So(inv.DebugLog, ShouldContainSubstring, "[22:42:05.000] Emitting a trigger trg3-2nd for job 3-triggered")
		So(inv.DebugLog, ShouldContainSubstring, "[22:42:05.000] Emitting a trigger trg4 for job 4-triggered") // twice.
		So(inv.DebugLog, ShouldContainSubstring, "[22:42:05.000] Emitting a trigger trg5 for job 5-triggered")
		...
		So(deliveredTriggers, ShouldResemble, map[string][]string{
			"abc/2-triggered": {"trg2"},
			"abc/3-triggered": {"trg3-1st", "trg3-2nd"},
			"abc/4-triggered": {"trg4"},
		})
	})
}
