...
func TestFullTriggeredFlow(t *testing.T) {
	Convey("full triggered flow", t, func() {
		...
		jobsDefinitions := []catalog.Definition{
			{
				...
				TriggeredJobNames: []string{"2-triggered", "3-triggered"},
			},
		}
		...
		So(job1.TriggeredJobNames, ShouldResemble, []string{"2-triggered", "3-triggered"})
		...
		mgr.launchTask = func(ctx context.Context, ctl task.Controller) error {
			...
			So(err, ShouldBeNil)
			So(inv.TriggeredJobNames, ShouldResemble, []string{"2-triggered", "3-triggered"})
