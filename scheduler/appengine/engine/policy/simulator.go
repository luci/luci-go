...
func (s *Simulator) AddTrigger(delay time.Duration, t ...internal.Trigger) {
	...
	for _, tr := range t {
		...
		tr.Created = ts
		if !s.seenTriggers.Has(tr.Id) {
			s.seenTriggers.Add(tr.Id)
...
func (s *Simulator) triage() {
	// Collect the unordered list of currently running invocation.
...
func (s *Simulator) AdvanceTime(d time.Duration) {
	...
	for {
		// Nothing is happening at all or events happens later than we wish to go?
