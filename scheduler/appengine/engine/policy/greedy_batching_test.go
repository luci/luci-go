...
func TestGreedyBatching(t *testing.T) {
	...
	Convey("Works", t, func(c C) {
		// A policy that allows two concurrent invocations.
		policy, err := GreedyBatchingPolicy(2)
		So(err, ShouldBeNil)
		...
		// Some time later, when the previous invocation is still running, submit
		...
		// Some time later, when the previous two invocations are still running,
