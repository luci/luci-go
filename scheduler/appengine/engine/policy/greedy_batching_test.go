...
func TestGreedyBatching(t *testing.T) {
	...
	ftt.Run("With simulator", t, func(c *ftt.Test) {
		...
		c.Run("Unlimited batching works", func(c *ftt.Test) {
			...
			assert.Loosely(c, s.Last().Request.TriggerIDs(), should.Resemble([]string{"t1", "t2", "t3"}))
			...
			assert.Loosely(c, s.Last().Request.TriggerIDs(), should.Resemble([]string{"t4", "t5"}))
			...
			assert.Loosely(c, s.Last().Request.TriggerIDs(), should.Resemble([]string{"t6", "t7"}))
		...
		c.Run("Limited batching works", func(c *ftt.Test) {
			...
			assert.Loosely(c, s.PendingTriggers, should.HaveLength(0))
			assert.Loosely(c, s.Invocations, should.HaveLength(2))
			assert.Loosely(c, s.Invocations[0].Request.TriggerIDs(), should.Resemble([]string{"t1", "t2", "t3"}))
			assert.Loosely(c, s.Invocations[1].Request.TriggerIDs(), should.Resemble([]string{"t4"}))
			...
			assert.Loosely(c, s.Invocations[2].Request.TriggerIDs(), should.Resemble([]string{"t5", "t6", "t7"}))
			assert.Loosely(c, s.Invocations[3].Request.TriggerIDs(), should.Resemble([]string{"t8"}))
