...
// basePolicy returns a generic policy customizable by the `reduce` func. Parameters
// `maxConcurrentInvs` and `maxBatchSize` control how many invocations can be running in parallel
// and maximum number of triggers that can be collapsed into a single invocation correspondingly.
// The `reducer` func allows to customize policy: it takes a list of outstanding triggers and must
// return a number of triggers to be collapsed. The returned number must be >= 0. The created
// invocation will collapse the specified number of triggers and take the properties of the latest
// one. If the returned number is 0, no invocation is created. If it is larger than `maxBatchSize`,
// the value is ignored and invocation collapsing `maxBatchSize` triggers is created.
