...
func LogarithmicBatchingPolicy(maxConcurrentInvs, maxBatchSize int, logBase float64) (Func, error) {
	// We use 1.0001 to ensure that operation below returns a value small enough to fit into int
	// and to avoid numerical stability issues resulting from very small values being
	// approximated as 0 and resulting in division-by-zero errors.
	if logBase < 1.0001 {
