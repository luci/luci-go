...
//
// Finally we do want to retry the entire two-part transaction. To indicate this
// the error is tagged with tq.Retry, it signals to the task queue layer (and
// to the pubsub) that the task should be redelivered later.
var errUpdateConflict = errors.New(
	"concurrent modifications of same Invocation",
	transient.Tag,
	abortTransaction,
	tq.Retry,
)
