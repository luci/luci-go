...
// consumeTimer removes given timer from invocation, if it is there.
//
// Returns true if it was there, false if it wasn't, or error on deserialization
// errors.
//
// We can't modify 'saved' in place, since it is supposed to always match
// what's is (or has been) in the datastore. So delay the entity modification
// until Save() call.
