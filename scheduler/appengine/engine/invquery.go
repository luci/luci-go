...
//
// Places the result into 'out' slice, returning the extended slice in the end.
//
// Return (..., true, nil) in case results of all queries has been exhausted.
...
func fetchInvsPage(c context.Context, job *Job, opts ListInvocationsOpts, lastScanned int64, out []*Invocation) ([]*Invocation, invsPage, error) {
	...
	// IDs list. Note that this may reduce the requested page size, in
	// pathological case to 0. 'ListInvocations' will compensate for that by
