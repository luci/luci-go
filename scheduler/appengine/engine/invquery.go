...
// invsPage is returned by fetchInvsPage.
type invsPage struct {
	invs        []*Invocation // list of invocations in the page
...
// listing.
//
// It is called (perhaps multiple times) by public ListInvocations to grab a
// full page of results.
//
// Returns (invs, false, nil) if there's potentially more results available or
// (invs, true, nil) if it was the last page of the results.
func fetchInvsPage(c context.Context, job *Job, opts ListInvocationsOpts, lastScanned int64) (*invsPage, error) {
	...
	out, final, err := mergeInvQueries(qs, opts.PageSize)
	if err != nil {
	...
	// at to resume our query from it. It is important to grab the ID
	// pre-filtering, otherwise we may end up stuck in an infinite loop that
	// fetches an empty page over and over again, not advancing the query.
	if len(out) == 0 {
		return &invsPage{nil, true, lastScanned}, nil
	}
	lastScanned = out[len(out)-1].ID

	// Inflate and filter shallow entities resulted from queries over IDs list.
	// Note that this may reduce the requested page size, in pathological case to
	// 0. 'ListInvocations' will compensate for that by calling 'fetchInvsPage'
	// again to fetch more stuff until the full page is fetched.
	if out, err = fillShallowInvs(c, out, opts); err != nil {
		return nil, err
	}

	return &invsPage{out, final, lastScanned}, nil
}
...
//
// May return less results than 'invs' due to filtering.
func fillShallowInvs(c context.Context, invs []*Invocation, opts ListInvocationsOpts) ([]*Invocation, error) {
	...
	filtered := make([]*Invocation, 0, len(invs))
	for _, inv := range invs {
