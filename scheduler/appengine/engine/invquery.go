...
// prepareInvsQueries returns a list of queries to use as source of invocation
// listings.
//
...
func prepareInvsQueries(c context.Context, job *Job, opts ListInvocationsOpts, lastScanned int64) []invQuery {
	var qs []invQuery
	...
	if !opts.ActiveOnly {
		...
		// And always use recently finished invocations from the Job, since they
		// may be more up-to-date and do not depend on Datastore index consistency
		// lag.
