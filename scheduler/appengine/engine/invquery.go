...
// Smallest IDs are returned first. IDs smaller or equal than lastReturned are
// skipped (this is used for pagination).
func finishedInvQuery(c context.Context, q *datastore.Query, parent *datastore.Key, lastReturned int64) *invDatastoreQuery {
	if lastReturned > 0 {
...
func fetchInvsPage(c context.Context, job *Job, opts ListInvocationsOpts, lastReturned int64) ([]*Invocation, bool, error) {
	...
	if !opts.ActiveOnly {
		...
		finishedQuery := finishedInvQuery(c, q, parent, lastReturned)
		...
		qs = append(qs, recentInvQuery(c, job, lastReturned))
	}

	// Prepare the query over active invocations.
	if !opts.FinishedOnly {
		qs = append(qs, activeInvQuery(c, job, lastReturned))
	}

	// Merge the results of all queries.
	out, all, err := mergeInvQueries(qs, opts.PageSize)
	if err != nil {
		return nil, false, errors.Annotate(err, "failed to query invocations").Tag(transient.Tag).Err()
	}
