...
type Trigger struct {
	...
	// external event. If not provided, the engine will set it to the time of the
	// first EmitTrigger call done by the invocation, i.e. all triggers emitted
	// by an invocation have same creation timestamp by default.
	//
	...
	// through a single RPC or by a single invocation), a trigger with smaller
	// 'order_in_batch' is considered to be older. Value of 'order_in_batch' for
	// triggers with different 'created' timestamps are not comparable.
	//
	// Should be set by whoever emits the trigger if 'created' timestamp was
	// supplied explicitly. Will be set by the engine based on the order of
	// EmitTrigger calls done by the invocation otherwise.
	//
	// Together with 'order_in_batch' used for weak ordering of triggers that
	// aren't directly comparable (e.g. git commits from different repositories).
