...
func TestLoadSaveCompression(t *testing.T) {
	...
	Convey("Compress lots of similar refs", t, func() {
		...
		So(getSentDistrValue(c, metricTaskGitilesStoredRefs, jobID), ShouldAlmostEqual, float64(many16Ki), 1)
		So(getSentDistrValue(c, metricTaskGitilesStoredSize, jobID), ShouldAlmostEqual, len(stored.CompressedState), 1)
	})
}
