...
func TestTriggerBuild(t *testing.T) {
	...
	Convey("LaunchTask Triggers Jobs", t, func() {
		...
		cfg := &messages.GitilesTask{
			...
			JobsToTrigger: []string{"j-buildbucket"},
		}
		...
		Convey("new refs are discovered", func() {
			...
			So(ctl.Triggers[0].TriggeredJobName, ShouldEqual, "j-buildbucket")
			So(ctl.Triggers[0].Trigger.ID, ShouldEqual,
				"https://a.googlesource.com/b.git/+/refs/heads/master@deadbeef0")
			p := internal.TriggerPayload{}
			So(proto.Unmarshal(ctl.Triggers[0].Trigger.Payload, &p), ShouldBeNil)
		...
		Convey("Updated, not changed and deleted refs", func() {
			...
			So(ctl.Triggers[0].Trigger.ID, ShouldEqual,
				"https://a.googlesource.com/b.git/+/refs/branch-heads/1.2.3@baadcafe0")
			So(ctl.Triggers[1].Trigger.ID, ShouldEqual,
				"https://a.googlesource.com/b.git/+/refs/heads/master@deadbeef1")
		})

		Convey("triggers all jobs in config", func() {
			gitilesMock.refs = func(ctx context.Context, repo string, path string) (map[string]string, error) {
				return map[string]string{"refs/heads/master": "deadbeef0"}, nil
			}
			cfg.JobsToTrigger = []string{"one", "two"}
			So(m.LaunchTask(c, ctl, nil), ShouldBeNil)
			So(ctl.Triggers, ShouldHaveLength, 2)
			So(ctl.Triggers[0].TriggeredJobName, ShouldEqual, "one")
			So(ctl.Triggers[1].TriggeredJobName, ShouldEqual, "two")
			So(ctl.Triggers[0].Trigger, ShouldResemble, ctl.Triggers[1].Trigger)
		})
	})
}
