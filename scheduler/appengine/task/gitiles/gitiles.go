...
func (m TaskManager) fetchRefsState(c context.Context, ctl task.Controller, cfg *messages.GitilesTask, g *gclient, repoURL *url.URL) (*refsState, error) {
	refs := &refsState{}
	...
	return refs, parallel.FanOutIn(func(work chan<- func() error) {
		...
		for _, wrs := range refs.watched.namespaces {
			wrs := wrs
			work <- func() error {
				return refs.fetchTips(c, wrs.namespace, g)
			}
		}
	})
}
...
func (m TaskManager) getGitilesClient(c context.Context, ctl task.Controller, cfg *messages.GitilesTask) (*gclient, error) {
	host, project, err := gitiles.ParseRepoURL(cfg.Repo)
	if err != nil {
		return nil, errors.Annotate(err, "invalid repo URL %q", cfg.Repo).Err()
	}
	r := &gclient{host: host, project: project}
...
// gclient embeds GitilesClient with useful metadata.
type gclient struct {
...
func (s *refsState) fetchCurrentTips(c context.Context, refsPath string, g *gclient) error {
