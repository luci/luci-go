...
func (w *watchedRefNamespace) addRegexp(refRegexp string) {
	refRegexp = "^" + refRegexp + "$"
	var err error
	if w.descendantRegexp == nil {
		w.descendantRegexp, err = regexp.Compile(refRegexp)
	} else {
		w.descendantRegexp, err = regexp.Compile(
			"(" + w.descendantRegexp.String() + ")|(" + refRegexp + ")")
	}
	if err != nil {
		panic(err)
	}
...
func (w *watchedRefs) init(refsConfig []string) {
	...
	for _, ref := range refsConfig {
		...
		case regexp != "":
			w.namespaces[ns].addRegexp(regexp)
		case suffix != "":
