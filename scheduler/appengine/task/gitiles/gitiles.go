...
// FetchRefTips fetches ref tips using fetcher func and writes them to refTips
// if they are in watchedRefs. The lock is used when writing to refTips map.
func FetchRefTips(fetcher func() (*gitilespb.RefsResponse, error), lock *sync.Mutex, refTips map[string]string, watchedRefs WatchedRefs) error {
	resp, err := fetcher()
	if err != nil {
		return err
	}
...
func (s *refsState) fetchCurrentTips(c context.Context, refsPath string, g *gitilesClient) error {
	...
	return FetchRefTips(func() (*gitilespb.RefsResponse, error) {
		return g.Refs(c, &gitilespb.RefsRequest{
			Project:  g.project,
			RefsPath: refsPath,
		})
	}, &s.currentLock, s.current, s.watched)
}
...
// WatchedRefs is a type that allows to keep a set of watched refs.
