...
// migrating from legacy to modern key on the fly.
func loadStateEntry(c context.Context, jobID, repo string) (*Repository, error) {
	modernID, err := repositoryID(jobID, repo)
	if err != nil {
		return nil, err
	}
	modern := &Repository{ID: modernID}
	if err := ds.Get(c, modern); err != ds.ErrNoSuchEntity {
		return modern, transient.Tag.Apply(err)
	}
	...
	legacy := Repository{ID: legacyID}
	switch err := ds.Get(c, &legacy); {
	case err == ds.ErrNoSuchEntity:
		return nil, err
	case err != nil:
		return nil, transient.Tag.Apply(err)
	}
	// Replace modern record with legacy one atomically.
	*modern = legacy
	modern.ID = modernID
	err = ds.RunInTransaction(c, func(c context.Context) error {
		if err := ds.Put(c, modern); err != nil {
			return err
		}
		return ds.Delete(c, &legacy)
