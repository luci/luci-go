...
var (
	// distribuitionBucketer1To1Mi covers range of 1 to 2**20.
	//
	// 0.06143 is math.log10(math.exp(math.log(1024*1024)/98)), which means
	// the last bucket will contain overflow of everything >= 1MiB.
	distribuitionBucketer1To1Mi = distribution.GeometricBucketer(math.Pow(10.0, 0.06143), 100)

	metricTaskGitilesStoredSize = metric.NewCumulativeDistribution(
		"luci/scheduler/task/gitiles/stored/size",
		"Size of serialized state in bytes.",
		&types.MetricMetadata{Units: types.Bytes},
		distribuitionBucketer1To1Mi,
		field.String("jobID"),
	)

	// distribuitionBucketer1To50K covers range of 1 to 50K.
	//
	// 0.04794 is math.log10(math.exp(math.log(50*1000)/98)), which means
	// the last bucket will contain overflow of everything >= 50K.
	distribuitionBucketer1To50K = distribution.GeometricBucketer(math.Pow(10.0, 0.04794), 100)

	metricTaskGitilesStoredRefs = metric.NewCumulativeDistribution(
		"luci/scheduler/task/gitiles/stored/refs",
		"Number of refs stored in a serialized state.",
		nil,
		// Current design can't store more than 50K refs because of 1MiB data store limit.
		distribuitionBucketer1To50K,
		field.String("jobID"),
	)
)
...
func saveState(c context.Context, jobID string, u *url.URL, refTips map[string]string) error {
	...
	metricTaskGitilesStoredRefs.Add(c, float64(len(refTips)), jobID)
	metricTaskGitilesStoredSize.Add(c, float64(compressed.Len()), jobID)
