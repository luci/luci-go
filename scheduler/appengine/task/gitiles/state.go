	...
	case len(stored.CompressedState) > 0:
		...
		for _, space := range state.Spaces {
			for _, child := range space.Children {
				heads[space.Prefix+"/"+child.Suffix] = hex.EncodeToString(child.Hsh)
			}
		}
		return heads, nil
...
func saveState(c context.Context, jobID string, u *url.URL, refTips map[string]string) error {
	...
	for ref, hsh := range refTips {
		hshBytes, err := hex.DecodeString(hsh)
		if err != nil {
			return err
		}
		ns, suffix := splitRef(ref)
		child := &Child{Hsh: hshBytes, Suffix: suffix}
