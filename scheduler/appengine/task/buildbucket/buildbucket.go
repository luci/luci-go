...
func (m TaskManager) AbortTask(c context.Context, ctl task.Controller) error {
	...
	err = m.withBuildbucket(c, ctl, func(c context.Context, bb *bbv1.Service) error {
		_, err := bb.Cancel(taskData.BuildID, nil).Context(c).Do()
		return err
	})
	if err != nil {
		ctl.DebugLog("Failed to cancel buildbucket build - %s", err)
