...
func (m TaskManager) LaunchTask(c context.Context, ctl task.Controller) error {
	...
	commitFromProps, poppedTags := popCommitFromProps(props, &tags)
	switch {
...
func popCommitFromProps(props *structpb.Struct, tags *[]string) (commit *bbpb.GitilesCommit, popped []string) {
	...
	if repo := pop("repository"); repo != "" {
		if repoURL, err := gitiles.NormalizeRepoURL(repo, false); err == nil {
			commit.Host = repoURL.Host
			commit.Project = strings.TrimPrefix(repoURL.Path, "/")
		}
	}

	// Require at least repo and commit. Can survive without `ref`.
...
func toBuildbucketPairs(s []string) []*bbpb.StringPair {
	...
	for i, kv := range s {
		k, v := "", ""
		switch parts := strings.SplitN(kv, ":", 2); {
		case len(parts) == 1:
			k = parts[0]
		case len(parts) == 2:
			k, v = parts[0], parts[1]
