...
func (m TaskManager) AbortTask(c context.Context, ctl task.Controller) error {
	...
	// and fail with a transient error instead. This avoids a collision on State
	// modification. When such collision happens, results of LaunchTask (including
	// a fresh build ID) are discarded (as the engine is unable to "merge"
	// conflicting mutations from two different state transitions). This is really
	// bad, since this process produces orphaned Buildbucket builds.
	//
	// So we pick a lesser evil and make AbortTask fail while invocation is
	// starting.
