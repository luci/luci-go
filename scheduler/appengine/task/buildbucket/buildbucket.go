...
func (m TaskManager) LaunchTask(c context.Context, ctl task.Controller, triggers []*internal.Trigger) error {
	...
	if gitilesTrigger := maybeGetTrigger(c, ctl, triggers); gitilesTrigger != nil {
		...
		if err != nil {
			...
			tags = append(tags, "buildset:"+buildset.String())
		}
...
func makeBuildSet(t *internal.GitilesTriggerData) (bb.BuildSet, error) {
	...
	p.Path = p.Path[len("/a/"):]
	return &bb.GitilesCommit{p.Host, p.Path, t.Revision}, nil
}
