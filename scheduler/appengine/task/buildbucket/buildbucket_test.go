...
func TestTriggeredFlow(t *testing.T) {
	Convey("LaunchTask with GitilesTrigger works", t, func(ctx C) {
		...
		triggers := []task.Trigger{
			task.Trigger{ID: "1", Payload: makePayload("https://repo.url", "refs/heads/master", "baadcafe")},
			task.Trigger{ID: "2", Payload: makePayload("https://repo.url", "refs/heads/master", "deadbeef")},
		}
