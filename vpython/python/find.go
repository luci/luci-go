...
// LookPathFilter is given a candidate file path to an executable, and
// returns nil if that executable passes the filter, and an error explaining
// why it failed otherwise
type LookPathFilter func (c context.Context, target string) error
...
func findInterpreter(c context.Context, name string, vers Version, lookPath LookPathFunc) (*Interpreter, error) {
	...
	testInterpreterVersion := func (c context.Context, path string) error {
		i := Interpreter{
			Python: path,
		}
		return versionCheck(c, i)
	}

	lpr, err := lookPath(c, name, testInterpreterVersion)
	if err != nil {
