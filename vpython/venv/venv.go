...
var pipIsolateOptions = []struct {
	...
	env string
	// installOpts is the set of options to pass to "pip install".
	installOpts []string
}{
	// Override any global configuration that prohibits binary (wheel) usage.
	{"PIP_NO_BINARY=:none:", []string{"--no-binary", ":none:"}},

	// Enforce that all packages must be installed with wheels.
	{"PIP_ONLY_BINARY=:all:", []string{"--only-binary", ":all:"}},

	// Override (deprecated) "--no-use-wheel" option.
	{"PIP_USE_WHEEL=1", []string{"--use-wheel"}},
}
...
func (e *Env) getStdlibPath(c context.Context, env []string) (string, error) {
	...
	cmd := e.Interpreter().IsolatedCommand(c, python.CommandTarget{
		Command: `import sys, site, os;sys.stdout.write(os.path.dirname(site.__file__))`,
	})
	cmd.Env = env
...
func (e *Env) installWheels(c context.Context, bootstrapDir, pkgDir string, env []string) error {
	...
	for _, opt := range pipIsolateOptions {
		pythonCmd = append(pythonCmd, opt.installOpts...)
	}
...
func (e *Env) isolatedSetupEnvironment(bootstrapDir string) environ.Env {
	...
	// has a global configuration, we are stuck overriding it with our own
	// preferred defaults.
	for _, opt := range pipIsolateOptions {
...
func (e *Env) writeSetupError(c context.Context, err error) {
	logging.Errorf(c, `Failed to setup VirtualEnv.
Error: %s
...
vpython is sensitive to global "pip" configuration files. Please check to
ensure that you are not overriding "pip"'s configuration. For more information,
see: https://pip.pypa.io/en/stable/user_guide/#configuration

If you cannot resolve the issue, please re-run your command using the
"-vpython-log-level=debug" flag and file a bug with the maintainers including
a copy of the debug output and this message.

Ensure that there is no sensitive information in the output before filing!

`,
		err, proto.MarshalTextString(e.Environment))
}
