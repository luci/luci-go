...
//
// If another process holds the lock, With will return an error (if cfg.Blocking
// is false) or try again until it obtains the lock (if cfg.Blocking is true).
func With(c context.Context, cfg Config, fn func(context.Context, *Env) error) error {
	...
	if cfg.HasWheels() {
		...
		if err := emptyEnv.ensure(c, !cfg.NonBlocking); err != nil {
	...
	return env.withImpl(c, !cfg.NonBlocking, usedEnvs, fn)
}
...
func (e *Env) installVirtualEnv(c context.Context, pkgDir string) error {
	...
	// cleaned up now.
	return withTempDir(logging.Get(c), e.Root, "home", func(tdir string) error {
		//Install  temporary home directory.
		env := e.isolatedSetupEnvironment()
		env.Set("HOME", tdir)
		envSorted := env.Sorted()
...
func (e *Env) getPEP425Tags(c context.Context) ([]*vpython.PEP425Tag, error) {
	...
	cmd.Env = e.isolatedSetupEnvironment().Sorted()
