...
type Config struct {
	...
	// VirtualEnv setup operations. If nil, environ.System() will be used.
	...
	// NonBlocking, if true, means that if a lock is encountered during VirtualEnv
	// operation, the VirtualEnv will exit immediately with an error. If false,
	// the VirtualEnv will block pending the lock's availability.
	NonBlocking bool
