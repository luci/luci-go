...
func validateConfigGroup(ctx *validation.Context, group *v2.ConfigGroup, knownNames stringset.Set) {
	re := regexp.MustCompile("^[a-zA-Z][a-zA-Z0-9_-]{0,39}$")
	switch {
	case group.Name == "":
	...
	if additionalModes := group.AdditionalModes; len(group.AdditionalModes) > 0 {
		ctx.Enter("additional_modes")
		modeRe := regexp.MustCompile("^[a-zA-Z][a-zA-Z0-9_-]{0,39}$")
		nameSet := stringset.New(len(additionalModes))
		for _, m := range additionalModes {
			switch name := m.Name; {
			case name == "":
				ctx.Errorf("`name` is required")
			case name == "DRY_RUN" || name == "FULL_RUN":
				ctx.Errorf("`name` MUST not be DRY_RUN or FULL_RUN")
			case !modeRe.MatchString(name):
				ctx.Errorf("`name` must match %q but %q is given", modeRe, name)
...
func validateTryjobVerifier(ctx *validation.Context, v *v2.Verifiers_Tryjob) {
	...
	visitBuilders(func(b *v2.Verifiers_Tryjob_Builder) {
		...
		if len(b.ModeRegexp)+len(b.ModeRegexpExclude) > 0 {
			...
			// TODO(crbug/1191855): See if CV should loose the following restriction.
			if b.TriggeredBy != "" {
