...
func validateTryjobVerifier(ctx *validation.Context, v *v2.Verifiers_Tryjob, supportedModes stringset.Set) {
			...
			if isAnalyzer {
				...
				for i, r := range b.LocationRegexp {
					if !strings.HasPrefix(r, `.+\.`) {
						...
						ctx.Errorf(`location_regexp must starts with ".+\." for tryjob run in %s mode`, analyzerRun)
						ctx.Exit()
					}
				}
	...
	if !hasNonAnalyzerBuilder {
		// (TODO:crbug/1202952): This is for preventing users defining new config
		// group purely for analyzer purpose which accidentally overlaps with users'
		// main cq config group before Tricium is merged into CV. Current known use
		// cases (i.e. defining Tricium config in auxillary LUCI project) will set
		// `lucicfg.config(tracked_files="tricium-prod.cfg")` so that no cq config
		// will be generated. This check can be removed after Tricium is merged
		// into CV so ANALYZER is treated the same as any other run modes supported
		// by CV.
