	...
	Convey("Validate Tryjob Verifier Config", t, func() {
		...
		Convey("no combinations", func() {
			...
			So(validate(`
				...
				builders { name: "c/d/e" } `),
				ShouldErrLike,
				"combining [location_regexp[_exclude] triggered_by] features not allowed")
		})
		...
		Convey("triggered_by", func() {
			...
			So(validate(`builders {name: "a/b/1" triggered_by: "a/b/0"}`),
				ShouldErrLike, `triggered_by must refer to existing builder, but "a/b/0" given`)

			So(validate(`
				builders {name: "a/b/0" experiment_percentage: 10}
				builders {name: "a/b/1" triggered_by: "a/b/0"}
			`), ShouldErrLike,
				`builder a/b/1): triggered_by must refer to existing builder without extra features`)
		})
	})
}
