...
// TODO(yiwzhang): clean up all the follow temporary structs once we have
// the formal data model in the new CV.
type CL struct {
	Key  string
	Deps []Dep
}
...
// dependencies. CLs are in the order approximately minimizing the number of
// broken soft dependencies.
//
// Panics if duplicate CLs are provided or the hard requirement dependencies
// solely can form a cycle (This is not possible within Gerrit context as they
// follow Git's child -> parent relationship). In case of a cycle introduced by
// soft dependencies, breaks cycle by arbitrarily but deterministically by
...
// the latest followed dep that is `Soft Requirement`
//
