...
var metricProcessed = metric.NewCounter(
	"luci/tumble/mutations/processed",
	"The number of mutations processed by tumble, but not necessarily deleted.",
	nil,
...
func processRoot(c context.Context, cfg *Config, root *ds.Key, banSet stringset.Set, cnt *counter) error {
	...
	if len(toDel) > 0 {
		...
		if err := ds.Delete(c, toDel); err != nil {
			// This is catagorized as failed because it's going to get retried again.
			metricFailed.Add(c, 1, root.Namespace())
			...
			metricDeleted.Add(c, 1, root.Namespace())
		}
	}
