...
func TestGetPoller(t *testing.T) {
	...
	ftt.Run("GetPoller works", t, func(t *ftt.Test) {
		...
		t.Run("with access", func(t *ftt.Test) {
			...
			assert.NoErr(t, err)
			assert.That(t, resp.GetUpdateTime().AsTime(), should.Match(now))
		})
	})
}
		...
		t.Run("with access", func(t *ftt.Test) {
			...
			t.Run("two runs of the same project", func(t *ftt.Test) {
				...
				t.Run("with page size exactly 2", func(t *ftt.Test) {
					...
					assert.NoErr(t, err)
					assert.That(t, idsOf(resp1), should.Match([]string{laterID, earlierID}))
				...
				t.Run("with page size of 1", func(t *ftt.Test) {
					...
					resp1, err := a.SearchRuns(ctx, req)
					assert.NoErr(t, err)
					assert.That(t, idsOf(resp1), should.Match([]string{laterID}))

					req.PageToken = resp1.GetNextPageToken()
					resp2, err := a.SearchRuns(ctx, req)
					assert.NoErr(t, err)
					assert.That(t, idsOf(resp2), should.Match([]string{earlierID}))
			...
			t.Run("filtering", func(t *ftt.Test) {
				...
				t.Run("exact", func(t *ftt.Test) {
					...
					assert.NoErr(t, err)
					assert.That(t, idsOf(resp), should.Match([]string{earlierID}))
				})
				...
				t.Run("with CL + paging", func(t *ftt.Test) {
					...
					assert.That(t, idsOf(total), should.Match([]string{laterID, earlierID}))
				})
				...
				t.Run("with CL across projects and paging", func(t *ftt.Test) {
					...
					total := fetchAll(ctx, req)
					assert.That(t, idsOf(total), should.Match([]string{diffProjectID, laterID, earlierID}))
				})
			...
			t.Run("runs aross all projects", func(t *ftt.Test) {
				...
				t.Run("just one project", func(t *ftt.Test) {
					...
					t.Run("without paging", func(t *ftt.Test) {
						...
						assert.NoErr(t, err)
						assertOrdered(resp)
						assert.That(t, idsOf(resp), should.Match(expIDs))
					})
					t.Run("with paging", func(t *ftt.Test) {
						total := fetchAll(ctx, &adminpb.SearchRunsRequest{PageSize: 2})
						assertOrdered(total)
						assert.That(t, idsOf(total), should.Match(expIDs))
					})
				})
	...
	ftt.Run("DeleteProjectEvents works", t, func(t *ftt.Test) {
		...
		t.Run("with access", func(t *ftt.Test) {
			...
			t.Run("All", func(t *ftt.Test) {
				...
				assert.NoErr(t, err)
				assert.That(t, resp.GetEvents(), should.Match(map[string]int64{"*prjpb.Event_Poke": 2, "*prjpb.Event_NewConfig": 1}))
			})
...
func TestRefreshProjectCLs(t *testing.T) {
	...
	ftt.Run("RefreshProjectCLs works", t, func(t *ftt.Test) {
		...
		t.Run("with access", func(t *ftt.Test) {
			...
			assert.NoErr(t, err)
			assert.That(t, resp.GetClVersions(), should.Match(map[int64]int64{1: 4}))
			...
			assert.That(t, scheduledIDs.ToSortedSlice(), should.Match([]string{string(cl.ExternalID)}))
		})
	})
}
