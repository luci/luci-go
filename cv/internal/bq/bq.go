...
type Client interface {
	...
	SendRow(ctx context.Context, dataset, table string, row proto.Message) error
}
...
//
// This may be called in the app's main function.
...
func (ins *Inserter) SendRow(ctx context.Context, dataset, table string, msg proto.Message) error {
	client := mustClient(ctx)
	tab := client.clientImpl.Dataset(dataset).Table(table)
	row := &lucibq.Row{
		Message: protov1.MessageV1(msg),
		// TODO(qyearsley): Add InsertID field if it is useful, e.g.
	...
	if err := tab.Inserter().Put(ctx, row); err != nil {
		...
		return err.Annotate(err, "unknown error sending row").Tag(transient.Tag).Err()
	}
	return nil
}
