...
type Run struct {
	...
	// TryjobQuota is the remaining quota available to retry failed Tryjobs.
...
// RunCL is the snapshot of a CL involved in this Run when this Run was
// triggered.
type RunCL struct {
...
//
// All detailed information about this Tryjob in the external system is
// stored in `Tryjob` entity.
type RunTryjob struct {
	...
	// Experimental tells whether this Tryjob is experimental or not.
	//
	...
	// Quota is the remaining quota available to retry failed Tryjobs.
	//
	...
	//
	// In other word, this Tryjob won't be launched unless all Tryjobs in this
	// slice report success status.
	WaitingOn []tryjob.ID
	// PrevRetries are the IDs of all previous retries of this Tryjobs.
