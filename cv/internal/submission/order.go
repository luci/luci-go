...
func ComputeOrder(cls []*changelist.CL) ([]*changelist.CL, error) {
...
func compute(cls []*changelist.CL) ([]*changelist.CL, int, error) {
	ret := make([]*changelist.CL, 0, len(cls))
	remainingCLs := make(map[common.CLID]*changelist.CL, len(cls))
	for _, cl := range cls {
	...
	var visit func(cl *changelist.CL) bool
	visit = func(cl *changelist.CL) bool {
		if _, ok := remainingCLs[cl.ID]; !ok {
			return false
		}
		...
		for _, dep := range cl.Snapshot.GetDeps() {
			...
			depCL, ok := remainingCLs[common.CLID(dep.GetClid())]
			if !ok {
				continue
			}
			if _, ok := brokenDeps[brokenDep{cl.ID, common.CLID(dep.GetClid())}]; ok {
				continue
			}
			for visit(remainingCLs[depCL.ID]) {
				...
				brokenDeps[brokenDep{cl.ID, common.CLID(dep.GetClid())}] = struct{}{}
				break
			}
		}
		delete(cycleDetector, cl.ID)
	...
	sortedCLs := make([]*changelist.CL, len(cls))
	copy(sortedCLs, cls)
