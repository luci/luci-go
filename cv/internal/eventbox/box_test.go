...
func (p *processor) GetState(ctx context.Context) (State, EVersion, error) {
...
func (p *processor) GetEVersion(ctx context.Context) (EVersion, error) {
	c, err := get(ctx, p.index)
	if err != nil {
		return 0, err
	}
...
func (p *processor) Mutate(ctx context.Context, events Events, s State) (ts []Transition, err error) {
		...
		case *population == 0:
			ts = append(ts, Transition{
				SideEffectFn: func(ctx context.Context) error {
					logging.Debugf(ctx, "advertising to %d to migrate", p.index+1)
	...
	if len(minus) > 0 {
		if *population > 1 {
			population = add(-1) // Migrate at most one to the left.
			ts = append(ts, Transition{
				SideEffectFn: func(ctx context.Context) error {
					logging.Debugf(ctx, "emigrating to %d", p.index-1)
					return Emit(ctx, []byte{'+'}, key(ctx, p.index-1))
				},
				Events:       minus,
				TransitionTo: population,
			...
			logging.Debugf(ctx, "%d noops", len(minus))
			ts = append(ts, Transition{
				SideEffectFn: nil,        // noop
				Events:       minus,      // just consume these events
				TransitionTo: population, // same
			})
		}
	}
	return
