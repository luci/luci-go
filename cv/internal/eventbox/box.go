...
func Emit(ctx context.Context, value []byte, to *datastore.Key) error {
	...
	id := fmt.Sprintf("%s/%d", uuid.New().String(), clock.Now(ctx).Unix())
...
func listAndCleanup(ctx context.Context, d *dsset.Set, maxEvents int) (*dsset.Listing, error) {
	...
	metricListingSizes.Add(ctx, float64(len(listing.Items)), recipient)
	metricListingOldestAgeS.Add(ctx, oldestEventAge(ctx, listing.Items).Seconds(), recipient)
...
func oldestEventAge(ctx context.Context, items []dsset.Item) time.Duration {
	...
	for _, item := range items {
		...
		if parts := strings.SplitN(item.ID, "/", 2); len(parts) == 2 {
			if unixSecs, err := strconv.Atoi(parts[1]); err == nil {
				if t := time.Unix(int64(unixSecs), 0); oldest.IsZero() || oldest.After(t) {
					oldest = t
				}
			}
		}
