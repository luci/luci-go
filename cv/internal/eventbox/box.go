...
func Emit(ctx context.Context, id string, value []byte, to *datastore.Key) error {
...
// Events returns unprocessed events. For use in tests only.
...
func ProcessBatch(ctx context.Context, recipient *datastore.Key, ctrl Controller) error {
	var state State
	var expectedEV EVersion
	eg, ectx := errgroup.WithContext(ctx)
	eg.Go(func() (err error) {
		state, expectedEV, err = ctrl.GetState(ectx)
		return
	})
	d := dsset.Set{
		Parent:          recipient,
	...
	err = datastore.RunInTransaction(ctx, func(ctx context.Context) error {
		switch latestEV, err := ctrl.GetEVersion(ctx); {
		case err != nil:
			return err
...
type Controller interface {
	...
	GetState(context.Context) (State, EVersion, error)
	...
	// isn't saved.
	GetEVersion(ctx context.Context) (EVersion, error)
