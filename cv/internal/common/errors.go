...
func (t TQIfy) Error(ctx context.Context, err error) error {
	...
	retry := matchesErrors(err, t.KnownRetry...) || matchErrorTags(err, t.KnownRetryTags...)
	ignore := matchesErrors(err, t.KnownIgnore...) || matchErrorTags(err, t.KnownIgnoreTags...)
	switch {
	case retry:
		if ignore {
...
// TODO(yiwzhang): make matching logic more efficient with only one around of
// walk. To achieve that,
//  * The callback in errors.Walk must tell whether an error is a leaf or not.
//  * Expose a method to get the tag value associated with the provided error
//    only (not including the tag value of its child errors).
...
func matchErrorTags(err error, knownTags ...errors.BoolTag) bool {
