...
type recipientCell struct {
	k *datastore.Key
}

func (r recipientCell) Key() *datastore.Key      { return r.k }
func (r recipientCell) MonitoringString() string { return fmt.Sprintf("cell-%d", r.k.IntID()) }

func mkRecipient(ctx context.Context, id int) recipientCell {
	return recipientCell{k: datastore.MakeKey(ctx, "cell", id)}
}
