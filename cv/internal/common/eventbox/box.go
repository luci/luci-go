...
func listAndCleanup(ctx context.Context, r Recipient, d *dsset.Set, maxEvents int) (*dsset.Listing, error) {
	...
	mRecipient := r.MonitoringString()
	metricListDurationsS.Add(ctx, float64(clock.Since(ctx, tStart).Milliseconds()), mRecipient, monitoringResult(err))
	if err != nil {
		return nil, err
	}
	metricSize.Set(ctx, int64(len(listing.Items)), mRecipient)
	metricOldestAgeS.Set(ctx, oldestEventAge(ctx, listing.Items).Seconds(), mRecipient)

	if err := dsset.CleanupGarbage(ctx, listing.Garbage); err != nil {
		return nil, err
	}
	metricRemoved.Add(ctx, int64(len(listing.Garbage)), mRecipient)
	return listing, nil
}
