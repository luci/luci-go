...
func TestSet(t *testing.T) {
	...
	ftt.Run("item one lifecycle", t, func(t *ftt.Test) {
		...
		// Add one item.
		assert.NoErr(t, set.Add(c, []Item{{ID: "abc"}}))

		// The item is returned by the listing.
		listing, err := set.List(c, limit)
		assert.NoErr(t, err)
		assert.That(t, listing.Items, should.Match([]Item{{ID: "abc"}}))
		...
		err = datastore.RunInTransaction(c, func(c context.Context) error {
			...
			assert.NoErr(t, err)
			assert.That(t, popped, should.Match([]string{"abc"}))
			return nil
		}, nil)
		assert.NoErr(t, err)
		...
		assert.NoErr(t, CleanupGarbage(c, listing.Garbage))
		listing, err = set.List(c, limit)
		assert.NoErr(t, err)
		assert.Loosely(t, listing.Items, should.BeNil)
		assert.Loosely(t, listing.Garbage, should.BeNil)
	...
	ftt.Run("List obeys limit", t, func(t *ftt.Test) {
		...
		assert.NoErr(t, set.Add(c, []Item{{ID: "abc"}}))
		assert.NoErr(t, set.Add(c, []Item{{ID: "def"}}))
		assert.NoErr(t, set.Add(c, []Item{{ID: "ghi"}}))
	...
	ftt.Run("delete items non-transactionally", t, func(t *ftt.Test) {
		...
		assert.NoErr(t, err)
		assert.That(t, l2.Items, should.Match([]Item{{ID: "ghi"}}))
	})
}
