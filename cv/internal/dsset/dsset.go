...
//
// Structurally dsset places 2 kinds of entities under provided root:
//   * items of the set.
...
type Listing struct {
	...
	owner      *datastore.Key            // set's Owner.
...
func (s *Set) List(ctx context.Context) (*Listing, error) {
	...
	tombsEntity := tombstonesEntity{Parent: s.Owner}
	...
	eg.Go(func() error {
		q := datastore.NewQuery("dsset.Item").Ancestor(s.Owner)
...
func (s *Set) BeginPop(c context.Context, listing *Listing) (*PopOp, error) {
	...
	entity := &tombstonesEntity{Parent: s.Owner}
