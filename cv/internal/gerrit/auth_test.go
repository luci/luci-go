	...
	Convey("factory.token works", t, func() {
		...
		Convey("works", func() {
			Convey("forced legacy", func() {
				...
				Convey("legacy caching", func() {
					So(datastore.Put(ctx, &netrcToken{"first.example.com", "legacy-2"}), ShouldBeNil)

					tclock.Add(cacheLegacyDuration - time.Second)
					t2, err := f.token(ctx, "first.example.com", "force-legacy")
					So(err, ShouldBeNil)
					So(t2, ShouldEqual, t) // same pointers

					tclock.Add(time.Second)
					t3, err := f.token(ctx, "first.example.com", "force-legacy")
					So(err, ShouldBeNil)
					So(t3, ShouldNotEqual, t) // diff pointers
					So(t3.AccessToken, ShouldEqual, "legacy-2")

					Convey("works with deletion", func() {
						So(datastore.Delete(ctx, &netrcToken{GerritHost: "first.example.com"}), ShouldBeNil)
						t4, err := f.token(ctx, "first.example.com", "force-legacy")
						So(err, ShouldBeNil)
						So(t4, ShouldEqual, t3) // same pointers

						tclock.Add(cacheLegacyDuration)
						_, err = f.token(ctx, "first.example.com", "force-legacy")
