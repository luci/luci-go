...
func TestTrigger(t *testing.T) {
	...
	Convey("Find", t, func() {
		...
		Convey("Abandoned CL", func() {
			...
			So(Find(&FindInput{ChangeInfo: ci, ConfigGroup: cg}), ShouldBeNil)
		})
		...
		Convey("Single vote", func() {
			...
			ts := Find(&FindInput{ChangeInfo: ci, ConfigGroup: cg})
			So(ts, ShouldResembleProto, &run.Triggers{CqVoteTrigger: &run.Trigger{
		...
		Convey("Additional modes", func() {
			...
			Convey("QuickDryRun despite other users' votes", func() {
				...
				ts := Find(&FindInput{ChangeInfo: ci, ConfigGroup: cg})
				So(ts.GetCqVoteTrigger().GetMode(), ShouldEqual, run.QuickDryRun)
			})
			...
			Convey("Not applicable cases", func() {
				Convey("Additional vote must have the same timestamp", func() {
					Convey("before", func() {
						...
						ts := Find(&FindInput{ChangeInfo: ci, ConfigGroup: cg})
						So(ts.GetCqVoteTrigger().GetMode(), ShouldEqual, run.DryRun)
					})
					Convey("after", func() {
						ci.Labels[quickLabel].GetAll()[0].Date.Seconds--
						ts := Find(&FindInput{ChangeInfo: ci, ConfigGroup: cg})
				...
				Convey("CQ vote must have correct value", func() {
					...
					ts := Find(&FindInput{ChangeInfo: ci, ConfigGroup: cg})
					So(ts.GetCqVoteTrigger().GetMode(), ShouldEqual, run.FullRun)
				})
			})
		})
	})
