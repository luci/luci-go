...
//
//   * Each CQ-Depend line is parsed independently from another,
//     any invalid ones are silently ignored. For example,
...
type Dep struct {
	// Subdomain is set of change is on a different Gerrit host.
	Subdomain string
	Change    int
}
...
func Parse(description string) (deps []Dep) {
	...
	// duplicates are highly unlikely in practice.
	l := 0
...
func parseSingleDep(v string) (Dep, error) {
	...
	case strings.HasSuffix(res[1], ":"):
		dep.Subdomain = res[1][:len(res[1])-1]
		fallthrough
	default:
		c, err := strconv.ParseInt(res[2], 10, 31)
		if err != nil {
