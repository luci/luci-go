...
func Update(ctx context.Context, project string) error {
	var toPut, toDelete []*gobWatchMap
	...
	case err != nil:
		return err
	case !meta.Exists() || meta.Status != config.StatusEnabled:
	...
	default:
		...
		toPut, toDelete = listUpdates(ctx, gwms, cgs, meta.Hash(), project)
	}
...
func listUpdates(ctx context.Context, gwms []*gobWatchMap, latestConfigGroups []*config.ConfigGroup,
	...
	existingHashes := make(map[string]string, len(gwms))
	for _, gwm := range gwms {
		hostRepo := gwm.Parent.StringID()
		existingHashes[hostRepo] = gwm.ConfigHash
	}
	...
	// List GobWatchMaps to delete; these are those that currently exist but
...
func internalGroups(configGroups []*config.ConfigGroup) map[string]*internal.Groups {
	...
	for _, g := range configGroups {
		for _, gerrit := range g.Content.Gerrit {
			...
			// TODO(qyearsley): use config.GerritHost
