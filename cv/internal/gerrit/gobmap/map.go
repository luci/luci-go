...
type gobWatchMap struct {
	...
	// Map keeps config groups of a LUCI project applicable to this host/repo.
	Map *internal.GroupMap
...
func Lookup(ctx context.Context, host, repo, ref string) ([]ProjectConfigGroupID, error) {
	...
	// 2. For each entity, which represents 1 LUCI project,, inspect the
	//    RefSpecMapGroup to determine which configs apply.
	//    If there is more than 1 config config of a LUCI project matching,
	//       then all fallback groups are ignored.
