...
type gobWatchMap struct {
	...
	// The ID of this GobWatchMap, of the form "host/repo~project".
	// It is assumed that neither host nor repo contain "~".
	ID string `gae:"$id"`
...
func Update(ctx context.Context, project string) error {
	...
	gwms := []*gobWatchMap{}
	q := datastore.NewQuery(gobWatchMapKind).Eq("Parent", project)
	if err := datastore.GetAll(ctx, q, &gwms); err != nil {
		return errors.Annotate(err, "failed to get GobWatchMap entities for project %s", project).Err()
	}
...
// from GobWatchMap.
func diffHostRepos(gwms []*gobWatchMap) (added []string, removed []string) {
	oldHostRepos = stringset.New()
	newHostRepos = stringset.New()

	for _, gwm := range gwms {
		oldHostRepos.Add(gobWatchMapHostRepo(gwm.ID))
	}
...
func gobWatchMapHostRepo(gobWatchMapID string) {
	parts := strings.SplitN(gobWatchMapID, "~", 2)
	if len(parts) != 2 {
		// Should never happen because host, repo and project
		// should not contain "~".
		panic("~ should occur once in a GobWatchMap ID")
	}
	return parts[0]
}
...
func Lookup(ctx context.Context, host, repo, ref string) (*changelist.ApplicableConfig, error) {
	...
	hostRepo := host + "/" + repo
	parentKey := datastore.NewKey(ctx, parentKind, hostRepo, 0, nil)
