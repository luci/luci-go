...
// atomically.
// Changes to individual Gerrit projects are atomic.  This means that
// IF Update() is in progress from config versions 1 to 2, identified by
//   hashes h1 and h2, respectively,
// AND both h1 and h2 watch specific Gerrit project, possibly among many others,
// THEN a concurrent Lookup(host,project,...) is guaranteed to return either
// based on @h1 or based on @h2.
//
// However, there is no atomicity across entire project config. This means that
// IF Update() is in progress from config versions 1 to 2, identified by
//   hashes h1 and h2, respectively,
// THEN two sequential calls to Lookup with diferent Gerrit projects may return
// results based on @h2 at first and @h1 for the second, ie:
//   Lookup(host1,projectA,...) -> @h2
//   Lookup(host1,projectB,...) -> @h1
//
// Thus, a failed Update() may leave gobmap in a corrupted state, whereby some
// Gerrit projects may represent old and some new config verisons. In such a
