...
func TestGobMap(t *testing.T) {
	...
	Convey("Update and Lookup", t, func() {
		...
		var cfg = &pb.Config{
			...
			ConfigGroups: []*pb.ConfigGroup{
				...
				},
				{
					...
					// the other group but not this one, even though it matches
					// the include regexp list.
					Gerrit: []*pb.ConfigGroup_Gerrit{
						{
							Url: "https://chromium-review.googlesource.com/",
							Projects: []*pb.ConfigGroup_Gerrit_Project{
		...
		Convey("Update nonexistent project does not store anything", func() {
			...
			So(err, ShouldBeNil)
			gwms := []*gobWatchMap{}
			q := datastore.NewQuery(gobWatchMapKind)
			So(datastore.GetAll(ctx, q, &gwms), ShouldBeNil)
			So(len(gwms), ShouldEqual, 0)
		})
		...
		Convey("Lookup nonexistent project returns empty ApplicableConfig", func() {
			...
			So(err, ShouldBeNil)
			So(len(ac.Projects), ShouldEqual, 0)
		})
		...
		Convey("Update and Lookup existing project", func() {
			...
			Convey("Lookup main ref", func() {
				...
				So(len(ac.Projects), ShouldEqual, 1)
	...
	Convey("listUpdates", t, func() {
		...
		// Sample GobWatchMap entities and ConfigGroups to use in the test below.
		gwms := []*gobWatchMap{
			&gobWatchMap{
				ID:      "foo",
				Project: "foo",
		...
		Convey("deletes all if latest config groups is empty", func() {
			put, delete := listUpdates(ctx, gwms, nil, "c0ffee", "foo")
			So(len(put), ShouldEqual, 0)
			So(len(delete), ShouldEqual, 3)
		})

		Convey("puts all if stored GWMs is empty", func() {
			put, delete := listUpdates(ctx, nil, cgs, "c0ffee", "foo")
			So(len(put), ShouldEqual, 3)
			So(len(delete), ShouldEqual, 0)
		})
		...
		Convey("Updates if groups exist but hash is different", func() {
			put, delete := listUpdates(ctx, nil, cgs, "newhash", "foo")
			So(len(put), ShouldEqual, 0)
			So(len(delete), ShouldEqual, 0)
		})
	})
