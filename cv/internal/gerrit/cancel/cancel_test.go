...
func TestCancel(t *testing.T) {
	...
	Convey("Cancel", t, func() {
		...
		Convey("Remove single votes", func() {
			...
			So(resultCI.Info.GetLabels()[trigger.CQLabelName].GetAll(), shouldVotesAllZero)
		...
		Convey("Post Message if CV has no permission to vote", func() {
			...
			resultCI := ct.GFake.GetChange(gHost, int(ci.GetNumber()))
			So(resultCI.Info.GetLabels()[trigger.CQLabelName].GetAll(), ShouldResembleProto, []*gerritpb.ApprovalInfo{
				{
					User:  user,
					Value: 2,
...
func shouldVotesAllZero(actual interface{}, expected ...interface{}) string {
	if len(expected) != 0 {
		return fmt.Sprintf("expected 0 value, got %d", len(expected))
	}
	approvalInfos, ok := actual.([]*gerritpb.ApprovalInfo)
	if !ok {
		return fmt.Sprintf("Wrong actual type %T, must be %T", actual, approvalInfos)
	}
	for _, ai := range approvalInfos {
		if ai.GetValue() != 0 {
			return fmt.Sprintf("Vote is non-zero %s", ai)
		}
	}
	return ""
}
