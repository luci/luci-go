...
func cancelLeased(ctx context.Context, c change, in Input) error {
	...
	removeErr := c.removeVotes(ctx, ci, in.Trigger, in.Message, in.Notify)
...
func applyLeaseForCL(ctx context.Context, clid common.CLID, duration time.Duration, requester string) (context.Context, func(), error) {
	...
	case err == lease.ErrConflict:
		return nil, nil, errors.Annotate(err, "CL is currently being mutated").Err()
	case err != nil:
		return nil, nil, err
	}
...
func (c *change) removeVotes(ctx context.Context, ci *gerritpb.ChangeInfo, t *run.Trigger, msg string, notify Notify) error {
	ais := ci.GetLabels()[trigger.CQLabelName].GetAll()
	...
	switch n, nd := notify.toGerritNotify(ais); {
	case errs.Get() != nil:
		return common.MostSevereError(errs.Get())
	case !triggerFound:
		return c.annotateGerritErr(ctx, c.postGerritMessage(ctx, ci, msg, t, n, nd), "post message")
	default:
...
func (c *change) postCancelMessage(ctx context.Context, ci *gerritpb.ChangeInfo, msg string, t *run.Trigger, runCLExternalIDs []changelist.ExternalID, notify Notify) (err error) {
	...
	if err := c.postGerritMessage(ctx, ci, msg, t, n, nd); err != nil {
		return c.annotateGerritErr(ctx, err, "post message")
	}
	return nil
}

// postGerritMessage posts the given message to Gerrit.
//
// Skips if duplicate message is found after triggering time.
func (c *change) postGerritMessage(ctx context.Context, ci *gerritpb.ChangeInfo, msg string, t *run.Trigger, n gerritpb.Notify, nd *gerritpb.NotifyDetails) (err error) {
