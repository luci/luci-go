...
func cancelLeased(ctx context.Context, c change, in Input) error {
	...
	if len(approvalInfos) == 0 {
		return nil
	}
	n, nd := in.Notify.toGerritNotify(approvalInfos)
	removeErr := c.removeVotes(ctx, approvalInfos, in.Trigger, in.Message, n, nd)
	...
	if postMsgErr := c.postCancelMessage(ctx, ci, msgBuilder.String(), in.Trigger, in.RunCLExternalIDs, n, nd); postMsgErr != nil {
...
func (c *change) postCancelMessage(ctx context.Context, ci *gerritpb.ChangeInfo, msg string, t *run.Trigger, runCLExternalIDs []changelist.ExternalID, n gerritpb.Notify, nd *gerritpb.NotifyDetails) (err error) {
	bd := botdata.BotData{
