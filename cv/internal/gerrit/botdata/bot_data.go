...
const (
	// BotDataPrefix is a string that prepends to the bot data message.
...
// UnmarshalJSON sets *a to the `Action` that the given bytes represents.
// Returns error for actions other than `Start` or `Cancel`.
func (a *ActionType) UnmarshalJSON(b []byte) error {
	var s string
	if err := json.Unmarshal(b, &s); err != nil {
		return errors.Annotate(err, "unmarshal action").Err()
	}
	switch action := ActionType(s); action {
	case Start, Cancel:
		*a = action
		return nil
	default:
		return errors.Reason("invalid action: %s", action).Err()
	}
}
...
func (c *ChangeID) UnmarshalText(b []byte) error {
	...
	if len(segs) != 2 {
		return errors.Reason("invalid ChangeID text: %s", string(b)).Err()
	}
...
type BotData struct {
	...
	// Revision is the revision (patch set) of the change at the time Action
	// is triggered.
	Revision string `json:"revision"`
	// CLs are IDs for all CLs that are included in the Run associated with
	// this Action.
...
// Append appends BotData (in JSON format) to the given human message.
...
// the BotData itself is already too long.
