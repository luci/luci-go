...
func (f *fetcher) setGitDeps(ctx context.Context, related []*gerritpb.GetRelatedChangesResponse_ChangeAndCommit) error {
	...
	// Note that diamond-shaped child->[parent1, parent2]->grantchild are
	...
	// under different revisions (patchets).
	...
	for i, r := range ordered[1:] {
		n := r.GetNumber()
		if n == f.change || added[n] {
			continue
		}
		added[n] = true
		deps = append(deps, &changelist.GerritGitDep{
			Change: n,
			// By constrution of ordered, immediate dependencies must be located at
...
func (f *fetcher) fetchFiles(ctx context.Context) error {
	resp, err := f.g.ListFiles(ctx, &gerritpb.ListFilesRequest{
		...
		Project:    f.gerritProjectIfKnown(),
		RevisionId: f.mustCurrentRevision(),
		// For CLs with >1 parent commit (aka merge commits), relies on Gerrit
