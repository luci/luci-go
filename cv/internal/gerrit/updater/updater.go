...
func (f *fetcher) shouldSkip(ctx context.Context) (skip bool, err error) {
	...
	case !f.updatedHint.IsZero() && f.priorCL.Snapshot.IsUpToDate(f.luciProject, f.updatedHint):
		...
		switch acfg, err := gobmap.Lookup(ctx, f.host, ci.Project, ci.Ref); {
		case err != nil:
			return false, err
		case acfg.ContainsProject(f.luciProject):
			logging.Debugf(ctx, "Updating %s to %s skipped, already at %s", f, f.updatedHint,
				f.priorCL.Snapshot.ExternalUpdateTime.AsTime())
			return true, nil
		default:
...
func (f *fetcher) update(ctx context.Context) error {
	...
	min, cur, err := gerrit.EquivalentPatchsetRange(f.newSnapshot.GetGerrit().Info)
	if err != nil {
		return err
	}
