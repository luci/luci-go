...
//
// pollInterval must be above 10ms.
func projectOffset(luciProject string, pollInterval time.Duration) time.Duration {
	...
	if pollInterval < 10*time.Millisecond {
		panic(fmt.Errorf("pollInterval %s < 10ms not supported", pollInterval))
	}
	return time.Duration(
		// int64(pollInterval) is >= 10^7 (ns) >= 2^23.
		// So, we can leave 7+ significant bits by ignoring 16 least significant.
		(int64(pollInterval) >> 16) *
			// Take 16 least significant bits of 32-bit hash.
			int64(h.Sum32()&0xFFFF))
}
