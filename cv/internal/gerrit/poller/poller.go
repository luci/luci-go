...
func schedule(ctx context.Context, luciProject string, after time.Time) error {
	...
	//     intervals.
	//   * keep polling load on Gerit at `1/pollInterval` per LUCI project;
	//   * avoid bursts of polls on Gerrit, ie distribute polls of diff projects
...
// projectOffset choses an offset per LUCI project in [0..pollInterval) range
// and aims for uniform distribuition across LUCI projects.
func projectOffset(luciProject string, pollInterval time.Duration) time.Duration {
	// Use fast hash function, as we don't need strong collission resistance.
	h := fnv.New32a()
	h.Write([]byte(luciProject))
	return time.Duration((int64(pollInterval) >> 32) * int64(h.Sum32()))
}
...
func pollWithConfig(ctx context.Context, luciProject string, meta config.Meta) error {
	...
	if saveErr := save(ctx, &stateBefore); saveErr != nil {
		// saving error dwarfs subpoller errors.
		return saveErr
	}
	err = internal.MostSevereError(errs)
	switch n, _ := errs.Summary(); {
	case n == len(errs):
		return errors.Annotate(err, "no progress on any poller, first error").Err()
	case err != nil:
	...
	err := datastore.RunInTransaction(ctx, func(ctx context.Context) error {
		...
		case err == datastore.ErrNoSuchEntity:
			if modified.EVersion > 0 {
				// At the beginning of the poll, we read an exisitng state.
