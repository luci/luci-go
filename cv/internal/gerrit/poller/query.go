...
func subpoll(ctx context.Context, luciProject string, meta config.Meta, sp *SubPoller) error {
	q := singleQuery{
...
func (q *singleQuery) scheduleTasks(ctx context.Context, changes []*gerritpb.ChangeInfo, forceNotifyPM bool) error {
	...
	if forceNotifyPM {
		...
		if err := notifyPMifKnown(ctx, q.luciProject, clids, 100); err != nil {
			return err
		}
	}
...
func scheduleRefreshTasks(ctx context.Context, luciProject, host string, changes []int64) error {
	...
	// Optimization: avoid RefreshGerritCL with forceNotifyPM=true if possible,
	// as this removes ability to de-duplicate.
	// Since all no longer matched CLs are under typically already in stored in
	...
	if err := notifyPMifKnown(ctx, luciProject, clids, 100); err != nil {
		return err
	}
