...
const (
	...
	//
	// Too low value here will first affects full polls, since they have to (re-)process all
	// interesting changes watched by a LUCI project.
	//
...
func (q *singleQuery) fetch(ctx context.Context, after time.Time) ([]*gerritpb.ChangeInfo, error) {
	...
	req := gerritpb.ListChangesRequest{
		Options: []gerritpb.QueryOption{
			...
			// CV doesnt actually need the whole current revision info, but that's the
			// only way to get actual "current_revision" hash.
...
func (q *singleQuery) scheduleTasks(ctx context.Context, changes []*gerritpb.ChangeInfo) error {
	// TODO(tandrii): optimize by checking if CV is interestd in the
