...
func partitionHostRepos(host string, repos []string, maxReposPerQuery int) []*SubPoller {
	...
	remaningRepos := repos[:0] // re-use the slice.
	for _, prefix := range prefixes {
		if shared := byPrefix[prefix]; len(shared) < minReposPerPrefixQuery {
			remaningRepos = append(remaningRepos, shared...)
		} else {
	...
	neededSubPollers := (len(remaningRepos)-1)/maxReposPerQuery + 1
	maxPerSubPoller := (len(remaningRepos)-1)/neededSubPollers + 1
	fmt.Printf("[WTF %d %d for %s\n", neededSubPollers, maxPerSubPoller, repos)
	sort.Strings(remaningRepos)
	for {
		sp := &SubPoller{Host: host}
		switch l := len(remaningRepos); {
		case l == 0:
			return subpollers
		case l <= maxPerSubPoller:
			sp.OrProjects = remaningRepos
		default:
			sp.OrProjects = remaningRepos[:maxPerSubPoller]
		}
		subpollers = append(subpollers, sp)
		remaningRepos = remaningRepos[len(sp.GetOrProjects()):]
	}
}
