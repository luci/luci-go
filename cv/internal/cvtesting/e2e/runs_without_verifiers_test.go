...
func TestCreatesMultiCLsFullRunSuccess(t *testing.T) {
	...
	Convey("CV creates 3 CLs Full Run, which succeeds", t, func() {
		...
		// Git Relationship: <base> -> ci1 -> ci2 -> ci3
		// ci1 depends on ci3 using Cq-Depend git footer.
		ci1 := gf.CI(
		...
		/////////////////////////    Run CV   ////////////////////////////////
		ct.LogPhase(ctx, "CV observes CLs")
		So(ct.PMNotifier.UpdateConfig(ctx, lProject), ShouldBeNil)
		...
		ct.LogPhase(ctx, "CV starts the Run")
		var r *run.Run
		ct.RunUntil(ctx, func() bool {
			r = ct.EarliestCreatedRunOf(ctx, lProject)
			return r != nil && r.Status == run.Status_RUNNING
		...
		if len(activeRuns) > 0 {
			So(activeRuns[0].Id, ShouldResemble, string(r.ID))
			So(excludedCLs, ShouldResemble, []string{
		...
		So(ci1.GetUpdated().AsTime(), ShouldHappenBefore, ci2.GetUpdated().AsTime())
