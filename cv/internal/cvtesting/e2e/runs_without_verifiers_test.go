...
func TestCreatesSingularQuickDryRunThenUpgradeToFullRunFailed(t *testing.T) {
	...
	Convey("CV creates 1 CL Quick Dry Run first and then upgrade to Full Run", t, func() {
		/////////////////////////    Setup   ////////////////////////////////
		ct := Test{}
		ctx, cancel := ct.SetUp()
		defer cancel()
		...
		ct.LogPhase(ctx, "User upgrades to Full Run but doesn't unvote Quick-Run")
		tStart2 := ct.Clock.Now()
		ct.GFake.MutateChange(gHost, gChange, func(c *gf.Change) {
			gf.CQ(+2, tStart2, gf.U("user-2"))(c.Info)
			gf.Updated(tStart2)(c.Info)
		})
		So(ct.MaxVote(ctx, gHost, gChange, quickLabel), ShouldEqual, 1) // still has vote
		var fr *run.Run
		ct.RunUntil(ctx, func() bool {
		...
		ct.RunUntil(ctx, func() bool {
			...
			return run.IsEnded(finalRun.Status) &&
				len(proj.State.GetComponents()) == 0 &&
				!cl.IncompleteRuns.ContainsSorted(fr.ID)
		})
