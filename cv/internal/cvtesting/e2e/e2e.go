...
func (t *Test) sweepTTQ(ctx context.Context) {
	if t.dsFlakiness > 0 {
		// TODO(tandrii): find a way to instantiate && launch Sweeper per test.
...
func init() {
	tq.Default.Sweeper = tq.NewInProcSweeper(tq.InProcSweeperOptions{SweepShards: 1})
}
