...
def main():
  ...
  checkout_repos(opts)
  cls = create_cls(opts, kind='internal')
...
def checkout_repos(opts):
  for kind, origin in KNOWN_REPOS.items():
    if os.path.exists(os.path.join(opts.workdir, kind)):
      continue
    git('clone', origin, kind, cwd=opts.workdir)


def create_cls(opts, kind):
  ...
  logging.info('Creating %d commits in %s %s', opts.number, kind, parent)
