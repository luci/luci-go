...
type Test struct {
	...
	GFake *gf.Fake
	// MockTreeClient is a mock client for Tree Status App.
	//
	// If nil, defaults to an always-open tree.
	MockTreeClient tree.Client
...
func (t *Test) SetUp() (ctx context.Context, deferme func()) {
	...
	if t.MockTreeClient == nil {
		t.MockTreeClient = &tree.MockClient{
	...
	ctx = tree.Install(ctx, t.MockTreeClient)
