...
//
// Such operations are generally long-running and non-idempotent. Therefore,
// Intent MUST be `Lease()`ed before `Fulfill()` to better achieve exclusive
// execution.
type Intent struct {
	_kind string `gae:"$kind,Intent"`
	// ID is intent id.
	ID common.IntentID `gae:"$id"`
	// Operations are all operations that are not fulfilled by this intent yet.
...
// Create creates a record for a new intent.
...
// Lease leases the intent for a given `duration`.
//
// Returns
...
//
// It saves the progress once an operation is done and eventually delete
// the intent if all operations completes successfully.
