...
// Package lease provides a way to acquire a "lock" on an external asset
// with expiration time so that concurrent processes/task executions can
// can achieve exclusive modification on the external asset.
package lease
...
type Lease struct {
	...
	// Number is 1-based auto-incremented number for each successful application.
	Number int64 `gae:",noindex"`
}

// ErrConflict is returned when Asset is in another lease term so that
// operations like `Apply`  can't proceed.
var ErrConflict = errors.New("Asset is in another lease term")

// Apply applies for a lease over the given `duration`.
//
// Returns ErrConflict if the asset is already in another lease term.
// Omits any value set in ExpirationTime and Number.
