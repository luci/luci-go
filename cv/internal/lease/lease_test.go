...
func TestLease(t *testing.T) {
	...
	Convey("Apply", t, func() {
		...
		application := Application{
			...
			Holder:     "holder",
			Payload:    []byte("Hey!"),
			ExpireTime: clock.Now(ctx).UTC().Add(1 * time.Minute),
		}
	...
	Convey("Terminate", t, func() {
		...
		Convey("Errors if lease is not current", func() {
			...
			So(err, ShouldBeNil)
			So(l.Terminate(ctx), ShouldErrLike, ErrConflict)
		})
	})
	...
	Convey("Extend", t, func() {
		...
		Convey("Works", func() {
			...
			expected := &Lease{
				ResourceID: rid,
				Holder:     "holder",
				ExpireTime: clock.Now(ctx).UTC().Add(2 * time.Minute),
				Token:      []byte{22, 63, 95, 15, 154, 98, 29, 114},
			}
		...
		Convey("Errors if lease doesn't exist in Datastore", func() {
			...
			err := l.Extend(ctx, 1*time.Minute)
			So(err, ShouldErrLike, "arget lease doesn't exist in datastore")
		})
