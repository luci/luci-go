...
func TestUpdateSnapshot(t *testing.T) {
	...
	Convey("UpdateSnapshot", t, func() {
		...
		Convey("new CL is created", func() {
			...
			err := UpdateSnapshot(ctx, eid, 0 /* uknown CLID */, snap)
			So(err, ShouldBeNil)
			cl, err := eid.Get(ctx)
			So(err, ShouldBeNil)
		...
		Convey("update existing", func() {
			cl, err := eid.GetOrInsert(ctx, func(*CL) { /* no snapshot atached yet*/ })
			So(err, ShouldBeNil)
...
func TestConcurrentUpdateSnapshot(t *testing.T) {
	...
	Convey("UpdateSnapshot called concurrently with flaky datastore", t, func() {
		epoch := testclock.TestRecentTimeUTC
		...
		So(cl.EVersion, ShouldBeLessThan, N+1) // N updates at most.
	})
}
