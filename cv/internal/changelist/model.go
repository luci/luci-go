...
// ExternalID is a unique CL ID determinstically constructed based on CL data.
//
...
type changeList struct {
	...
	ExternalID ExternalID `gae:"external_id,noindex"` // string. Indexed in CLMap entities.

	// Patchset is incremental number of the latest patchset (aka revision).
	Patchset int `gae:"patchset,noindex"`
	...
	// can be canceled.
	MinEquivalentPatchset int `gae:"min_eq_patchset,noindex"`
...
type clMap struct {
	...
	// InternalID is auto-generated by Datastore.
...
func (eid ExternalID) get(ctx context.Context) (*changeList, error) {
	...
	case err == datastore.ErrNoSuchEntity:
		// This should not happen in practice except for very old CL which is
		// being deleted due to retention policy. Log error but return it as
		// transient as it's expected that CLMap entity would be removed soon,
...
func (eid ExternalID) getOrInsert(ctx context.Context, populate func(cl *changeList)) (*changeList, error) {
	...
	err := datastore.RunInTransaction(ctx, func(ctx context.Context) error {
		...
		// Create new entry by writing both n
		cl = &changeList{
