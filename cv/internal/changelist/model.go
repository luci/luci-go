...
type AsDepMetaAddittion struct {
}
...
func (u UpdateFields) apply(cl *CL) (changed bool) {
	...
	switch {
	case u.AddAsDepMeta == nil:
	case cl.AsDepMeta == nil || cl.AsDepMeta.GetByProject() == nil:
		cl.AsDepMeta = u.AddAsDepMeta
		changed = true
	default:
		e := cl.AsDepMeta.GetByProject()
		for lProject, ameta := range u.AddAsDepMeta.GetByProject() {
		...
		if knownCLID == 0 {
			...
			case err == datastore.ErrNoSuchEntity:
				...
				_, err = insert(ctx, eid, func(cl *CL) {
					...
					cl.AsDepMeta = fields.AddAsDepMeta
				})
				return err
			case err != nil:
