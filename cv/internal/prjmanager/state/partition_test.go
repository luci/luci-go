			...
			s0 := &State{PB: &prjpb.PState{
				...
				Pcls: []*prjpb.PCL{
					...
					},
					{
						...
						// ConfigGroupIndexes: []int32{0},
						Deps: []*changelist.Dep{
							{Clid: 11, Kind: changelist.DepKind_SOFT},
							{Clid: 12, Kind: changelist.DepKind_HARD},
						},
						Status: prjpb.PCL_OK,
						Trigger: &run.Trigger{
				...
				CreatedPruns: []*prjpb.PRun{
					{Id: "chromium/8999-1-aa11", Clids: []int64{11}},
					{Id: "chromium/8999-1-aa13", Clids: []int64{13}},
				},
			}}

			cat := s0.categorizeCLs(ctx)
			So(cat.active, ShouldResemble, clidsSet{11: {}, 13: {}})
			So(cat.deps, ShouldResemble, clidsSet{12: {}})
