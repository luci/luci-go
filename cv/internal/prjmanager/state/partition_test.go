...
func TestRepartition(t *testing.T) {
	...
	ftt.Run("repartition works", t, func(t *ftt.Test) {
		...
		check := func(t testing.TB) {
			...
			assert.That(t, actual, should.Match(state.pclIndex), truth.LineContext())
		}
...
func TestPartitionSpecialCases(t *testing.T) {
	...
	ftt.Run("Special cases of partitioning", t, func(t *ftt.Test) {
		...
		t.Run("crbug/1217775", func(t *ftt.Test) {
			...
			assert.That(t, cat.active, should.Match(common.CLIDsSet{10: {}, 12: {}}))
			assert.That(t, cat.deps, should.Match(common.CLIDsSet{11: {}}))
			...
			// both Runs.
			assert.That(t, s1.PB.GetComponents(), should.Match([]*prjpb.Component{
				{
					Clids: []int64{10, 12},
					Pruns: []*prjpb.PRun{
						{Id: "chromium/8999-1-aa10", Clids: []int64{10}},
