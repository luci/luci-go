...
type RunBuilder struct {
	...
	cls []*changelist.CL
	// run is the read if already existing or the saved Run.
	run *run.Run
}
...
func (rb *RunBuilder) checkProjectState(ctx context.Context) {
	...
	rb.dsBatcher.register(ps, func(err error) error {
		...
		case ps.Status != Status_STARTED:
			return errors.Reason("project %q status is %s, expected STARTED", rb.LUCIProject, ps.Status.String()).Err()
...
func (ml *dsBatcher) reset() {
	ml.entities = ml.entities[:0]
	ml.callbacks = ml.callbacks[:0]
}
...
func (ml *dsBatcher) register(entity interface{}, callback func(error) error) {
	ml.entities = append(ml.entities, entity)
	ml.callbacks = append(ml.callbacks, callback)
}
...
func (ml *dsBatcher) get(ctx context.Context) error {
	err := datastore.Get(ctx, ml.entities)
	if err == nil {
		return ml.execCallbacks(nil)
	}
	...
	case len(errs) != len(ml.entities):
		panic(fmt.Errorf("%d errors for %d entities", len(errs), len(ml.entities)))
	default:
		return ml.execCallbacks(errs)
	}
}
...
// Aborts and returns the first non-nil error returned by a callback.
// Oherwise, returns nil.
func (ml *dsBatcher) put(ctx context.Context) error {
	err := datastore.Put(ctx, ml.entities)
	if err == nil {
		return ml.execCallbacks(nil)
	}
	switch errs, ok := err.(errors.MultiError); {
...
func (ml *dsBatcher) execCallbacks(errs errors.MultiError) error {
	if errs == nil {
		for _, f := range ml.callbacks {
