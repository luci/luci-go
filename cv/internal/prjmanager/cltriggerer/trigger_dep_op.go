...
func (op *triggerDepOp) isAlreadyVoted(ctx context.Context, depCL *changelist.CL) bool {
...
func (op *triggerDepOp) execute(ctx context.Context, gFactory gerrit.Factory, luciPrj string) error {
	...
	if op.isAlreadyVoted(ctx, depCL) {
	...
	return gFactory.MakeMirrorIterator(ctx).RetryIfStale(func(opt grpc.CallOption) error {
		if op.isCanceled.Load() {
			return nil
		}
...
func (op *triggerDepOp) makeSetReviewRequest(depCL *changelist.CL) *gerritpb.SetReviewRequest {
	...
	return &gerritpb.SetReviewRequest{
		...
		Tag: "autogenerated:trigger-dep-cl",
	}
}
