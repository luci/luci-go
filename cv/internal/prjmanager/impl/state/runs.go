...
func (s *State) addCreatedRuns(ctx context.Context, ids map[common.RunID]struct{}) error {
	...
	s.PB.Components, _ = s.PB.COWComponents(func(c *prjinternal.Component) *prjinternal.Component {
		...
		var toAdd []*prjinternal.PRun
		for idx, count := range matchedRunIdx {
			if count == len(runs[idx].CLs) {
				added[idx] = true
				toAdd = append(toAdd, prjinternal.MakePRun(runs[idx]))
			}
		}
		if len(toAdd) == 0 {
			return c
	...
	for i, r := range runs {
		if !added[i] {
			toAdd = append(toAdd, prjinternal.MakePRun(r))
		}
	}
...
func (s *State) removeFinishedRuns(ids map[common.RunID]struct{}) int {
	delIfFinished := func(r *prjinternal.PRun) *prjinternal.PRun {
	...
	removeFromComponent := func(c *prjinternal.Component) *prjinternal.Component {
		if len(ids) == 0 {
			return c
		}
