...
func TestUpdateConfig(t *testing.T) {
	...
	Convey("updateConfig works", t, func() {
		...
		Convey("initializes newly started project", func() {
			...
			So(s1.PB, ShouldResembleProto, &prjinternal.PState{
		...
		s1 := NewExisting(prjmanager.Status_STARTED, &prjinternal.PState{
			LuciProject:      ct.lProject,
			ConfigHash:       meta.Hash(),
			ConfigGroupNames: []string{"g0", "g1"},
			Pcls: []*prjinternal.PCL{
				{
					Clid:               int64(cl101.ID),
					Eversion:           1,
					ConfigGroupIndexes: []int32{0}, // g0
					Status:             prjinternal.PCL_OK,
			...
			Components: []*prjinternal.Component{
				{
					Clids: []int64{int64(cl101.ID)},
					Pruns: []*prjinternal.PRun{
						{
		...
		Convey("existing projects is updated without touching components", func() {
			...
			So(s2.PB, ShouldResembleProto, &prjinternal.PState{
				LuciProject:      ct.lProject,
				...
				Pcls: []*prjinternal.PCL{
					{
						Clid:     int64(cl101.ID),
						Eversion: 1,
						Status:   prjinternal.PCL_DELETED,
					},
					pb1.Pcls[1], // #202 didn't change.
					pb1.Pcls[2], // #203 didn't change.
				},
		...
		Convey("makePCL with full snapshot works", func() {
			...
			Convey("snapshot from diff project requires waiting", func() {
				...
				So(s1.makePCL(ctx, cl101), ShouldResembleProto, &prjinternal.PCL{
					Clid:     int64(cl101.ID),
					Eversion: int64(cl101.EVersion),
					Status:   prjinternal.PCL_UNKNOWN,
				})
			})
			...
			Convey("CL from diff project is unwatched", func() {
				...
				So(s1.makePCL(ctx, cl101), ShouldResembleProto, &prjinternal.PCL{
					Clid:     int64(cl101.ID),
					Eversion: int64(cl101.EVersion),
					Status:   prjinternal.PCL_UNWATCHED,
				})
			})
...
func TestOnCLsUpdated(t *testing.T) {
	...
	Convey("OnCLsUpdated works", t, func() {
		...
		s0 := NewExisting(prjmanager.Status_STARTED, &prjinternal.PState{
			LuciProject:      ct.lProject,
			ConfigHash:       meta.Hash(),
			ConfigGroupNames: []string{"g0", "g1"},
		})
		...
		Convey("One simple CL", func() {
			s1, sideEffect, err := s0.OnCLsUpdated(ctx, []*prjinternal.CLUpdated{
			...
			Convey("Noop based on EVersion", func() {
				s2, sideEffect, err := s1.OnCLsUpdated(ctx, []*prjinternal.CLUpdated{
	...
	Convey("OnRunsCreated and OnRunsFinished works", t, func() {
		...
		s1 := NewExisting(prjmanager.Status_STARTED, &prjinternal.PState{
			...
			Components: []*prjinternal.Component{
				{
					Clids: []int64{101},
					Pruns: []*prjinternal.PRun{{Id: ct.lProject + "/101-aaa", Clids: []int64{1}}},
				},
				{
					Clids: []int64{202, 203, 204},
				},
			},
			CreatedPruns: []*prjinternal.PRun{
...
func TestLoadActiveIntoPCLs(t *testing.T) {
	...
	Convey("loadActiveIntoPCLs works", t, func() {
		...
		state := NewExisting(prjmanager.Status_STARTED, &prjinternal.PState{
			LuciProject:      ct.lProject,
			ConfigHash:       meta.Hash(),
			ConfigGroupNames: []string{"g0", "g1"},
			DirtyComponents:  true,
		...
		Convey("just categorization", func() {
			state.PB.Pcls = sortPCLs([]*prjinternal.PCL{
				defaultPCL(cls[5]),
				defaultPCL(cls[6]),
				defaultPCL(cls[7]),
				defaultPCL(cls[8]),
				defaultPCL(cls[9]),
				{Clid: 12, Eversion: 1, Status: prjinternal.PCL_UNKNOWN},
			})
			state.PB.Components = []*prjinternal.Component{
				{
					Clids: i64sorted(cls[5].ID, cls[6].ID),
					Pruns: []*prjinternal.PRun{prjinternal.MakePRun(run56)},
				},
			...
			state.PB.CreatedPruns = []*prjinternal.PRun{prjinternal.MakePRun(run789)}
		...
		Convey("loads unloaded dependencies and active CLs without recursion", func() {
			state.PB.Pcls = []*prjinternal.PCL{
				defaultPCL(cls[3]), // depends on 2, which in turns depends on 1.
			}
			state.PB.CreatedPruns = []*prjinternal.PRun{prjinternal.MakePRun(run56)}
			pb := backupPB(state)
			...
			pb.Pcls = sortPCLs([]*prjinternal.PCL{
				defaultPCL(cls[2]),
				defaultPCL(cls[3]),
				defaultPCL(cls[5]),
				defaultPCL(cls[6]),
		...
		Convey("loads incomplete Run with unloaded deps", func() {
			...
			state.PB.CreatedPruns = []*prjinternal.PRun{prjinternal.MakePRun(runStale)}
			pb := backupPB(state)
	...
	Convey("repartition works", t, func() {
		...
		Convey("nothing to do, except resetting DirtyComponents", func() {
			Convey("totally empty", func() {
				...
				So(state.PB, ShouldResembleProto, &prjinternal.PState{})
			})
			Convey("1 active CL in 1 component", func() {
				cat.active.resetI64(1)
				state.PB.Components = []*prjinternal.Component{{Clids: []int64{1}}}
				state.PB.Pcls = []*prjinternal.PCL{{Clid: 1}}
				pb := backupPB(state)
			...
			Convey("1 active CL in 1 dirty component with 1 Run", func() {
				...
				state.PB.Components = []*prjinternal.Component{{
					Clids: []int64{1},
					Pruns: []*prjinternal.PRun{{Clids: []int64{1}, Id: "id"}},
					Dirty: true,
				}}
				state.PB.Pcls = []*prjinternal.PCL{{Clid: 1}}
				pb := backupPB(state)
		...
		Convey("CreatedRuns are moved into components", func() {
			Convey("Simple", func() {
				...
				state.PB.CreatedPruns = []*prjinternal.PRun{{Clids: []int64{1, 2}, Id: "id"}}
				orig := backupPB(state)
				...
				So(state.PB, ShouldResembleProto, &prjinternal.PState{
					...
					Components: []*prjinternal.Component{
						{
							Clids: []int64{1, 2},
							Pruns: []*prjinternal.PRun{{Clids: []int64{1, 2}, Id: "id"}},
							Dirty: true,
						},
					},
				})
			...
			Convey("Force-merge 2 existing components", func() {
				...
				state.PB.Components = []*prjinternal.Component{
					{Clids: []int64{1}, Pruns: []*prjinternal.PRun{{Clids: []int64{1}, Id: "1"}}},
					{Clids: []int64{2}, Pruns: []*prjinternal.PRun{{Clids: []int64{2}, Id: "2"}}},
				}
				state.PB.CreatedPruns = []*prjinternal.PRun{{Clids: []int64{1, 2}, Id: "12"}}
				orig := backupPB(state)
				...
				So(state.PB, ShouldResembleProto, &prjinternal.PState{
					...
					Components: []*prjinternal.Component{
						{
							Clids: []int64{1, 2},
							Pruns: []*prjinternal.PRun{ // must be sorted by ID
								{Clids: []int64{1}, Id: "1"},
		...
		Convey("Does all at once", func() {
			...
			state.PB.Components = []*prjinternal.Component{
				{Clids: []int64{1, 2, 3}, Pruns: []*prjinternal.PRun{{Clids: []int64{1}, Id: "1"}}},
				{Clids: []int64{4}, Pruns: []*prjinternal.PRun{{Clids: []int64{4}, Id: "4"}}},
				{Clids: []int64{5}, Pruns: []*prjinternal.PRun{{Clids: []int64{5}, Id: "5"}}},
			}
			state.PB.CreatedPruns = []*prjinternal.PRun{
			...
			So(state.PB, ShouldResembleProto, &prjinternal.PState{
				...
				Components: []*prjinternal.Component{
					{Clids: []int64{1, 2}, Dirty: true, Pruns: []*prjinternal.PRun{{Clids: []int64{1}, Id: "1"}}},
					{Clids: []int64{4, 5}, Dirty: true, Pruns: []*prjinternal.PRun{
						{Clids: []int64{4}, Id: "4"},
						{Clids: []int64{4, 5}, Id: "45"},
						{Clids: []int64{5}, Id: "5"},
					}},
					{Clids: []int64{6}, Dirty: true, Pruns: []*prjinternal.PRun{{Clids: []int64{6}, Id: "6"}}},
				},
			})
		})
	})
...
// wasn't modified.
func backupPB(s *State) *prjinternal.PState {
	ret := &prjinternal.PState{}
...
func defaultPCL(cl *changelist.CL) *prjinternal.PCL {
	p := &prjinternal.PCL{
		Clid:               int64(cl.ID),
...
func customPCL(cl *changelist.CL, override *prjinternal.PCL) *prjinternal.PCL {
	p := defaultPCL(cl)
...
func sortPCLs(vs []*prjinternal.PCL) []*prjinternal.PCL {
...
func sortByFirstCL(cs []*prjinternal.Component) []*prjinternal.Component {
