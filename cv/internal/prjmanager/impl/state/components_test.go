	...
	Convey("Component actions logic work in the abstract", t, func() {
		...
		makeDirtySetup := func(indexes ...int) {
			for _, i := range indexes {
				state.PB.GetComponents()[i].ReevalRequired = true
			}
			pb = backupPB(state)
		}

		unDirty := func(c *prjpb.Component) *prjpb.Component {
			if !c.GetReevalRequired() {
				panic(fmt.Errorf("must be dirty to unDirty"))
			}
			o := c.CloneShallow()
			o.ReevalRequired = false
			return o
		}
		...
		Convey("triage called on dirty components or when decision time is <= now", func() {
			...
			Convey("ExecDeferred", func() {
				...
				pb.Components[3].ReevalRequired = false
		...
		Convey("partial failure in triage", func() {
			...
			Convey("ExecDeferred", func() {
				...
				pb.Components[2].ReevalRequired = false
				pb.Components[3].ReevalRequired = false
				So(state2.PB, ShouldResembleProto, pb)
		...
		Convey("With Run Creation", func() {
			...
			Convey("100% success", func() {
				...
				So(err, ShouldBeNil)
				So(sideEffect, ShouldBeNil)
				pb.Components[1].ReevalRequired = false // must be saved, since Run Creation succeeded.
				So(state2.PB, ShouldResembleProto, pb)
			...
			Convey("Partial failure", func() {
				...
				So(findRunOf(1), ShouldNotBeNil)
				pb.Components[1].ReevalRequired = false
