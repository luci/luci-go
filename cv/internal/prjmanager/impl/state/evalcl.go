...
func (s *State) makePCL(ctx context.Context, cl *changelist.CL) *internal.PCL {
	...
	if uptodate := s.tryUsingApplicableConfigGroups(ap, pcl); !uptodate {
		// Unless Project's config has just been updated.
		s.setApplicableConfigGroups(cl.Snapshot, pcl)
	}
...
// setApplicableConfigGroups sets ConfigGroup indexes of PCL by matching
// it against project config directly.
//
// Modifies the passed PCL.
func (s *State) setApplicableConfigGroups(snapshot *changelist.Snapshot, pcl *internal.PCL) {
...
// per-CL errors, and a top level error if it can't be attributed to any CL.
//
// The per-CL error is not annotated and is nil if CL was loaded successfully.
