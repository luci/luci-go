...
func (s *State) reevalPCLs(ctx context.Context) error {
	...
	newPCLs := make([]*prjinternal.PCL, len(cls))
	for i, cl := range cls {
		old := s.PB.GetPcls()[i]
...
func (s *State) evalCLsFromDS(ctx context.Context, cls []*changelist.CL) error {
	...
	newPCLs := make([]*prjinternal.PCL, 0, len(oldPCLs)+len(cls))
	changed := false
	for i, cl := range cls {
...
// doens't exist.
func (s *State) filterOutUpToDate(events []*prjinternal.CLUpdated) map[common.CLID]int64 {
...
func (s *State) makePCLFromDS(ctx context.Context, cl *changelist.CL, err error, old *prjinternal.PCL) (*prjinternal.PCL, error) {
	switch {
...
func (s *State) makePCL(ctx context.Context, cl *changelist.CL) *prjinternal.PCL {
	if s.cfgMatcher == nil {
	...
	case ap == nil:
		...
		pcl.Status = prjinternal.PCL_UNWATCHED
		return pcl
...
func (s *State) setApplicableConfigGroups(ap *changelist.ApplicableConfig_Project, snapshot *changelist.Snapshot, pcl *prjinternal.PCL) {
...
func (s *State) tryUsingApplicableConfigGroups(ap *changelist.ApplicableConfig_Project, pcl *prjinternal.PCL) bool {
