...
func (s *State) repartition(ctx context.Context) error {
	_, err := s.loadActiveIntoPCLs(ctx)
	if err != nil {
		return err
	}
...
// loadActiveIntoPCLs ensures PCLs contain all active CLs.
//
// Doesn't guarantee that all their deps are loaded.
func (s *State) loadActiveIntoPCLs(ctx context.Context) (*categorizedCLs, error) {
...
type categorizedCLs struct {
	...
	//
	// Similar to active CLs of incomplete Run, these CLs may be not even be
	// watched by this project or be with status=DELETED, but such state should be
	// temporary.
	deps clidsSet
...
func (s *State) categorizeCLs() *categorizedCLs {
	...
	for _, pcl := range pcls {
		id := pcl.GetClid()
		if isActiveStandalonePCL(pcl) && !res.active.hasI64(id) {
			fmt.Printf("[WTF: %d active!\n", id)
			res.active.addI64(id)
		}
	}
