...
func (s *State) repartition(cat *categorizedCLs) {
	...
	// Remove unused PCLs while updating pclIndex.
	pcls := make([]*prjinternal.PCL, 0, len(s.PB.GetPcls())-len(cat.unused))
...
func (s *State) planPartition(cat *categorizedCLs) disjointset.DisjointSet {
	...
	s.PB.IterIncompleteRuns(func(r *prjinternal.PRun, _ *prjinternal.Component) (stop bool) {
		clids := r.GetClids()
...
func (s *State) execPartition(cat *categorizedCLs, d disjointset.DisjointSet) []*prjinternal.Component {
	...
	reused := make(map[int]*prjinternal.Component, d.Count())
	var leftComponents []*prjinternal.Component
	...
	created := make(map[int]*prjinternal.Component, d.Count()-len(reused))
	...
	addPRuns := func(pruns []*prjinternal.PRun) {
		for _, r := range pruns {
	...
	out := make([]*prjinternal.Component, 0, len(reused)+len(created))
	for _, c := range reused {
		out = append(out, c)
	}
	for _, c := range created {
		prjinternal.SortPRuns(c.Pruns)
		out = append(out, c)
	}
	return out
}
