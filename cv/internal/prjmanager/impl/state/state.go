...
func NewInitial(luciProject string) *State {
	return &State{
		...
		PB:     &prjinternal.PState{LuciProject: luciProject},
	}
}

// NewExisting returns state from its parts.
func NewExisting(status prjmanager.Status, pb *prjinternal.PState) *State {
	return &State{
		Status: status,
		PB:     pb,
	}
...
func (s *State) OnCLsUpdated(ctx context.Context, events []*prjinternal.CLUpdated) (*State, SideEffect, error) {
	s.ensureNotYetCloned()
