...
type Actor struct {
	...
	// cls provides info per each CL of the component.
	cls map[int64]*clInfo
	// reverseDeps maps dep -> which CLs depend on it.
	//
	// Only for CLs with clInfo.ready being true.
	reverseDeps map[int64][]int64
	// Tracks visitedCLs during stageNewRuns.
	...
	// toPurge are CLs that should be newly purged.
