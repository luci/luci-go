		...
		Convey("Creates Runs", func() {
			...
			Convey("Singular", func() {
				Convey("OK", func() {
					...
					oldC := &prjpb.Component{Clids: []int64{33}, ReevalRequired: true}
					res := mustTriage(oldC)
					So(res.NewValue, ShouldResembleProto, undirty(oldC))
					So(res.CLsToPurge, ShouldBeEmpty)
				...
				Convey("Noop when Run already exists", func() {
					_, pcl := putPCL(33, singIdx, run.DryRun, ct.Clock.Now())
					pm.pb.Pcls = []*prjpb.PCL{pcl}
					oldC := &prjpb.Component{Clids: []int64{33}, ReevalRequired: true, Pruns: makePruns("run-id", 33)}
					res := mustTriage(oldC)
					So(res.NewValue, ShouldResembleProto, undirty(oldC))
					So(res.CLsToPurge, ShouldBeEmpty)
					So(res.RunsToCreate, ShouldBeEmpty)
				...
				Convey("EVersion mismatch is a transient error", func() {
					...
					So(datastore.Put(ctx, cl), ShouldBeNil)
					pm.pb.Pcls = []*prjpb.PCL{pcl}
					err := failTriage(&prjpb.Component{Clids: []int64{33}, ReevalRequired: true})
				...
				Convey("OK with resolved deps", func() {
					...
					oldC := &prjpb.Component{Clids: []int64{32, 33}, ReevalRequired: true}
					res := mustTriage(oldC)
					So(res.NewValue, ShouldResembleProto, undirty(oldC))
					So(res.CLsToPurge, ShouldBeEmpty)
			...
			Convey("Combinable", func() {
				Convey("OK after obeying stabilization delay", func() {
					...
					pm.pb.Pcls = []*prjpb.PCL{pcl31, pcl32, pcl33}
					oldC := &prjpb.Component{Clids: []int64{31, 32, 33}, ReevalRequired: true}
					res := mustTriage(oldC)
					So(res.NewValue.GetReevalRequired(), ShouldBeFalse)
				...
				Convey("Noop if there is existing Run encompassing all the CLs", func() {
					...
					oldC := &prjpb.Component{Clids: []int64{31, 32}, ReevalRequired: true, Pruns: makePruns("runID", 31, 32)}
					res := mustTriage(oldC)
					So(res.NewValue.GetReevalRequired(), ShouldBeFalse)
					So(res.CLsToPurge, ShouldBeEmpty)
					So(res.RunsToCreate, ShouldBeEmpty)
				...
				Convey("Component growing to N+1 CLs that could form a Run while Run on N CLs is already running", func() {
					...
					oldC := &prjpb.Component{Clids: []int64{31, 41, 51}, ReevalRequired: true, Pruns: makePruns("41-31", 31, 41)}
					res := mustTriage(oldC)
					So(res.NewValue.GetReevalRequired(), ShouldBeFalse)
					So(res.NewValue.GetDecisionTime(), ShouldBeNil)
					So(res.CLsToPurge, ShouldBeEmpty)
