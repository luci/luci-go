	...
	Convey("Triage works", t, func() {
		...
		Convey("Creates Runs", func() {
			...
			Convey("Combinable", func() {
				...
				Convey("Doesn't react to submits in progress", func() {
					// Simulate scenario of a multi-CL Run being submitted.
					pcl31 := &prjpb.PCL{Clid: 31, Eversion: 10, Status: prjpb.PCL_OK, Submitted: true}
					_, pcl41 := putPCL(41, combIdx, run.FullRun, ct.Clock.Now().Add(-time.Hour), 31)
					_, pcl51 := putPCL(51, combIdx, run.FullRun, ct.Clock.Now().Add(-time.Hour), 31, 41)
					pm.pb.Pcls = []*prjpb.PCL{pcl31, pcl41, pcl51}
					oldC := &prjpb.Component{Clids: []int64{31, 41, 51}, Dirty: true, Pruns: makePruns("sub/mitting", 31, 41, 51)}
					res := mustTriage(oldC)
					So(res.NewValue.GetDirty(), ShouldBeFalse)
					So(res.NewValue.GetDecisionTime(), ShouldBeNil)
					So(res.CLsToPurge, ShouldBeEmpty)
