...
func (a *Actor) stageNewRunsCombo(ctx context.Context, info *clInfo, cg *config.ConfigGroup) (time.Time, error) {
	// First, maximimize Run's CL # to include not only all reachable dependencies, but also
	...
	if combo.withNotYetLoadedDeps != nil {
		return a.postponeDueNotYetLoadedDeps(ctx, combo.withNotYetLoadedDeps)
	}
	if len(combo.notReady) > 0 {
		...
		// Check conclusions meet reality.
		var err error
		clids := make([]int64, len(combo.notReady))
		for i, x := range combo.notReady {
			clids[i] = x.pcl.GetClid()
			if x.purgeReason == nil && x.purgingCL == nil && len(x.runIndexes) == 0 {
		...
		return a.postponeDueNotReadyDeps(ctx, &combo)
	}
	...
	//   have been considered, but dismissed for eiter of 2 reasons:
	...
	if runCounts := combo.runsOverlap(); len(runCounts) > 0 {
		for runIndex, cnt := range runCounts { // take first and only Run
			...
			case l > cnt:
				...
				return a.postponeDueExistingRunDiffScope(ctx, &combo, prun)
...
func (a *Actor) expandCombo(clid int64, result *combo) {
	info := a.cls[clid]
	if info == nil {
		// Can only happen if clid is not yet loaded dep (otherwise dep would be in
		// this component, and hence info would be set).
		return
	}
...
func (a *Actor) postponeDueNotReadyDeps(ctx context.Context, combo *combo) (time.Time, error) {
	// TODO(tandrii): for safety, this should not wait forever.
	sb := strings.Builder{}
	fmt.Fprintf(
...
func (a *Actor) postponeDueExistingRunDiffScope(ctx context.Context, combo *combo, r *prjpb.PRun) (time.Time, error) {
	// TODO(tandrii): for safety, this should not wait forever.
...
func (a *Actor) makeRunBuilder(ctx context.Context, combo *combo, cg *config.ConfigGroup) (*prjmanager.RunBuilder, error) {
	...
	// TODO(tandrii): handle email is unset case.
