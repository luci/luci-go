...
func (a *Actor) stageNewRuns(ctx context.Context, now time.Time) (time.Time, error) {
	a.visitedCLs = map[int64]struct{}{}
	defer func() { a.visitedCLs = nil }() // won't be useful afterwards.

	var t time.Time
	for clid, info := range a.cls {
		switch nt, err := a.stageNewRunsFrom(ctx, now, clid, info); {
		case err != nil:
			return t, err
...
func (a *Actor) stageNewRunsFrom(ctx context.Context, now time.Time, clid int64, info *clInfo) (time.Time, error) {
	...
	if cg.Content.GetCombineCls() == nil {
		return a.stageNewRunsSingle(ctx, now, info, cg)
	}
	return a.stageNewRunsCombo(ctx, now, info, cg)
}

func (a *Actor) stageNewRunsSingle(ctx context.Context, now time.Time, info *clInfo, cg *config.ConfigGroup) (time.Time, error) {
...
func (a *Actor) stageNewRunsCombo(ctx context.Context, now time.Time, info *clInfo, cg *config.ConfigGroup) (time.Time, error) {
