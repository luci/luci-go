	...
	if runs := combo.overlappingRuns(); len(runs) > 0 {
		for runIndex, sharedCLsCount := range runs { // take the first and only Run
			...
			case l > sharedCLsCount:
				...
				//
				// This may happen when existing multi-CL Run is broken due to to new
				// configuration making one or more of its CLs no longer in scope for
				// the same LUCI project or CQ group.
			...
			default:
				...
				// Howver, the problem with this potential approach is that if user
...
func (a *runStage) postponeDueToNotReadyDeps(ctx context.Context, combo *combo) (*runcreator.Creator, time.Time, error) {
	// TODO(tandrii): for safety, this should not wait forever.
	logging.Warningf(ctx, "%s waits for not yet read CLs", combo)
	return nil, time.Time{}, nil
}
