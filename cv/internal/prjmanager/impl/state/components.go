...
func (s *State) prepareComponentActions(ctx context.Context) ([]cAction, error) {
	...
	for i, c := range s.PB.GetComponents() {
		if actor, skip := evaluator.shouldEvaluate(pclGetter, c); !skip {
...
// componentPreevaluator checks if a components has to be deeply evaluated.
type componentPreevaluator interface {
	...
	shouldEvaluate(p pclGetter, c *prjpb.Component) (ce componentActor, skip bool)
}
