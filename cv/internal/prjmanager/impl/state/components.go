		...
		for i, oldC := range s.PB.GetComponents() {
			...
			work <- func() (err error) {
				defer paniccatcher.Catch(func(p *paniccatcher.Panic) {
					...
					logging.Errorf(ctx, "caught panic dbg: now %s prior %s component %s", now, oldWhen, protojson.Format(oldC))
					logging.Errorf(ctx, "caught panic dbg: PCLs\n%s", protojson.Format(s.PB))
		...
		for _, a := range actions {
			...
			work <- func() (err error) {
				defer paniccatcher.Catch(func(p *paniccatcher.Panic) {
					atomic.AddInt32(&paniced, 1)
					logging.Errorf(ctx, "caught panic %s:\n%s", p.Reason, p.Stack)
					logging.Errorf(ctx, "caught panic: component %s", protojson.Format(components[a.componentIndex]))
					err = errors.Reason("caught panic: %s", p.Reason).Err()
				})
