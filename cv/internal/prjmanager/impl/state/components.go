...
type actorSupporter interface {
	...
	ConfigGroup(index int32) (*cfgpb.ConfigGroup, error)
}
...
func (a *actorSupporterImpl) ConfigGroup(index int32) (*cfgpb.ConfigGroup, error) {
...
func (a *componentActorImpl) triageDeps(pcl *prjpb.PCL, cgIndex int32, cg *cfgpb.ConfigGroup) triagedDeps {
	res := triagedDeps{}
	for _, dep := range pcl.GetDeps() {
		dPCL := a.s.PCL(dep.GetClid())
		if ptrSlice := res.categorize(pcl, cgIndex, cg, dPCL); ptrSlice != nil {
			*ptrSlice = append(*ptrSlice, dep)
		}
...
//
// Categories are exclusive. "Good" deps are not recorded here to avoid
// unnecesary allocations in the most common case, but they do affect
// lastTriggered time.
type triagedDeps struct {
	...
	// submitted are already submitted deps wathed by this project, though not
	// necessarily the same config group as the dependent CL.
	...
	// dep is triggered. Set iff applicable ConfigGroup has `combine_cls`.
	lastTriggered time.Time
}

// categorize returns *slice to which dep should be added or nil if dep is OK.
//
// pcl is dependent PCL, which must be triggered.
// Its dep is represented by dPCL.
func (t *triagedDeps) categorize(pcl *prjpb.PCL, cgIndex int32, cg *cfgpb.ConfigGroup, dPCL *prjpb.PCL) *[]*changelist.Dep {
	if dPCL == nil {
		return &t.notYetLoaded
	}
	...
	default:
		panic(fmt.Errorf("unrecognized CL %d dep %d status %d %s", pcl.GetClid(), dPCL.GetClid(), s, s))
	}
	...
	if cg.GetCombineCls() == nil {
		return t.categorizeSingle(tr, dtr)
	}
	return t.categorizeCombinable(tr, dtr)
}

func (t *triagedDeps) categorizeCombinable(tr, dtr *run.Trigger) *[]*changelist.Dep {
	// During the `combine_cls.stablization_delay` since the last triggered CL in
	// a group, a user can change their mind. Since the full group of CLs isn't known here,
	// categorization decision may or may not be final.
	switch {
	case dtr.GetMode() == tr.GetMode():
		return nil // Happy path.
	case dtr == nil:
		return &t.incompatMode
	default:
...
func (t *triagedDeps) categorizeSingle(tr, dtr *run.Trigger) *[]*changelist.Dep {
