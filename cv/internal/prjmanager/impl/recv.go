...
func (pm *projectManager) GetState(ctx context.Context) (eventbox.State, eventbox.EVersion, error) {
...
// isn't saved.
func (pm *projectManager) GetEVersion(ctx context.Context) (eventbox.EVersion, error) {
	p := &prjmanager.Project{ID: pm.luciProject}
	switch err := datastore.Get(ctx, p); {
	case err == datastore.ErrNoSuchEntity:
		return 0, nil
