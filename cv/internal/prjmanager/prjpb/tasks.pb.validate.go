...
// Validate checks the field values on TriggeringCLsTask with the rules defined
// in the proto definition for this message. If any rules are violated, the
// first error encountered is returned, or nil if there are no violations.
func (m *TriggeringCLsTask) Validate() error {
	return m.validate(false)
}

// ValidateAll checks the field values on TriggeringCLsTask with the rules
// defined in the proto definition for this message. If any rules are
// violated, the result is a list of violation errors wrapped in
// TriggeringCLsTaskMultiError, or nil if none found.
func (m *TriggeringCLsTask) ValidateAll() error {
	return m.validate(true)
}

func (m *TriggeringCLsTask) validate(all bool) error {
	if m == nil {
		return nil
	}
	...
	for idx, item := range m.GetTriggeringCls() {
		_, _ = idx, item
		...
		if all {
			...
			case interface{ ValidateAll() error }:
				if err := v.ValidateAll(); err != nil {
					errors = append(errors, TriggeringCLsTaskValidationError{
						field:  fmt.Sprintf("TriggeringCls[%v]", idx),
						reason: "embedded message failed validation",
						cause:  err,
					})
				}
			...
			if err := v.Validate(); err != nil {
				return TriggeringCLsTaskValidationError{
					field:  fmt.Sprintf("TriggeringCls[%v]", idx),
					reason: "embedded message failed validation",
					cause:  err,
				}
	...
	if len(errors) > 0 {
		return TriggeringCLsTaskMultiError(errors)
	}

	return nil
}

// TriggeringCLsTaskMultiError is an error wrapping multiple validation errors
// returned by TriggeringCLsTask.ValidateAll() if the designated constraints
// aren't met.
type TriggeringCLsTaskMultiError []error

// Error returns a concatenation of all the error messages it wraps.
func (m TriggeringCLsTaskMultiError) Error() string {
	var msgs []string
	for _, err := range m {
		msgs = append(msgs, err.Error())
	}
...
// AllErrors returns a list of validation violation errors.
func (m TriggeringCLsTaskMultiError) AllErrors() []error { return m }

// TriggeringCLsTaskValidationError is the validation error returned by
// TriggeringCLsTask.Validate if the designated constraints aren't met.
type TriggeringCLsTaskValidationError struct {
	field  string
	reason string
	cause  error
	key    bool
...
// Field function returns field value.
func (e TriggeringCLsTaskValidationError) Field() string { return e.field }

// Reason function returns reason value.
func (e TriggeringCLsTaskValidationError) Reason() string { return e.reason }

// Cause function returns cause value.
func (e TriggeringCLsTaskValidationError) Cause() error { return e.cause }

// Key function returns key value.
func (e TriggeringCLsTaskValidationError) Key() bool { return e.key }

// ErrorName returns error name.
func (e TriggeringCLsTaskValidationError) ErrorName() string {
	return "TriggeringCLsTaskValidationError"
}

// Error satisfies the builtin error interface
func (e TriggeringCLsTaskValidationError) Error() string {
	cause := ""
	if e.cause != nil {
		cause = fmt.Sprintf(" | caused by: %v", e.cause)
	}
	...
	return fmt.Sprintf(
		"invalid %sTriggeringCLsTask.%s: %s%s",
		key,
		e.field,
		e.reason,
		cause)
...
var _ error = TriggeringCLsTaskValidationError{}
