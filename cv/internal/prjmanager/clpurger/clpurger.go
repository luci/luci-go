...
func loadConfigGroups(ctx context.Context, task *prjpb.PurgeCLTask) ([]*prjcfg.ConfigGroup, error) {
	...
	res := make([]*prjcfg.ConfigGroup, 0, 1)
	for _, pcgs := range task.GetConfigGroups() {
		for _, id := range pcgs.GetConfigGroups() {
			cg, err := prjcfg.GetConfigGroup(ctx, pcgs.GetLuciProject(), prjcfg.ConfigGroupID(id))
			if err != nil {
