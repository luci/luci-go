...
func UpdateConfig(ctx context.Context, luciProject string) error {
	return send(ctx, luciProject, &prjinternal.Event{
		Event: &prjinternal.Event_NewConfig{
			NewConfig: &prjinternal.NewConfig{},
		},
	})
}
...
func Poke(ctx context.Context, luciProject string) error {
	return send(ctx, luciProject, &prjinternal.Event{
		Event: &prjinternal.Event_Poke{
			Poke: &prjinternal.Poke{},
		},
	})
}
...
func NotifyCLUpdated(ctx context.Context, luciProject string, clid common.CLID, eversion int) error {
	return send(ctx, luciProject, &prjinternal.Event{
		Event: &prjinternal.Event_ClUpdated{
			ClUpdated: &prjinternal.CLUpdated{
				Clid:     int64(clid),
...
func NotifyRunCreated(ctx context.Context, runID common.RunID) error {
	return sendWithoutDispatch(ctx, runID.LUCIProject(), &prjinternal.Event{
		Event: &prjinternal.Event_RunCreated{
			RunCreated: &prjinternal.RunCreated{
				RunId: string(runID),
			},
		},
	})
...
func NotifyRunFinished(ctx context.Context, runID common.RunID) error {
	return send(ctx, runID.LUCIProject(), &prjinternal.Event{
		Event: &prjinternal.Event_RunFinished{
			RunFinished: &prjinternal.RunFinished{
				RunId: string(runID),
			},
		},
	})
...
func send(ctx context.Context, luciProject string, e *prjinternal.Event) error {
	if err := sendWithoutDispatch(ctx, luciProject, e); err != nil {
		return err
	}
	return prjinternal.Dispatch(ctx, luciProject, time.Time{} /*asap*/)
}

func sendWithoutDispatch(ctx context.Context, luciProject string, e *prjinternal.Event) error {
