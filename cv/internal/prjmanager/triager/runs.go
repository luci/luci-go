...
func (a *runStage) stageNewRunsFrom(ctx context.Context, clid int64, info *clInfo) (*runcreator.Creator, time.Time, error) {
	...
	switch err := datastore.Get(ctx, &existing); err != nil {
	...
	default:
		...
		logging.Errorf(ctx, "Run %q already exists, finalized %s ago; will purge CLs with reused triggers", existing.ID, len(combo.all))
		// TODO(crbug/1220909):  mark each CL for purging.
		return nil, time.Time{}, nil
	}
}
