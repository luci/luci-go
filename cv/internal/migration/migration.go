...
func (m *MigrationServer) FetchRunStatus(ctx context.Context, req *migrationpb.FetchRunStatusRequest) (resp *migrationpb.FetchRunStatusResponse, err error) {
	...
	// active attempts set. Most likely, by this time CV hasn't finalized the
	// Run. However, Runs are finalized if either:
	//  (a) CV decided to stop the Run (e.g. CQ+2 vote removed)
	//  (b) CQDaemon decided to fail the Run (e.g. tryjob failed)
	// This function is useful for CQDaemon only in case of (a).
	if !run.IsEnded(r.Status) {
		...
		// TODO(tandrii,yiwzhang): when (a) happens, store somewhere a better
		// reason. Store a most likely reason in the meantime.
		res.Event = "patch_failed"
		res.Extra = "CQ bit was unchecked."
		return res, nil
	}
	// Easy case of finalized Run, but not really important as this code path is
	...
	case run.Status_CANCELLED:
		...
		res.Extra = "CQ bit was unchecked."
	default:
		// Not worth panicking/error in this case, just report what's known.
