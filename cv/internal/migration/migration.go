...
func (m *MigrationServer) PostGerritMessage(ctx context.Context, req *migrationpb.PostGerritMessageRequest) (resp *migrationpb.PostGerritMessageResponse, err error) {
	...
	// TruncateMessage breaks the check above, but it's better than
	// post 2 duplicate comments than to continuously fail posting one comment.
	...
	_, err = gc.SetReview(ctx, makeGerritSetReviewRequest(r, ci, msg, req.GetSendEmail()))
