...
// Close stops CQDaemon fake and waits for it complete.
...
//
// NOTE: the FetchExcludedCLs will still be applied on the output of hte
// callback.
type CandidatesClbk func() []*migrationpb.Run

// Set (re)sets callback func called per CQDaemon loop if CQDaemon is in charge.
//
...
func (cqd *CQDFake) migrationApiContext(ctx context.Context) context.Context {
	// TODO(tandrii): find a way to fake X-LUCI-Project.
	// For now, pretend CQD belongs to a the `migration.allowGroup` not imported
