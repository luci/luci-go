	...
	ftt.Run("Diff works", t, func(t *ftt.Test) {
		...
		t.Run("Compare definitions", func(t *ftt.Test) {
			...
			t.Run("Target has one extra", func(t *ftt.Test) {
				...
				assert.That(t, res.UnchangedDefs[shared], should.Match(shared))
				assert.Loosely(t, res.RemovedDefs, should.BeEmpty)
			})
			...
			t.Run("Target has one changed", func(t *ftt.Test) {
				...
				check := func(t testing.TB) {
					...
					assert.That(t, res.ChangedDefs[baseDef], should.Match(targetDef), truth.LineContext())
					assert.Loosely(t, res.ChangedDefsReverse, should.HaveLength(1), truth.LineContext())
					assert.That(t, res.ChangedDefsReverse[targetDef], should.Match(baseDef), truth.LineContext())
			...
			t.Run("Multiple Definitions", func(t *ftt.Test) {
				...
				assert.That(t, res.ChangedDefs[builder3], should.Match(builder3Changed))
				assert.That(t, res.ChangedDefs[builderDiffProj], should.Match(builderDiffProjChanged))
				assert.Loosely(t, res.ChangedDefsReverse, should.HaveLength(2))
				assert.That(t, res.ChangedDefsReverse[builder3Changed], should.Match(builder3))
				assert.That(t, res.ChangedDefsReverse[builderDiffProjChanged], should.Match(builderDiffProj))
				assert.Loosely(t, res.UnchangedDefs, should.HaveLength(1))
				assert.That(t, res.UnchangedDefs[builder1], should.Match(builder1))
			})
		})
