...
// Tasks returns all Run tasks sorted by ETA and task proto message name.
func Tasks(in tqtesting.TaskList) []proto.Message {
	ret := make([]proto.Message, len(in))
	for i, t := range in.SortByETA() {
		switch v := t.Payload.(type) {
	...
	sort.SliceStable(ret, func(i, j int) bool {
		return ret[i].ProtoReflect().Descriptor().Name() < ret[j].ProtoReflect().Descriptor().Name()
	})
	return ret
}
...
// AssertNotInEventbox asserts none of the target events exist in the Eventbox.
