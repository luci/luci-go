...
func TestIDs(t *testing.T) {
	...
	Convey("IDs helper works", t, func() {
		...
		var nill IDs
		So(nill.WithoutSorted(MakeIDs("1")), ShouldEqual, nil)
...
func assertSameSlice(a, b IDs) {
	// Go doesn't allow comparing slices,
	// so compare their contents and ensure pointers to the first element are the
	// same.
