...
func NewExporter(tqd *tq.Dispatcher, bqc cvbq.Client) *Exporter {
	...
	return &Exporter{tqd, bqc}
}

// Schedule enqueues a task to send a row to BQ for a Run.
func (s *Exporter) Schedule(ctx context.Context, runID string) error {
	return tq.AddTask(ctx, &tq.Task{Payload: &ExportRunToBQTask{RunId: runID}})
}
