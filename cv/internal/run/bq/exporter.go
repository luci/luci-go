...
const exportRunToBQTaskClass = "export-run-to-bq"
...
func NewExporter(tqd *tq.Dispatcher, bqc cvbq.Client) *Exporter {
	...
	tqd.RegisterTaskClass(tq.TaskClass{
		...
		Handler: func(ctx context.Context, payload proto.Message) error {
			...
			err := SendRun(ctx, exporter.bqc, common.RunID(task.RunId))
...
func (s *Exporter) Schedule(ctx context.Context, t *ExportRunToBQTask) error {
	return tq.AddTask(ctx, &tq.Task{Payload: t})
}
