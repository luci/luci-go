// Copyright 2021 The LUCI Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//      http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

package bq

import (
	"testing"
	"time"

	. "github.com/smartystreets/goconvey/convey"
	"google.golang.org/protobuf/types/known/timestamppb"

	"go.chromium.org/luci/common/proto/gerrit"
	. "go.chromium.org/luci/common/testing/assertions"
	cvbqpb "go.chromium.org/luci/cv/api/bigquery/v1"
	migrationpb "go.chromium.org/luci/cv/api/migration"
	"go.chromium.org/luci/gae/service/datastore"

	"go.chromium.org/luci/cv/internal/changelist"
	"go.chromium.org/luci/cv/internal/common"
	"go.chromium.org/luci/cv/internal/cvtesting"
	"go.chromium.org/luci/cv/internal/migration"
	"go.chromium.org/luci/cv/internal/run"
)

func TestMakeAttempt(t *testing.T) {

	Convey("makeAttempt", t, func() {
		ct := cvtesting.Test{}
		ctx, cancel := ct.SetUp()
		defer cancel()
		now := ct.Clock.Now().UTC()

		Convey("given run with one CL", func() {
			runID := common.MakeRunID("lproject", now, 1, []byte("aaa"))
			r := &run.Run{
				ID:            common.RunID(runID),
				Status:        run.Status_SUCCEEDED,
				ConfigGroupID: "sha256:deadbeefdeadbeef/cgroup",
				CreateTime:    now,
				StartTime:     now.Add(time.Minute * 2),
				EndTime:       now.Add(time.Minute * 25),
				CLs:           common.CLIDs{1},
				Submission: &run.Submission{
					Cls:          []int64{1},
					SubmittedCls: []int64{},
				},
				Mode: run.DryRun,
			}
			So(datastore.Put(ctx, r), ShouldBeNil)
			So(datastore.Put(ctx, &run.RunCL{
				ID:  1,
				Run: datastore.MakeKey(ctx, run.RunKind, string(runID)),
				Detail: &changelist.Snapshot{
					LuciProject:           "lproject",
					Patchset:              2,
					MinEquivalentPatchset: 2,
					Kind: &changelist.Snapshot_Gerrit{
						Gerrit: &changelist.Gerrit{
							Host: "foo-review.googlesource.com",
							Info: &gerrit.ChangeInfo{
								Number:  111,
								Project: "gproject",
								Ref:     "refs/heads/main",
							},
						},
					},
				},
				Trigger: &run.Trigger{Time: timestamppb.New(now)},
			}), ShouldBeNil)
			So(nil, ShouldBeNil)

			a, err := makeAttempt(ctx, common.RunID(runID))
			So(err, ShouldBeNil)
			So(a, ShouldResembleProto, &cvbqpb.Attempt{
				Key:                  string(runID),
				LuciProject:          "lproject",
				ConfigGroup:          "cgroup",
				ClGroupKey:           "",
				EquivalentClGroupKey: "",
				StartTime:            timestamppb.New(now),
				EndTime:              timestamppb.New(now.Add(25 * time.Minute)),
				GerritChanges: []*cvbqpb.GerritChange{
					&cvbqpb.GerritChange{
						Host:                       "foo-review.googlesource.com",
						Project:                    "gproject",
						Change:                     111,
						Patchset:                   2,
						EarliestEquivalentPatchset: 2,
						TriggerTime:                timestamppb.New(now),
						Mode:                       cvbqpb.Mode_DRY_RUN,
						SubmitStatus:               cvbqpb.GerritChange_FAILURE,
					},
				},
				Status:    cvbqpb.AttemptStatus_SUCCESS,
				Substatus: cvbqpb.AttemptSubstatus_NO_SUBSTATUS,
			})
		})

		Convey("given run with partially submitted CLs", func() {
			runID := common.MakeRunID("lproject", now, 1, []byte("f00"))
			r := &run.Run{
				ID:            runID,
				Status:        run.Status_SUCCEEDED,
				ConfigGroupID: "sha256:deadbeefdeadbeef/cgroup",
				CreateTime:    now,
				StartTime:     now.Add(time.Minute * 2),
				EndTime:       now.Add(time.Minute * 30),
				CLs:           common.CLIDs{3, 5},
				Submission: &run.Submission{
					// CL 3 was submitted, but CL 5 was not.
					Cls:          []int64{3, 5},
					SubmittedCls: []int64{3},
				},
				Mode: run.FullRun,
			}
			So(datastore.Put(ctx, r), ShouldBeNil)
			So(datastore.Put(ctx, &run.RunCL{
				ID:  3,
				Run: datastore.MakeKey(ctx, run.RunKind, string(runID)),
				Detail: &changelist.Snapshot{
					LuciProject:           "lproject",
					Patchset:              12,
					MinEquivalentPatchset: 11,
					Kind: &changelist.Snapshot_Gerrit{
						Gerrit: &changelist.Gerrit{
							Host: "foo-review.googlesource.com",
							Info: &gerrit.ChangeInfo{
								Number:  333,
								Project: "gproject",
								Ref:     "refs/heads/main",
							},
						},
					},
				},
				Trigger: &run.Trigger{Time: timestamppb.New(now)},
			}), ShouldBeNil)
			So(datastore.Put(ctx, &run.RunCL{
				ID:  5,
				Run: datastore.MakeKey(ctx, run.RunKind, string(runID)),
				Detail: &changelist.Snapshot{
					LuciProject:           "lproject",
					Patchset:              6,
					MinEquivalentPatchset: 4,
					Kind: &changelist.Snapshot_Gerrit{
						Gerrit: &changelist.Gerrit{
							Host: "foo-review.googlesource.com",
							Info: &gerrit.ChangeInfo{
								Number:  555,
								Project: "gproject",
								Ref:     "refs/heads/main",
							},
						},
					},
				},
				Trigger: &run.Trigger{Time: timestamppb.New(now)},
			}), ShouldBeNil)

			a, err := makeAttempt(ctx, common.RunID(runID))
			So(err, ShouldBeNil)
			So(a, ShouldResembleProto, &cvbqpb.Attempt{
				Key:                  string(runID),
				LuciProject:          "lproject",
				ConfigGroup:          r.ConfigGroupID.Name(),
				ClGroupKey:           "",
				EquivalentClGroupKey: "",
				StartTime:            timestamppb.New(r.CreateTime),
				EndTime:              timestamppb.New(r.EndTime),
				GerritChanges: []*cvbqpb.GerritChange{
					&cvbqpb.GerritChange{
						Host:                       "foo-review.googlesource.com",
						Project:                    "gproject",
						Change:                     333,
						Patchset:                   12,
						EarliestEquivalentPatchset: 11,
						TriggerTime:                timestamppb.New(now),
						Mode:                       cvbqpb.Mode_FULL_RUN,
						SubmitStatus:               cvbqpb.GerritChange_SUCCESS,
					},
					&cvbqpb.GerritChange{
						Host:                       "foo-review.googlesource.com",
						Project:                    "gproject",
						Change:                     555,
						Patchset:                   6,
						EarliestEquivalentPatchset: 4,
						TriggerTime:                timestamppb.New(now),
						Mode:                       cvbqpb.Mode_FULL_RUN,
						SubmitStatus:               cvbqpb.GerritChange_FAILURE,
					},
				},
				Status:    cvbqpb.AttemptStatus_SUCCESS,
				Substatus: cvbqpb.AttemptSubstatus_NO_SUBSTATUS,
			})
		})
	})
}

func TestUsingAttemptInfoFromCQDaemon(t *testing.T) {
	Convey("fetchCQDAttempt", t, func() {
		ct := cvtesting.Test{}
		ctx, cancel := ct.SetUp()
		defer cancel()
		t := ct.Clock.Now().UTC()

		runID := common.MakeRunID("chromium", t, 1, []byte("cafecafe"))

		attempt := &cvbqpb.Attempt{
			Key:                  "f001234",
			ConfigGroup:          "maingroup",
			LuciProject:          "chromium",
			ClGroupKey:           "b004321",
			EquivalentClGroupKey: "c003333",
			StartTime:            timestamppb.New(t.Add(-2 * time.Minute)),
			EndTime:              timestamppb.New(t.Add(-1 * time.Minute)),
			Builds: []*cvbqpb.Build{
				{
					Id:       423143214321,
					Host:     "cr-buildbucket.appspot.com",
					Origin:   cvbqpb.Build_NOT_REUSED,
					Critical: false,
				},
			},
			GerritChanges: []*cvbqpb.GerritChange{
				{
					Host:                       "https://chromium-review.googlesource.com/",
					Project:                    "chromium/src",
					Change:                     11111,
					Patchset:                   7,
					EarliestEquivalentPatchset: 6,
					Mode:                       cvbqpb.Mode_FULL_RUN,
					SubmitStatus:               cvbqpb.GerritChange_PENDING,
				},
			},
			Status:               cvbqpb.AttemptStatus_SUCCESS,
			Substatus:            cvbqpb.AttemptSubstatus_NO_SUBSTATUS,
			HasCustomRequirement: false,
		}

		vr := &migration.VerifiedCQDRun{
			ID: runID,
			Payload: &migrationpb.ReportVerifiedRunRequest{
				Run: &migrationpb.Run{
					Attempt: attempt,
					Id:      string(runID),
					Cls:     []*migrationpb.RunCL{},
				},
			},
		}
		So(datastore.Put(ctx, vr), ShouldBeNil)

		Convey("returns Attempt from datastore", func() {
			a, err := fetchCQDAttempt(ctx, runID)
			So(err, ShouldBeNil)
			So(a, ShouldResembleProto, attempt)
		})

		Convey("returns nil when not in datastore", func() {
			runID := common.MakeRunID("x", t, 1, []byte("aaa"))
			a, err := fetchCQDAttempt(ctx, runID)
			So(err, ShouldBeNil)
			So(a, ShouldBeNil)
		})
	})

	Convey("reconcileAttempts", t, func() {
		ct := cvtesting.Test{}
		_, cancel := ct.SetUp()
		defer cancel()
		now := ct.Clock.Now().UTC()

		Convey("overwrites fields using values from CQD", func() {
			runID := common.MakeRunID("chromium", now, 1, []byte("cafec0ffee"))
			a := &cvbqpb.Attempt{
				Key:                  string(runID),
				LuciProject:          "chromium",
				ConfigGroup:          "cgroup",
				ClGroupKey:           "",
				EquivalentClGroupKey: "",
				StartTime:            timestamppb.New(now),
				EndTime:              timestamppb.New(now.Add(5 * time.Minute)),
				Status:               cvbqpb.AttemptStatus_FAILURE,
				Substatus:            cvbqpb.AttemptSubstatus_NO_SUBSTATUS,
			}
			cqda := &cvbqpb.Attempt{
				Key:                  "f001234",
				ConfigGroup:          "maingroup",
				LuciProject:          "chromium",
				ClGroupKey:           "b004321",
				EquivalentClGroupKey: "c003333",
				StartTime:            timestamppb.New(now),
				EndTime:              timestamppb.New(now.Add(5 * time.Minute)),
				Builds: []*cvbqpb.Build{
					{
						Id:       423143214321,
						Host:     "cr-buildbucket.appspot.com",
						Origin:   cvbqpb.Build_NOT_REUSED,
						Critical: false,
					},
				},
				Status:               cvbqpb.AttemptStatus_FAILURE,
				Substatus:            cvbqpb.AttemptSubstatus_FAILED_TRYJOBS,
				HasCustomRequirement: true,
			}
			So(reconcileAttempts(a, cqda), ShouldResembleProto, &cvbqpb.Attempt{
				Key:                  string(runID),
				LuciProject:          "chromium",
				ConfigGroup:          "cgroup",
				ClGroupKey:           "",
				EquivalentClGroupKey: "",
				StartTime:            timestamppb.New(now),
				EndTime:              timestamppb.New(now.Add(5 * time.Minute)),
				Builds: []*cvbqpb.Build{
					{
						Id:       423143214321,
						Host:     "cr-buildbucket.appspot.com",
						Origin:   cvbqpb.Build_NOT_REUSED,
						Critical: false,
					},
				},
				Status:               cvbqpb.AttemptStatus_FAILURE,
				Substatus:            cvbqpb.AttemptSubstatus_FAILED_TRYJOBS,
				HasCustomRequirement: true,
			})
		})

		Convey("does not overwrite substatus in case of success", func() {
			runID := common.MakeRunID("chromium", now, 1, []byte("cafe"))
			a := &cvbqpb.Attempt{
				Key:                  string(runID),
				LuciProject:          "chromium",
				ConfigGroup:          "cgroup",
				ClGroupKey:           "",
				EquivalentClGroupKey: "",
				StartTime:            timestamppb.New(now),
				EndTime:              timestamppb.New(now.Add(5 * time.Minute)),
				Status:               cvbqpb.AttemptStatus_SUCCESS,
				Substatus:            cvbqpb.AttemptSubstatus_NO_SUBSTATUS,
			}
			cqda := &cvbqpb.Attempt{
				Key:                  "f001234",
				ConfigGroup:          "maingroup",
				LuciProject:          "chromium",
				ClGroupKey:           "b004321",
				EquivalentClGroupKey: "c003333",
				StartTime:            timestamppb.New(now),
				EndTime:              timestamppb.New(now.Add(5 * time.Minute)),
				// Anomalous status/substatus combination, not expected to
				// actually appear in valid data.
				Status:    cvbqpb.AttemptStatus_SUCCESS,
				Substatus: cvbqpb.AttemptSubstatus_FAILED_TRYJOBS,
			}
			So(reconcileAttempts(a, cqda), ShouldResembleProto, &cvbqpb.Attempt{
				Key:                  string(runID),
				LuciProject:          "chromium",
				ConfigGroup:          "cgroup",
				ClGroupKey:           "",
				EquivalentClGroupKey: "",
				StartTime:            timestamppb.New(now),
				EndTime:              timestamppb.New(now.Add(5 * time.Minute)),
				Status:               cvbqpb.AttemptStatus_SUCCESS,
				Substatus:            cvbqpb.AttemptSubstatus_NO_SUBSTATUS,
			})
		})
	})
}
