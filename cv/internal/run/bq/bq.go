...
func makeAttempt(ctx context.Context, id common.RunID) (*cvbqpb.Attempt, error) {
	...
	for _, cl := range runCLs {
		gerritChanges = append(gerritChanges, toGerritChange(cl, submittedSet, r.Mode))
	}
	...
	a := &cvbqpb.Attempt{
		...
		Builds: nil,
		Status: attemptStatus(ctx, r.Status),
		...
		// would set status = SUCCESS, substatus = FAILED_SUBMIT.)
...
func reconcileAttempts(a, cqda *cvbqpb.Attempt) *cvbqpb.Attempt {
	...
	// Copy over attempt status and substatus. For example, in the case
	// where checks are passed but submit fails, status will be SUCCESS,
	// for backwards compatibility.
	//
	// Note: Substatus generally indicates a failure reason, which is
	// known once one of the checks fails. CQDaemon may specify
	// a substatus in the case of abort (substatus: MANUAL_CANCEL)
	// or failure (FAILED_TRYJOBS etc.). In other cases, CQDaemon
	// Attempt substatus.
...
func attemptStatus(ctx context.Context, s run.Status) cvbqpb.AttemptStatus {
	switch s {
	...
	default:
		logging.Errorf(ctx, "Unexpected attempt status %q", s)
