...
func (tr *triageResult) triage(ctx context.Context, item eventbox.Event) {
	...
	case *eventpb.Event_SubmissionCompleted:
		...
		case sc.GetAttempt() == tr.submissionCompletedEvents.attempt && sc.GetResult() != tr.submissionCompletedEvents.result:
			panic(fmt.Errorf("recived mismatched result [%s and %s] for same submission attempt %d ", sc.GetResult(), tr.submissionCompletedEvents.result, sc.GetAttempt()))
		}
