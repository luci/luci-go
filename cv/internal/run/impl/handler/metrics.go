...
var (
	...
	metricPickupLatencyS = metric.NewCumulativeDistribution(
		"cv/pickup/latency",
		"Time between triggering CQ and when CQ processing actually starts",
	...
	metricPickupLatencyAdjustedS = metric.NewCumulativeDistribution(
		...
		"Time between triggering CQ and when CQ processing actually starts but adjusted by the configured stabilization delay",
		&types.MetricMetadata{Units: types.Seconds},
		distribution.GeometricBucketer(math.Pow(10, 0.04), 100),
		field.String("project"),
	)
