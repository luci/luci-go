...
func TestSubmitter(t *testing.T) {
	Convey("Submitter", t, func() {
		...
		const (
			lProject = "test_proj"
			gHost    = "test-proj-review.example.com"
			treeURL  = "tree.example.com"
		)
		...
		Convey("PreCondition Failure", func() {
			...
			Convey("Submit queue not acquired", func() {
				...
				So(log, memlogger.ShouldHaveLog, logging.Warning, "run is no longer current in submit queue")
			})
			Convey("Tree closed", func() {
				...
				So(log, memlogger.ShouldHaveLog, logging.Warning, "tree \"tree.example.com\" is closed when submission starts")
			})
		...
		Convey("Submit successfully", func() {
			So(s.submit(ctx), ShouldBeNil)
			verifyRunReleased(s.runID)
			for _, clid := range s.clids {
				runtest.AssertReceivedCLSubmitted(ctx, s.runID, clid)
				c := ct.GFake.GetChange(gHost, int(clid))
				So(c.Info.GetStatus(), ShouldEqual, gerritpb.ChangeStatus_MERGED)
			}
			runtest.AssertReceivedSubmissionCompleted(ctx, s.runID,
				&eventpb.SubmissionCompleted{
		...
		Convey("Submit fails permanently when", func() {
			Convey("No submit privilege", func() {
				// submit cl(1) successfully but lack of submission right to cl(2).
				...
				So(ct.GFake.GetChange(gHost, 1).Info.GetStatus(), ShouldEqual, gerritpb.ChangeStatus_MERGED)
				runtest.AssertNotReceivedCLSubmitted(ctx, s.runID, 2)
				So(ct.GFake.GetChange(gHost, 2).Info.GetStatus(), ShouldEqual, gerritpb.ChangeStatus_NEW)
				runtest.AssertReceivedSubmissionCompleted(ctx, s.runID,
					&eventpb.SubmissionCompleted{
						Result:  eventpb.SubmissionResult_FAILED_PERMANENT,
						Attempt: 2,
						FatalMessage: "CV couldn't submit your CL because CV is not " +
			...
			Convey("A new revision is uploaded ", func() {
				...
				runtest.AssertReceivedSubmissionCompleted(ctx, s.runID,
					&eventpb.SubmissionCompleted{
						Result:  eventpb.SubmissionResult_FAILED_PERMANENT,
						Attempt: 2,
						FatalMessage: fmt.Sprintf("Gerrit rejected submission with "+
							"error: rpc error: code = FailedPrecondition desc = revision "+
							"%s is not current revision\nHint: rebasing CL in Gerrit UI "+
							"and re-submitting through CQ usually works",
							ci2.GetCurrentRevision()),
					},
				)
			})
		})

		Convey("Change is already merged", func() {
			ct.GFake.MutateChange(gHost, 1, func(c *gf.Change) {
			...
			// for Submit RPC. But the subsequent GetChange will figure out
			// Change has been merged already and consider submission as a success.
			So(s.submit(ctx), ShouldBeNil)
			verifyRunReleased(s.runID)
			for _, clid := range s.clids {
				runtest.AssertReceivedCLSubmitted(ctx, s.runID, clid)
