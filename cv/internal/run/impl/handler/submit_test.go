		...
		for _, status := range []run.Status{run.Status_RUNNING, run.Status_WAITING_FOR_SUBMISSION} {
			Convey(fmt.Sprintf("When status is %s", status), func() {
				...
				Convey("Revisit after 1 mintues if tree is closed", func() {
					...
					// Must release the submit queue completely
					current, waitlist, err := submit.LoadCurrentAndWaitlist(ctx, rid)
					So(err, ShouldBeNil)
					So(current, ShouldNotEqual, rs.Run.ID)
					So(waitlist.Index(rid), ShouldBeLessThan, 0)
				})
