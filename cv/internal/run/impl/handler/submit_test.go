		...
		Convey("When status is SUBMITTING", func() {
			...
			Convey("Continue submission if TaskID matches and within deadline", func() {
				rs.Run.Submission = &run.Submission{
					Deadline:     timestamppb.New(now.Add(10 * time.Minute)), // with in deadline
					AttemptCount: 1,
			...
			Convey("Sends Poke if TaskID matches doesn't match and within deadline", func() {
				rs.Run.Submission = &run.Submission{
					Deadline:     timestamppb.New(now.Add(10 * time.Minute)), // with in deadline
					AttemptCount: 1,
					TaskId:       "task-bar",
...
func TestOnCLSubmitted(t *testing.T) {
	...
	Convey("OnCLSubmitted", t, func() {
		...
		Convey("Error for unknown CLs", func() {
			...
			So(err, ShouldErrLike, "received CLSubmitted event for cls don't belong to this Run: [9 11]")
			So(res, ShouldBeNil)
		})
	})
}
