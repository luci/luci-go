			...
			Convey("Deadline has expired", func() {
				...
				Convey("None of the CLs are submitted", func() {
					...
					for _, ci := range []*gerritpb.ChangeInfo{ci1, ci2} {
						...
						So(lastMessage, ShouldContainSubstring, "CV failed to submit any CLs in the Run after exhausted all the allocated time")
				...
				Convey("CLs partially submitted", func() {
					...
					So(change.Info.GetMessages(), ShouldNotBeEmpty)
					lastMessage := change.Info.GetMessages()[len(change.Info.GetMessages())-1].Message
					So(lastMessage, ShouldContainSubstring, "CV is running out of time to submit all your CLs in the Run and they are partially submitted.")
					...
					So(lastMessage, ShouldContainSubstring, "Please try resubmitting through CV. If the error persists, please contact LUCI team https://bit.ly/3sMReYs.")
					for _, vote := range change.Info.GetLabels()[trigger.CQLabelName].GetAll() {
						So(vote.GetValue(), ShouldEqual, 0)
					}
				})
