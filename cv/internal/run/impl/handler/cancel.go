...
func (impl *Impl) Cancel(ctx context.Context, rs *state.RunState) (*Result, error) {
	...
	switch yes, err := migrationcfg.IsCQDUsingMyRuns(ctx, rs.Run.ID.LUCIProject()); {
	case err != nil:
		return nil, err
	case yes && rs.Run.DelayCancelUntil.IsZero():
	...
	case yes && clock.Now(ctx).Before(rs.Run.DelayCancelUntil):
