	...
	default: // precondition check passed
		if cls, err := loadRunCLs(ctx, s.clids, s.runID); err != nil {
			...
			if fatalMsg, err := s.submitCLs(dctx, cls); err != nil {
				sc = s.computeResultEvent(dctx, err, fatalMsg)
			}
		}
	}
...
// for transient failure. For fatal error, RM will fail the submission and
// post `fatalMsg` on all not-yet-submitted CLs and notify the users.
// Therefore, please be aware of what's included in the `fatalMsg` to avoid
...
func (s submitter) submitCLs(ctx context.Context, cls []*run.RunCL) (fatalMsg string, err error) {
	for _, cl := range cls {
		...
		}, retry.LogCallback(ctx, fmt.Sprintf("submit cl %d", cl.ID)))
		switch {
		case err == nil:
		case fatalMsg != "":
			return fatalMsg, err
...
func (s submitter) checkPrecondition(ctx context.Context) (passed bool, err error) {
	...
	if s.deadline.Sub(clock.Now(ctx)) < 0 {
