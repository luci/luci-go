...
type submitter struct {
	...
	runID common.RunID
	// deadline indicates when this submission should be stopped.
	deadline time.Time
	// attempt is the current submission attempt count.
	attempt int32
	// treeURL is used at the beginning of submission to verify if tree is
	// closed.
	treeURL string
	// clids should only contains ids of cls to be submitted.
	//
	// Should in submission order
	clids common.CLIDs
...
func (s submitter) submit(ctx context.Context) error {
	...
	switch passed, err := s.verifyPreCondition(ctx); {
	case err != nil:
		errors.Log(ctx, err)
	...
	default: // precondition check passed
		// Set aside 500ms so that there will be enough time to release queue
		// and notify RM.
		dctx, cancel := clock.WithDeadline(ctx, s.deadline.Add(-500*time.Millisecond))
		defer cancel()
...
// will fail the submission and will post `fatalMsg` on the CL to notify the
...
func (s submitter) submitImpl(ctx context.Context) (fatalMsg string, err error) {
	...
	for _, cl := range cls {
		if err := s.submitCL(ctx, cl); err != nil {
			if fatalMsg = triageGerritErr(err); fatalMsg != "" {
				// Reconstruct the error in case it is accidentally tagged with
				// transient.
				return fatalMsg, errors.New(err.Error())
			}
	...
	return defaultFatalMsg, nil
}

func (s submitter) verifyPreCondition(ctx context.Context) (passed bool, err error) {
	...
	if s.deadline.Before(clock.Now(ctx)) {
...
func (s submitter) submitCL(ctx context.Context, cl *run.RunCL) error {
	...
	if _, ok := s.cachedGCByHost[host]; !ok {
		...
		if s.cachedGCByHost == nil {
			s.cachedGCByHost = make(map[string]gerrit.Client)
		}
	...
	// Sometimes, Gerrit may return error but change is submitted successfully.
	// Load the change again to check whether it is actually submitted.
...
func triageGerritErr(err error) string {
	...
	case codes.PermissionDenied:
		return "CV couldn't submit your CL because CV is not " +
	...
	case codes.FailedPrecondition:
		...
		return fmt.Sprintf("Gerrit rejected submission with error: %s\n"+
			"Hint: rebasing CL in Gerrit UI and re-submitting through CQ "+
			"usually works", err)
	case codes.ResourceExhausted, codes.Internal:
		return ""
	default:
		return fmt.Sprintf("CV failed to submit your change because of unexpected error from Gerrit: %s", err)
	}
}
...
func notifyCLSubmitted(ctx context.Context, runID common.RunID, clid common.CLID) error {
	...
	// to Run's mailbox, but does not dispatch the task. This is because it is
	// okay to process all such events together to record the submission result
	// for each individual CLs after the attempt to submit completes. Waking up
	// RM unnecessarily MAY increase the contention of RM.
