...
func cancelNotSubmittedCLTriggers(ctx context.Context, runID common.RunID, submission *run.Submission, sc *eventpb.SubmissionCompleted, cg *prjcfg.ConfigGroup) error {
	...
	case sc.GetClFailures() != nil:
		...
		l := len(failed) + len(pending)
		wg.Add(l)
		errs := make(errors.MultiError, l)
		...
		// cancel triggers of CLs that CV never attempts to submit.
		var sb strings.Builder
		fmt.Fprint(&sb, "CV didn't attempt to submit this CL because CV failed to submit its dependent CL(s):")
		// TODO(yiwzhang): Once CV learns how to submit multi-CLs in parallel, this
		// should be optimized to print out failed CLs that each pending CLs depends
		// on instead of printing out all failed CLs. i.e. consider a CL group.
		//   A (submitted)
		//   |
		//   |--> B (failed) --> D (pending)
		//   |
