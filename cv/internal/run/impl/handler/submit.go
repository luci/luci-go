...
func continueSubmissionIfPossible(ctx context.Context, rs *state.RunState, rm RM) (*Result, error) {
	...
	case expired:
		...
		case submittedCnt == 0: // None submitted
			rs.Run.Status = run.Status_FAILED
			allCLIDs := common.MakeCLIDs(rs.Run.Submission.GetCls()...)
			allRunCLs, err := run.LoadRunCLs(ctx, rs.Run.ID, allCLIDs)
			if err != nil {
				return nil, err
			}
			runCLExternalIDs := make([]changelist.ExternalID, len(allRunCLs))
			urls := make([]string, len(allRunCLs))
			for i, runCL := range allRunCLs {
				runCLExternalIDs[i] = runCL.ExternalID
				url, err := runCL.ExternalID.URL()
				if err != nil {
					return nil, err
				}
				urls[i] = url
			}
			msg := fmt.Sprintf(noneSubmittedMsgFmt, strings.Join(urls, ", "))
			if err := cancelCLTriggers(ctx, rs.Run.ID, allRunCLs, runCLExternalIDs, msg); err != nil {
				return nil, err
			}
		case submittedCnt != len(rs.Run.Submission.GetCls()): // partially submitted
			rs.Run.Status = run.Status_FAILED
			allCLIDs := common.MakeCLIDs(rs.Run.Submission.GetCls()...)
			submitted := common.MakeCLIDs(rs.Run.Submission.GetSubmittedCls()...).Set()
			allRunCLs, err := run.LoadRunCLs(ctx, rs.Run.ID, allCLIDs)
			if err != nil {
				return nil, err
			}
			...
			if err := cancelCLTriggers(ctx, rs.Run.ID, unsubmittedRunCLs, runCLExternalIDs, msg); err != nil {
				return nil, err
			}
		default: // fully submitted
...
func cancelCLTriggers(ctx context.Context, runID common.RunID, toCancel []*run.RunCL, runCLExternalIDs []changelist.ExternalID, message string) error {
	...
	errs := errors.NewLazyMultiError(len(cls))
	...
	case err != nil:
		return err
	case errs.Get() != nil:
		return errors.Annotate(common.MostSevereError(errs.Get()), "failed to cancel unsubmitted cls").Err()
	}
	return nil
}
...
const (
	...
	noneSubmittedMsgFmt = "CV failed to submit any CLs in the Run after " +
		"exhausted all the allocated time.\nCLs: [%s]\nThis generally should " +
		"not happen, please contact LUCI team https://bit.ly/3sMReYs."
	partiallySubmittedMsgFmt = "CV is running out of time to submit all your " +
		"CLs in the Run and they are partially submitted.\nNot submitted: [%s]" +
		"\nSubmitted: [%s]\nPlease try resubmitting through CV. If the error " +
		"persists, please contact LUCI team https://bit.ly/3sMReYs."
