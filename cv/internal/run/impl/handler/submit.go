...
func (*Impl) OnCLSubmitted(ctx context.Context, rs *state.RunState, clids common.CLIDs) (*Result, error) {
	if status := rs.Run.Status; status != run.Status_SUBMITTING {
		return nil, errors.Reason("expected status %s, got %s", run.Status_SUBMITTING, status).Err()
	}
	rs = rs.ShallowCopy()
	...
	if len(submitted) > 0 {
		...
		unexpected.Sort()
		return nil, errors.Reason("received CLSubmitted event for cls don't belong to this Run: %v", unexpected).Err()
	}
	return &Result{State: rs}, nil
}
...
var ErrTransientSubmissionFailure error = errors.New("submission failed transiently", transient.Tag)
