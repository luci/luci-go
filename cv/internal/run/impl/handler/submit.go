...
func (impl *Impl) OnReadyForSubmission(ctx context.Context, rs *state.RunState) (*Result, error) {
	...
	case status == run.Status_RUNNING:
		// This could happen when this Run transitioned from RUNNING status to
		// WAITING_FOR_SUBMISSION, acquired the submit queue but failed before
		// saving the state transition. This Run is receiving this event because
...
func (s submitter) submit(ctx context.Context) error {
	...
	case cur != s.runID:
		logging.Errorf(ctx, "run no longer holds submit queue, currently held by %q", cur)
