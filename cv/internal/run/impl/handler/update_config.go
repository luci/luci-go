...
func formatNoLongerFeasibleRunReason(
	...
	// users.
	fmt.Fprintf(&buf, "Run is no longer feasible due to project config change to %q", newMeta.Hash())
	if len(unwatched) > 0 {
	...
	if len(multiple) > 0 {
		fmt.Fprintf(&buf, "%d CLs no match 2+ config groups:\n", len(multiple))
		for _, cl := range multiple {
			fmt.Fprintf(&buf, "  * %s\n", cl.ExternalID.MustURL())
		}
	}
	if len(diffTrigger) > 0 {
		fmt.Fprintf(&buf, "%d CLs have new triggers:\n", len(diffTrigger))
