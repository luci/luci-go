	...
	Convey("Poke", t, func() {
		...
		Convey("Recursive", func() {
			...
			for i := 0; i < 10; i++ {
				...
				ct.TQ.Run(ctx, tqtesting.StopAfterTask("manage-run")) // KickPokeRunTask
				ct.TQ.Run(ctx, tqtesting.StopAfterTask("poke-manage-run"))
			}
		})

		Convey("Existing event due during the interval", func() {
			nextETA := timestamppb.New(clock.Now(ctx).Add(30 * time.Second))
			...
			So(runtest.Tasks(ct.TQ.Tasks()), ShouldResembleProto, []proto.Message{
				&eventpb.KickPokeRunTask{RunId: string(runID), Eta: nextETA},
				&eventpb.PokeRunTask{RunId: string(runID)},
			})
		})
	})
}
