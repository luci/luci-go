...
func TestCLQueryBuilder(t *testing.T) {
	...
	ftt.Run("CLQueryBuilder works", t, func(t *ftt.Test) {
		...
		t.Run("CL without Runs", func(t *ftt.Test) {
			...
			assert.That(t, getAll(t, qb), should.Match(common.RunIDs(nil)))
		})

		t.Run("CL with some Runs", func(t *ftt.Test) {
			qb := CLQueryBuilder{CLID: clB}
			assert.That(t, getAll(t, qb), should.Match(common.RunIDs{bond4, rust1}))
		})

		t.Run("CL with all Runs", func(t *ftt.Test) {
			qb := CLQueryBuilder{CLID: clA}
			assert.That(t, getAll(t, qb), should.Match(common.RunIDs{bond9, bond4, bond2, dart5, dart3, rust1, xero7}))
		})
		...
		t.Run("Filtering by Project and Min with diff project", func(t *ftt.Test) {
			...
			assert.That(t, getAll(t, qb), should.Match(common.RunIDs{dart5, dart3}))
		...
		t.Run("Before", func(t *ftt.Test) {
			...
			assert.That(t, getAll(t, qb), should.Match(common.RunIDs{bond2}))
		...
		t.Run("After", func(t *ftt.Test) {
			...
			assert.That(t, getAll(t, qb), should.Match(common.RunIDs{bond9}))
		...
		t.Run("Obeys limit and returns correct page token", func(t *ftt.Test) {
			...
			assert.NoErr(t, err)
			assert.That(t, idsOf(runs1), should.Match(common.RunIDs{bond9}))
			assert.Loosely(t, pageToken1, should.NotBeNil)

			qb = qb.PageToken(pageToken1)
			assert.That(t, qb.MinExcl, should.Match(bond9))
			runs2, pageToken2, err := qb.LoadRuns(ctx)
			assert.NoErr(t, err)
			assert.That(t, idsOf(runs2), should.Match(common.RunIDs{bond4}))
