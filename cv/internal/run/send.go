...
import (
	...
	"go.chromium.org/luci/cv/internal/run/runinternal"
)

// Start tells RunManager to start the given run.
func Start(ctx context.Context, runID common.RunID) error {
	return send(ctx, runID, &runinternal.Event{
		Event: &runinternal.Event_Start{
			Start: &runinternal.Start{},
		},
	})
}
...
func Poke(ctx context.Context, runID common.RunID) error {
	return send(ctx, runID, &runinternal.Event{
		Event: &runinternal.Event_Poke{
			Poke: &runinternal.Poke{},
		},
	})
}
...
func UpdateConfig(ctx context.Context, runID common.RunID, hash string, eversion int64) error {
	return send(ctx, runID, &runinternal.Event{
		Event: &runinternal.Event_UpdateConfig{
			UpdateConfig: &runinternal.UpdateConfig{
				Hash:     hash,
				Eversion: eversion,
			},
		},
...
func Cancel(ctx context.Context, runID common.RunID) error {
	return send(ctx, runID, &runinternal.Event{
		Event: &runinternal.Event_Cancel{
			Cancel: &runinternal.Cancel{},
		},
	})
}

func send(ctx context.Context, runID common.RunID, evt *runinternal.Event) error {
	...
	return runinternal.Dispatch(ctx, rid, time.Time{} /*asap*/)
}
