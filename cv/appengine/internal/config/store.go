...
type ProjectConfig struct {
	...
	// has CV config file.
	Enabled bool
	// UpdatedTime is the latest timestamp when this ProjectConfig was updated.
	...
	// and prefixed with hashing algorithm (e.g. sha256:deadbeefdeadbeef).
	//
	// The hash string is an hex-encoded string of the first 8 bytes (i.e. 16
	// char in length) of sha256(binary serialized Config). See `computeHash()`.
...
//
// It is primarily used for cleanup purpose to decide which `Hash` can be
// and its corresponding `ConfigGroup`s can be safely deleted.
type ConfigHashInfo struct {
	...
	// EVersion is largest version of ProjectConfig that this `Hash` maps to.
	//
	...
	// but only one new Hash). Only the largest EVersions matters when cleanup
	// job runs (i.e. CV will keep the last 5 EVersions).
	EVersion int64 `gae:",noindex"`
	// ImportTime is the last imported timestamp of this `Hash`.
	ImportTime time.Time `gae:",noindex"`
...
func putConfigGroups(ctx context.Context, cfg *pb.Config, project, hash string) error {
	...
	if err != nil {
		return errors.Annotate(err, "checking existence of ConfigGroups").Err()
	}
	...
	if err := datastore.Put(ctx, entities); err != nil {
		return errors.Annotate(err, "put ConfigGroups").Err()
	}
	return nil
}
