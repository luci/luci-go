...
// char in length) of sha256(deterministically binary serailied Config proto).
// Note that, determinitic marshalling does NOT guarantee the same output
...
func putConfigGroups(ctx context.Context, cfg *pb.Config, project, hash string) error {
	...
	// TODO(yiwzhang/tandrii): batch up to ~10 entities to avoid hitting
	// 10MiB limit of request size.
