...
func makeConfigGroupID(configHash string, cg *pb.ConfigGroup, index int) ConfigGroupID {
	name := cg.Name
	if name == "" {
...
func writeConfigGroups(ctx context.Context, cfg *pb.Config, project, configHash string, version int64) ([]ConfigGroupID, error) {
	if len(cfg.GetConfigGroups()) == 0 {
		return nil, nil
	}

	projKey := datastore.MakeKey(ctx, projectConfigKind, project)
	now := clock.Now(ctx)
	ids := make([]ConfigGroupID, len(cfg.GetConfigGroups()))
	for index, cg := range cfg.ConfigGroups {
		id := makeConfigGroupID(configHash, cg, index)
		ids[index] = id
		entity := ConfigGroup{
			Project:           projKey,
			ID:                id,
		...
		if err := datastore.Put(ctx, &entity); err != nil {
			return nil, errors.Annotate(err, "failed to write ConfigGroup %q for project %q", id, project).Tag(transient.Tag).Err()
		}
	}
	return ids, nil
}
