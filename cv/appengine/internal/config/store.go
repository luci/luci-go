...
type ProjectConfig struct {
	...
	// its Change Verifier config file.
	Enabled bool `gae:"enabled"`
	// LatestRevision is the content hash of most recent Change Verifier config
	// file CV imported from LUCI Config.
	LatestRevision string `gae:"latest_rev"`
	// UpdatedTime is the timestamp when the `LatestRevision` is imported.
	UpdatedTime time.Time `gae:"updated_time,noindex"`
	// GitSHA is the git SHA1 when the `LatestRevision` was fetched from Git
	// repository.
	...
	//
	// Technically, this should also be version controlled. However, the
	// historical value is useless for CV. CV just need the latest value and
	// act based on that. On the other hand, this option is very likely to be
	// deprecated in the future as we switch to an RPC-based start/stop of a
	// LUCI Project.
...
type ConfigGroup struct {
	...
	// Index is the index 0-based of this ConfigGroup in the Change Verifier
