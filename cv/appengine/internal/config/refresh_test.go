...
func TestSubmitRefreshTasks(t *testing.T) {
	...
	Convey("Submit Refresh Task", t, func() {
		...
		Convey("Disable project that doesn't have CV config", func() {
			ctx = cfgclient.Use(ctx, cfgmemory.New(map[config.Set]cfgmemory.Files{
		...
		Convey("Disable project that doesn't exist in LUCI Config", func() {
			ctx = cfgclient.Use(ctx, cfgmemory.New(map[config.Set]cfgmemory.Files{}))
			So(datastore.Put(ctx, &ProjectConfig{
				Project: "chromium",
				Enabled: true,
...
func TestUpdateProject(t *testing.T) {
	Convey("Update Project", t, func() {
		...
		Convey("Create New ProjectConfig", func() {
			...
			err := UpdateProjectIfNeeded(ctx, "chromium")
			So(err, ShouldBeNil)
			verify(ctx, 1)
			testClock.Add(10 * time.Minute)
