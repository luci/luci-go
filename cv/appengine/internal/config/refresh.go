...
func init() {
	tq.RegisterTaskClass(tq.TaskClass{
		...
		Handler: func(ctx context.Context, payload proto.Message) error {
			...
			if project := task.GetProject(); task.GetDisable() {
				if err := DisableProject(ctx, project); err != nil {
					errors.Log(ctx, err)
					...
					// Cron.
					return errors.Reason("failed to disable project %q: %s", project, err).Err()
				}
			} else {
				if err := UpdateProjectIfNeeded(ctx, project); err != nil {
					errors.Log(ctx, err)
...
// SubmitRefreshTasks submits tasks that updates config for LUCI projects
// or disable projects that do not have CV config in LUCI Config.
func SubmitRefreshTasks(ctx context.Context) error {
	projectsInLUCIConfig, err := cfgclient.ProjectsWithConfig(ctx, configFileName)
	if err != nil {
	...
	tasks := make([]*tq.Task, len(projectsInLUCIConfig)+len(curEnabledProjects))
	for i, p := range projectsInLUCIConfig {
		tasks[i] = &tq.Task{
	...
	projectSet := stringset.NewFromSlice(projectsInLUCIConfig...)
...
// UpdateProjectIfNeeded imports the latest CV Config for a given LUCI Project
// from LUCI Config if config in CV is outdated.
func UpdateProjectIfNeeded(ctx context.Context, project string) error {
