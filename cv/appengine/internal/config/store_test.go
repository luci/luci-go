...
func TestGetAllProjectIDs(t *testing.T) {
	...
	Convey("Get Project IDs", t, func() {
		...
		So(datastore.Put(ctx, &enabledPC, &disabledPC), ShouldErrLike, nil)
	...
	Convey("PutConfigGroups", t, func() {
		...
		Convey("New Configs", func() {
			...
			Convey("Skip if already exists", func() {
				cctx := datastore.AddRawFilters(ctx, func(ic context.Context, rds datastore.RawInterface) datastore.RawInterface {
					return readOnlyFilter{rds}
				})
				putConfigGroups(cctx, testCfg, "chromium", computeHash(testCfg))
			})
		})
	})
}
