...
// ReuseWindow specifies the conditions under which a tryjob can be reused.
//
// This configuration is only supported for projects that use monotonic
// commit positions in their commit footers (e.g., via the `git-numberer`
// plugin), since they are used to calculate commit distance.
type ReuseWindow struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Limit reuse by maximum commit distance.
	//
	// If the distance between the commit position of the base revision of a
	// previous build and the current branch tip exceeds this value, the build
	// cannot be reused.
	//
	// This requires the target branch to have monotonic commit positions.
	MaxCommitDistance uint32 `protobuf:"varint,1,opt,name=max_commit_distance,json=maxCommitDistance,proto3" json:"max_commit_distance,omitempty"`
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}

func (x *ReuseWindow) Reset() {
	*x = ReuseWindow{}
...
func (x *ReuseWindow) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReuseWindow) ProtoMessage() {}

func (x *ReuseWindow) ProtoReflect() protoreflect.Message {
	mi := &file_go_chromium_org_luci_cv_api_config_v2_config_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use ReuseWindow.ProtoReflect.Descriptor instead.
func (*ReuseWindow) Descriptor() ([]byte, []int) {
	return file_go_chromium_org_luci_cv_api_config_v2_config_proto_rawDescGZIP(), []int{7}
}

func (x *ReuseWindow) GetMaxCommitDistance() uint32 {
	if x != nil {
		return x.MaxCommitDistance
	}
	return 0
}
...
type Verifiers_Tryjob_Builder struct {
	...
	// Optional. The reuse window configuration for this builder.
	//
	// This allows reusing past successful builds based on "commit distance"
	// (number of commits between the build's base revision and the branch tip)
	// rather than just elapsed time. Only builds that completed successfully
	// are eligible for reuse.
	//
	// Note: This feature requires the project to have monotonically
	// increasing commit positions exposed in commit message footers (e.g.,
	// `Cr-Commit-Position: refs/heads/main@{#123456}`, typically generated
	// by the `git-numberer` Gerrit plugin). Validation and support are
	// restricted to allowlisted projects satisfying this condition (e.g.,
	// Chromium/Chrome).
	//
	// If unset, default 24h time-based reuse window is used.
	ReuseWindow   *ReuseWindow `protobuf:"bytes,19,opt,name=reuse_window,json=reuseWindow,proto3" json:"reuse_window,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Verifiers_Tryjob_Builder) GetReuseWindow() *ReuseWindow {
	if x != nil {
...
func (x *UserLimit_Tryjob) Reset() {
	*x = UserLimit_Tryjob{}
	mi := &file_go_chromium_org_luci_cv_api_config_v2_config_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}
...
const file_go_chromium_org_luci_cv_api_config_v2_config_proto_rawDesc = "" +
	"\n" +
	"2go.chromium.org/luci/cv/api/config/v2/config.proto\x12\tcv.config\x1a\x1egoogle/protobuf/duration.proto\x1a\x17validate/validate.proto\x1a/go.chromium.org/luci/common/proto/options.proto\x1a(go.chromium.org/luci/cv/api/v1/run.proto\"\xd4\x04\n" +
	"\x06Config\x122\n" +
	"\x13draining_start_time\x18\x01 \x01(\tB\x02\x18\x01R\x11drainingStartTime\x12(\n" +
	"\x0ecq_status_host\x18\x02 \x01(\tB\x02\x18\x01R\fcqStatusHost\x12?\n" +
	"\x0esubmit_options\x18\x03 \x01(\v2\x18.cv.config.SubmitOptionsR\rsubmitOptions\x12;\n" +
	"\rconfig_groups\x18\x04 \x03(\v2\x16.cv.config.ConfigGroupR\fconfigGroups\x12K\n" +
	"\x16project_scoped_account\x18\x05 \x01(\x0e2\x11.cv.config.ToggleB\x02\x18\x01R\x14projectScopedAccount\x12?\n" +
	"\x1chonor_gerrit_linked_accounts\x18\x06 \x01(\bR\x19honorGerritLinkedAccounts\x12V\n" +
	"\x14gerrit_listener_type\x18\a \x01(\x0e2$.cv.config.Config.GerritListenerTypeR\x12gerritListenerType\"\x87\x01\n" +
	"\x12GerritListenerType\x12$\n" +
	" GERRIT_LISTENER_TYPE_UNSPECIFIED\x10\x00\x12\x1f\n" +
	"\x1bGERRIT_LISTENER_TYPE_PUBSUB\x10\x01\x12*\n" +
	"\"GERRIT_LISTENER_TYPE_LEGACY_POLLER\x10\x02\x1a\x02\b\x01\"\xae\f\n" +
	"\vConfigGroup\x12\x12\n" +
	"\x04name\x18\x06 \x01(\tR\x04name\x125\n" +
	"\x06gerrit\x18\x01 \x03(\v2\x1d.cv.config.ConfigGroup.GerritR\x06gerrit\x126\n" +
	"\vcombine_cls\x18\x04 \x01(\v2\x15.cv.config.CombineCLsR\n" +
	"combineCls\x122\n" +
	"\tverifiers\x18\x02 \x01(\v2\x14.cv.config.VerifiersR\tverifiers\x12-\n" +
	"\bfallback\x18\x05 \x01(\x0e2\x11.cv.config.ToggleR\bfallback\x12:\n" +
	"\x10additional_modes\x18\a \x03(\v2\x0f.cv.config.ModeR\x0fadditionalModes\x125\n" +
	"\vuser_limits\x18\b \x03(\v2\x14.cv.config.UserLimitR\n" +
	"userLimits\x12B\n" +
	"\x12user_limit_default\x18\t \x01(\v2\x14.cv.config.UserLimitR\x10userLimitDefault\x12S\n" +
	"\fpost_actions\x18\n" +
	" \x03(\v2!.cv.config.ConfigGroup.PostActionB\r\xfaB\n" +
	"\x92\x01\a\"\x05\x8a\x01\x02\x10\x01R\vpostActions\x12e\n" +
	"\x12tryjob_experiments\x18\v \x03(\v2'.cv.config.ConfigGroup.TryjobExperimentB\r\xfaB\n" +
	"\x92\x01\a\"\x05\x8a\x01\x02\x10\x01R\x11tryjobExperiments\x1a\xc9\x01\n" +
	"\x06Gerrit\x12\x10\n" +
	"\x03url\x18\x01 \x01(\tR\x03url\x12A\n" +
	"\bprojects\x18\x02 \x03(\v2%.cv.config.ConfigGroup.Gerrit.ProjectR\bprojects\x1aj\n" +
	"\aProject\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x1d\n" +
	"\n" +
	"ref_regexp\x18\x02 \x03(\tR\trefRegexp\x12,\n" +
	"\x12ref_regexp_exclude\x18\x03 \x03(\tR\x10refRegexpExclude\x1a\xa1\x04\n" +
	"\n" +
	"PostAction\x12\x1b\n" +
	"\x04name\x18\x01 \x01(\tB\a\xfaB\x04r\x02\x10\x01R\x04name\x12f\n" +
	"\n" +
	"conditions\x18\x02 \x03(\v25.cv.config.ConfigGroup.PostAction.TriggeringConditionB\x0f\xfaB\f\x92\x01\t\b\x01\"\x05\x8a\x01\x02\x10\x01R\n" +
	"conditions\x12b\n" +
	"\x12vote_gerrit_labels\x18\x03 \x01(\v22.cv.config.ConfigGroup.PostAction.VoteGerritLabelsH\x00R\x10voteGerritLabels\x1ar\n" +
	"\x13TriggeringCondition\x12\x1b\n" +
	"\x04mode\x18\x01 \x01(\tB\a\xfaB\x04r\x02\x10\x01R\x04mode\x12>\n" +
	"\bstatuses\x18\x02 \x03(\x0e2\x11.cv.v1.Run.StatusB\x0f\xfaB\f\x92\x01\t\b\x01\"\x05\x82\x01\x02\x10\x01R\bstatuses\x1a\xa6\x01\n" +
	"\x10VoteGerritLabels\x12W\n" +
	"\x05votes\x18\x01 \x03(\v27.cv.config.ConfigGroup.PostAction.VoteGerritLabels.VoteB\b\xfaB\x05\x92\x01\x02\b\x01R\x05votes\x1a9\n" +
	"\x04Vote\x12\x1b\n" +
	"\x04name\x18\x01 \x01(\tB\a\xfaB\x04r\x02\x10\x01R\x04name\x12\x14\n" +
	"\x05value\x18\x02 \x01(\x05R\x05valueB\r\n" +
	"\x06action\x12\x03\xf8B\x01\x1a\xcf\x01\n" +
	"\x10TryjobExperiment\x12\x1b\n" +
	"\x04name\x18\x01 \x01(\tB\a\xfaB\x04r\x02\x10\x01R\x04name\x12O\n" +
	"\tcondition\x18\x02 \x01(\v21.cv.config.ConfigGroup.TryjobExperiment.ConditionR\tcondition\x1aM\n" +
	"\tCondition\x12@\n" +
	"\x15owner_group_allowlist\x18\x01 \x03(\tB\f\xfaB\t\x92\x01\x06\"\x04r\x02\x10\x01R\x13ownerGroupAllowlistJ\x04\b\x03\x10\x04\"h\n" +
	"\rSubmitOptions\x12\x1b\n" +
	"\tmax_burst\x18\x01 \x01(\x05R\bmaxBurst\x12:\n" +
	"\vburst_delay\x18\x02 \x01(\v2\x19.google.protobuf.DurationR\n" +
	"burstDelay\"\x82\x02\n" +
	"\x04Mode\x12S\n" +
	"\x04name\x18\x01 \x01(\tB?\xfaB<r:2\x13^[A-Z][A-Z_]{0,39}$Z\aDRY_RUNZ\bFULL_RUNZ\x10NEW_PATCHSET_RUNR\x04name\x12/\n" +
	"\x0ecq_label_value\x18\x02 \x01(\x05B\t\xfaB\x06\x1a\x040\x010\x02R\fcqLabelValue\x12@\n" +
	"\x10triggering_label\x18\x03 \x01(\tB\x15\xfaB\x12r\x10\x10\x01Z\fCommit-QueueR\x0ftriggeringLabel\x122\n" +
	"\x10triggering_value\x18\x04 \x01(\x05B\a\xfaB\x04\x1a\x02 \x00R\x0ftriggeringValue\"X\n" +
	"\n" +
	"CombineCLs\x12J\n" +
	"\x13stabilization_delay\x18\x01 \x01(\v2\x19.google.protobuf.DurationR\x12stabilizationDelay\"\xf0\x14\n" +
	"\tVerifiers\x12P\n" +
	"\x11gerrit_cq_ability\x18\x01 \x01(\v2$.cv.config.Verifiers.GerritCQAbilityR\x0fgerritCqAbility\x12@\n" +
	"\vtree_status\x18\x02 \x01(\v2\x1f.cv.config.Verifiers.TreeStatusR\n" +
	"treeStatus\x123\n" +
	"\x06tryjob\x18\x03 \x01(\v2\x1b.cv.config.Verifiers.TryjobR\x06tryjob\x129\n" +
	"\bcqlinter\x18\x04 \x01(\v2\x1d.cv.config.Verifiers.CQLinterR\bcqlinter\x12-\n" +
	"\x04fake\x18\x05 \x01(\v2\x19.cv.config.Verifiers.FakeR\x04fake\x1a\xf0\x03\n" +
	"\x0fGerritCQAbility\x12%\n" +
	"\x0ecommitter_list\x18\x01 \x03(\tR\rcommitterList\x12-\n" +
	"\x13dry_run_access_list\x18\x02 \x03(\tR\x10dryRunAccessList\x12>\n" +
	"\x1cnew_patchset_run_access_list\x18\x05 \x03(\tR\x18newPatchsetRunAccessList\x12<\n" +
	"\x1ballow_submit_with_open_deps\x18\x03 \x01(\bR\x17allowSubmitWithOpenDeps\x12j\n" +
	"\x1aallow_owner_if_submittable\x18\x04 \x01(\x0e2-.cv.config.Verifiers.GerritCQAbility.CQActionR\x17allowOwnerIfSubmittable\x121\n" +
	"\x15trust_dry_runner_deps\x18\x06 \x01(\bR\x12trustDryRunnerDeps\x12:\n" +
	"\x1aallow_non_owner_dry_runner\x18\a \x01(\bR\x16allowNonOwnerDryRunner\".\n" +
	"\bCQAction\x12\t\n" +
	"\x05UNSET\x10\x00\x12\v\n" +
	"\aDRY_RUN\x10\x01\x12\n" +
	"\n" +
	"\x06COMMIT\x10\x02\x1a?\n" +
	"\n" +
	"TreeStatus\x12\x1b\n" +
	"\ttree_name\x18\x02 \x01(\tR\btreeName\x12\x14\n" +
	"\x03url\x18\x01 \x01(\tB\x02\x18\x01R\x03url\x1a\x96\r\n" +
	"\x06Tryjob\x12?\n" +
	"\bbuilders\x18\x01 \x03(\v2#.cv.config.Verifiers.Tryjob.BuilderR\bbuilders\x12J\n" +
	"\fretry_config\x18\x02 \x01(\v2'.cv.config.Verifiers.Tryjob.RetryConfigR\vretryConfig\x12G\n" +
	"\x14cancel_stale_tryjobs\x18\x03 \x01(\x0e2\x11.cv.config.ToggleB\x02\x18\x01R\x12cancelStaleTryjobs\x1a\xb1\b\n" +
	"\aBuilder\x12\x12\n" +
	"\x04host\x18\x10 \x01(\tR\x04host\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12'\n" +
	"\x0fincludable_only\x18\t \x01(\bR\x0eincludableOnly\x12D\n" +
	"\x11result_visibility\x18\n" +
	" \x01(\x0e2\x17.cv.config.CommentLevelR\x10resultVisibility\x12#\n" +
	"\rdisable_reuse\x18\x02 \x01(\bR\fdisableReuse\x122\n" +
	"\x15disable_reuse_footers\x18\x12 \x03(\tR\x13disableReuseFooters\x124\n" +
	"\fcancel_stale\x18\v \x01(\x0e2\x11.cv.config.ToggleR\vcancelStale\x123\n" +
	"\x15experiment_percentage\x18\x04 \x01(\x02R\x14experimentPercentage\x12R\n" +
	"\requivalent_to\x18\x05 \x01(\v2-.cv.config.Verifiers.Tryjob.EquivalentBuilderR\fequivalentTo\x12]\n" +
	"\x10location_filters\x18\x0f \x03(\v22.cv.config.Verifiers.Tryjob.Builder.LocationFilterR\x0flocationFilters\x122\n" +
	"\x15owner_whitelist_group\x18\b \x03(\tR\x13ownerWhitelistGroup\x12%\n" +
	"\x0emode_allowlist\x18\x0e \x03(\tR\rmodeAllowlist\x12Q\n" +
	"\fskip_footers\x18\x11 \x03(\v2..cv.config.Verifiers.Tryjob.Builder.SkipFooterR\vskipFooters\x129\n" +
	"\freuse_window\x18\x13 \x01(\v2\x16.cv.config.ReuseWindowR\vreuseWindow\x1a\xd9\x01\n" +
	"\x0eLocationFilter\x12,\n" +
	"\x12gerrit_host_regexp\x18\x01 \x01(\tR\x10gerritHostRegexp\x122\n" +
	"\x15gerrit_project_regexp\x18\x02 \x01(\tR\x13gerritProjectRegexp\x12*\n" +
	"\x11gerrit_ref_regexp\x18\x05 \x01(\tR\x0fgerritRefRegexp\x12\x1f\n" +
	"\vpath_regexp\x18\x03 \x01(\tR\n" +
	"pathRegexp\x12\x18\n" +
	"\aexclude\x18\x04 \x01(\bR\aexclude\x1aA\n" +
	"\n" +
	"SkipFooter\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12!\n" +
	"\fvalue_regexp\x18\x02 \x01(\tR\vvalueRegexpJ\x04\b\x03\x10\x04J\x04\b\f\x10\rJ\x04\b\r\x10\x0e\x1a{\n" +
	"\x11EquivalentBuilder\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x1e\n" +
	"\n" +
	"percentage\x18\x02 \x01(\x02R\n" +
	"percentage\x122\n" +
	"\x15owner_whitelist_group\x18\x03 \x01(\tR\x13ownerWhitelistGroup\x1a'\n" +
	"\x11IncludableBuilder\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x1a\xdb\x01\n" +
	"\vRetryConfig\x12!\n" +
	"\fsingle_quota\x18\x01 \x01(\x05R\vsingleQuota\x12!\n" +
	"\fglobal_quota\x18\x02 \x01(\x05R\vglobalQuota\x12%\n" +
	"\x0efailure_weight\x18\x03 \x01(\x05R\rfailureWeight\x128\n" +
	"\x18transient_failure_weight\x18\x04 \x01(\x05R\x16transientFailureWeight\x12%\n" +
	"\x0etimeout_weight\x18\x05 \x01(\x05R\rtimeoutWeight\x1a\n" +
	"\n" +
	"\bCQLinter\x1aW\n" +
	"\x04Fake\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12%\n" +
	"\x0eeventual_state\x18\x02 \x01(\tR\reventualState\x12\x14\n" +
	"\x05delay\x18\x03 \x01(\x05R\x05delay\"\x99\x03\n" +
	"\tUserLimit\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x1e\n" +
	"\n" +
	"principals\x18\x02 \x03(\tR\n" +
	"principals\x12*\n" +
	"\x03run\x18\x05 \x01(\v2\x18.cv.config.UserLimit.RunR\x03run\x123\n" +
	"\x06tryjob\x18\x06 \x01(\v2\x1b.cv.config.UserLimit.TryjobR\x06tryjob\x1aH\n" +
	"\x05Limit\x12\x16\n" +
	"\x05value\x18\x01 \x01(\x03H\x00R\x05value\x12\x1e\n" +
	"\tunlimited\x18\x02 \x01(\bH\x00R\tunlimitedB\a\n" +
	"\x05limit\x1ah\n" +
	"\x03Run\x129\n" +
	"\n" +
	"max_active\x18\x01 \x01(\v2\x1a.cv.config.UserLimit.LimitR\tmaxActive\x12&\n" +
	"\x0freach_limit_msg\x18\x02 \x01(\tR\rreachLimitMsg\x1aC\n" +
	"\x06Tryjob\x129\n" +
	"\n" +
	"max_active\x18\x01 \x01(\v2\x1a.cv.config.UserLimit.LimitR\tmaxActive\"F\n" +
	"\vReuseWindow\x127\n" +
	"\x13max_commit_distance\x18\x01 \x01(\rB\a\xfaB\x04*\x02 \x00R\x11maxCommitDistance*]\n" +
	"\fCommentLevel\x12\x17\n" +
	"\x13COMMENT_LEVEL_UNSET\x10\x00\x12\x16\n" +
	"\x12COMMENT_LEVEL_FULL\x10\x01\x12\x1c\n" +
	"\x18COMMENT_LEVEL_RESTRICTED\x10\x02*$\n" +
	"\x06Toggle\x12\t\n" +
	"\x05UNSET\x10\x00\x12\a\n" +
	"\x03YES\x10\x01\x12\x06\n" +
	"\x02NO\x10\x02Bl\xa2\xfe#;\n" +
	"9https://config.luci.app/schemas/projects:commit-queue.cfgZ+go.chromium.org/luci/cv/api/config/v2;cfgpbb\x06proto3"
...
var file_go_chromium_org_luci_cv_api_config_v2_config_proto_msgTypes = make([]protoimpl.MessageInfo, 30)
var file_go_chromium_org_luci_cv_api_config_v2_config_proto_goTypes = []any{
	...
	(*ReuseWindow)(nil),                                  // 11: cv.config.ReuseWindow
	(*ConfigGroup_Gerrit)(nil),                           // 12: cv.config.ConfigGroup.Gerrit
	(*ConfigGroup_PostAction)(nil),                       // 13: cv.config.ConfigGroup.PostAction
	(*ConfigGroup_TryjobExperiment)(nil),                 // 14: cv.config.ConfigGroup.TryjobExperiment
	(*ConfigGroup_Gerrit_Project)(nil),                   // 15: cv.config.ConfigGroup.Gerrit.Project
	(*ConfigGroup_PostAction_TriggeringCondition)(nil),   // 16: cv.config.ConfigGroup.PostAction.TriggeringCondition
	(*ConfigGroup_PostAction_VoteGerritLabels)(nil),      // 17: cv.config.ConfigGroup.PostAction.VoteGerritLabels
	(*ConfigGroup_PostAction_VoteGerritLabels_Vote)(nil), // 18: cv.config.ConfigGroup.PostAction.VoteGerritLabels.Vote
	(*ConfigGroup_TryjobExperiment_Condition)(nil),       // 19: cv.config.ConfigGroup.TryjobExperiment.Condition
	(*Verifiers_GerritCQAbility)(nil),                    // 20: cv.config.Verifiers.GerritCQAbility
	(*Verifiers_TreeStatus)(nil),                         // 21: cv.config.Verifiers.TreeStatus
	(*Verifiers_Tryjob)(nil),                             // 22: cv.config.Verifiers.Tryjob
	(*Verifiers_CQLinter)(nil),                           // 23: cv.config.Verifiers.CQLinter
	(*Verifiers_Fake)(nil),                               // 24: cv.config.Verifiers.Fake
	(*Verifiers_Tryjob_Builder)(nil),                     // 25: cv.config.Verifiers.Tryjob.Builder
	(*Verifiers_Tryjob_EquivalentBuilder)(nil),           // 26: cv.config.Verifiers.Tryjob.EquivalentBuilder
	(*Verifiers_Tryjob_IncludableBuilder)(nil),           // 27: cv.config.Verifiers.Tryjob.IncludableBuilder
	(*Verifiers_Tryjob_RetryConfig)(nil),                 // 28: cv.config.Verifiers.Tryjob.RetryConfig
	(*Verifiers_Tryjob_Builder_LocationFilter)(nil),      // 29: cv.config.Verifiers.Tryjob.Builder.LocationFilter
	(*Verifiers_Tryjob_Builder_SkipFooter)(nil),          // 30: cv.config.Verifiers.Tryjob.Builder.SkipFooter
	(*UserLimit_Limit)(nil),                              // 31: cv.config.UserLimit.Limit
	(*UserLimit_Run)(nil),                                // 32: cv.config.UserLimit.Run
	(*UserLimit_Tryjob)(nil),                             // 33: cv.config.UserLimit.Tryjob
	(*durationpb.Duration)(nil),                          // 34: google.protobuf.Duration
	(v1.Run_Status)(0),                                   // 35: cv.v1.Run.Status
}
var file_go_chromium_org_luci_cv_api_config_v2_config_proto_depIdxs = []int32{
	...
	12, // 4: cv.config.ConfigGroup.gerrit:type_name -> cv.config.ConfigGroup.Gerrit
	...
	13, // 11: cv.config.ConfigGroup.post_actions:type_name -> cv.config.ConfigGroup.PostAction
	14, // 12: cv.config.ConfigGroup.tryjob_experiments:type_name -> cv.config.ConfigGroup.TryjobExperiment
	34, // 13: cv.config.SubmitOptions.burst_delay:type_name -> google.protobuf.Duration
	34, // 14: cv.config.CombineCLs.stabilization_delay:type_name -> google.protobuf.Duration
	20, // 15: cv.config.Verifiers.gerrit_cq_ability:type_name -> cv.config.Verifiers.GerritCQAbility
	21, // 16: cv.config.Verifiers.tree_status:type_name -> cv.config.Verifiers.TreeStatus
	22, // 17: cv.config.Verifiers.tryjob:type_name -> cv.config.Verifiers.Tryjob
	23, // 18: cv.config.Verifiers.cqlinter:type_name -> cv.config.Verifiers.CQLinter
	24, // 19: cv.config.Verifiers.fake:type_name -> cv.config.Verifiers.Fake
	32, // 20: cv.config.UserLimit.run:type_name -> cv.config.UserLimit.Run
	33, // 21: cv.config.UserLimit.tryjob:type_name -> cv.config.UserLimit.Tryjob
	15, // 22: cv.config.ConfigGroup.Gerrit.projects:type_name -> cv.config.ConfigGroup.Gerrit.Project
	16, // 23: cv.config.ConfigGroup.PostAction.conditions:type_name -> cv.config.ConfigGroup.PostAction.TriggeringCondition
	17, // 24: cv.config.ConfigGroup.PostAction.vote_gerrit_labels:type_name -> cv.config.ConfigGroup.PostAction.VoteGerritLabels
	19, // 25: cv.config.ConfigGroup.TryjobExperiment.condition:type_name -> cv.config.ConfigGroup.TryjobExperiment.Condition
	35, // 26: cv.config.ConfigGroup.PostAction.TriggeringCondition.statuses:type_name -> cv.v1.Run.Status
	18, // 27: cv.config.ConfigGroup.PostAction.VoteGerritLabels.votes:type_name -> cv.config.ConfigGroup.PostAction.VoteGerritLabels.Vote
	...
	25, // 29: cv.config.Verifiers.Tryjob.builders:type_name -> cv.config.Verifiers.Tryjob.Builder
	28, // 30: cv.config.Verifiers.Tryjob.retry_config:type_name -> cv.config.Verifiers.Tryjob.RetryConfig
	...
	26, // 34: cv.config.Verifiers.Tryjob.Builder.equivalent_to:type_name -> cv.config.Verifiers.Tryjob.EquivalentBuilder
	29, // 35: cv.config.Verifiers.Tryjob.Builder.location_filters:type_name -> cv.config.Verifiers.Tryjob.Builder.LocationFilter
	30, // 36: cv.config.Verifiers.Tryjob.Builder.skip_footers:type_name -> cv.config.Verifiers.Tryjob.Builder.SkipFooter
	11, // 37: cv.config.Verifiers.Tryjob.Builder.reuse_window:type_name -> cv.config.ReuseWindow
	31, // 38: cv.config.UserLimit.Run.max_active:type_name -> cv.config.UserLimit.Limit
	31, // 39: cv.config.UserLimit.Tryjob.max_active:type_name -> cv.config.UserLimit.Limit
...
func file_go_chromium_org_luci_cv_api_config_v2_config_proto_init() {
	...
	file_go_chromium_org_luci_cv_api_config_v2_config_proto_msgTypes[9].OneofWrappers = []any{
		(*ConfigGroup_PostAction_VoteGerritLabels_)(nil),
	}
	file_go_chromium_org_luci_cv_api_config_v2_config_proto_msgTypes[27].OneofWrappers = []any{
